...
| tab (UI)         | pestanya             | «Tabulador» is the _key_, not the UI tab                                                           |
...
- **`token` stays English.** The corpora say «testimoni» (KDE 5, GNOME 2, MS 4), but that is the
  ...
  _"If established practice keeps an English loanword or acronym, keep it rather than inventing a
  translation."_
...
- **`diff` stays English**, unlike `fork` and `worktree`, because Git itself does not settle it:
  ...
  Where the English says _non-diff lines_ rather than _diff_, a paraphrase is fine — «línies sense
...
- **Verb form depends on who is speaking.** This is the rule most often broken and it is not a
  matter of taste — see Softcatalà, _Formes verbals_:
  ...
  - **Computer → user** (dialogs, status text, questions, descriptions): imperative **2nd person
    plural** (_vós_). `Escriviu…`, `Trieu una carpeta`, `Voleu continuar?`
  - Never use the infinitive as a command. `Tancar` → `Tanca`; `Enviar` → `Envia`; `Desfer` → `Desfés`.
  - **A label that names a completed action is a noun, not a command.** Tool titles and activity
    labels are read as things, and they follow a count (`Used 2 …`), so an imperative breaks the
    grammar. `Write` → `Escriptura` (not «Escriu»); `Read` → `Lectura`; `Edit` → `Edició`;
    `Execute` → `Execució`. Most of the family is already nominal (`Llista`, `Pedaç`, `Tasca`).
...
- **`web` takes the gender of the elided noun** (TERMCAT): «un web» is _un lloc web_, «una web» is
  _una pàgina web_. It is not a free choice. Every article-bearing `web` string in this codebase
...
- Avoid mixing _tu_ and _vós_ within a surface once the speaker direction is fixed.
- Avoid «tests» for software _tests_; use «proves». The corpora are unanimous on this
