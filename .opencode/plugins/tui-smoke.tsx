...
import {
	...
	RGBA,
	VignetteEffect,
} from "../../packages/opencode/node_modules/@opentui/core";
import {
	type BindingConfig,
	createBindingLookup,
...
import {
	useBindings,
	useKeymapSelector,
} from "../../packages/opencode/node_modules/@opentui/keymap/src/solid/index.js";
import { useTerminalDimensions } from "../../packages/opencode/node_modules/@opentui/solid";
...
const cfg = (options: Record<string, unknown> | undefined) => {
	return {
		...
		keybinds: record(options?.keybinds)
			? (options.keybinds as SmokeBindings)
			: undefined,
	};
};
...
const Screen = (props: {
	...
	const screenActive = () =>
		props.api.route.current.name === props.route.screen;
	...
	useBindings(() => ({
		enabled: () =>
			...
			current(props.api, props.route).local > 0,
		commands: [
			{
		...
		bindings: props.keys.gather("smoke.local", [
			command.local_push,
			command.local_pop,
		]),
	}));
	...
	const shortcuts = useKeymapSelector((keymap: TuiKeymap) => {
		...
		return {
			screen_home:
				props.api.keys.formatBindings(bindings.get(command.screen_home)) ?? "",
			screen_up:
				props.api.keys.formatBindings(bindings.get(command.screen_up)) ?? "",
			screen_down:
				props.api.keys.formatBindings(bindings.get(command.screen_down)) ?? "",
			screen_modal:
				props.api.keys.formatBindings(bindings.get(command.screen_modal)) ?? "",
			screen_alert:
				props.api.keys.formatBindings(bindings.get(command.screen_alert)) ?? "",
			screen_confirm:
				props.api.keys.formatBindings(bindings.get(command.screen_confirm)) ??
				"",
			screen_prompt:
				props.api.keys.formatBindings(bindings.get(command.screen_prompt)) ??
				"",
			screen_select:
				props.api.keys.formatBindings(bindings.get(command.screen_select)) ??
				"",
			screen_local:
				props.api.keys.formatBindings(bindings.get(command.screen_local)) ?? "",
			screen_host:
				props.api.keys.formatBindings(bindings.get(command.screen_host)) ?? "",
			local_push:
				props.api.keys.formatBindings(bindings.get(command.local_push)) ?? "",
			local_pop:
				props.api.keys.formatBindings(bindings.get(command.local_pop)) ?? "",
		};
	});
		...
		>
			...
			>
				...
				>
					...
					{value.tab === 2 ? (
						<box flexDirection="column" gap={1}>
							<text fg={skin.muted}>
								{shortcuts().screen_modal} modal | {shortcuts().screen_alert}{" "}
								alert | {shortcuts().screen_confirm} confirm |{" "}
								{shortcuts().screen_prompt} prompt | {shortcuts().screen_select}{" "}
								select
							</text>
							<text fg={skin.muted}>
								{shortcuts().screen_local} local stack |{" "}
								{shortcuts().screen_host} host stack
							</text>
							<text fg={skin.muted}>
								local open: {shortcuts().local_push} push nested ·{" "}
								{shortcuts().local_pop} close
							</text>
							<text fg={skin.muted}>
			...
			>
				...
				>
					...
					<text fg={skin.muted}>
						{shortcuts().local_push} push nested · {shortcuts().local_pop}{" "}
						pop/close
					</text>
					<box flexDirection="row" gap={1}>
...
const Modal = (props: {
	...
	useBindings(() => ({
		...
		bindings: props.keys.gather("smoke.modal", [
			command.modal_accept,
			command.modal_close,
		]),
	}));
	const shortcuts = useKeymapSelector((keymap: TuiKeymap) => {
		...
		return {
			...
			modal_accept:
				props.api.keys.formatBindings(bindings.get(command.modal_accept)) ?? "",
			modal_close:
				props.api.keys.formatBindings(bindings.get(command.modal_close)) ?? "",
		};
	});
		...
		<box width="100%" height="100%" backgroundColor={skin.panel}>
			<Dialog onClose={() => props.api.route.navigate("home")}>
				...
				>
					...
					<text fg={skin.muted}>
						{shortcuts().modal_accept} opens screen · {shortcuts().modal_close}{" "}
						closes
					</text>
					<box flexDirection="row" gap={1}>
						<Btn
...
const home = (api: TuiPluginApi, input: Cfg) => ({
	slots: {
		...
		home_prompt_right(
			ctx: TuiSlotContext,
			value: TuiHostSlotMap["home_prompt_right"],
		) {
			const skin = look(ctx.theme.current);
		...
		smoke_prompt_right(
			ctx: TuiSlotContext,
			value: { workspace_id?: string; label?: string },
		) {
			const skin = look(ctx.theme.current);
			const id =
...
const block = (
	...
	slots: {
		sidebar_content(
			ctx: TuiSlotContext,
			value: TuiHostSlotMap["sidebar_content"],
		) {
			const skin = look(ctx.theme.current);
