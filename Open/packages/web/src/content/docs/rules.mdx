...
You can specify custom instruction files in your `opendeepseek.json` or the global `~/.config/opencode/opendeepseek.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
### Using opendeepseek.json

The recommended approach is to use the `instructions` field in `opendeepseek.json`:
...
:::tip
For monorepos or projects with shared standards, using `opendeepseek.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
