...
const { client } = await createOpenDeepSeek()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `opendeepseek.json`, but you can override or add configuration inline:
...
```typescript
import type { Session, Message, Part } from "@opendeepseek/sdk"
```
