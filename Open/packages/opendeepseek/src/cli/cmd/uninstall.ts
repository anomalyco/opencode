...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opendeepseek-ai",
      pnpm: "pnpm uninstall -g opendeepseek-ai",
      bun: "bun remove -g opendeepseek-ai",
      yarn: "yarn global remove opendeepseek-ai",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "opendeepseek-ai"],
      yarn: ["yarn", "global", "remove", "opendeepseek-ai"],
      brew: ["brew", "uninstall", "opendeepseek"],
    }
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opendeepseek")) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# opencode") || content.includes(".opendeepseek/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opendeepseek/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".opendeepseek/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".opendeepseek"))
    ) {
      continue
    }
