...
- Status: `todo`
- Goal: make the harness run the smallest upstream case end to end in dry-run mode.
- Depends on: `BMK-003`
...
## BMK-005 Load fullflow suite from upstream jobs

- Status: `todo`
- Goal: turn upstream `*.json` jobs into the `fullflow` suite.
- Depends on: `BMK-004`
- Deliverables: fullflow suite loader and suite manifest.
- Done when: the runner can execute or list the fullflow suite from upstream inventory.
...
- Depends on: `BMK-005`
- Deliverables: stage suite loaders and derived design-only manifests.
...
- Status: `todo`
- Goal: emit stable summary files for both humans and automation.
- Depends on: `BMK-006`
- Deliverables: `summary.json`, `summary.md`, per-case `result.json`.
- Done when: failed and successful runs both produce structured summaries.
- Benchmark gate: `fullflow smoke`
