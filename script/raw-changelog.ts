...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/opencode packages/tui packages/cli packages/sdk packages/plugin packages/core github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/tui/") || file.startsWith("packages/opencode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/opencode/") || file.startsWith("packages/core/")) areas.add("core")
      ...
      else if (file.startsWith("github/")) areas.add("extensions")
    }
