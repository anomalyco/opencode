...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "pencode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".pencode/glossary/zh-cn.md"
  if (locale === "zht") return ".pencode/glossary/zh-tw.md"
  return `.pencode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Pencode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Pencode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Pencode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Pencode runs for 'all' (default: 4)
      --model <provider/id>  Pencode model (default: pencode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Pencode
...
`)
  ...
  if (failed.length) console.error(`\nPencode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.PENCODE_DISABLE_PROJECT_CONFIG = "1"
  env.PENCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["pencode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.PENCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["pencode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.PENCODE_CONFIG
  delete env.PENCODE_CONFIG_DIR
  delete env.PENCODE_CONFIG_CONTENT
  delete env.PENCODE_PERMISSION
  delete env.PENCODE_AUTO_SHARE
  return env
}
