<#
.SYNOPSIS
    打包 LINGXI CODE 麒麟 OS V10 ARM64 离线部署包
.DESCRIPTION
...
$ArtifactDir = Join-Path $ProjectRoot "artifacts\kylin-arm64\release"
$ArtifactDirAttached = Join-Path $ProjectRoot "artifacts\kylin-arm64\attached"

if (-not $OutputDir) {
    $OutputDir = Join-Path $ProjectRoot "dist-offline-kylin-arm64"
}
