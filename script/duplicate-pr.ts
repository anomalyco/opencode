...
import { createPencode } from "@pencode-ai/sdk"
...
`)
  ...
  const pencode = await createPencode({ port: 0 })
  ...
  try {
    ...
    const session = await pencode.client.session.create()
    const result = await pencode.client.session
      .prompt({
