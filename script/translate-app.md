...
- Preserve technical terms and artifacts exactly: OpenCode, API names, identifiers, code, commands, flags, paths, URLs, versions, config keys, and placeholder tokens. Translate ordinary user-facing error labels and descriptions; do not retain English merely because the copy reports an error.
...
- Name the developer corpora and language authorities consulted in the final response so reviewers can verify terminology choices.
