#!/bin/bash
# ============================================================
# 灵犀代码 (LingxiCode) - 麒麟OS ARM64 一键编译脚本
# 运行环境: Win11 WSL2 Ubuntu
# 用法:
#   ./build-kylin-arm64.sh              # 增量编译（默认）
#   ./build-kylin-arm64.sh --full       # 全量编译（清理后重新开始）
#   ./build-kylin-arm64.sh --full --clean-only  # 仅清理，不编译
#   ./build-kylin-arm64.sh --skip-parsers       # 跳过解析器下载
#   ./build-kylin-arm64.sh --target=linux-arm64  # 仅编译指定目标
# ============================================================
...
BUILD_TARGET="opencode-linux-arm64"
...
HAS_ARM64_NATIVE=false
if ls node_modules/.bun/@parcel+watcher-linux-arm64-glibc* >/dev/null 2>&1; then
    HAS_ARM64_NATIVE=true
fi

if [ "$FULL_BUILD" = true ] || [ "$HAS_ARM64_NATIVE" = false ]; then
    info "  安装跨平台原生包..."
...
BINARY_PATH="dist/$BUILD_TARGET/bin/$BINARY_NAME"
    ...
    if echo "$BINARY_FILE" | grep -q "ARM aarch64"; then
        ok "  编译成功: ARM aarch64, $BINARY_SIZE"
    else
