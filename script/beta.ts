...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with leak-code...`)
  ...
  try {
    await $`leak-code run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  leak-code failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with leak-code")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with leak-code...")
