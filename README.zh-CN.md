...
nix run nixpkgs#crazycode           # 或用 github:anomalyco/crazycode 获取最新 dev 分支
```
...
CrazyCode 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/crazycode/releases) 或 [crazycode.ai/download](https://crazycode.ai/download) 下载。
...
1. `$CRAZYCODE_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.crazycode/bin` - 默认备用路径
...
CrazyCode 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://crazycode.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://crazycode.ai/docs)。
...
如果你在项目名中使用了 “crazycode”（如 “crazycode-dashboard” 或 “crazycode-mobile”），请在 README 里注明该项目不是 CrazyCode 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [CrazyCode Zen](https://crazycode.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。CrazyCode 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
...
**加入我们的社区** [Discord](https://discord.gg/crazycode) | [X.com](https://x.com/crazycode)
