...
TUI 会话视图中有一个垂直滚动条，默认隐藏。可通过命令面板（`Ctrl+P`）选择 **"Toggle Session Scrollbar"** 来显示。

## 问题描述

该滚动条的 thumb 无论内容多长，始终充满整个轨道高度。用户无法通过滚动条直观判断当前所在位置。
...
2. 通过命令面板（`Ctrl+P`）选择 **"Toggle Session Scrollbar"** 来显示滚动条。
