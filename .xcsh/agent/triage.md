...
**Only** add if it's specifically about the Desktop application or `xcsh web` view. **Do not** add for terminal, TUI, or general xcsh issues.
...
**Only** add if the issue mentions "zen" or "xcsh zen" or "xcsh black".

If the issue doesn't have "zen" or "xcsh black" in it then don't add zen label

#### core

Use for core server issues in `packages/xcsh/`, excluding `packages/xcsh/src/cli/cmd/tui/`.
...
TUI (`packages/xcsh/src/cli/cmd/tui/...`):
...
Core (`packages/xcsh/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for xcsh server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
