...
OpenCode opens every server with the classic MCP `initialize` handshake by default. Set `protocol` to talk to servers built on the 2026-07-28 revision:
...
Set a default under `mcp.protocol`; a server's `protocol` field overrides it. Probing a local `legacy` server adds a short-lived extra process and can wait up to the startup timeout when the server ignores unknown requests, so keep `auto` scoped to servers that need it.
