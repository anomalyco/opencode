...
<Callout type="note">OpenCode V2 recognizes `AGENTS.md` only. It does not use `CLAUDE.md` as a fallback.</Callout>
...
Use the `instructions` array to add files, glob patterns, or HTTP(S) URLs alongside discovered `AGENTS.md` files:
...
Relative patterns are resolved upward from the current workspace to the project root. Absolute paths and `~/` paths resolve directly, and remote instructions use a five-second request timeout. See [Config](/config) for configuration locations and precedence.
