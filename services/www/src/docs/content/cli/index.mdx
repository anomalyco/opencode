...
Start the prompt with `/command` to invoke a registered command:

```bash
opencode run "/review check the latest changes"
```

Unrecognized commands are sent as ordinary prompt text.
