...
Use `--command` to invoke a registered command. The message and any piped input become its arguments:

```bash
opencode run --command review "check the latest changes"
opencode run --command review
```

Unknown commands fail with an error. Without `--command`, the message is sent as ordinary prompt text, including any
leading `/name`.
