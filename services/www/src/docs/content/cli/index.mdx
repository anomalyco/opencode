...
Mention `@skill-id` in the prompt to attach a skill:

```bash
opencode run "@my-skill help with this task"
```

Skills are matched by ID, just like skill mentions in the interactive prompt. Multiple mentions are supported; repeated
mentions attach the skill once. Unrecognized skill mentions remain ordinary prompt text.
