...
| `session.tool_details`  | `show` or `hide`            | Shows or hides completed tool calls by default.                              |
...
When `session.tool_details` is `hide`, completed tool calls are omitted while running and failed calls remain visible. Use
**Hide tool details** in the command palette or bind `session.toggle.tool_details` to change it while a session is open.
