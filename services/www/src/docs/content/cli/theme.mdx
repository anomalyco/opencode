...
Terminals like Warp, Ghostty, iTerm2, and WezTerm can show blur or opacity behind OpenCode. The `system` theme only makes the main background transparent, so panels and dialogs can still look like solid boxes.

Press `Ctrl+P`, select **Open settings**, then set **Transparency** under Appearance:

| Value | Behavior |
| --- | --- |
| `auto` | Respect each theme as authored. This is the default. |
| `on` | Force main, raised, and dialog surfaces to transparent for any theme. |
| `off` | Force opaque surfaces, even for transparent custom themes. |

This maps to the `transparency` field inside `theme` in your CLI config:
