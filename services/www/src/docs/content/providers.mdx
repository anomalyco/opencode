...
Requests wait up to five minutes for response headers and up to five minutes between streamed response chunks. Set
`settings.headerTimeout` or `settings.chunkTimeout` to a number of milliseconds to change a limit, or to `false` to
disable it.
...
A request that exceeds either limit fails with a transport error and follows the normal retry policy.
