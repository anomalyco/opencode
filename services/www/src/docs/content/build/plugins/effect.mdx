...
Tools default to Code Mode. Set `codemode: false` for native model calls, using the effective name `acme_greeting`
rather than the Code Mode function path `tools.acme.greeting`:
...
The same [direct tool and permission rules](/build/plugins#tools) apply to Promise and Effect plugins.
