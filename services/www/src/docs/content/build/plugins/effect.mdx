...
`yield* ctx.tool.list()` reads the tools currently registered, after every transform, outside of a transform
callback. Each tool has an `id` containing its effective name.
