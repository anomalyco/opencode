...
- `compaction` runs for checkpoint summaries. Set `result` to record the compaction yourself and skip the model
  call; it takes the same fields as a completed compaction message, so a plugin can write its own summary or install a
  provider-native `replacement` transcript.
