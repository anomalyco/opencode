...
Update, interrupt, or wait for a session.
...
Answer `question` tool forms from an external system. A form appears when the agent asks a
question; replying settles it and unblocks the turn.

```ts
const pending = await ctx.session.form.list({ sessionID })
await ctx.session.form.reply({ sessionID, formID: pending[0].id, answer: { choice: "yes" } })
```
...
interface SessionContext {
  ...
  form: {
    list(input: SessionFormListInput, requestOptions?: RequestOptions): Promise<readonly FormInfo[]>
