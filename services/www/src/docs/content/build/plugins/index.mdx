...
- `compaction` runs for checkpoint summaries. Set `result` to supply the summary yourself and skip the model call.
...
every request must register for each kind; `event.kind` says which one is running.
...
interface SessionHooks {
  ...
  compaction: SessionContextHook & { result?: { summary: string; recent?: string } }
