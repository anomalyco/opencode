...
Every V2 theme declares `version: 2`, one complete `base` token tree, and at least one `light` or `dark` hue palette.
...
`$background.default` follow the dialog background:
...
`@dialog` accepts the same token groups as the base mode. An action's contextual `default` also becomes the fallback
