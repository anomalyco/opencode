...
OpenCode 內建三個主要 Agent，您可以使用 `Tab` 鍵在它們之間切換：
...
- **plan** - 唯讀模式，用於分析與代碼探索。
  ...
  - 執行 bash 指令前會先詢問權限。
  - 適合探索不熟悉的代碼庫或規劃變更。
- **chat** - 隔離模式，用於原型設計與專注討論。
  - 在臨時目錄中運作。
  - 僅限上下文操作，預設不追蹤舊檔案。

此外，還包含一個 **general** 子 Agent，用於複雜的搜尋和多步任務。
這在內部使用，也可以在訊息中使用 `@general` 呼叫。

了解更多關於 [Agent](https://opencode.ai/docs/agents) 的資訊。
