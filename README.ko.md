...
OpenCode 에는 내장 에이전트 3개가 있으며 `Tab` 키로 전환할 수 있습니다.

- **build** - 기본값, 개발 작업을 위한 전체 권한 에이전트
- **ask** - 질문하고, 코드를 이해하고, 아이디어를 브레인스토밍/평가하기 위한 읽기 전용 에이전트
  - 파일을 수정할 수 없음
  - 낯선 코드베이스를 탐색하고 작동 방식을 이해할 때 적합
- **plan** - 구현 계획을 생성하기 위한 읽기 전용 에이전트
  - 기본적으로 파일 편집을 거부
  - bash 명령 실행 전에 권한을 요청
  - Build 에이전트가 실행할 상세한 구현 계획을 생성
