...
- Ambient project instruction discovery honors `OPENCODE_DISABLE_PROJECT_CONFIG` and `OPENCODE_DISABLE_PROJECT_INSTRUCTIONS`; global instructions remain eligible. `OPENCODE_DISABLE_PROJECT_INSTRUCTIONS` suppresses only repository-controlled project `AGENTS.md`/`CLAUDE.md` discovery (plus the nearby-instructions-on-read walk) and is exposed as the global `--no-project-instructions` CLI switch; trusted global user instructions remain eligible.
