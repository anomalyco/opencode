...
:::caution
Modes are now configured through the `agent` option in the chalice config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in chalice allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the chalice config.
...
- Global: `~/.config/chalice-code/mode/`
- Project: `.chalicecode/mode/`

```markdown title="~/.config/chalice-code/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
If no temperature is specified, chalice uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global chalice config and the project specific config.
...
Create mode files in `.chalicecode/mode/` for project-specific modes or `~/.config/chalice-code/mode/` for global modes:

```markdown title=".chalicecode/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/chalice-code/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
