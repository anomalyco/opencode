...
:::caution
Modes are now configured through the `agent` option in the cyberstrike config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in cyberstrike allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the cyberstrike config.
...
cyberstrike comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.cyberstrike/plans/*.md` to detail the plan itself
...
Configure modes in your `cyberstrike.json` config file:
...
both the global cyberstrike config and the project specific config.
...
Create mode files in `.cyberstrike/modes/` for project-specific modes or `~/.config/cyberstrike/modes/` for global modes:
