...
:::caution
Modes are now configured through the `agent` option in the crazycode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in crazycode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the crazycode config.
...
crazycode comes with two built-in modes.
...
Configure modes in your `crazycode.json` config file:
...
- Global: `~/.config/crazycode/mode/`
- Project: `.crazycode/mode/`

```markdown title="~/.config/crazycode/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global crazycode config and the project specific config.
...
```json title="crazycode.json" {4-14}
{
  "$schema": "https://crazycode.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.crazycode/mode/` for project-specific modes or `~/.config/crazycode/mode/` for global modes:

```markdown title=".crazycode/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/crazycode/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
