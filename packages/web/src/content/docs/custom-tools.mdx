---
title: Custom Tools
description: Create tools the LLM can call in leak-code.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside leak-code's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.leak-code/tools/` directory of your project.
- Or globally, by placing them in `~/.config/leak-code/tools/`.
...
```ts title=".leak-code/tools/database.ts" {1}
...
```ts title=".leak-code/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".leak-code/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".leak-code/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".leak-code/tools/add.py"
import sys
...
```ts title=".leak-code/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".leak-code/tools/add.py")
