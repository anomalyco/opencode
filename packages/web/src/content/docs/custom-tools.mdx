...
:::note
Use the plural `tools/` directory in examples above. For backwards compatibility, the loader also accepts the singular `tool/` directory (for example, `.opencode/tool/` or `~/.config/opencode/tool/`).
:::
