---
title: Custom Tools
description: Create tools the LLM can call in crazycode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside crazycode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.crazycode/tool/` directory of your project.
- Or globally, by placing them in `~/.config/crazycode/tool/`.
...
```ts title=".crazycode/tool/database.ts" {1}
...
```ts title=".crazycode/tool/math.ts"
import { tool } from "@crazycode-ai/plugin"
...
```ts title=".crazycode/tool/project.ts" {8}
import { tool } from "@crazycode-ai/plugin"
...
```python title=".crazycode/tool/add.py"
import sys
...
```ts title=".crazycode/tool/python-add.ts" {10}
import { tool } from "@crazycode-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .crazycode/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
