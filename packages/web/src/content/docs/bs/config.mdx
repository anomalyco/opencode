---
title: Konfiguracija
description: Korištenje Pencode JSON konfiguracije.
---

Možete konfigurirati Pencode koristeći JSON konfiguracijski fajl.

---

## Format

Pencode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/pencode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/pencode/pencode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`PENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`pencode.json` u projektu) - postavke specifične za projekat
5. **`.pencode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`PENCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.pencode` i `~/.config/pencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/pencode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Pencode konfiguraciju u `~/.config/pencode/pencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/pencode/tui.json`.
...
Dodajte `pencode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Pencode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `PENCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `PENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.pencode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.pencode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`pencode.ai/config.json`**](https://pencode.ai/config.json).

TUI konfiguracija koristi [**`pencode.ai/tui.json`**](https://pencode.ai/tui.json).
...
Koristite `PENCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `pencode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `pencode serve` i `pencode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Pencode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `pencode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Pencode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Pencode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/pencode/agents/` ili `.pencode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Pencode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`pencode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/pencode/commands/` ili `.pencode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Pencode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Pencode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Pencode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.pencode/plugins/` ili `~/.config/pencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Pencode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
