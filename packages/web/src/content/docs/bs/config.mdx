...
- `diff_style` - Kontrola prikaza razlike. `"auto"` se prilagođava širini terminala, `"stacked"` uvijek prikazuje jednu kolonu.
  [Ovdje saznajte više o korištenju TUI](./tui).
...
- `cors` - Dodatni izvori koji omogućavaju CORS kada koristite HTTP server iz klijenta baziranog na pretraživaču. Vrijednosti moraju biti punog porijekla (šema + host + opcijski port), npr. `https://app.example.com`.
  [Saznajte više o serveru](./server) ovdje.
...
[Saznajte više o alatima](./tools) ovdje.
...
- `setCacheKey` - Osigurajte da je ključ keš memorije uvijek postavljen za određenog provajdera.
  Također možete konfigurirati [lokalni modeli](./models#local). [Saznajte više](./models).
...
- `endpoint` - URL prilagođene krajnje tačke za VPC krajnje tačke. Ovo je pseudonim za generičku opciju `baseURL` koristeći terminologiju specifičnu za AWS. Ako su oba navedena, `endpoint` ima prednost.
  :::napomena
  Tokeni nosioca (`AWS_BEARER_TOKEN_BEDROCK` ili `/connect`) imaju prednost nad autentifikacijom zasnovanom na profilu. Pogledajte [prednost autentifikacije](./providers#authentication-precedence) za detalje.
  :::
  [Saznajte više o konfiguraciji Amazon Bedrock](./providers#amazon-bedrock).
...
Također možete definirati agente koristeći mardown datoteke u `~/.config/opencode/agents/` ili `.opencode/agents/`. [Saznajte više ovdje](./agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](./agents) koji ste definirali. Ako navedeni agent ne postoji ili je subagent, OpenCode će se vratiti na `"build"` s upozorenjem.
...
Možete konfigurirati funkciju [share](./share) putem opcije `share`.
...
Također možete definirati komande koristeći mardown fajlove u `~/.config/opencode/commands/` ili `.opencode/commands/`. [Saznajte više ovdje](./commands).
...
[Saznajte više o formatterima](./formatters) ovdje.
...
[Ovdje saznajte više o ](./permissions) dozvolama.
...
[Saznajte više ovdje](./mcp-servers).

---

### Extras

[Plugins](./plugins) proširuju OpenCode sa prilagođenim alatima, kukicama i integracijama.
