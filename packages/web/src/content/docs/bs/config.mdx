---
title: Konfiguracija
description: Korištenje Octopus JSON konfiguracije.
---

Možete konfigurirati Octopus koristeći JSON konfiguracijski fajl.

---

## Format

Octopus podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.

```jsonc title="octopus.jsonc"
{
...
2. **Globalna konfiguracija** (`~/.config/octopus/octopus.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OCTOPUS_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`octopus.json` u projektu) - postavke specifične za projekat
5. **`.opencode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OCTOPUS_CONFIG_CONTENT` env var) - runtime preinačenja
...
Postavite svoju globalnu Octopus konfiguraciju u `~/.config/octopus/octopus.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/octopus/tui.json`.
...
Dodajte `octopus.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Octopus pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OCTOPUS_CONFIG`.

```bash
export OCTOPUS_CONFIG=/path/to/my/custom-config.json
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OCTOPUS_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.

```bash
export OCTOPUS_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```
...
Koristite `OCTOPUS_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `octopus.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Octopus server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Octopus konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Octopus pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/octopus/agents/` ili `.octopus/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Octopus će se vratiti na `"build"` s upozorenjem.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/octopus/commands/` ili `.octopus/commands/`. [Saznajte više ovdje](/docs/commands).
...
Octopus će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Octopus **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Octopus sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.octopus/plugins/` ili `~/.config/octopus/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Octopus da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
