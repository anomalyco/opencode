...
3. **Prilagođena konfiguracija** (`IMECODE_CONFIG` env var) - prilagođena preinačenja
...
6. **Inline konfiguracija** (`IMECODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `IMECODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `IMECODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Koristite `IMECODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.
