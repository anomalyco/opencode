...
2. **Globalna konfiguracija** (`~/.config/leak-code/leak-code.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LEAKCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`leak-code.json` u projektu) - postavke specifične za projekat
5. **`.leak-code` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LEAKCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.leak-code` i `~/.config/leak-code` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/leak-code/leak-code.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/leak-code/tui.json`.
...
Dodajte `leak-code.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LEAKCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LEAKCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.leak-code` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.leak-code`, tako da **može nadjačati** njihove postavke.
...
Koristite `LEAKCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `leak-code.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `leak-code serve` i `leak-code web` putem opcije `server`.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/leak-code/agents/` ili `.leak-code/agents/`. [Saznajte više ovdje](/docs/agents).
...
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`leak-code run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/leak-code/commands/` ili `.leak-code/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.leak-code/plugins/` ili `~/.config/leak-code/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
