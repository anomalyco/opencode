...
`OPENCODE_CONFIG_DIRS` može navesti više prilagođenih konfiguracijskih direktorija koristeći isti separator kao sistemski `PATH` (`:` na Linux/macOS, `;` na Windows). Direktoriji se učitavaju slijeva nadesno, pa kasniji direktoriji nadjačavaju ranije. Ako je postavljen i `OPENCODE_CONFIG_DIR`, dodaje se na kraj i postaje prilagođeni direktorij najvišeg prioriteta i globalni cilj za pisanje.
