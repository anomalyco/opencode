...
:::caution
Kada koristite `--hostname 0.0.0.0`, postavite `LEAKCODE_SERVER_PASSWORD` da zastitite server.
...
1. **Pokrenite `leak-code web` u WSL terminalu** umjesto u PowerShell-u:
...
Pokretanje `leak-code web` iz WSL-a osigurava ispravan pristup fajl sistemu i terminalsku integraciju, a i dalje je dostupno iz Windows browsera.
...
- Vase OpenCode konfiguracije i sesije cuvaju se u WSL okruzenju na `~/.local/share/leak-code/`
