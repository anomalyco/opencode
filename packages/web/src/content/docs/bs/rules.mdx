...
Mozete imati i globalna pravila u `~/.config/leak-code/AGENTS.md`. Ona se primjenjuju u svim opencode sesijama.
...
- **Globalna pravila**: `~/.claude/CLAUDE.md` (koristi se ako ne postoji `~/.config/leak-code/AGENTS.md`)
...
2. **Globalna datoteka** na `~/.config/leak-code/AGENTS.md`
3. **Claude Code datoteka** na `~/.claude/CLAUDE.md` (osim ako je iskljucena)

Prva pronadena datoteka pobjeduje u svakoj kategoriji. Na primjer, ako imate i `AGENTS.md` i `CLAUDE.md`, koristi se samo `AGENTS.md`. Isto tako, `~/.config/leak-code/AGENTS.md` ima prednost nad `~/.claude/CLAUDE.md`.

---

## Prilagođena uputstva

Mozete navesti prilagodene datoteke uputstava u `leak-code.json` ili globalnom `~/.config/leak-code/leak-code.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje leak-code.json

Preporuceni pristup je da koristite `instructions` polje u `leak-code.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `leak-code.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
