...
Prilagođene komande su dodatak ugrađenim komandama kao što su `/init`, `/undo`, `/redo`, `/share`, `/help`. [Saznajte više](./tui#commands).
...
### JSON
Koristite opciju `command` u svom OpenCode [config](./config):
...
### Izlaz ljuske
Koristite _!`command`_ da ubacite [bash command](./tui#bash-commands) izlaz u svoj prompt.
...
---
### Agent
Koristite `agent` konfiguraciju da opciono odredite koji [agent](./agents) treba da izvrši ovu naredbu.
Ako je ovo [subagent](./agents/#subagents) naredba će po defaultu pokrenuti pozivanje subagenta.
...
### Subdatatak
Koristite `subtask` boolean da prisilite naredbu da pokrene [subagent](./agents/#subagents) pozivanje.
Ovo je korisno ako želite da naredba ne zagađuje vaš primarni kontekst i da će **primorati** agenta da djeluje kao subagent,
čak i ako je `mode` postavljeno na `primary` u konfiguraciji [agent](./agents).
...
## Embedded
opencode uključuje nekoliko ugrađenih naredbi kao što su `/init`, `/undo`, `/redo`, `/share`, `/help`; [saznaj više](./tui#commands).
:::napomena
