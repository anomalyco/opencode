...
Postoje dvije vrste agenata u CyberStrike: primarni agenti i podagenti.
...
CyberStrike dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
CyberStrike dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

CyberStrike dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `cyberstrike.json`:
...
- Globalno: `~/.config/cyberstrike/agents/`
- Po projektu: `.cyberstrike/agents/`
...
Ako temperatura nije navedena, CyberStrike koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu CyberStrike konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj CyberStrike konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [CyberStrike Zen](/docs/zen), koristili biste `cyberstrike/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `cyberstrike models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/CyberStrikeus/cyberstrike).
:::
