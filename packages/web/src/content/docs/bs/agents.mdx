...
Postoje dvije vrste agenata u Octopus: primarni agenti i podagenti.
...
Octopus dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Octopus dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Octopus dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Octopus-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `octopus.json`:
...
- Globalno: `~/.config/octopus/agents/`
- Po projektu: `.octopus/agents/`

```markdown title="~/.config/octopus/agents/review.md"
---
...
Ako temperatura nije navedena, Octopus koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Octopus konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Octopus konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Octopus Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="octopus.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="octopus.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/octopus).
:::
...
```markdown title="~/.config/octopus/agents/docs-writer.md"
---
...
```markdown title="~/.config/octopus/agents/security-auditor.md"
---
