...
:::tip
Ako ne navedete model, primarni agenti koriste [model globalno konfiguriran](./config#models) dok će podagenti koristiti model primarnog agenta koji je pozvao subagenta.
:::
```json title="opencode.json"
{
"agent": {
...
ID modela u vašoj OpenCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenCode Zen](./zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
[Saznajte više o dozvolama](./permissions).
