...
Postoje dvije vrste agenata u xcsh: primarni agenti i podagenti.
...
xcsh dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
xcsh dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

xcsh dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `xcsh.json`:
...
- Globalno: `~/.config/xcsh/agents/`
- Po projektu: `.xcsh/agents/`
...
Ako temperatura nije navedena, xcsh koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu xcsh konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj xcsh konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [xcsh Zen](/docs/zen), koristili biste `xcsh/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `xcsh models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/f5xc-salesdemos/xcsh).
:::
