...
:::tip[Bez automatskog otvaranja]
Da biste pokrenuli web server bez automatskog otvaranja pregledača, koristite umjesto toga `opencode serve`:

```bash
opencode serve --hostname 0.0.0.0 --port 4096
```

Zatim ručno pristupite `http://localhost:4096` (ili vašoj mrežnoj adresi) u pregledaču. Kada se vezujete za `0.0.0.0`, postavite `OPENCODE_SERVER_PASSWORD` da zaštitite pristup.
:::
