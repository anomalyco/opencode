...
:::caution
Ako `LEAKCODE_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::

:::tip[Windows korisnici]
Za najbolje iskustvo, pokrenite `leak-code web` iz [WSL](/docs/windows-wsl) umjesto PowerShell-a. Ovo osigurava pravilan pristup sistemu datoteka i integraciju terminala.
:::
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `LEAKCODE_SERVER_PASSWORD`:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Korisničko ime podrazumevano je `opencode`, ali se može promeniti sa `LEAKCODE_SERVER_USERNAME`.
...
Također možete konfigurirati postavke servera u svom `leak-code.json` konfiguracijskom fajlu:
