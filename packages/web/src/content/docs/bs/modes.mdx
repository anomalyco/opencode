...
ove ili konfigurirajte svoje putem xcsh config.
...
xcsh dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.xcsh/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `xcsh.json` konfiguracijskom fajlu:
...
Ako temperatura nije navedena, xcsh koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.xcsh/modes/` za specifične načine rada ili `~/.config/xcsh/modes/` za globalne načine:
