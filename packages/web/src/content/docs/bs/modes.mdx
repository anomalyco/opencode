...
ove ili konfigurirajte svoje putem cyberstrike config.
...
cyberstrike dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.cyberstrike/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `cyberstrike.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/cyberstrike/modes/`
- Projekat: `.cyberstrike/modes/`
...
Ako temperatura nije navedena, cyberstrike koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.cyberstrike/modes/` za specifične načine rada ili `~/.config/cyberstrike/modes/` za globalne načine:
