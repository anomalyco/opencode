...
xcsh Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
xcsh Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u xcsh-u. Pretplatite se na xcsh Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili xcsh.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa xcsh-om.
:::
...
xcsh Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

xcsh Go radi kao bilo koji drugi provajder u xcsh-u.

1. Prijavite se na **<a href={console}>xcsh Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `xcsh Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na xcsh Go.
:::
...
xcsh Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj xcsh konfiguraciji
koristi format `xcsh-go/<model-id>`. Na primjer, za Kimi K2.5, koristili biste
`xcsh-go/kimi-k2.5` u svojoj konfiguraciji.
...
Napravili smo xcsh Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz xcsh koristite i bilo kojeg drugog provajdera.
