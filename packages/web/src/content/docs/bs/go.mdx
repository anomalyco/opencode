...
Pencode Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Pencode-u. Pretplatite se na Pencode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Pencode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Pencode-om.
:::
...
Pencode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

Pencode Go radi kao bilo koji drugi provajder u Pencode-u.

1. Prijavite se na **<a href={console}>Pencode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Pencode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Pencode Go.
:::
...
Pencode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Pencode konfiguraciji
koristi format `pencode-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`pencode-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Pencode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Pencode koristite i bilo kojeg drugog provajdera.
