...
Možete dodati vanjske alate u Pencode koristeći _Model Context Protocol_, ili MCP. Pencode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Pencode Config](https://pencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/pencode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Pencode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Pencode će:
...
Ako server zahtijeva autentifikaciju, Pencode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `pencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Pencode sigurno čuva tokene u `~/.local/share/pencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Pencode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Pencode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Pencode sa vašim Sentry nalogom.
