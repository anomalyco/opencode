...
Možete dodati vanjske alate u Octopus koristeći _Model Context Protocol_, ili MCP. Octopus podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Octopus Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Octopus automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Octopus će:
...
Ako server zahtijeva autentifikaciju, Octopus će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Octopus sigurno čuva tokene u `~/.local/share/octopus/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Octopus, zajedno s ugrađenim alatima. Možete njima upravljati kroz Octopus konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Octopus sa vašim Sentry nalogom.
