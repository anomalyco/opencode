...
Možete dodati vanjske alate u CyberStrike koristeći _Model Context Protocol_, ili MCP. CyberStrike podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CyberStrike Config](https://cyberstrike.io/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/cyberstrike`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
CyberStrike automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CyberStrike će:
...
Ako server zahtijeva autentifikaciju, CyberStrike će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `cyberstrike mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CyberStrike sigurno čuva tokene u `~/.local/share/cyberstrike/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CyberStrike, zajedno s ugrađenim alatima. Možete njima upravljati kroz CyberStrike konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CyberStrike sa vašim Sentry nalogom.
