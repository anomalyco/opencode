...
Možete dodati vanjske alate u xcsh koristeći _Model Context Protocol_, ili MCP. xcsh podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/xcsh`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
xcsh automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, xcsh će:
...
Ako server zahtijeva autentifikaciju, xcsh će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `xcsh mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, xcsh sigurno čuva tokene u `~/.local/share/xcsh/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u xcsh, zajedno s ugrađenim alatima. Možete njima upravljati kroz xcsh konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete xcsh sa vašim Sentry nalogom.
