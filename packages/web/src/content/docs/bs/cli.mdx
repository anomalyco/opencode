...
OpenCode CLI po defaultu pokreće [TUI](./tui) kada se pokrene bez ikakvih argumenata.
...
Ovo postavlja neophodni tok rada GitHub Actions i vodi vas kroz proces konfiguracije. [Saznajte više](./github).
...
Ovo je korisno za pronalaženje tačnog naziva modela za korištenje u [vašoj konfiguraciji](./config/).
...
### poslužiti
Pokrenite OpenCode server bez glave za pristup API-ju. Pogledajte [server docs](./server) za kompletan HTTP interfejs.
