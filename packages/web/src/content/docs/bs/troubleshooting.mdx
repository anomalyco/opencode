...
Da biste otklonili probleme s CyberStrike, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\cyberstrike\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `cyberstrike --log-level DEBUG`.

---

## Pohrana

CyberStrike pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/cyberstrike/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\cyberstrike`
...
CyberStrike Desktop pokreće lokalni CyberStrike server (`cyberstrike-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CyberStrike` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/cyberstrike/cyberstrike.jsonc` (ili `~/.config/cyberstrike/cyberstrike.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/cyberstrike/cyberstrike.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\cyberstrike\cyberstrike.jsonc`
...
CyberStrike također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/cyberstrike/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\cyberstrike\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CyberStrike mogao ponovo izgraditi.

1. Potpuno zatvorite CyberStrike Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/cyberstrike`
- **Linux**: obrišite `~/.cache/cyberstrike` (ili pokrenite `rm -rf ~/.cache/cyberstrike`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\cyberstrike`

3. Ponovo pokrenite CyberStrike Desktop.

---

### Rješavanje problema sa vezom na serveru

CyberStrike Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `cyberstrike.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CYBERSTRIKE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CYBERSTRIKE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, CyberStrike Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CyberStrike karakteristikama.

---

### Obavještenja se ne prikazuju

CyberStrike Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CyberStrike u postavkama vašeg OS-a, i
...
1. Zatvorite CyberStrike Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CyberStrike Desktop aplikacije):

- `cyberstrike.settings.dat` (URL zadanog servera za desktop)
- `cyberstrike.global.dat` i `cyberstrike.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s CyberStrike:
...
Da saznate kojim modelima imate pristup, pokrenite `cyberstrike models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\cyberstrike`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CyberStrike dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\cyberstrike`

2. Ponovo pokrenite CyberStrike da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CyberStrike da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CyberStrike će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
