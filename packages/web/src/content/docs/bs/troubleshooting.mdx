...
Da biste otklonili probleme s xcsh, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\xcsh\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `xcsh --log-level DEBUG`.

---

## Pohrana

xcsh pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/xcsh/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\xcsh`
...
xcsh Desktop pokreće lokalni xcsh server (`xcsh-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `xcsh` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/xcsh/xcsh.jsonc` (ili `~/.config/xcsh/xcsh.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/xcsh/xcsh.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\xcsh\xcsh.jsonc`
...
xcsh također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/xcsh/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\xcsh\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga xcsh mogao ponovo izgraditi.

1. Potpuno zatvorite xcsh Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/xcsh`
- **Linux**: obrišite `~/.cache/xcsh` (ili pokrenite `rm -rf ~/.cache/xcsh`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\xcsh`

3. Ponovo pokrenite xcsh Desktop.

---

### Rješavanje problema sa vezom na serveru

xcsh Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `xcsh.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, xcsh Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa xcsh karakteristikama.

---

### Obavještenja se ne prikazuju

xcsh Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za xcsh u postavkama vašeg OS-a, i
...
1. Zatvorite xcsh Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka xcsh Desktop aplikacije):

- `xcsh.settings.dat` (URL zadanog servera za desktop)
- `xcsh.global.dat` i `xcsh.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `xcsh upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `xcsh models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\xcsh`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. xcsh dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\xcsh`

2. Ponovo pokrenite xcsh da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti xcsh da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
xcsh će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
