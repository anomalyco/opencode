...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (ili `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Ako vaš `leak-code.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LEAKCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LEAKCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
3. Uvjerite se da imate najnoviju verziju sa `leak-code upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `leak-code models`
