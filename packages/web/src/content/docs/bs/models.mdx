...
Saznajte više o [providers](./providers).
---
...
Ovdje je puni ID `provider_id/model_id`. Na primjer, ako koristite [OpenCode Zen](./zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
Ako ste konfigurirali [prilagođenog provajdera](./providers#custom), `provider_id` je ključ iz `provider` dijela vaše konfiguracije, a `model_id` je ključ iz `provider.models`.
---
...
Također možete konfigurirati ove opcije za sve agente koje koristite. Konfiguracija agenta poništava sve globalne opcije ovdje. [Saznajte više](./agents/#additional).
...
### Varijante ciklusa
Koristite vezu `variant_cycle` za brzo prebacivanje između varijanti. [Saznajte više](./keybinds).
---
