...
U Pencode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Pencode koristi nasu `pencode` temu.
...
Pencode dolazi sa vise ugradenih tema.
...
- Zele da Pencode odgovara izgledu njihovog terminala
...
Pencode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/pencode/themes/*.json` ili `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.pencode/themes/*.json`
4. **Trenutni radni direktorij** - `./.pencode/themes/*.json`
