...
U CyberStrike mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CyberStrike koristi nasu `cyberstrike` temu.
...
CyberStrike dolazi sa vise ugradenih tema.
...
- Zele da CyberStrike odgovara izgledu njihovog terminala
...
CyberStrike podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/cyberstrike/themes/*.json` ili `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.cyberstrike/themes/*.json`
4. **Trenutni radni direktorij** - `./.cyberstrike/themes/*.json`
