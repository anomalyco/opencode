...
U xcsh mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, xcsh koristi nasu `xcsh` temu.
...
xcsh dolazi sa vise ugradenih tema.
...
- Zele da xcsh odgovara izgledu njihovog terminala
...
xcsh podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/xcsh/themes/*.json` ili `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.xcsh/themes/*.json`
4. **Trenutni radni direktorij** - `./.xcsh/themes/*.json`
