...
U Octopus mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Octopus koristi nasu `opencode` temu.
...
Octopus dolazi sa vise ugradenih tema.
...
- Zele da Octopus odgovara izgledu njihovog terminala
...
Octopus podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/octopus/themes/*.json` ili `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.octopus/themes/*.json`
4. **Trenutni radni direktorij** - `./.octopus/themes/*.json`
