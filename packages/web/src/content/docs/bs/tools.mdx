...
:::note
Ovaj alat je dostupan samo kada je `IMECODE_EXPERIMENTAL_LSP_TOOL=true` (ili `IMECODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `IMECODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
