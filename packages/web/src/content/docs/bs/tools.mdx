...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Pencode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Pencode.
...
:::note
Ovaj alat je dostupan samo kada je `PENCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `PENCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Pencode provajdera ili kada je varijabla `PENCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Pencode:
