...
:::note
Ovaj alat je dostupan samo kada je `LEAKCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `LEAKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `LEAKCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
