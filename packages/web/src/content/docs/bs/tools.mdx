...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CyberStrike dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CyberStrike.
...
:::note
Ovaj alat je dostupan samo kada je `CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true` (ili `CYBERSTRIKE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CyberStrike provajdera ili kada je varijabla `CYBERSTRIKE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CyberStrike:
