...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Octopus dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Octopus.
...
:::note
Ovaj alat je dostupan samo kada je `OCTOPUS_EXPERIMENTAL_LSP_TOOL=true` (ili `OCTOPUS_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Octopus provajdera ili kada je varijabla `OCTOPUS_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Octopus:
