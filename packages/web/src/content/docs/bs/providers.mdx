---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Pencode.
---

import config from "../../../../config.mjs"
export const console = config.console

Pencode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Pencode konfiguraciji.
...
u `~/.local/share/pencode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Pencode
config.
...
Pencode Zen je lista modela koje je obezbedio Pencode tim koji su bili
testirano i potvrđeno da dobro radi sa Pencode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Pencode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite pencode i idite na [pencode.ai/auth](https://pencode.ai/auth).
...
Radi kao i svaki drugi provajder u Pencode i potpuno je opcionalan za korištenje.

---

## Pencode Go

Pencode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Pencode tim i koji su testirani i verificirani da dobro rade s Pencode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Pencode Go` i idite na [pencode.ai/auth](https://pencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Pencode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete pencode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `pencode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Pencode.
:::
...
Možete konfigurirati pencode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi pencode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju pencode konfiguraciju.
...
da pohrani token u pencode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Pencode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Pencode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`pencode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `pencode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s pencode:
...
Za korištenje Google Vertex AI s Pencode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće pencode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem pencodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session) da automatski prijavite svaki Pencode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Pencode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati pencode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati pencode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati pencode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Pencode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/pencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Pencode:
...
3. Kopirajte API ključ za korištenje u Pencode.
...
6. **Važno**: Prije upotrebe modela oblaka u Pencode, morate lokalno povući informacije o modelu:
...
Pencode Zen je lista testiranih i verifikovanih modela koju je obezbedio Pencode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Pencode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Pencode Zen**.

```txt
   /connect
```

3. Unesite svoj Pencode API ključ.
...
Također možete dodati dodatne modele putem vaše pencode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše pencode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Pencodeom:
...
Također možete prilagoditi modele kroz svoju pencode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s pencode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `pencode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Pencode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `pencode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Pencode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj pencode konfiguraciji.
