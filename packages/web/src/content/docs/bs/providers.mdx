---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

Octopus koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Octopus konfiguraciji.
...
u `~/.local/share/octopus/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Octopus
config.
...
Octopus Zen je lista modela koje je obezbedio Octopus tim koji su bili
testirano i potvrđeno da dobro radi sa Octopus. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Octopus Zen.
:::
...
Radi kao i svaki drugi provajder u Octopus i potpuno je opcionalan za korištenje.

---

## Octopus Go

Octopus Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Octopus tim i koji su testirani i verificirani da dobro rade s Octopus-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Octopus Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Octopus:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `octopus.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Octopus.
:::
...
Octopus koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Octopus
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`octopus.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `octopus.json`:
...
Za korištenje Google Vertex AI s Octopus:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Octopus razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Octopus razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za Octopus. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Octopus:
...
3. Kopirajte API ključ za korištenje u Octopus.
...
6. **Važno**: Prije upotrebe modela oblaka u Octopus, morate lokalno povući informacije o modelu:
...
Octopus Zen je lista testiranih i verifikovanih modela koju je obezbedio Octopus tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Octopus Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Octopus Zen**.

```txt
   /connect
```

3. Unesite svoj Octopus API ključ.
...
4. Kreirajte ili ažurirajte svoju `octopus.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Octopus da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite Octopus konfiguraciju i:
