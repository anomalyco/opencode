...
u `~/.local/share/leak-code/auth.json`.
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `leak-code.json`:
...
da pohrani token u leak-code auth memoriju.
:::
...
`leak-code.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `leak-code.json`:
...
4. Kreirajte ili ažurirajte svoju `leak-code.json` datoteku u direktoriju projekta:
...
1. **Provjerite postavke autentifikacije**: Pokrenite `leak-code auth list` da vidite da li su vjerodajnice
