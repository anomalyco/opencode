---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CyberStrike.
---

import config from "../../../../config.mjs"
export const console = config.console

CyberStrike koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CyberStrike konfiguraciji.
...
u `~/.local/share/cyberstrike/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CyberStrike
config.
...
CyberStrike Zen je lista modela koje je obezbedio CyberStrike tim koji su bili
testirano i potvrđeno da dobro radi sa CyberStrike. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa CyberStrike Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite cyberstrike i idite na [cyberstrike.io/auth](https://cyberstrike.io/auth).
...
Radi kao i svaki drugi provajder u CyberStrike i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s CyberStrike:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete cyberstrike:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `cyberstrike.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CyberStrike.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi cyberstrike ispravno radio.
   :::
...
Također možete dodati modele kroz svoju cyberstrike konfiguraciju.
...
da pohrani token u cyberstrike auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CyberStrike koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CyberStrike
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`cyberstrike.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `cyberstrike.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s cyberstrike:
...
Za korištenje Google Vertex AI s CyberStrike:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće cyberstrike.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem cyberstrikea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) da automatski prijavite svaki CyberStrike razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CyberStrike razgovor naveden kao zasebna sesija.
...
Možete konfigurirati cyberstrike za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati cyberstrike za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati cyberstrike za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CyberStrike. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/cyberstrike) za detalje.
:::
...
Da biste koristili Ollama Cloud s CyberStrike:
...
3. Kopirajte API ključ za korištenje u CyberStrike.
...
6. **Važno**: Prije upotrebe modela oblaka u CyberStrike, morate lokalno povući informacije o modelu:
...
CyberStrike Zen je lista testiranih i verifikovanih modela koju je obezbedio CyberStrike tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>CyberStrike Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **CyberStrike Zen**.

```txt
   /connect
```

3. Unesite svoj CyberStrike API ključ.
...
Također možete dodati dodatne modele putem vaše cyberstrike konfiguracije.
...
5. Također ih možete prilagoditi putem vaše cyberstrike konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Cyberstrikeom:
...
Također možete prilagoditi modele kroz svoju cyberstrike konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s cyberstrike-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `cyberstrike.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CyberStrike da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `cyberstrike auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CyberStrike konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj cyberstrike konfiguraciji.
