---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj xcsh konfiguraciji.
...
u `~/.local/share/xcsh/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem xcsh
config.
...
xcsh Zen je lista modela koje je obezbedio xcsh tim koji su bili
testirano i potvrđeno da dobro radi sa xcsh. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa xcsh Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite xcsh i idite na [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/auth).
...
Radi kao i svaki drugi provajder u xcsh i potpuno je opcionalan za korištenje.

---

## xcsh Go

xcsh Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža xcsh tim i koji su testirani i verificirani da dobro rade s xcsh-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `xcsh Go` i idite na [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Da biste koristili Amazon Bedrock s xcsh:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete xcsh:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `xcsh.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u xcsh.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi xcsh ispravno radio.
   :::
...
Također možete dodati modele kroz svoju xcsh konfiguraciju.
...
da pohrani token u xcsh auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
xcsh koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate xcsh
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`xcsh.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth).

##### Konfiguracija

Prilagodite putem `xcsh.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s xcsh:
...
Za korištenje Google Vertex AI s xcsh:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće xcsh.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem xcsh, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) da automatski prijavite svaki xcsh razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki xcsh razgovor naveden kao zasebna sesija.
...
Možete konfigurirati xcsh za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati xcsh za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati xcsh za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za xcsh. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/xcsh) za detalje.
:::
...
Da biste koristili Ollama Cloud s xcsh:
...
3. Kopirajte API ključ za korištenje u xcsh.
...
6. **Važno**: Prije upotrebe modela oblaka u xcsh, morate lokalno povući informacije o modelu:
...
xcsh Zen je lista testiranih i verifikovanih modela koju je obezbedio xcsh tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>xcsh Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **xcsh Zen**.

```txt
   /connect
```

3. Unesite svoj xcsh API ključ.
...
Također možete dodati dodatne modele putem vaše xcsh konfiguracije.
...
5. Također ih možete prilagoditi putem vaše xcsh konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju xcsh konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s xcsh-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `xcsh.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju xcsh da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `xcsh auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite xcsh konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj xcsh konfiguraciji.
