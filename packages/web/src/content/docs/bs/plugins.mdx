---
title: Dodaci
description: Napišite vlastite dodatke za proširenje xcsh.
---

Dodaci vam omogućavaju da proširite xcsh spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja xcsh.
...
- `.xcsh/plugins/` - Dodaci na nivou projekta
- `~/.config/xcsh/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/xcsh/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/xcsh/xcsh.json`)
2. Konfiguracija projekta (`xcsh.json`)
3. Globalni direktorij dodataka (`~/.config/xcsh/plugins/`)
4. Direktorij dodataka projekta (`.xcsh/plugins/`)
...
xcsh pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje xcsh.
...
Spriječite xcsh da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u xcsh:
...
Pomoćnik `tool` kreira prilagođeni alat koji xcsh može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za xcsh zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) za detalje.
