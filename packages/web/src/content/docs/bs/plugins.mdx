---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CyberStrike.
---

Dodaci vam omogućavaju da proširite CyberStrike spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CyberStrike.
...
- `.cyberstrike/plugins/` - Dodaci na nivou projekta
- `~/.config/cyberstrike/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/cyberstrike/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/cyberstrike/cyberstrike.json`)
2. Konfiguracija projekta (`cyberstrike.json`)
3. Globalni direktorij dodataka (`~/.config/cyberstrike/plugins/`)
4. Direktorij dodataka projekta (`.cyberstrike/plugins/`)
...
CyberStrike pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Cyberstrike SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CyberStrike.
...
:::note
Ako koristite desktop aplikaciju CyberStrike, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite cyberstrike da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u cyberstrike:
...
Pomoćnik `tool` kreira prilagođeni alat koji cyberstrike može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za cyberstrike zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://cyberstrike.io/docs/sdk) za detalje.
