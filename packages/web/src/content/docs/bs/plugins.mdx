---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Pencode.
---

Dodaci vam omogućavaju da proširite Pencode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Pencode.
...
- `.pencode/plugins/` - Dodaci na nivou projekta
- `~/.config/pencode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/pencode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/pencode/pencode.json`)
2. Konfiguracija projekta (`pencode.json`)
3. Globalni direktorij dodataka (`~/.config/pencode/plugins/`)
4. Direktorij dodataka projekta (`.pencode/plugins/`)
...
Pencode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Pencode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Pencode.
...
Spriječite pencode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u pencode:
...
Pomoćnik `tool` kreira prilagođeni alat koji pencode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za pencode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://pencode.ai/docs/sdk) za detalje.
