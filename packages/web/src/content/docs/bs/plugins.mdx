---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Octopus.
---

Dodaci vam omogućavaju da proširite Octopus spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Octopus.
...
- `.octopus/plugins/` - Dodaci na nivou projekta
- `~/.config/octopus/plugins/` - Globalni dodaci
...
1. Globalna konfiguracija (`~/.config/octopus/octopus.json`)
2. Konfiguracija projekta (`octopus.json`)
3. Globalni direktorij dodataka (`~/.config/octopus/plugins/`)
4. Direktorij dodataka projekta (`.octopus/plugins/`)
...
Octopus pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Octopus.
