...
Agent skills omogucavaju Octopus da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Octopus pretrazuje ove lokacije:

- Konfiguracija projekta: `.octopus/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/octopus/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Octopus ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.octopus/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.octopus/skills/git-release/SKILL.md` ovako:
...
Octopus navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `octopus.json`:
...
**Za ugradene agente** (u `octopus.json`):
