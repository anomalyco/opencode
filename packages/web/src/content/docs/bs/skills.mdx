...
Agent skills omogucavaju CyberStrike da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
CyberStrike pretrazuje ove lokacije:

- Konfiguracija projekta: `.cyberstrike/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, CyberStrike ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.cyberstrike/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.cyberstrike/skills/git-release/SKILL.md` ovako:
...
CyberStrike navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `cyberstrike.json`:
...
**Za ugradene agente** (u `cyberstrike.json`):
