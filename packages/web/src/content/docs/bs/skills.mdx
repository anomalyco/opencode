...
Agent skills omogucavaju Pencode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Pencode pretrazuje ove lokacije:

- Konfiguracija projekta: `.pencode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/pencode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Pencode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.pencode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.pencode/skills/git-release/SKILL.md` ovako:
...
Pencode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `pencode.json`:
...
**Za ugradene agente** (u `pencode.json`):
