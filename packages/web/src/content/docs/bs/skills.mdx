...
Agent skills omogucavaju xcsh da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
xcsh pretrazuje ove lokacije:

- Konfiguracija projekta: `.xcsh/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/xcsh/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, xcsh ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.xcsh/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.xcsh/skills/git-release/SKILL.md` ovako:
...
xcsh navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `xcsh.json`:
...
**Za ugradene agente** (u `xcsh.json`):
