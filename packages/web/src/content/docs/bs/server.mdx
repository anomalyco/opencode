---
title: Server
description: Komunicirajte s leak-code serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `leak-code serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opencode klijent moze koristiti.
...
Postavite `LEAKCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opencode`, ili postavite `LEAKCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `leak-code serve` i za `leak-code web`.
...
:::tip
Koristite leak-code server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava opencode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `leak-code serve` da startate standalone server. Ako je opencode TUI vec pokrenut, `leak-code serve` ce pokrenuti novi server.
...
leak-code server izlaže sljedece API-je.
