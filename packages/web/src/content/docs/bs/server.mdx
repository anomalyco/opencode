---
title: Server
description: Komunicirajte s cyberstrike serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `cyberstrike serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji cyberstrike klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `cyberstrike.local` |
...
Postavite `CYBERSTRIKE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `cyberstrike`, ili postavite `CYBERSTRIKE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `cyberstrike serve` i za `cyberstrike web`.
...
Kada pokrenete `cyberstrike`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite cyberstrike server za programsku interakciju sa cyberstrike-om.
:::

Ova arhitektura omogucava cyberstrike podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `cyberstrike serve` da startate standalone server. Ako je cyberstrike TUI vec pokrenut, `cyberstrike serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CyberStrike [IDE](/docs/ide) pluginovi.
...
cyberstrike server izlaže sljedece API-je.
