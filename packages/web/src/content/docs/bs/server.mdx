---
title: Server
description: Komunicirajte s xcsh serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `xcsh serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji xcsh klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `xcsh.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `xcsh`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `xcsh serve` i za `xcsh web`.
...
Kada pokrenete `xcsh`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite xcsh server za programsku interakciju sa xcsh-om.
:::

Ova arhitektura omogucava xcsh podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `xcsh serve` da startate standalone server. Ako je xcsh TUI vec pokrenut, `xcsh serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste xcsh [IDE](/docs/ide) pluginovi.
...
xcsh server izlaže sljedece API-je.
