---
title: Server
description: Komunicirajte s pencode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `pencode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji pencode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `pencode.local` |
...
Postavite `PENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `pencode`, ili postavite `PENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `pencode serve` i za `pencode web`.
...
Kada pokrenete `pencode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite pencode server za programsku interakciju sa pencode-om.
:::

Ova arhitektura omogucava pencode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `pencode serve` da startate standalone server. Ako je pencode TUI vec pokrenut, `pencode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Pencode [IDE](/docs/ide) pluginovi.
...
pencode server izlaže sljedece API-je.
