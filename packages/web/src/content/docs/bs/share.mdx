...
Da eksplicitno postavite rucni nacin u [config datoteci](./config):
...
Mozete ukljuciti automatsko dijeljenje za sve nove razgovore tako sto `share` postavite na `"auto"` u [config datoteci](./config):
...
Dijeljenje mozete potpuno iskljuciti tako sto `share` postavite na `"disabled"` u [config datoteci](./config):
...
[Saznajte vise](./enterprise) o koristenju opencode u organizaciji.
