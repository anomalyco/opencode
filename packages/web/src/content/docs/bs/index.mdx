...
Za najbolje iskustvo na Windows-u preporučujemo korištenje [Windows Subsystem for Linux (WSL)](./windows-wsl). Pruža bolje performanse i potpunu kompatibilnost sa OpenCode karakteristikama.
:::
...
Ako ste tek počeli koristiti LLM provajdere, preporučujemo korištenje [OpenCode Zen](./zen).
...
Alternativno, možete odabrati jednog od drugih provajdera. [Saznajte više](./providers#directory).
...
Da biste to učinili svojim, preporučujemo [odabir teme](./themes), [prilagođavanje povezivanja tipki](./keybinds), [konfiguriranje formatera koda](./formatters), [kreiranje prilagođenih komandi](./commands), ili igranje sa [OpenCode config](./config).
