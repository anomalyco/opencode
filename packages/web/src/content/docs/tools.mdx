...
Search developer sources.

:::note
This tool is only available when the Firecrawl integration is connected, either through the `/connect` command or the `FIRECRAWL_API_KEY` environment variable. It is separate from `websearch` and does not take part in web search provider selection.
:::
...
Searches an index built for coding agents: repositories, GitHub issues, merged pull requests, READMEs, and curated documentation sites. Results include the matched passages, so the model gets the relevant documentation excerpt or issue thread rather than a page summary.

:::tip
Use `devsearch` for how a library or API behaves, what an error message means, or whether a bug was fixed. Use `websearch` for everything else.
:::
...
Find and run catalogued data providers.
...
Pass a `query` to discover capabilities in Firecrawl Alexandria, a catalogue of official APIs, licensed publishers, and Firecrawl indexes. Each match comes back with its input contract, response fields, and price. Pass a `provider` and `capability` returned by discovery, plus `options` matching that contract, to run it and get structured records back.

Discovery is free. Running a capability spends Firecrawl credits, and each returned item may carry an `error` instead of a result.

:::tip
Prefer `alexandria` over scraping when a catalogued provider covers the data; it returns structured records instead of page content.
:::
