...
Stream a large HTTP or HTTPS file directly to disk.
...
The download remains inside one tool call while OpenCode displays byte, speed, and estimated-time progress. The model does not receive another turn until the transfer completes or fails. Downloads are written to an adjacent temporary file, optionally verified with SHA-256, and then moved into place.

Use this tool instead of `webfetch` or a shell command when saving a file that may be large or slow.
