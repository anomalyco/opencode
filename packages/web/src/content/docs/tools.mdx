...
Tools allow the LLM to perform actions in your codebase. Pencode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Pencode.
...
:::note
This tool is only available when `PENCODE_EXPERIMENTAL_LSP_TOOL=true` (or `PENCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Pencode or Pencode Go provider, or when the `PENCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Pencode:
