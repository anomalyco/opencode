...
- **Exa** (default). Used when `OPENCODE_ENABLE_EXA=1` is set or when the hosted `opencode` provider is in use.
- **Perplexity Search API** (opt-in). Set `OPENCODE_ENABLE_PERPLEXITY=1` and provide a `PERPLEXITY_API_KEY` (or `PPLX_API_KEY`). Calls Perplexity's `/search` REST endpoint directly.

:::note
This tool is only available when using the OpenCode provider, when the `OPENCODE_ENABLE_EXA` environment variable is set to any truthy value, or when `OPENCODE_ENABLE_PERPLEXITY=1` is set with a Perplexity API key.
