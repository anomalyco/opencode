...
:::note
This tool is only available when using the OpenCode provider or when the `OPENCODE_ENABLE_EXA` environment variable is set to `true`.
:::
...
- `numResults` (optional): Number of results to return (default: 8)
- `type` (optional): Search type - `"auto"` (balanced), `"fast"` (quick results), or `"deep"` (comprehensive) (default: "auto")
- `livecrawl` (optional): Live crawl mode - `"fallback"` (use live if cached unavailable) or `"preferred"` (prioritize live crawling) (default: "fallback")
- `contextMaxCharacters` (optional): Maximum characters to return for LLM context (default: 10000)
