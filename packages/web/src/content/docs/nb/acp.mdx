---
title: ACP-støtte
description: Bruk xcsh i en hvilken som helst ACP-kompatibel editor.
---

xcsh støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke xcsh via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `xcsh acp`.

Kommandoen starter xcsh som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'xcsh'-agenten i AI Chat-agentvelgeren.
...
For å bruke xcsh som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke xcsh som ACP-agent for chat.
...
xcsh fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i xcsh-konfigurasjonen
