...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/leak-code/themes/*.json` eller `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.leak-code/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.leak-code/themes/*.json`
