...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (eller `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Hvis `leak-code.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `LEAKCODE_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `LEAKCODE_PORT` (eller velg en ledig port) og start på nytt.
