...
description: Lag verktøy som LLM kan kalle inn Octopus.
---

Egendefinerte verktøy er funksjoner du oppretter som LLM kan kalle under samtaler. De fungerer sammen med Octopus sine [innebygde verktøy](/docs/tools) som `read`, `write` og `bash`.
...
- Lokalt ved å plassere dem i `.octopus/tools/`-katalogen til prosjektet ditt.
- Eller globalt, ved å plassere dem i `~/.config/octopus/tools/`.
...
```ts title=".octopus/tools/bash.ts"
import { tool } from "@octopus-ai/plugin"
