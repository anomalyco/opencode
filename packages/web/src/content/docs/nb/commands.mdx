...
Opprett `.octopus/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom Octopus-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Octopus [config](/docs/config):
...
Octopus inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
