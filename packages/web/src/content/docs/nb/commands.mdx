...
Egendefinerte kommandoer kommer i tillegg til de innebygde kommandoene som `/init`, `/undo`, `/redo`, `/share`, `/help`. [Finn ut mer](./tui#commands).
...
Bruk alternativet `command` i OpenCode [config](./config):
...
Bruk _!`command`_ for å injisere [bash-kommando](./tui#bash-commands)-utdata i ledeteksten.
...
Bruk `agent`-konfigurasjonen for å spesifisere hvilken [agent](./agents) som skal utføre denne kommandoen.
Hvis dette er en [subagent](./agents/#subagents), vil kommandoen utløse en subagent påkalling som standard.
...
Bruk `subtask` boolsk for å tvinge kommandoen til å utløse en [subagent](./agents/#subagents) påkalling.
...
selv om `mode` er satt til `primary` på [agent](./agents) konfigurasjonen.
...
opencode inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](./tui#commands).
