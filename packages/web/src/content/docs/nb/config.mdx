...
[Les mer om bruk av TUI her](./tui).
...
[Les mer om serveren her](./server).
...
Du kan også konfigurere [lokale modeller](./models#local). [Finn ut mer](./models).
...
:::note
Bærer-tokens (`AWS_BEARER_TOKEN_BEDROCK` eller `/connect`) har forrang over profilbasert autentisering. Se [godkjenningsprioritet](./providers#authentication-precedence) for detaljer.
:::

[Les mer om konfigurasjon av Amazons grunnfjell](./providers#amazon-bedrock).
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.opencode/agents/`. [Les mer her](./agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](./agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil OpenCode falle tilbake til `"build"` med en advarsel.
...
Du kan konfigurere [share](./share)-funksjonen gjennom alternativet `share`.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.opencode/commands/`. [Les mer her](./commands).
...
[Les mer om formattere her](./formatters).
...
[Les mer om tillatelser her](./permissions).
...
[Les mer her](./mcp-servers).

---

### Plugins

[Plugins](./plugins) utvider OpenCode med tilpassede verktøy, kroker og integrasjoner.
