...
2. **Global konfigurasjon** (`~/.config/leak-code/leak-code.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`LEAKCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`leak-code.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.leak-code` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`LEAKCODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.leak-code` og `~/.config/leak-code` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/leak-code/leak-code.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `leak-code.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `LEAKCODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `LEAKCODE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.leak-code` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.leak-code`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan konfigurere serverinnstillinger for kommandoene `leak-code serve` og `leak-code web` gjennom alternativet `server`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/leak-code/agents/` eller `.leak-code/agents/`. [Les mer her](/docs/agents).
...
Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`leak-code run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/leak-code/commands/` eller `.leak-code/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.leak-code/plugins/` eller `~/.config/leak-code/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
