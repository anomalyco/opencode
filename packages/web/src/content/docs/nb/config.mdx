...
Du kan konfigurere Octopus ved å bruke en JSON konfigurasjonsfil.

---

## Format

Octopus støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global konfigurasjon** (`~/.config/octopus/octopus.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OCTOPUS_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`octopus.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.opencode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`OCTOPUS_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
Plasser din globale Octopus-konfigurasjon i `~/.config/octopus/octopus.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `octopus.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Octopus starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `OCTOPUS_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `OCTOPUS_CONFIG_DIR`
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Octopus-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Octopus-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Octopus å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/octopus/agents/` eller `.octopus/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Octopus falle tilbake til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/octopus/commands/` eller `.octopus/commands/`. [Les mer her](/docs/commands).
...
Octopus vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Octopus alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Octopus med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.octopus/plugins/` eller `~/.config/octopus/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Octopus til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
