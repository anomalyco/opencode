...
`OPENCODE_CONFIG_DIRS` kan angi flere tilpassede konfigurasjonskataloger med samme skilletegn som systemets `PATH` (`:` på Linux/macOS, `;` på Windows). Katalogene lastes fra venstre mot høyre, så senere kataloger overstyrer tidligere kataloger. Hvis `OPENCODE_CONFIG_DIR` også er satt, legges den til sist og blir den tilpassede katalogen med høyest prioritet og globalt skrivemål.
