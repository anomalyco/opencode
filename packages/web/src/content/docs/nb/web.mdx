...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `LEAKCODE_SERVER_PASSWORD`:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Brukernavnet er satt til `opencode` som standard, men kan endres med `LEAKCODE_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `leak-code.json` konfigurasjonsfilen:
