...
Finn ut mer om [leverandører](./providers).
...
Her er hele ID `provider_id/model_id`. For eksempel, hvis du bruker [OpenCode Zen](./zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.

Hvis du har konfigurert en [egendefinert leverandør](./providers#custom), er `provider_id` nøkkelen fra `provider`-delen av konfigurasjonen din, og `model_id` er nøkkelen fra `provider.models`.
...
Du kan også konfigurere disse alternativene for alle agenter du bruker. Agentkonfigurasjonen overstyrer alle globale alternativer her. [Finn ut mer](./agents/#additional).
...
Bruk tastebindingen `variant_cycle` for raskt å bytte mellom varianter. [Finn ut mer](./keybinds).
