...
Du kan også ha globale regler i en `~/.config/leak-code/AGENTS.md`-fil. Dette blir brukt på alle OpenCode-økter.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (brukes hvis ingen `~/.config/leak-code/AGENTS.md` eksisterer)
...
2. **Global fil** på `~/.config/leak-code/AGENTS.md`
3. **Claude Code-fil** på `~/.claude/CLAUDE.md` (med mindre de er deaktivert)

Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, brukes bare `AGENTS.md`. På samme måte har `~/.config/leak-code/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Egendefinerte instruksjoner

Du kan spesifisere egendefinerte instruksjonsfiler i din `leak-code.json` eller den globale `~/.config/leak-code/leak-code.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
### Bruker leak-code.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `leak-code.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `leak-code.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
