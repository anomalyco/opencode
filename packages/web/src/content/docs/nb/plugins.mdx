...
description: Skriv dine egne programtillegg for å utvide Octopus.
---

Plugins lar deg utvide Octopus ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Octopus.
...
- `.octopus/plugins/` - Programtillegg på prosjektnivå
...
1. Global konfigurasjon (`~/.config/octopus/octopus.json`)
2. Prosjektkonfigurasjon (`octopus.json`)
3. Global plugin-katalog (`~/.config/octopus/plugins/`)
4. Prosjektpluginkatalog (`.octopus/plugins/`)
...
Octopus kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Octopus SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Octopus.
...
:::note
Hvis du bruker Octopus-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Octopus fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Octopus:
...
`tool`-hjelperen lager et tilpasset verktøy som Octopus kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Octopus sammen med innebygde verktøy.
