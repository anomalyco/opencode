...
description: Skriv dine egne programtillegg for å utvide xcsh.
---

Plugins lar deg utvide xcsh ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til xcsh.
...
- `.xcsh/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/xcsh/node_modules/`.
...
1. Global konfigurasjon (`~/.config/xcsh/xcsh.json`)
2. Prosjektkonfigurasjon (`xcsh.json`)
3. Global plugin-katalog (`~/.config/xcsh/plugins/`)
4. Prosjektpluginkatalog (`.xcsh/plugins/`)
...
xcsh kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En xcsh SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide xcsh.
...
:::note
Hvis du bruker xcsh-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre xcsh fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til xcsh:
...
`tool`-hjelperen lager et tilpasset verktøy som xcsh kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for xcsh sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) for detaljer.
