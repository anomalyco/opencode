...
description: Skriv dine egne programtillegg for å utvide cyberstrike.
---

Plugins lar deg utvide CyberStrike ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CyberStrike.
...
- `.cyberstrike/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/cyberstrike/node_modules/`.
...
1. Global konfigurasjon (`~/.config/cyberstrike/cyberstrike.json`)
2. Prosjektkonfigurasjon (`cyberstrike.json`)
3. Global plugin-katalog (`~/.config/cyberstrike/plugins/`)
4. Prosjektpluginkatalog (`.cyberstrike/plugins/`)
...
CyberStrike kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CyberStrike SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CyberStrike.
...
:::note
Hvis du bruker CyberStrike-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CyberStrike fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CyberStrike:
...
`tool`-hjelperen lager et tilpasset verktøy som CyberStrike kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CyberStrike sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://cyberstrike.io/docs/sdk) for detaljer.
