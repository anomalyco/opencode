...
description: Skriv dine egne programtillegg for å utvide Pencode.
---

Plugins lar deg utvide Pencode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Pencode.
...
- `.pencode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/pencode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/pencode/pencode.json`)
2. Prosjektkonfigurasjon (`pencode.json`)
3. Global plugin-katalog (`~/.config/pencode/plugins/`)
4. Prosjektpluginkatalog (`.pencode/plugins/`)
...
Pencode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Pencode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Pencode.
...
:::note
Hvis du bruker Pencode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Pencode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Pencode:
...
`tool`-hjelperen lager et tilpasset verktøy som Pencode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Pencode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://pencode.ai/docs/sdk) for detaljer.
