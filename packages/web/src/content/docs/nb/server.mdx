...
Kommandoen `leak-code serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCode-klient kan bruke.
...
Sett `LEAKCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `LEAKCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `leak-code serve` og `leak-code web`.
...
Du kan kjøre `leak-code serve` for å starte en frittstående server. Hvis du har
OpenCode TUI kjører, vil `leak-code serve` starte en ny server.
