---
title: Tjener
description: Samhandle med xcsh-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `xcsh serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en xcsh-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `xcsh.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `xcsh` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `xcsh serve` og `xcsh web`.
...
Når du kjører `xcsh` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk xcsh-serveren til å samhandle med xcsh programmatisk.
:::

Denne arkitekturen lar xcsh støtte flere klienter og lar deg samhandle med xcsh programmatisk.

Du kan kjøre `xcsh serve` for å starte en frittstående server. Hvis du har
xcsh TUI kjører, vil `xcsh serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av xcsh [IDE](/docs/ide) plugins.
...
xcsh-serveren viser følgende APIer.
