---
title: Tjener
description: Samhandle med CyberStrike-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `cyberstrike serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CyberStrike-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `cyberstrike.local` |
...
Sett `CYBERSTRIKE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `cyberstrike` som standard, eller sett `CYBERSTRIKE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `cyberstrike serve` og `cyberstrike web`.
...
Når du kjører `cyberstrike` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CyberStrike-serveren til å samhandle med CyberStrike programmatisk.
:::

Denne arkitekturen lar CyberStrike støtte flere klienter og lar deg samhandle med CyberStrike programmatisk.

Du kan kjøre `cyberstrike serve` for å starte en frittstående server. Hvis du har
CyberStrike TUI kjører, vil `cyberstrike serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CyberStrike [IDE](/docs/ide) plugins.
...
cyberstrike-serveren viser følgende APIer.
