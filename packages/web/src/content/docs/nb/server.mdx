---
title: Tjener
description: Samhandle med Octopus-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `opencode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Octopus-klient kan bruke.
...
Sett `OCTOPUS_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `OCTOPUS_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opencode serve` og `opencode web`.
...
:::tip
Bruk Octopus-serveren til å samhandle med Octopus programmatisk.
:::

Denne arkitekturen lar Octopus støtte flere klienter og lar deg samhandle med Octopus programmatisk.

Du kan kjøre `opencode serve` for å starte en frittstående server. Hvis du har
Octopus TUI kjører, vil `opencode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Octopus [IDE](/docs/ide) plugins.
