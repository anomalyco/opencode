---
title: Tjener
description: Samhandle med Pencode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `pencode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Pencode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `pencode.local` |
...
Sett `PENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `pencode` som standard, eller sett `PENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `pencode serve` og `pencode web`.
...
Når du kjører `pencode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Pencode-serveren til å samhandle med Pencode programmatisk.
:::

Denne arkitekturen lar Pencode støtte flere klienter og lar deg samhandle med Pencode programmatisk.

Du kan kjøre `pencode serve` for å starte en frittstående server. Hvis du har
Pencode TUI kjører, vil `pencode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Pencode [IDE](/docs/ide) plugins.
...
pencode-serveren viser følgende APIer.
