...
:::note
Når macOS-sandkassing er aktivert, kjører bash med filsystembegrensninger og kan be om et separat nytt forsøk uten sandkasse ved behov.
Hvis du vet at en kommando må starte utenfor sandkassen, legg `# opencode:unsandboxed <reason>` på den første ikke-tomme linjen i kommandoen.
Se [sandbox-konfigurasjon](/docs/config#sandbox) for støttet oppførsel og begrensninger.
:::
