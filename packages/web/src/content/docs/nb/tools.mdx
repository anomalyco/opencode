...
:::note
Dette verktøyet er bare tilgjengelig når `LEAKCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `LEAKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `LEAKCODE_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
