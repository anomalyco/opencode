...
:::note
Dette verktøyet er bare tilgjengelig når `IMECODE_EXPERIMENTAL_LSP_TOOL=true` (eller `IMECODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `IMECODE_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
