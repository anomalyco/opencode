...
Agentferdigheter lar CyberStrike oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
cyberstrike søker etter disse stedene:

- Prosjektkonfigurasjon: `.cyberstrike/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
For prosjektlokale stier går cyberstrike opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.cyberstrike/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.cyberstrike/skills/git-release/SKILL.md` slik:
...
cyberstrike viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `cyberstrike.json`:
...
**For innebygde agenter** (i `cyberstrike.json`):
