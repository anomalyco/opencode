...
Agentferdigheter lar xcsh oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
xcsh søker etter disse stedene:

- Prosjektkonfigurasjon: `.xcsh/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/xcsh/skills/<name>/SKILL.md`
...
For prosjektlokale stier går xcsh opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.xcsh/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.xcsh/skills/git-release/SKILL.md` slik:
...
xcsh viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `xcsh.json`:
...
**For innebygde agenter** (i `xcsh.json`):
