...
Octopus gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Octopus starter TUI for gjeldende katalog.
...
Når du bruker Octopus TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Octopus. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Octopus. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `octopus.json`, som konfigurerer server-/kjøretidsoppførsel.
...
Bruk `OCTOPUS_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
