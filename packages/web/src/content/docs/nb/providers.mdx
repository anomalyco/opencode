---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i xcsh-konfigurasjonen.
...
i `~/.local/share/xcsh/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i xcsh
konfig.
...
xcsh Zen er en liste over modeller levert av xcsh-teamet som har vært
testet og verifisert for å fungere godt med xcsh. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med xcsh Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `xcsh Zen` og gå til [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Det fungerer som alle andre leverandører i xcsh og er helt valgfritt å bruke.

---

## xcsh Go

xcsh Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av xcsh-teamet som har vært
testet og verifisert for å fungere godt med xcsh.

1. Kjør kommandoen `/connect` i TUI, velg `xcsh Go`, og gå til [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Slik bruker du Amazon Bedrock med xcsh:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører xcsh:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `xcsh.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i xcsh støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at xcsh skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom xcsh-konfigurasjonen.
...
for å lagre token i xcsh auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
xcsh bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse xcsh
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`xcsh.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `xcsh.json`:
...
Slik bruker du GitHub Copilot-abonnementet med xcsh:
...
Slik bruker du Google Vertex AI med xcsh:
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver xcsh-samtale oppført som en separat økt.
...
Du kan konfigurere xcsh for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere xcsh for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere xcsh for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for xcsh. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/xcsh) for detaljer.
:::
...
Slik bruker du Ollama Cloud med xcsh:
...
3. Kopier API-nøkkelen for bruk i xcsh.
...
6. **Viktig**: Før du bruker skymodeller i xcsh, må du hente modellinformasjonen lokalt:
...
xcsh Zen er en liste over testede og verifiserte modeller levert av xcsh-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>xcsh Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **xcsh Zen**.

   ```txt
   /connect
   ```

3. Skriv inn xcsh API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom xcsh-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom xcsh-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med xcsh:
...
Du kan også tilpasse modeller gjennom xcsh-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med xcsh. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `xcsh.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar xcsh forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `xcsh auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk xcsh-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i xcsh-konfigurasjonen.
