...
i `~/.local/share/leak-code/auth.json`.
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `leak-code.json`:
...
`leak-code.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `leak-code.json`:
...
4. Opprett eller oppdater `leak-code.json`-filen i prosjektkatalogen:
...
1. **Sjekk autentiseringsoppsettet**: Kjør `leak-code auth list` for å se om legitimasjonen
