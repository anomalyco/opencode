---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

Octopus bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Octopus-konfigurasjonen.
...
i `~/.local/share/octopus/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Octopus
konfig.
...
Octopus Zen er en liste over modeller levert av Octopus-teamet som har vært
testet og verifisert for å fungere godt med Octopus. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Octopus Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Octopus Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i Octopus og er helt valgfritt å bruke.

---

## Octopus Go

Octopus Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Octopus-teamet som har vært
testet og verifisert for å fungere godt med Octopus.

1. Kjør kommandoen `/connect` i TUI, velg `Octopus Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Octopus:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Octopus:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `octopus.json`:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Octopus skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Octopus-konfigurasjonen.
...
for å lagre token i Octopus auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Octopus bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Octopus
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`octopus.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Octopus:
...
Slik bruker du Google Vertex AI med Octopus:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Octopus.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Octopus-samtale oppført som en separat økt.
...
Du kan konfigurere Octopus for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Octopus for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Octopus for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Octopus. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Octopus:
...
3. Kopier API-nøkkelen for bruk i Octopus.
...
6. **Viktig**: Før du bruker skymodeller i Octopus, må du hente modellinformasjonen lokalt:
...
Octopus Zen er en liste over testede og verifiserte modeller levert av Octopus-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Octopus Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Octopus Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Octopus API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Octopus-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Octopus-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Octopus:
...
Du kan også tilpasse modeller gjennom Octopus-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Octopus. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `octopus.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Octopus forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk Octopus-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Octopus-konfigurasjonen.
