---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CyberStrike.
---

import config from "../../../../config.mjs"
export const console = config.console

CyberStrike bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CyberStrike-konfigurasjonen.
...
i `~/.local/share/cyberstrike/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i CyberStrike
konfig.
...
CyberStrike Zen er en liste over modeller levert av CyberStrike-teamet som har vært
testet og verifisert for å fungere godt med CyberStrike. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med CyberStrike Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg cyberstrike og gå til [cyberstrike.io/auth](https://cyberstrike.io/auth).
...
Det fungerer som alle andre leverandører i CyberStrike og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med CyberStrike:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CyberStrike:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `cyberstrike.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i cyberstrike støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CyberStrike skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CyberStrike-konfigurasjonen.
...
for å lagre token i CyberStrike auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CyberStrike bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CyberStrike
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`cyberstrike.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `cyberstrike.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CyberStrike:
...
Slik bruker du Google Vertex AI med CyberStrike:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CyberStrike.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CyberStrike-samtale oppført som en separat økt.
...
Du kan konfigurere CyberStrike for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CyberStrike for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CyberStrike for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CyberStrike. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/cyberstrike) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CyberStrike:
...
3. Kopier API-nøkkelen for bruk i CyberStrike.
...
6. **Viktig**: Før du bruker skymodeller i CyberStrike, må du hente modellinformasjonen lokalt:
...
CyberStrike Zen er en liste over testede og verifiserte modeller levert av CyberStrike-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>CyberStrike Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **CyberStrike Zen**.

   ```txt
   /connect
   ```

3. Skriv inn CyberStrike API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CyberStrike-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CyberStrike-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CyberStrike:
...
Du kan også tilpasse modeller gjennom CyberStrike-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CyberStrike. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `cyberstrike.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CyberStrike forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `cyberstrike auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CyberStrike-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CyberStrike-konfigurasjonen.
