---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Pencode.
---

import config from "../../../../config.mjs"
export const console = config.console

Pencode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Pencode-konfigurasjonen.
...
i `~/.local/share/pencode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Pencode
konfig.
...
Pencode Zen er en liste over modeller levert av Pencode-teamet som har vært
testet og verifisert for å fungere godt med Pencode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Pencode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Pencode Zen` og gå til [pencode.ai/auth](https://pencode.ai/zen).
...
Det fungerer som alle andre leverandører i Pencode og er helt valgfritt å bruke.

---

## Pencode Go

Pencode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Pencode-teamet som har vært
testet og verifisert for å fungere godt med Pencode.

1. Kjør kommandoen `/connect` i TUI, velg `Pencode Go`, og gå til [pencode.ai/auth](https://pencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Pencode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Pencode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `pencode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i pencode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere pencode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Pencode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Pencode-konfigurasjonen.
...
for å lagre token i Pencode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Pencode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Pencode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`pencode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `pencode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Pencode:
...
Slik bruker du Google Vertex AI med Pencode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Pencode.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Pencode-samtale oppført som en separat økt.
...
Du kan konfigurere Pencode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Pencode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Pencode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Pencode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/pencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Pencode:
...
3. Kopier API-nøkkelen for bruk i Pencode.
...
6. **Viktig**: Før du bruker skymodeller i Pencode, må du hente modellinformasjonen lokalt:
...
Pencode Zen er en liste over testede og verifiserte modeller levert av Pencode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Pencode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Pencode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Pencode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Pencode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Pencode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Pencode:
...
Du kan også tilpasse modeller gjennom Pencode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Pencode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `pencode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Pencode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `pencode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Pencode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Pencode-konfigurasjonen.
