...
Du kan legge til eksterne verktøy i CyberStrike ved å bruke _Model Context Protocol_ eller MCP. CyberStrike støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CyberStrike Config](https://cyberstrike.io/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/cyberstrike`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
CyberStrike håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CyberStrike:
...
Hvis serveren krever autentisering, vil CyberStrike be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `cyberstrike mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CyberStrike lagre tokens sikkert i `~/.local/share/cyberstrike/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i CyberStrike, sammen med innebygde verktøy. Så du kan administrere dem gjennom CyberStrike-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CyberStrike til Sentry-kontoen din.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CyberStrike, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) plugin for automatisk å logge hver CyberStrike-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CyberStrike-samtale oppført som en separat økt.
