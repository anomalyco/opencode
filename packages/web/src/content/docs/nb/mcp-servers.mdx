...
Du kan legge til eksterne verktøy i xcsh ved å bruke _Model Context Protocol_ eller MCP. xcsh støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="xcsh.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/xcsh`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
xcsh håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil xcsh:
...
Hvis serveren krever autentisering, vil xcsh be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `xcsh mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil xcsh lagre tokens sikkert i `~/.local/share/xcsh/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i xcsh, sammen med innebygde verktøy. Så du kan administrere dem gjennom xcsh-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble xcsh til Sentry-kontoen din.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom xcsh, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) plugin for automatisk å logge hver xcsh-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver xcsh-samtale oppført som en separat økt.
