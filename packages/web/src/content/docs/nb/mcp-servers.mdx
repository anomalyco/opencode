...
Du kan legge til eksterne verktøy i Pencode ved å bruke _Model Context Protocol_ eller MCP. Pencode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Pencode Config](https://pencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="pencode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/pencode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Pencode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Pencode:
...
Hvis serveren krever autentisering, vil Pencode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `pencode mcp auth <server-name>`.
...
```bash
pencode mcp auth my-oauth-server
```
...
```bash
pencode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Pencode lagre tokens sikkert i `~/.local/share/pencode/mcp-auth.json`.
...
pencode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Pencode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Pencode-konfigurasjonen som alle andre verktøy.
...
```json title="pencode.json" {11, 14-18}
{
  "$schema": "https://pencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
pencode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Pencode til Sentry-kontoen din.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Pencode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session) plugin for automatisk å logge hver Pencode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Pencode-samtale oppført som en separat økt.
