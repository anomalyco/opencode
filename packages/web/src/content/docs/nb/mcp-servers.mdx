...
Du kan legge til eksterne verktøy i Octopus ved å bruke _Model Context Protocol_ eller MCP. Octopus støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Octopus Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="octopus.jsonc" {6}
{
...
Octopus håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Octopus:
...
Hvis serveren krever autentisering, vil Octopus be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Octopus lagre tokens sikkert i `~/.local/share/octopus/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Octopus, sammen med innebygde verktøy. Så du kan administrere dem gjennom Octopus-konfigurasjonen som alle andre verktøy.
...
```json title="octopus.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Octopus til Sentry-kontoen din.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Octopus, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Octopus-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Octopus-samtale oppført som en separat økt.
