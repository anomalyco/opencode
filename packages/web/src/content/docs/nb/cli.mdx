...
OpenCode CLI starter som standard [TUI](./tui) når den kjøres uten noen argumenter.
...
Dette setter opp den nødvendige GitHub Actions-arbeidsflyten og veileder deg gjennom konfigurasjonsprosessen. [Finn ut mer](./github).
...
Dette er nyttig for å finne ut det eksakte modellnavnet som skal brukes i [din konfigurasjon](./config/).
...
Start en hodeløs OpenCode-server for API-tilgang. Sjekk ut [server-dokumentene](./server) for hele HTTP-grensesnittet.
