---
title: CLI
description: xcsh CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
xcsh
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med xcsh programmatisk.
...
Start xcsh-terminalbrukergrensesnittet.
...
Koble en terminal til en allerede kjørende xcsh-backend-server startet via `serve` eller `web` kommandoer.

```bash
xcsh attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern xcsh-backend. For eksempel:
...
xcsh drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `xcsh auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

Når xcsh starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i xcsh.
...
Kjør xcsh i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `xcsh serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende xcsh-server (f.eks. http://localhost:4096)     |
...
Start en headless xcsh-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
xcsh serve
```

Dette starter en HTTP-server som gir API tilgang til xcsh-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `xcsh`).
...
Vis tokenbruk og kostnadsstatistikk for xcsh-øktene dine.
...
Importer øktdata fra en JSON-fil eller en xcsh share-URL.
...
Start en headless xcsh-server med et webgrensesnitt.

```bash
xcsh web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til xcsh via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `xcsh`).
...
Avinstaller xcsh og fjern alle relaterte filer.
...
Oppdaterer xcsh til siste versjon eller en spesifikk versjon.
...
xcsh CLI bruker følgende globale flagg.
...
xcsh kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `xcsh`) |
