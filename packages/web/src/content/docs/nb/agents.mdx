...
Det er to typer agenter i xcsh; primære agenter og underagenter.
...
xcsh kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
xcsh kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

xcsh kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `xcsh.json`:
...
- Globalt: `~/.config/xcsh/agents/`
- Per prosjekt: `.xcsh/agents/`
...
Hvis ingen temperatur er spesifisert, bruker xcsh modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale xcsh-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i xcsh-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [xcsh Zen](/docs/zen), vil du bruke `xcsh/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `xcsh models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/f5xc-salesdemos/xcsh).
:::
