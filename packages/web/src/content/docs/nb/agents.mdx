...
Det er to typer agenter i Pencode; primære agenter og underagenter.
...
Pencode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Pencode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Pencode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `pencode.json`:
...
Hvis ingen temperatur er spesifisert, bruker Pencode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Pencode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Pencode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Pencode Zen](/docs/zen), vil du bruke `pencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `pencode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/kiyosh11/pencode).
:::
