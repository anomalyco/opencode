...
:::tupp
Hvis du ikke spesifiserer en modell, bruker primæragenter [modell globalt konfigurert](./config#models) mens subagenter vil bruke modellen til primæragenten som påkalte subagenten.
:::
...
Modellen ID i OpenCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCode Zen](./zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
[Finn ut mer om tillatelser](./permissions).
