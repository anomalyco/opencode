...
Det er to typer agenter i Octopus; primære agenter og underagenter.
...
Octopus kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Octopus kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Octopus kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `octopus.json`:
...
Hvis ingen temperatur er spesifisert, bruker Octopus modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Octopus-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Octopus-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Octopus Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/octopus).
:::
