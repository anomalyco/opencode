...
Det er to typer agenter i CyberStrike; primære agenter og underagenter.
...
CyberStrike kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CyberStrike kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

CyberStrike kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `cyberstrike.json`:
...
- Globalt: `~/.config/cyberstrike/agents/`
- Per prosjekt: `.cyberstrike/agents/`
...
Hvis ingen temperatur er spesifisert, bruker CyberStrike modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CyberStrike-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CyberStrike-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [CyberStrike Zen](/docs/zen), vil du bruke `cyberstrike/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `cyberstrike models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/CyberStrikeus/cyberstrike).
:::
