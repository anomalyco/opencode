...
| [opencode-status-popup](https://github.com/LucasInstra/opencode-status-popup)                      | Always-on-top status pill or tray icon (Windows) that shows when the agent works, retries, fails, or needs a permission |
