...
| [@acramsay/opencode-shield-bash](https://github.com/acramsay/opencode-shield-bash)                 | Gate every bash tool call through a second judge session that returns an allow/deny verdict from a fixed safety policy before the command runs |
