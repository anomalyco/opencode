...
To debug any issues with ChaliceCode, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/chalice-code/log/`
...
- **macOS/Linux**: `~/.local/share/chalice-code/`
...
If you're experiencing issues with ChaliceCode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/sst/chalice-code/issues**](https://github.com/sst/chalice-code/issues)
...
2. **Join our Discord**
   ...
   [**opencode.ai/discord**](https://chalice-code.ai/discord)
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/chalice-code
   ```

3. Re-authenticate with your provider:
   ```bash
   chalice auth login <provider>
   ```
...
If you encounter API call errors, this may be due to outdated provider packages. chalice dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/chalice-code
   ```

2. Restart chalice to reinstall the latest provider packages

This will force chalice to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
