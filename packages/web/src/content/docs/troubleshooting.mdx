...
To debug issues with xcsh, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\xcsh\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `xcsh --log-level DEBUG`.

---

## Storage

xcsh stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/xcsh/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\xcsh`
...
xcsh Desktop runs a local xcsh server (the `xcsh-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `xcsh` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/xcsh/xcsh.jsonc` (or `~/.config/xcsh/xcsh.json`)
- **macOS/Linux** (older installs): `~/.local/share/xcsh/xcsh.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\xcsh\xcsh.jsonc`
...
xcsh can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/xcsh/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\xcsh\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so xcsh can rebuild it.

1. Quit xcsh Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/xcsh`
- **Linux**: delete `~/.cache/xcsh` (or run `rm -rf ~/.cache/xcsh`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\xcsh`

3. Restart xcsh Desktop.

---

### Fix server connection issues

xcsh Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `xcsh.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, xcsh Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with xcsh's features.

---

### Notifications not showing

xcsh Desktop only shows system notifications when:

- notifications are enabled for xcsh in your OS settings, and
...
1. Quit xcsh Desktop.
2. Find and delete these files (they live in the xcsh Desktop app data directory):

- `xcsh.settings.dat` (desktop default server URL)
- `xcsh.global.dat` and `xcsh.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with xcsh:
...
3. Ensure you have the latest version with `xcsh upgrade`
...
To figure out what models you have access to, run `xcsh models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\xcsh`
...
If you encounter API call errors, this may be due to outdated provider packages. xcsh dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\xcsh`

2. Restart xcsh to reinstall the latest provider packages

This will force xcsh to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
xcsh will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
