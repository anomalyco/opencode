...
To debug issues with Octopus, start by checking the logs and local data it stores on disk.
...
Octopus Desktop runs a local Octopus server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Octopus` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/octopus/octopus.jsonc` (or `~/.config/octopus/octopus.json`)
- **macOS/Linux** (older installs): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Octopus can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Octopus can rebuild it.

1. Quit Octopus Desktop completely.
...
3. Restart Octopus Desktop.

---

### Fix server connection issues

Octopus Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `octopus.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `OCTOPUS_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `OCTOPUS_PORT` (or pick a free port) and restart.
...
On Windows, Octopus Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Octopus's features.

---

### Notifications not showing

Octopus Desktop only shows system notifications when:

- notifications are enabled for Octopus in your OS settings, and
...
1. Quit Octopus Desktop.
2. Find and delete these files (they live in the Octopus Desktop app data directory):
...
If you're experiencing issues with Octopus:
