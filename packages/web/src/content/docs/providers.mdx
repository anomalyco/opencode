...
1. Head over to the [Requesty dashboard](https://app.requesty.ai/api-keys), click **Create API Key**, and copy the key.

2. Run the `/connect` command and search for Requesty.
...
4. Many Requesty models are preloaded by default, run the `/models` command to select the one you want.
...
5. Requesty [routing policies](https://app.requesty.ai/routing-policies) let you add failover, load balancing, or latency-based routing. Reference a policy by using `policy/<your-policy-name>` as the model ID.
...
6. For GDPR data residency, route through Requesty's EU endpoint.
