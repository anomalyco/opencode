   ...
   ```jsonc title="~/.config/opencode/opencode.jsonc"
   {
     ...
     "provider": {
       "helicone": {
         ...
         "models": {
           "gpt-4o": {  // Model ID (from Helicone's model directory page)
...
You can use template variables in header values that get resolved at request time:

| Template | Description |
|----------|-------------|
| `{session:id}` | UUID for the current session (consistent across requests) |
| `{session:title}` | Session title (only injected after LLM generates it) |
| `{project:id}` | Project ID |
| `{request:id}` | Request/message ID |

##### Session Tracking

Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. With the dynamic header templates, you can automatically log each OpenCode conversation as a single session in Helicone.
       ...
       "helicone": {
         ...
         "options": {
           ...
           "headers": {
             ...
             "Helicone-Session-Name": "{session:title}"
           }
         },
         "models": {
           "claude-sonnet-4-20250514": {
...
- **Every request in a single OpenCode conversation** gets the same `Helicone-Session-Id`, grouping them together as one session in Helicone
- The `{session:id}` is a consistent UUID derived from the OpenCode session, so all messages in that conversation share the same ID
- The `{session:title}` is the conversation title (e.g., "Fix authentication bug") that appears in Helicone's Sessions page

In Helicone's Sessions page, you'll see each OpenCode conversation listed as a separate session, with all the individual LLM requests nested inside. This makes it easy to track costs, latency, and token usage per conversation.
...
| `Helicone-Session-Name` | Human-readable session name |
