---
title: Providers
description: Using any LLM provider in xcsh.
---

import config from "../../../config.mjs"
export const console = config.console

xcsh uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your xcsh config.
...
You can customize the providers through the `provider` section in your xcsh
config.
...
xcsh Zen is a list of models provided by the xcsh team that have been
tested and verified to work well with xcsh. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with xcsh Zen.
:::

1. Run the `/connect` command in the TUI, select `xcsh Zen`, and head to [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
It works like any other provider in xcsh and is completely optional to use.

---

## xcsh Go

xcsh Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the xcsh team that have been
tested and verified to work well with xcsh.

1. Run the `/connect` command in the TUI, select `xcsh Go`, and head to [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
To use Amazon Bedrock with xcsh:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running xcsh:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `xcsh.json`:
...
xcsh. Anthropic explicitly prohibits this.

Previous versions of xcsh came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in xcsh with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for xcsh to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your xcsh config.
...
GitLab Duo support in xcsh is experimental. Features, configuration, and
behavior may change in future releases.
:::

xcsh integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
xcsh uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock xcsh
to only use your own GitLab-hosted instance, add the following to your
`xcsh.json` file. It is also recommended to disable session sharing.
...
More documentation on [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, xcsh will:
...
4. Route tool execution requests through xcsh's permission-gated tool system
...
To use your GitHub Copilot subscription with xcsh:
...
To use Google Vertex AI with xcsh:
...
In the event you see a feature or model from Helicone that isn't configured automatically through xcsh, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) plugin to automatically log each xcsh conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each xcsh conversation listed as a separate session.
...
You can configure xcsh to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure xcsh to use local models through LM Studio.
...
You can configure xcsh to use local models through Ollama.

:::tip
Ollama can automatically configure itself for xcsh. See the [Ollama integration docs](https://docs.ollama.com/integrations/xcsh) for details.
:::
...
3. Copy the API key for use in xcsh.
...
6. **Important**: Before using cloud models in xcsh, you must pull the model information locally:
...
xcsh Zen is a list of tested and verified models provided by the xcsh team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>xcsh Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **xcsh Zen**.

   ```txt
   /connect
   ```

3. Enter your xcsh API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your xcsh config.
...
5. You can also customize them through your xcsh config. Here's an example of specifying a provider
...
You can also customize models through your xcsh config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with xcsh. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `xcsh.json` file in your project directory:
...
The `limit` fields allow xcsh to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `xcsh auth list` to see if the credentials
...
2. For custom providers, check the xcsh config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your xcsh config.
