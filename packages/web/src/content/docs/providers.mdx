...
Jev runs server-side through TypeSafe. Set `TYPESAFE_API_KEY` in the environment used to launch OpenCode. Jev receives the first prompt, attachment names and MIME types, agent name, and OpenAI model metadata. It does not receive repository files, attachment contents, credentials, or prior conversation history. TypeSafe usage is separate from the ChatGPT Plus subscription.

```dotenv title=".env"
TYPESAFE_API_KEY=your_typesafe_key
```

If the key is missing or Jev cannot be reached, OpenCode chooses the newest suitable general OpenAI model at medium reasoning effort and labels the message `Jev fallback`.
