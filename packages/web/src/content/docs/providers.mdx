...
`@ai-sdk/amazon-bedrock/mantle-anthropic` to route it there. AWS credentials are signed with SigV4,
so profiles, SSO, and IRSA work as they do elsewhere in Bedrock. A Bedrock API key is also accepted.
