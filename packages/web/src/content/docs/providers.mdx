...
Jev runs through [OpenRouter](https://openrouter.ai/docs/api_reference/overview) using the `~typesafe/jev-latest` model. Set `OPENROUTER_API_KEY` in the environment used to launch OpenCode. OpenRouter usage is separate from the ChatGPT Plus subscription: the key is used only for the bounded selector request, while the selected OpenAI model still runs through your existing ChatGPT OAuth connection. Jev receives the first prompt, attachment names and MIME types, agent name, and live OpenAI model metadata. It does not receive repository files, attachment contents, credentials, tool output, or prior conversation history.
