...
- **attachment**: Whether the model supports file attachments. For a new custom model without catalog metadata, this defaults to `false`; an existing catalog model inherits its value unless overridden.
- **modalities.input**: The input types the model accepts (`text`, `image`, `audio`, `video`, `pdf`).

:::warning
For a new custom model that accepts images, set both `attachment: true` and `modalities.input: ["text", "image"]`. OpenCode uses these flags to determine the model's capabilities; it does not infer vision support from an OpenAI-compatible endpoint. These flags describe upstream support and do not add vision capabilities to a text-only model.
:::
