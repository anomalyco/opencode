...
2. **Configure authentication** using one of the following methods:
   ...
   **Available options:**
   ...
   - `awsAuthRefresh` - Command to run when AWS credentials expire (e.g., `aws sso login --profile myprofile`)
...
#### Advanced: Automatic Credential Refresh
    ...
    When AWS SSO tokens expire, OpenCode can automatically run `aws sso login` and retry the request.
    ...
    **How it works:**
    - When credentials expire, OpenCode shows a toast notification and runs the refresh command
    - Browser-based SSO flows are supported - output is shown in the terminal
    - After successful refresh, the original request is automatically retried
    - Timeout: 2 minutes | Cooldown: 1 minute between refresh attempts

    **Supported error types:**
    - SSO token expired (`Token is expired`, `ExpiredToken`)
    - SSO session expired or invalid
    - Web identity token issues (EKS IRSA)
    - Security token expired

    **Requirements:**
    - The refresh command must start with `aws` or be an absolute path
    - Requires an interactive terminal (TTY) for browser-based SSO flows
    - For bearer tokens (via `/connect`), manually re-run `opencode auth amazon-bedrock` to refresh

    :::note
    Non-interactive environments (CI/CD, background processes) cannot complete browser-based SSO. In these cases, manually refresh credentials before running OpenCode.
    :::
