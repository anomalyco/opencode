---
title: Providers
description: Using any LLM provider in ChaliceCode.
---

import config from "../../../config.mjs"
export const console = config.console

ChaliceCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using `opencode auth login`.
2. Configure the provider in your ChaliceCode config.
...
in `~/.local/share/chalice-code/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your ChaliceCode
config.
...
ChaliceCode Zen is a list of models provided by the ChaliceCode team that have been
tested and verified to work well with ChaliceCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with ChaliceCode Zen.
:::
...
It works like any other provider in ChaliceCode. And is completely optional to use
it.
...
To use Amazon Bedrock with ChaliceCode:
...
1. You'll need either to set one of the following environment variables:
   ...
   Once you have one of the above, set it while running chalice.

   ```bash
   AWS_ACCESS_KEY_ID=XXX chalice
   ```
...
We recommend signing up for [Claude Pro](https://www.anthropic.com/news/claude-pro) or [Max](https://www.anthropic.com/max), it's the most cost-effective way to use chalice.
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for chalice to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX chalice
   ```
...
To use your GitHub Copilot subscription with chalice:
...
To use Google Vertex AI with ChaliceCode:
...
2. Set the required environment variables:
   ...
   Set them while running chalice.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id chalice
   ```
...
You can configure chalice to use local models through LM Studio.
...
You can configure chalice to use local models through Ollama.
...
ChaliceCode Zen is a list of tested and verified models provided by the ChaliceCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>ChaliceCode Zen</a>** and click **Create API Key**.

2. Run `opencode auth login` and select **ChaliceCode Zen**.
...
3. Enter your ChaliceCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your chalice config.
...
5. You can also customize them through your chalice config. Here's an example of specifying a provider
...
For a limited time, you can use xAI's Grok Code for free with chalice.

:::tip
Grok Code is available for free for a limited time on chalice.
:::

1. Make sure you are on the latest version of chalice.
...
:::tip
You can use any OpenAI-compatible provider with chalice. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need configure it in chalice.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
The `limit` fields allow ChaliceCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
2. For custom providers, check the chalice config and:
   - Make sure the provider ID used in `opencode auth login` matches the ID in your chalice config.
