...
You can use your Azure CLI session instead of an API key. [Install the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli), run `az login`, then run `/connect`, select **Azure**, choose **Microsoft Entra ID (Azure CLI)**, and enter your Resource name. Use `az login --tenant TENANT_ID` if the Resource belongs to a different tenant.
