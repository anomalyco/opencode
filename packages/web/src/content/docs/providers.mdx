...
TrustedRouter is OpenAI-compatible and can be configured as a custom provider.
It uses `https://api.trustedrouter.com/v1` as the API base URL.

1. Create a TrustedRouter API key from the [TrustedRouter console](https://trustedrouter.com/console/keys).

2. Run the `/connect` command, choose **Other**, enter `trustedrouter` as the provider ID, and paste the API key.

   ```txt
   /connect
   ```

3. Add TrustedRouter to your `opencode.json`.

   ```json title="opencode.json" ""trustedrouter""
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "trustedrouter": {
...
4. Run `/models` and select one of the TrustedRouter models.
