---
title: Providers
description: Using any LLM provider in OpenSec.
---

import config from "../../../config.mjs"
export const console = config.console

OpenSec uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenSec config.
...
You can customize the providers through the `provider` section in your OpenSec
config.
...
OpenSec Zen is a list of models provided by the OpenSec team that have been
tested and verified to work well with OpenSec. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenSec Zen.
:::
...
It works like any other provider in OpenSec and is completely optional to use.
...
To use Amazon Bedrock with OpenSec:
...
:::info
Using your Claude Pro/Max subscription in OpenSec is not officially supported by [Anthropic](https://anthropic.com).
:::
...
OpenSec uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenSec
...
To use Google Vertex AI with OpenSec:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each OpenSec conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenSec conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for OpenSec. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with OpenSec:
...
3. Copy the API key for use in OpenSec.
...
6. **Important**: Before using cloud models in OpenSec, you must pull the model information locally:
...
OpenSec Zen is a list of tested and verified models provided by the OpenSec team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenSec Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenSec Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenSec API key.
...
The `limit` fields allow OpenSec to understand how much context you have left. Standard providers pull these from models.dev automatically.
