...
If the key is missing, OpenRouter times out, or Jev returns an invalid route, OpenCode chooses the newest suitable general OpenAI model at medium reasoning effort and labels the message `Jev fallback`. See the [Jev model page](https://openrouter.ai/~typesafe/jev-latest) for OpenRouter model availability and pricing.
