...
Some providers can discover their available models automatically instead of
requiring every model to be listed by hand. Discovered models are merged with
any models you've configured manually, without overwriting them.

GitLab Duo Agent Platform workflow models work this way out of the box —
they're fetched from your GitLab instance instead of being hardcoded.

Any provider that speaks the **OpenAI-compatible** protocol (`npm:
"@ai-sdk/openai-compatible"`, which is also the default for a provider not
otherwise known to OpenCode) can opt into this with `enable_model_discovery`,
as long as `options.baseURL` is also set. OpenCode then calls `GET
{baseURL}/models` (5 second timeout) and expects the standard OpenAI-style
response (`{ "data": [{ "id": "..." }, ...] }`). This means tools like LM
Studio, Ollama, vLLM, or any custom OpenAI-compatible gateway don't need a
`models` section at all:
...
This is opt-in, not automatic, because it means OpenCode makes an outbound
request to whatever `baseURL` you've configured every time providers are
resolved (each time OpenCode starts). Without `enable_model_discovery` set,
nothing changes from today's behavior — you still list `models` by hand.

Models you do list explicitly under `models` always take precedence — discovery
only fills in models you haven't already configured.

GitLab's discovery is a different mechanism (its own built-in integration, not
the generic openai-compatible one above) and stays on by default, since it
already ships that way. You can turn it off with `disable_model_discovery`:
...
If both `enable_model_discovery` and `disable_model_discovery` are set on the
same provider, discovery is skipped.

##### Implementing discovery in your own provider

If you run an OpenAI-compatible inference server or gateway — not just LM
Studio/Ollama, but something like Bifrost, llama-swap, LiteLLM, or your own
proxy — your users can opt into this by setting `enable_model_discovery` and
`options.baseURL` in their config, pointed at your server. There's no
OpenCode-side registration or plugin hook required. To make sure discovery
actually returns useful results, your server needs to:

1. Expose `GET {baseURL}/models` (OpenCode strips a trailing slash from
   `baseURL`, then appends `/models`), responding within 5 seconds.
2. Return HTTP 200 with a JSON body shaped like the standard OpenAI
   `/v1/models` response:

   ```json
   {
     "object": "list",
     "data": [{ "id": "my-model-name" }, { "id": "another-model" }]
   }
   ```

   Only `data[].id` is read today — other fields (`created`, `owned_by`,
   context length, pricing, etc.) are ignored.

3. Accept the same auth OpenCode already sends on chat requests: if
   `options.apiKey` is set, OpenCode sends `Authorization: Bearer <apiKey>`;
   any `options.headers` you've configured are forwarded too.

If your endpoint is slow, unreachable, times out, or returns something
OpenCode can't parse, discovery just fails silently for that provider — it
never blocks startup or the models your users already configured by hand.
...
are dynamically discovered from your GitLab instance. See [Model
discovery](#model-discovery) to turn this off.
...
`models` is optional — llama-server exposes a `/v1/models` endpoint, so
OpenCode can discover the loaded model automatically if you add
`"enable_model_discovery": true`. See [Model discovery](#model-discovery).
...
`models` is optional — LM Studio exposes a `/v1/models` endpoint, so OpenCode
can discover loaded models automatically if you add
`"enable_model_discovery": true`. See [Model discovery](#model-discovery).
...
`models` is optional — Ollama exposes a `/v1/models` endpoint, so OpenCode can
discover pulled models automatically if you add `"enable_model_discovery":
true`. See [Model discovery](#model-discovery).
...
4. Create or update your `opencode.json` file in your project directory:
   ...
   - **models**: Available models. Optional if you set `enable_model_discovery` and the provider exposes a `/v1/models` endpoint — see [Model discovery](#model-discovery).
...
5. Run the `/models` command and your custom provider and models will appear in the selection list. If you set `enable_model_discovery` and skipped `models` above, this list is populated from your provider's `/v1/models` endpoint.
