...
Kilo Gateway provides [free anonymous access](https://kilo.ai/docs/gateway/authentication#anonymous-access),
as well as paid access tied to an account API key. 

1. If using a paid plan, head over to the [profile page](https://app.kilo.ai/profile) to obtain the API key.

2. Run the `/connect` command and search for Kilo Gateway.

   ```txt
   /connect
   ```

3. Enter either the API key for usage tied to account, or a random string for anonymous access.
