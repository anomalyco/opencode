---
title: Providers
description: Using any LLM provider in Octopus.
---

import config from "../../../config.mjs"
export const console = config.console

Octopus uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Octopus config.
...
You can customize the providers through the `provider` section in your Octopus
config.
...
Octopus Zen is a list of models provided by the Octopus team that have been
tested and verified to work well with Octopus. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Octopus Zen.
:::

1. Run the `/connect` command in the TUI, select `Octopus Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Octopus and is completely optional to use.

---

## Octopus Go

Octopus Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Octopus team that have been
tested and verified to work well with Octopus.

1. Run the `/connect` command in the TUI, select `Octopus Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Octopus:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `octopus.json`:
...
Octopus. Anthropic explicitly prohibits this.

Previous versions of Octopus came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Octopus with zero setup:
...
GitLab Duo support in Octopus is experimental. Features, configuration, and
behavior may change in future releases.
:::

Octopus integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Octopus uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Octopus
to only use your own GitLab-hosted instance, add the following to your
`octopus.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Octopus will:
...
4. Route tool execution requests through Octopus's permission-gated tool system
...
To use Google Vertex AI with Octopus:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Octopus conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Octopus conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Octopus. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Octopus:
...
3. Copy the API key for use in Octopus.
...
6. **Important**: Before using cloud models in Octopus, you must pull the model information locally:
...
Octopus Zen is a list of tested and verified models provided by the Octopus team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Octopus Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Octopus Zen**.

   ```txt
   /connect
   ```

3. Enter your Octopus API key.
...
4. Create or update your `octopus.json` file in your project directory:
...
The `limit` fields allow Octopus to understand how much context you have left. Standard providers pull these from models.dev automatically.
