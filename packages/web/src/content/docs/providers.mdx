...
1. **Stored credentials**: Workspace URL + PAT saved via `/connect`
2. **Stored Databricks profile**: Profile selected in `/connect` and saved in `auth.json`
3. **Configuration file**: Host/profile from `opencode.json` config
4. **Environment variables**: `DATABRICKS_HOST`, `DATABRICKS_TOKEN`, or `DATABRICKS_CONFIG_PROFILE`
5. **Databricks CLI profile**: Host from `~/.databrickscfg` and token from `~/.databricks/token-cache.json`
6. **OAuth M2M**: Set `DATABRICKS_CLIENT_ID` and `DATABRICKS_CLIENT_SECRET` environment variables
7. **Azure AD Service Principal**: Set `ARM_CLIENT_ID`, `ARM_CLIENT_SECRET`, and `ARM_TENANT_ID` environment variables
8. **Azure CLI**: Automatically used for Azure Databricks workspaces when logged in with `az login`
...
When you choose **Databricks** in `/connect`, OpenCode prompts you to pick a Databricks CLI profile first.
The selected profile is saved to OpenCode `auth.json`, and OpenCode then uses Databricks SDK resolution for that profile.
If profile-based SDK auth is unavailable, `/connect` falls back to workspace host + PAT prompts.
