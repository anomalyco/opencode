---
title: Providers
description: Using any LLM provider in Pencode.
---

import config from "../../../config.mjs"
export const console = config.console

Pencode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Pencode config.
...
in `~/.local/share/pencode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Pencode
config.
...
Pencode Zen is a list of models provided by the Pencode team that have been
tested and verified to work well with Pencode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Pencode Zen.
:::

1. Run the `/connect` command in the TUI, select `Pencode Zen`, and head to [pencode.ai/auth](https://pencode.ai/zen).
...
It works like any other provider in Pencode and is completely optional to use.

---

## Pencode Go

Pencode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Pencode team that have been
tested and verified to work well with Pencode.

1. Run the `/connect` command in the TUI, select `Pencode Go`, and head to [pencode.ai/auth](https://pencode.ai/zen).
...
To use Amazon Bedrock with Pencode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running pencode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `pencode.json`:
...
Pencode. Anthropic explicitly prohibits this.

Previous versions of Pencode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Pencode with zero setup:
...
You can configure pencode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for pencode to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your pencode config.
...
Pencode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Pencode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Pencode. Sign in and approve.

   :::note
   Pencode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Pencode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Pencode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Pencode is experimental. Features, configuration, and
behavior may change in future releases.
:::

Pencode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Pencode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Pencode
to only use your own GitLab-hosted instance, add the following to your
`pencode.json` file. It is also recommended to disable session sharing.
...
More documentation on [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, Pencode will:
...
4. Route tool execution requests through Pencode's permission-gated tool system
...
To use your GitHub Copilot subscription with pencode:
...
To use Google Vertex AI with Pencode:
...
2. Set the required environment variables:
   ...
   Set them while running pencode.
...
In the event you see a feature or model from Helicone that isn't configured automatically through pencode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session) plugin to automatically log each Pencode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Pencode conversation listed as a separate session.
...
You can configure pencode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure pencode to use local models through LM Studio.
...
You can configure pencode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Pencode. See the [Ollama integration docs](https://docs.ollama.com/integrations/pencode) for details.
:::
...
To use Ollama Cloud with Pencode:
...
3. Copy the API key for use in Pencode.
...
6. **Important**: Before using cloud models in Pencode, you must pull the model information locally:
...
Pencode Zen is a list of tested and verified models provided by the Pencode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Pencode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Pencode Zen**.

   ```txt
   /connect
   ```

3. Enter your Pencode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your pencode config.
...
5. You can also customize them through your pencode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Pencode:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=pencode&utm_medium=docs&utm_campaign=cortex-provider).

pencode's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Pencode captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting pencode:
...
The model catalog is provided automatically. A minimal `pencode.json` is all that's needed:
...
You can also customize models through your pencode config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Pencode opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Pencode. Pencode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Pencode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with pencode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `pencode.json` file in your project directory:
...
The `limit` fields allow Pencode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `pencode auth list` to see if the credentials
...
2. For custom providers, check the pencode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your pencode config.
