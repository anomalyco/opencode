---
title: Providers
description: Using any LLM provider in Leak Code.
---

import config from "../../../config.mjs"
export const console = config.console

Leak Code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Leak Code config.
...
in `~/.local/share/leak-code/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Leak Code
config.
...
OpenCode Zen is a list of models provided by the Leak Code team that have been
tested and verified to work well with Leak Code. [Learn more](/docs/zen).
...
It works like any other provider in Leak Code and is completely optional to use.

---

## OpenCode Go

OpenCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Leak Code team that have been
tested and verified to work well with Leak Code.
...
To use Amazon Bedrock with Leak Code:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running leak-code:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY leak-code

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile leak-code

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX leak-code
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `leak-code.json`:
...
Leak Code. Anthropic explicitly prohibits this.

Previous versions of Leak Code came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Leak Code with zero setup:
...
You can configure leak-code to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for leak-code to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX leak-code
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX leak-code
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your leak-code config.
...
Leak Code supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Leak Code uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Leak Code. Sign in and approve.

   :::note
   Leak Code requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Leak Code surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Leak Code forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Leak Code is experimental. Features, configuration, and
behavior may change in future releases.
:::

Leak Code integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Leak Code uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Leak Code
to only use your own GitLab-hosted instance, add the following to your
`leak-code.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Leak Code will:
...
4. Route tool execution requests through Leak Code's permission-gated tool system
...
To use your GitHub Copilot subscription with leak-code:
...
To use GMI Cloud with Leak Code:
...
To use Google Vertex AI with Leak Code:
...
2. Set the required environment variables:
   ...
   Set them while running leak-code.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id leak-code
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through leak-code, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Leak Code conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Leak Code conversation listed as a separate session.
...
You can configure leak-code to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure leak-code to use local models through LM Studio.
...
You can configure leak-code to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Leak Code. See the [Ollama integration docs](https://docs.ollama.com/integrations/leak-code) for details.
:::
...
To use Ollama Cloud with Leak Code:
...
3. Copy the API key for use in Leak Code.
...
6. **Important**: Before using cloud models in Leak Code, you must pull the model information locally:
...
OpenCode Zen is a list of tested and verified models provided by the Leak Code team. [Learn more](/docs/zen).
...
3. Enter your Leak Code API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your leak-code config.
...
5. You can also customize them through your leak-code config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' leak-code
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group leak-code
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=leak-code&utm_medium=docs&utm_campaign=cortex-provider).

leak-code's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Leak Code captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting leak-code:
...
The model catalog is provided automatically. A minimal `leak-code.json` is all that's needed:
...
You can also customize models through your leak-code config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Leak Code opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Leak Code. Leak Code polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Leak Code refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with leak-code. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `leak-code.json` file in your project directory:
...
The `limit` fields allow Leak Code to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `leak-code auth list` to see if the credentials
...
2. For custom providers, check the leak-code config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your leak-code config.
