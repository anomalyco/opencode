...
To use Modal's OpenAI-compatible endpoint with OpenCode:

1. Head over to [Modal GLM-5 endpoint](https://modal.com/glm-5-endpoint) and generate your free API Token.

2. Run the `/connect` command and search for **Modal**.

   ```txt
   /connect
   ```

3. Enter your Modal API key.
...
4. Run the `/models` command to select from the models available to your Modal key.
...
:::tip
Modal uses an OpenAI-compatible base URL. The default configured in OpenCode is `https://api.us-west-2.modal.direct/v1`.
:::
