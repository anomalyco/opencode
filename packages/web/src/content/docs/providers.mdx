...
For `type: "api"` auth entries, you can also include an optional `options`
object. These values are merged into the provider's runtime options.
...
This is useful when a provider needs connection settings in addition to an API
key, such as a proxy `baseURL`, custom headers, or mTLS files.

For example, if you run Anthropic through Azure Foundry or another enterprise
gateway that requires client certificates, you can keep the API key and
connection-specific options together in `auth.json`:
...
```json title="~/.local/share/opencode/auth.json"
{
  "my-anthropic-proxy": {
    ...
    "key": "your-api-key",
    "options": {
      "baseURL": "https://fabric.example.com/openai-proxy-api/claude/v1",
      "tls": {
...
Supported `options` fields depend on the provider, but common ones include:

- `baseURL` - Override the provider endpoint.
- `headers` - Add custom request headers.
- `timeout` - Override request timeout in milliseconds, or set `false` to disable it.
- `tls.key` - Path to a client private key PEM file.
- `tls.cert` - Path to a client certificate PEM file.
- `tls.ca` - Path to a CA PEM file, or an array of CA file paths.

:::note
`auth.json` can override how an existing provider connects, but it does not
define a brand new provider by itself. For a new custom provider ID, define the
provider and model list in `opencode.json`, then put secrets and runtime
connection overrides in `auth.json`.
:::
...
4. Create or update your `opencode.json` file in your project directory:
   ...
   If you prefer, you can keep the API key and connection-specific runtime
   options in `~/.local/share/opencode/auth.json` instead of `opencode.json`.
   This is useful for endpoints that require mTLS or per-user credentials.

   ```json title="~/.local/share/opencode/auth.json" ""myprovider""
   {
     "myprovider": {
       "type": "api",
       "key": "your-api-key",
   ...
   In this split:
   - `opencode.json` defines the provider ID, SDK package, and model list.
   - `auth.json` stores credentials and connection-specific overrides.
...
2. For custom providers, check the opencode config and:
   ...
   - If the provider requires mTLS, make sure `auth.json` points to readable `tls.key`, `tls.cert`, and `tls.ca` files.
