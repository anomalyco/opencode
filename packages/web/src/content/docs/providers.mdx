...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [crazycode-helicone-session](https://github.com/H2Shami/crazycode-helicone-session) plugin to automatically log each CrazyCode conversation as a session in Helicone.

```bash
npm install -g crazycode-helicone-session
```
...
```json title="crazycode.json"
{
  "plugin": ["crazycode-helicone-session"]
}
```
