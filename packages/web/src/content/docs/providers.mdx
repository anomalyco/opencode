...
1. Head over to the [xKiro dashboard](https://xkiro.com/dashboard), create an account, and generate an API key.

2. Run the `/connect` command and search for **xKiro**.

   ```txt
   /connect
   ```

3. Enter your xKiro API key.
...
4. Run the `/models` command to select a model.
   ...
   xKiro offers free-tier models alongside paid ones — see their [opencode guide](https://docs.xkiro.com/guides/tools/opencode/) for pinning specific model IDs via a custom provider config.
