...
Models are discovered at runtime (they are not in models.dev). To tune context/output budgeting and pass backend-specific fields, override per model under `provider.openwebui.models` in `opencode.json` (or `opencode.jsonc`). `limit.context` / `limit.output` control OpenCode compaction and max output tokens; `options` are merged into requests for that model (e.g. Ollama-style fields when your stack supports them).
