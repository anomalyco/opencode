...
You can quickly add a custom provider using the `opencode provider add` command, which handles both the API key storage and config file updates in one step:

```bash
opencode provider add <id> \
  --name "Provider Name" \
  --url "https://api.provider.com/v1" \
  --key "your-api-key" \
  --models "model-id:'Model Display Name'"
```

For example, to add a local Ollama instance:
...
opencode provider add ollama_local \
  ...
  --key "ollama" \
  --models "llama3:'Llama 3 8B',codegemma:'Code Gemma 7B'"
```

See the [CLI documentation](/docs/cli#provider) for full details and more examples.

### Manual Configuration

Alternatively, you can manually configure a custom provider:
