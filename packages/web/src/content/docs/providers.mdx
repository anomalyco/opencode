...
2. **Configure authentication** using one of the following methods:
   ...
   **Available options:**
   ...
   This works with browser-based SSO flows - the command output is shown so you can complete authentication.

   The refresh command must start with `aws` or be an absolute path. Includes timeout (2 min) and cooldown (1 min) to prevent issues.
