...
For easier setup, you can use Google Vertex Express Mode with an API key.

1. Generate a Vertex Express API key in the Google Cloud Console
2. Set the `GOOGLE_VERTEX_API_KEY` environment variable:

   ```bash title="~/.bash_profile"
   export GOOGLE_VERTEX_API_KEY="your-express-api-key"
   ```

You don't need to set GOOGLE_CLOUD_LOCATION or GOOGLE_CLOUD_PROJECT for Vertex
Express.

Or run the `/connect` command and search for Vertex.
...
3. Run the `/models` command to select the one you want.
...
:::note
Express Mode uses the global endpoint for better performance and availability. Use standard Vertex AI for production workloads requiring regional data residency.
:::

For more information, see the [Google Vertex AI Express documentation](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#express-mode).
