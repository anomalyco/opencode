...
The picker also includes **Jev OpenAI Auto** for ChatGPT Plus/Pro OAuth. It asks Jev to choose one of the currently available OpenAI models and its reasoning effort from the first prompt, then keeps that route for the rest of the session. The transcript shows the selected model, effort, and whether Jev used the normal route or a fallback.
