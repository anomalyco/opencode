...
Vercel AI Gateway lets you access models from OpenAI, Anthropic, Google, SpaceXAI, and more through a unified endpoint. Models are offered at list price with no markup.
...
SpaceXAI is the rebranded xAI provider for Grok models. OpenCode keeps the provider ID, SDK package, API key environment variable, and API endpoint as `xai`, `@ai-sdk/xai`, `XAI_API_KEY`, and `https://api.x.ai/v1` because SpaceXAI's developer platform still uses those technical names.

Three ways to authenticate: a SuperGrok subscription via browser OAuth, the same SuperGrok subscription via a headless device-code flow (for VPS / SSH / Docker), or a pay-as-you-go API key from the SpaceXAI console.

#### Option A — SuperGrok OAuth (browser login)

1. Run the `/connect` command and search for **SpaceXAI**.

   ```txt
   /connect
   ```

2. Select **SpaceXAI Grok OAuth (SuperGrok Subscription)**. OpenCode opens SpaceXAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Use this when OpenCode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running OpenCode — instead SpaceXAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).

1. Run the `/connect` command on the remote host and search for **SpaceXAI**.

   ```txt
   /connect
   ```

2. Select **SpaceXAI Grok OAuth (Headless / Remote / VPS)**. OpenCode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. OpenCode polls SpaceXAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.

#### Option C — API key

1. Head over to the [SpaceXAI console](https://console.x.ai/), create an account, and generate an API key.

2. Run the `/connect` command and search for **SpaceXAI**.

   ```txt
   /connect
   ```

3. Select **Manually enter API Key** and paste your SpaceXAI API key.
