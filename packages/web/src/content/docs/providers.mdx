...
1. Generate a bearer token for Snowflake Cortex (for example: [OAuth token](https://docs.snowflake.com/en/developer-guide/snowflake-rest-api/authentication) or [Programmatic Access Token (PAT)](https://docs.snowflake.com/en/user-guide/programmatic-access-tokens)).
...
3. Enter your [account identifier](https://docs.snowflake.com/en/user-guide/admin-account-identifier) when prompted (required), for example `myorg-myaccount` or `xy12345.us-east-1`.
...
4. Choose your login method:
   ...
   - **Login with Snowflake (External Browser)** *(recommended)*: opens your browser, signs in with your IdP/SSO, and stores the OAuth bearer token automatically.
   - **Paste bearer token manually**: paste an OAuth token, JWT, or PAT.
...
5. Enter your [account identifier](https://docs.snowflake.com/en/user-guide/admin-account-identifier) and optional role scope.
...
export SNOWFLAKE_CORTEX_TOKEN=your-token
```

`SNOWFLAKE_CORTEX_PAT` is still supported for backward compatibility.
