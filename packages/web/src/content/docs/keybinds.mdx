...
On macOS, the default keybinds automatically swap `ctrl` and `cmd` (super). For example, `leader` is `cmd+x` on macOS and `ctrl+x` on other platforms.
...
| -------- | -------------------------------------------------- |
| `ctrl`   | Control key                                       |
| `alt`    | Option (Alt) key - alias: `meta`, `option`     |
| `super`  | Windows key on Windows, **cmd** on macOS - alias: `cmd`, `command` |
| `shift`  | Shift key                                       |

You can use:
- `super`, `cmd`, or `command` interchangeably
- `alt`, `meta`, or `option` interchangeably
