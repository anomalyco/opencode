...
:::note
هذه الأداة متاحة فقط عند ضبط `LEAKCODE_EXPERIMENTAL_LSP_TOOL=true` (أو `LEAKCODE_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `LEAKCODE_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
