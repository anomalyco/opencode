...
:::note
هذه الأداة متاحة فقط عند ضبط `IMECODE_EXPERIMENTAL_LSP_TOOL=true` (أو `IMECODE_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `IMECODE_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
