...
تتيح مهارات الوكيل لـ CyberStrike اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث CyberStrike في هذه المواقع:

- إعدادات المشروع: `.cyberstrike/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج CyberStrike صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.cyberstrike/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/cyberstrike/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.cyberstrike/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض CyberStrike المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `cyberstrike.json`:
...
**للوكلاء المدمجين** (في `cyberstrike.json`):
