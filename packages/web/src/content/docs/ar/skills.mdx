...
- إعدادات المشروع: `.leak-code/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/leak-code/skills/<name>/SKILL.md`
...
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.leak-code/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/leak-code/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.leak-code/skills/git-release/SKILL.md` بهذا الشكل:
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `leak-code.json`:
...
**للوكلاء المدمجين** (في `leak-code.json`):
