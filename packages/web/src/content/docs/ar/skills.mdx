...
تتيح مهارات الوكيل لـ Octopus اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Octopus في هذه المواقع:

- إعدادات المشروع: `.octopus/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/octopus/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Octopus صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.octopus/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/octopus/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.octopus/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Octopus المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `octopus.json`:
...
**للوكلاء المدمجين** (في `octopus.json`):
