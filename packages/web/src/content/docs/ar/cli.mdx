---
title: CLI
description: أوامر وخيارات CLI في xcsh.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ xcsh افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
xcsh
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع xcsh برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ xcsh.
...
يتضمن CLI الخاص بـ xcsh أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ xcsh قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
xcsh attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ xcsh تعمل عن بعد. على سبيل المثال:
...
يعتمد xcsh على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `xcsh auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

عند تشغيل xcsh يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في xcsh.
...
تشغيل xcsh في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `xcsh serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| `--attach`   |         | الإرفاق بخادم xcsh قيد التشغيل (مثل http://localhost:4096)    |
...
بدء خادم xcsh بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
xcsh serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف xcsh دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `xcsh`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات xcsh لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة xcsh.

```bash
xcsh import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة xcsh.
...
بدء خادم xcsh بدون واجهة مع واجهة ويب.

```bash
xcsh web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى xcsh عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `xcsh`).
...
إلغاء تثبيت xcsh وإزالة جميع الملفات ذات الصلة.
...
تحديث xcsh إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر xcsh الخيارات العامة التالية.
...
يمكن تهيئة xcsh باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `xcsh`) |
