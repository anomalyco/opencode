---
title: CLI
description: أوامر وخيارات CLI في Octopus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Octopus افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opencode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Octopus برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Octopus.
...
يتضمن CLI الخاص بـ Octopus أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Octopus قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Octopus تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OCTOPUS_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OCTOPUS_SERVER_USERNAME` أو `opencode`) |
...
يعتمد Octopus على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `opencode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/octopus/auth.json`.

```bash
opencode auth login
```

عند تشغيل Octopus يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Octopus.
...
بدء خادم Octopus بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
opencode serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف opencode دون واجهة TUI. اضبط `OCTOPUS_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Octopus لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Octopus.

```bash
opencode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Octopus.
...
بدء خادم Octopus بدون واجهة مع واجهة ويب.

```bash
opencode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Octopus عبر واجهة ويب. اضبط `OCTOPUS_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
إلغاء تثبيت Octopus وإزالة جميع الملفات ذات الصلة.
...
يمكن تهيئة Octopus باستخدام متغيرات البيئة.

| المتغير                              | النوع   | الوصف                                                     |
| ------------------------------------ | ------- | --------------------------------------------------------- |
| `OCTOPUS_AUTO_SHARE`                 | boolean | مشاركة الجلسات تلقائيا                                    |
| `OCTOPUS_GIT_BASH_PATH`              | string  | مسار ملف Git Bash التنفيذي على Windows                    |
| `OCTOPUS_CONFIG`                     | string  | مسار ملف التهيئة                                          |
| `OCTOPUS_CONFIG_DIR`                 | string  | مسار دليل التهيئة                                         |
| `OCTOPUS_CONFIG_CONTENT`             | string  | محتوى تهيئة JSON مُضمّن                                   |
| `OCTOPUS_DISABLE_AUTOUPDATE`         | boolean | تعطيل التحقق التلقائي من التحديثات                        |
| `OCTOPUS_DISABLE_PRUNE`              | boolean | تعطيل تنقية البيانات القديمة                              |
| `OCTOPUS_DISABLE_TERMINAL_TITLE`     | boolean | تعطيل تحديث عنوان terminal تلقائيا                        |
| `OCTOPUS_PERMISSION`                 | string  | تهيئة أذونات JSON مُضمّنة                                 |
| `OCTOPUS_DISABLE_DEFAULT_PLUGINS`    | boolean | تعطيل الإضافات الافتراضية                                 |
| `OCTOPUS_DISABLE_LSP_DOWNLOAD`       | boolean | تعطيل تنزيل خوادم LSP تلقائيا                             |
| `OCTOPUS_ENABLE_EXPERIMENTAL_MODELS` | boolean | تفعيل النماذج التجريبية                                   |
| `OCTOPUS_DISABLE_AUTOCOMPACT`        | boolean | تعطيل ضغط السياق تلقائيا                                  |
| `OCTOPUS_DISABLE_CLAUDE_CODE`        | boolean | تعطيل القراءة من `.claude` (الموجه + المهارات)            |
| `OCTOPUS_DISABLE_CLAUDE_CODE_PROMPT` | boolean | تعطيل قراءة `~/.claude/CLAUDE.md`                         |
| `OCTOPUS_DISABLE_CLAUDE_CODE_SKILLS` | boolean | تعطيل تحميل `.claude/skills`                              |
| `OCTOPUS_DISABLE_MODELS_FETCH`       | boolean | تعطيل جلب النماذج من مصادر بعيدة                          |
| `OCTOPUS_FAKE_VCS`                   | string  | مزود VCS وهمي لأغراض الاختبار                             |
| `OCTOPUS_CLIENT`                     | string  | معرّف العميل (الافتراضي `cli`)                            |
| `OCTOPUS_ENABLE_EXA`                 | boolean | تفعيل أدوات بحث الويب من Exa                              |
| `OCTOPUS_SERVER_PASSWORD`            | string  | تفعيل المصادقة الأساسية لخادمي `serve`/`web`              |
| `OCTOPUS_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `opencode`) |
| `OCTOPUS_MODELS_URL`                 | string  | رابط مخصص لجلب تهيئة النماذج                              |
...
| المتغير                                        | النوع   | الوصف                                       |
| ---------------------------------------------- | ------- | ------------------------------------------- |
| `OCTOPUS_EXPERIMENTAL`                         | boolean | تفعيل جميع الميزات التجريبية                |
| `OCTOPUS_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | تفعيل اكتشاف الأيقونات                      |
| `OCTOPUS_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | تعطيل النسخ عند التحديد في TUI              |
| `OCTOPUS_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | المهلة الافتراضية لأوامر bash بالميلي ثانية |
| `OCTOPUS_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | الحد الأقصى لرموز المخرجات لاستجابات LLM    |
| `OCTOPUS_EXPERIMENTAL_FILEWATCHER`             | boolean | تفعيل مراقب الملفات لكامل الدليل            |
| `OCTOPUS_EXPERIMENTAL_OXFMT`                   | boolean | تفعيل مُنسّق oxfmt                          |
| `OCTOPUS_EXPERIMENTAL_LSP_TOOL`                | boolean | تفعيل أداة LSP تجريبية                      |
| `OCTOPUS_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | تعطيل مراقب الملفات                         |
| `OCTOPUS_EXPERIMENTAL_EXA`                     | boolean | تفعيل ميزات Exa التجريبية                   |
| `OCTOPUS_EXPERIMENTAL_LSP_TY`                  | boolean | تمكين TY LSP لملفات python                  |
| `OCTOPUS_EXPERIMENTAL_MARKDOWN`                | boolean | تفعيل ميزات markdown تجريبية                |
| `OCTOPUS_EXPERIMENTAL_PLAN_MODE`               | boolean | تفعيل وضع الخطة                             |
