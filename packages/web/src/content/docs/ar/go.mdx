...
3. يرسل معرّف جلسة ثابتًا في `x-opencode-session` لكل محادثة حتى نتمكن من تحسين التوجيه
   ...
   ويعيد استخدام المعرّف نفسه في كل طلبات المحادثة، بما فيها الطلبات المساعدة، بدلًا من إنشاء معرّف لكل طلب أو لكل
   عملية، ويرسله على كل نقاط النهاية التي يستخدمها العميل (`/responses` و`/chat/completions` و`/messages`)؛
   فالترويسة المضافة في وسيط SDK واحد فقط لا تصل إلى البقية.
