---
title: المزوّدون
description: استخدام أي موفّر LLM في xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم xcsh ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات xcsh.
...
في `~/.local/share/xcsh/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات xcsh.
...
xcsh Zen هي قائمة نماذج يوفّرها فريق xcsh وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع xcsh. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع xcsh Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر xcsh، ثم انتقل إلى [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/auth).
...
يعمل مثل أي مزوّد آخر في xcsh واستخدامه اختياري بالكامل.

---

## xcsh Go

xcsh Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق xcsh، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع xcsh.

1. شغّل الأمر `/connect` في TUI، واختر `xcsh Go`، ثم انتقل إلى [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
لاستخدام Amazon Bedrock مع xcsh:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل xcsh:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `xcsh.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل xcsh غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل xcsh بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات xcsh.
...
تخزين الرمز في مساحة تخزين مصادقة xcsh.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم xcsh نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر xcsh
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`xcsh.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع xcsh:
...
لاستخدام Google Vertex AI مع xcsh:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر xcsh، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) لتسجيل كل محادثة xcsh تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة xcsh مدرجة كجلسة مستقلة.
...
يمكنك ضبط xcsh لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط xcsh لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط xcsh لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع xcsh. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/xcsh) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في xcsh.
...
6. **مهم**: قبل استخدام نماذج السحابة في xcsh، يجب سحب معلومات النموذج محليا:
...
xcsh Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق xcsh. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>xcsh Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **xcsh Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ xcsh.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات xcsh.
...
5. يمكنك أيضا تخصيصها عبر إعدادات xcsh. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات xcsh. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع xcsh. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `xcsh.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ xcsh بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `xcsh auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات xcsh وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات xcsh.
