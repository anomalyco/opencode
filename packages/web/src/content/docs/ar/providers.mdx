---
title: المزوّدون
description: استخدام أي موفّر LLM في Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Octopus ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Octopus.
...
في `~/.local/share/octopus/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Octopus.
...
Octopus Zen هي قائمة نماذج يوفّرها فريق Octopus وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Octopus. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Octopus Zen.
:::
...
يعمل مثل أي مزوّد آخر في Octopus واستخدامه اختياري بالكامل.

---

## Octopus Go

Octopus Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Octopus، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Octopus.

1. شغّل الأمر `/connect` في TUI، واختر `Octopus Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Octopus:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `octopus.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Octopus غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم Octopus نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Octopus
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`octopus.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام Google Vertex AI مع Octopus:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Octopus تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Octopus مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Octopus. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Octopus:
...
3. انسخ مفتاح API لاستخدامه في Octopus.
...
6. **مهم**: قبل استخدام نماذج السحابة في Octopus، يجب سحب معلومات النموذج محليا:
...
Octopus Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Octopus. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Octopus Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Octopus Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Octopus.
...
4. أنشئ أو حدّث ملف `octopus.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Octopus بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
