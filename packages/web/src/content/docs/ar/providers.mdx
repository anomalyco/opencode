...
في `~/.local/share/leak-code/auth.json`.
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `leak-code.json`:
...
`leak-code.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
4. أنشئ أو حدّث ملف `leak-code.json` في دليل مشروعك:
...
1. **تحقق من إعداد auth**: شغّل `leak-code auth list` لمعرفة ما إذا كانت بيانات الاعتماد
