---
title: المزوّدون
description: استخدام أي موفّر LLM في CyberStrike.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CyberStrike ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CyberStrike.
...
في `~/.local/share/cyberstrike/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CyberStrike.
...
CyberStrike Zen هي قائمة نماذج يوفّرها فريق CyberStrike وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CyberStrike. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CyberStrike Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر cyberstrike، ثم انتقل إلى [cyberstrike.io/auth](https://cyberstrike.io/auth).
...
يعمل مثل أي مزوّد آخر في CyberStrike واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع CyberStrike:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل cyberstrike:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `cyberstrike.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CyberStrike غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل cyberstrike بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات cyberstrike.
...
تخزين الرمز في مساحة تخزين مصادقة cyberstrike.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم CyberStrike نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CyberStrike
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`cyberstrike.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع cyberstrike:
...
لاستخدام Google Vertex AI مع CyberStrike:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل cyberstrike.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر cyberstrike، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) لتسجيل كل محادثة CyberStrike تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CyberStrike مدرجة كجلسة مستقلة.
...
يمكنك ضبط cyberstrike لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط cyberstrike لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط cyberstrike لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CyberStrike. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/cyberstrike) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CyberStrike:
...
3. انسخ مفتاح API لاستخدامه في CyberStrike.
...
6. **مهم**: قبل استخدام نماذج السحابة في CyberStrike، يجب سحب معلومات النموذج محليا:
...
CyberStrike Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CyberStrike. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CyberStrike Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CyberStrike Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CyberStrike.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات cyberstrike.
...
5. يمكنك أيضا تخصيصها عبر إعدادات cyberstrike. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Cyberstrike:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات cyberstrike. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع cyberstrike. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `cyberstrike.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ CyberStrike بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `cyberstrike auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات cyberstrike وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات cyberstrike.
