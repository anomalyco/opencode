---
title: المزوّدون
description: استخدام أي موفّر LLM في Pencode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Pencode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Pencode.
...
في `~/.local/share/pencode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Pencode.
...
Pencode Zen هي قائمة نماذج يوفّرها فريق Pencode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Pencode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Pencode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر pencode، ثم انتقل إلى [pencode.ai/auth](https://pencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Pencode واستخدامه اختياري بالكامل.

---

## Pencode Go

Pencode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Pencode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Pencode.

1. شغّل الأمر `/connect` في TUI، واختر `Pencode Go`، ثم انتقل إلى [pencode.ai/auth](https://pencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Pencode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل pencode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `pencode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Pencode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين pencode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل pencode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات pencode.
...
تخزين الرمز في مساحة تخزين مصادقة pencode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Pencode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Pencode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`pencode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth).

##### التهيئة

خصّص عبر `pencode.json`:
...
لاستخدام اشتراك GitHub Copilot مع pencode:
...
لاستخدام Google Vertex AI مع Pencode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر pencode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session) لتسجيل كل محادثة Pencode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Pencode مدرجة كجلسة مستقلة.
...
يمكنك ضبط pencode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط pencode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط pencode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Pencode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/pencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Pencode:
...
3. انسخ مفتاح API لاستخدامه في Pencode.
...
6. **مهم**: قبل استخدام نماذج السحابة في Pencode، يجب سحب معلومات النموذج محليا:
...
Pencode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Pencode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Pencode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Pencode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Pencode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات pencode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات pencode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Pencode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات pencode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع pencode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `pencode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Pencode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `pencode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات pencode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات pencode.
