...
- `bash:unsandboxed` — إعادة تشغيل أمر shell خارج البيئة المعزولة بعد الرفض أو بعد طلب صريح لتشغيله بدون عزل
...
## التفاعل مع البيئة المعزولة

عند تفعيل البيئة المعزولة على macOS، يمكن أن يؤدي حظر أمر bash إلى تشغيل طلب إذن منفصل `bash:unsandboxed`.
يحدث ذلك عندما يكتشف OpenCode رفضًا محتملًا من البيئة المعزولة، أو عندما يطلب الأمر صراحةً تخطي البيئة المعزولة عبر `# opencode:unsandboxed <reason>` في أول سطر غير فارغ.
اضبط البيئة المعزولة نفسها في [`experimental.sandbox`](/docs/config#sandbox).
