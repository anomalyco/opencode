...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في pencode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في pencode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.pencode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/pencode/tools/`.
