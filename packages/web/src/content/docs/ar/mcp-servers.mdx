...
يمكنك إضافة أدوات خارجية إلى CyberStrike باستخدام _Model Context Protocol_، أو MCP. يدعم CyberStrike خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات CyberStrike](https://cyberstrike.io/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/cyberstrike` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى CyberStrike تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم CyberStrike بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك CyberStrike المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `cyberstrike mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن CyberStrike الرموز بشكل آمن في `~/.local/share/cyberstrike/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل CyberStrike إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات CyberStrike مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط CyberStrike بحساب Sentry الخاص بك.
