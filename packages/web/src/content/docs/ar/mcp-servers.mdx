...
يمكنك إضافة أدوات خارجية إلى xcsh باستخدام _Model Context Protocol_، أو MCP. يدعم xcsh خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات xcsh](https://f5xc-salesdemos.github.io/xcsh/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/xcsh` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى xcsh تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم xcsh بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك xcsh المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `xcsh mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن xcsh الرموز بشكل آمن في `~/.local/share/xcsh/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل xcsh إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات xcsh مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط xcsh بحساب Sentry الخاص بك.
