...
أنشئ الملف `.xcsh/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات xcsh أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ xcsh:
...
- على مستوى النظام: `~/.config/xcsh/commands/`
- لكل مشروع: `.xcsh/commands/`
...
يتضمن xcsh عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
