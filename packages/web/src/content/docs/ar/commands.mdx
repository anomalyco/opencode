...
تُعدّ الأوامر المخصصة إضافةً إلى الأوامر المضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`. [اعرف المزيد](./tui#commands).
...
استخدم خيار `command` في [config](./config) الخاص بـ OpenCode:
...
استخدم _!`command`_ لحقن خرج [أمر bash](./tui#bash-commands) داخل مُطالبتك.
...
استخدم إعداد `agent` لتحديد أي [agent](./agents) ينبغي أن ينفّذ هذا الأمر (اختياريًا).
إذا كان هذا [subagent](./agents/#subagents)، فسيؤدي الأمر افتراضيًا إلى تشغيل استدعاء subagent.
...
استخدم القيمة المنطقية `subtask` لفرض أن يُشغِّل الأمر استدعاء [subagent](./agents/#subagents).
يفيد هذا إذا أردت ألا يلوّث الأمر السياق الأساسي لديك، إذ سيُجبر (**force**) الـ agent على العمل كـ subagent،
حتى لو كان `mode` مضبوطًا على `primary` في إعدادات [agent](./agents).
...
يتضمن opencode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](./tui#commands).
