...
أنشئ الملف `.pencode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Pencode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Pencode:
...
- على مستوى النظام: `~/.config/pencode/commands/`
- لكل مشروع: `.pencode/commands/`
...
يتضمن pencode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
