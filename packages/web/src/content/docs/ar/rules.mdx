...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/octopus/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات opencode.
...
للمستخدمين الذين ينتقلون من Claude Code، يدعم Octopus اصطلاحات ملفات Claude Code كبدائل احتياطية:

- **قواعد المشروع**: `CLAUDE.md` في دليل مشروعك (يُستخدم إذا لم يوجد `AGENTS.md`)
- **قواعد عامة**: `~/.claude/CLAUDE.md` (يُستخدم إذا لم يوجد `~/.config/octopus/AGENTS.md`)
...
2. **ملف عام** في `~/.config/octopus/AGENTS.md`
3. **ملف Claude Code** في `~/.claude/CLAUDE.md` (ما لم يتم تعطيله)

يتم اعتماد أول ملف مطابق ضمن كل فئة. على سبيل المثال، إذا كان لديك كل من `AGENTS.md` و`CLAUDE.md` فسيتم استخدام `AGENTS.md` فقط. وبالمثل، تكون الأولوية لملف `~/.config/octopus/AGENTS.md` على `~/.claude/CLAUDE.md`.

---

## تعليمات مخصصة

يمكنك تحديد ملفات تعليمات مخصصة في `octopus.json` أو في الملف العام `~/.config/octopus/octopus.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام octopus.json

النهج الموصى به هو استخدام الحقل `instructions` في `octopus.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `octopus.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
