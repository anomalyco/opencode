...
تعرّف على المزيد حول [المزوّدين](./providers).
...
المعرّف الكامل هنا يكون بالشكل `provider_id/model_id`. على سبيل المثال، إذا كنت تستخدم [OpenCode Zen](./zen) فستستخدم `opencode/gpt-5.1-codex` لنموذج GPT 5.1 Codex.

إذا كنت قد هيّأت [مزوّداً مخصّصاً](./providers#custom)، فإن `provider_id` هو المفتاح الموجود ضمن قسم `provider` في إعداداتك، و`model_id` هو المفتاح الموجود ضمن `provider.models`.
...
يمكنك أيضاً ضبط هذه الخيارات لأي وكلاء تستخدمهم. تتجاوز إعدادات الوكيل أي خيارات عامة هنا. [اعرف المزيد](./agents/#additional).
...
استخدم اختصار لوحة المفاتيح `variant_cycle` للتبديل بسرعة بين المتغيّرات. [اعرف المزيد](./keybinds).
