---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Octopus.
---

تتيح لك الإضافات توسيع Octopus عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Octopus.
...
- `.octopus/plugins/` - إضافات على مستوى المشروع
- `~/.config/octopus/plugins/` - إضافات عامة
...
1. إعدادات عامة (`~/.config/octopus/octopus.json`)
2. إعدادات المشروع (`octopus.json`)
3. دليل الإضافات العام (`~/.config/octopus/plugins/`)
4. دليل إضافات المشروع (`.octopus/plugins/`)
...
يشغّل Octopus الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
:::note
إذا كنت تستخدم تطبيق Octopus لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
