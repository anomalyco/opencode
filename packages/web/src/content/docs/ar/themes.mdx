...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/leak-code/themes/*.json` أو `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.leak-code/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.leak-code/themes/*.json`
