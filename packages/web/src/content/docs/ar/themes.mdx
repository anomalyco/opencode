...
مع CyberStrike يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم CyberStrike سمتنا `cyberstrike`.
...
يأتي CyberStrike مع عدة سمات مدمجة.
...
- يريدون أن يطابق CyberStrike مظهر terminal لديهم
...
يدعم CyberStrike نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/cyberstrike/themes/*.json` أو `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.cyberstrike/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.cyberstrike/themes/*.json`
