...
مع Pencode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Pencode سمتنا `pencode`.
...
يأتي Pencode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Pencode مظهر terminal لديهم
...
يدعم Pencode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/pencode/themes/*.json` أو `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.pencode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.pencode/themes/*.json`
