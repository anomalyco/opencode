...
مع Octopus يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Octopus سمتنا `opencode`.
...
يأتي Octopus مع عدة سمات مدمجة.
...
- يريدون أن يطابق Octopus مظهر terminal لديهم
...
يدعم Octopus نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/octopus/themes/*.json` أو `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.octopus/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.octopus/themes/*.json`
