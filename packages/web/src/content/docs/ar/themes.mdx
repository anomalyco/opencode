...
مع xcsh يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم xcsh سمتنا `xcsh`.
...
- يريدون أن يطابق xcsh مظهر terminal لديهم
...
يدعم xcsh نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/xcsh/themes/*.json` أو `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.xcsh/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.xcsh/themes/*.json`
