...
2. **الإعدادات العامة** (`~/.config/leak-code/leak-code.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`LEAKCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`leak-code.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.leak-code`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`LEAKCODE_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.leak-code` و`~/.config/leak-code` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
ضع إعدادات OpenCode العامة في `~/.config/leak-code/leak-code.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `leak-code.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `LEAKCODE_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `LEAKCODE_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.leak-code`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.leak-code`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك ضبط إعدادات الخادم لأوامر `leak-code serve` و`leak-code web` عبر الخيار `server`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/leak-code/agents/` أو `.leak-code/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`leak-code run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/leak-code/commands/` أو `.leak-code/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
ضع ملفات الإضافات في `.leak-code/plugins/` أو `~/.config/leak-code/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
