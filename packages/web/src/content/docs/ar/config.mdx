...
يمكن لـ `OPENCODE_CONFIG_DIRS` تحديد عدة أدلة إعدادات مخصصة باستخدام الفاصل نفسه المستخدم في `PATH` على نظامك (`:` على Linux/macOS، و`;` على Windows). تُحمّل الأدلة من اليسار إلى اليمين، لذلك تتجاوز الأدلة اللاحقة الأدلة السابقة. إذا تم تعيين `OPENCODE_CONFIG_DIR` أيضًا، فسيُضاف في النهاية ويصبح دليل الإعدادات المخصص الأعلى أولوية وهدف الكتابة العام.
