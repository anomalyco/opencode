...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات cyberstrike. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في cyberstrike تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات cyberstrike.
...
يأتي cyberstrike مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.cyberstrike/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `cyberstrike.json` لديك:
...
- عالمي: `~/.config/cyberstrike/modes/`
- للمشروع: `.cyberstrike/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم cyberstrike القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات cyberstrike العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.cyberstrike/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/cyberstrike/modes/` لأوضاع عالمية:
