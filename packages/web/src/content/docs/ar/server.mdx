---
title: الخادم
description: تفاعل مع خادم xcsh عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `xcsh serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل xcsh استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `xcsh.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `xcsh`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `xcsh serve` و `xcsh web`.
...
عند تشغيل `xcsh` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم xcsh للتفاعل مع xcsh برمجيا.
:::

تتيح هذه البنية لـ xcsh دعم عدة عملاء وتمكّنك من التفاعل مع xcsh برمجيا.

يمكنك تشغيل `xcsh serve` لبدء خادم مستقل. إذا كانت واجهة xcsh في terminal (TUI)
قيد التشغيل، فسيبدأ `xcsh serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات xcsh لـ [IDE](/docs/ide).
...
يعرض خادم xcsh واجهات API التالية.
