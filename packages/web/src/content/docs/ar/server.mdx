...
يشغّل الأمر `leak-code serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل opencode استخدامها.
...
عيّن `LEAKCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `opencode`، أو عيّن `LEAKCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `leak-code serve` و `leak-code web`.
...
يمكنك تشغيل `leak-code serve` لبدء خادم مستقل. إذا كانت واجهة opencode في terminal (TUI)
قيد التشغيل، فسيبدأ `leak-code serve` خادما جديدا.
