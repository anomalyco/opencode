...
:::caution
إذا لم يتم تعيين `LEAKCODE_SERVER_PASSWORD` فسيكون الخادم دون حماية. هذا مناسب للاستخدام المحلي، لكنه يجب أن يكون مُعينا عند إتاحة الوصول عبر الشبكة.
:::

:::tip[مستخدمو Windows]
لأفضل تجربة، شغّل `leak-code web` من [WSL](/docs/windows-wsl) بدلا من PowerShell. يضمن ذلك وصولا صحيحا إلى نظام الملفات وتكاملا أفضل مع terminal.
:::
...
لحماية الوصول، عيّن كلمة مرور عبر متغير البيئة `LEAKCODE_SERVER_PASSWORD`:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

اسم المستخدم الافتراضي هو `opencode`، ويمكن تغييره عبر `LEAKCODE_SERVER_USERNAME`.
...
يمكنك أيضا ضبط إعدادات الخادم داخل ملف الإعدادات `leak-code.json`:
