...
:::tip[بدون فتح تلقائي]
لتشغيل خادم الويب دون فتح المتصفح تلقائيا، استخدم `opencode serve` بدلا من ذلك:

```bash
opencode serve --hostname 0.0.0.0 --port 4096
```

ثم انتقل يدويا إلى `http://localhost:4096` (أو عنوان شبكتك) في متصفحك. عند الربط بـ `0.0.0.0`، قم بتعيين `OPENCODE_SERVER_PASSWORD` لحماية الوصول.
:::
