...
Du kan tilføje eksterne værktøjer til Octopus ved hjælp af _Model Context Protocol_ eller MCP. Octopus understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Octopus Config](https://opencode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Octopus håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Octopus:
...
Hvis serveren kræver godkendelse, vil Octopus bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Octopus opbevare tokens sikkert i `~/.local/share/octopus/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Octopus-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Octopus til din Sentry-konto.
