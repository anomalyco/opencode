...
Du kan tilføje eksterne værktøjer til xcsh ved hjælp af _Model Context Protocol_ eller MCP. xcsh understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/xcsh`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
xcsh håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil xcsh:
...
Hvis serveren kræver godkendelse, vil xcsh bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `xcsh mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil xcsh opbevare tokens sikkert i `~/.local/share/xcsh/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem xcsh-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde xcsh til din Sentry-konto.
