...
Du kan også ha globale regler i en `~/.config/leak-code/AGENTS.md`-fil. Dette blir brugt på alle opencode-sessioner.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (bruges hvis ingen `~/.config/leak-code/AGENTS.md` eksisterer)
...
Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, bruges bare `AGENTS.md`. På samme måte har `~/.config/leak-code/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Brugerdefinerede instruktioner

Du kan spesifisere egendefinerte instruksjonsfiler i din `leak-code.json` eller den globale `~/.config/leak-code/leak-code.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af leak-code.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `leak-code.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `leak-code.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
