...
Agentferdigheter lar Pencode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Pencode søker etter disse stedene:

- Prosjektkonfigurasjon: `.pencode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/pencode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Pencode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.pencode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.pencode/skills/git-release/SKILL.md` slik:
...
Pencode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `pencode.json`:
...
**For innebygde agenter** (i `pencode.json`):
