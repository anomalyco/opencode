...
Agentferdigheter lar Octopus oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Octopus søker etter disse stedene:

- Prosjektkonfigurasjon: `.octopus/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/octopus/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Octopus opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.octopus/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.octopus/skills/git-release/SKILL.md` slik:
...
Octopus viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `octopus.json`:
...
**For innebygde agenter** (i `octopus.json`):
