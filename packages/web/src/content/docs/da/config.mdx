...
Du kan også konfigurere [local models](./models#local). [Learn more](./models).
...
:::note
Bærer-tokens (`AWS_BEARER_TOKEN_BEDROCK` eller `/connect`) har forrang over profilbaseret godkendelse. Se [authentication precedence](./providers#authentication-precedence) for detaljer.
:::
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.opencode/agents/`. [Learn more here](./agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](./agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil OpenCode falde tilbage til `"build"` med en advarsel.
...
Du kan konfigurere funktionen [share](./share) gennem indstillingen `share`.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.opencode/commands/`. [Learn more here](./commands).
...
[Plugins](./plugins) udvide OpenCode med brugerdefinerede værktøjer, kroge og integrationer.
