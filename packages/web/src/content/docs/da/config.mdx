---
title: Konfiguration
description: Ved at bruge Octopus JSON konfig.
---

Du kan konfigurere Octopus ved hjælp af en JSON-konfigurationsfil.

---

## Format

Octopus understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/octopus/octopus.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OCTOPUS_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`octopus.json` i projekt) - projektspecifikke indstillinger
5. **`.opencode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OCTOPUS_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
Placer din globale Octopus-konfiguration i `~/.config/octopus/octopus.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `octopus.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Octopus starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OCTOPUS_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OCTOPUS_CONFIG_DIR`
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Octopus-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Octopus-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Octopus at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Octopus-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/octopus/agents/` eller `.octopus/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Octopus falde tilbage til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/octopus/commands/` eller `.octopus/commands/`. [Learn more here](/docs/commands).
...
Octopus vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide Octopus med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.octopus/plugins/` eller `~/.config/octopus/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Octopus til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
