...
`OPENCODE_CONFIG_DIRS` kan angive flere brugerdefinerede konfigurationsmapper med samme separator som systemets `PATH` (`:` på Linux/macOS, `;` på Windows). Mapper indlæses fra venstre mod højre, så senere mapper tilsidesætter tidligere mapper. Hvis `OPENCODE_CONFIG_DIR` også er sat, tilføjes den sidst og bliver den brugerdefinerede konfigurationsmappe med højeste prioritet og globalt skrivemål.
