...
2. **Global config** (`~/.config/leak-code/leak-code.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`LEAKCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`leak-code.json` i projekt) - projektspecifikke indstillinger
5. **`.leak-code` mapper** - agent, kommandoer, plugins
6. **Inline config** (`LEAKCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.leak-code` og `~/.config/leak-code` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale OpenCode-konfiguration i `~/.config/leak-code/leak-code.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `leak-code.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `LEAKCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `LEAKCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.leak-code` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.leak-code` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `leak-code serve` og `leak-code web` gennem indstillingen `server`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/leak-code/agents/` eller `.leak-code/agents/`. [Learn more here](/docs/agents).
...
Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`leak-code run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/leak-code/commands/` eller `.leak-code/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.leak-code/plugins/` eller `~/.config/leak-code/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
