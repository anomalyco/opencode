---
title: Plugins
description: Skriv dine egne plugins for at udvide Octopus.
---

Plugins giver dig mulighed for at udvide Octopus ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Octopuss standardadfærd.
...
- `.octopus/plugins/` - Plugins på projektniveau
...
1. Global konfiguration (`~/.config/octopus/octopus.json`)
2. Projektkonfiguration (`octopus.json`)
3. Global plugin-mappe (`~/.config/octopus/plugins/`)
4. Projekt plugin bibliotek (`.octopus/plugins/`)
...
Octopus kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
:::note
Hvis du bruger Octopus desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
