---
title: Plugins
description: Skriv dine egne plugins for at udvide xcsh.
---

Plugins giver dig mulighed for at udvide xcsh ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre xcshs standardadfærd.
...
- `.xcsh/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/xcsh/node_modules/`.
...
1. Global konfiguration (`~/.config/xcsh/xcsh.json`)
2. Projektkonfiguration (`xcsh.json`)
3. Global plugin-mappe (`~/.config/xcsh/plugins/`)
4. Projekt plugin bibliotek (`.xcsh/plugins/`)
...
xcsh kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En xcsh SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide xcsh.
...
:::note
Hvis du bruger xcsh desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå xcsh i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til xcsh:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som xcsh kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for xcsh sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) for detaljer.
