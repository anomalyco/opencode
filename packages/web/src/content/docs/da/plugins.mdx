---
title: Plugins
description: Skriv dine egne plugins for at udvide CyberStrike.
---

Plugins giver dig mulighed for at udvide CyberStrike ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CyberStrikes standardadfærd.
...
- `.cyberstrike/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/cyberstrike/node_modules/`.
...
1. Global konfiguration (`~/.config/cyberstrike/cyberstrike.json`)
2. Projektkonfiguration (`cyberstrike.json`)
3. Global plugin-mappe (`~/.config/cyberstrike/plugins/`)
4. Projekt plugin bibliotek (`.cyberstrike/plugins/`)
...
CyberStrike kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En cyberstrike SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide cyberstrike.
...
:::note
Hvis du bruger CyberStrike desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå cyberstrike i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til cyberstrike:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som cyberstrike kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for cyberstrike sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://cyberstrike.io/docs/sdk) for detaljer.
