---
title: Plugins
description: Skriv dine egne plugins for at udvide Pencode.
---

Plugins giver dig mulighed for at udvide Pencode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Pencodes standardadfærd.
...
- `.pencode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/pencode/node_modules/`.
...
1. Global konfiguration (`~/.config/pencode/pencode.json`)
2. Projektkonfiguration (`pencode.json`)
3. Global plugin-mappe (`~/.config/pencode/plugins/`)
4. Projekt plugin bibliotek (`.pencode/plugins/`)
...
Pencode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En pencode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide pencode.
...
:::note
Hvis du bruger Pencode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå pencode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til pencode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som pencode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for pencode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://pencode.ai/docs/sdk) for detaljer.
