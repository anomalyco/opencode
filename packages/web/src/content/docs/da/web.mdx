...
:::caution
Hvis `IMECODE_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `IMECODE_SERVER_PASSWORD` miljøvariablen:

```bash
IMECODE_SERVER_PASSWORD=secret opencode web
```

Brugernavnet er som standard `opencode`, men kan ændres med `IMECODE_SERVER_USERNAME`.
