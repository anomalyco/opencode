...
:::caution
Hvis `LEAKCODE_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::

:::tip[Windows-brugere]
For den bedste oplevelse skal du køre `leak-code web` fra [WSL](/docs/windows-wsl) i stedet for PowerShell. Dette sikrer korrekt filsystemadgang og terminalintegration.
:::
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `LEAKCODE_SERVER_PASSWORD` miljøvariablen:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Brugernavnet er som standard `opencode`, men kan ændres med `LEAKCODE_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `leak-code.json`-konfigurationsfil:
