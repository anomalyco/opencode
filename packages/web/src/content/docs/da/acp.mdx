...
description: Brug xcsh i enhver ACP-kompatibel editor.
---

xcsh understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge xcsh via ACP, konfigurer din editor til at køre kommandoen `xcsh acp`.

Kommandoen starter xcsh som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'xcsh'-agent i AI Chat-agentvælgeren.
...
For at bruge xcsh som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge xcsh som ACP-agent til chat.
...
xcsh fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din xcsh-konfiguration
