---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

Octopus bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Octopus-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/octopus/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Octopus-konfigurationen.
...
Octopus Zen er en liste over modeller leveret af Octopus-teamet, som er blevet testet og verificeret til at fungere godt med Octopus. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Octopus Zen.
:::
...
Det fungerer som alle andre udbydere i Octopus og er helt valgfrit at bruge.

---

## Octopus Go

Octopus Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Octopus-teamet, som er testet og verificeret til at fungere godt med Octopus.

1. Kør kommandoen `/connect` i TUI, vælg `Octopus Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Octopus:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `octopus.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Octopus understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Octopus bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Octopus
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`octopus.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Octopus:
...
Sådan bruger du Google Vertex AI med Octopus:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Octopus-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Octopus-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for Octopus. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Octopus:
...
3. Kopier API-nøglen til brug i Octopus.
...
6. **Vigtigt**: Før du bruger skymodeller i Octopus, skal du hente modelinformationen lokalt:
...
Octopus Zen er en liste over testede og verificerede modeller leveret af Octopus-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Octopus Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Octopus Zen**.

   ```txt
   /connect
   ```

3. Indtast Octopus API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Octopus:
...
4. Opret eller opdater `octopus.json`-filen i projektkataloget:
...
`limit`-felterne lader Octopus forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
