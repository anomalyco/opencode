---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Pencode.
---

import config from "../../../../config.mjs"
export const console = config.console

Pencode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Pencode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/pencode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Pencode-konfigurationen.
...
Pencode Zen er en liste over modeller leveret af Pencode-teamet, som er blevet testet og verificeret til at fungere godt med Pencode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Pencode Zen.
:::
...
Det fungerer som alle andre udbydere i Pencode og er helt valgfrit at bruge.

---

## Pencode Go

Pencode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Pencode-teamet, som er testet og verificeret til at fungere godt med Pencode.

1. Kør kommandoen `/connect` i TUI, vælg `Pencode Go`, og gå til [pencode.ai/auth](https://pencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Pencode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører pencode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `pencode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Pencode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere pencode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at pencode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem pencode-konfigurationen.
...
gemme tokenet i pencode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Pencode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Pencode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`pencode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `pencode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Pencode:
...
Sådan bruger du Google Vertex AI med Pencode:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem pencode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session) plugin for automatisk at logge hver Pencode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Pencode-samtale opført som en separat session.
...
Du kan konfigurere pencode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere pencode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere pencode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Pencode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/pencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Pencode:
...
3. Kopier API-nøglen til brug i Pencode.
...
6. **Vigtigt**: Før du bruger skymodeller i Pencode, skal du hente modelinformationen lokalt:
...
Pencode Zen er en liste over testede og verificerede modeller leveret af Pencode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Pencode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Pencode Zen**.

   ```txt
   /connect
   ```

3. Indtast Pencode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem pencode-konfigurationen.
...
5. Du kan også tilpasse dem gennem pencode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Pencode:
...
Du kan også tilpasse modeller gennem pencode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med pencode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `pencode.json`-filen i projektkataloget:
...
`limit`-felterne lader Pencode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `pencode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek pencode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i pencode-konfigurationen.
