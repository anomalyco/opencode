...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/leak-code/auth.json`.
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `leak-code.json`:
...
gemme tokenet i leak-code auth-lagring.
:::
...
`leak-code.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
4. Opret eller opdater `leak-code.json`-filen i projektkataloget:
...
1. **Tjek godkendelsesopsætningen**: Kør `leak-code auth list` for at se om legitimationsoplysningerne
