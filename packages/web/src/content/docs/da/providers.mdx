---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i xcsh-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/xcsh/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i xcsh-konfigurationen.
...
xcsh Zen er en liste over modeller leveret af xcsh-teamet, som er blevet testet og verificeret til at fungere godt med xcsh. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med xcsh Zen.
:::
...
Det fungerer som alle andre udbydere i xcsh og er helt valgfrit at bruge.

---

## xcsh Go

xcsh Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af xcsh-teamet, som er testet og verificeret til at fungere godt med xcsh.

1. Kør kommandoen `/connect` i TUI, vælg `xcsh Go`, og gå til [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Sådan bruger du Amazon Bedrock med xcsh:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører xcsh:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `xcsh.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i xcsh understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at xcsh skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem xcsh-konfigurationen.
...
gemme tokenet i xcsh auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
xcsh bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse xcsh
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`xcsh.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `xcsh.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med xcsh:
...
Sådan bruger du Google Vertex AI med xcsh:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem xcsh, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) plugin for automatisk at logge hver xcsh-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver xcsh-samtale opført som en separat session.
...
Du kan konfigurere xcsh til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere xcsh til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere xcsh til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for xcsh. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/xcsh) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med xcsh:
...
3. Kopier API-nøglen til brug i xcsh.
...
6. **Vigtigt**: Før du bruger skymodeller i xcsh, skal du hente modelinformationen lokalt:
...
xcsh Zen er en liste over testede og verificerede modeller leveret af xcsh-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>xcsh Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **xcsh Zen**.

   ```txt
   /connect
   ```

3. Indtast xcsh API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem xcsh-konfigurationen.
...
5. Du kan også tilpasse dem gennem xcsh-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med xcsh:
...
Du kan også tilpasse modeller gennem xcsh-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med xcsh. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `xcsh.json`-filen i projektkataloget:
...
`limit`-felterne lader xcsh forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `xcsh auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek xcsh-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i xcsh-konfigurationen.
