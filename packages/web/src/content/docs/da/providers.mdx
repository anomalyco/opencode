---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CyberStrike.
---

import config from "../../../../config.mjs"
export const console = config.console

CyberStrike bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CyberStrike-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/cyberstrike/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i CyberStrike-konfigurationen.
...
CyberStrike Zen er en liste over modeller leveret af CyberStrike-teamet, som er blevet testet og verificeret til at fungere godt med CyberStrike. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med CyberStrike Zen.
:::
...
Det fungerer som alle andre udbydere i CyberStrike og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med CyberStrike:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører cyberstrike:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `cyberstrike.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CyberStrike understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at cyberstrike skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem cyberstrike-konfigurationen.
...
gemme tokenet i cyberstrike auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CyberStrike bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CyberStrike
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`cyberstrike.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Tilpas gennem `cyberstrike.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med CyberStrike:
...
Sådan bruger du Google Vertex AI med CyberStrike:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører cyberstrike.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem cyberstrike, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) plugin for automatisk at logge hver CyberStrike-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CyberStrike-samtale opført som en separat session.
...
Du kan konfigurere cyberstrike til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere cyberstrike til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere cyberstrike til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CyberStrike. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/cyberstrike) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CyberStrike:
...
3. Kopier API-nøglen til brug i CyberStrike.
...
6. **Vigtigt**: Før du bruger skymodeller i CyberStrike, skal du hente modelinformationen lokalt:
...
CyberStrike Zen er en liste over testede og verificerede modeller leveret af CyberStrike-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>CyberStrike Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **CyberStrike Zen**.

   ```txt
   /connect
   ```

3. Indtast CyberStrike API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem cyberstrike-konfigurationen.
...
5. Du kan også tilpasse dem gennem cyberstrike-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CyberStrike:
...
Du kan også tilpasse modeller gennem cyberstrike-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med cyberstrike. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `cyberstrike.json`-filen i projektkataloget:
...
`limit`-felterne lader CyberStrike forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `cyberstrike auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek cyberstrike-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i cyberstrike-konfigurationen.
