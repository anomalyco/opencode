...
Brugerdefinerede kommandoer er ud over de indbyggede kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`. [Learn more](./tui#commands).
...
Brug indstillingerne `command` i din OpenCode [config](./config):
...
Brug _!`command`_ til at injicere [bash command](./tui#bash-commands) output i din prompt.
...
Brug `agent`-konfigurationen til valgfrit at angive, hvilken [agent](./agents) der skal udføre denne kommando.
Hvis dette er en [subagent](./agents/#subagents), vil kommandoen som standard udløse en subagent påkaldelse.
...
Brug `subtask` boolean til at tvinge kommandoen til at udløse en [subagent](./agents/#subagents) påkaldelse.
...
øjeblikkelig `mode` er sat til `primary` på [agent](./agents) konfigurationen.
...
opencode indeholder flere indbyggede kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](./tui#commands).
