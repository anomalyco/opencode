...
:::tip
Hvis du ikke angiver en model, bruger primære agenter [model globally configured](./config#models), mens subagenter vil bruge modeller for den primære agent, der påkaldte subagenten.
:::
...
Model-id'et i din OpenCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenCode Zen](./zen), vil du bruge `opencode/gpt-5.1-codex` til GPT 5.1 Codex.
