...
Der er to typer agenter i xcsh; primære agenter og subagenter.
...
xcsh leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
xcsh leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

xcsh leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `xcsh.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger xcsh modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale xcsh-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din xcsh-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [xcsh Zen](/docs/zen), vil du bruge `xcsh/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `xcsh models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/f5xc-salesdemos/xcsh).
:::
