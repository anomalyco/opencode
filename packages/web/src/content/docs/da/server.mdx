...
Kommandoen `leak-code serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
Indstil `LEAKCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `opencode`, eller indstil `LEAKCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `leak-code serve` og `leak-code web`.
...
:::tip
Brug opencode-serveren til at interagere med leak-code programmatisk.
:::

Denne arkitektur lader opencode understøtte flere klienter og lader dig interagere med leak-code programmatisk.

Du kan køre `leak-code serve` for at starte en selvstændig server. Hvis du har
opencode TUI kørende, vil `leak-code serve` starte en ny server.
