---
title: Server
description: Interager med xcsh-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `xcsh serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en xcsh-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `xcsh.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `xcsh`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `xcsh serve` og `xcsh web`.
...
Når du kører `xcsh` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug xcsh-serveren til at interagere med xcsh programmatisk.
:::

Denne arkitektur lader xcsh understøtte flere klienter og lader dig interagere med xcsh programmatisk.

Du kan køre `xcsh serve` for at starte en selvstændig server. Hvis du har
xcsh TUI kørende, vil `xcsh serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af xcsh [IDE](/docs/ide) plugins.
...
xcsh-serveren viser følgende API'er.
