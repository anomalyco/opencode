---
title: Server
description: Interager med pencode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `pencode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en pencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `pencode.local` |
...
Indstil `PENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `pencode`, eller indstil `PENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `pencode serve` og `pencode web`.
...
Når du kører `pencode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug pencode-serveren til at interagere med pencode programmatisk.
:::

Denne arkitektur lader pencode understøtte flere klienter og lader dig interagere med pencode programmatisk.

Du kan køre `pencode serve` for at starte en selvstændig server. Hvis du har
pencode TUI kørende, vil `pencode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Pencode [IDE](/docs/ide) plugins.
...
Pencode-serveren viser følgende API'er.
