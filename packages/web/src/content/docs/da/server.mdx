---
title: Server
description: Interager med cyberstrike-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `cyberstrike serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en cyberstrike-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `cyberstrike.local` |
...
Indstil `CYBERSTRIKE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `cyberstrike`, eller indstil `CYBERSTRIKE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `cyberstrike serve` og `cyberstrike web`.
...
Når du kører `cyberstrike` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug cyberstrike-serveren til at interagere med cyberstrike programmatisk.
:::

Denne arkitektur lader cyberstrike understøtte flere klienter og lader dig interagere med cyberstrike programmatisk.

Du kan køre `cyberstrike serve` for at starte en selvstændig server. Hvis du har
cyberstrike TUI kørende, vil `cyberstrike serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CyberStrike [IDE](/docs/ide) plugins.
...
CyberStrike-serveren viser følgende API'er.
