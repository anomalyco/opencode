...
Med CyberStrike kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CyberStrike vårt eget `cyberstrike`-tema.
...
CyberStrike kommer med flere innebygde temaer.
...
- Vil at CyberStrike skal matche terminalens utseende
...
CyberStrike støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/cyberstrike/themes/*.json` eller `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.cyberstrike/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.cyberstrike/themes/*.json`
