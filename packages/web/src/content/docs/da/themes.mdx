...
Med xcsh kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger xcsh vårt eget `xcsh`-tema.
...
xcsh kommer med flere innebygde temaer.
...
- Vil at xcsh skal matche terminalens utseende
...
xcsh støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/xcsh/themes/*.json` eller `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.xcsh/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.xcsh/themes/*.json`
