...
Med Pencode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Pencode vårt eget `pencode`-tema.
...
Pencode kommer med flere innebygde temaer.
...
- Vil at Pencode skal matche terminalens utseende
...
Pencode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/pencode/themes/*.json` eller `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.pencode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.pencode/themes/*.json`
