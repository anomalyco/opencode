...
Med Octopus kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Octopus vårt eget `opencode`-tema.
...
Octopus kommer med flere innebygde temaer.
...
- Vil at Octopus skal matche terminalens utseende
...
Octopus støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/octopus/themes/*.json` eller `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.octopus/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.octopus/themes/*.json`
