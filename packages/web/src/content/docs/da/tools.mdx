...
:::note
Dette verktøyet er bare tilgængelig når `LEAKCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `LEAKCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `LEAKCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
