...
:::note
Dette verktøyet er bare tilgængelig når `IMECODE_EXPERIMENTAL_LSP_TOOL=true` (eller `IMECODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `IMECODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
