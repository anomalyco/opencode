...
Her er alle de innebygde verktøyene som er tilgængelige i Pencode.
...
:::note
Dette verktøyet er bare tilgængelig når `PENCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `PENCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Pencode-leverandøren eller når miljøvariabelen `PENCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
