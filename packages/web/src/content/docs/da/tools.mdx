...
Her er alle de innebygde verktøyene som er tilgængelige i CyberStrike.
...
:::note
Dette verktøyet er bare tilgængelig når `CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true` (eller `CYBERSTRIKE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger CyberStrike-leverandøren eller når miljøvariabelen `CYBERSTRIKE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
