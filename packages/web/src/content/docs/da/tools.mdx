...
:::note
Når macOS-sandboxing er aktiveret, kører bash med filsystembegrænsninger og kan anmode om et separat genforsøg uden sandbox, når det er nødvendigt.
Hvis du ved, at en kommando skal starte uden for sandboxen, sæt `# opencode:unsandboxed <reason>` på den første ikke-tomme linje i kommandoen.
Se [sandbox-konfiguration](/docs/config#sandbox) for understøttet adfærd og begrænsninger.
:::
