...
Her er alle de innebygde verktøyene som er tilgængelige i Octopus.
...
:::note
Dette verktøyet er bare tilgængelig når `OCTOPUS_EXPERIMENTAL_LSP_TOOL=true` (eller `OCTOPUS_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Octopus-leverandøren eller når miljøvariabelen `OCTOPUS_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
