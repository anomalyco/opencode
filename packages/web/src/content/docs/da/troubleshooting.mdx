...
Hvis du har `IMECODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `IMECODE_PORT` (eller vælg en ledig port) og start på ny.
