...
- **macOS/Linux** (ældre installationer): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Hvis `leak-code.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `LEAKCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `LEAKCODE_PORT` (eller vælg en ledig port) og start på ny.
...
3. Sørg for at du har den nyeste version med `leak-code upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `leak-code models`
