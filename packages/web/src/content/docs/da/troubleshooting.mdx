...
For at fejlfinde problemer med Pencode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\pencode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `pencode --log-level DEBUG`.

---

## Lagring

pencode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/pencode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\pencode`
...
Pencode Desktop kører en lokal Pencode-server (`pencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Pencode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/pencode/pencode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\pencode\pencode.jsonc`
...
Pencode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/pencode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\pencode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Pencode kan genopbygge den.

1. Afslut Pencode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/pencode`
- **Linux**: slet `~/.cache/pencode` (eller kør `rm -rf ~/.cache/pencode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\pencode`

3. Start Pencode Desktop på ny.

---

### Løs problemer med serverforbindelse

Pencode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `pencode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `PENCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `PENCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Pencode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Pencode.

---

### Meddelelser vises ikke

Pencode Desktop viser kun systemvarsler når:

- varsler er aktiveret for Pencode i dine OS-indstillinger, og
...
1. Afslut Pencode Desktop.
2. Find og slet disse filer (de findes i Pencode Desktop-appens datamappe):

- `pencode.settings.dat` (skrivebordsstandardserver URL)
- `pencode.global.dat` og `pencode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Pencode:
...
For at finde ud af hvilke modeller du har adgang til, kør `pencode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\pencode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. pencode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\pencode`

2. Start pencode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge pencode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
pencode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
