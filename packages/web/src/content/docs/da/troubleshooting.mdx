...
For at fejlfinde problemer med CyberStrike, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\cyberstrike\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `cyberstrike --log-level DEBUG`.

---

## Lagring

cyberstrike gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/cyberstrike/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\cyberstrike`
...
CyberStrike Desktop kører en lokal CyberStrike-server (`cyberstrike-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `CyberStrike`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/cyberstrike/cyberstrike.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\cyberstrike\cyberstrike.jsonc`
...
CyberStrike kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/cyberstrike/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\cyberstrike\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så CyberStrike kan genopbygge den.

1. Afslut CyberStrike Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/cyberstrike`
- **Linux**: slet `~/.cache/cyberstrike` (eller kør `rm -rf ~/.cache/cyberstrike`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\cyberstrike`

3. Start CyberStrike Desktop på ny.

---

### Løs problemer med serverforbindelse

CyberStrike Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `cyberstrike.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CYBERSTRIKE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CYBERSTRIKE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver CyberStrike Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i CyberStrike.

---

### Meddelelser vises ikke

CyberStrike Desktop viser kun systemvarsler når:

- varsler er aktiveret for CyberStrike i dine OS-indstillinger, og
...
1. Afslut CyberStrike Desktop.
2. Find og slet disse filer (de findes i CyberStrike Desktop-appens datamappe):

- `cyberstrike.settings.dat` (skrivebordsstandardserver URL)
- `cyberstrike.global.dat` og `cyberstrike.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med CyberStrike:
...
For at finde ud af hvilke modeller du har adgang til, kør `cyberstrike models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\cyberstrike`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. cyberstrike installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\cyberstrike`

2. Start cyberstrike på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge cyberstrike til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
cyberstrike vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
