...
For at fejlfinde problemer med Octopus, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Octopus Desktop kører en lokal Octopus-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Octopus`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Octopus kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Octopus kan genopbygge den.

1. Afslut Octopus Desktop helt.
2. Slet cache-mappen:
...
3. Start Octopus Desktop på ny.

---

### Løs problemer med serverforbindelse

Octopus Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `octopus.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OCTOPUS_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OCTOPUS_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Octopus Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Octopus.

---

### Meddelelser vises ikke

Octopus Desktop viser kun systemvarsler når:

- varsler er aktiveret for Octopus i dine OS-indstillinger, og
...
1. Afslut Octopus Desktop.
2. Find og slet disse filer (de findes i Octopus Desktop-appens datamappe):
...
Hvis du oplever problemer med Octopus:
