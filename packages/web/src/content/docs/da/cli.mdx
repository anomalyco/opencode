---
title: CLI
description: xcsh CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
xcsh
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med xcsh programmatisk.
...
Start xcsh terminalbrugergrænsefladen.
...
xcsh CLI har også følgende kommandoer.

---

### agent

Administrer agenter i xcsh.
...
Tilslut en terminal til en allerede kørende xcsh backend-server startet via `serve` eller `web` kommandoer.

```bash
xcsh attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern xcsh backend. For eksempel:
...
xcsh drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `xcsh auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

Når xcsh starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i xcsh.
...
Kør xcsh i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `xcsh serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| `--attach`   |      | Tilslut til en kørende xcsh-server (f.eks. http://localhost:4096)               |
...
Start en hovedløs xcsh-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
xcsh serve
```

Dette starter en HTTP-server, der giver API-adgang til xcsh-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `xcsh`).
...
Administrator xcsh sessionsholder.
...
Liste over alle xcsh sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine xcsh-sessioner.
...
Importer sessionsdata fra en JSON fil eller xcsh del URL.

```bash
xcsh import <file>
```

Du kan importere fra en lokal fil eller en xcsh share URL.
...
Start en hovedløs xcsh-server med en webgrænseflade.

```bash
xcsh web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til xcsh via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `xcsh`).
...
Afinstaller xcsh og fjern alle relaterede filer.
...
Opdaterer xcsh til den nyeste version eller en specifik version.
...
xcsh CLI tager følgende globale flag.
...
xcsh kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `xcsh`) |
