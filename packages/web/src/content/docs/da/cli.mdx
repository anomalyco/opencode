...
OpenCode CLI starter som standard [TUI](./tui), når den kører uden nogen argumenter.
...
Dette opsætter det nødvendige GitHub Actions workflow og guider dig gennem konfigurationsprocessen. [Learn more](./github).
...
Dette er nyttigt til at finde ud af det nøjagtige modelnavn, der skal bruges i [your config](./config/).
...
Start en hovedløs OpenCode-server til API-adgang. Tjek [server docs](./server) for den fulde HTTP-grænseflade.
