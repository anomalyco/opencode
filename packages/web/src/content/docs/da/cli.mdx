---
title: CLI
description: Octopus CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Octopus CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Octopus programmatisk.
...
Start Octopus terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende Octopus backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Octopus backend. For eksempel:
...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Sessions-id for at fortsætte                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)     |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OCTOPUS_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OCTOPUS_SERVER_USERNAME` eller `opencode`) |
...
Octopus drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/octopus/auth.json`.

```bash
opencode auth login
```

Når Octopus starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Octopus.
...
Start en hovedløs Octopus-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
opencode serve
```

Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `OCTOPUS_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Administrator Octopus sessionsholder.
...
Liste over alle Octopus sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Octopus-sessioner.
...
Importer sessionsdata fra en JSON fil eller Octopus del URL.
...
Start en hovedløs Octopus-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Octopus via en webgrænseflade. Indstil `OCTOPUS_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller Octopus og fjern alle relaterede filer.
...
Opdaterer Octopus til den nyeste version eller en specifik version.
...
Octopus kan konfigureres ved hjælp af miljøvariabler.

| Variabel                             | Skriv   | Beskrivelse                                                           |
| ------------------------------------ | ------- | --------------------------------------------------------------------- |
| `OCTOPUS_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `OCTOPUS_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `OCTOPUS_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `OCTOPUS_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `OCTOPUS_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `OCTOPUS_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `OCTOPUS_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `OCTOPUS_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `OCTOPUS_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `OCTOPUS_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `OCTOPUS_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `OCTOPUS_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `OCTOPUS_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `OCTOPUS_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `OCTOPUS_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `OCTOPUS_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `OCTOPUS_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `OCTOPUS_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `OCTOPUS_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `OCTOPUS_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `OCTOPUS_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `OCTOPUS_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `OCTOPUS_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `OCTOPUS_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                       | Skriv   | Beskrivelse                                |
| ---------------------------------------------- | ------- | ------------------------------------------ |
| `OCTOPUS_EXPERIMENTAL`                         | boolean | Aktiver alle eksperimentelle funktioner    |
| `OCTOPUS_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                      |
| `OCTOPUS_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI              |
| `OCTOPUS_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms  |
| `OCTOPUS_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar           |
| `OCTOPUS_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir        |
| `OCTOPUS_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                    |
| `OCTOPUS_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj           |
| `OCTOPUS_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                   |
| `OCTOPUS_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner        |
| `OCTOPUS_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer            |
| `OCTOPUS_EXPERIMENTAL_MARKDOWN`                | boolean | Aktive eksperimentelle markdown-funktioner |
| `OCTOPUS_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                       |
