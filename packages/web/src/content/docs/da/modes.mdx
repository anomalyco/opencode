...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i cyberstrike-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i cyberstrike giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem cyberstrike-konfigurationen.
...
cyberstrike leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.cyberstrike/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `cyberstrike.json`-konfigurationsfil:
...
- Globalt: `~/.config/cyberstrike/modes/`
...
Hvis der ikke er angivet nogen temperatur, bruger cyberstrike modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale cyberstrike-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.cyberstrike/modes/` for projektspecifikke tilstande eller `~/.config/cyberstrike/modes/` for globale tilstande:
