...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i xcsh-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i xcsh giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem xcsh-konfigurationen.
...
xcsh leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.xcsh/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `xcsh.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger xcsh modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale xcsh-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.xcsh/modes/` for projektspecifikke tilstande eller `~/.config/xcsh/modes/` for globale tilstande:
