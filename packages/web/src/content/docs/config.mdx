...
OpenCode uses snapshots to track file changes and enable revert functionality during agent operations. By default, snapshots are enabled to allow the agent to safely undo changes if something goes wrong.

For very large repositories with many submodules, the snapshot system can cause slow indexing as it tracks all changes using an internal git repository. You can disable snapshots using the `snapshot` option (`true` by default).
...
:::caution[Disabling Snapshots]
When snapshots are disabled:
- You lose the safety net for undoing agent modifications

Consider disabling snapshots only for repositories with:
- Many submodules that are separate repositories
- Extremely large codebases where indexing takes too long
:::
