...
You can load plugins from npm packages through the `plugin` option.
...
OpenCode automatically installs these packages at startup. Both regular and scoped npm packages are supported.

You can also create local plugins by placing files in `.opencode/plugin/` or `~/.config/opencode/plugin/`. [Learn more here](/docs/plugins).
