...
The default is `5`. Setting it to `2` restores the previous spawn behavior (the main session spawns subagents, subagents do not); custom agents that previously nested without limit are now bounded. Setting it to `1` is the kill switch: the `task` tool is removed everywhere, including from the main session.
...
The default is `200`. Resumed subagents do not count against it, and the counter is not persisted — it resets on each OpenCode process restart. Reaching the limit fails further spawns with a typed `SubagentTreeLimitError`.
