...
Legacy `theme` and `keybinds` keys in `opencode.json` are automatically migrated to `tui.json` when possible. The `tui` key in `opencode.json` is now active and can be used to configure TUI settings directly:
