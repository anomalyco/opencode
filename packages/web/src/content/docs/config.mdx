...
If a provider uses `type: "api"` auth, you can also place runtime connection
overrides in `~/.local/share/opencode/auth.json` under that provider's
`options` object. This is useful for per-user settings like `baseURL`, custom
headers, or mTLS files, while keeping the provider definition and model list in
`opencode.json`.
...
OpenCode merges these auth-side `options` into the provider runtime options at
startup.
