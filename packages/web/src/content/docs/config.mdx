...
OpenCode can automatically find and add relevant files to the session context based on your query.
...
|--------|------|---------|-------------|
| `autoAdd` | boolean | `false` | Automatically add relevant files to context |
| `maxFiles` | integer | `10` | Maximum number of files to auto-add |
| `includeTests` | boolean | `true` | Include test files matching the current module |

---

### Multi-Agent Teams

Configure how the `/team` command executes tasks using multiple agents.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `multiAgent.defaultMode` | string | `"parallel"` | Default team mode: `parallel`, `pipeline`, or `supervisor` |
| `multiAgent.maxAgents` | integer | `3` | Maximum number of agents in a team |

---

### Mermaid Diagrams

Enable the model to generate diagrams using Mermaid syntax.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `diagrams.enabled` | boolean | `false` | Enable mermaid diagram generation |
| `diagrams.format` | string | `"svg"` | Output format for diagrams |

---

### Auto-Fix

After each LLM turn, automatically run linters and type-checkers, reporting errors back to the model for iterative fixing.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `autoFix.enabled` | boolean | `false` | Enable auto-fix after each turn |
| `autoFix.maxIterations` | integer | `3` | Maximum fix iterations |
| `autoFix.tools` | string[] | all available | Lint tools to run: `tsc`, `biome`, `eslint`, `oxlint` |

---

### Git Configuration

Configure how the `/commit` command generates commit messages.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `git.autoStage` | boolean | `false` | Auto-stage all changes |
| `git.conventional` | boolean | `true` | Use Conventional Commits format |

---

### Testing Configuration

Configure how the `/test` command generates and runs tests.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `testing.enabled` | boolean | `true` | Enable test command |
| `testing.framework` | string | auto-detected | Testing framework: `vitest`, `jest`, `mocha`, `bun` |
| `testing.testCommand` | string | auto-detected | Custom test command |
| `testing.maxFixIterations` | integer | `2` | Max fix iterations when tests fail |
