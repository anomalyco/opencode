---
title: Config
description: Using the Leak Code JSON config.
---

You can configure Leak Code using a JSON config file.

---

## Format

Leak Code supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
2. **Global config** (`~/.config/leak-code/leak-code.json`) - user preferences
3. **Custom config** (`LEAKCODE_CONFIG` env var) - custom overrides
4. **Project config** (`leak-code.json` in project) - project-specific settings
5. **`.leak-code` directories** - agents, commands, plugins
6. **Inline config** (`LEAKCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/leak-code` on macOS) - admin-controlled
...
:::note
The `.leak-code` and `~/.config/leak-code` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global Leak Code config in `~/.config/leak-code/leak-code.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/leak-code/tui.json`.
...
Add `leak-code.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Leak Code starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `LEAKCODE_CONFIG` environment variable.
...
Specify a custom config directory using the `LEAKCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.leak-code` directory, and should
...
The custom directory is loaded after the global config and `.leak-code` directories, so it **can override** their settings.
...
Drop a `leak-code.json` or `leak-code.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/leak-code` |
| Linux    | `/etc/leak-code`                         |
| Windows  | `%ProgramData%\leak-code`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Leak Code reads managed preferences from the `ai.leak-code.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Leak Code checks these paths:

1. `/Library/Managed Preferences/<user>/ai.leak-code.managed.plist`
2. `/Library/Managed Preferences/ai.leak-code.managed.plist`

The plist keys map directly to `leak-code.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.leak-code.managed` PayloadType. The Leak Code config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.leak-code.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.leak-code.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.leak-code</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `LEAKCODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `leak-code.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `leak-code serve` and `leak-code web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Leak Code server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `leak-code.local`. Useful for running multiple instances on the same network.
...
If not specified, Leak Code will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Leak Code config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Leak Code tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Leak Code actions on configured resources. Currently, policies can control which providers Leak Code may use.
...
Leak Code normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Leak Code omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/leak-code/agents/` or `.leak-code/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Leak Code will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`leak-code run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/leak-code/commands/` or `.leak-code/commands/`. [Learn more here](/docs/commands).
...
Leak Code uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Leak Code will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, leak-code **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Leak Code with custom tools, hooks, and integrations.

Place plugin files in `.leak-code/plugins/` or `~/.config/leak-code/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Leak Code to only use specific providers rather than disabling them one by one.
