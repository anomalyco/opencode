...
You can configure Octopus using a JSON config file.

---

## Format

Octopus supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
2. **Global config** (`~/.config/octopus/octopus.json`) - user preferences
3. **Custom config** (`OCTOPUS_CONFIG` env var) - custom overrides
4. **Project config** (`octopus.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`OCTOPUS_CONFIG_CONTENT` env var) - runtime overrides
...
Place your global Octopus config in `~/.config/octopus/octopus.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/octopus/tui.json`.
...
Add `octopus.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Octopus starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `OCTOPUS_CONFIG` environment variable.
...
Specify a custom config directory using the `OCTOPUS_CONFIG_DIR`
...
Drop an `octopus.json` or `octopus.jsonc` file in the system managed config directory:
...
On macOS, Octopus reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Octopus checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `octopus.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The Octopus config keys go directly in the payload dict:
...
Use `OCTOPUS_TUI_CONFIG` to point to a custom TUI config file.

Legacy `theme`, `keybinds`, and `tui` keys in `octopus.json` are deprecated and automatically migrated when possible.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Octopus server.
...
If not specified, Octopus will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Octopus config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Octopus tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can also define agents using markdown files in `~/.config/octopus/agents/` or `.octopus/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Octopus will fall back to `"build"` with a warning.
...
You can also define commands using markdown files in `~/.config/octopus/commands/` or `.octopus/commands/`. [Learn more here](/docs/commands).
...
Octopus uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Octopus will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend Octopus with custom tools, hooks, and integrations.

Place plugin files in `.octopus/plugins/` or `~/.config/octopus/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Octopus to only use specific providers rather than disabling them one by one.
