...
Use `disabled_models` to remove specific models per provider.
...
Use `enabled_models` to limit which models are available per provider. An empty list removes the provider, effectively disabling that provider.
