...
You can configure Pencode using a JSON config file.

---

## Format

Pencode supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/pencode`) - organizational defaults
2. **Global config** (`~/.config/pencode/pencode.json`) - user preferences
3. **Custom config** (`PENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`pencode.json` in project) - project-specific settings
5. **`.pencode` directories** - agents, commands, plugins
6. **Inline config** (`PENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/pencode/` on macOS) - admin-controlled
...
:::note
The `.pencode` and `~/.config/pencode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/pencode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global Pencode config in `~/.config/pencode/pencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/pencode/tui.json`.
...
Add `pencode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Pencode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `PENCODE_CONFIG` environment variable.
...
Specify a custom config directory using the `PENCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.pencode` directory, and should
...
The custom directory is loaded after the global config and `.pencode` directories, so it **can override** their settings.
...
Drop an `pencode.json` or `pencode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/pencode/` |
| Linux    | `/etc/pencode/`                         |
| Windows  | `%ProgramData%\pencode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Pencode reads managed preferences from the `ai.pencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Pencode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.pencode.managed.plist`
2. `/Library/Managed Preferences/ai.pencode.managed.plist`

The plist keys map directly to `pencode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.pencode.managed` PayloadType. The Pencode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.pencode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.pencode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.pencode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`pencode.ai/config.json`**](https://pencode.ai/config.json).

TUI config uses [**`pencode.ai/tui.json`**](https://pencode.ai/tui.json).
...
Use `PENCODE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `pencode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `pencode serve` and `pencode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Pencode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `pencode.local`. Useful for running multiple instances on the same network.
...
If not specified, Pencode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Pencode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Pencode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Pencode actions on configured resources. Currently, policies can control which providers Pencode may use.
...
Pencode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Pencode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/pencode/agents/` or `.pencode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Pencode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`pencode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/pencode/commands/` or `.pencode/commands/`. [Learn more here](/docs/commands).
...
Pencode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Pencode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, pencode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Pencode with custom tools, hooks, and integrations.

Place plugin files in `.pencode/plugins/` or `~/.config/pencode/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Pencode to only use specific providers rather than disabling them one by one.
