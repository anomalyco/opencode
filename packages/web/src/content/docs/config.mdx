...
For nested repositories, you can opt into searching one enclosing Git root beyond the current project root.

```jsonc title="opencode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "config": {
    "find_up": "git_submodule"
  }
}
```

With `config.find_up: "git_submodule"`, opencode keeps the current repo behavior and then continues upward to exactly the next enclosing Git root. This also applies to local `.opencode/*` files, `AGENTS.md`, and relative `instructions` globs.
