...
You can also provide configuration for specific models within a provider using `models` and `options`. This is useful for fine-tuning behavior on a per-model basis.
...
In this example, the `skip-title` option tells OpenCode not to generate a session title for this specific model.
