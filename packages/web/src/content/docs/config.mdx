...
You can configure project file watching through the `watcher` option.
...
- `enabled` - Watch the current project for external file changes (default: `true`). Config files continue to hot reload when disabled.
- `ignore` - Exclude additional paths using glob syntax. Common dependency, VCS, build, and cache directories are excluded from recursive project watching at any depth; Git `HEAD` is watched separately.
