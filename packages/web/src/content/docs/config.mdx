...
You can also automatically switch the theme based on the active model or provider by setting `theme` under the provider or model settings in `opencode.json`. OpenCode looks for a theme in this order: `model theme` → `provider theme` → `tui theme` → `default opencode theme`.
