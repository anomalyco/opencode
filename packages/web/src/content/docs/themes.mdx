...
With xcsh you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, xcsh uses our own `xcsh` theme.
...
xcsh comes with several built-in themes.
...
- Want xcsh to match their terminal's appearance
...
xcsh supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/xcsh/themes/*.json` or `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.xcsh/themes/*.json`
4. **Current working directory** - Defined in `./.xcsh/themes/*.json`
