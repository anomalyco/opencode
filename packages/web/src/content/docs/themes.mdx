...
### Auto-selecting a theme based on the active model or provider

You can automatically change the TUI theme based on the currently selected model or provider. This is configured in `opencode.json`. When a model is selected, OpenCode will look for a theme in this order: `model theme` → `provider theme` → `tui theme` → `default opencode theme`.

```json title="opencode.json" {6,11}
{
