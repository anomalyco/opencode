...
With ChaliceCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, ChaliceCode uses our own `opencode` theme.
...
ChaliceCode comes with several built-in themes.
...
- Want ChaliceCode to match their terminal's appearance
...
ChaliceCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/chalice-code/themes/*.json` or `$XDG_CONFIG_HOME/chalice-code/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.chalicecode/themes/*.json`
4. **Current working directory** - Defined in `./.chalicecode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/chalice-code/themes
vim ~/.config/chalice-code/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .chalicecode/themes
vim .chalicecode/themes/my-theme.json
```
...
{
  "$schema": "https://chalice-code.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
