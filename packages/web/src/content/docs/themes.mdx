...
With Pencode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Pencode uses our own `pencode` theme.
...
Pencode comes with several built-in themes.
...
- Want Pencode to match their terminal's appearance
...
Pencode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/pencode/themes/*.json` or `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.pencode/themes/*.json`
4. **Current working directory** - Defined in `./.pencode/themes/*.json`
