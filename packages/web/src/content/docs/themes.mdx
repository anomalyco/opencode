...
With CrazyCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CrazyCode uses our own `crazycode` theme.
...
CrazyCode comes with several built-in themes.
...
- Want CrazyCode to match their terminal's appearance
...
CrazyCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/crazycode/themes/*.json` or `$XDG_CONFIG_HOME/crazycode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.crazycode/themes/*.json`
4. **Current working directory** - Defined in `./.crazycode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/crazycode/themes
vim ~/.config/crazycode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .crazycode/themes
vim .crazycode/themes/my-theme.json
```
...
{
  "$schema": "https://crazycode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
