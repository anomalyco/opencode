...
With Leak Code you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Leak Code uses our own `leak-code` theme.
...
Leak Code comes with several built-in themes.
...
- Want Leak Code to match their terminal's appearance
...
Leak Code supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/leak-code/themes/*.json` or `$XDG_CONFIG_HOME/leak-code/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.leak-code/themes/*.json`
4. **Current working directory** - Defined in `./.leak-code/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/leak-code/themes
vim ~/.config/leak-code/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .leak-code/themes
vim .leak-code/themes/my-theme.json
```
