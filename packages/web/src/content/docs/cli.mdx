...
Manage automations from the CLI.

```bash
opencode automation [command]
```
...
| Flag          | Short | Description                          |
| ------------- | ----- | ------------------------------------ |
| `--max-count` | `-n`  | Limit to N most recent automations   |
| `--format`    |       | Output format: table or json (table) |
...
```bash
opencode automation create --name "Daily summary" --prompt "Summarize changes" --project /path/to/project --schedule "0 9 * * 1-5"
```
...
| `--project`  | Project directory (repeat) |
...
```bash
opencode automation update <id> --name "Weekly review"
```
...
```bash
opencode automation history <id>
```
...
```bash
opencode automation export --all
opencode automation export <id>
```
...
| Flag        | Description                                |
| ----------- | ------------------------------------------ |
| `--all`     | Export all automations                     |
| `--dir`     | Export to a directory                      |
| `--project` | Export to .opencode/automations in project |
...
```bash
opencode automation import automations.json
opencode automation import --dir .opencode/automations
```
...
| Flag        | Description                                  |
| ----------- | -------------------------------------------- |
| `--dir`     | Import all JSON files from a directory       |
| `--project` | Import from .opencode/automations in project |
