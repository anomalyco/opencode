...
Opens the TUI directly on a screen listing sessions from every project stored in the global database. Use the arrow keys to navigate, `enter` or `right` to open the selected session, and `left` (with an empty prompt) inside a session to return to the list. Opening a session switches the TUI working context to the directory the session was started in.

A `new session` input sits at the bottom of the list (focus it with `ctrl+o` or `tab`). Type a prompt to start a session in the current directory, or start with `@path` to choose the working directory of the new session — the text after the path becomes the first prompt:
