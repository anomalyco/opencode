...
Use `--bare` for deterministic automation that should not inherit user or project configuration:

```bash
OPENCODE_CONFIG=./ci/opencode.json opencode run --bare "Review this change"
```

Bare runs keep explicit configuration, authentication, and managed organization policy. They skip automatically
discovered user and project configuration, external plugins and skills, instruction files such as `AGENTS.md`,
Well-known remote configuration, configuration-directory dependency installation, and LSP startup. Bare mode is not
a sandbox or offline mode. It starts a new local, non-interactive session and cannot be combined with `--attach`,
`--continue`, `--session`, `--fork`, or `--interactive`.
...
| <nobr><code>{"--auto"}</code></nobr>     |       | Auto-approve permissions that are not explicitly denied                     |
| <nobr><code>{"--bare"}</code></nobr>     |       | Run without automatic user or project configuration and external extensions |
