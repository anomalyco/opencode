...
| <nobr><code>{"--ephemeral"}</code></nobr>   |       | Do not persist sessions created in this instance after exiting          |
...
For fire-and-forget runs that shouldn't clutter your session history, use `--ephemeral`. The session never appears in session lists, cannot be shared, and is deleted when the run completes, fails, or is interrupted:

```bash
opencode run --ephemeral "Suggest a commit message for the staged changes"
```

This is useful in scripts, CI pipelines, and shell integrations where each invocation would otherwise leave a session behind. If the process is killed before it can clean up, the leftover session is still hidden from lists and is deleted automatically on a later startup. Since ephemeral sessions are fire-and-forget, `--ephemeral` cannot be combined with `--continue`, `--session`, `--attach`, or `--share`.

The interactive interfaces support it too — `opencode --ephemeral` (or `opencode --mini --ephemeral`) starts a scratch instance whose sessions are deleted when you exit.
...
| <nobr><code>{"--ephemeral"}</code></nobr> |       | Do not persist the session after the run completes                         |
| <nobr><code>{"--model"}</code></nobr>     | `-m`  | Model to use in the form of provider/model                                 |
| <nobr><code>{"--agent"}</code></nobr>     |       | Agent to use                                                               |
