...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `IMECODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `IMECODE_SERVER_USERNAME` or `opencode`) |
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `IMECODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `IMECODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `IMECODE_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `IMECODE_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `IMECODE_CONFIG`                     | string  | Path to config file                               |
| `IMECODE_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `IMECODE_CONFIG_DIR`                 | string  | Path to config directory                          |
| `IMECODE_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `IMECODE_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `IMECODE_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `IMECODE_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `IMECODE_PERMISSION`                 | string  | Inlined json permissions config                   |
| `IMECODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `IMECODE_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `IMECODE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `IMECODE_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `IMECODE_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `IMECODE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `IMECODE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `IMECODE_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `IMECODE_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `IMECODE_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `IMECODE_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `IMECODE_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `IMECODE_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `IMECODE_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`) |
| `IMECODE_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `IMECODE_EXPERIMENTAL`                         | boolean | Enable the experimental umbrella flag   |
| `IMECODE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `IMECODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `IMECODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `IMECODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `IMECODE_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `IMECODE_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `IMECODE_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `IMECODE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `IMECODE_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `IMECODE_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `IMECODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
| `IMECODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagent tasks        |
| `IMECODE_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Enable experimental event system        |
| `IMECODE_EXPERIMENTAL_NATIVE_LLM`              | boolean | Enable native LLM request path          |
| `IMECODE_EXPERIMENTAL_PARALLEL`                | boolean | Enable parallel web search execution    |
| `IMECODE_EXPERIMENTAL_SCOUT`                   | boolean | Enable Scout subagent                   |
| `IMECODE_EXPERIMENTAL_WORKSPACES`              | boolean | Enable workspace support                |
