...
Costs are the base rate per 1M tokens, before any context-tier pricing. The default output stays one plain `provider/model` per line, so it can still be piped into other commands.
