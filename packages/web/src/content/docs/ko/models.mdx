...
전체 ID는 `provider_id/model_id`입니다. 예를 들어, [OpenCode Zen](./zen)을 사용한다면, GPT 5.1 Codex에 `opencode/gpt-5.1-codex`를 사용할 수 있습니다.

[custom Provider](./providers#custom)를 구성하면 `provider_id`는 구성의 `provider` 부분에서 키이며 `model_id`는 `provider.models`에서 키입니다.
...
사용중인 에이전트에 이러한 옵션을 구성할 수 있습니다. 에이전트 구성은 여기에 모든 글로벌 옵션을 overrides. [더 알아보기](./agents/#additional).
...
keybind `variant_cycle`를 사용하여 변형 사이 신속하게 전환합니다. [더 알아보기](./keybinds).
