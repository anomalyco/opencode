...
opencode는 인기있는 언어 및 프레임 워크에 대한 몇 가지 내장 포매터와 함께 제공됩니다. 아래는 포매터, 지원된 파일 확장 및 명령 또는 구성 옵션의 목록입니다.
...
1. 모든 활성화된 포매터에 대한 파일 확장을 확인합니다.
...
opencode config의 `formatter` 섹션을 통해 포매터를 사용자 정의 할 수 있습니다.
...
| 속성          | 유형     | 묘사                               |
| ------------- | -------- | ---------------------------------- |
| `disabled`    | boolean  | `true`로 설정 시 포매터 비활성화   |
| `command`     | string[] | 포매터를 실행하는 명령             |
| `environment` | object   | 포매터 실행 시 적용할 환경 변수    |
| `extensions`  | string[] | 포매터가 처리할 파일 확장자        |
...
**모든** 포매터를 전역적으로 비활성화하려면 `formatter`를 `false`로 설정:
...
**특정** 포매터를 비활성화하려면 `disabled`를 `true`로 설정:
...
내장 포매터를 무시하거나 명령, 환경 변수 및 파일 확장을 지정하여 새로운 것을 추가 할 수 있습니다.
