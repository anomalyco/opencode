...
로컬 설정 값은 원격 기본값을 무시합니다. 자세한 내용은 [config precedence](./config#precedence-order)를 참조하십시오.
...
1. 글로벌 도구로 사용 가능.
2. [agent config](./agents#tools)에서 MCP 서버를 도구로 사용할 수 있습니다.
...
대안으로, 당신은이 같은 뭔가를 추가 할 수 있습니다 [AGENTS.md](./rules/).
