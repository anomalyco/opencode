...
끝점. 이 엔드포인트는 [SDK](./sdk)을 생성하는 데도 사용됩니다.
...
TUI를 시작하면 무작위로 포트와 호스트 이름을 할당합니다. 대신 `--hostname`와 `--port` [flags](./cli)에서 전달할 수 있습니다. 그런 다음 서버에 연결하십시오.

[`/tui`](#tui) 엔드포인트는 서버를 통해 TUI를 구동하는 데 사용될 수 있습니다. 예를 들어 미리 작성하거나 프롬프트를 실행할 수 있습니다. 이 설정은 OpenCode [IDE](./ide) 플러그인에 의해 사용됩니다.
