...
:::caution
`LEAKCODE_SERVER_PASSWORD`가 설정되지 않은 경우 서버가 중단되지 않습니다. 이것은 로컬 사용을위한 괜찮지만 네트워크 액세스를 설정해야합니다.
:::

:::tip[Windows 사용자]
최고의 경험을 위해서는 PowerShell보다 `leak-code web`를 실행하십시오. 적절한 파일 시스템 액세스 및 terminal 통합을 보장합니다.
:::
...
액세스를 보호하려면 `LEAKCODE_SERVER_PASSWORD` 환경 변수를 사용하여 암호를 설정하십시오.

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

`opencode`에 대한 사용자의 기본값은 `LEAKCODE_SERVER_USERNAME`로 변경할 수 있습니다.
...
`leak-code.json` config 파일에서 서버 설정을 구성할 수 있습니다:
