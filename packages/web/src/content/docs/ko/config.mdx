...
2. **Global config**(`~/.config/leak-code/leak-code.json`) - 사용자 기본 설정
3. **Custom config**(`LEAKCODE_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `leak-code.json`) - 프로젝트별 설정
5. **`.leak-code` directories** - agents, commands, plugins
6. **Inline config**(`LEAKCODE_CONFIG_CONTENT` env var) - 런타임 override

즉, 프로젝트 config는 전역 기본값을 override할 수 있고, 전역 config는 조직의 Remote 기본값을 override할 수 있습니다.

:::note
`.leak-code` 및 `~/.config/leak-code` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
전역 OpenCode config는 `~/.config/leak-code/leak-code.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.

TUI 관련 설정은 `~/.config/leak-code/tui.json`을 사용하세요.
...
프로젝트 루트에 `leak-code.json`을 추가하세요. 프로젝트 config는 표준 config 파일 중 우선순위가 가장 높아 전역 및 Remote config를 모두 override합니다.
...
`LEAKCODE_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`LEAKCODE_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.leak-code` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
custom 디렉토리는 전역 config와 `.leak-code` 디렉토리 뒤에 로드되므로 해당 설정을 **override할 수 있습니다**.
...
`LEAKCODE_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.

`leak-code.json`의 기존 `theme`, `keybinds`, `tui` 키는 더 이상 사용되지 않으며(deprecated) 가능한 경우 자동으로 마이그레이션됩니다.
...
`server` 옵션으로 `leak-code serve`와 `leak-code web` 명령의 server 설정을 구성할 수 있습니다.
...
`~/.config/leak-code/agents/` 또는 `.leak-code/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
이 설정은 TUI, CLI(`leak-code run`), 데스크톱 앱, GitHub Action 등 모든 인터페이스에 적용됩니다.
...
`~/.config/leak-code/commands/` 또는 `.leak-code/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
plugin 파일은 `.leak-code/plugins/` 또는 `~/.config/leak-code/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
