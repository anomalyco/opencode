...
- `bash:unsandboxed` — 샌드박스 거부 후 또는 명시적 비샌드박스 요청 후 셸 명령을 샌드박스 밖에서 다시 실행
...
macOS 샌드박싱이 활성화되면, 차단된 bash 명령은 별도의 `bash:unsandboxed` 권한 요청을 트리거할 수 있습니다.
이는 OpenCode가 샌드박스 거부를 감지하거나, 명령이 첫 번째 비어있지 않은 줄에 `# opencode:unsandboxed <reason>`을 넣어 명시적으로 샌드박스를 건너뛰도록 요청할 때 발생합니다.
샌드박스 자체는 [`experimental.sandbox`](/docs/config#sandbox)에서 설정하세요.
