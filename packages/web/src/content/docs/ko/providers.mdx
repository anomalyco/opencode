...
`~/.local/share/leak-code/auth.json`
...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   프로젝트별 또는 영구 구성을 위해 `leak-code.json`을 사용하십시오:
...
OpenCode를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `leak-code.json` 파일에 다음을 추가하십시오.
...
4. 프로젝트 디렉토리에 `leak-code.json` 파일을 만들거나 업데이트하십시오.
...
1. **인증 설정 확인**: `leak-code auth list`를 실행하여 공급자의 자격 증명이 구성에 추가되었는지 확인하십시오.
