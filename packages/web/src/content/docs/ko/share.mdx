...
[config 파일](./config)에서 수동 모드를 명시적으로 설정하려면:
...
`share` 옵션을 설정하여 모든 새로운 대화에 대한 자동 공유를 활성화 할 수 있습니다 `"auto"` 당신의 [config 파일](./config):
...
`share` 옵션을 설정하여 완전히 공유할 수 있습니다. `"disabled"`에 [config file](./config):
...
[Learn more](./enterprise) 귀하의 조직에서 opencode를 사용하여.
