---
title: LSP 서버
description: CyberStrike는 LSP 서버와 통합되어 작동합니다.
---

CyberStrike는 언어 서버 프로토콜(LSP)과 통합되어 LLM이 코드베이스와 상호 작용할 수 있게 돕습니다. 이를 통해 진단 정보를 사용하여 LLM에 피드백을 제공합니다.

---

## 기본 제공 (Built-in)

CyberStrike는 인기 있는 언어들에 대해 여러 내장 LSP 서버를 제공합니다.
...
:::note
`CYBERSTRIKE_DISABLE_LSP_DOWNLOAD` 환경 변수를 `true`로 설정하여 자동 LSP 서버 다운로드를 비활성화 할 수 있습니다.
:::
...
CyberStrike가 파일을 열 때, 다음과 같이 작동합니다:
...
CyberStrike 설정의 `lsp` 섹션을 통해 LSP 서버를 사용자 정의할 수 있습니다.
