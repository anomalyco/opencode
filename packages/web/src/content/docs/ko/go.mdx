...
3. 라우팅과 프롬프트 캐싱을 최적화할 수 있도록 각 대화에서 안정적인 세션 ID를 `x-opencode-session`으로 전송합니다.
   요청마다 또는 프로세스마다 새 ID를 만들지 말고, 보조 호출을 포함한 대화의 모든 요청에서 같은 ID를 재사용하며,
   클라이언트가 사용하는 모든 엔드포인트(`/responses`, `/chat/completions`, `/messages`)에 전송합니다.
   하나의 SDK 미들웨어에서만 추가한 헤더는 다른 엔드포인트에는 전달되지 않습니다.
...
| DeepSeek V4.1 Flash (Off-Peak)          | $0.15  | $0.60  | $0.003      | -            | ~~$15~~ **$60**<br /><small>4x · 9월 27일 종료</small> |
| DeepSeek V4.1 Flash (Peak)              | $0.30  | $1.20  | $0.006      | -            | ~~$15~~ **$60**<br /><small>4x · 9월 27일 종료</small> |
...
| DeepSeek V4.1 Flash<br /><small>4x · 9월 27일 종료</small> | ~~6,500~~<br />**26,000** | ~~16,250~~<br />**65,000** | ~~32,500~~<br />**130,000** |
