...
이 한도는 모든 모델이 공유하는 사용 할당량에 대한 비율입니다.

### 모델별 사용량

포함된 월간 사용량은 모델마다 다르며 공식 API 가격을 기준으로 금액을 산정합니다. 토큰 가격은 1M tokens 기준입니다.

| Model                                   | Input  | Output | Cached Read | Cached Write | 월간 사용량 |
| --------------------------------------- | ------ | ------ | ----------- | ------------ | ----- |
...
사용 가능한 요청 횟수는 모델의 월간 사용량과 API 가격에 따라 달라집니다. 아래 예상치는 일반적인 Go 사용 패턴을 기준으로 합니다.
