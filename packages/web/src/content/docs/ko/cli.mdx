...
| <nobr><code>{"--password"}</code></nobr> | `-p` | 기본 인증 비밀번호 (기본값: `OCTOPUS_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 기본 인증 사용자 이름 (기본값: `OCTOPUS_SERVER_USERNAME` 또는 `opencode`) |
...
Octopus는 [Models.dev](https://models.dev)의 provider 목록을 기반으로 동작하므로, `opencode auth login`으로 원하는 provider의 API 키를 설정할 수 있습니다. 인증 정보는 `~/.local/share/octopus/auth.json`에 저장됩니다.

```bash
opencode auth login
```

Octopus 시작 시 인증 파일에서 provider 정보를 불러오며, 시스템 환경 변수나 프로젝트의 `.env`에 정의된 키도 함께 로드합니다.
...
`--refresh` 플래그를 사용하면 캐시된 모델 목록을 갱신할 수 있습니다. provider에 새 모델이 추가된 뒤 Octopus에서 바로 확인하고 싶을 때 유용합니다.
...
API 접근용 headless Octopus 서버를 시작합니다. 전체 HTTP 인터페이스는 [server docs](/docs/server)를 참고하세요.

```bash
opencode serve
```

이 명령은 TUI 없이 opencode 기능에 접근할 수 있는 HTTP 서버를 시작합니다. `OCTOPUS_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).
...
Octopus 세션의 토큰 사용량과 비용 통계를 표시합니다.
...
JSON 파일 또는 Octopus 공유 URL에서 세션 데이터를 가져옵니다.

```bash
opencode import <file>
```

로컬 파일이나 Octopus 공유 URL에서 가져올 수 있습니다.
...
웹 인터페이스를 포함한 headless Octopus 서버를 시작합니다.

```bash
opencode web
```

이 명령은 HTTP 서버를 시작하고 웹 브라우저를 열어 웹 인터페이스로 Octopus에 접속합니다. `OCTOPUS_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).
...
Octopus는 환경 변수로도 구성할 수 있습니다.

| 변수                                  | 타입    | 설명                                           |
| ------------------------------------- | ------- | ---------------------------------------------- |
| `OCTOPUS_AUTO_SHARE`                 | boolean | 세션 자동 공유                                 |
| `OCTOPUS_GIT_BASH_PATH`              | string  | Windows에서 Git Bash 실행 파일 경로            |
| `OCTOPUS_CONFIG`                     | string  | 설정 파일 경로                                 |
| `OCTOPUS_CONFIG_DIR`                 | string  | 설정 디렉터리 경로                             |
| `OCTOPUS_CONFIG_CONTENT`             | string  | 인라인 JSON 설정 내용                          |
| `OCTOPUS_DISABLE_AUTOUPDATE`         | boolean | 자동 업데이트 확인 비활성화                    |
| `OCTOPUS_DISABLE_PRUNE`              | boolean | 오래된 데이터 정리(prune) 비활성화             |
| `OCTOPUS_DISABLE_TERMINAL_TITLE`     | boolean | 터미널 제목 자동 업데이트 비활성화             |
| `OCTOPUS_PERMISSION`                 | string  | 인라인 JSON 권한 설정                          |
| `OCTOPUS_DISABLE_DEFAULT_PLUGINS`    | boolean | 기본 플러그인 비활성화                         |
| `OCTOPUS_DISABLE_LSP_DOWNLOAD`       | boolean | LSP 서버 자동 다운로드 비활성화                |
| `OCTOPUS_ENABLE_EXPERIMENTAL_MODELS` | boolean | 실험적 모델 활성화                             |
| `OCTOPUS_DISABLE_AUTOCOMPACT`        | boolean | 자동 컨텍스트 컴팩션 비활성화                  |
| `OCTOPUS_DISABLE_CLAUDE_CODE`        | boolean | `.claude`(프롬프트 + 스킬) 읽기 비활성화       |
| `OCTOPUS_DISABLE_CLAUDE_CODE_PROMPT` | boolean | `~/.claude/CLAUDE.md` 읽기 비활성화            |
| `OCTOPUS_DISABLE_CLAUDE_CODE_SKILLS` | boolean | `.claude/skills` 로드 비활성화                 |
| `OCTOPUS_DISABLE_MODELS_FETCH`       | boolean | 원격 소스에서 모델 목록 가져오기 비활성화      |
| `OCTOPUS_FAKE_VCS`                   | string  | 테스트용 가짜 VCS provider                     |
| `OCTOPUS_CLIENT`                     | string  | 클라이언트 식별자(기본값: `cli`)               |
| `OCTOPUS_ENABLE_EXA`                 | boolean | Exa 웹 검색 도구 활성화                        |
| `OCTOPUS_SERVER_PASSWORD`            | string  | `serve`/`web` 기본 인증 활성화                 |
| `OCTOPUS_SERVER_USERNAME`            | string  | 기본 인증 사용자명 오버라이드(기본 `opencode`) |
| `OCTOPUS_MODELS_URL`                 | string  | 모델 설정을 가져올 사용자 지정 URL             |
...
| 변수                                            | 타입    | 설명                             |
| ----------------------------------------------- | ------- | -------------------------------- |
| `OCTOPUS_EXPERIMENTAL`                         | boolean | 모든 실험 기능 활성화            |
| `OCTOPUS_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 아이콘 탐색 활성화               |
| `OCTOPUS_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | TUI에서 선택 시 복사 비활성화    |
| `OCTOPUS_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 명령 기본 타임아웃(ms)      |
| `OCTOPUS_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 응답 최대 출력 토큰 수       |
| `OCTOPUS_EXPERIMENTAL_FILEWATCHER`             | boolean | 전체 디렉터리 파일 감시 활성화   |
| `OCTOPUS_EXPERIMENTAL_OXFMT`                   | boolean | oxfmt 포매터 활성화              |
| `OCTOPUS_EXPERIMENTAL_LSP_TOOL`                | boolean | 실험적 LSP 도구 활성화           |
| `OCTOPUS_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 파일 감시 비활성화               |
| `OCTOPUS_EXPERIMENTAL_EXA`                     | boolean | 실험적 Exa 기능 활성화           |
| `OCTOPUS_EXPERIMENTAL_LSP_TY`                  | boolean | python 파일에 대해 TY LSP 활성화 |
| `OCTOPUS_EXPERIMENTAL_MARKDOWN`                | boolean | 실험적 Markdown 기능 활성화      |
| `OCTOPUS_EXPERIMENTAL_PLAN_MODE`               | boolean | Plan mode 활성화                 |
