...
기본적으로 OpenCode CLI는 어떤 인수 없이 실행할 때 [TUI](./tui)를 시작합니다.
...
필요한 GitHub Actions 워크플로우를 설정하고 구성 프로세스를 통해 안내합니다. [더 알아보기](./github).
...
이것은 [your config](./config/)에서 사용하는 정확한 모델 이름을 파악하는 데 유용합니다.
...
API 액세스를 위한 headless OpenCode 서버를 시작합니다. 완전한 HTTP 인터페이스를 위해 [server docs](./server)를 체크하십시오.
