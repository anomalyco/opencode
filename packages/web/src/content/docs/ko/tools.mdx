...
도구는 LLM을 사용하여 코드베이스에서 작업을 수행 할 수 있습니다. OpenCode는 내장 도구 세트로 제공되지만, [custom tools](./custom-tools) 또는 [MCP 서버](./mcp-servers)로 확장할 수 있습니다.

기본적으로 모든 도구는**enabled**이며, 실행할 권한이 없습니다. [permissions](./permissions)를 통해 공구 동작을 제어할 수 있습니다.
...
[Learn more](./permissions) configuring 권한에 대해.
...
LSP 서버가 프로젝트에 사용할 수 있는 구성하려면 [LSP Servers](./lsp)를 참조하십시오.
...
[skill](./skills) (`SKILL.md` 파일)을로드하고 대화의 내용을 반환합니다.
...
:::note
이 도구는 기본으로 시약을 비활성화하지만 수동으로 활성화 할 수 있습니다. [더 알아보기](./agents/#permissions)
:::
...
[Learn more](./custom-tools) 사용자 정의 도구 만들기.
...
MCP 서버 구성에 대한 [Learn more](./mcp-servers).
