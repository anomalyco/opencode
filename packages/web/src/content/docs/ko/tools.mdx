...
기본적으로 `grep` 및 `glob` 같은 도구는 `.gitignore` 패턴을 존중하며 `.gitignore`에 나열된 파일 및 디렉토리를 검색 및 목록에서 제외합니다.
...
예를 들어, 이 `.ignore` 파일은 `node_modules/`, `dist/`, `build/` 디렉토리가 `.gitignore`에 있어도 검색할 수 있게 해줍니다.
