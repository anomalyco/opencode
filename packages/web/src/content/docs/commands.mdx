...
Create `.leak-code/commands/test.md`:
...
You can add custom commands through the Leak Code config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Leak Code [config](/docs/config):
...
- Per-project: `.leak-code/commands/`
...
leak-code includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
