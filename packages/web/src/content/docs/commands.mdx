...
Create text files in the `commands/` directory to define custom commands.
...
OpenCode discovers text-based command files such as `.md`, `.txt`, `.prompt`, or extensionless scripts.
...
You can add custom commands through the OpenCode config or by creating text-based files in the `commands/` directory.
...
You can also define commands using text-based files. Place them in:

- Global: `~/.config/opencode/commands/`
- Per-project: `.opencode/commands/`

```markdown title="~/.config/opencode/commands/test.txt"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
The file name becomes the command name. For example, `test.txt` lets
you run:
