...
Create `.pencode/commands/test.md`:
...
You can add custom commands through the Pencode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Pencode [config](/docs/config):
...
pencode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
