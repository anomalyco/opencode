...
La mayoría de los comandos también tienen una combinación de teclas que utiliza `ctrl+x` como tecla principal, donde `ctrl+x` es la tecla principal predeterminada. [Más información](./keybinds).
...
Cree o actualice el archivo `AGENTS.md`. [Más información](./rules).
...
Compartir la sesión actual. [Más información](./share).
...
Dejar de compartir la sesión actual. [Más información](./share#un-sharing).
