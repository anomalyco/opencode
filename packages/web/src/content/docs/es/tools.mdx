...
:::note
Cuando el sandbox de macOS está habilitado, bash se ejecuta con restricciones del sistema de archivos y puede solicitar un reintento separado sin sandbox cuando sea necesario.
Si sabe que un comando debe iniciarse fuera del sandbox, ponga `# opencode:unsandboxed <motivo>` en la primera línea no vacía del comando.
Consulte la [configuración del sandbox](/docs/config#sandbox) para el comportamiento y los límites admitidos.
:::
