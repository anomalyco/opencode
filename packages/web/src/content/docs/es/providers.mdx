...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `leak-code.json`:
...
Archivo `leak-code.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `leak-code.json`:
...
4. Cree o actualice su archivo `leak-code.json` en el directorio de su proyecto:
...
1. **Verifique la configuración de autenticación**: Ejecute `leak-code auth list` para ver si las credenciales
