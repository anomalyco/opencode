---
title: Proveedores
description: Usando cualquier proveedor LLM en Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

Octopus usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Octopus.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Octopus
configuración.
...
Octopus Zen es una lista de modelos proporcionados por el equipo Octopus que han sido
probado y verificado para funcionar bien con Octopus. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Octopus Zen.
:::
...
Funciona como cualquier otro proveedor en Octopus y su uso es completamente opcional.

---

## Octopus Go

Octopus Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Octopus que han sido
probado y verificado para funcionar bien con Octopus.

1. Ejecute el comando `/connect` en TUI, seleccione `Octopus Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con Octopus:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `octopus.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Octopus no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Octopus utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Octopus
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `octopus.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `octopus.json`:
...
Para utilizar Google Vertex AI con Octopus:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Octopus como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Octopus enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para Octopus. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Octopus:
...
3. Copie la clave API para usarla en Octopus.
...
6. **Importante**: Antes de usar modelos en la nube en Octopus, debe obtener la información del modelo localmente:
...
Octopus Zen es una lista de modelos probados y verificados proporcionada por el equipo Octopus. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Octopus Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Octopus Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Octopus API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Octopus:
...
4. Cree o actualice su archivo `octopus.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Octopus comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
