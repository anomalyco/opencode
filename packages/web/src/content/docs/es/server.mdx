---
title: Servidor
description: Interactuar con el servidor cyberstrike a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `cyberstrike serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente cyberstrike puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `cyberstrike.local` |
...
Configure `CYBERSTRIKE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `cyberstrike`, o configure `CYBERSTRIKE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `cyberstrike serve` como a `cyberstrike web`.
...
Cuando ejecuta `cyberstrike`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor cyberstrike para interactuar con cyberstrike mediante programación.
:::

Esta arquitectura permite que cyberstrike admita múltiples clientes y le permite interactuar con cyberstrike mediante programación.

Puede ejecutar `cyberstrike serve` para iniciar un servidor independiente. Si tienes el
cyberstrike TUI ejecutándose, `cyberstrike serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CyberStrike [IDE](/docs/ide).
...
El servidor cyberstrike expone las siguientes API.
