---
title: Servidor
description: Interactuar con el servidor pencode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `pencode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente pencode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `pencode.local` |
...
Configure `PENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `pencode`, o configure `PENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `pencode serve` como a `pencode web`.
...
Cuando ejecuta `pencode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor pencode para interactuar con pencode mediante programación.
:::

Esta arquitectura permite que pencode admita múltiples clientes y le permite interactuar con pencode mediante programación.

Puede ejecutar `pencode serve` para iniciar un servidor independiente. Si tienes el
pencode TUI ejecutándose, `pencode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Pencode [IDE](/docs/ide).
...
El servidor pencode expone las siguientes API.
