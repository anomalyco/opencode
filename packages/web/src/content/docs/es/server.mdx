---
title: Servidor
description: Interactuar con el servidor xcsh a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `xcsh serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente xcsh puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `xcsh.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `xcsh`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `xcsh serve` como a `xcsh web`.
...
Cuando ejecuta `xcsh`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor xcsh para interactuar con xcsh mediante programación.
:::

Esta arquitectura permite que xcsh admita múltiples clientes y le permite interactuar con xcsh mediante programación.

Puede ejecutar `xcsh serve` para iniciar un servidor independiente. Si tienes el
xcsh TUI ejecutándose, `xcsh serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos xcsh [IDE](/docs/ide).
...
El servidor xcsh expone las siguientes API.
