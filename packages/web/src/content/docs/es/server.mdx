...
El comando `leak-code serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente opencode puede usar.
...
Configure `LEAKCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `opencode`, o configure `LEAKCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `leak-code serve` como a `leak-code web`.
...
Puede ejecutar `leak-code serve` para iniciar un servidor independiente. Si tienes el
opencode TUI ejecutándose, `leak-code serve` iniciará un nuevo servidor.
