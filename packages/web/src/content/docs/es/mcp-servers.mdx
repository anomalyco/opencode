...
Puede agregar herramientas externas a xcsh utilizando el _Model Context Protocol_ o MCP. xcsh admite servidores locales y remotos.
...
Puede definir servidores MCP en su [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/xcsh`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
xcsh maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, xcsh:
...
Si el servidor requiere autenticación, xcsh le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `xcsh mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, xcsh almacenará los tokens de forma segura en `~/.local/share/xcsh/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en xcsh, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración xcsh como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar xcsh a su cuenta Sentry.
