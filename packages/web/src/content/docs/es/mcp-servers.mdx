...
Puede agregar herramientas externas a CyberStrike utilizando el _Model Context Protocol_ o MCP. CyberStrike admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CyberStrike Config](https://cyberstrike.io/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/cyberstrike`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
CyberStrike maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CyberStrike:
...
Si el servidor requiere autenticación, CyberStrike le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `cyberstrike mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CyberStrike almacenará los tokens de forma segura en `~/.local/share/cyberstrike/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CyberStrike, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CyberStrike como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CyberStrike a su cuenta Sentry.
