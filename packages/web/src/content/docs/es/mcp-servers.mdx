...
Puede agregar herramientas externas a Octopus utilizando el _Model Context Protocol_ o MCP. Octopus admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Octopus Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Octopus maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Octopus:
...
Si el servidor requiere autenticación, Octopus le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Octopus almacenará los tokens de forma segura en `~/.local/share/octopus/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Octopus, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Octopus como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Octopus a su cuenta Sentry.
