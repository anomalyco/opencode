---
title: Complementos
description: Escriba sus propios complementos para extender CyberStrike.
---

Los complementos le permiten extender CyberStrike al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CyberStrike.
...
- `.cyberstrike/plugins/` - Complementos a nivel de proyecto
- `~/.config/cyberstrike/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/cyberstrike/node_modules/`.
...
1. Configuración global (`~/.config/cyberstrike/cyberstrike.json`)
2. Configuración del proyecto (`cyberstrike.json`)
3. Directorio global de complementos (`~/.config/cyberstrike/plugins/`)
4. Directorio de complementos del proyecto (`.cyberstrike/plugins/`)
...
CyberStrike ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK cyberstrike para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar cyberstrike.
...
:::note
Si está utilizando la aplicación de escritorio CyberStrike, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que cyberstrike lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a cyberstrike:
...
El ayudante `tool` crea una herramienta personalizada a la que cyberstrike puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para cyberstrike junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://cyberstrike.io/docs/sdk) para obtener más detalles.
