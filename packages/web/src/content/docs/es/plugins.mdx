---
title: Complementos
description: Escriba sus propios complementos para extender Octopus.
---

Los complementos le permiten extender Octopus al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Octopus.
...
- `.octopus/plugins/` - Complementos a nivel de proyecto
- `~/.config/octopus/plugins/` - Complementos globales
...
1. Configuración global (`~/.config/octopus/octopus.json`)
2. Configuración del proyecto (`octopus.json`)
3. Directorio global de complementos (`~/.config/octopus/plugins/`)
4. Directorio de complementos del proyecto (`.octopus/plugins/`)
...
Octopus ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
:::note
Si está utilizando la aplicación de escritorio Octopus, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
