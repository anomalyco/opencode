---
title: Complementos
description: Escriba sus propios complementos para extender xcsh.
---

Los complementos le permiten extender xcsh al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de xcsh.
...
- `.xcsh/plugins/` - Complementos a nivel de proyecto
- `~/.config/xcsh/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/xcsh/node_modules/`.
...
1. Configuración global (`~/.config/xcsh/xcsh.json`)
2. Configuración del proyecto (`xcsh.json`)
3. Directorio global de complementos (`~/.config/xcsh/plugins/`)
4. Directorio de complementos del proyecto (`.xcsh/plugins/`)
...
xcsh ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK xcsh para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar xcsh.
...
:::note
Si está utilizando la aplicación de escritorio xcsh, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que xcsh lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a xcsh:
...
El ayudante `tool` crea una herramienta personalizada a la que xcsh puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para xcsh junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) para obtener más detalles.
