---
title: Complementos
description: Escriba sus propios complementos para extender Pencode.
---

Los complementos le permiten extender Pencode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Pencode.
...
- `.pencode/plugins/` - Complementos a nivel de proyecto
- `~/.config/pencode/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/pencode/node_modules/`.
...
1. Configuración global (`~/.config/pencode/pencode.json`)
2. Configuración del proyecto (`pencode.json`)
3. Directorio global de complementos (`~/.config/pencode/plugins/`)
4. Directorio de complementos del proyecto (`.pencode/plugins/`)
...
Pencode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK pencode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar pencode.
...
:::note
Si está utilizando la aplicación de escritorio Pencode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que pencode lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a pencode:
...
El ayudante `tool` crea una herramienta personalizada a la que pencode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para pencode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://pencode.ai/docs/sdk) para obtener más detalles.
