...
Con xcsh puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, xcsh usa nuestro propio tema `xcsh`.
...
xcsh viene con varios temas integrados.
...
- Quiere que xcsh coincida con la apariencia de su terminal
...
xcsh admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/xcsh/themes/*.json` o `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.xcsh/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.xcsh/themes/*.json`
