...
Con Pencode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Pencode usa nuestro propio tema `pencode`.
...
Pencode viene con varios temas integrados.
...
- Quiere que Pencode coincida con la apariencia de su terminal
...
Pencode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/pencode/themes/*.json` o `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.pencode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.pencode/themes/*.json`
