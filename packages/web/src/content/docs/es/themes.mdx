...
Con CyberStrike puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, CyberStrike usa nuestro propio tema `cyberstrike`.
...
CyberStrike viene con varios temas integrados.
...
- Quiere que CyberStrike coincida con la apariencia de su terminal
...
CyberStrike admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/cyberstrike/themes/*.json` o `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.cyberstrike/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.cyberstrike/themes/*.json`
