...
Con Octopus puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Octopus usa nuestro propio tema `opencode`.
...
Octopus viene con varios temas integrados.
...
- Quiere que Octopus coincida con la apariencia de su terminal
...
Octopus admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/octopus/themes/*.json` o `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.octopus/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.octopus/themes/*.json`
