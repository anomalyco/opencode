---
title: CLI
description: Opciones y comandos de la CLI de xcsh.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de xcsh de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
xcsh
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con xcsh mediante programación.
...
Inicie la interfaz de usuario del terminal xcsh.
...
La CLI de xcsh también tiene los siguientes comandos.

---

### agent

Administrar agentes para xcsh.
...
Conecte una terminal a un servidor backend xcsh que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
xcsh attach [url]
```

Esto permite usar TUI con un backend remoto xcsh. Por ejemplo:
...
xcsh funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `xcsh auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

Cuando se inicia xcsh, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en xcsh.
...
Ejecute xcsh en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `xcsh serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| `--attach`   |       | Adjuntar a un servidor xcsh en ejecución (por ejemplo, http://localhost:4096)   |
...
Inicie un servidor xcsh sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
xcsh serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad xcsh sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `xcsh`).
...
Enumere todas las sesiones xcsh.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones xcsh.
...
Importe datos de sesión desde un archivo JSON o una URL compartida xcsh.

```bash
xcsh import <file>
```

Puede importar desde un archivo local o una URL compartida xcsh.
...
Inicie un servidor xcsh sin cabeza con una interfaz web.

```bash
xcsh web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a xcsh a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `xcsh`).
...
Desinstale xcsh y elimine todos los archivos relacionados.
...
Actualiza xcsh a la última versión o a una versión específica.
...
La CLI de xcsh toma las siguientes banderas globales.
...
xcsh se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `xcsh`) |
