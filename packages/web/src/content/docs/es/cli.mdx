...
El OpenCode CLI de forma predeterminada inicia el [TUI](./tui) cuando se ejecuta sin ningún argumento.
...
Esto configura el flujo de trabajo de acciones GitHub necesario y lo guía a través del proceso de configuración. [Más información](./github).
...
Esto es útil para determinar el nombre exacto del modelo que se usará en [su configuración](./config/).
...
Inicie un servidor OpenCode sin cabeza para acceso API. Consulte los [documentos del servidor](./server) para conocer la interfaz HTTP completa.
