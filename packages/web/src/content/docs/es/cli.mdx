---
title: CLI
description: Opciones y comandos de la CLI de Octopus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Octopus de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Octopus mediante programación.
...
Inicie la interfaz de usuario del terminal Octopus.
...
La CLI de Octopus también tiene los siguientes comandos.

---

### agent

Administrar agentes para Octopus.
...
Conecte una terminal a un servidor backend Octopus que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto Octopus. Por ejemplo:
...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | ID de sesión para continuar                                                               |
| <nobr><code>{"--fork"}</code></nobr>     |       | Bifurcar la sesión al continuar (usar con `--continue` o `--session`)                     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OCTOPUS_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OCTOPUS_SERVER_USERNAME` u `opencode`) |
...
Octopus funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/octopus/auth.json`.

```bash
opencode auth login
```

Cuando se inicia Octopus, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Octopus.
...
Inicie un servidor Octopus sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
opencode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad opencode sin la interfaz TUI. Configure `OCTOPUS_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Enumere todas las sesiones Octopus.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Octopus.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Octopus.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida Octopus.
...
Inicie un servidor Octopus sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Octopus a través de una interfaz web. Configure `OCTOPUS_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale Octopus y elimine todos los archivos relacionados.
...
La CLI de Octopus toma las siguientes banderas globales.
...
Octopus se puede configurar mediante variables de entorno.

| Variable                              | Type     | Description                                                                     |
| ------------------------------------- | -------- | ------------------------------------------------------------------------------- |
| `OCTOPUS_AUTO_SHARE`                 | booleano | Compartir sesiones automáticamente                                              |
| `OCTOPUS_GIT_BASH_PATH`              | cadena   | Ruta al ejecutable de Git Bash en Windows                                       |
| `OCTOPUS_CONFIG`                     | cadena   | Ruta al archivo de configuración                                                |
| `OCTOPUS_CONFIG_DIR`                 | cadena   | Ruta al directorio de configuración                                             |
| `OCTOPUS_CONFIG_CONTENT`             | cadena   | Contenido de configuración json en línea                                        |
| `OCTOPUS_DISABLE_AUTOUPDATE`         | booleano | Deshabilitar las comprobaciones automáticas de actualizaciones                  |
| `OCTOPUS_DISABLE_PRUNE`              | booleano | Deshabilitar la poda de datos antiguos                                          |
| `OCTOPUS_DISABLE_TERMINAL_TITLE`     | booleano | Deshabilitar las actualizaciones automáticas de títulos de terminal             |
| `OCTOPUS_PERMISSION`                 | cadena   | Configuración de permisos json incorporados                                     |
| `OCTOPUS_DISABLE_DEFAULT_PLUGINS`    | booleano | Deshabilitar complementos predeterminados                                       |
| `OCTOPUS_DISABLE_LSP_DOWNLOAD`       | booleano | Deshabilitar las descargas automáticas del servidor LSP                         |
| `OCTOPUS_ENABLE_EXPERIMENTAL_MODELS` | booleano | Habilitar modelos experimentales                                                |
| `OCTOPUS_DISABLE_AUTOCOMPACT`        | booleano | Deshabilitar la compactación automática de contexto                             |
| `OCTOPUS_DISABLE_CLAUDE_CODE`        | booleano | Deshabilitar la lectura desde `.claude` (mensaje + habilidades)                 |
| `OCTOPUS_DISABLE_CLAUDE_CODE_PROMPT` | booleano | Desactivar lectura `~/.claude/CLAUDE.md`                                        |
| `OCTOPUS_DISABLE_CLAUDE_CODE_SKILLS` | booleano | Deshabilitar la carga `.claude/skills`                                          |
| `OCTOPUS_DISABLE_MODELS_FETCH`       | booleano | Deshabilitar la recuperación de modelos desde fuentes remotas                   |
| `OCTOPUS_FAKE_VCS`                   | cadena   | Proveedor de VCS falso para fines de prueba                                     |
| `OCTOPUS_CLIENT`                     | cadena   | Identificador de cliente (por defecto `cli`)                                    |
| `OCTOPUS_ENABLE_EXA`                 | booleano | Habilitar las herramientas de búsqueda web de Exa                               |
| `OCTOPUS_SERVER_PASSWORD`            | cadena   | Habilite la autenticación básica para `serve`/`web`                             |
| `OCTOPUS_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `opencode`) |
| `OCTOPUS_MODELS_URL`                 | cadena   | URL personalizada para buscar la configuración de modelos                       |
...
| Variable                                        | Type     | Description                                                |
| ----------------------------------------------- | -------- | ---------------------------------------------------------- |
| `OCTOPUS_EXPERIMENTAL`                         | booleano | Habilitar todas las funciones experimentales               |
| `OCTOPUS_EXPERIMENTAL_ICON_DISCOVERY`          | booleano | Habilitar descubrimiento de íconos                         |
| `OCTOPUS_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booleano | Deshabilitar copia al seleccionar en TUI                   |
| `OCTOPUS_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | número   | Tiempo de espera predeterminado para comandos bash en ms   |
| `OCTOPUS_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | número   | Tokens de salida máximos para respuestas LLM               |
| `OCTOPUS_EXPERIMENTAL_FILEWATCHER`             | booleano | Habilite el observador de archivos para todo el directorio |
| `OCTOPUS_EXPERIMENTAL_OXFMT`                   | booleano | Habilitar el formateador oxfmt                             |
| `OCTOPUS_EXPERIMENTAL_LSP_TOOL`                | booleano | Habilitar herramienta experimental LSP                     |
| `OCTOPUS_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booleano | Deshabilitar el observador de archivos                     |
| `OCTOPUS_EXPERIMENTAL_EXA`                     | booleano | Habilitar funciones experimentales de Exa                  |
| `OCTOPUS_EXPERIMENTAL_LSP_TY`                  | booleano | Habilitar Habilitar TY LSP para archivos python            |
| `OCTOPUS_EXPERIMENTAL_MARKDOWN`                | booleano | Habilitar funciones de Markdown experimentales             |
| `OCTOPUS_EXPERIMENTAL_PLAN_MODE`               | booleano | Habilitar modo de plan                                     |
