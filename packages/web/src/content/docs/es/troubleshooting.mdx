...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (o `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Si su `leak-code.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LEAKCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LEAKCODE_PORT` (o elija un puerto libre) y reinicie.
...
3. Asegúrese de tener la última versión con `leak-code upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `leak-code models`
