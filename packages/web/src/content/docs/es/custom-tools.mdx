...
description: Cree herramientas que LLM pueda llamar en xcsh.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de xcsh como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.xcsh/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/xcsh/tools/`.
...
```ts title=".xcsh/tools/database.ts" {1}
...
```ts title=".xcsh/tools/project.ts" {8}
import { tool } from "@f5xc-salesdemos/plugin"
...
```python title=".xcsh/tools/add.py"
import sys
...
```ts title=".xcsh/tools/python-add.ts" {10}
import { tool } from "@f5xc-salesdemos/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".xcsh/tools/add.py")
