...
description: Cree herramientas que LLM pueda llamar en pencode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de pencode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.pencode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/pencode/tools/`.
...
```ts title=".pencode/tools/database.ts" {1}
...
```ts title=".pencode/tools/math.ts"
import { tool } from "@pencode-ai/plugin"
...
```ts title=".pencode/tools/bash.ts"
import { tool } from "@pencode-ai/plugin"
...
```ts title=".pencode/tools/project.ts" {8}
import { tool } from "@pencode-ai/plugin"
...
```python title=".pencode/tools/add.py"
import sys
...
```ts title=".pencode/tools/python-add.ts" {10}
import { tool } from "@pencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".pencode/tools/add.py")
