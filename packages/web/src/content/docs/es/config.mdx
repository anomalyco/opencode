---
title: Configuración
description: Usando la configuración Octopus JSON.
---

Puede configurar Octopus usando un archivo de configuración JSON.

---

## Formato

Octopus admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
2. **Configuración global** (`~/.config/octopus/octopus.json`) - preferencias del usuario
3. **Configuración personalizada** (`OCTOPUS_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`octopus.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.opencode`** - agentes, comandos, complementos
6. **Configuración en línea** (`OCTOPUS_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
Coloque su configuración global Octopus en `~/.config/octopus/octopus.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `octopus.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia Octopus, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `OCTOPUS_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `OCTOPUS_CONFIG_DIR`
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Octopus.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Octopus a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Octopus intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Octopus a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/octopus/agents/` o `.octopus/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Octopus recurrirá a `"build"` con una advertencia.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/octopus/commands/` o `.octopus/commands/`. [Más información aquí](/docs/commands).
...
Octopus descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían Octopus con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.octopus/plugins/` o `~/.config/octopus/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir Octopus para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
