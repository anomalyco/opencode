...
5. **Configuración `.opencode`** (`.opencode/opencode.json` en el proyecto): igual que la configuración del proyecto pero separada
6. Directorios **`.opencode`** - agentes, comandos, complementos
7. **Configuración en línea** (`OPENCODE_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
