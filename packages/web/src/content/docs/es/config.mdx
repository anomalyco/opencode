...
2. **Configuración global** (`~/.config/leak-code/leak-code.json`) - preferencias del usuario
3. **Configuración personalizada** (`LEAKCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`leak-code.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.leak-code`** - agentes, comandos, complementos
6. **Configuración en línea** (`LEAKCODE_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.leak-code` y `~/.config/leak-code` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global OpenCode en `~/.config/leak-code/leak-code.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `leak-code.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `LEAKCODE_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `LEAKCODE_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.leak-code`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.leak-code`, por lo que **puede anular** su configuración.
...
Puede configurar los ajustes del servidor para los comandos `leak-code serve` y `leak-code web` a través de la opción `server`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/leak-code/agents/` o `.leak-code/agents/`. [Más información aquí](/docs/agents).
...
Esta configuración se aplica en todas las interfaces: TUI, CLI (`leak-code run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/leak-code/commands/` o `.leak-code/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.leak-code/plugins/` o `~/.config/leak-code/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
