...
:::consejo
Si no especifica un modelo, los agentes primarios usarán el [modelo configurado globalmente](./config#models), mientras que los subagentes usarán el modelo del agente primario que invocó al subagente.
:::
...
El ID del modelo en su configuración OpenCode usa el formato `provider/model-id`. Por ejemplo, si usa [OpenCode Zen](./zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
[Más información sobre herramientas](./tools).
...
[Más información sobre permisos](./permissions).
