...
Las habilidades del agente permiten a Pencode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Pencode busca estas ubicaciones:

- Configuración del proyecto: `.pencode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/pencode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Pencode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.pencode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.pencode/skills/git-release/SKILL.md` así:
...
Pencode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `pencode.json`:
...
**Para agentes integrados** (en `pencode.json`):
