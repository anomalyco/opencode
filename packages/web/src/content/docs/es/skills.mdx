...
Las habilidades del agente permiten a CyberStrike descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CyberStrike busca estas ubicaciones:

- Configuración del proyecto: `.cyberstrike/skills/<name>/SKILL.md`
- Configuración global: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CyberStrike sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.cyberstrike/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.cyberstrike/skills/git-release/SKILL.md` así:
...
CyberStrike enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `cyberstrike.json`:
...
**Para agentes integrados** (en `cyberstrike.json`):
