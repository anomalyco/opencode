...
Las habilidades del agente permiten a xcsh descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
xcsh busca estas ubicaciones:

- Configuración del proyecto: `.xcsh/skills/<name>/SKILL.md`
- Configuración global: `~/.config/xcsh/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, xcsh sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.xcsh/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.xcsh/skills/git-release/SKILL.md` así:
...
xcsh enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `xcsh.json`:
...
**Para agentes integrados** (en `xcsh.json`):
