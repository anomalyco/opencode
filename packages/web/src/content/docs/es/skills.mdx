...
Las habilidades del agente permiten a Octopus descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Octopus busca estas ubicaciones:

- Configuración del proyecto: `.octopus/skills/<name>/SKILL.md`
- Configuración global: `~/.config/octopus/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Octopus sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.octopus/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.octopus/skills/git-release/SKILL.md` así:
...
Octopus enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `octopus.json`:
...
**Para agentes integrados** (en `octopus.json`):
