...
Para configurar explícitamente el modo manual en su [archivo de configuración](./config):
...
Puede habilitar el uso compartido automático para todas las conversaciones nuevas configurando la opción `share` en `"auto"` en su [archivo de configuración](./config):
...
Puede desactivar el uso compartido por completo configurando la opción `share` en `"disabled"` en su [archivo de configuración](./config):
...
[Obtenga más información](./enterprise) sobre el uso de opencode en su organización.
