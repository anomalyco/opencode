...
:::caution
Si no se establece `LEAKCODE_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::

:::tip[Windows Usuarios]
Para obtener la mejor experiencia, ejecute `leak-code web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `LEAKCODE_SERVER_PASSWORD`:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

El nombre de usuario predeterminado es `opencode` pero se puede cambiar con `LEAKCODE_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `leak-code.json`:
