...
También puedes tener reglas globales en un archivo `~/.config/leak-code/AGENTS.md`. Esto se aplica en todas las sesiones opencode.
...
- **Reglas globales**: `~/.claude/CLAUDE.md` (se usa si no existe `~/.config/leak-code/AGENTS.md`)
...
2. **Archivo global** en `~/.config/leak-code/AGENTS.md`
3. **Archivo de Claude Code** en `~/.claude/CLAUDE.md` (a menos que esté deshabilitado)

El primer archivo coincidente gana en cada categoría. Por ejemplo, si tiene `AGENTS.md` y `CLAUDE.md`, solo se usa `AGENTS.md`. De manera similar, `~/.config/leak-code/AGENTS.md` tiene prioridad sobre `~/.claude/CLAUDE.md`.

---

## Instrucciones personalizadas

Puede especificar archivos de instrucciones personalizados en su `leak-code.json` o el `~/.config/leak-code/leak-code.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
El enfoque recomendado es utilizar el campo `instructions` en `leak-code.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `leak-code.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
