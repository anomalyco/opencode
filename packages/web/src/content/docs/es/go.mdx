...
xcsh Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
xcsh Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en xcsh. Te suscribes a xcsh Go y
...
xcsh Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

xcsh Go funciona como cualquier otro proveedor en xcsh.

1. Inicias sesión en **<a href={console}>xcsh Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `xcsh Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a xcsh Go.
:::
...
xcsh Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de xcsh
usa el formato `xcsh-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías
`xcsh-go/kimi-k2.5` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con xcsh.
