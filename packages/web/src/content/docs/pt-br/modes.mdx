...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do opencode. A opção `mode` agora está obsoleta. [Saiba mais](./agents).
:::
...
Veja também: [Formatadores](./formatters) para informações sobre configuração de formatação de código.
