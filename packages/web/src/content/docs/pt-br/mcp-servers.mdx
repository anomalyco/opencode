...
Os valores da sua configuração local substituem os padrões remotos. Veja [precedência de configuração](./config#precedence-order) para mais detalhes.
...
2. Em sua [configuração de agente](./agents#tools), habilite o servidor MCP como uma ferramenta.
...
Alternativamente, você pode adicionar algo assim ao seu [AGENTS.md](./rules/).
