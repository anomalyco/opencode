...
5. **Configuração `.opencode`** (`.opencode/opencode.json` no projeto) - mesma coisa que a configuração do projeto, mas separada
6. **Diretórios `.opencode`** - agentes, comandos, plugins
7. **Configuração inline** (`OPENCODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
