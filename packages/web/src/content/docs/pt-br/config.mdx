...
2. **Configuração global** (`~/.config/octopus/octopus.json`) - preferências do usuário
3. **Configuração personalizada** (`OCTOPUS_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`octopus.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.opencode`** - agentes, comandos, plugins
6. **Configuração inline** (`OCTOPUS_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
Coloque sua configuração global do opencode em `~/.config/octopus/octopus.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/octopus/tui.json`.
...
Adicione `octopus.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `OCTOPUS_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OCTOPUS_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
Use `OCTOPUS_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `octopus.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/octopus/agents/` ou `.octopus/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/octopus/commands/` ou `.octopus/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.octopus/plugins/` ou `~/.config/octopus/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
