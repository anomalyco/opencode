...
2. **Configuração global** (`~/.config/leak-code/leak-code.json`) - preferências do usuário
3. **Configuração personalizada** (`LEAKCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`leak-code.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.leak-code`** - agentes, comandos, plugins
6. **Configuração inline** (`LEAKCODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.leak-code` e `~/.config/leak-code` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/leak-code/leak-code.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/leak-code/tui.json`.
...
Adicione `leak-code.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `LEAKCODE_CONFIG`.

```bash
export LEAKCODE_CONFIG=/path/to/my/custom-config.json
leak-code run "Hello world"
```
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `LEAKCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.leak-code`, e deve seguir a mesma estrutura.

```bash
export LEAKCODE_CONFIG_DIR=/path/to/my/config-directory
leak-code run "Hello world"
```

O diretório personalizado é carregado após a configuração global e os diretórios `.leak-code`, então ele **pode substituir** suas configurações.
...
Use `LEAKCODE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `leak-code.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `leak-code serve` e `leak-code web` através da opção `server`.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/leak-code/agents/` ou `.leak-code/agents/`. [Saiba mais aqui](/docs/agents).
...
Essa configuração se aplica a todas as interfaces: TUI, CLI (`leak-code run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/leak-code/commands/` ou `.leak-code/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.leak-code/plugins/` ou `~/.config/leak-code/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
