...
3. **Configuração personalizada** (`IMECODE_CONFIG` var de ambiente) - substituições personalizadas
...
6. **Configuração inline** (`IMECODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `IMECODE_CONFIG`.

```bash
export IMECODE_CONFIG=/path/to/my/custom-config.json
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `IMECODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.

```bash
export IMECODE_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```
...
Use `IMECODE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.
