...
[Saiba mais sobre o uso do TUI aqui](./tui).
...
[Saiba mais sobre o servidor aqui](./server).
...
[Saiba mais sobre ferramentas aqui](./tools).
...
Você também pode configurar [modelos locais](./models#local). [Saiba mais](./models).
...
:::note
Tokens Bearer (`AWS_BEARER_TOKEN_BEDROCK` ou `/connect`) têm precedência sobre a autenticação baseada em perfil. Veja [precedência de autenticação](./providers#authentication-precedence) para detalhes.
:::

[Saiba mais sobre a configuração do Amazon Bedrock](./providers#amazon-bedrock).
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.opencode/agents/`. [Saiba mais aqui](./agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](./agents) que você definiu. Se o agente especificado não existir ou for um subagente, o OpenCode recuará para `"build"` com um aviso.
...
Você pode configurar o recurso [share](./share) através da opção `share`.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.opencode/commands/`. [Saiba mais aqui](./commands).
...
[Saiba mais sobre formatadores aqui](./formatters).
...
[Saiba mais sobre permissões aqui](./permissions).
...
[Saiba mais aqui](./mcp-servers).

---

### Plugins

[Plugins](./plugins) estendem o OpenCode com ferramentas, hooks e integrações personalizadas.
...
Isso aceita um array de caminhos e padrões glob para arquivos de instrução. [Saiba mais sobre regras aqui](./rules).
