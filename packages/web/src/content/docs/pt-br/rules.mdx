...
Você também pode ter regras globais em um arquivo `~/.config/leak-code/AGENTS.md`. Isso é aplicado em todas as sessões do opencode.
...
- **Regras globais**: `~/.claude/CLAUDE.md` (usado se não existir `~/.config/leak-code/AGENTS.md`)
...
2. **Arquivo global** em `~/.config/leak-code/AGENTS.md`
3. **Arquivo Claude Code** em `~/.claude/CLAUDE.md` (a menos que desabilitado)

O primeiro arquivo correspondente vence em cada categoria. Por exemplo, se você tiver tanto `AGENTS.md` quanto `CLAUDE.md`, apenas `AGENTS.md` é usado. Da mesma forma, `~/.config/leak-code/AGENTS.md` tem precedência sobre `~/.claude/CLAUDE.md`.

---

## Instruções Personalizadas

Você pode especificar arquivos de instrução personalizados no seu `leak-code.json` ou no global `~/.config/leak-code/leak-code.json`. Isso permite que você e sua equipe reutilizem regras existentes em vez de ter que duplicá-las no AGENTS.md.
...
### Usando leak-code.json

A abordagem recomendada é usar o campo `instructions` em `leak-code.json`:
...
:::tip
Para monorepos ou projetos com padrões compartilhados, usar `leak-code.json` com padrões glob (como `packages/*/AGENTS.md`) é mais sustentável do que instruções manuais.
:::
