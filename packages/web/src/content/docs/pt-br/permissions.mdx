...
- `destructive_bash` — confirmação única para comandos de shell destrutivos no perfil `secure-development`
...
Edições, testes, builds e inspeção de Git no projeto seguem automáticos. Operações destrutivas de shell (`rm`, operações destrutivas de Git, reescritas in-place, ferramentas de disco e publicação) usam `destructive_bash`, sempre com aprovação única e sem criar regra `always`. A rede permanece desabilitada no container; use uma integração revisada explicitamente quando a operação precisar alcançar um serviço externo.

O sandbox não substitui permissões do projeto: `external_directory` continua em `ask`, e o workspace montado é gravável de propósito para que o agente desenvolva nele. Nunca defina `bash: "allow"` global sem sandbox.
