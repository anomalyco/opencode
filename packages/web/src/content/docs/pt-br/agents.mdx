...
Existem dois tipos de agentes no cyberstrike; agentes primários e subagentes.
...
cyberstrike vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
cyberstrike vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

cyberstrike vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `cyberstrike.json`:
...
- Por projeto: `.cyberstrike/agents/`
...
Se nenhuma temperatura for especificada, o cyberstrike usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do cyberstrike quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do cyberstrike usa o formato `provider/model-id`. Por exemplo, se você estiver usando [CyberStrike Zen](/docs/zen), você usaria `cyberstrike/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `cyberstrike models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/CyberStrikeus/cyberstrike).
:::
