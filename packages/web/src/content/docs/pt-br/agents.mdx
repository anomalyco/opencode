...
Existem dois tipos de agentes no xcsh; agentes primários e subagentes.
...
xcsh vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
xcsh vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

xcsh vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `xcsh.json`:
...
- Global: `~/.config/xcsh/agents/`
- Por projeto: `.xcsh/agents/`

```markdown title="~/.config/xcsh/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o xcsh usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do xcsh quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do xcsh usa o formato `provider/model-id`. Por exemplo, se você estiver usando [xcsh Zen](/docs/zen), você usaria `xcsh/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `xcsh models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/f5xc-salesdemos/xcsh).
:::
...
```markdown title="~/.config/xcsh/agents/docs-writer.md"
---
...
```markdown title="~/.config/xcsh/agents/security-auditor.md"
---
