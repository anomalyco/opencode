...
Existem dois tipos de agentes no pencode; agentes primários e subagentes.
...
pencode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Pencode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Pencode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Pencode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `pencode.json`:
...
- Global: `~/.config/pencode/agents/`
- Por projeto: `.pencode/agents/`

```markdown title="~/.config/pencode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o pencode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do pencode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do pencode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Pencode Zen](/docs/zen), você usaria `pencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="pencode.json" {3-6,9-12}
{
  "$schema": "https://pencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="pencode.json" {3-5,8-10}
{
  "$schema": "https://pencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `pencode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/kiyosh11/pencode).
:::
...
```markdown title="~/.config/pencode/agents/docs-writer.md"
---
...
```markdown title="~/.config/pencode/agents/security-auditor.md"
---
