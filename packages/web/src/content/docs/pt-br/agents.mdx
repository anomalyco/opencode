...
:::tip
Se você não especificar um modelo, os agentes primários usam o [modelo configurado globalmente](./config#models) enquanto subagentes usarão o modelo do agente primário que invocou o subagente.
:::
...
O ID do modelo em sua configuração do OpenCode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](./zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
[Saiba mais sobre ferramentas](./tools).
...
[Saiba mais sobre permissões](./permissions).
