...
- Apenas macOS: menu `Octopus` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/octopus/octopus.jsonc` (ou `~/.config/octopus/octopus.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Se seu `octopus.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `OCTOPUS_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `OCTOPUS_PORT` (ou escolha uma porta livre) e reinicie.
