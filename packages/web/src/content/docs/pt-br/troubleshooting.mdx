...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (ou `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Se seu `leak-code.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `LEAKCODE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `LEAKCODE_PORT` (ou escolha uma porta livre) e reinicie.
...
3. Certifique-se de que você tem a versão mais recente com `leak-code upgrade`
...
Para descobrir quais modelos você tem acesso, execute `leak-code models`
