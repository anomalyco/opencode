...
Para depurar problemas com o pencode, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **macOS/Linux**: `~/.local/share/pencode/log/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\pencode\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `pencode --log-level DEBUG`.

---

## Armazenamento

O pencode armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/pencode/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\pencode`
...
O pencode Desktop executa um servidor pencode local (o sidecar `pencode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Pencode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/pencode/pencode.jsonc` (ou `~/.config/pencode/pencode.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/pencode/pencode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\pencode\pencode.jsonc`
...
O pencode também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/pencode/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\pencode\plugins`
- **Plugins de projeto** (apenas se você usar configuração por projeto)
  - `<your-project>/.pencode/plugins/`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o pencode possa reconstruí-lo.

1. Saia completamente do pencode Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/pencode`
- **Linux**: exclua `~/.cache/pencode` (ou execute `rm -rf ~/.cache/pencode`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\pencode`

3. Reinicie o pencode Desktop.

---

### Corrigindo problemas de conexão com o servidor

O pencode Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `pencode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `PENCODE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `PENCODE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o pencode Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do pencode.

---

### Notificações não aparecendo

O pencode Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o pencode nas configurações do seu sistema operacional, e
...
1. Saia do pencode Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo pencode Desktop):

- `pencode.settings.dat` (URL do servidor padrão do desktop)
- `pencode.global.dat` e `pencode.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o pencode:
...
1. **Relatar problemas no GitHub**
   ...
   [**github.com/kiyosh11/pencode/issues**](https://github.com/kiyosh11/pencode/issues)
...
2. **Junte-se ao nosso Discord**
   ...
   [**pencode.ai/discord**](https://pencode.ai/discord)
...
Para descobrir quais modelos você tem acesso, execute `pencode models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   ```bash
   rm -rf ~/.local/share/pencode
   ```

   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\pencode`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O pencode instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\pencode`

2. Reinicie o pencode para reinstalar os pacotes de provedores mais recentes

Isso forçará o pencode a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O pencode detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
