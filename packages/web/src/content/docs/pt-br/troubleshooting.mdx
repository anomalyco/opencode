...
Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](./windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do OpenCode.
...
1. Primeiro, verifique se seu provedor está configurado corretamente seguindo o [guia de provedores](./providers)
