...
Para depurar problemas com o cyberstrike, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\cyberstrike\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `cyberstrike --log-level DEBUG`.

---

## Armazenamento

O cyberstrike armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/cyberstrike/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\cyberstrike`
...
O cyberstrike Desktop executa um servidor cyberstrike local (o sidecar `cyberstrike-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `CyberStrike` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/cyberstrike/cyberstrike.jsonc` (ou `~/.config/cyberstrike/cyberstrike.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/cyberstrike/cyberstrike.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\cyberstrike\cyberstrike.jsonc`
...
O cyberstrike também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/cyberstrike/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\cyberstrike\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o cyberstrike possa reconstruí-lo.

1. Saia completamente do cyberstrike Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/cyberstrike`
- **Linux**: exclua `~/.cache/cyberstrike` (ou execute `rm -rf ~/.cache/cyberstrike`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\cyberstrike`

3. Reinicie o cyberstrike Desktop.

---

### Corrigindo problemas de conexão com o servidor

O cyberstrike Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `cyberstrike.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `CYBERSTRIKE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `CYBERSTRIKE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o cyberstrike Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do cyberstrike.

---

### Notificações não aparecendo

O cyberstrike Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o cyberstrike nas configurações do seu sistema operacional, e
...
1. Saia do cyberstrike Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo cyberstrike Desktop):

- `cyberstrike.settings.dat` (URL do servidor padrão do desktop)
- `cyberstrike.global.dat` e `cyberstrike.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o cyberstrike:
...
1. **Relatar problemas no GitHub**
   ...
   [**github.com/CyberStrikeus/cyberstrike/issues**](https://github.com/CyberStrikeus/cyberstrike/issues)
...
Para descobrir quais modelos você tem acesso, execute `cyberstrike models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\cyberstrike`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O cyberstrike instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\cyberstrike`

2. Reinicie o cyberstrike para reinstalar os pacotes de provedores mais recentes

Isso forçará o cyberstrike a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O cyberstrike detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
