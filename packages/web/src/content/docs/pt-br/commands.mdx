...
Comandos personalizados são adicionais aos comandos integrados como `/init`, `/undo`, `/redo`, `/share`, `/help`. [Saiba mais](./tui#commands).
...
Use a opção `command` na sua [configuração](./config) do OpenCode:
...
Use _!`comando`_ para injetar a saída do [comando bash](./tui#bash-commands) no seu prompt.
...
Use a configuração `agent` para especificar opcionalmente qual [agente](./agents) deve executar este comando.
Se este for um [subagente](./agents/#subagents), o comando acionará uma invocação de subagente por padrão.
...
Use o booleano `subtask` para forçar o comando a acionar uma invocação de [subagente](./agents/#subagents).
Isso é útil se você quiser que o comando não polua seu contexto principal e **forçará** o agente a agir como um subagente,
mesmo que `mode` esteja definido como `primary` na configuração do [agente](./agents).
...
opencode inclui vários comandos integrados como `/init`, `/undo`, `/redo`, `/share`, `/help`; [saiba mais](./tui#commands).
