...
:::caution
Ao usar `--hostname 0.0.0.0`, defina `LEAKCODE_SERVER_PASSWORD` para proteger o servidor.
...
1. **Execute `leak-code web` no terminal WSL** em vez do PowerShell:
...
Executar `leak-code web` a partir do WSL garante acesso correto ao sistema de arquivos e integração com o terminal, continuando acessível no navegador do Windows.
...
- Sua configuração e suas sessões do opencode ficam armazenadas no ambiente WSL em `~/.local/share/leak-code/`
