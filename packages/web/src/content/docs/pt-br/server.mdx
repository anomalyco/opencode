---
title: Servidor
description: Interaja com o servidor xcsh via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `xcsh serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente xcsh pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `xcsh.local` |
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `xcsh`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `xcsh serve` quanto ao `xcsh web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password xcsh serve
```
...
Quando você executa `xcsh`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor xcsh para interagir com o xcsh programaticamente.
:::

Essa arquitetura permite que o xcsh suporte múltiplos clientes e permite que você interaja com o xcsh programaticamente.

Você pode executar `xcsh serve` para iniciar um servidor autônomo. Se você tiver o TUI do xcsh em execução, `xcsh serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do xcsh [IDE](/docs/ide).
...
O servidor xcsh expõe as seguintes APIs.
