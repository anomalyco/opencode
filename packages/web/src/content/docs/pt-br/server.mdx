---
title: Servidor
description: Interaja com o servidor pencode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `pencode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente pencode pode usar.
...
```bash
pencode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `pencode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
pencode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `PENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `pencode`, ou defina `PENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `pencode serve` quanto ao `pencode web`.

```bash
PENCODE_SERVER_PASSWORD=your-password pencode serve
```
...
Quando você executa `pencode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor pencode para interagir com o pencode programaticamente.
:::

Essa arquitetura permite que o pencode suporte múltiplos clientes e permite que você interaja com o pencode programaticamente.

Você pode executar `pencode serve` para iniciar um servidor autônomo. Se você tiver o TUI do pencode em execução, `pencode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do pencode [IDE](/docs/ide).
...
O servidor pencode expõe as seguintes APIs.
