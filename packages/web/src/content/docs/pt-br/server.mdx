...
O comando `leak-code serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opencode pode usar.
...
```bash
leak-code serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
leak-code serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `LEAKCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencode`, ou defina `LEAKCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `leak-code serve` quanto ao `leak-code web`.

```bash
LEAKCODE_SERVER_PASSWORD=your-password leak-code serve
```
...
:::tip
Use o servidor opencode para interagir com o leak-code programaticamente.
:::

Essa arquitetura permite que o opencode suporte múltiplos clientes e permite que você interaja com o leak-code programaticamente.

Você pode executar `leak-code serve` para iniciar um servidor autônomo. Se você tiver o TUI do opencode em execução, `leak-code serve` iniciará um novo servidor.
