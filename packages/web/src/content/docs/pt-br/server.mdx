---
title: Servidor
description: Interaja com o servidor cyberstrike via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `cyberstrike serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente cyberstrike pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `cyberstrike.local` |
...
Defina `CYBERSTRIKE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `cyberstrike`, ou defina `CYBERSTRIKE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `cyberstrike serve` quanto ao `cyberstrike web`.
...
Quando você executa `cyberstrike`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor cyberstrike para interagir com o cyberstrike programaticamente.
:::

Essa arquitetura permite que o cyberstrike suporte múltiplos clientes e permite que você interaja com o cyberstrike programaticamente.

Você pode executar `cyberstrike serve` para iniciar um servidor autônomo. Se você tiver o TUI do cyberstrike em execução, `cyberstrike serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do cyberstrike [IDE](/docs/ide).
...
O servidor cyberstrike expõe as seguintes APIs.
