...
Para a melhor experiência no Windows, recomendamos usar o [Windows Subsystem for Linux (WSL)](./windows-wsl). Ele oferece melhor desempenho e total compatibilidade com os recursos do OpenCode.
:::
...
Se você é novo no uso de provedores de LLM, recomendamos usar o [OpenCode Zen](./zen).
...
Alternativamente, você pode selecionar um dos outros provedores. [Saiba mais](./providers#directory).
...
As conversas que você tem com o OpenCode podem ser [compartilhadas com sua equipe](./share).
...
Para torná-lo seu, recomendamos [escolher um tema](./themes), [personalizar os atalhos de teclado](./keybinds), [configurar formatadores de código](./formatters), [criar comandos personalizados](./commands) ou brincar com a [configuração do OpenCode](./config).