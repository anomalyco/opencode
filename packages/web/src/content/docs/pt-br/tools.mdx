...
:::note
Esta ferramenta está disponível apenas quando `IMECODE_EXPERIMENTAL_LSP_TOOL=true` (ou `IMECODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `IMECODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o opencode:

```bash
IMECODE_ENABLE_EXA=1 opencode
```
