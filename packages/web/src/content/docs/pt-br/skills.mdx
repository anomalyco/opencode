...
As habilidades do agente permitem que o xcsh descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O xcsh pesquisa nesses locais:

- Configuração do projeto: `.xcsh/skills/<name>/SKILL.md`
- Configuração global: `~/.config/xcsh/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o xcsh sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.xcsh/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.xcsh/skills/git-release/SKILL.md` assim:
...
O xcsh lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `xcsh.json`:
...
**Para agentes embutidos** (em `xcsh.json`):
