...
As habilidades do agente permitem que o pencode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O pencode pesquisa nesses locais:

- Configuração do projeto: `.pencode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/pencode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o pencode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.pencode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.pencode/skills/git-release/SKILL.md` assim:
...
O pencode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `pencode.json`:
...
**Para agentes embutidos** (em `pencode.json`):
