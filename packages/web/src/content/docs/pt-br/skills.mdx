...
As habilidades do agente permitem que o cyberstrike descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O cyberstrike pesquisa nesses locais:

- Configuração do projeto: `.cyberstrike/skills/<name>/SKILL.md`
- Configuração global: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o cyberstrike sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.cyberstrike/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.cyberstrike/skills/git-release/SKILL.md` assim:
...
O cyberstrike lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `cyberstrike.json`:
...
**Para agentes embutidos** (em `cyberstrike.json`):
