...
- Configuração do projeto: `.octopus/skills/<name>/SKILL.md`
- Configuração global: `~/.config/octopus/skills/<name>/SKILL.md`
...
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.octopus/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.octopus/skills/git-release/SKILL.md` assim:
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `octopus.json`:
...
**Para agentes embutidos** (em `octopus.json`):
