---
title: Plugins
description: Escreva seus próprios plugins para estender o xcsh.
---

Plugins permitem que você estenda o xcsh conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do xcsh.
...
- `.xcsh/plugins/` - Plugins em nível de projeto
- `~/.config/xcsh/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/xcsh/node_modules/`.
...
1. Configuração global (`~/.config/xcsh/xcsh.json`)
2. Configuração do projeto (`xcsh.json`)
3. Diretório de plugins global (`~/.config/xcsh/plugins/`)
4. Diretório de plugins do projeto (`.xcsh/plugins/`)
...
O xcsh executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do xcsh para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o xcsh.
...
:::note
Se você estiver usando o aplicativo desktop xcsh, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o xcsh de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao xcsh:
...
O helper `tool` cria uma ferramenta personalizada que o xcsh pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o xcsh junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) para detalhes.
