---
title: Plugins
description: Escreva seus próprios plugins para estender o cyberstrike.
---

Plugins permitem que você estenda o cyberstrike conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do cyberstrike.
...
- `.cyberstrike/plugins/` - Plugins em nível de projeto
- `~/.config/cyberstrike/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/cyberstrike/node_modules/`.
...
1. Configuração global (`~/.config/cyberstrike/cyberstrike.json`)
2. Configuração do projeto (`cyberstrike.json`)
3. Diretório de plugins global (`~/.config/cyberstrike/plugins/`)
4. Diretório de plugins do projeto (`.cyberstrike/plugins/`)
...
O cyberstrike executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do cyberstrike para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o cyberstrike.
...
:::note
Se você estiver usando o aplicativo desktop cyberstrike, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o cyberstrike de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao cyberstrike:
...
O helper `tool` cria uma ferramenta personalizada que o cyberstrike pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o cyberstrike junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://cyberstrike.io/docs/sdk) para detalhes.
