---
title: Plugins
description: Escreva seus próprios plugins para estender o pencode.
---

Plugins permitem que você estenda o pencode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do pencode.
...
- `.pencode/plugins/` - Plugins em nível de projeto
- `~/.config/pencode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/pencode/node_modules/`.
...
1. Configuração global (`~/.config/pencode/pencode.json`)
2. Configuração do projeto (`pencode.json`)
3. Diretório de plugins global (`~/.config/pencode/plugins/`)
4. Diretório de plugins do projeto (`.pencode/plugins/`)
...
O pencode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do pencode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o pencode.
...
:::note
Se você estiver usando o aplicativo desktop pencode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o pencode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao pencode:
...
O helper `tool` cria uma ferramenta personalizada que o pencode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o pencode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://pencode.ai/docs/sdk) para detalhes.
