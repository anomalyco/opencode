...
- `.leak-code/plugins/` - Plugins em nível de projeto
- `~/.config/leak-code/plugins/` - Plugins globais
...
1. Configuração global (`~/.config/leak-code/leak-code.json`)
2. Configuração do projeto (`leak-code.json`)
3. Diretório de plugins global (`~/.config/leak-code/plugins/`)
4. Diretório de plugins do projeto (`.leak-code/plugins/`)
...
```json title=".leak-code/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
```ts title=".leak-code/plugins/my-plugin.ts"
...
```js title=".leak-code/plugins/example.js"
...
```js title=".leak-code/plugins/notification.js"
...
```javascript title=".leak-code/plugins/env-protection.js"
...
```javascript title=".leak-code/plugins/inject-env.js"
...
```ts title=".leak-code/plugins/custom-tools.ts"
...
```ts title=".leak-code/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".leak-code/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
