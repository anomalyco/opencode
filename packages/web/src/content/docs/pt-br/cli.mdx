...
Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o leak-code programaticamente.
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `LEAKCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `LEAKCODE_SERVER_USERNAME` ou `opencode`) |
...
O opencode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `leak-code auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/leak-code/auth.json`.
...
```bash "leak-code run"
leak-code run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `leak-code serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
leak-code run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `LEAKCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `LEAKCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `LEAKCODE_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `LEAKCODE_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `LEAKCODE_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `LEAKCODE_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `LEAKCODE_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `LEAKCODE_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `LEAKCODE_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `LEAKCODE_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `LEAKCODE_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `LEAKCODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `LEAKCODE_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `LEAKCODE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `LEAKCODE_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `LEAKCODE_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `LEAKCODE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `LEAKCODE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `LEAKCODE_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `LEAKCODE_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `LEAKCODE_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `LEAKCODE_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `LEAKCODE_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `LEAKCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `LEAKCODE_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `LEAKCODE_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `LEAKCODE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `LEAKCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `LEAKCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `LEAKCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `LEAKCODE_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `LEAKCODE_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `LEAKCODE_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `LEAKCODE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `LEAKCODE_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `LEAKCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `LEAKCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `LEAKCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `LEAKCODE_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `LEAKCODE_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `LEAKCODE_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `LEAKCODE_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `LEAKCODE_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
