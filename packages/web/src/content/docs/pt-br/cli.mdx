---
title: CLI
description: Opções e comandos da CLI do xcsh.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do xcsh, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
xcsh
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o xcsh programaticamente.
...
Inicie a interface de usuário do terminal do xcsh.
...
A CLI do xcsh também possui os seguintes comandos.

---

### agent

Gerencie agentes para o xcsh.
...
Anexe um terminal a um servidor backend do xcsh já em execução, iniciado via comandos `serve` ou `web`.

```bash
xcsh attach [url]
```

Isso permite usar o TUI com um backend xcsh remoto. Por exemplo:
...
O xcsh é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `xcsh auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

Quando o xcsh é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no xcsh.
...
Execute o xcsh em modo não interativo passando um prompt diretamente.
...
```bash "xcsh run"
xcsh run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `xcsh serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
xcsh run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor xcsh em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor xcsh sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
xcsh serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do xcsh sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `xcsh`).
...
Liste todas as sessões do xcsh.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do xcsh.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do xcsh.

```bash
xcsh import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do xcsh.
...
Inicie um servidor xcsh sem cabeça com uma interface web.

```bash
xcsh web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o xcsh através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `xcsh`).
...
Desinstale o xcsh e remova todos os arquivos relacionados.
...
Atualiza o xcsh para a versão mais recente ou uma versão específica.
...
A CLI do xcsh aceita as seguintes flags globais.
...
O xcsh pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `xcsh`) |
