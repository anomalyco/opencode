...
A CLI do OpenCode, por padrão, inicia o [TUI](./tui) quando executada sem argumentos.
...
Isso configura o fluxo de trabalho necessário do GitHub Actions e o guia pelo processo de configuração. [Saiba mais](./github).
...
Isso é útil para descobrir o nome exato do modelo a ser usado em [sua configuração](./config/).
...
Inicie um servidor OpenCode sem cabeça para acesso à API. Confira a [documentação do servidor](./server) para a interface HTTP completa.
