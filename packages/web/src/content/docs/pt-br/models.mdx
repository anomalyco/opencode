...
Saiba mais sobre [provedores](./providers).
...
Aqui, o ID completo é `provider_id/model_id`. Por exemplo, se você estiver usando [OpenCode Zen](./zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.

Se você configurou um [provedor personalizado](./providers#custom), o `provider_id` é a chave da parte `provider` da sua configuração, e o `model_id` é a chave de `provider.models`.
...
Você também pode configurar essas opções para quaisquer agentes que estiver usando. A configuração do agente substitui quaisquer opções globais aqui. [Saiba mais](./agents/#additional).
...
Use a tecla de atalho `variant_cycle` para alternar rapidamente entre variantes. [Saiba mais](./keybinds).
