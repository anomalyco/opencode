...
Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/octopus/auth.json`.
...
Octopus Zen é uma lista de modelos fornecidos pela equipe do opencode que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Octopus Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Octopus Zen` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Octopus Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do opencode que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `Octopus Go` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `octopus.json`:
...
arquivo `octopus.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `octopus.json`:
...
Octopus Zen é uma lista de modelos testados e verificados fornecidos pela equipe do opencode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Octopus Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Octopus Zen**.
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no octopus.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `octopus.json` no diretório do seu projeto:
