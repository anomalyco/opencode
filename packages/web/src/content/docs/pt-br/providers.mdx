---
title: Provedores
description: Usando qualquer provedor LLM no xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do xcsh.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/xcsh/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do xcsh.
...
xcsh Zen é uma lista de modelos fornecidos pela equipe do xcsh que foram testados e verificados para funcionar bem com o xcsh. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o xcsh Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `xcsh Zen` e acesse [f5xc-salesdemos.github.io/xcsh/zen](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Funciona como qualquer outro provedor no xcsh e é completamente opcional.

---

## xcsh Go

xcsh Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do xcsh que foram
testados e verificados para funcionar bem com o xcsh.

1. Execute o comando `/connect` no TUI, selecione `xcsh Go` e acesse [f5xc-salesdemos.github.io/xcsh/zen](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Para usar o Amazon Bedrock com o xcsh:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o xcsh:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `xcsh.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no xcsh não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o xcsh funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do xcsh.
...
armazenar o token no armazenamento de autenticação do xcsh.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
xcsh usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o xcsh
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `xcsh.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth).

##### Configuração

Personalize através do `xcsh.json`:
...
Para usar sua assinatura do GitHub Copilot com o xcsh:
...
Para usar o Google Vertex AI com o xcsh:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do xcsh, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) para registrar automaticamente cada conversa do xcsh como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do xcsh listada como uma sessão separada.
...
Você pode configurar o xcsh para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o xcsh para usar modelos locais através do LM Studio.
...
Você pode configurar o xcsh para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o xcsh. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/xcsh) para detalhes.
:::
...
Para usar o Ollama Cloud com o xcsh:
...
3. Copie a chave da API para uso no xcsh.
...
6. **Importante**: Antes de usar modelos em nuvem no xcsh, você deve puxar as informações do modelo localmente:
...
xcsh Zen é uma lista de modelos testados e verificados fornecidos pela equipe do xcsh. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>xcsh Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **xcsh Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do xcsh.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do xcsh.
...
5. Você também pode personalizá-los através da sua configuração do xcsh. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o xcsh:
...
Você também pode personalizar modelos através da sua configuração do xcsh. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o xcsh. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no xcsh.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `xcsh.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o xcsh entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `xcsh auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do xcsh e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do xcsh.
