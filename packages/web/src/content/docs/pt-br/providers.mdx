...
Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/leak-code/auth.json`.
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `leak-code.json`:
...
arquivo `leak-code.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `leak-code.json`:
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no leak-code.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `leak-code.json` no diretório do seu projeto:
...
1. **Verifique a configuração de autenticação**: Execute `leak-code auth list` para ver se as credenciais
