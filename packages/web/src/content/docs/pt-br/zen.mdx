---
title: Zen
description: Lista selecionada de modelos fornecidos pelo Octopus.
---
...
O Octopus Zen é uma lista de modelos testados e verificados fornecidos pela equipe do Octopus.

:::note
O Octopus Zen está atualmente em beta.
:::

O Zen funciona como qualquer outro provedor no Octopus. Você faz login no Octopus Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o Octopus.
...
O Octopus Zen é um gateway de AI que dá a você acesso a esses modelos.

---

## Como funciona

O Octopus Zen funciona como qualquer outro provedor no Octopus.

1. Você faz login no **<a href={console}>Octopus Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona Octopus Zen e cola sua chave de API.
...
O [model id](/docs/config/#models) na sua configuração do Octopus usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.5, você usaria `opencode/gpt-5.5` na sua configuração.
...
- MiniMax M2.5 Free está disponível no Octopus por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Ring 2.6 1T Free está disponível no Octopus por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3 Super Free está disponível no Octopus por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo stealth que está gratuito no Octopus por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o Octopus também.
