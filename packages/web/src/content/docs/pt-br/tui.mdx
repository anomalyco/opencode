...
A maioria dos comandos também possui atalhos usando `ctrl+x` como a tecla líder, onde `ctrl+x` é a tecla líder padrão. [Saiba mais](./keybinds).
...
Crie ou atualize o arquivo `AGENTS.md`. [Saiba mais](./rules).
...
Compartilhe a sessão atual. [Saiba mais](./share).
...
Descompartilhe a sessão atual. [Saiba mais](./share#un-sharing).
