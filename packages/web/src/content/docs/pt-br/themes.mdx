...
Com o cyberstrike, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o cyberstrike usa nosso próprio tema `cyberstrike`.
...
O cyberstrike vem com vários temas embutidos.
...
- Querem que o cyberstrike corresponda à aparência do seu terminal
...
O cyberstrike suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/cyberstrike/themes/*.json` ou `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.cyberstrike/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.cyberstrike/themes/*.json`
