...
Com o xcsh, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o xcsh usa nosso próprio tema `xcsh`.
...
O xcsh vem com vários temas embutidos.
...
- Querem que o xcsh corresponda à aparência do seu terminal
...
O xcsh suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/xcsh/themes/*.json` ou `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.xcsh/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.xcsh/themes/*.json`
