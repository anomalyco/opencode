...
Com o pencode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o pencode usa nosso próprio tema `pencode`.
...
O pencode vem com vários temas embutidos.
...
- Querem que o pencode corresponda à aparência do seu terminal
...
O pencode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/pencode/themes/*.json` ou `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.pencode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.pencode/themes/*.json`
