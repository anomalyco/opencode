...
:::caution
Se `LEAKCODE_SERVER_PASSWORD` não estiver definido, o servidor ficará sem segurança. Isso é aceitável para uso local, mas deve ser configurado para acesso à rede.
:::

:::tip[Usuários do Windows]
Para a melhor experiência, execute `leak-code web` a partir do [WSL](/docs/windows-wsl) em vez do PowerShell. Isso garante acesso adequado ao sistema de arquivos e integração com o terminal.
:::
...
Para proteger o acesso, defina uma senha usando a variável de ambiente `LEAKCODE_SERVER_PASSWORD`:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

O nome de usuário padrão é `opencode`, mas pode ser alterado com `LEAKCODE_SERVER_USERNAME`.
...
Você também pode configurar as configurações do servidor no seu arquivo de configuração `leak-code.json`:
