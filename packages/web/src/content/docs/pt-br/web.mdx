...
Para a melhor experiência, execute `opencode web` a partir do [WSL](./windows-wsl) em vez do PowerShell. Isso garante acesso adequado ao sistema de arquivos e integração com o terminal.
:::
...
Você pode configurar o servidor web usando flags de linha de comando ou no seu [arquivo de configuração](./config).
