...
O xcsh Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O xcsh Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no xcsh. Você assina o xcsh Go e
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o xcsh.
:::
...
O xcsh Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O xcsh Go funciona como qualquer outro provedor no xcsh.

1. Você entra no **<a href={console}>xcsh Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `xcsh Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o xcsh Go.
:::
...
O xcsh Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do xcsh
usa o formato `xcsh-go/<model-id>`. Por exemplo, para o Kimi K2.5, você usaria
`xcsh-go/kimi-k2.5` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o xcsh também.
