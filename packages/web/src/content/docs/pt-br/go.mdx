...
O Pencode Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Pencode. Você assina o Pencode Go e
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o Pencode.
:::
...
O Pencode Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O Pencode Go funciona como qualquer outro provedor no Pencode.

1. Você entra no **<a href={console}>Pencode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Pencode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Pencode Go.
:::
...
O Pencode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Pencode
usa o formato `pencode-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`pencode-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Pencode também.
