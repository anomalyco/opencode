...
工具允許 LLM 在您的代碼庫中執行操作。 OpenCode 附帶了一組內置工具，但您可以使用[定制工具](./custom-tools) 或[MCP服務器](./mcp-servers) 對其進行擴展。

默認情況下，所有工具都是**啟用**並且不需要運行權限。您可以通過[權限](./permissions)控制工具行為。
...
要配置哪些 LSP 服務器可用於您的項目，請參閱[LSP服務器](./lsp)。
...
加載[技能](./skills)（`SKILL.md` 文件）並在對話中返回其內容。
...
:::note
默認情況下，子代理禁用此工具，但您可以手動啟用它。 [了解更多](./agents/#permissions)
:::
