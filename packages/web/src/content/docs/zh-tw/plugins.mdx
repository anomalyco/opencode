...
例如，查看社區創建的[插件](./ecosystem#plugins)。
...
瀏覽[生態系統](./ecosystem#plugins)中的可用插件。
...
**本地插件**直接從插件目錄加載。要使用外部包，您必須在配置目錄中創建`package.json`（請參閱[依賴關係](#dependencies)），或將插件發佈到npm和[將其添加到您的配置中](./config#plugins)。
