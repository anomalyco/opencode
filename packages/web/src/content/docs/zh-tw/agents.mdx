...
:::tip
如果您不指定模型，主代理將使用[全局配置的模型](./config#models)，而子代理將使用調用子代理的主代理的模型。
:::
...
OpenCode 配置中的模型 ID 使用格式`provider/model-id`。例如，如果您使用 [開放代碼禪](./zen)，則您將使用 `opencode/gpt-5.1-codex` 來表示 GPT 5.1 Codex。
