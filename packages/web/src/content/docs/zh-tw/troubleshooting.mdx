...
如果您在 Windows 上遇到性能緩慢、文件訪問問題或終端問題，請嘗試使用[WSL（適用於 Linux 的 Windows 子系統）](./windows-wsl)。 WSL 提供了一個可以與 OpenCode 功能更加無縫協作的 Linux 環境。
...
1. 首先，按照 [供應商指南](./providers) 驗證您的提供商是否已正確設置
