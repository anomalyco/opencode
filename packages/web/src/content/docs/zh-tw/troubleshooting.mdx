...
- **macOS/Linux**（舊版安裝）: `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
如果你的 `leak-code.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。

#### 檢查環境變數

如果你在環境中設定了 `LEAKCODE_PORT`，桌面應用程式將嘗試使用該連接埠作為本地伺服器連接埠。

- 取消設定 `LEAKCODE_PORT`（或選擇一個空閒連接埠）並重新啟動。
...
3. 使用 `leak-code upgrade` 確保你使用的是最新版本
...
要查看你有權存取哪些模型，請執行 `leak-code models`
