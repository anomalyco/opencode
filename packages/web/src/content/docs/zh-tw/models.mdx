...
這裡完整的 ID 是`provider_id/model_id`。例如，如果您使用 [開放代碼禪](./zen)，則您將使用 `opencode/gpt-5.1-codex` 來表示 GPT 5.1 Codex。

如果您配置了[定制提供商](./providers#custom)，則`provider_id` 是配置中`provider` 部分的密鑰，`model_id` 是`provider.models` 中的密鑰。
...
您還可以為您正在使用的任何代理配置這些選項。代理配置會覆蓋此處的所有全局選項。 [了解更多](./agents/#additional)。
...
使用鍵綁定 `variant_cycle` 在變體之間快速切換。 [了解更多](./keybinds)。
