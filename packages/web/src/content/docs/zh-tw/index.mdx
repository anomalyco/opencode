...
為了在 Windows 上獲得最佳體驗，我們建議使用[適用於 Linux 的 Windows 子系統 (WSL)](./windows-wsl)。它提供了更好的性能並與 OpenCode 的功能完全兼容。
:::
...
如果您不熟悉使用 LLM 提供商，我們建議使用[開放代碼禪](./zen)。
...
或者，您可以選擇其他提供商之一。 [了解更多](./providers#directory)。
...
要使其成為您自己的，我們建議 [選擇一個主題](./themes)、[自定義按鍵綁定](./keybinds)、[配置代碼格式化程序](./formatters)、[創建自定義命令](./commands) 或使用 [開放代碼配置](./config)。
