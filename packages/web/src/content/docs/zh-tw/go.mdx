...
OpenCode Go 包含以下基準限制：

- **5 小時限制** — 20% 的使用配額
- **每週限制** — 50% 的使用配額
- **每月限制** — 100% 的使用配額

所有模型共享使用配額，以上限制以該配額的百分比表示。

### 各模型的使用額度

各模型包含的每月使用額度不同，按官方 API 價格換算。Token 價格按每 1M tokens 列示。

| 模型                                    | 輸入   | 輸出   | 快取讀取  | 快取寫入 | 每月使用額度 |
| --------------------------------------- | ------ | ------ | --------- | -------- | ------ |
...
可用請求次數取決於模型的每月使用額度和 API 價格。以下預估以典型的 Go 使用模式為基礎。
...
- Omen Alpha — 每次請求 300 個輸入 token、40,000 個快取 token、100 個輸出 token
...
- **DeepSeek V4 Flash:** ZDR 協議每月續簽。目前的協議有效至 2026 年 9 月 30 日。
