...
4. 輸入您的 Cloudflare API Token。
   ...
   `CF_AIG_TOKEN` 也可作為 `CLOUDFLARE_API_TOKEN` 的別名使用。
...
:::tip
`global` 區域可以提高可用性並減少錯誤，且不會產生額外費用。透過設定 `VERTEX_LOCATION=global` 來使用它。如果有資料駐留需求，請使用區域端點（例如 `us-central1`）。[了解更多](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
