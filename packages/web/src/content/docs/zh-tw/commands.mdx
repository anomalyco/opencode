...
除了`/init`、`/undo`、`/redo`、`/share`、`/help` 等內置命令之外，還有自定義命令。 [了解更多](./tui#commands)。
...
在 OpenCode [配置](./config) 中使用 `command` 選項：
...
使用 _!`command`_ 將 [bash命令](./tui#bash-commands) 輸出注入到提示符中。
...
使用`agent` 配置可選擇指定哪個[代理人](./agents) 應執行此命令。
如果這是 [子代理](./agents/#subagents) 該命令將默認觸發子代理調用。
...
使用`subtask` 布爾值強制命令觸發[子代理](./agents/#subagents) 調用。
如果您希望命令不污染您的主要上下文並且將**強制**代理充當子代理，那麼這非常有用，
即使`mode` 在[代理人](./agents) 配置上設置為`primary`。
...
opencode包含`/init`、`/undo`、`/redo`、`/share`、`/help`等內置命令； [了解更多](./tui#commands)。
