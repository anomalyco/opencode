---
title: Zen
description: 由 Octopus 提供的精選模型列表。
---
...
Octopus Zen 是由 Octopus 團隊提供、經過測試與驗證的模型清單。

:::note
Octopus Zen 目前仍處於 beta。
:::

Zen 的運作方式和 Octopus 中的其他供應商一樣。你登入 Octopus Zen 並取得
...
:::tip
我們測試了一組與 Octopus 搭配良好的精選模型與供應商。
:::
...
Octopus Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

Octopus Zen 的運作方式和 Octopus 中的其他供應商一樣。

1. 你登入 **<a href={console}>Octopus Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 Octopus Zen，然後貼上你的 API 金鑰。
...
Octopus 設定中的 [模型 ID](/docs/config/#models) 會使用 `opencode/<model-id>`
...
- MiniMax M2.5 Free 在 Octopus 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Ring 2.6 1T Free 在 Octopus 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Super Free 在 Octopus 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 Octopus 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
