...
Most commands also have keybind using `ctrl+x` as the leader key, where `ctrl+x` is the default leader key. [了解更多](./keybinds)。
...
取消共享當前會話。 [了解更多](./share#un-sharing)。
