...
您還可以配置[本地模型](./models#local)。 [了解更多](./models)。
...
:::note
承載令牌（`AWS_BEARER_TOKEN_BEDROCK` 或`/connect`）優先於基於配置文件的身份驗證。詳情請參閱[認證優先級](./providers#authentication-precedence)。
:::

[了解有關 Amazon Bedrock 配置的更多信息](./providers#amazon-bedrock)。
...
您還可以使用 `~/.config/opencode/agents/` 或 `.opencode/agents/` 中的 markdown 文件定義代理。 [在這裡了解更多](./agents)。
...
默認代理必須是主代理（而不是子代理）。這可以是內置代理，例如`"build"`或`"plan"`，或者您定義的[定制代理](./agents)。如果指定的代理不存在或者是子代理，OpenCode 將回退到 `"build"` 並發出警告。
...
您還可以使用 `~/.config/opencode/commands/` 或 `.opencode/commands/` 中的 Markdown 文件定義命令。 [在這裡了解更多](./commands)。
...
[插件](./plugins) 使用自定義工具、掛鉤和集成擴展 OpenCode。
