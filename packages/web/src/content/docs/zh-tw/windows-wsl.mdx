...
:::caution
使用 `--hostname 0.0.0.0` 時，請設定 `LEAKCODE_SERVER_PASSWORD` 以保護伺服器安全。
:::
...
1. **在 WSL 終端機中執行 `leak-code web`**，而非在 PowerShell 中執行：
...
從 WSL 中執行 `leak-code web` 可確保正確的檔案系統存取和終端機整合，同時仍可透過 Windows 瀏覽器進行存取。
...
- OpenCode 的設定和工作階段資料儲存在 WSL 環境中的 `~/.local/share/leak-code/`
