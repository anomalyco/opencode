...
默認情況下，OpenCode CLI 在不帶任何參數運行時啟動[途易](./tui)。
...
這將設置必要的 GitHub Actions 工作流程並指導您完成配置過程。 [了解更多](./github)。
...
這對於確定[你的配置](./config/) 中使用的確切模型名稱很有用。
...
啟動無頭 OpenCode 服務器以進行 API 訪問。查看[服務器文檔](./server) 以獲取完整的 HTTP 接口。
