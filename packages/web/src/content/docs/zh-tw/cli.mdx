---
title: CLI
description: xcsh CLI 選項和指令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh CLI 在不帶任何參數執行時，預設啟動 [TUI](/docs/tui)。

```bash
xcsh
```

但它也接受本頁面中記錄的指令，使您可以透過程式方式與 xcsh 進行互動。
...
將終端機連接到已透過 `serve` 或 `web` 指令啟動的 xcsh 後端伺服器。

```bash
xcsh attach [url]
```

這允許將 TUI 與遠端 xcsh 後端搭配使用。例如：
...
xcsh 基於 [Models.dev](https://models.dev) 的供應商列表運作，因此您可以使用 `xcsh auth login` 為任何想要使用的供應商設定 API 金鑰。金鑰儲存在 `~/.local/share/xcsh/auth.json` 中。

```bash
xcsh auth login
```

xcsh 啟動時會從憑證檔案載入供應商資訊，同時也會載入環境變數或專案中 `.env` 檔案中定義的金鑰。
...
使用 `--refresh` 旗標可以更新快取的模型列表。當供應商新增了模型並且您希望在 xcsh 中看到它們時，此功能非常有用。
...
您還可以連接到正在執行的 `xcsh serve` 實例，以避免每次執行時 MCP 伺服器的冷啟動時間：
...
| `--attach`   |      | 連接到正在執行的 xcsh 伺服器（例如 http://localhost:4096） |
...
啟動無介面的 xcsh 伺服器以提供 API 存取。查看[伺服器文件](/docs/server)了解完整的 HTTP 介面。

```bash
xcsh serve
```

此指令啟動一個 HTTP 伺服器，提供對 xcsh 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `xcsh`）。
...
從 JSON 檔案或 xcsh 分享連結匯入工作階段資料。

```bash
xcsh import <file>
```

您可以從本地檔案或 xcsh 分享連結匯入。
...
啟動帶有 Web 介面的無介面 xcsh 伺服器。

```bash
xcsh web
```

此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 xcsh。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `xcsh`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `xcsh`） |
