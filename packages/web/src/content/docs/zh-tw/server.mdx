...
當您啟動 TUI 時，它會隨機分配端口和主機名。您可以改為傳入`--hostname` 和`--port` [旗幟](./cli)。然後使用它連接到其服務器。

[`/tui`](#tui) 端點可用於通過服務器驅動 TUI。例如，您可以預填充或運行提示。此設置由 OpenCode [集成開發環境](./ide) 插件使用。
