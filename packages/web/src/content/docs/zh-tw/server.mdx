...
設定 `OCTOPUS_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencode`，也可以設定 `OCTOPUS_SERVER_USERNAME` 來覆蓋它。這適用於 `opencode serve` 和 `opencode web`。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 Octopus [IDE](/docs/ide) 外掛所使用。
