...
If you've configured a [custom provider](/docs/providers#custom-provider), the `provider_id` is the key from the `provider` part of your config, and the `model_id` is the key from `provider.models`.
