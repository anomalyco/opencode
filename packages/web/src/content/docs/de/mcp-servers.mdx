...
Ihre lokalen Konfigurationswerte überschreiben die Remote-Standardwerte. Weitere Einzelheiten finden Sie unter [config precedence](./config#precedence-order).
...
2. Aktivieren Sie in Ihrem [agent config](./agents#tools) den MCP-Server als Tool.
...
Alternativ können Sie so etwas zu Ihrem [AGENTS.md](./rules/) hinzufügen.
