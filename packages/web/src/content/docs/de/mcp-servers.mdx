...
Sie können externe Tools zu xcsh hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. xcsh unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/xcsh`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
xcsh übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird xcsh Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von xcsh beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `xcsh mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert xcsh die Token sicher in `~/.local/share/xcsh/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in xcsh verfügbar. Sie können sie also wie jedes andere Tool über die xcsh-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und xcsh mit Ihrem Sentry-Konto zu verbinden.
