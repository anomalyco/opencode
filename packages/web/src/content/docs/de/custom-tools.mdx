...
description: Erstellen Sie Tools, die der LLM in Octopus aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Octopus wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.octopus/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/octopus/tools/` platzieren.
...
```ts title=".octopus/tools/database.ts" {1}
import { tool } from "@octopus-ai/plugin"
...
```ts title=".octopus/tools/math.ts"
import { tool } from "@octopus-ai/plugin"
...
```ts title=".octopus/tools/project.ts" {8}
import { tool } from "@octopus-ai/plugin"
...
```python title=".octopus/tools/add.py"
import sys
...
```ts title=".octopus/tools/python-add.ts" {10}
import { tool } from "@octopus-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".octopus/tools/add.py")
