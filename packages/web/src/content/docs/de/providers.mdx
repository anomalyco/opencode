...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `leak-code.json`:
...
`leak-code.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
4. Erstellen oder aktualisieren Sie Ihre `leak-code.json`-Datei in Ihrem Projektverzeichnis:
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `leak-code auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
