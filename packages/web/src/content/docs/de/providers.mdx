---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer xcsh-Konfiguration.
...
in `~/.local/share/xcsh/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem xcsh anpassen
config.
...
xcsh Zen ist eine Liste von Modellen, die vom xcsh-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit xcsh funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit xcsh Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in xcsh und ist völlig optional.

---

## xcsh Go

xcsh Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom xcsh-Team getestet und verifiziert wurden, dass sie gut mit xcsh funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `xcsh Go` und gehen Sie zu [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
So verwenden Sie Amazon Bedrock mit xcsh:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie xcsh ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `xcsh.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in xcsh wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit xcsh ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre xcsh-Konfiguration hinzufügen.
...
um das Token im xcsh-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
xcsh verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von xcsh
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`xcsh.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `xcsh.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit xcsh:
...
So verwenden Sie Google Vertex AI mit xcsh:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie xcsh ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über xcsh konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session)-Plugin, um jede xcsh-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede xcsh-Konversation als separate Sitzung aufgeführt.
...
Sie können xcsh für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können xcsh über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können xcsh über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für xcsh konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/xcsh).
:::
...
So verwenden Sie Ollama Cloud mit xcsh:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in xcsh.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in xcsh verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
xcsh Zen ist eine Liste getesteter und verifizierter Modelle, die vom xcsh-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>xcsh Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **xcsh Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren xcsh API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre xcsh-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre xcsh-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit xcsh:
...
Sie können Modelle auch über Ihre xcsh-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit xcsh verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `xcsh.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann xcsh erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `xcsh auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die xcsh-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer xcsh-Konfiguration übereinstimmt.
