---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in CyberStrike.
---

import config from "../../../../config.mjs"
export const console = config.console

CyberStrike verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer CyberStrike-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem CyberStrike anpassen
config.
...
CyberStrike Zen ist eine Liste von Modellen, die vom CyberStrike-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit CyberStrike funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit CyberStrike Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in CyberStrike und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit CyberStrike:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie CyberStrike ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `cyberstrike.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in CyberStrike wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit CyberStrike ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre CyberStrike-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `CyberStrike`, Bereiche: `api`
...
um das Token im CyberStrike-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
CyberStrike verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von CyberStrike
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`cyberstrike.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Anpassen über `cyberstrike.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit CyberStrike:
...
So verwenden Sie Google Vertex AI mit CyberStrike:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie CyberStrike ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über CyberStrike konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session)-Plugin, um jede CyberStrike-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede CyberStrike-Konversation als separate Sitzung aufgeführt.
...
Sie können CyberStrike für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können CyberStrike über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können CyberStrike über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für CyberStrike konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/cyberstrike).
:::
...
So verwenden Sie Ollama Cloud mit CyberStrike:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in CyberStrike.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in CyberStrike verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
CyberStrike Zen ist eine Liste getesteter und verifizierter Modelle, die vom CyberStrike-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>CyberStrike Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **CyberStrike Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren CyberStrike API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre CyberStrike-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre CyberStrike-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit CyberStrike:
...
Sie können Modelle auch über Ihre CyberStrike-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit CyberStrike verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `cyberstrike.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann CyberStrike erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `cyberstrike auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die CyberStrike-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer CyberStrike-Konfiguration übereinstimmt.
