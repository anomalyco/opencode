---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um xcsh zu erweitern.
---

Mit Plugins können Sie xcsh erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von xcsh zu ändern.
...
- `.xcsh/plugins/` – Plugins auf Projektebene
- `~/.config/xcsh/plugins/` – Globale Plugins
...
```json title="xcsh.json"
{
  ...
  "plugin": ["xcsh-helicone-session", "xcsh-wakatime", "@my-org/custom-plugin"]
}
```
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/xcsh/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/xcsh/xcsh.json`)
2. Projektkonfiguration (`xcsh.json`)
3. Globales Plugin-Verzeichnis (`~/.config/xcsh/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.xcsh/plugins/`)
...
xcsh führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".xcsh/plugins/my-plugin.ts"
...
```js title=".xcsh/plugins/example.js"
...
- `client`: Ein xcsh-SDK-Client für die Interaktion mit AI.
...
import type { Plugin } from "@f5xc-salesdemos/plugin"
...
Hier sind einige Beispiele für Plugins, mit denen Sie xcsh erweitern können.
...
```js title=".xcsh/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "xcsh"'`
      }
    },
  }
}
...
:::note
Wenn Sie die xcsh-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass xcsh `.env`-Dateien liest:

```javascript title=".xcsh/plugins/env-protection.js"
...
```javascript title=".xcsh/plugins/inject-env.js"
...
Plugins können xcsh auch benutzerdefinierte Tools hinzufügen:

```ts title=".xcsh/plugins/custom-tools.ts"
import { type Plugin, tool } from "@f5xc-salesdemos/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das xcsh aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für xcsh zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://f5xc-salesdemos.github.io/xcsh/docs/sdk).
...
```ts title=".xcsh/plugins/compaction.ts"
import type { Plugin } from "@f5xc-salesdemos/plugin"
...
```ts title=".xcsh/plugins/custom-compaction.ts"
import type { Plugin } from "@f5xc-salesdemos/plugin"
