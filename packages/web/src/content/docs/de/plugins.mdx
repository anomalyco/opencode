---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Pencode zu erweitern.
---

Mit Plugins können Sie Pencode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Pencode zu ändern.
...
- `.pencode/plugins/` – Plugins auf Projektebene
- `~/.config/pencode/plugins/` – Globale Plugins
...
```json title="pencode.json"
{
  ...
  "plugin": ["pencode-helicone-session", "pencode-wakatime", "@my-org/custom-plugin"]
}
```
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/pencode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/pencode/pencode.json`)
2. Projektkonfiguration (`pencode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/pencode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.pencode/plugins/`)
...
```json title=".pencode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Pencode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".pencode/plugins/my-plugin.ts"
...
```js title=".pencode/plugins/example.js"
...
- `client`: Ein Pencode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Pencode erweitern können.
...
```js title=".pencode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "pencode"'`
      }
    },
  }
}
...
:::note
Wenn Sie die Pencode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Pencode `.env`-Dateien liest:

```javascript title=".pencode/plugins/env-protection.js"
...
```javascript title=".pencode/plugins/inject-env.js"
...
Plugins können Pencode auch benutzerdefinierte Tools hinzufügen:

```ts title=".pencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@pencode-ai/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Pencode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Pencode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://pencode.ai/docs/sdk).
...
```ts title=".pencode/plugins/compaction.ts"
import type { Plugin } from "@pencode-ai/plugin"
...
```ts title=".pencode/plugins/custom-compaction.ts"
import type { Plugin } from "@pencode-ai/plugin"
