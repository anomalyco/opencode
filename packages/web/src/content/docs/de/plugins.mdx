---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Octopus zu erweitern.
---

Mit Plugins können Sie Octopus erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Octopus zu ändern.
...
- `.octopus/plugins/` – Plugins auf Projektebene
- `~/.config/octopus/plugins/` – Globale Plugins
...
1. Globale Konfiguration (`~/.config/octopus/octopus.json`)
2. Projektkonfiguration (`octopus.json`)
3. Globales Plugin-Verzeichnis (`~/.config/octopus/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.octopus/plugins/`)
...
```json title=".octopus/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Octopus führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".octopus/plugins/my-plugin.ts"
...
```js title=".octopus/plugins/example.js"
...
- `client`: Ein Octopus-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Octopus erweitern können.
...
```js title=".octopus/plugins/notification.js"
...
:::note
Wenn Sie die Octopus-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Octopus `.env`-Dateien liest:

```javascript title=".octopus/plugins/env-protection.js"
...
```javascript title=".octopus/plugins/inject-env.js"
...
Plugins können Octopus auch benutzerdefinierte Tools hinzufügen:

```ts title=".octopus/plugins/custom-tools.ts"
import { type Plugin, tool } from "@octopus-ai/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Octopus aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Octopus zur Verfügung.
...
```ts title=".octopus/plugins/compaction.ts"
import type { Plugin } from "@octopus-ai/plugin"
...
```ts title=".octopus/plugins/custom-compaction.ts"
import type { Plugin } from "@octopus-ai/plugin"
