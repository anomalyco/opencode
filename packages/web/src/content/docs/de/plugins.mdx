---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CyberStrike zu erweitern.
---

Mit Plugins können Sie CyberStrike erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CyberStrike zu ändern.
...
- `.cyberstrike/plugins/` – Plugins auf Projektebene
- `~/.config/cyberstrike/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/cyberstrike/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/cyberstrike/cyberstrike.json`)
2. Projektkonfiguration (`cyberstrike.json`)
3. Globales Plugin-Verzeichnis (`~/.config/cyberstrike/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.cyberstrike/plugins/`)
...
CyberStrike führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CyberStrike-SDK-Client für die Interaktion mit AI.
...
import type { Plugin } from "@cyberstrike-ai/plugin"
...
Hier sind einige Beispiele für Plugins, mit denen Sie CyberStrike erweitern können.
...
:::note
Wenn Sie die CyberStrike-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CyberStrike `.env`-Dateien liest:
...
```javascript title=".cyberstrike/plugins/inject-env.js"
...
Plugins können CyberStrike auch benutzerdefinierte Tools hinzufügen:

```ts title=".cyberstrike/plugins/custom-tools.ts"
import { type Plugin, tool } from "@cyberstrike-ai/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CyberStrike aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CyberStrike zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://cyberstrike.io/docs/sdk).
