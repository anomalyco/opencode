...
OpenCode bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](./custom-tools) oder [MCP-Server](./mcp-servers) erweitern.

Standardmaessig sind alle Tools **aktiviert** und brauchen keine Freigabe.
Das Verhalten steuerst du ueber [Berechtigungen](./permissions).
...
[Mehr dazu](./permissions), wie du Berechtigungen konfigurierst.
...
To configure which LSP servers are available for your project, see [LSP Servers](./lsp).
...
Load a [skill](./skills) (a `SKILL.md` file) and return its content in the conversation.
...
:::note
This tool is disabled for subagents by default, but you can enable it manually. [Learn more](./agents/#permissions)
:::
...
[Mehr dazu](./custom-tools), wie du Custom Tools erstellst.
...
[Mehr dazu](./mcp-servers), wie du MCP-Server konfigurierst.
