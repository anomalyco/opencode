...
Wenn Octopus Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Octopus Desktop startet im Hintergrund einen lokalen Octopus-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Octopus`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/octopus/octopus.jsonc` (oder `~/.config/octopus/octopus.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Octopus kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Octopus ihn neu aufbauen kann.

1. Beende Octopus Desktop komplett.
...
3. Starte Octopus Desktop neu.

---

### Server-Verbindungsprobleme beheben

Octopus Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `octopus.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OCTOPUS_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OCTOPUS_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Octopus Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Octopus-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Octopus Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Octopus in den OS-Einstellungen aktiviert sind, und
...
1. Beende Octopus Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Octopus Desktop):
...
Wenn du Probleme mit Octopus hast:

1. **Probleme auf GitHub melden**

   Bugs und Feature-Wuensche meldest du am besten im GitHub-Repository:

   [**github.com/anomalyco/octopus/issues**](https://github.com/anomalyco/octopus/issues)
