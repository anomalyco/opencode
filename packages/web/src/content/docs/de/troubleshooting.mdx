...
Wenn Pencode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\pencode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `pencode --log-level DEBUG`.

---

## Speicher

pencode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/pencode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\pencode`
...
Pencode Desktop startet im Hintergrund einen lokalen Pencode-Server (`pencode-cli`-Sidecar).
...
- Nur macOS: `Pencode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/pencode/pencode.jsonc` (oder `~/.config/pencode/pencode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/pencode/pencode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\pencode\pencode.jsonc`
...
Pencode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/pencode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\pencode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Pencode ihn neu aufbauen kann.

1. Beende Pencode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/pencode`
- **Linux**: loesche `~/.cache/pencode` (oder fuehre aus: `rm -rf ~/.cache/pencode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\pencode`

3. Starte Pencode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Pencode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `pencode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `PENCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `PENCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Pencode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Pencode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Pencode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Pencode in den OS-Einstellungen aktiviert sind, und
...
1. Beende Pencode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Pencode Desktop):

- `pencode.settings.dat` (desktop default server URL)
- `pencode.global.dat` und `pencode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Pencode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `pencode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\pencode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. pencode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\pencode`

2. Starte pencode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt pencode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
pencode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
