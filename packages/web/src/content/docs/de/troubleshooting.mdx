...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (oder `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Wenn deine `leak-code.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `LEAKCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `LEAKCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
3. Stelle sicher, dass du die neueste Version hast: `leak-code upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `leak-code models` aus.
