...
Wenn CyberStrike Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `cyberstrike --log-level DEBUG`.

---

## Speicher

cyberstrike speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
CyberStrike Desktop startet im Hintergrund einen lokalen CyberStrike-Server (`cyberstrike-cli`-Sidecar).
...
- Nur macOS: `CyberStrike`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit CyberStrike hast:
