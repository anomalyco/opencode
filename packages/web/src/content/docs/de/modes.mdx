...
:::caution
Modi werden jetzt über die Option `agent` in der CyberStrike-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in CyberStrike können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die CyberStrike-Konfiguration.
...
CyberStrike verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.cyberstrike/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `cyberstrike.json`-Konfigurationsdatei:
...
- Global: `~/.config/cyberstrike/modes/`
- Projekt: `.cyberstrike/modes/`

```markdown title="~/.config/cyberstrike/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Wenn keine Temperatur angegeben ist, verwendet CyberStrike modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale CyberStrike-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.cyberstrike/modes/` für projektspezifische Modi oder `~/.config/cyberstrike/modes/` für globale Modi:

```markdown title=".cyberstrike/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/cyberstrike/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
