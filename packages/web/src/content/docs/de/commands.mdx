...
Benutzerdefinierte Befehle gibt es zusätzlich zu den integrierten Befehlen wie `/init`, `/undo`, `/redo`, `/share`, `/help`. [Learn more](./tui#commands).
...
Verwenden Sie die Option `command` in Ihrem OpenCode [config](./config):
...
Verwenden Sie _!`command`_, um die Ausgabe [bash command](./tui#bash-commands) in Ihre Eingabeaufforderung einzufügen.
...
Verwenden Sie die `agent`-Konfiguration, um optional anzugeben, welche [agent](./agents) diesen Befehl ausführen soll.
Wenn es sich um einen [subagent](./agents/#subagents) handelt, löst der Befehl standardmäßig einen Subagentenaufruf aus.
...
Verwenden Sie den booleschen Wert `subtask`, um zu erzwingen, dass der Befehl einen [subagent](./agents/#subagents)-Aufruf auslöst.
...
auch wenn `mode` in der [agent](./agents)-Konfiguration auf `primary` gesetzt ist.
...
Opencode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](./tui#commands).
