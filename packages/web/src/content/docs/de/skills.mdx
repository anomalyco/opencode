...
Agent Skills erlauben Octopus, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Octopus sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Octopus vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.octopus/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.octopus/skills/git-release/SKILL.md` zum Beispiel so:
...
Octopus listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `octopus.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `octopus.json`):
