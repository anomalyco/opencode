...
Agent Skills erlauben xcsh, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
xcsh sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft xcsh vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.xcsh/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.xcsh/skills/git-release/SKILL.md` zum Beispiel so:
...
xcsh listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `xcsh.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `xcsh.json`):
