...
Agent Skills erlauben Pencode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Pencode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Pencode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.pencode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.pencode/skills/git-release/SKILL.md` zum Beispiel so:
...
Pencode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `pencode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `pencode.json`):
