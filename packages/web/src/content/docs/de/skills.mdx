...
Dabei werden passende `skills/*/SKILL.md` in `.leak-code/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/leak-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.leak-code/skills/git-release/SKILL.md` zum Beispiel so:
...
Steuere in `leak-code.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `leak-code.json`):
