...
Agent Skills erlauben CyberStrike, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CyberStrike sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CyberStrike vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.cyberstrike/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.cyberstrike/skills/git-release/SKILL.md` zum Beispiel so:
...
CyberStrike listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `cyberstrike.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `cyberstrike.json`):
