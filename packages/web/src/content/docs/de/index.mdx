...
Für das beste Erlebnis unter Windows empfehlen wir die Verwendung von [Windows Subsystem for Linux (WSL)](./windows-wsl). Es bietet eine bessere Leistung und vollständige Kompatibilität mit den Funktionen von OpenCode.
:::
...
Wenn Sie mit der Verwendung von LLM-Anbietern noch nicht vertraut sind, empfehlen wir die Verwendung von [OpenCode Zen](./zen).
...
Alternativ können Sie auch einen der anderen Anbieter auswählen. [Learn more](./providers#directory).
...
Hier ist ein [example conversation](https://opencode.ai/s/4XP1fce5) mit OpenCode.
...
Um es zu Ihrem eigenen zu machen, empfehlen wir [picking a theme](./themes), [customizing the keybinds](./keybinds), [configuring code formatters](./formatters), [creating custom commands](./commands) oder experimentieren Sie mit [OpenCode config](./config).
