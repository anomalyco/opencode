---
title: Regeln
description: Lege eigene Anweisungen fuer cyberstrike fest.
---

Du kannst cyberstrike ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in cyberstrike den Befehl `/init` aus.
...
So kann cyberstrike sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht cyberstrike sie zu erweitern.
...
cyberstrike kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/cyberstrike/AGENTS.md` speichern.
Diese gelten in allen cyberstrike-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CyberStrike dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/cyberstrike/AGENTS.md` existiert)
...
Beim Start sucht cyberstrike in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/cyberstrike/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `cyberstrike.json` oder in `~/.config/cyberstrike/cyberstrike.json` angeben.
...
cyberstrike parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit cyberstrike.json

Empfohlen ist das `instructions`-Feld in der `cyberstrike.json`:
...
Du kannst cyberstrike explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `cyberstrike.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
