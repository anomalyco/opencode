...
Du kannst auch globale Regeln in `~/.config/octopus/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt Octopus dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/octopus/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/octopus/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `octopus.json` oder in `~/.config/octopus/octopus.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `octopus.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `octopus.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
