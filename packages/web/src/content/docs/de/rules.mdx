---
title: Regeln
description: Lege eigene Anweisungen fuer pencode fest.
---

Du kannst pencode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in pencode den Befehl `/init` aus.
...
So kann pencode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht pencode sie zu erweitern.
...
pencode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/pencode/AGENTS.md` speichern.
Diese gelten in allen pencode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Pencode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/pencode/AGENTS.md` existiert)
...
Beim Start sucht pencode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/pencode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `pencode.json` oder in `~/.config/pencode/pencode.json` angeben.
...
pencode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit pencode.json

Empfohlen ist das `instructions`-Feld in der `pencode.json`:
...
Du kannst pencode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `pencode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
