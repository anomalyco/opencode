---
title: Regeln
description: Lege eigene Anweisungen fuer xcsh fest.
---

Du kannst xcsh ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in xcsh den Befehl `/init` aus.
...
So kann xcsh sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht xcsh sie zu erweitern.
...
xcsh kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/xcsh/AGENTS.md` speichern.
Diese gelten in allen xcsh-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt xcsh dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/xcsh/AGENTS.md` existiert)
...
Beim Start sucht xcsh in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/xcsh/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `xcsh.json` oder in `~/.config/xcsh/xcsh.json` angeben.
...
xcsh parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit xcsh.json

Empfohlen ist das `instructions`-Feld in der `xcsh.json`:
...
Du kannst xcsh explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `xcsh.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
