...
Du kannst auch globale Regeln in `~/.config/leak-code/AGENTS.md` speichern.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/leak-code/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/leak-code/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `leak-code.json` oder in `~/.config/leak-code/leak-code.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `leak-code.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `leak-code.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
