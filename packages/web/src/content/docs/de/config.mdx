...
2. **Globale Konfiguration** (`~/.config/leak-code/leak-code.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`LEAKCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`leak-code.json` im Projekt) – projektspezifische Einstellungen
5. **`.leak-code` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`LEAKCODE_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.leak-code` und `~/.config/leak-code` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/leak-code/leak-code.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `leak-code.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `LEAKCODE_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `LEAKCODE_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.leak-code` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.leak-code` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Servereinstellungen für die Befehle `leak-code serve` und `leak-code web` über die Option `server` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/leak-code/agents/` oder `.leak-code/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`leak-code run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/leak-code/commands/` oder `.leak-code/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.leak-code/plugins/` oder `~/.config/leak-code/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
