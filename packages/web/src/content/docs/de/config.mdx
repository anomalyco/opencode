...
[Learn more about using the TUI here](./tui).
...
[Learn more about the server here](./server).
...
[Learn more about tools here](./tools).
...
Sie können auch [local models](./models#local) konfigurieren. [Learn more](./models).
...
:::note
Inhabertoken (`AWS_BEARER_TOKEN_BEDROCK` oder `/connect`) haben Vorrang vor der profilbasierten Authentifizierung. Weitere Informationen finden Sie unter [authentication precedence](./providers#authentication-precedence).
:::

[Learn more about Amazon Bedrock configuration](./providers#amazon-bedrock).
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/opencode/agents/` oder `.opencode/agents/` definieren. [Learn more here](./agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](./agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift OpenCode mit einer Warnung auf `"build"` zurück.
...
Sie können die Funktion [share](./share) über die Option `share` konfigurieren.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/opencode/commands/` oder `.opencode/commands/` definieren. [Learn more here](./commands).
...
[Learn more about formatters here](./formatters).
...
[Learn more about permissions here](./permissions).
...
[Learn more here](./mcp-servers).

---

### Plugins

[Plugins](./plugins) erweitert OpenCode mit benutzerdefinierten Tools, Hooks und Integrationen.
