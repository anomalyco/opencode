---
title: Konfiguration
description: Verwenden Sie die Konfiguration Octopus JSON.
---

Sie können Octopus mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

Octopus unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
2. **Globale Konfiguration** (`~/.config/octopus/octopus.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OCTOPUS_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`octopus.json` im Projekt) – projektspezifische Einstellungen
5. **`.opencode` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`OCTOPUS_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
Platzieren Sie Ihre globale Octopus-Konfiguration in `~/.config/octopus/octopus.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `octopus.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn Octopus gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `OCTOPUS_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `OCTOPUS_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres Octopus-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer Octopus-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht Octopus, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer Octopus-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/octopus/agents/` oder `.octopus/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift Octopus mit einer Warnung auf `"build"` zurück.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/octopus/commands/` oder `.octopus/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Octopus lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
Octopus erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert Octopus mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.octopus/plugins/` oder `~/.config/octopus/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie Octopus darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
