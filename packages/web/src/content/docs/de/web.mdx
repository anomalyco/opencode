...
:::caution
Wenn `LEAKCODE_SERVER_PASSWORD` nicht gesetzt ist, ist der Server ungesichert. Dies ist für den lokalen Gebrauch in Ordnung, sollte aber für den Netzwerkzugriff eingestellt werden.
:::

:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `leak-code web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Um den Zugriff zu schützen, legen Sie mithilfe der Umgebungsvariablen `LEAKCODE_SERVER_PASSWORD` ein Passwort fest:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Der Benutzername ist standardmäßig `opencode`, kann aber mit `LEAKCODE_SERVER_USERNAME` geändert werden.
...
Sie können Servereinstellungen auch in Ihrer `leak-code.json`-Konfigurationsdatei konfigurieren:
