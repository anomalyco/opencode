---
title: Server
description: Nutze den xcsh-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `xcsh serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein xcsh-Client nutzen kann.
...
| `--mdns-domain` | Custom domain name for mDNS service | `xcsh.local` |
...
Der Benutzername ist standardmaessig `xcsh`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `xcsh serve` und `xcsh web`.
...
Wenn du `xcsh` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den xcsh-Server, um xcsh programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `xcsh serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `xcsh serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die xcsh-[IDE](/docs/ide)-Plugins.
...
Der xcsh-Server stellt folgende APIs bereit.
