---
title: Server
description: Nutze den cyberstrike-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `cyberstrike serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein cyberstrike-Client nutzen kann.
...
Setze `CYBERSTRIKE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `cyberstrike`, kann aber mit `CYBERSTRIKE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `cyberstrike serve` und `cyberstrike web`.
...
Wenn du `cyberstrike` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den cyberstrike-Server, um cyberstrike programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `cyberstrike serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `cyberstrike serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CyberStrike-[IDE](/docs/ide)-Plugins.
...
Der cyberstrike-Server stellt folgende APIs bereit.
