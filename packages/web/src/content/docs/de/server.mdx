---
title: Server
description: Nutze den pencode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `pencode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein pencode-Client nutzen kann.
...
| `--mdns-domain` | Custom domain name for mDNS service | `pencode.local` |
...
Setze `PENCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `pencode`, kann aber mit `PENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `pencode serve` und `pencode web`.
...
Wenn du `pencode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den pencode-Server, um pencode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `pencode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `pencode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Pencode-[IDE](/docs/ide)-Plugins.
...
Der pencode-Server stellt folgende APIs bereit.
