...
Mit Pencode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Pencode das `pencode`-Theme.
...
Pencode liefert mehrere eingebaute Themes mit.
...
- Pencode optisch ans Terminal anpassen wollen
...
Pencode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/pencode/themes/*.json` oder `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.pencode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.pencode/themes/*.json`
