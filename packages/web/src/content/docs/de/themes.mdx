...
Mit xcsh kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet xcsh das `xcsh`-Theme.
...
xcsh liefert mehrere eingebaute Themes mit.
...
- xcsh optisch ans Terminal anpassen wollen
...
xcsh unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/xcsh/themes/*.json` oder `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.xcsh/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.xcsh/themes/*.json`
