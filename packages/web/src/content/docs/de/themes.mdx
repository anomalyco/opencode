...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/leak-code/themes/*.json` oder `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.leak-code/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.leak-code/themes/*.json`
