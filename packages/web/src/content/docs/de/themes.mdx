...
Mit Octopus kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Octopus das `opencode`-Theme.
...
Octopus liefert mehrere eingebaute Themes mit.
...
- Octopus optisch ans Terminal anpassen wollen
...
Octopus unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/octopus/themes/*.json` oder `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.octopus/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.octopus/themes/*.json`
