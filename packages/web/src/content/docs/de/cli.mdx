...
Der OpenCode CLI startet standardmäßig den [TUI](./tui), wenn er ohne Argumente ausgeführt wird.
...
Dadurch wird der erforderliche GitHub-Aktionsworkflow eingerichtet und Sie durch den Konfigurationsprozess geführt. [Learn more](./github).
...
Dies ist nützlich, um die genauen Modellnamen herauszufinden, die in [your config](./config/) verwendet werden sollen.
...
Starten Sie einen Headless-OpenCode-Server für den API-Zugriff. Sehen Sie sich [server docs](./server) für die vollständige HTTP-Schnittstelle an.
