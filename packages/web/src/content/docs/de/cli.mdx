---
title: CLI
description: xcsh CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der xcsh CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
xcsh
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit xcsh interagieren.
...
Starten Sie die Terminalbenutzeroberfläche xcsh.
...
Der xcsh CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für xcsh verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden xcsh-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
xcsh attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend xcsh. Zum Beispiel:
...
xcsh wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `xcsh auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/xcsh/auth.json` gespeichert.

```bash
xcsh auth login
```

Wenn xcsh startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in xcsh sehen möchten.
...
Führen Sie xcsh im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `xcsh serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| `--attach`   |      | An einen laufenden xcsh-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-xcsh-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
xcsh serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf xcsh-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `xcsh`).
...
Listen Sie alle xcsh-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre xcsh-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer xcsh-Freigabe-URL.

```bash
xcsh import <file>
```

Sie können aus einer lokalen Datei oder einer xcsh-Freigabe-URL importieren.
...
Starten Sie einen Headless-xcsh-Server mit einer Weboberfläche.

```bash
xcsh web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf xcsh zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `xcsh`).
...
Deinstallieren Sie xcsh und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert xcsh auf die neueste Version oder eine bestimmte Version.
...
Der xcsh CLI akzeptiert die folgenden globalen Flags.
...
xcsh kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `xcsh`) |
