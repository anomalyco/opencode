...
Es gibt zwei Arten von Agenten in CyberStrike; Primäragenten und Subagenten.
...
CyberStrike verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `cyberstrike.json`-Konfigurationsdatei:
...
- Pro Projekt: `.cyberstrike/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet CyberStrike modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale CyberStrike-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer CyberStrike-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [CyberStrike Zen](/docs/zen) verwenden, würden Sie `cyberstrike/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `cyberstrike models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/CyberStrikeus/cyberstrike).
:::
