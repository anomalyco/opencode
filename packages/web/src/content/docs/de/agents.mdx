...
Es gibt zwei Arten von Agenten in Pencode; Primäragenten und Subagenten.
...
Pencode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `pencode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Pencode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Pencode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Pencode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Pencode Zen](/docs/zen) verwenden, würden Sie `pencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `pencode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/kiyosh11/pencode).
:::
