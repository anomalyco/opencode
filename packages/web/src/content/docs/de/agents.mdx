...
:::tip
Wenn Sie kein Modell angeben, verwenden Sie den primären Agenten [model globally configured](./config#models), während die Subagenten das Modell des primären Agenten verwenden, der den Subagenten aufgerufen hat.
:::
...
Das Modell ID in Ihrer OpenCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenCode Zen](./zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
