...
Es gibt zwei Arten von Agenten in Octopus; Primäragenten und Subagenten.
...
Octopus verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `octopus.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Octopus modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Octopus-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Octopus-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Octopus Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/octopus).
:::
