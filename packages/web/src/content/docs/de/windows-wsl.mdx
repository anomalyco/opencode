...
:::caution
Wenn du `--hostname 0.0.0.0` verwendest, setze `LEAKCODE_SERVER_PASSWORD`, um den Server zu schuetzen.
...
1. **Fuehre `leak-code web` im WSL-Terminal aus**, nicht in PowerShell:
...
Wenn du `leak-code web` aus WSL startest, bleiben Dateizugriff und Terminal-Integration sauber.
...
- OpenCode-Konfiguration und Sitzungen liegen in WSL unter `~/.local/share/leak-code/`
