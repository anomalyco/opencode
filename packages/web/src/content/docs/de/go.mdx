...
xcsh Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
xcsh Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in xcsh. Du abonnierst xcsh Go und
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Providern getestet, die gut mit xcsh funktionieren.
:::
...
xcsh Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

xcsh Go funktioniert wie jeder andere Provider in xcsh.

1. Du meldest dich bei **<a href={console}>xcsh Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `xcsh Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann xcsh Go abonnieren.
:::
...
xcsh Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner xcsh Config verwendet das Format `xcsh-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `xcsh-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben xcsh Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit xcsh zu nutzen.
