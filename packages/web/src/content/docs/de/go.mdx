...
Pencode Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Pencode. Du abonnierst Pencode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Pencode zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Providern getestet, die gut mit Pencode funktionieren.
:::
...
Pencode Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

Pencode Go funktioniert wie jeder andere Provider in Pencode.

1. Du meldest dich bei **<a href={console}>Pencode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Pencode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Pencode Go abonnieren.
:::
...
Pencode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Pencode Config verwendet das Format `pencode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `pencode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Pencode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Pencode zu nutzen.
