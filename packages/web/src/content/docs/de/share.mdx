...
Um den manuellen Modus explizit in der [Konfiguration](./config) zu setzen:
...
Du kannst automatisches Teilen fuer neue Unterhaltungen aktivieren, indem du in der [Konfiguration](./config) `share` auf `"auto"` setzt:
...
Du kannst Teilen komplett deaktivieren, indem du in der [Konfiguration](./config) `share` auf `"disabled"` setzt:
...
[Mehr dazu](./enterprise), wie du opencode in deiner Organisation einsetzt.
