...
:::tip[No auto-open]
To start the web server without automatically opening a browser, use `opencode serve` instead:

```bash
opencode serve --hostname 0.0.0.0 --port 4096
```

Then navigate to `http://localhost:4096` (or your network address) manually in your browser. When binding to `0.0.0.0`, set `OPENCODE_SERVER_PASSWORD` to protect access.
:::
