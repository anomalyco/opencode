...
:::caution
If server auth is disabled, the server will be unsecured. This is fine for local use but authentication should be enabled for network access.
:::
...
To protect access locally, set a password using the `OPENCODE_SERVER_PASSWORD` environment variable:
...
For network or internet-facing use, configure OIDC instead:
...
Browser requests are redirected to the identity provider and API clients can use `Authorization: Bearer <jwt>`. `OPENCODE_AUTH_SESSION_SECRET` must be at least 32 characters; set `OPENCODE_AUTH_COOKIE_SECURE=true` when serving over HTTPS.
...
OIDC auth can also be configured in the same file:
