...
To change the browser tab title, set the `OPENCODE_WEB_UI_TITLE` environment variable. This is useful when running multiple instances, since every tab would otherwise be titled `OpenCode`:
