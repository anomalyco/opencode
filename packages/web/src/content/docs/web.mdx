...
:::caution
If `IMECODE_SERVER_PASSWORD` is not set, the server will be unsecured. This is fine for local use but should be set for network access.
:::
...
To protect access, set a password using the `IMECODE_SERVER_PASSWORD` environment variable:

```bash
IMECODE_SERVER_PASSWORD=secret opencode web
```

The username defaults to `opencode` but can be changed with `IMECODE_SERVER_USERNAME`.
