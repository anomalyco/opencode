...
Les commandes personnalisées s'ajoutent aux commandes intégrées telles que `/init`, `/undo`, `/redo`, `/share`, `/help`. [En savoir plus](./tui#commands).
...
Utilisez l'option `command` dans votre OpenCode [config](./config) :
...
Utilisez _!`command`_ pour injecter la sortie [bash command](./tui#bash-commands) dans votre invite.
...
Utilisez la configuration `agent` pour spécifier éventuellement quel [agent](./agents) doit exécuter cette commande.
S'il s'agit d'un [subagent](./agents/#subagents), la commande déclenchera un appel de sous-agent par défaut.
...
Utilisez le booléen `subtask` pour forcer la commande à déclencher un invocation de [subagent](./agents/#subagents).
...
même si `mode` est défini sur `primary` dans la configuration [agent](./agents).
...
opencode comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](./tui#commands).
