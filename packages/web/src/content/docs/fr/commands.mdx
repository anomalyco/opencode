...
Créez `.cyberstrike/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration CyberStrike ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre CyberStrike [config](/docs/config) :
...
- Par projet : `.cyberstrike/commands/`
...
cyberstrike comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
