...
- Configuration du projet : `.leak-code/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/leak-code/skills/<name>/SKILL.md`
...
Il charge tout `skills/*/SKILL.md` correspondant dans `.leak-code/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/leak-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.leak-code/skills/git-release/SKILL.md` comme ceci :
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `leak-code.json` :
...
**Pour les agents intégrés** (dans `leak-code.json`) :
