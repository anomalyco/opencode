...
- **macOS/Linux** (anciennes installations) : `~/.local/share/leak-code/leak-code.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Si votre `leak-code.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `LEAKCODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `LEAKCODE_PORT` (ou choisissez un port libre) et redémarrez.
...
3. Assurez-vous d'avoir la dernière version avec `leak-code upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `leak-code models`
