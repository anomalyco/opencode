...
Pour déboguer les problèmes avec Octopus, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Octopus Desktop exécute un serveur Octopus local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Octopus` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/octopus/octopus.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Octopus peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Octopus puisse le reconstruire.

1. Quittez complètement Octopus Desktop.
...
3. Redémarrez le bureau Octopus.

---

### Résoudre les problèmes de connexion au serveur

Octopus Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `octopus.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `OCTOPUS_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `OCTOPUS_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Octopus Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Octopus.

---

### Les notifications ne s'affichent pas

Octopus Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Octopus dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Octopus.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Octopus Desktop) :
...
Si vous rencontrez des problèmes avec Octopus :
