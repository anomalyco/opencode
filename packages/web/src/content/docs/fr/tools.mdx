...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. OpenCode est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](./custom-tools) ou [serveurs MCP](./mcp-servers).

Par défaut, tous les outils sont **activés** et n'ont pas besoin d'autorisation pour s'exécuter. Vous pouvez contrôler le comportement de l'outil via [permissions](./permissions).
...
[En savoir plus](./permissions) sur la configuration des autorisations.
...
Pour configurer les serveurs LSP disponibles pour votre projet, consultez [LSP Servers](./lsp).
...
Chargez une [skill](./skills) (un fichier `SKILL.md`) et renvoyez son contenu dans la conversation.
...
:::note
Cet outil est désactivé par défaut pour les sous-agents, mais vous pouvez l'activer manuellement. [En savoir plus](./agents/#permissions)
:::
...
[En savoir plus](./custom-tools) sur la création d'outils personnalisés.
...
[En savoir plus](./mcp-servers) sur la configuration des serveurs MCP.
