...
description: Créez des outils que le LLM peut appeler dans cyberstrike.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de cyberstrike comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.cyberstrike/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/cyberstrike/tools/`.
...
import { tool } from "@cyberstrike-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".cyberstrike/tools/add.py")
