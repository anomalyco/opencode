...
Pour une expérience optimale sur Windows, nous vous recommandons d'utiliser le sous-système [Windows pour Linux (WSL)](./windows-wsl). Il offre de meilleures performances et une compatibilité totale avec les fonctionnalités de OpenCode.
:::
...
Si vous débutez avec les fournisseurs LLM, nous vous recommandons d'utiliser [OpenCode Zen](./zen).
...
Vous pouvez également sélectionner l'un des autres fournisseurs. [En savoir plus](./providers#directory).
...
Pour vous l'approprier, nous vous recommandons de [choisir un thème](./themes), [de personnaliser les raccourcis clavier](./keybinds), de [configurer les formateurs de code](./formatters), de [créer des commandes personnalisées](./commands) ou de jouer avec la [OpenCode config](./config).
