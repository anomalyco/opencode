...
Octopus Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

:::note
Octopus Go est actuellement en version bêta.
:::

Go fonctionne comme n'importe quel autre fournisseur dans Octopus. Vous vous abonnez à Octopus Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Octopus.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec Octopus.
:::
...
Octopus Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

Octopus Go fonctionne comme n'importe quel autre fournisseur dans Octopus.

1. Vous vous connectez à **<a href={console}>Octopus Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `Octopus Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à Octopus Go.
:::
...
Octopus Go inclut les limites suivantes :
...
L'[ID de modèle](/docs/config/#models) dans votre configuration Octopus utilise le format `opencode-go/<model-id>`. Par exemple, pour Kimi K2.6, vous utiliseriez `opencode-go/kimi-k2.6` dans votre configuration.
...
Nous avons créé Octopus Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec Octopus également.
