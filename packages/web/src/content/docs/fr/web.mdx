...
:::caution
Si `LEAKCODE_SERVER_PASSWORD` n'est pas défini, le serveur ne sera pas sécurisé. C'est bien pour une utilisation locale mais doit être défini pour l'accès au réseau.
:::

:::tip[Windows Utilisateurs]
Pour une expérience optimale, exécutez `leak-code web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Pour protéger l'accès, définissez un mot de passe à l'aide de la variable d'environnement `LEAKCODE_SERVER_PASSWORD` :

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Le nom d'utilisateur par défaut est `opencode` mais peut être modifié avec `LEAKCODE_SERVER_USERNAME`.
...
Vous pouvez également configurer les paramètres du serveur dans votre fichier de configuration `leak-code.json` :
