...
:::tip[Sans ouverture automatique]
Pour démarrer le serveur web sans ouvrir automatiquement un navigateur, utilisez `opencode serve` à la place :

```bash
opencode serve --hostname 0.0.0.0 --port 4096
```

Naviguez ensuite manuellement vers `http://localhost:4096` (ou votre adresse réseau). Lors de la liaison à `0.0.0.0`, définissez `OPENCODE_SERVER_PASSWORD` pour protéger l'accès.
:::
