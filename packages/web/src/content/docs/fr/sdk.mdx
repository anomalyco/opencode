---
title: SDK
description: Client JS de type sécurisé pour le serveur xcsh.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK xcsh JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler xcsh par programme.
...
```bash
npm install @f5xc-salesdemos/sdk
```
...
Créez une instance de xcsh :

```javascript
import { createOpencode } from "@f5xc-salesdemos/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `xcsh.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@f5xc-salesdemos/sdk"

const xcsh = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${xcsh.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de xcsh, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@f5xc-salesdemos/sdk"
```
