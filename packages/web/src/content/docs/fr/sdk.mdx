---
title: SDK
description: Client JS de type sécurisé pour le serveur octopus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK octopus JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler octopus par programme.
...
Créez une instance de octopus :

```javascript
import { createOctopus } from "@octopus-ai/sdk"

const { client } = await createOctopus()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `octopus.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOctopus } from "@octopus-ai/sdk"

const octopus = await createOctopus({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${octopus.server.url}`)

octopus.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de octopus, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@octopus-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*octopus" },
})
