---
title: SDK
description: Client JS de type sécurisé pour le serveur pencode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK pencode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler pencode par programme.
...
Créez une instance de pencode :

```javascript
import { createPencode } from "@pencode-ai/sdk"

const { client } = await createPencode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `pencode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createPencode } from "@pencode-ai/sdk"

const pencode = await createPencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${pencode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de pencode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@pencode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*pencode" },
})
