---
title: SDK
description: Client JS de type sécurisé pour le serveur cyberstrike.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK cyberstrike JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler cyberstrike par programme.
...
```bash
npm install @cyberstrike-ai/sdk
```
...
Créez une instance de cyberstrike :

```javascript
import { createCyberstrike } from "@cyberstrike-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `cyberstrike.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
Si vous disposez déjà d'une instance en cours d'exécution de cyberstrike, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createCyberstrikeClient } from "@cyberstrike-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@cyberstrike-ai/sdk"
```
