---
title: Serveur
description: Interagissez avec le serveur xcsh via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `xcsh serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client xcsh peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `xcsh.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `xcsh`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `xcsh serve` et à `xcsh web`.
...
Lorsque vous exécutez `xcsh`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur xcsh pour interagir avec xcsh par programmation.
:::

Cette architecture permet à xcsh de prendre en charge plusieurs clients et vous permet d'interagir avec xcsh par programme.

Vous pouvez exécuter `xcsh serve` pour démarrer un serveur autonome. Si vous avez le
xcsh TUI en cours d'exécution, `xcsh serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins xcsh [IDE](/docs/ide).
...
Le serveur xcsh expose les API suivantes.
