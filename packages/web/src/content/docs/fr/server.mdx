...
La commande `leak-code serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opencode peut utiliser.
...
Définissez `LEAKCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `opencode`, ou définissez `LEAKCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `leak-code serve` et à `leak-code web`.
...
Vous pouvez exécuter `leak-code serve` pour démarrer un serveur autonome. Si vous avez le
opencode TUI en cours d'exécution, `leak-code serve` démarrera un nouveau serveur.
