---
title: Serveur
description: Interagissez avec le serveur pencode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `pencode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client pencode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `pencode.local` |
...
Définissez `PENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `pencode`, ou définissez `PENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `pencode serve` et à `pencode web`.
...
Lorsque vous exécutez `pencode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur pencode pour interagir avec pencode par programmation.
:::

Cette architecture permet à pencode de prendre en charge plusieurs clients et vous permet d'interagir avec pencode par programme.

Vous pouvez exécuter `pencode serve` pour démarrer un serveur autonome. Si vous avez le
pencode TUI en cours d'exécution, `pencode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Pencode [IDE](/docs/ide).
...
Le serveur pencode expose les API suivantes.
