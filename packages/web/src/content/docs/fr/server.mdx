---
title: Serveur
description: Interagissez avec le serveur cyberstrike via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `cyberstrike serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client cyberstrike peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `cyberstrike.local` |
...
Définissez `CYBERSTRIKE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `cyberstrike`, ou définissez `CYBERSTRIKE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `cyberstrike serve` et à `cyberstrike web`.
...
Lorsque vous exécutez `cyberstrike`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur cyberstrike pour interagir avec cyberstrike par programmation.
:::

Cette architecture permet à cyberstrike de prendre en charge plusieurs clients et vous permet d'interagir avec cyberstrike par programme.

Vous pouvez exécuter `cyberstrike serve` pour démarrer un serveur autonome. Si vous avez le
cyberstrike TUI en cours d'exécution, `cyberstrike serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CyberStrike [IDE](/docs/ide).
...
Le serveur cyberstrike expose les API suivantes.
