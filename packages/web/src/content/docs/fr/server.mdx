...
point final. Ce point de terminaison est également utilisé pour générer un [SDK](./sdk).
...
Lorsque vous démarrez le TUI, il attribue de manière aléatoire un port et un nom d'hôte. Vous pouvez à la place transmettre les `--hostname` et `--port` [flags](./cli). Utilisez-le ensuite pour vous connecter à son serveur.

Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenCode [IDE](./ide).
