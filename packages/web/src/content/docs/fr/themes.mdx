...
Avec CyberStrike, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CyberStrike utilise notre propre thème `cyberstrike`.
...
CyberStrike est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CyberStrike corresponde à l'apparence de leur terminal
...
CyberStrike prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/cyberstrike/themes/*.json` ou `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.cyberstrike/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.cyberstrike/themes/*.json`
