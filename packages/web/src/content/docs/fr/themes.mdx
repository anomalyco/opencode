...
Avec Octopus, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Octopus utilise notre propre thème `opencode`.
...
Octopus est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Octopus corresponde à l'apparence de leur terminal
...
Octopus prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/octopus/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.octopus/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.octopus/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/octopus/themes
vim ~/.config/octopus/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .octopus/themes
vim .octopus/themes/my-theme.json
```
