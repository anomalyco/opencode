...
Avec Pencode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Pencode utilise notre propre thème `pencode`.
...
Pencode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Pencode corresponde à l'apparence de leur terminal
...
Pencode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/pencode/themes/*.json` ou `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.pencode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.pencode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/pencode/themes
vim ~/.config/pencode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .pencode/themes
vim .pencode/themes/my-theme.json
```
...
{
  "$schema": "https://pencode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
