...
Avec xcsh, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, xcsh utilise notre propre thème `xcsh`.
...
xcsh est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que xcsh corresponde à l'apparence de leur terminal
...
xcsh prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/xcsh/themes/*.json` ou `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.xcsh/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.xcsh/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/xcsh/themes
vim ~/.config/xcsh/themes/my-theme.json
```
...
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
