...
testé et vérifié pour fonctionner correctement avec OpenCode. [En savoir plus](./zen).
...
OpenCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenCode. [En savoir plus](./zen).
...
- **apiKey** : défini à l'aide de la syntaxe de variable `env`, [en savoir plus](./config#env-vars).
