---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration xcsh.
...
en `~/.local/share/xcsh/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre xcsh
configuration.
...
xcsh Zen est une liste de modèles fournis par l'équipe xcsh qui ont été
testé et vérifié pour fonctionner correctement avec xcsh. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par xcsh Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez xcsh et dirigez-vous vers [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans xcsh et son utilisation est totalement facultative.

---

## xcsh Go

xcsh Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe xcsh qui ont été
testé et vérifié pour fonctionner correctement avec xcsh.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `xcsh Go` et rendez-vous sur [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Pour utiliser Amazon Bedrock avec xcsh :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de xcsh :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY xcsh

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile xcsh

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX xcsh
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `xcsh.json` :

```json title="xcsh.json"
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans xcsh n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que xcsh fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX xcsh
   ```
...
Vous pouvez également ajouter des modèles via votre configuration xcsh.
...
pour stocker le jeton dans le stockage d'authentification xcsh.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
xcsh utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller xcsh
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `xcsh.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth).

##### Configuration

Personnalisez via `xcsh.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec xcsh :
...
Pour utiliser Google Vertex AI avec xcsh :
...
Définissez-les lors de l'exécution de xcsh.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id xcsh
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via xcsh, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/xcsh/xcsh.jsonc"
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) pour enregistrer automatiquement chaque conversation xcsh en tant que session dans Helicone.

```bash
npm install -g xcsh-helicone-session
```
...
```json title="xcsh.json"
{
  "plugin": ["xcsh-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation xcsh répertoriée comme une session distincte.
...
Vous pouvez configurer xcsh pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="xcsh.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer xcsh pour utiliser des modèles locaux via LM Studio.

```json title="xcsh.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer xcsh pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour xcsh. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/xcsh) pour plus de détails.
:::

```json title="xcsh.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec xcsh :
...
3. Copiez la clé API à utiliser dans xcsh.
...
6. **Important** : Avant d'utiliser des modèles cloud dans xcsh, vous devez extraire les informations du modèle localement :
...
xcsh Zen est une liste de modèles testés et vérifiés fournie par l'équipe xcsh. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>xcsh Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **xcsh Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé xcsh API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration xcsh.
...
5. Vous pouvez également les personnaliser via votre configuration xcsh. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' xcsh
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group xcsh
   ```
...
Vous pouvez également personnaliser les modèles via votre configuration xcsh. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec xcsh. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in xcsh.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `xcsh.json` dans le répertoire de votre projet :

   ```json title="xcsh.json" ""myprovider"" {5-15}
   {
     "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
     "provider": {
       "myprovider": {
...
```json title="xcsh.json" {9,11,17-20}
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à xcsh de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `xcsh auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration xcsh et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration xcsh.
