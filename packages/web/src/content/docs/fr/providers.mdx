...
en `~/.local/share/leak-code/auth.json`.
...
Pour une configuration spécifique au projet ou persistante, utilisez `leak-code.json` :
...
Fichier `leak-code.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `leak-code.json` :
...
4. Créez ou mettez à jour votre fichier `leak-code.json` dans le répertoire de votre projet :
...
1. **Vérifiez la configuration de l'authentification** : exécutez `leak-code auth list` pour voir si les informations d'identification
