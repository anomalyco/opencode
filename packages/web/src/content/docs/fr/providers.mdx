---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en CyberStrike.
---

import config from "../../../../config.mjs"
export const console = config.console

CyberStrike utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration CyberStrike.
...
en `~/.local/share/cyberstrike/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre CyberStrike
configuration.
...
CyberStrike Zen est une liste de modèles fournis par l'équipe CyberStrike qui ont été
testé et vérifié pour fonctionner correctement avec CyberStrike. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par CyberStrike Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez cyberstrike et dirigez-vous vers [cyberstrike.io/auth](https://cyberstrike.io/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans CyberStrike et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec CyberStrike :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de cyberstrike :
...
Pour une configuration spécifique au projet ou persistante, utilisez `cyberstrike.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans CyberStrike n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que cyberstrike fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration cyberstrike.
...
pour stocker le jeton dans le stockage d'authentification cyberstrike.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
CyberStrike utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller CyberStrike
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `cyberstrike.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `cyberstrike.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec cyberstrike :
...
Pour utiliser Google Vertex AI avec CyberStrike :
...
Définissez-les lors de l'exécution de cyberstrike.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via cyberstrike, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/cyberstrike/cyberstrike.jsonc"
{
  "$schema": "https://cyberstrike.io/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) pour enregistrer automatiquement chaque conversation CyberStrike en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation CyberStrike répertoriée comme une session distincte.
...
Vous pouvez configurer cyberstrike pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer cyberstrike pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer cyberstrike pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour CyberStrike. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/cyberstrike) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec CyberStrike :
...
3. Copiez la clé API à utiliser dans CyberStrike.
...
6. **Important** : Avant d'utiliser des modèles cloud dans CyberStrike, vous devez extraire les informations du modèle localement :
...
CyberStrike Zen est une liste de modèles testés et vérifiés fournie par l'équipe CyberStrike. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>CyberStrike Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **CyberStrike Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé CyberStrike API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration cyberstrike.
...
5. Vous pouvez également les personnaliser via votre configuration cyberstrike. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Cyberstrike :
...
Vous pouvez également personnaliser les modèles via votre configuration cyberstrike. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec cyberstrike. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `cyberstrike.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à CyberStrike de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `cyberstrike auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration cyberstrike et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration cyberstrike.
