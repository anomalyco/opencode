---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

Octopus utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Octopus.
...
en `~/.local/share/octopus/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Octopus
configuration.
...
Octopus Zen est une liste de modèles fournis par l'équipe Octopus qui ont été
testé et vérifié pour fonctionner correctement avec Octopus. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Octopus Zen.
:::
...
Il fonctionne comme n’importe quel autre fournisseur dans Octopus et son utilisation est totalement facultative.

---

## Octopus Go

Octopus Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Octopus qui ont été
testé et vérifié pour fonctionner correctement avec Octopus.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Octopus Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec Octopus :
...
Pour une configuration spécifique au projet ou persistante, utilisez `octopus.json` :

```json title="octopus.json"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Octopus n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
```json title="octopus.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
Octopus utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Octopus
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `octopus.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `octopus.json` :
...
Pour utiliser Google Vertex AI avec Octopus :
...
```jsonc title="~/.config/octopus/octopus.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Octopus en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Octopus répertoriée comme une session distincte.
...
```json title="octopus.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="octopus.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
:::tip
Ollama peut se configurer automatiquement pour Octopus. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::

```json title="octopus.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Octopus :
...
3. Copiez la clé API à utiliser dans Octopus.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Octopus, vous devez extraire les informations du modèle localement :
...
Octopus Zen est une liste de modèles testés et vérifiés fournie par l'équipe Octopus. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Octopus Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Octopus Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Octopus API.
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in octopus.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `octopus.json` dans le répertoire de votre projet :

   ```json title="octopus.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="octopus.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Octopus de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
