---
title: Zen
description: Liste organisée de modèles fournis par Pencode.
---
...
Pencode Zen est une liste de modèles testés et vérifiés fournie par l'équipe Pencode.

Zen fonctionne comme n'importe quel autre fournisseur dans Pencode. Vous vous connectez à Pencode Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Pencode.
...
Pencode Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

Pencode Zen fonctionne comme n'importe quel autre fournisseur dans Pencode.

1. Vous vous connectez à **<a href={console}>Pencode Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez Pencode Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration Pencode utilise le format `pencode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `pencode/gpt-5.5` dans votre configuration.
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. Pencode utilise ces modèles pour générer les titres des sessions.
...
- MiMo-V2.5 Free est disponible sur Pencode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Hy3 Free est disponible sur Pencode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur Pencode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3.5 Lightning Free est disponible sur Pencode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur Pencode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Ox Alpha Free est un modèle stealth gratuit sur Pencode pour une durée limitée. Son fournisseur applique une politique de conservation nulle et n'utilise pas vos données pour entraîner des modèles.
- Muse Spark 1.2 Contributor Free est disponible sur Pencode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé Pencode Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec Pencode également.
