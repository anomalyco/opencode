...
Le [id du modèle](./config/#models) dans votre configuration OpenCode
...
Vous remarquerez peut-être _Claude Haiku 3.5_ dans votre historique d'utilisation. Il s'agit d'un [modèle low cost](./config/#models) qui est utilisé pour générer les titres de vos sessions.
