...
2. **Configuration globale** (`~/.config/leak-code/leak-code.json`) - préférences utilisateur
3. **Configuration personnalisée** (`LEAKCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`leak-code.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.leak-code`** - agents, commandes, plugins
6. **Configuration en ligne** (`LEAKCODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.leak-code` et `~/.config/leak-code` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Placez votre configuration globale OpenCode dans `~/.config/leak-code/leak-code.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/leak-code/tui.json`.
...
Ajoutez `leak-code.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `LEAKCODE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `LEAKCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.leak-code`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.leak-code`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `LEAKCODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `leak-code.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `leak-code serve` et `leak-code web` via l'option `server`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/leak-code/agents/` ou `.leak-code/agents/`. [En savoir plus ici](/docs/agents).
...
Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`leak-code run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/leak-code/commands/` ou `.leak-code/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.leak-code/plugins/` ou `~/.config/leak-code/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
