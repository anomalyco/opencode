...
3. **Configuration personnalisée** (`IMECODE_CONFIG` env var) - remplacements personnalisés
...
6. **Configuration en ligne** (`IMECODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `IMECODE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `IMECODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.
...
Utilisez `IMECODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.
