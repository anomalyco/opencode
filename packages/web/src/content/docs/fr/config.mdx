---
title: Configuration
description: Utilisation de la configuration JSON Octopus.
---

Vous pouvez configurer Octopus à l'aide d'un fichier de configuration JSON.

---

## Format

Octopus prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
2. **Configuration globale** (`~/.config/octopus/octopus.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OCTOPUS_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`octopus.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.opencode`** - agents, commandes, plugins
6. **Configuration en ligne** (`OCTOPUS_CONFIG_CONTENT` env var) - remplacements d'exécution
...
Placez votre configuration globale Octopus dans `~/.config/octopus/octopus.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/octopus/tui.json`.
...
Ajoutez `octopus.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Octopus démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `OCTOPUS_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OCTOPUS_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.
...
Utilisez `OCTOPUS_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `octopus.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Octopus.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Octopus via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Octopus essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/octopus/agents/` ou `.octopus/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Octopus reviendra à `"build"` avec un avertissement.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/octopus/commands/` ou `.octopus/commands/`. [En savoir plus ici](/docs/commands).
...
Octopus téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent Octopus avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.octopus/plugins/` ou `~/.config/octopus/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre Octopus à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
