...
[En savoir plus sur l'utilisation du TUI ici](./tui).
...
[En savoir plus sur le serveur ici](./server).
...
[En savoir plus sur les outils ici](./tools).
...
Vous pouvez également configurer [modèles locaux](./models#local). [En savoir plus](./models).
...
:::note
Les jetons du porteur (`AWS_BEARER_TOKEN_BEDROCK` ou `/connect`) ont priorité sur l'authentification basée sur le profil. Voir [précédence d'authentification](./providers#authentication-precedence) pour plus de détails.
:::

[En savoir plus sur la configuration d'Amazon Bedrock](./providers#amazon-bedrock).
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/opencode/agents/` ou `.opencode/agents/`. [En savoir plus ici](./agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](./agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, OpenCode reviendra à `"build"` avec un avertissement.
...
Vous pouvez configurer la fonctionnalité [share](./share) via l'option `share`.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/opencode/commands/` ou `.opencode/commands/`. [En savoir plus ici](./commands).
...
[En savoir plus sur les formateurs ici](./formatters).
...
[En savoir plus sur les autorisations ici](./permissions).
...
[En savoir plus ici](./mcp-servers).

---

### Plugins

[Plugins](./plugins) étendent OpenCode avec des outils, des hooks et des intégrations personnalisés.
