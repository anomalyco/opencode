...
Vos valeurs de configuration locales remplacent les valeurs par défaut distantes. Voir [config precedence](./config#precedence-order) pour plus de détails.
...
2. Dans votre [agent config](./agents#tools), activez le serveur MCP en tant qu'outil.
...
Alternativement, vous pouvez ajouter quelque chose comme ceci à votre [AGENTS.md](./rules/).
