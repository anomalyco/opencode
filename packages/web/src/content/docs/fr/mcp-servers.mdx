...
Vous pouvez ajouter des outils externes à Octopus à l'aide du _Model Context Protocol_ ou MCP. Octopus prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Octopus Config](https://opencode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Octopus gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Octopus :
...
Si le serveur nécessite une authentification, Octopus vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `opencode mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Octopus stockera les jetons en toute sécurité dans `~/.local/share/octopus/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Octopus, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Octopus comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Octopus à votre compte Sentry.
