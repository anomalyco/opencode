...
La plupart des commandes ont également une liaison clavier utilisant `ctrl+x` comme touche leader, où `ctrl+x` est la touche leader par défaut. [En savoir plus](./keybinds).
...
Créez ou mettez à jour le fichier `AGENTS.md`. [En savoir plus](./rules).
...
Partager la session en cours. [En savoir plus](./share).
...
Annuler le partage de la session en cours. [En savoir plus](./share#un-sharing).
