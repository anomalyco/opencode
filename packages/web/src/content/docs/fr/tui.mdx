---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Pencode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Pencode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Pencode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Pencode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Pencode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `pencode.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Pencode attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `pencode.default`.
