...
Pour des exemples, consultez les [plugins](./ecosystem#plugins) créés par la communauté.
...
Parcourez les plugins disponibles dans le [ecosystem](./ecosystem#plugins).
...
Les **plugins locaux** sont chargés directement depuis le répertoire des plugins. Pour utiliser des packages externes, vous devez créer un `package.json` dans votre répertoire de configuration (voir [Dépendances](#dependencies)), ou publier le plugin sur npm et [l'ajouter à votre config](./config#plugins).
