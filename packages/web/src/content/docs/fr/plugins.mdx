---
title: Plugins
description: Écrivez vos propres plugins pour étendre Pencode.
---

Les plugins vous permettent d'étendre Pencode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Pencode.
...
- `.pencode/plugins/` - Plugins au niveau du projet
- `~/.config/pencode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/pencode/node_modules/`.
...
1. Configuration globale (`~/.config/pencode/pencode.json`)
2. Configuration du projet (`pencode.json`)
3. Répertoire global des plugins (`~/.config/pencode/plugins/`)
4. Répertoire des plugins du projet (`.pencode/plugins/`)
...
Pencode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK pencode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre pencode.
...
:::note
Si vous utilisez l'application de bureau Pencode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez pencode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à pencode :
...
L'assistant `tool` crée un outil personnalisé que pencode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour pencode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://pencode.ai/docs/sdk) pour plus de détails.
