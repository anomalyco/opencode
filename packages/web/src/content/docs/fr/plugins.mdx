---
title: Plugins
description: Écrivez vos propres plugins pour étendre Octopus.
---

Les plugins vous permettent d'étendre Octopus en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Octopus.
...
- `.octopus/plugins/` - Plugins au niveau du projet
- `~/.config/octopus/plugins/` - Plugins globaux
...
1. Configuration globale (`~/.config/octopus/octopus.json`)
2. Configuration du projet (`octopus.json`)
3. Répertoire global des plugins (`~/.config/octopus/plugins/`)
4. Répertoire des plugins du projet (`.octopus/plugins/`)
...
Octopus exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
:::note
Si vous utilisez l'application de bureau Octopus, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
