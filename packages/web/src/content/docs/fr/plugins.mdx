---
title: Plugins
description: Écrivez vos propres plugins pour étendre CyberStrike.
---

Les plugins vous permettent d'étendre CyberStrike en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CyberStrike.
...
- `.cyberstrike/plugins/` - Plugins au niveau du projet
- `~/.config/cyberstrike/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/cyberstrike/node_modules/`.
...
1. Configuration globale (`~/.config/cyberstrike/cyberstrike.json`)
2. Configuration du projet (`cyberstrike.json`)
3. Répertoire global des plugins (`~/.config/cyberstrike/plugins/`)
4. Répertoire des plugins du projet (`.cyberstrike/plugins/`)
...
CyberStrike exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK cyberstrike pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre cyberstrike.
...
:::note
Si vous utilisez l'application de bureau CyberStrike, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez cyberstrike de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à cyberstrike :
...
L'assistant `tool` crée un outil personnalisé que cyberstrike peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour cyberstrike aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://cyberstrike.io/docs/sdk) pour plus de détails.
