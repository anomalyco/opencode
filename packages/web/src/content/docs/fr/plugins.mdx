---
title: Plugins
description: Écrivez vos propres plugins pour étendre xcsh.
---

Les plugins vous permettent d'étendre xcsh en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de xcsh.
...
- `.xcsh/plugins/` - Plugins au niveau du projet
- `~/.config/xcsh/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/xcsh/node_modules/`.
...
1. Configuration globale (`~/.config/xcsh/xcsh.json`)
2. Configuration du projet (`xcsh.json`)
3. Répertoire global des plugins (`~/.config/xcsh/plugins/`)
4. Répertoire des plugins du projet (`.xcsh/plugins/`)
...
xcsh exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK xcsh pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre xcsh.
...
:::note
Si vous utilisez l'application de bureau xcsh, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez xcsh de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à xcsh :
...
L'assistant `tool` crée un outil personnalisé que xcsh peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour xcsh aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) pour plus de détails.
