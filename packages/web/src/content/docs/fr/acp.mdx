---
title: Support ACP
description: Utilisez xcsh dans n’importe quel éditeur compatible ACP.
---

xcsh prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser xcsh via ACP, configurez votre éditeur pour exécuter la commande `xcsh acp`.

La commande démarre xcsh en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "xcsh": {
      "command": "/absolute/path/bin/xcsh",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « xcsh » dans le sélecteur d'agent AI Chat.
...
Pour utiliser xcsh comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser xcsh comme agent ACP pour le chat.
...
xcsh fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration xcsh
