...
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `OPENCODE_AUTH_CONTENT                ` | chaîne  | Identifiants d'authentification JSON en ligne |
...
| `OPENCODE_DISABLE_EXTERNAL_SKILLS     ` | booléen | Désactiver le chargement des compétences depuis les paquets npm |
...
| `OPENCODE_DISABLE_MOUSE               ` | booléen | Désactiver la capture de la souris dans le TUI |
| `OPENCODE_DISABLE_PRUNE               ` | booléen | Désactiver le nettoyage des anciennes données |
| `OPENCODE_DISABLE_PROJECT_CONFIG      ` | booléen | Désactiver le chargement de la config au niveau du projet |
...
| `OPENCODE_MODELS_PATH                 ` | chaîne  | Chemin personnalisé pour le fichier cache JSON des modèles |
...
| `EXA_API_KEY                          ` | chaîne  | Clé API pour la recherche web Exa (requiert `OPENCODE_ENABLE_EXA`) |
| `OTEL_EXPORTER_OTLP_ENDPOINT          ` | chaîne  | Point de terminaison de l'exportateur OpenTelemetry OTLP |
| `OTEL_EXPORTER_OTLP_HEADERS           ` | chaîne  | En-têtes de l'exportateur OpenTelemetry OTLP |
| `OTEL_RESOURCE_ATTRIBUTES             ` | chaîne  | Attributs de ressource OpenTelemetry (paires clé=valeur) |
...
| `OPENCODE_EXPERIMENTAL_HTTPAPI                  ` | booléen | Activer le pont Effect HttpApi |
...
| `OPENCODE_EXPERIMENTAL_WORKSPACES               ` | booléen | Activer les espaces de travail expérimentaux |
