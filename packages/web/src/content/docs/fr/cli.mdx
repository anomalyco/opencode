---
title: CLI
description: Options et commandes de la CLI Octopus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI Octopus démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
opencode
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec Octopus par programme.
...
Démarrez l’interface utilisateur du terminal Octopus.
...
La CLI Octopus dispose également des commandes suivantes.

---

### agent

Gérer les agents pour Octopus.
...
Attachez un terminal à un serveur backend Octopus déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
opencode attach [url]
```

Cela permet d'utiliser le TUI avec un backend Octopus distant. Par exemple :
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `OCTOPUS_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OCTOPUS_SERVER_USERNAME` ou `opencode`) |
...
Octopus est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `opencode auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/octopus/auth.json`.

```bash
opencode auth login
```

Lorsque Octopus démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans Octopus.
...
Démarrez un serveur Octopus sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
opencode serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'Octopus sans l'interface TUI. Définissez `OCTOPUS_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Liste toutes les sessions Octopus.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions Octopus.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage Octopus.

```bash
opencode import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage Octopus.
...
Démarrez un serveur Octopus sans interface graphique avec une interface Web.

```bash
opencode web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à Octopus via une interface Web. Définissez `OCTOPUS_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Désinstallez Octopus et supprimez tous les fichiers associés.
...
Octopus peut être configuré à l'aide de variables d'environnement.

| Variables                             | Type    | Description                                                                       |
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `OCTOPUS_AUTO_SHARE`                 | booléen | Partager automatiquement des sessions                                             |
| `OCTOPUS_GIT_BASH_PATH`              | chaîne  | Chemin vers l'exécutable Git Bash sur Windows                                     |
| `OCTOPUS_CONFIG`                     | chaîne  | Chemin d'accès au fichier de configuration                                        |
| `OCTOPUS_TUI_CONFIG`                 | chaîne  | Chemin d'accès au fichier de configuration TUI                                    |
| `OCTOPUS_CONFIG_DIR`                 | chaîne  | Chemin d'accès au répertoire de configuration                                     |
| `OCTOPUS_CONFIG_CONTENT`             | chaîne  | Contenu de configuration JSON en ligne                                            |
| `OCTOPUS_DISABLE_AUTOUPDATE`         | booléen | Désactiver les vérifications automatiques des mises à jour                        |
| `OCTOPUS_DISABLE_PRUNE`              | booléen | Désactiver le nettoyage des anciennes données                                     |
| `OCTOPUS_DISABLE_TERMINAL_TITLE`     | booléen | Désactiver les mises à jour automatiques des titres du terminal                   |
| `OCTOPUS_PERMISSION`                 | chaîne  | Configuration des autorisations JSON intégrées                                    |
| `OCTOPUS_DISABLE_DEFAULT_PLUGINS`    | booléen | Désactiver les plugins par défaut                                                 |
| `OCTOPUS_DISABLE_LSP_DOWNLOAD`       | booléen | Désactiver les téléchargements automatiques du serveur LSP                        |
| `OCTOPUS_ENABLE_EXPERIMENTAL_MODELS` | booléen | Activer les modèles expérimentaux                                                 |
| `OCTOPUS_DISABLE_AUTOCOMPACT`        | booléen | Désactiver le compactage automatique du contexte                                  |
| `OCTOPUS_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (prompt + compétences)                         |
| `OCTOPUS_DISABLE_CLAUDE_CODE_PROMPT` | booléen | Désactiver la lecture `~/.claude/CLAUDE.md`                                       |
| `OCTOPUS_DISABLE_CLAUDE_CODE_SKILLS` | booléen | Désactiver le chargement de `.claude/skills`                                      |
| `OCTOPUS_DISABLE_MODELS_FETCH`       | booléen | Désactiver la récupération de modèles à partir de sources distantes               |
| `OCTOPUS_FAKE_VCS`                   | chaîne  | Faux fournisseur VCS à des fins de test                                           |
| `OCTOPUS_CLIENT`                     | chaîne  | Identifiant du client (par défaut `cli`)                                          |
| `OCTOPUS_ENABLE_EXA`                 | booléen | Activer les outils de recherche Web Exa                                           |
| `OCTOPUS_SERVER_PASSWORD`            | chaîne  | Activer l'authentification de base pour `serve`/`web`                             |
| `OCTOPUS_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencode`) |
| `OCTOPUS_MODELS_URL`                 | chaîne  | URL personnalisé pour récupérer la configuration des modèles                      |
...
| Variables                                       | Type    | Description                                                     |
| ----------------------------------------------- | ------- | --------------------------------------------------------------- |
| `OCTOPUS_EXPERIMENTAL`                         | booléen | Activer toutes les fonctionnalités expérimentales               |
| `OCTOPUS_EXPERIMENTAL_ICON_DISCOVERY`          | booléen | Activer la découverte d'icônes                                  |
| `OCTOPUS_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booléen | Désactiver la copie lors de la sélection dans TUI               |
| `OCTOPUS_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash en ms     |
| `OCTOPUS_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | numéro  | Nombre maximum de jetons de sortie pour les réponses LLM        |
| `OCTOPUS_EXPERIMENTAL_FILEWATCHER`             | booléen | Activer l'observateur de fichiers pour l'ensemble du répertoire |
| `OCTOPUS_EXPERIMENTAL_OXFMT`                   | booléen | Activer le formateur oxfmt                                      |
| `OCTOPUS_EXPERIMENTAL_LSP_TOOL`                | booléen | Activer l'outil expérimental LSP                                |
| `OCTOPUS_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booléen | Désactiver l'observateur de fichiers                            |
| `OCTOPUS_EXPERIMENTAL_EXA`                     | booléen | Activer les fonctionnalités Exa expérimentales                  |
| `OCTOPUS_EXPERIMENTAL_LSP_TY`                  | booléen | Activer TY LSP pour les fichiers python                         |
| `OCTOPUS_EXPERIMENTAL_MARKDOWN`                | booléen | Activer les fonctionnalités Markdown expérimentales             |
| `OCTOPUS_EXPERIMENTAL_PLAN_MODE`               | booléen | Activer le mode plan                                            |
