...
La CLI OpenCode démarre par défaut le [TUI](./tui) lorsqu'il est exécuté sans aucun argument.
...
Cela configure le flux de travail GitHub Actions nécessaire et vous guide tout au long du processus de configuration. [En savoir plus](./github).
...
Ceci est utile pour déterminer le nom exact du modèle à utiliser dans [votre config](./config/).
...
Démarrez un serveur OpenCode sans tête pour un accès API. Consultez le [serveur docs](./server) pour l'interface complète HTTP.
