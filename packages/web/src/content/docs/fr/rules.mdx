...
leak-code prend également en charge la lecture du fichier `AGENTS.md` à partir de plusieurs emplacements. Et cela répond à des objectifs différents.
...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/leak-code/AGENTS.md`. Ceci est appliqué à toutes les sessions opencode.
...
- **Règles globales** : `~/.claude/CLAUDE.md` (utilisé si aucun `~/.config/leak-code/AGENTS.md` n'existe)
...
2. **Fichier global** à `~/.config/leak-code/AGENTS.md`
3. **Fichier Claude Code** à `~/.claude/CLAUDE.md` (sauf si désactivé)

Le premier fichier correspondant gagne dans chaque catégorie. Par exemple, si vous possédez à la fois `AGENTS.md` et `CLAUDE.md`, seul `AGENTS.md` est utilisé. De même, `~/.config/leak-code/AGENTS.md` a priorité sur `~/.claude/CLAUDE.md`.

---

## Instructions personnalisées

Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `leak-code.json` ou dans le `~/.config/leak-code/leak-code.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de leak-code.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `leak-code.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `leak-code.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
