...
Il existe deux types d'agents dans CyberStrike ; agents primaires et sous-agents.
...
CyberStrike est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
CyberStrike est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

CyberStrike est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `cyberstrike.json` :
...
Si aucune température n'est spécifiée, CyberStrike utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale CyberStrike et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration CyberStrike utilise le format `provider/model-id`. Par exemple, si vous utilisez [CyberStrike Zen](/docs/zen), vous utiliserez `cyberstrike/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `cyberstrike models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/CyberStrikeus/cyberstrike).
:::
