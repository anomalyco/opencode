...
:::tip
Si vous ne spécifiez pas de modèle, les agents principaux utilisent le [modèle globalement configuré](./config#models) tandis que les sous-agents utiliseront le modèle de l'agent principal qui a appelé le sous-agent.
:::
...
L'ID de modèle dans votre configuration OpenCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenCode Zen](./zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
[En savoir plus sur les outils](./tools).
...
[En savoir plus sur les autorisations](./permissions).
