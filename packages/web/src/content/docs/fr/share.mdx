...
Pour définir explicitement le mode manuel dans votre [fichier de configuration](./config) :
...
Vous pouvez activer le partage automatique pour toutes les nouvelles conversations en définissant l'option `share` sur `"auto"` dans votre [fichier de configuration](./config) :
...
Vous pouvez désactiver entièrement le partage en définissant l'option `share` sur `"disabled"` dans votre [fichier de configuration](./config) :
...
[En savoir plus](./enterprise) sur l'utilisation de opencode dans votre organisation.
