...
[marimo](https://marimo.io) connects to ACP agents over WebSocket. Use `stdio-to-ws` to bridge OpenCode's stdio transport:
...
Then open the agents panel in marimo's sidebar, select OpenCode from the dropdown, and start chatting.

:::tip
To have cells run automatically when the agent modifies your notebook, add this to your `pyproject.toml`:
