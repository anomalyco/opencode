...
Respond via `postSessionByIdPermissionsByPermissionId` using the root `sessionID`. An `always` response applies instance-wide for the rest of the OpenCode process — it covers the entire session tree, and also any other sessions served by the same instance.
