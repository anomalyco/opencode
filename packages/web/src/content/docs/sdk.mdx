...
| `session.command({ path, body })`                          | Send command to session            | `body.noReply: true` returns UserMessage without running the agent (the `command.executed` event's `messageID` is then the user message). Default returns <a href={typesUrl}><code>AssistantMessage</code></a> with AI response. |
