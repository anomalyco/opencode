...
Respond via `postSessionByIdPermissionsByPermissionId` using the root `sessionID`. An `always` response sticks to the root session and applies to the entire session tree.
