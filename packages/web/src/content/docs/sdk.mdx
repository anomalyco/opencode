---
title: SDK
description: Type-safe JS client for chalice server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The chalice JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control chalice programmatically.
...
```bash
npm install @chalicecode-ai/sdk
```
...
const chalice = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
If you already have a running instance of chalice, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@chalicecode-ai/sdk"
```
