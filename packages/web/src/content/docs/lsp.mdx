---
title: LSP Servers
description: Leak Code integrates with your LSP servers.
---

Leak Code can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Leak Code comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `LEAKCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and leak-code opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your leak-code config.
...
```json title="leak-code.json" {5-13}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "custom-lsp": {
