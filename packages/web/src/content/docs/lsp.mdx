...
You can add custom LSP servers by specifying the command and file extensions. Most LSP servers use `--stdio` to communicate over standard input/output:
...
```json title="opencode.json" {4-10}
{
  ...
  "lsp": {
    "luau-lsp": {
      "command": ["luau-lsp.exe", "lsp", "--stdio"],
      "extensions": [".lua", ".luau"]
    }
  }
}
...
You can also pass additional arguments and environment variables:
...
```json title="opencode.json" {4-13}
{
  ...
  "lsp": {
    "custom-lsp": {
      "command": ["custom-lsp-server", "lsp", "--settings", "path/to/settings.json"],
      "extensions": [".custom"],
      "env": {
        "CUSTOM_VAR": "value"
      }
...
:::note
Not all LSP servers use `--stdio`. Check your LSP server's documentation for the correct arguments. Common alternatives include `--langserver` or running without flags.
:::
