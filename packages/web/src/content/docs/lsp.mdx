...
OpenCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent. The [`lsp`](/docs/tools#lsp) tool no longer requires `OPENCODE_EXPERIMENTAL_LSP_TOOL` and is now available by default. LSP servers themselves remain disabled by default and must be enabled via `lsp` config.
