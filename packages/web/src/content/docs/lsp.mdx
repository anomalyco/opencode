...
| pyrefly            | .py, .pyi                                                           | `OPENCODE_EXPERIMENTAL_LSP_PYREFLY=true` and `pyrefly` command available |
...
Pyrefly is experimental and opt-in. Install it with `uv tool install pyrefly`, then set `OPENCODE_EXPERIMENTAL_LSP_PYREFLY=true`. When Pyrefly is enabled, OpenCode disables the built-in `pyright` and experimental `ty` servers for Python files to avoid duplicate diagnostics.
