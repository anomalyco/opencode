...
Built-in LSP servers can start automatically when OpenCode opens a matching file and the server's requirements are met. If there is no matching file, the dependency is missing, or you disable a server in config, that LSP stays inactive.
...
You can customize LSP servers through the `lsp` section in any OpenCode config file, such as `config.json`, `opencode.json`, or `opencode.jsonc`.
