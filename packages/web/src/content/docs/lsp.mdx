...
3. Collects diagnostics (errors, warnings) from the LSP server and provides them to the LLM.

When [`ai.useToonEncoding`](/docs/config#toon-encoding) is enabled, LSP diagnostics use the TOON format for improved token efficiency.
