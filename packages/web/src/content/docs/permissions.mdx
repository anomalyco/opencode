...
- `destructive_bash` — one-time confirmation for destructive shell commands in the secure-development profile
...
Normal project work such as edits, tests, builds, and Git inspection stays autonomous. Destructive shell operations (`rm`, destructive Git operations, in-place rewrites, disk tools, and publishing) use `destructive_bash`, which is one-time only and cannot create an `always` rule. Network access remains disabled in the container; use an explicitly reviewed integration when an operation must reach an external service.

The sandbox is not a replacement for project permissions: `external_directory` remains `ask`, and a mounted workspace is intentionally writable so the agent can develop in it. Never set global `bash: "allow"` without a sandbox.
