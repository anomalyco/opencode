...
This fork disables approvals by default (equivalent to `permission: "allow"`). You will not see permission prompts unless you explicitly configure `ask` rules. Built-in hard denies still apply (for example, the plan agent’s edit restrictions, and hidden system agents like compaction/title/summary).

To re-enable prompts, set `ask` rules in your config. For example:
