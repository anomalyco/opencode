...
:::caution
For custom agents, declaring an agent-level `permission` block makes unspecified actions deny by default. List every action the agent needs explicitly; built-in agents and custom agents without a permission block retain the normal defaults.
:::
