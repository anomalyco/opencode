...
- `always` — approve future requests matching the suggested patterns (for the rest of the current OpenCode process)
...
When the request comes from a [nested subagent](/docs/agents#nesting), the prompt is attributed to the **root session** of the session tree — you answer it there, without navigating into child sessions. The prompt's `metadata.originSessionID`, `metadata.originAgent`, and `metadata.originDepth` identify the subagent that asked. Answering `always` applies instance-wide for the rest of the OpenCode process — it covers the whole session tree, and also any other sessions in the same instance.
