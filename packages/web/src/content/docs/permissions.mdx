...
The `external_directory` permission only applies to OpenCode's built-in file tools (`read`, `write`, `edit`, `ls`, `glob`, `grep`). It does not restrict access through the `bash` tool, which can run arbitrary commands.
