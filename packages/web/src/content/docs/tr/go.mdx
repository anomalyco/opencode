...
Octopus Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Octopus Go şu anda beta olarak mevcut.
:::

Go, Octopus'daki diğer sağlayıcılar gibi çalışır. Octopus Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Octopus'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Octopus Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Octopus Go, Octopus'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Octopus Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Octopus Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Octopus Go'ya abone olabilir.
:::
...
Octopus Go aşağıdaki limitleri içerir:
...
Octopus yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `opencode-go/kimi-k2.6` kullanmalısınız.
...
Octopus Go'yu şu amaçlarla oluşturduk:
...
4. Octopus ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
