...
3. Yönlendirmeyi ve istem önbelleğe almayı optimize edebilmemiz için her konuşmada `x-opencode-session` içinde değişmeyen bir oturum kimliği göndermelidir.
   Aynı kimlik, yardımcı çağrılar dahil bir konuşmadaki tüm isteklerde yeniden kullanılmalı, istek veya süreç başına
   yeni bir kimlik üretilmemeli ve istemcinin kullandığı tüm uç noktalara (`/responses`, `/chat/completions` ve
   `/messages`) gönderilmelidir; yalnızca tek bir SDK'nın ara katmanına eklenen bir başlık diğerlerine ulaşmaz.
