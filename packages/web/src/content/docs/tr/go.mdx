...
Pencode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, Pencode'daki diğer sağlayıcılar gibi çalışır. Pencode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Pencode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Pencode Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Pencode Go, Pencode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Pencode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Pencode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Pencode Go'ya abone olabilir.
:::
...
Pencode Go aşağıdaki limitleri içerir:
...
Pencode yapılandırmanızdaki [model id](/docs/config/#models) formatı `pencode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `pencode-go/kimi-k3` kullanmalısınız.
...
Pencode Go'yu şu amaçlarla oluşturduk:
...
4. Pencode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
