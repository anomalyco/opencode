...
xcsh Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
xcsh Go şu anda beta olarak mevcut.
:::

Go, xcsh'daki diğer sağlayıcılar gibi çalışır. xcsh Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve xcsh'u kullanmak için bunu kullanmanıza gerek yoktur.
...
xcsh Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

xcsh Go, xcsh'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>xcsh Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `xcsh Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye xcsh Go'ya abone olabilir.
:::
...
xcsh Go aşağıdaki limitleri içerir:
...
xcsh yapılandırmanızdaki [model id](/docs/config/#models) formatı `xcsh-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `xcsh-go/kimi-k2.5` kullanmalısınız.
...
xcsh Go'yu şu amaçlarla oluşturduk:
...
4. xcsh ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
