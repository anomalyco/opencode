...
`~/.local/share/leak-code/auth.json` içinde.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `leak-code.json` kullanın:
...
to store token in leak-code auth storage.
:::
...
`leak-code.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`leak-code.json` üzerinden özelleştirin:
...
4. Proje dizininizde `leak-code.json` dosyanızı oluşturun veya güncelleyin:
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `leak-code auth list` komutunu çalıştırın.
