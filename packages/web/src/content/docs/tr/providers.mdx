---
title: Sağlayıcılar
description: cyberstrike'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

cyberstrike, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı cyberstrike yapılandırmanızda yapılandırın.
...
`~/.local/share/cyberstrike/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları cyberstrike'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
CyberStrike Zen, cyberstrike ekibi tarafından sağlanan modellerin bir listesidir.
cyberstrike ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz CyberStrike Zen ile başlamanızı öneririz.
:::
...
cyberstrike'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u cyberstrike ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   cyberstrike'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `cyberstrike.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi cyberstrike'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   cyberstrike'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Cyberstrike yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
cyberstrike, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. cyberstrike'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`cyberstrike.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`cyberstrike.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi cyberstrike'la kullanmak için:
...
Google Vertex AI'yi cyberstrike ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları cyberstrike çalıştırılırken ayarlayın.
...
Helicone'un cyberstrike aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her cyberstrike görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her cyberstrike görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
cyberstrike'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla cyberstrike'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla cyberstrike'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini cyberstrike için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/cyberstrike)'a bakın.
:::
...
Ollama Cloud'u cyberstrike ile kullanmak için:
...
3. cyberstrike'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: cyberstrike'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
CyberStrike Zen, cyberstrike ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>CyberStrike Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **CyberStrike Zen**'i arayın.

   ```txt
   /connect
   ```

3. cyberstrike API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   cyberstrike yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca cyberstrike yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Cyberstrike ile kullanmak için:
...
Modelleri cyberstrike yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı cyberstrike ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `cyberstrike.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları cyberstrike'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `cyberstrike auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için cyberstrike yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, cyberstrike yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
