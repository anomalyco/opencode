...
`~/.local/share/octopus/auth.json` içinde.
...
Octopus Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
opencode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Octopus Zen ile başlamanızı öneririz.
:::
...
Octopus Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Octopus Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `octopus.json` kullanın:
...
`octopus.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`octopus.json` üzerinden özelleştirin:
...
Octopus Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Octopus Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Octopus Zen**'i arayın.
...
4. Proje dizininizde `octopus.json` dosyanızı oluşturun veya güncelleyin:
