---
title: Sağlayıcılar
description: pencode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

pencode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı pencode yapılandırmanızda yapılandırın.
...
`~/.local/share/pencode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları pencode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Pencode Zen, pencode ekibi tarafından sağlanan modellerin bir listesidir.
pencode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Pencode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, pencode'u seçin ve [pencode.ai/auth](https://pencode.ai/auth)'ye gidin.
...
pencode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Pencode Go

Pencode Go, pencode ile iyi çalıştığı test edilmiş ve doğrulanmış, pencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Pencode Go`'yu seçin ve [pencode.ai/auth](https://pencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u pencode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   pencode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `pencode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi pencode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
pencode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   pencode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Pencode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in pencode auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
pencode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. pencode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`pencode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`pencode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi pencode'la kullanmak için:
...
Google Vertex AI'yi pencode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları pencode çalıştırılırken ayarlayın.
...
Helicone'un pencode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her pencode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her pencode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
pencode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla pencode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla pencode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini pencode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/pencode)'a bakın.
:::
...
Ollama Cloud'u pencode ile kullanmak için:
...
3. pencode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: pencode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Pencode Zen, pencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Pencode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Pencode Zen**'i arayın.

   ```txt
   /connect
   ```

3. pencode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   pencode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca pencode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Pencode ile kullanmak için:
...
Modelleri pencode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı pencode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `pencode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları pencode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `pencode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için pencode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, pencode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
