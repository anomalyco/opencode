---
title: Sağlayıcılar
description: xcsh'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı xcsh yapılandırmanızda yapılandırın.
...
`~/.local/share/xcsh/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları xcsh'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
xcsh Zen, xcsh ekibi tarafından sağlanan modellerin bir listesidir.
xcsh ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz xcsh Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, xcsh'u seçin ve [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/auth)'ye gidin.
...
xcsh'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## xcsh Go

xcsh Go, xcsh ile iyi çalıştığı test edilmiş ve doğrulanmış, xcsh ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `xcsh Go`'yu seçin ve [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen) adresine gidin.
...
Amazon Bedrock'u xcsh ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   xcsh'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `xcsh.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi xcsh'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   xcsh'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in xcsh auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
xcsh, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. xcsh'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`xcsh.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth) ana sayfasında.

##### Yapılandırma

`xcsh.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi xcsh'la kullanmak için:
...
Google Vertex AI'yi xcsh ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları xcsh çalıştırılırken ayarlayın.
...
Helicone'un xcsh aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her xcsh görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her xcsh görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
xcsh'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla xcsh'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla xcsh'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini xcsh için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/xcsh)'a bakın.
:::
...
Ollama Cloud'u xcsh ile kullanmak için:
...
3. xcsh'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: xcsh'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
xcsh Zen, xcsh ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>xcsh Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **xcsh Zen**'i arayın.

   ```txt
   /connect
   ```

3. xcsh API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   xcsh yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca xcsh yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri xcsh yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı xcsh ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `xcsh.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları xcsh'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `xcsh auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için xcsh yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, xcsh yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
