...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. cyberstrike yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
cyberstrike içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true` (veya `CYBERSTRIKE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca cyberstrike provider kullanılırken veya `CYBERSTRIKE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

cyberstrike başlatırken etkinleştirmek için:
