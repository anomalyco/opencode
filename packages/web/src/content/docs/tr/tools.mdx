...
:::note
Bu araç yalnızca `LEAKCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `LEAKCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca leak-code provider kullanılırken veya `LEAKCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.
