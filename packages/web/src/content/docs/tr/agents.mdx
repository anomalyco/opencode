...
cyberstrike'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
cyberstrike, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
cyberstrike, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

cyberstrike iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `cyberstrike.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse cyberstrike modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global cyberstrike yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
cyberstrike hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [CyberStrike Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `cyberstrike/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `cyberstrike models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/CyberStrikeus/cyberstrike).
:::
