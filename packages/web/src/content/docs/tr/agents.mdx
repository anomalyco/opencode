...
xcsh'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
xcsh, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
xcsh, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

xcsh iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `xcsh.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse xcsh modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global xcsh yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
xcsh yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [xcsh Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `xcsh/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `xcsh models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/f5xc-salesdemos/xcsh).
:::
