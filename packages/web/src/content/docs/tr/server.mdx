...
Sunucu bir OpenAPI 3.1 spec endpoint'i acar. Bu endpoint [SDK](./sdk) uretiminde de kullanilir.
...
TUI baslarken rastgele bir port ve hostname atanir. Bunun yerine `--hostname` ve `--port` [bayraklarini](./cli) verebilirsiniz.

[`/tui`](#tui) endpoint'i sunucu uzerinden TUI'yi surmek icin kullanilabilir. Ornegin bir istemi onceden doldurabilir veya calistirabilirsiniz. Bu kurulum OpenCode [IDE](./ide) eklentileri tarafindan kullanilir.
