---
title: Sunucu
description: pencode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`pencode serve` komutu, pencode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `pencode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `PENCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `pencode` değeridir; değiştirmek isterseniz `PENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `pencode serve` hem de `pencode web` için geçerlidir.
...
`pencode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
pencode ile programatik etkileşim için pencode sunucusunu kullanın.
:::

Bu mimari, pencode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`pencode serve` ile bağımsız bir sunucu başlatabilirsiniz. pencode TUI açıksa, `pencode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum pencode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
pencode sunucusu aşağıdaki API'leri sunar.
