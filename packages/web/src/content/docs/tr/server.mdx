...
`leak-code serve` komutu, opencode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `LEAKCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencode` değeridir; değiştirmek isterseniz `LEAKCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `leak-code serve` hem de `leak-code web` için geçerlidir.
...
`leak-code serve` ile bağımsız bir sunucu başlatabilirsiniz. opencode TUI açıksa, `leak-code serve` yeni bir sunucu başlatır.
