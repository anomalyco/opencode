---
title: Sunucu
description: cyberstrike sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`cyberstrike serve` komutu, cyberstrike istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `cyberstrike.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CYBERSTRIKE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `cyberstrike` değeridir; değiştirmek isterseniz `CYBERSTRIKE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `cyberstrike serve` hem de `cyberstrike web` için geçerlidir.
...
`cyberstrike` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
cyberstrike ile programatik etkileşim için cyberstrike sunucusunu kullanın.
:::

Bu mimari, cyberstrike'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`cyberstrike serve` ile bağımsız bir sunucu başlatabilirsiniz. cyberstrike TUI açıksa, `cyberstrike serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum cyberstrike [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
cyberstrike sunucusu aşağıdaki API'leri sunar.
