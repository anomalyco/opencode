...
OpenCode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](./tui)'yi başlatır.
...
Bu, gerekli GitHub Eylemlerinin iş ayrıntılandırma ayarlarını ve süreç sürecini boyut boyunca gösterir. [Learn more](./github).
...
Bu, [your config](./config/)'da kullanılacak tam modelin adını bulmak için kullanışlıdır.
...
API erişimi için başsız bir OpenCode tarayıcısı başlatılır. Tam HTTP arayüzü için [server docs](./server)'e göz atın.
