---
title: CLI
description: xcsh CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
xcsh
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, xcsh ile programlı olarak etkileşim kurmanıza olanak tanır.
...
xcsh terminal kullanıcı arayüzünü başlatır.
...
xcsh CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

xcsh için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir xcsh arka uç sunucusuna bir terminal ekleyin.

```bash
xcsh attach [url]
```

Bu, TUI öğesinin uzak bir xcsh arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
xcsh, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `xcsh auth login` kullanabilirsiniz. Bu `~/.local/share/xcsh/auth.json`'de saklanıyor.

```bash
xcsh auth login
```

xcsh başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları xcsh'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek xcsh'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `xcsh serve` örneğine de ekleyebilirsiniz:
...
| `--attach`   |      | Çalışan bir xcsh sunucusuna ekleyin (ör. http://localhost:4096)               |
...
API erişimi için headless bir xcsh sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
xcsh serve
```

Bu, TUI arayüzü olmadan xcsh işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `xcsh` şeklindedir).
...
Tüm xcsh oturumlarını listeleyin.
...
xcsh oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya xcsh paylaşımından URL oturum verilerini içe aktarın.

```bash
xcsh import <file>
```

Yerel bir dosyadan veya xcsh paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir xcsh sunucusu başlatın.

```bash
xcsh web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla xcsh'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `xcsh` şeklindedir).
...
xcsh'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
xcsh'u en son sürüme veya belirli bir sürüme günceller.
...
xcsh CLI aşağıdaki global bayrakları destekler.
...
xcsh ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `xcsh`) |
