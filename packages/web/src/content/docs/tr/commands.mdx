...
`.pencode/commands/test.md` oluştur:
...
pencode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

pencode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/pencode/commands/`
- Proje başına: `.pencode/commands/`
...
pencode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
