...
Özel komutlar, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi komutlara ek olarak sunulur. [Learn more](./tui#commands).
...
OpenCode [config](./config)'deki `command` seçeneğini kullanın:
...
İsteminize [bash command](./tui#bash-commands) çıktısını enjekte etmek için \__!`command`_ kullanın.
...
İsteğe bağlı olarak bu komutu hangi [agent](./agents)'nin yürütmesi gerektiğini belirtmek için `agent` yapılandırmasını kullanın.
Bu bir [subagent](./agents/#subagents) ise, komut varsayılan olarak bir alt aracı çağrısını tetikleyecektir.
...
Komutu bir [subagent](./agents/#subagents) çağrısını tetiklemeye zorlamak için `subtask` boolean'ını kullanın.
...
`mode`, [agent](./agents) yapılandırmasında `primary` olarak ayarlanmış olsa bile.
...
opencode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](./tui#commands).
