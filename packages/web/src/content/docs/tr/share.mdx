...
Manuel modu acikca ayarlamak icin [config dosyaniza](./config) sunu ekleyin:
...
Tum yeni konusmalar icin otomatik paylasimi acmak isterseniz, [config dosyanizda](./config) `share` degerini `"auto"` yapin:
...
Paylasimi tamamen kapatmak icin [config dosyanizda](./config) `share` degerini `"disabled"` yapin:
...
Kurulusunuzda opencode kullanimi icin [daha fazla bilgi alin](./enterprise).
