...
3. **Özel yapılandırma** (`IMECODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `opencode.json`) - projeye özgü ayarlar
5. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`IMECODE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
`IMECODE_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`IMECODE_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
