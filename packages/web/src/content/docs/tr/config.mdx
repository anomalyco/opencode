...
2. **Genel yapılandırma** (`~/.config/octopus/octopus.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`OCTOPUS_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `octopus.json`) - projeye özgü ayarlar
5. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`OCTOPUS_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
Global opencode yapılandırmanızı `~/.config/octopus/octopus.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `octopus.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`OCTOPUS_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`OCTOPUS_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
...
Agent'ları `~/.config/octopus/agents/` veya `.octopus/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/octopus/commands/` veya `.octopus/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.octopus/plugins/` veya `~/.config/octopus/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
