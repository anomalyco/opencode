...
`OPENCODE_CONFIG_DIRS`, sistem `PATH` ile aynı ayırıcıyı kullanarak birden çok özel yapılandırma dizini belirtebilir (`:` Linux/macOS'ta, `;` Windows'ta). Dizinler soldan sağa yüklenir; bu nedenle sonraki dizinler öncekileri geçersiz kılar. `OPENCODE_CONFIG_DIR` de ayarlanmışsa en sona eklenir ve en yüksek öncelikli özel dizin ile global yazma hedefi olur.
