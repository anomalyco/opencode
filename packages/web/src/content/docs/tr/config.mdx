...
5. **`.opencode` yapılandırması** (projedeki `.opencode/opencode.json`) - proje yapılandırmasıyla aynı ama ayrı tutulur
6. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
7. **Satır içi yapılandırma** (`OPENCODE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
