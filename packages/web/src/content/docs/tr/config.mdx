...
2. **Genel yapılandırma** (`~/.config/leak-code/leak-code.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`LEAKCODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `leak-code.json`) - projeye özgü ayarlar
5. **`.leak-code` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`LEAKCODE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.leak-code` ve `~/.config/leak-code` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global opencode yapılandırmanızı `~/.config/leak-code/leak-code.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `leak-code.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`LEAKCODE_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`LEAKCODE_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.leak-code` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.leak-code` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
`leak-code serve` ve `leak-code web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/leak-code/agents/` veya `.leak-code/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`leak-code run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/leak-code/commands/` veya `.leak-code/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.leak-code/plugins/` veya `~/.config/leak-code/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
