---
title: Eklentiler
description: Pencode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Pencode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Pencode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.pencode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/pencode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/pencode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/pencode/pencode.json`)
2. Project config (`pencode.json`)
3. Global eklenti dizini (`~/.config/pencode/plugins/`)
4. Proje eklenti dizini (`.pencode/plugins/`)
...
Pencode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Pencode SDK istemcisi.
...
Pencode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Pencode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Pencode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Pencode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Pencode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Pencode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://pencode.ai/docs/sdk) bakın.
