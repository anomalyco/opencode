---
title: Eklentiler
description: xcsh'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek xcsh'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya xcsh'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.xcsh/plugins/` - Proje düzeyinde eklentiler
- `~/.config/xcsh/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/xcsh/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/xcsh/xcsh.json`)
2. Project config (`xcsh.json`)
3. Global eklenti dizini (`~/.config/xcsh/plugins/`)
4. Proje eklenti dizini (`.xcsh/plugins/`)
...
xcsh bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir xcsh SDK istemcisi.
...
xcsh'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
xcsh masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
xcsh'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca xcsh'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, xcsh'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra xcsh'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) bakın.
