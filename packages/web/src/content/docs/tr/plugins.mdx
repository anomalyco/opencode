---
title: Eklentiler
description: cyberstrike'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek cyberstrike'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya cyberstrike'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.cyberstrike/plugins/` - Proje düzeyinde eklentiler
- `~/.config/cyberstrike/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/cyberstrike/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/cyberstrike/cyberstrike.json`)
2. Project config (`cyberstrike.json`)
3. Global eklenti dizini (`~/.config/cyberstrike/plugins/`)
4. Proje eklenti dizini (`.cyberstrike/plugins/`)
...
cyberstrike bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için cyberstrike'lu bir SDK istemcisi.
...
cyberstrike'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
cyberstrike masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
cyberstrike'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca cyberstrike'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, cyberstrike'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://cyberstrike.io/docs/sdk)'e bakın.
