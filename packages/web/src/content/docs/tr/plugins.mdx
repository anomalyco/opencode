---
title: Eklentiler
description: Octopus'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Octopus'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Octopus'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.octopus/plugins/` - Proje düzeyinde eklentiler
- `~/.config/octopus/plugins/` - Genel eklentiler
...
1. Global config (`~/.config/octopus/octopus.json`)
2. Project config (`octopus.json`)
3. Global eklenti dizini (`~/.config/octopus/plugins/`)
4. Proje eklenti dizini (`.octopus/plugins/`)
...
Octopus bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Octopus SDK istemcisi.
...
Octopus'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Octopus masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Octopus'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Octopus'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Octopus'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Octopus'a da hazır olacaktır.
