...
:::caution
Modlar artık cyberstrike yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

cyberstrike'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya cyberstrike yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
cyberstrike iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.cyberstrike/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`cyberstrike.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, cyberstrike modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global cyberstrike yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.cyberstrike/modes/` veya genel modlar için `~/.config/cyberstrike/modes/`'de mod dosyaları oluşturun:
