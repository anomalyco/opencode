...
:::caution
Modlar artık xcsh yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

xcsh'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya xcsh yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
xcsh iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.xcsh/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`xcsh.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, xcsh modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global xcsh yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.xcsh/modes/` veya genel modlar için `~/.config/xcsh/modes/`'de mod dosyaları oluşturun:
