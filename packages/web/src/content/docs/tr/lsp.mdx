...
description: cyberstrike, LSP sunucularınızla bütünleşir.
---

cyberstrike, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

cyberstrike, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CYBERSTRIKE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
Cyberstrike bir dosyayı açtığında:
...
LSP sunucularını, cyberstrike yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
