...
Windows'ta en iyi deneyim için [Windows Subsystem for Linux (WSL)](./windows-wsl) kullanılmasını öneririz. Daha iyi performans ve OpenCode'un özellikleriyle tam uyumluluğu sağlar.
:::
...
LLM sağlayıcılarını kullanmaya yeni başlıyorsanız, [OpenCode Zen](./zen) kullanmanızı öneririz.
...
Alternatif olarak diğer sağlayıcılardan birini seçebilirsiniz. [Learn more](./providers#directory).
...
İşte OpenCode'lu bir [example conversation](https://opencode.ai/s/4XP1fce5).
...
Kendinize ait hale getirmek için [picking a theme](./themes), [customizing the keybinds](./keybinds), [configuring code formatters](./formatters), [creating custom commands](./commands) veya [OpenCode config](./config) ile oynamanızı öneririz.
