...
Ajan becerileri, cyberstrike'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
cyberstrike su konumlari tarar:

- Proje konfigurasyonu: `.cyberstrike/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
Proje yerel yollarinda cyberstrike, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.cyberstrike/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.cyberstrike/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
cyberstrike, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`cyberstrike.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`cyberstrike.json` icinde):
