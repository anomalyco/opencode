...
Ajan becerileri, pencode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
pencode su konumlari tarar:

- Proje konfigurasyonu: `.pencode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/pencode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda pencode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.pencode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.pencode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
pencode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`pencode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`pencode.json` icinde):
