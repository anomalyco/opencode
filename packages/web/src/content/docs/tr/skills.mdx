...
Ajan becerileri, xcsh'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
xcsh su konumlari tarar:

- Proje konfigurasyonu: `.xcsh/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/xcsh/skills/<name>/SKILL.md`
...
Proje yerel yollarinda xcsh, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.xcsh/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.xcsh/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
xcsh, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`xcsh.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`xcsh.json` icinde):
