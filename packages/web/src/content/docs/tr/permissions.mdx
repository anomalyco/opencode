...
- `bash:unsandboxed` — sandbox reddinden sonra veya açık bir sandbox dışı istekten sonra bir kabuk komutunu sandbox dışında yeniden çalıştırma
...
macOS sandbox'ı etkinleştirildiğinde, engellenen bir bash komutu ayrı bir `bash:unsandboxed` izin isteği tetikleyebilir.
Bu, OpenCode muhtemel bir sandbox reddini tespit ettiğinde veya komut ilk boş olmayan satırda `# opencode:unsandboxed <neden>` ile açıkça sandbox'ı atlamayı istediğinde gerçekleşir.
Sandbox'ın kendisini [`experimental.sandbox`](/docs/config#sandbox) içinde yapılandırın.
