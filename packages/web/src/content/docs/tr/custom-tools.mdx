...
Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. Açık kodun [built-in tools](./tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
