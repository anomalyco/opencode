...
_Model Bağlam Protokolü_ veya MCP kullanarak pencode'a harici araçlar ekleyebilirsiniz. pencode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [pencode Config](https://pencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/pencode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
pencode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde pencode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa pencode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `pencode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından pencode, belirteçleri `~/.local/share/pencode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra pencode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi pencode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve pencode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
