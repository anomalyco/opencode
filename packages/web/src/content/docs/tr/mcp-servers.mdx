...
_Model Bağlam Protokolü_ veya MCP kullanarak cyberstrike'a harici araçlar ekleyebilirsiniz. cyberstrike hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [cyberstrike Config](https://cyberstrike.io/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/cyberstrike` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
cyberstrike, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde cyberstrike şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa cyberstrike, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `cyberstrike mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından cyberstrike, belirteçleri `~/.local/share/cyberstrike/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra cyberstrike'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi cyberstrike yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve cyberstrike'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
