...
_Model Bağlam Protokolü_ veya MCP kullanarak xcsh'a harici araçlar ekleyebilirsiniz. xcsh hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/xcsh` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
xcsh, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde xcsh şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa xcsh, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `xcsh mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından xcsh, belirteçleri `~/.local/share/xcsh/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra xcsh'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi xcsh yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve xcsh'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
