...
:::caution
`LEAKCODE_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::

:::tip[Windows Kullanıcıları]
En iyi deneyim için PowerShell yerine `leak-code web`'yi [WSL](/docs/windows-wsl)'den çalıştırın. Bu, uygun dosya sistemi erişimini ve terminal entegrasyonunu sağlar.
:::
...
Erişimi korumak için `LEAKCODE_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Kullanıcı adı varsayılan olarak `opencode` şeklindedir ancak `LEAKCODE_SERVER_USERNAME` ile değiştirilebilir.
...
Sunucu ayarlarını `leak-code.json` yapılandırma dosyanızda da yapılandırabilirsiniz:
