---
title: ACP Desteği
description: xcsh'u ACP uyumlu herhangi bir editörde kullanın.
---

xcsh, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
xcsh'u ACP aracılığıyla kullanmak için düzenleyicinizi `xcsh acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, xcsh'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'xcsh' ajanını kullanın.
...
xcsh'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı xcsh'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
xcsh, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- xcsh yapılandırmanızda yapılandırılmış MCP sunucuları
