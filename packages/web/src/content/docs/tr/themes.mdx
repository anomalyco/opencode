...
xcsh ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak xcsh kendi `xcsh` temasını kullanır.
...
xcsh birden fazla yerleşik temayla gelir.
...
- xcsh'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
xcsh, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/xcsh/themes/*.json` veya `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.xcsh/themes/*.json`
4. **Geçerli çalışma dizini** - `./.xcsh/themes/*.json`
