...
cyberstrike ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak cyberstrike kendi `cyberstrike` temasını kullanır.
...
cyberstrike birden fazla yerleşik temayla gelir.
...
- cyberstrike'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
cyberstrike, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/cyberstrike/themes/*.json` veya `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.cyberstrike/themes/*.json`
4. **Geçerli çalışma dizini** - `./.cyberstrike/themes/*.json`
