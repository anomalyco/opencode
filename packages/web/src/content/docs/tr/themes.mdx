...
pencode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak pencode kendi `pencode` temasını kullanır.
...
pencode birden fazla yerleşik temayla gelir.
...
- pencode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
pencode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/pencode/themes/*.json` veya `$XDG_CONFIG_HOME/pencode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.pencode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.pencode/themes/*.json`
