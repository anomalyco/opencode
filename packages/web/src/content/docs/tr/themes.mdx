...
2. **Kullanıcı yapılandırma dizini** - `~/.config/leak-code/themes/*.json` veya `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.leak-code/themes/*.json`
4. **Geçerli çalışma dizini** - `./.leak-code/themes/*.json`
