...
:::caution
`--hostname 0.0.0.0` kullaniyorsaniz sunucuyu guvene almak icin `LEAKCODE_SERVER_PASSWORD` ayarlayin.
...
1. **`leak-code web` komutunu PowerShell yerine WSL terminalinde calistirin**:
...
`leak-code web` komutunu WSL'den calistirmak, dosya sistemi erisimi ve terminal entegrasyonunu daha tutarli hale getirirken tarayicidan erisimi korur.
...
- opencode konfigurasyonunuz ve oturumlariniz WSL ortaminda `~/.local/share/leak-code/` konumunda saklanir
