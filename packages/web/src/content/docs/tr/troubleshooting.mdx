...
- **macOS/Linux**: `~/.config/octopus/octopus.jsonc` (veya `~/.config/octopus/octopus.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\octopus.jsonc` yapistirin
...
`octopus.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `OCTOPUS_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `OCTOPUS_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
