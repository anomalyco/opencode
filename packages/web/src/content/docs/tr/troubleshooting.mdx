...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (veya `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\leak-code.jsonc` yapistirin
...
`leak-code.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `LEAKCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `LEAKCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
3. `leak-code upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `leak-code models` calistirin.
