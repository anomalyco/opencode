...
xcsh ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\xcsh\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `xcsh --log-level DEBUG`.

---

## Depolama

xcsh, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/xcsh/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\xcsh` yapistirin
...
xcsh Desktop arka planda yerel bir xcsh sunucusu (`xcsh-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `xcsh` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/xcsh/xcsh.jsonc` (veya `~/.config/xcsh/xcsh.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/xcsh/xcsh.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\xcsh\xcsh.jsonc` yapistirin
...
xcsh diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/xcsh/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\xcsh\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), xcsh'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. xcsh Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/xcsh`
- **Linux**: `~/.cache/xcsh` dizinini silin (veya `rm -rf ~/.cache/xcsh` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\xcsh` yapistirin

3. xcsh Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

xcsh Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`xcsh.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta xcsh Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, xcsh ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

xcsh Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda xcsh icin bildirimler etkinse
...
1. xcsh Desktop'u kapatin
2. Su dosyalari bulun ve silin (xcsh Desktop uygulama veri dizininde yer alirlar):

- `xcsh.settings.dat` (desktop varsayilan sunucu URL'si)
- `xcsh.global.dat` ve `xcsh.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
xcsh ile ilgili bir sorun yasiyorsaniz:
...
3. `xcsh upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `xcsh models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\xcsh`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. xcsh, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\xcsh`

2. En guncel provider paketlerini yeniden kurmak icin xcsh'u yeniden baslatin

Bu, xcsh'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
xcsh, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
