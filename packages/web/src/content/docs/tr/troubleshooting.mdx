...
pencode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\pencode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `pencode --log-level DEBUG`.

---

## Depolama

pencode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/pencode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\pencode` yapistirin
...
pencode Desktop arka planda yerel bir pencode sunucusu (`pencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `pencode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/pencode/pencode.jsonc` (veya `~/.config/pencode/pencode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/pencode/pencode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\pencode\pencode.jsonc` yapistirin
...
pencode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/pencode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\pencode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), pencode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. pencode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/pencode`
- **Linux**: `~/.cache/pencode` dizinini silin (veya `rm -rf ~/.cache/pencode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\pencode` yapistirin

3. pencode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

pencode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`pencode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `PENCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `PENCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta pencode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, pencode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

pencode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda pencode icin bildirimler etkinse
...
1. pencode Desktop'u kapatin
2. Su dosyalari bulun ve silin (pencode Desktop uygulama veri dizininde yer alirlar):

- `pencode.settings.dat` (desktop varsayilan sunucu URL'si)
- `pencode.global.dat` ve `pencode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
pencode ile ilgili bir sorun yasiyorsaniz:
...
3. `pencode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `pencode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\pencode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. pencode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\pencode`

2. En guncel provider paketlerini yeniden kurmak icin pencode'u yeniden baslatin

Bu, pencode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
pencode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
