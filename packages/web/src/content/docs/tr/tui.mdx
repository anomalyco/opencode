...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `leak-code.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `LEAKCODE_TUI_CONFIG` kullanın.
