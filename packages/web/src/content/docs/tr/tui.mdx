---
title: TUI
description: xcsh terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

xcsh'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
xcsh TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
xcsh'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
xcsh'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `xcsh.json` dosyasından ayrıdır.
