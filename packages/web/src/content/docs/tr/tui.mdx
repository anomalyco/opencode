---
title: TUI
description: Octopus terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Octopus, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Octopus'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Octopus TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Octopus'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Octopus'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `octopus.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `OCTOPUS_TUI_CONFIG` kullanın.
