...
- `intent.updated` — Intent created, waiting for user response
- `intent.replied` — User responded to intent
...
### User input with Intents

Plugins can request user input through the TUI using the `ui` helpers. This enables interactive tools that ask questions, get confirmations, or collect structured input.

```ts title=".opencode/plugin/interactive-tool.ts"
import { type Plugin, tool } from "@opencode-ai/plugin"

export const InteractivePlugin: Plugin = async ({ ui }) => {
  return {
    tool: {
      deploy: tool({
        ...
        async execute({ target }, { sessionID, messageID }) {
          if (!ui) {
          ...
          const confirmed = await ui.confirm(
            { sessionID, messageID },
            {
              ...
              message: `Deploy to ${target}? This cannot be undone.`,
              variant: "danger",
            },
          )
...
- `ui.confirm()` — Yes/no confirmation dialog
- `ui.select()` — Single-choice selection
- `ui.multiselect()` — Multi-choice selection
- `ui.form()` — Multi-field input form
- `ui.toast()` — Non-blocking notification

[Learn more](/docs/intent) about using Intents in plugins.
