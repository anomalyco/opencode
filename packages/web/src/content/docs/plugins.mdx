...
You can also reference a local plugin file directly from `opencode.json`:
...
Use the full `file://` URL with an absolute path.
...
Git-backed packages are also supported when you provide an explicit package
name alias:

```json title="opencode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["opencode-wakatime@git+https://github.com/angristan/opencode-wakatime"]
}
```

Use this `name@git+...` form for git installs so the package name is explicit.
