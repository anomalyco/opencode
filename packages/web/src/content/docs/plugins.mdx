...
1. `.opendeepseek/plugin` directory either in your project
...
```js title=".opendeepseek/plugin/example.js"
...
import type { Plugin } from "@opendeepseek/plugin"
...
```js title=".opendeepseek/plugin/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "opendeepseek"'`
      }
    },
  }
}
...
```javascript title=".opendeepseek/plugin/env-protection.js"
...
```ts title=".opendeepseek/plugin/custom-tools.ts"
import { type Plugin, tool } from "@opendeepseek/plugin"
...
```ts title=".opendeepseek/plugin/compaction.ts"
import type { Plugin } from "@opendeepseek/plugin"
