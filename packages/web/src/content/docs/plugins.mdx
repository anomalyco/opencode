...
Send notifications when certain events occur, with parentID to distinguish between agent and subagent sessions:
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        // Extract parentID from different event structures
        const parentID = event.properties.info?.parentID || event.properties.parentID;
        
        if (parentID) {
          await $`osascript -e 'display notification "Subagent session completed!" with title "opencode"'`
        } else {
          await $`osascript -e 'display notification "Agent session completed!" with title "opencode"'`
        }
      }
    },
  }
...
We are using `osascript` to run AppleScript on macOS. Here we are using it to send notifications with different messages for agent vs subagent sessions.
