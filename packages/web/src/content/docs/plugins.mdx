---
title: Plugins
description: Write your own plugins to extend Octopus.
---

Plugins allow you to extend Octopus by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Octopus's default behavior.
...
- `.octopus/plugins/` - Project-level plugins
- `~/.config/octopus/plugins/` - Global plugins
...
3. Global plugin directory (`~/.config/octopus/plugins/`)
4. Project plugin directory (`.octopus/plugins/`)
...
Octopus runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the Octopus desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
