...
OpenCode uses Bun to install npm plugins at startup. Packages and their dependencies are cached in `~/.cache/opencode/node_modules/`.

Local plugins in `.opencode/plugin/` or `~/.config/opencode/plugin/` are loaded directly. If your local plugin has dependencies, add a `package.json` to the plugin directory and run `bun install` manually.
