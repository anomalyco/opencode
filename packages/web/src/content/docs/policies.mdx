---
title: Policies
description: Control which configured resources Pencode may use.
---

Policies control whether Pencode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `pencode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Pencode may use a resource such as an LLM provider.
...
Pencode currently supports one policy action:
