...
Aby debugować problemy z xcsh, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\xcsh\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `xcsh --log-level DEBUG`.

---

## Przechowywanie danych

xcsh przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/xcsh/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\xcsh`
...
xcsh Desktop uruchamia w tle lokalny serwer xcsh (sidecar `xcsh-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `xcsh` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/xcsh/xcsh.jsonc` (lub `~/.config/xcsh/xcsh.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/xcsh/xcsh.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\xcsh\xcsh.jsonc`
...
xcsh może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/xcsh/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\xcsh\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby xcsh mógł ją odbudować.

1. Całkowicie zamknij xcsh Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/xcsh`
- **Linux**: usuń `~/.cache/xcsh` (lub uruchom `rm -rf ~/.cache/xcsh`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\xcsh`

3. Uruchom ponownie xcsh Desktop.

---

### Napraw problemy z połączeniem z serwerem

xcsh Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `xcsh.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows xcsh Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami xcsh.

---

### Brak powiadomień

xcsh Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla xcsh w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie xcsh Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji xcsh Desktop):

- `xcsh.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `xcsh.global.dat` i `xcsh.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `xcsh upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `xcsh models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\xcsh`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. xcsh dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\xcsh`

2. Uruchom ponownie kod xcsh, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to xcsh do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
xcsh wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
