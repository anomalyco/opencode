...
Aby debugować problemy z cyberstrike, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\cyberstrike\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `cyberstrike --log-level DEBUG`.

---

## Przechowywanie danych

cyberstrike przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/cyberstrike/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\cyberstrike`
...
cyberstrike Desktop uruchamia w tle lokalny serwer cyberstrike (sidecar `cyberstrike-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `CyberStrike` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/cyberstrike/cyberstrike.jsonc` (lub `~/.config/cyberstrike/cyberstrike.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/cyberstrike/cyberstrike.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\cyberstrike\cyberstrike.jsonc`
...
cyberstrike może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/cyberstrike/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\cyberstrike\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby cyberstrike mógł ją odbudować.

1. Całkowicie zamknij cyberstrike Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/cyberstrike`
- **Linux**: usuń `~/.cache/cyberstrike` (lub uruchom `rm -rf ~/.cache/cyberstrike`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\cyberstrike`

3. Uruchom ponownie cyberstrike Desktop.

---

### Napraw problemy z połączeniem z serwerem

cyberstrike Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `cyberstrike.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CYBERSTRIKE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CYBERSTRIKE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows cyberstrike Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami cyberstrike.

---

### Brak powiadomień

cyberstrike Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla cyberstrike w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie cyberstrike Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji cyberstrike Desktop):

- `cyberstrike.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `cyberstrike.global.dat` i `cyberstrike.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z cyberstrike:
...
3. Upewnij się, że masz najnowszą wersję z `cyberstrike upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `cyberstrike models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\cyberstrike`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. cyberstrike dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\cyberstrike`

2. Uruchom ponownie kod cyberstrike, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to cyberstrike do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
cyberstrike wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
