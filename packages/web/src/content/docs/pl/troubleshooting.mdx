...
leak-code przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc` (lub `~/.config/leak-code/leak-code.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Jeśli Twój `leak-code.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `LEAKCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `LEAKCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
3. Upewnij się, że masz najnowszą wersję z `leak-code upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `leak-code models`
