...
- Tylko macOS: menu `Octopus` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/octopus/octopus.jsonc` (lub `~/.config/octopus/octopus.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Jeśli Twój `octopus.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OCTOPUS_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OCTOPUS_PORT` (lub wybierz wolny port) i uruchom ponownie.
