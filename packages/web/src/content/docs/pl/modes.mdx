...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji cyberstrike.
...
te lub skonfiguruj własne za pomocą konfiguracji cyberstrike.
...
cyberstrike ma dwa puste tryby.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.cyberstrike/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `cyberstrike.json`:
...
- Globalnie: `~/.config/cyberstrike/modes/`
...
Jeśli nie ma wpływu na temperaturę, cyberstrike stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna cyberstrike, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.cyberstrike/modes/` dla trybów zapisanych dla projektu lub `~/.config/cyberstrike/modes/` dla trybów globalnych:
