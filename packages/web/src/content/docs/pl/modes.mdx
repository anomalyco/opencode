...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji xcsh.
...
te lub skonfiguruj własne za pomocą konfiguracji xcsh.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.xcsh/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `xcsh.json`:
...
Jeśli nie ma wpływu na temperaturę, xcsh stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna xcsh, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.xcsh/modes/` dla trybów zapisanych dla projektu lub `~/.config/xcsh/modes/` dla trybów globalnych:
