...
Dzięki xcsh możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie xcsh używa naszego własnego motywu `xcsh`.
...
xcsh ma kilka wbudowanych motywów.
...
- Chcą, aby xcsh pasował do wyglądu ich terminala
...
xcsh obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/xcsh/themes/*.json` lub `$XDG_CONFIG_HOME/xcsh/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.xcsh/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.xcsh/themes/*.json`
