...
- Chcą, aby Octopus pasował do wyglądu ich terminala
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/octopus/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.octopus/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.octopus/themes/*.json`
