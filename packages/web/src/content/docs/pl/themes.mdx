...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/leak-code/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.leak-code/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.leak-code/themes/*.json`
