...
Dzięki cyberstrike możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie cyberstrike używa naszego własnego motywu `cyberstrike`.
...
cyberstrike ma kilka wbudowanych motywów.
...
cyberstrike obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/cyberstrike/themes/*.json` lub `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.cyberstrike/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.cyberstrike/themes/*.json`
