...
Aby znaleźć się z przykładami, przejrzyj [wtyczki](./ecosystem#plugins) utworzone przez społeczność.
...
Przeglądaj dostępną wtyczkę w [ekosystemie](./ecosystem#plugins).
...
**Wtyczki lokalne** są ładowane bezpośrednio z katalogu wtyczek. Aby uzyskać dostęp do zewnętrznych pakietów, musisz zainstalować `package.json` w swoim katalogu konfiguracyjnym (zobacz [Zależności](#dependencies)) lub opublikować wtyczkę w npm i [dodaj ją do swojej konfiguracji](./config#plugins).
