---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć xcsh.
---

Wtyczki umożliwiające wyłączenie xcsh poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania xcsh.
...
- `.xcsh/plugins/` - Wtyczki na poziomie projektu
- `~/.config/xcsh/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/xcsh/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/xcsh/xcsh.json`)
2. Konfiguracja projektu (`xcsh.json`)
3. Globalny katalog wtyczek (`~/.config/xcsh/plugins/`)
4. Katalog wtyczek projektu (`.xcsh/plugins/`)
...
xcsh uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z xcsh do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do xcsh.
...
:::note
Jeśli korzystasz z aplikacji komputerowej xcsh, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj xcsh czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do xcsh:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod xcsh. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://f5xc-salesdemos.github.io/xcsh/docs/sdk).
