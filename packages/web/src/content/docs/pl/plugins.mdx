---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć pencode.
---

Wtyczki umożliwiające wyłączenie pencode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania pencode.
...
- `.pencode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/pencode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/pencode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/pencode/pencode.json`)
2. Konfiguracja projektu (`pencode.json`)
3. Globalny katalog wtyczek (`~/.config/pencode/plugins/`)
4. Katalog wtyczek projektu (`.pencode/plugins/`)
...
pencode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z pencode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do pencode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej pencode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj pencode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do pencode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod pencode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://pencode.ai/docs/sdk).
