...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Pencode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Pencode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `PENCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `PENCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Pencode lub gdy zmienna środowiskowa `PENCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Pencode:
