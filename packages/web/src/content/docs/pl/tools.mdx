...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Octopus posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Octopus.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OCTOPUS_EXPERIMENTAL_LSP_TOOL=true` (lub `OCTOPUS_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Octopus lub gdy zmienna środowiskowa `OCTOPUS_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Octopus:
