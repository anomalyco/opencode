...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CyberStrike posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CyberStrike.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true` (lub `CYBERSTRIKE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CyberStrike lub gdy zmienna środowiskowa `CYBERSTRIKE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CyberStrike:
