...
Aby najlepiej wykorzystać działanie w systemie Windows, zalecamy udostępnienie [Podsystemu Windows dla systemu Linux (WSL)](./windows-wsl). pozwolenie na użytkowanie i pełna kompatybilność z funkcjami OpenCode.
:::
...
Jeśli nadal będziesz korzystać z dostawców LLM, zalecamy skorzystanie z [OpenCode Zen](./zen).
...
Alternatywnie możesz wybrać jednego z dodatkowych dostawców. [Dowiedz się więcej](./providers#directory).
...
Aby dostosować go do potrzeb, zalecamy [wybranie motywu](./themes), [dostosowanie skrótów klawiaturowych](./keybinds), [konfigurowanie programów formatujących kod](./formatters), [tworzenie niestandardowych kodów](./commands) lub grę z [konfiguracją OpenCode](./config).
