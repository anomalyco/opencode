...
Pencode Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Pencode. Subskrybujesz Pencode Go i
...
Pencode Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

Pencode Go działa jak każdy inny dostawca w Pencode.

1. Logujesz się do **<a href={console}>Pencode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Pencode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Pencode Go.
:::
...
Pencode Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Pencode
używa formatu `pencode-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`pencode-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Pencode.
