---
title: CLI
description: Opcje i polecenia wiersza poleceń xcsh.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI xcsh domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
xcsh
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać xcsh programowo.
...
Uruchamia interfejs terminalowy (TUI) xcsh.
...
Interfejs CLI xcsh zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami xcsh.
...
Dołącz terminal do już działającego serwera backendu xcsh uruchomionego za pomocą `serve` lub `web`.

```bash
xcsh attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem xcsh. Na przykład:
...
xcsh integruje się z wieloma dostawcami. Możesz użyć `xcsh auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

Podczas uruchamiania xcsh ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w xcsh.
...
Uruchom xcsh w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `xcsh serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera xcsh (np. http://localhost:4096)     |
...
Uruchom serwer xcsh (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
xcsh serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API xcsh bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `xcsh`).
...
Pokaż statystyki wykorzystania tokenów i sesji xcsh.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania xcsh.
...
Uruchom serwer xcsh z interfejsem internetowym.

```bash
xcsh web
```

Uruchamia to serwer HTTP i udostępnia xcsh przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `xcsh`).
...
Odinstaluj xcsh i usuń wszystkie powiązane pliki.
...
Aktualizuj xcsh do najnowszej lub określonej wersji.
...
Interfejs CLI xcsh przyjmuje następujące flagi globalne dla każdego polecenia.
...
xcsh można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `xcsh`)     |
