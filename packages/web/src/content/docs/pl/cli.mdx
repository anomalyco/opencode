...
Interfejs CLI OpenCode uruchamia się [TUI](./tui), gdy jest uruchamiany bez żadnych argumentów.
...
Spowoduje to konieczność uruchomienia akcji GitHub i przeprowadzi cię przez proces tworzenia. [Dowiedz się więcej](./github).
...
Uruchom bezgłowy serwer OpenCode w celu uzyskania dostępu do API. Pełny interfejs HTTP w [dokumentacji serwera](./server).
