...
3. **Korzystaj z leak-code przez WSL**
...
:::caution
Przy uzyciu `--hostname 0.0.0.0` ustaw `LEAKCODE_SERVER_PASSWORD`, aby zabezpieczyc serwer.
...
1. **Uruchamiaj `leak-code web` w terminalu WSL**, a nie w PowerShell:
...
Uruchamianie `leak-code web` z WSL zapewnia poprawny dostep do systemu plikow i integracje z terminalem, a nadal pozostaje dostepne z przegladarki Windows.
...
- Konfiguracja opencode i sesje sa przechowywane w srodowisku WSL pod `~/.local/share/leak-code/`
