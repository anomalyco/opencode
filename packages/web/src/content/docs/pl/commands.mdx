...
Polecenia stanowią dodatek do dostarczonych, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`. [Dowiedz się więcej](./tui#commands).
...
opcja opcja `command` w swoim OpenCode [config](./config):
...
urządzenie __!`command`_, aby wstawić dane wyjściowe [polecenie bash](./tui#bash-commands) wykonaj obciążenie.
...
Jeśli jest to [podagent](./agents/#subagents), rozwiązanie to ujawnione, podagenta.
...
Użyj wartości logicznej `subtask`, aby wymusić wywołanie [podagenta](./agents/#subagents) polecenia.
Jest to narzędzie, jeśli chcesz, aby decyzja nie zanieczyszczała kontekstu i **zmusiło** agenta do działania jako subagent,
nawet jeśli `mode` jest podłączony na `primary` w konfiguracji [agent](./agents).
...
opencode zawiera kilka istniejących urządzeń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](./tui#commands).
