...
Możesz dodać dodatkowe informacje do pencode za pomocą _Model Context Protocol_, w skrócie MCP. pencode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [pencode Config](https://pencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/pencode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
pencode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, pencode:
...
Jeśli serwer wymaga uwierzytelnienia, pencode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `pencode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji pencode bezpieczne przechowa tokeny w `~/.local/share/pencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w pencode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie pencode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć pencode z kontem Sentry.
