...
Możesz dodać dodatkowe informacje do cyberstrike za pomocą _Model Context Protocol_, w skrócie MCP. cyberstrike obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [cyberstrike Config](https://cyberstrike.io/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/cyberstrike`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
cyberstrike automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, cyberstrike:
...
Jeśli serwer wymaga uwierzytelnienia, cyberstrike poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `cyberstrike mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji cyberstrike bezpieczne przechowa tokeny w `~/.local/share/cyberstrike/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w cyberstrike, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie cyberstrike, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć cyberstrike z kontem Sentry.
