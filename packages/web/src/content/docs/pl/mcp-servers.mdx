...
Możesz dodać dodatkowe informacje do xcsh za pomocą _Model Context Protocol_, w skrócie MCP. xcsh obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/xcsh`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
xcsh automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, xcsh:
...
Jeśli serwer wymaga uwierzytelnienia, xcsh poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `xcsh mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji xcsh bezpieczne przechowa tokeny w `~/.local/share/xcsh/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w xcsh, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie xcsh, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć xcsh z kontem Sentry.
