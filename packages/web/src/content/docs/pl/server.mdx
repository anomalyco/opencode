...
Komenda `leak-code serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opencode.
...
Ustaw `LEAKCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `opencode` lub ustaw `LEAKCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `leak-code serve`, jak i `leak-code web`.
...
Możesz uruchomić `leak-code serve`, aby uruchomić samodzielny serwer. Jeśli masz
opencode TUI działa, `leak-code serve` uruchomi nowy serwer.
