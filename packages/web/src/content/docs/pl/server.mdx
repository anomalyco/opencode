---
title: Serwer
description: Interakcja z serwerem cyberstrike za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `cyberstrike serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient cyberstrike.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `cyberstrike.local` |
...
Ustaw `CYBERSTRIKE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `cyberstrike` lub ustaw `CYBERSTRIKE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `cyberstrike serve`, jak i `cyberstrike web`.
...
Kiedy uruchomisz `cyberstrike`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera cyberstrike do programowej interakcji z cyberstrike.
:::

Ta architektura umożliwia obsługę wielu klientów przez cyberstrike i programową interakcję z kodem otwartym.

Możesz uruchomić `cyberstrike serve`, aby uruchomić samodzielny serwer. Jeśli masz
cyberstrike TUI działa, `cyberstrike serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki cyberstrike [IDE](/docs/ide).
...
Serwer cyberstrike udostępnia następujące interfejsy API.
