...
punkt końcowy. Ten punkt końcowy jest również używany do generowania [SDK](./sdk).
...
Po uruchomieniu TUI losowo przypisuje port i nazwę hosta. Zamiast tego możesz przekazać `--hostname` i `--port` [flagi](./cli). Następnie użyj tego, aby połączyć się z serwerem.

Punktu końcowego [`/tui`](#tui) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki OpenCode [IDE](./ide).
