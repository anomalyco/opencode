---
title: Serwer
description: Interakcja z serwerem pencode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `pencode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient pencode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `pencode.local` |
...
Ustaw `PENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `pencode` lub ustaw `PENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `pencode serve`, jak i `pencode web`.
...
Kiedy uruchomisz `pencode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera pencode do programowej interakcji z pencode.
:::

Ta architektura umożliwia obsługę wielu klientów przez pencode i programową interakcję z kodem otwartym.

Możesz uruchomić `pencode serve`, aby uruchomić samodzielny serwer. Jeśli masz
pencode TUI działa, `pencode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki pencode [IDE](/docs/ide).
...
Serwer pencode udostępnia następujące interfejsy API.
