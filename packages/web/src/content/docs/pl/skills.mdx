...
Umiejętności agenta pozwalają pencode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
pencode przeszukuje te lokalizacje:

- Project config: `.pencode/skills/<name>/SKILL.md`
- Global config: `~/.config/pencode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu pencode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.pencode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.pencode/skills/git-release/SKILL.md` w ten sposób:
...
pencode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `pencode.json`:
...
**Dla agentów wbudowanych** (w `pencode.json`):
