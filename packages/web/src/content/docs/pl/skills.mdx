...
leak-code przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu leak-code przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.leak-code/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/leak-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.leak-code/skills/git-release/SKILL.md` w ten sposób:
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `leak-code.json`:
...
**Dla agentów wbudowanych** (w `leak-code.json`):
