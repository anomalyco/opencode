...
Umiejętności agenta pozwalają xcsh odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
xcsh przeszukuje te lokalizacje:

- Project config: `.xcsh/skills/<name>/SKILL.md`
- Global config: `~/.config/xcsh/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu xcsh przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.xcsh/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.xcsh/skills/git-release/SKILL.md` w ten sposób:
...
xcsh wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `xcsh.json`:
...
**Dla agentów wbudowanych** (w `xcsh.json`):
