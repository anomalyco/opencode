...
Umiejętności agenta pozwalają cyberstrike odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
cyberstrike przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu cyberstrike przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.cyberstrike/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.cyberstrike/skills/git-release/SKILL.md` w ten sposób:
...
cyberstrike wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `cyberstrike.json`:
...
**Dla agentów wbudowanych** (w `cyberstrike.json`):
