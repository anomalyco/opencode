...
:::tip[Bez automatycznego otwierania]
Aby uruchomić serwer web bez automatycznego otwierania przeglądarki, użyj zamiast tego `opencode serve`:

```bash
opencode serve --hostname 0.0.0.0 --port 4096
```

Następnie przejdź ręcznie do `http://localhost:4096` (lub swojego adresu sieciowego). Przy użyciu `0.0.0.0` ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić dostęp.
:::
