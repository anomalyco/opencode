...
W CyberStrike obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
CyberStrike zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
CyberStrike ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

CyberStrike ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `cyberstrike.json`:
...
- Globalnie: `~/.config/cyberstrike/agents/`
- W projekcie: `.cyberstrike/agents/`
...
Jeśli nie zostanie skonfigurowana, CyberStrike używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji CyberStrike, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji CyberStrike w `provider/model-id`. Na przykład, [CyberStrike Zen](/docs/zen), użyj `cyberstrike/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `cyberstrike models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/CyberStrikeus/cyberstrike).
:::
