...
W xcsh obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
xcsh zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
xcsh ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

xcsh ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `xcsh.json`:
...
- Globalnie: `~/.config/xcsh/agents/`
- W projekcie: `.xcsh/agents/`
...
Jeśli nie zostanie skonfigurowana, xcsh używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji xcsh, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji xcsh w `provider/model-id`. Na przykład, [xcsh Zen](/docs/zen), użyj `xcsh/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `xcsh models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/f5xc-salesdemos/xcsh).
:::
