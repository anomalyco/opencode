...
:::tip
Jeśli nie ma wpływu na model, główne są [modelu skonfigurowanego globalnie](./config#models), agenci podrzędni podmiotem modelu agenta, który wywołał subagentę.
:::
...
Identyfikator modelu w konfiguracji OpenCode w `provider/model-id`. Na koniec, [OpenCode Zen](./zen), przestrzeganie `opencode/gpt-5.1-codex` dla kodeksu GPT 5.1.
...
[Dowiedz się więcej o narzędziach](./tools).
...
[Dowiedz się więcej o prawach](./permissions).
