---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w cyberstrike.
---

import config from "../../../../config.mjs"
export const console = config.console

cyberstrike używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji cyberstrike.
...
w `~/.local/share/cyberstrike/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim cyberstrike
config.
...
CyberStrike Zen to lista modeli dostarczonych przez zespół cyberstrike, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z cyberstrike. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od CyberStrike Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz cyberstrike i przejdź do [cyberstrike.io/auth](https://cyberstrike.io/auth).
...
Działa jak każdy inny dostawca w cyberstrike i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z cyberstrike:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania cyberstrike:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `cyberstrike.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w cyberstrike nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod cyberstrike działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji cyberstrike.
...
cyberstrike używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować cyberstrike
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`cyberstrike.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem cyberstrike:
...
Aby używać Google Vertex AI z cyberstrike:
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą cyberstrike, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [cyberstrike-helicone-session](https://github.com/H2Shami/cyberstrike-helicone-session), aby automatycznie rejestrować każdą konwersację cyberstrike jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja cyberstrike będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować cyberstrike tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować cyberstrike tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować cyberstrike tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem cyberstrike. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/cyberstrike).
:::
...
Aby korzystać z Ollama Cloud z cyberstrike:
...
3. Skopiuj klucz API do użycia w cyberstrike.
...
6. **Ważne**: Przed użyciem modeli chmurowych w cyberstrike musisz pobrać informacje o modelu lokalnie:
...
CyberStrike Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół cyberstrike. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>CyberStrike Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **CyberStrike Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API cyberstrike.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji cyberstrike.
...
5. Możesz je także dostosować za pomocą konfiguracji cyberstrike. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z cyberstrike:
...
Możesz także dostosować modele za pomocą konfiguracji cyberstrike. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z cyberstrike. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `cyberstrike.json` w katalogu projektu:
...
Pola `limit` pozwalają cyberstrike zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `cyberstrike auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację cyberstrike i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji cyberstrike.
