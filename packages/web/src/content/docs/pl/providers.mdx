...
w `~/.local/share/leak-code/auth.json`.
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `leak-code.json`:
...
`leak-code.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `leak-code.json`:
...
4. Utwórz lub zaktualizuj plik `leak-code.json` w katalogu projektu:
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `leak-code auth list`, aby sprawdzić, czy poświadczenia
