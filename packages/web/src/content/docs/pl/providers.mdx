...
w `~/.local/share/octopus/auth.json`.
...
Octopus Zen to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Octopus Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Octopus Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Octopus Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Octopus Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `octopus.json`:
...
`octopus.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `octopus.json`:
...
Octopus Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Octopus Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Octopus Zen**.
...
4. Utwórz lub zaktualizuj plik `octopus.json` w katalogu projektu:
