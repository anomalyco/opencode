---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji xcsh.
...
w `~/.local/share/xcsh/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim xcsh
config.
...
xcsh Zen to lista modeli dostarczonych przez zespół xcsh, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z xcsh. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od xcsh Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `xcsh Zen` i przejdź do [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Działa jak każdy inny dostawca w xcsh i jest całkowicie opcjonalny w użyciu.

---

## xcsh Go

xcsh Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół xcsh, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z xcsh.

1. Uruchom polecenie `/connect` w TUI, wybierz `xcsh Go` i przejdź do [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/zen).
...
Aby używać Amazon Bedrock z xcsh:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania xcsh:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `xcsh.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w xcsh nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod xcsh działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji xcsh.
...
xcsh używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować xcsh
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`xcsh.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth).

##### Konfiguracja

Customize through `xcsh.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem xcsh:
...
Aby używać Google Vertex AI z xcsh:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania xcsh.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą xcsh, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session), aby automatycznie rejestrować każdą konwersację xcsh jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja xcsh będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować xcsh tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować xcsh tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować xcsh tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem xcsh. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/xcsh).
:::
...
Aby korzystać z Ollama Cloud z xcsh:
...
3. Skopiuj klucz API do użycia w xcsh.
...
6. **Ważne**: Przed użyciem modeli chmurowych w xcsh musisz pobrać informacje o modelu lokalnie:
...
xcsh Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół xcsh. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>xcsh Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **xcsh Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API xcsh.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji xcsh.
...
5. Możesz je także dostosować za pomocą konfiguracji xcsh. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z xcsh:
...
Możesz także dostosować modele za pomocą konfiguracji xcsh. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z xcsh. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `xcsh.json` w katalogu projektu:
...
Pola `limit` pozwalają xcsh zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `xcsh auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację xcsh i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji xcsh.
