---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w pencode.
---

import config from "../../../../config.mjs"
export const console = config.console

pencode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji pencode.
...
w `~/.local/share/pencode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim pencode
config.
...
Pencode Zen to lista modeli dostarczonych przez zespół pencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z pencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Pencode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Pencode Zen` i przejdź do [pencode.ai/auth](https://pencode.ai/zen).
...
Działa jak każdy inny dostawca w pencode i jest całkowicie opcjonalny w użyciu.

---

## Pencode Go

Pencode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół pencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z pencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Pencode Go` i przejdź do [pencode.ai/auth](https://pencode.ai/zen).
...
Aby używać Amazon Bedrock z pencode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania pencode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `pencode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w pencode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować pencode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod pencode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji pencode.
...
pencode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować pencode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`pencode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [pencode-gitlab-auth](https://www.npmjs.com/package/pencode-gitlab-auth).

##### Konfiguracja

Customize through `pencode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem pencode:
...
Aby używać Google Vertex AI z pencode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania pencode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą pencode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [pencode-helicone-session](https://github.com/H2Shami/pencode-helicone-session), aby automatycznie rejestrować każdą konwersację pencode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja pencode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować pencode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować pencode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować pencode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem pencode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/pencode).
:::
...
Aby korzystać z Ollama Cloud z pencode:
...
3. Skopiuj klucz API do użycia w pencode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w pencode musisz pobrać informacje o modelu lokalnie:
...
Pencode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół pencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Pencode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Pencode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API pencode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji pencode.
...
5. Możesz je także dostosować za pomocą konfiguracji pencode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z pencode:
...
Możesz także dostosować modele za pomocą konfiguracji pencode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z pencode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `pencode.json` w katalogu projektu:
...
Pola `limit` pozwalają pencode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `pencode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację pencode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji pencode.
