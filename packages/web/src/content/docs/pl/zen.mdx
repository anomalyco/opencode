...
Jev to model System One od TypeSafe AI przeznaczony do szybkiego podejmowania ustrukturyzowanych decyzji. Zamiast generować tekst, ocenia `state` według pytań z określonymi typami i zwraca wartości oraz prawdopodobieństwa, które Twój kod może wykorzystać bezpośrednio. Obsługuje pytania tak/nie (`noul`), wielokrotnego wyboru (`choice`) i oparte na kryteriach oceny (`score`).

Użyj klucza API OpenCode Zen z endpointem `https://opencode.ai/zen/v1/systemone`. Ten przykład sprawdza, czy zgłoszenie do pomocy technicznej jest pilne:
...
W jednym żądaniu możesz zadać wiele pytań. Jev ocenia je równolegle i zwraca każdą odpowiedź pod identyfikatorem odpowiadającego jej pytania:
...
Aby użyć bezpłatnego modelu dostępnego przez ograniczony czas, zastosuj `jev-1.13-free` zamiast `jev-1.13`. Szczegółowe informacje o typach pytań i polach odpowiedzi znajdziesz w [dokumentacji TypeSafe AI](https://docs.typesafe.ai/).
...
| Jev 1.13 Free                     | Darmowe | Darmowe | -              | -              |
| Jev 1.13                          | $0.042  | Darmowe | -              | -              |
