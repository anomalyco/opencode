...
5. **Konfiguracja `.opencode`** (`.opencode/opencode.json` w projekcie) - taka sama jak konfiguracja projektu, ale oddzielna
6. **Katalogi `.opencode`** - agenci, polecenia, umiejętności
7. **Konfiguracja wbudowana** (zmienna `OPENCODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
