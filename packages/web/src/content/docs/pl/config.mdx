...
2. **Konfiguracja globalna** (`~/.config/leak-code/leak-code.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `LEAKCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`leak-code.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.leak-code`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `LEAKCODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.leak-code` i `~/.config/leak-code` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/leak-code/leak-code.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/leak-code/tui.json`.
...
Dodaj `leak-code.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `LEAKCODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `LEAKCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.leak-code` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.leak-code`, więc **może być łączony** z ich zawartością.
...
Użyj `LEAKCODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `leak-code.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `leak-code serve` i `leak-code web` za pomocą opcji `server`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/leak-code/agents/` lub `.leak-code/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Dotyczy to wszystkich interfejsów: TUI, CLI (`leak-code run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/leak-code/commands/` lub `.leak-code/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.leak-code/plugins/` lub `~/.config/leak-code/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
