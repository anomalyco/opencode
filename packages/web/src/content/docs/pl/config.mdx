...
[Dowiedz się więcej o korzystaniu z TUI tutaj](./tui).
...
[Dowiedz się więcej o tutaj](./server).
...
[Dowiedz się więcej o narzędziach tutaj](./tools).
...
Możesz także mieć [model lokalny](./models#local). [Dowiedz się więcej](./models).
...
:::note
Token okaziciela (`AWS_BEARER_TOKEN_BEDROCK` lub `/connect`) ma pierwszeństwo przed uwierzytelnianiem opartym na profilu. Aby uzyskać szczegółowe informacje, zobacz [pierwszeństwo uwierzytelniania](./providers#authentication-precedence).
:::

[Dowiedz się więcej o budowie Amazon Bedrock](./providers#amazon-bedrock).
...
[Dowiedz się więcej tutaj](./themes).
...
Możesz także uniknąć agentów przy użyciu plików przecen w `~/.config/opencode/agents/` lub `.opencode/agents/`. [Dowiedz się więcej tutaj](./agents).
...
Agent domyślny musi być agentem (nie subagentem). Może być agentem aplikacji, taki jak `"build"` lub `"plan"`, albo ukryty przez Ciebie [agent niestandardowy](./agents). Jeśli agent nie istnieje lub jest podagentem, OpenCode powróć do `"build"` z ostrzeżeniem.
...
Możesz udostępnić [udostępnij](./share) za pomocą opcji `share`.
...
Można także definiować polecenia przy użyciu plików Markdown w `~/.config/opencode/commands/` lub `.opencode/commands/`. [Dowiedz się więcej tutaj](./commands).
...
[Dowiedz się więcej tutaj](./keybinds).
...
[Dowiedz się więcej o formaterach tutaj](./formatters).
...
[Dowiedz się więcej o prawach tutaj](./permissions).
...
[Dowiedz się więcej tutaj](./mcp-servers).

---

### Wtyki

[Wtyczki](./plugins) rozszerzają OpenCode o podstawowe narzędzia, hooki i integracje.
...
[Dowiedz się więcej tutaj](./plugins).
