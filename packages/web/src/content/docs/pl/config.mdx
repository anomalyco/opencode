---
title: Konfiguracja
description: Ustawienia Octopus w pliku JSON.
---

Możesz dostosować Octopus za pomocą pliku konfiguracyjnego JSON.

---

## Format

Octopus obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
2. **Konfiguracja globalna** (`~/.config/octopus/octopus.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OCTOPUS_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`octopus.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.opencode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OCTOPUS_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
Umieść swoją globalną konfigurację Octopus w `~/.config/octopus/octopus.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/octopus/tui.json`.
...
Dodaj `octopus.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Octopus się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OCTOPUS_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OCTOPUS_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.
...
Użyj `OCTOPUS_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `octopus.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Octopus.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Octopus wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/octopus/agents/` lub `.octopus/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Octopus powróci do `"build"` z ostrzeżeniem.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/octopus/commands/` lub `.octopus/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Octopus może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Octopus nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Octopus **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Octopus o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.octopus/plugins/` lub `~/.config/octopus/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Octopus tylko do określonych dostawców.
