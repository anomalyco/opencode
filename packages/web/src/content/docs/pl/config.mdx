---
title: Konfiguracja
description: Ustawienia CyberStrike w pliku JSON.
---

Możesz dostosować CyberStrike za pomocą pliku konfiguracyjnego JSON.

---

## Format

CyberStrike obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "cyberstrike"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/cyberstrike`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/cyberstrike/cyberstrike.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `CYBERSTRIKE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`cyberstrike.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.cyberstrike`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `CYBERSTRIKE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.cyberstrike` i `~/.config/cyberstrike` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/cyberstrike`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację CyberStrike w `~/.config/cyberstrike/cyberstrike.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `cyberstrike.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy CyberStrike się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CYBERSTRIKE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CYBERSTRIKE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.cyberstrike` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.cyberstrike`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`cyberstrike.io/config.json`**](https://cyberstrike.io/config.json).
...
Możesz skonfigurować serwer dla `cyberstrike serve` i `cyberstrike web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CyberStrike.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `cyberstrike.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CyberStrike wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu CyberStrike za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/cyberstrike/agents/` lub `.cyberstrike/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CyberStrike powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`cyberstrike run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/cyberstrike/commands/` lub `.cyberstrike/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
CyberStrike może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CyberStrike nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CyberStrike **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CyberStrike o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.cyberstrike/plugins/` lub `~/.config/cyberstrike/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CyberStrike tylko do określonych dostawców.
