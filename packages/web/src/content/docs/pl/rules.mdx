...
Możesz także mieć reguły globalne w pliku `~/.config/octopus/AGENTS.md`. Jest to stosowane we wszystkich sesjach opencode.
...
- **Zasady globalne**: `~/.claude/CLAUDE.md` (używane, jeśli nie istnieje `~/.config/octopus/AGENTS.md`)
...
2. **Plik globalny** w `~/.config/octopus/AGENTS.md`
3. **Plik Claude Code** w `~/.claude/CLAUDE.md` (chyba że wyłączony)

Pierwszy pasujący plik wygrywa w każdej kategorii. Na przykład, jeśli masz zarówno `AGENTS.md`, jak i `CLAUDE.md`, używany jest tylko `AGENTS.md`. Podobnie `~/.config/octopus/AGENTS.md` ma pierwszeństwo przed `~/.claude/CLAUDE.md`.

---

## Instrukcje niestandardowe

Możesz określić niestandardowe pliki instrukcji w swoim `octopus.json` lub globalnym `~/.config/octopus/octopus.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie octopus.json

Zalecanym podejściem jest użycie pola `instructions` w `octopus.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `octopus.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
