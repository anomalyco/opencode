...
Możesz także mieć reguły globalne w pliku `~/.config/leak-code/AGENTS.md`. Jest to stosowane we wszystkich sesjach opencode.
...
- **Zasady globalne**: `~/.claude/CLAUDE.md` (używane, jeśli nie istnieje `~/.config/leak-code/AGENTS.md`)
...
2. **Plik globalny** w `~/.config/leak-code/AGENTS.md`
3. **Plik Claude Code** w `~/.claude/CLAUDE.md` (chyba że wyłączony)

Pierwszy pasujący plik wygrywa w każdej kategorii. Na przykład, jeśli masz zarówno `AGENTS.md`, jak i `CLAUDE.md`, używany jest tylko `AGENTS.md`. Podobnie `~/.config/leak-code/AGENTS.md` ma pierwszeństwo przed `~/.claude/CLAUDE.md`.

---

## Instrukcje niestandardowe

Możesz określić niestandardowe pliki instrukcji w swoim `leak-code.json` lub globalnym `~/.config/leak-code/leak-code.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie leak-code.json

Zalecanym podejściem jest użycie pola `instructions` w `leak-code.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `leak-code.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
