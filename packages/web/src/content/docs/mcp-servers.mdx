...
You can add external tools to Pencode using the _Model Context Protocol_, or MCP. Pencode supports both local and remote servers.
...
You can define MCP servers in your [Pencode Config](https://pencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/pencode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Pencode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Pencode will:
...
If the server requires authentication, Pencode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `pencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Pencode will store the tokens securely in `~/.local/share/pencode/mcp-auth.json`.
...
Your MCPs are available as tools in Pencode, alongside built-in tools. So you can manage them through the Pencode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Pencode to your Sentry account.
