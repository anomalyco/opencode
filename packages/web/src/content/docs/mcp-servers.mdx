...
For remote MCP servers that require OAuth authentication, you can configure the `oauth` field. OpenCode supports:

- **Dynamic Client Registration (RFC 7591)**: If the server supports it, OpenCode will automatically register as a client.
- **Pre-registered Clients**: If you have a client ID from the MCP server provider, you can configure it directly.

#### Dynamic Registration

If the MCP server supports dynamic client registration, you can simply add an empty `oauth` object:
...
After configuring an OAuth-enabled MCP server, you need to authenticate:
