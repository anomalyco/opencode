...
Add the [opencode-history-mcp](https://github.com/singleflo/opencode-history-mcp) server to search your past OpenCode conversations before starting new work on a module or file.
...
Before first use, build the local search index from your `opencode.db`:

```bash
uvx opencode-history-mcp --build-index
```

It runs entirely locally — it reads OpenCode's own SQLite database read-only and builds a private FTS5 index next to it, no network calls. Exposes 7 tools for keyword search, finding sessions that touched a specific file, and browsing session history.

Add something like this to your [AGENTS.md](/docs/rules/) so agents check history before starting new work:

```md title="AGENTS.md"
Before exploring files or writing code for a task that touches an
existing module or file, call `find_related_work` and
`find_sessions_by_file` first to check if this was already worked on.
```
