...
You can add external tools to Octopus using the _Model Context Protocol_, or MCP. Octopus supports both local and remote servers.
...
You can define MCP servers in your [Octopus Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Octopus automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Octopus will:
...
If the server requires authentication, Octopus will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Octopus will store the tokens securely in `~/.local/share/octopus/mcp-auth.json`.
...
Your MCPs are available as tools in Octopus, alongside built-in tools. So you can manage them through the Octopus config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Octopus to your Sentry account.
