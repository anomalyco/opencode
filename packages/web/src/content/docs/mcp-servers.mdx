...
You can add external tools to OpenSec using the _Model Context Protocol_, or MCP. OpenSec supports both local and remote servers.
...
You can define MCP servers in your [OpenSec Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
OpenSec automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenSec will:
...
If the server requires authentication, OpenSec will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenSec will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in OpenSec, alongside built-in tools. So you can manage them through the OpenSec config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenSec to your Sentry account.
