...
You can add external tools to xcsh using the _Model Context Protocol_, or MCP. xcsh supports both local and remote servers.
...
You can define MCP servers in your [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/xcsh` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
xcsh automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, xcsh will:
...
If the server requires authentication, xcsh will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `xcsh mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, xcsh will store the tokens securely in `~/.local/share/xcsh/mcp-auth.json`.
...
Your MCPs are available as tools in xcsh, alongside built-in tools. So you can manage them through the xcsh config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect xcsh to your Sentry account.
