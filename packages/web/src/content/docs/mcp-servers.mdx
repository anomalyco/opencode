...
Add the [QVeris MCP server](https://github.com/QVerisAI/qveris-agent-toolkit/tree/main/packages/mcp) to discover, inspect, and call external tools.

Create a [QVeris API key](https://qveris.ai/account?page=api-keys), set it as `QVERIS_API_KEY`, and add the server to your config:
...
You can then ask OpenCode to use QVeris when a task needs an external capability:

```txt "use qveris"
Discover a weather capability and get the current weather in Tokyo. use qveris
```
