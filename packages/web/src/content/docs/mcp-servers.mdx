...
You can add external tools to ChaliceCode using the _Model Context Protocol_, or MCP.

ChaliceCode supports both:
...
You can define MCP servers in your ChaliceCode config under `mcp`. Add each MCP
...
Your MCPs are available as tools in ChaliceCode, alongside built-in tools. So you
can manage them through the ChaliceCode config like any other tool.
