...
You can add external tools to Leak Code using the _Model Context Protocol_, or MCP. Leak Code supports both local and remote servers.
...
You can define MCP servers in your [Leak Code Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Leak Code automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Leak Code will:
...
If the server requires authentication, Leak Code will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `leak-code mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Leak Code will store the tokens securely in `~/.local/share/leak-code/mcp-auth.json`.
...
Your MCPs are available as tools in Leak Code, alongside built-in tools. So you can manage them through the Leak Code config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Leak Code to your Sentry account.
