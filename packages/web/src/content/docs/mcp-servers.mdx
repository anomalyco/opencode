...
Some MCP screenshot tools return image-only content with no text payload. OpenCode now normalizes this for Kimi k2.5 models so tool results are still valid and the session can continue.

If an MCP tool response has image data but empty text, OpenCode keeps the attachment and injects a minimal non-empty tool-result text automatically.
