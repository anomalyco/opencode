...
Pair a phone running [Build Remote Agent](https://grokbuildremote.com/) as a **spectator** for this OpenCode session. Protocol `gbr/1`. Independent product; not affiliated with xAI or SpaceX.

Install the free MIT `gbr-agent` (v0.6.0+), pair (browser QR **and** printed 8-char code), then leave `gbr-agent run` going. Attach only via MCP stdio `gbr-mcp` or Bot API `http://127.0.0.1:8788`. The phone does not orchestrate OpenCode.
...
Point `command` at the cloned `gbr-mcp.js` path on your machine. Never put mailbox keys in `opencode.json`. After `gbr-agent run`:
...
Add `use gbr` to a prompt when you want OpenCode to talk to the pairing device. Agent source: [GrokBuildRemote-Agents](https://github.com/LinespottingOrg/GrokBuildRemote-Agents).
