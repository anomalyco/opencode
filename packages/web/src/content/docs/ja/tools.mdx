...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 OpenCode には一連の組み込みツールが付属していますが、[カスタム ツール](./custom-tools) または [MCP サーバー](./mcp-servers).

デフォルトでは、すべてのツールは**有効**になっており、実行するための権限は必要ありません。 [permissions](./permissions).
...
[アクセス許可の構成について詳しくは、](./permissions) をご覧ください。
...
プロジェクトで使用できる LSP サーバーを構成するには、「LSP Servers](./lsp).
...
[skill](./skills) (`SKILL.md` ファイル) をロードし、会話でその内容を返します。
...
:::注記
このツールはデフォルトではサブエージェントに対して無効になっていますが、手動で有効にすることができます。 [詳細はこちら](./agents/#permissions)
:::
...
[カスタム ツールの作成について詳しくは、](./custom-tools) をご覧ください。
...
[MCP サーバーの構成について詳しくは、](./mcp-servers) をご覧ください。
