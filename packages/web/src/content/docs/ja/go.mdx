...
3. ルーティングとプロンプトキャッシュを最適化できるよう、会話ごとに安定したセッションIDを`x-opencode-session`で送信する。
   会話内のすべてのリクエスト（補助的な呼び出しを含む）で同じIDを再利用し、リクエストごとやプロセスごとに新しいIDを生成しない。
   また、クライアントが使用するすべてのエンドポイント（`/responses`、`/chat/completions`、`/messages`）で送信する。
   1つのSDKのミドルウェアだけでヘッダーを付与すると、他のエンドポイントには届かない。
