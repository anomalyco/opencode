...
これらの制限は、全モデルで共有する利用枠に対する割合です。

### モデル別の利用枠

含まれる月間利用枠はモデルによって異なり、公式API価格に基づく金額で示しています。トークン単価は100万トークンあたりです。

| Model                                   | Input  | Output | Cached Read | Cached Write | 月間利用枠 |
| --------------------------------------- | ------ | ------ | ----------- | ------------ | ----- |
...
利用できるリクエスト数は、モデルの月間利用枠とAPI価格によって異なります。以下の推定値は、一般的なGoの利用パターンに基づいています。
