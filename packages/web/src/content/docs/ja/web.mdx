...
:::tip[Windows ユーザー]
最高のエクスペリエンスを得るには、PowerShell ではなく [WSL](./windows-wsl) から `opencode web` を実行します。これにより、ファイル システムへの適切なアクセスと端末の統合が保証されます。
:::
...
コマンド ライン フラグを使用するか、[config file](./config).config ファイル] で Web サーバーを設定できます。
