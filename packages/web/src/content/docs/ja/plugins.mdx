---
title: プラグイン
description: xcsh を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで xcsh を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、xcsh のデフォルトの動作を変更したりできます。
...
- `.xcsh/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/xcsh/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/xcsh/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/xcsh/xcsh.json`)
2. プロジェクト設定 (`xcsh.json`)
3. グローバルプラグインディレクトリ (`~/.config/xcsh/plugins/`)
4. プロジェクトプラグインディレクトリ (`.xcsh/plugins/`)
...
xcsh は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための xcsh SDK クライアント。
...
ここでは、xcsh を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) を参照してください。
...
プラグインは xcsh にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、xcsh が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して xcsh で使用できます。
