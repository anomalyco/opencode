---
title: プラグイン
description: Octopus を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Octopus を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Octopus のデフォルトの動作を変更したりできます。
...
- `.octopus/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/octopus/plugins/` - グローバルプラグイン
...
1. グローバル設定 (`~/.config/octopus/octopus.json`)
2. プロジェクト設定 (`octopus.json`)
3. グローバルプラグインディレクトリ (`~/.config/octopus/plugins/`)
4. プロジェクトプラグインディレクトリ (`.octopus/plugins/`)
...
Octopus は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Octopus SDK クライアント。
...
ここでは、Octopus を拡張するために使用できるプラグインの例をいくつか示します。
...
Octopus が `.env` ファイルを読み取らないようにします。
...
プラグインは Octopus にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Octopus が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Octopus で使用できます。
