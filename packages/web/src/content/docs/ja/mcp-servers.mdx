...
ローカルの設定値はリモートのデフォルト値をオーバーライドします。詳細については、「config precedence](./config#precedence-order)」を参照してください。
...
2. [エージェント config](./agents#tools) で、MCP サーバーをツールとして有効にします。
