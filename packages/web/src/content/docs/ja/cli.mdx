...
| 変数                                  | タイプ   | 説明                                                                |
| ------------------------------------- | -------- | ------------------------------------------------------------------- |
| `OPENCODE_AUTH_CONTENT                ` | 文字列     | インライン JSON 認証情報 |
...
| `OPENCODE_DISABLE_EXTERNAL_SKILLS     ` | ブール値    | npm パッケージからのスキル読み込みを無効にする |
| `OPENCODE_DISABLE_LSP_DOWNLOAD        ` | ブール値    | LSP サーバーの自動ダウンロードを無効にする |
| `OPENCODE_DISABLE_MODELS_FETCH        ` | ブール値    | リモートソースからのモデルの取得を無効にする |
| `OPENCODE_DISABLE_MOUSE               ` | ブール値    | TUI でのマウスキャプチャを無効にする |
| `OPENCODE_DISABLE_PRUNE               ` | ブール値    | 古いデータのプルーニングを無効にする |
| `OPENCODE_DISABLE_PROJECT_CONFIG      ` | ブール値    | プロジェクトレベルの設定読み込みを無効にする |
...
| `OPENCODE_MODELS_PATH                 ` | 文字列     | モデル JSON キャッシュファイルのカスタムパス |
...
| `OPENCODE_TUI_CONFIG                  ` | 文字列     | TUI 構成ファイルへのパス |
| `EXA_API_KEY                          ` | 文字列     | Exa ウェブ検索の API キー（`OPENCODE_ENABLE_EXA` が必要） |
| `OTEL_EXPORTER_OTLP_ENDPOINT          ` | 文字列     | OpenTelemetry OTLP エクスポーターエンドポイント |
| `OTEL_EXPORTER_OTLP_HEADERS           ` | 文字列     | OpenTelemetry OTLP エクスポーターヘッダー |
| `OTEL_RESOURCE_ATTRIBUTES             ` | 文字列     | OpenTelemetry リソース属性（key=value ペア） |
...
| `OPENCODE_EXPERIMENTAL_HTTPAPI                  ` | ブール値    | Effect HttpApi ブリッジを有効にする |
...
| `OPENCODE_EXPERIMENTAL_WORKSPACES               ` | ブール値    | 実験的なワークスペースを有効にする |
