...
OpenCode CLI は、引数なしで実行すると、デフォルトで [TUI](./tui) を開始します。
...
これにより、必要な GitHub Actions ワークフローが設定され、構成プロセスがガイドされます。 [詳細はこちら](./github)。
...
これは、config](./config/) で使用する正確なモデル名を把握するのに役立ちます。
...
API アクセスのためにヘッドレス OpenCode サーバーを起動します。完全な HTTP インターフェイスについては、[server docs](./server)] を確認してください。
