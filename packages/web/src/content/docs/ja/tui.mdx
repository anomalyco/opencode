...
ほとんどのコマンドには、`ctrl+x` をリーダー キーとして使用するキーバインドもあります。`ctrl+x` がデフォルトのリーダー キーです。 [詳細はこちら](./keybinds)。
...
`AGENTS.md` ファイルを作成または更新します。 [詳細はこちら](./rules)。
...
現在のセッションの共有を解除します。 [詳細はこちら](./share#un-sharing)。
