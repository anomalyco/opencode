...
- プロジェクト設定: `.leak-code/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/leak-code/skills/<name>/SKILL.md`
...
途中で、一致する `skills/*/SKILL.md` を `.leak-code/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/leak-code/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.leak-code/skills/git-release/SKILL.md` を作成します。
...
`leak-code.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`leak-code.json` 内):
