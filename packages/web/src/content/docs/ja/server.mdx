---
title: サーバー
description: HTTP 経由で CyberStrike サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`cyberstrike serve` コマンドは、CyberStrike クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `cyberstrike.local` |
...
HTTP 基本認証でサーバーを保護するには、`CYBERSTRIKE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `cyberstrike` になるか、`CYBERSTRIKE_SERVER_USERNAME` を設定してオーバーライドします。これは、`cyberstrike serve` と `cyberstrike web` の両方に当てはまります。
...
`cyberstrike` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
CyberStrike サーバーを使用して、プログラムで CyberStrike と対話します。
:::

`cyberstrike serve` を実行してスタンドアロンサーバーを起動できます。すでに CyberStrike TUI を実行している場合でも、`cyberstrike serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、CyberStrike [IDE](/docs/ide) プラグインによって使用されます。
...
CyberStrike サーバーは次の API を公開します。
