---
title: サーバー
description: HTTP 経由で Pencode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`pencode serve` コマンドは、Pencode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `pencode.local` |
...
HTTP 基本認証でサーバーを保護するには、`PENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `pencode` になるか、`PENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`pencode serve` と `pencode web` の両方に当てはまります。
...
`pencode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Pencode サーバーを使用して、プログラムで Pencode と対話します。
:::

`pencode serve` を実行してスタンドアロンサーバーを起動できます。すでに Pencode TUI を実行している場合でも、`pencode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Pencode [IDE](/docs/ide) プラグインによって使用されます。
