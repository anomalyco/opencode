---
title: Octopus Zen
description: Octopus によって提供されるモデルの厳選されたリスト。
---
...
Octopus Zen は、Octopus チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

:::note
Octopus Zen は現在ベータ版です。
:::

Zen は Octopus のほかのプロバイダーと同じように動作します。Octopus Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Octopus を使うために必須ではありません。
...
:::tip
Octopus でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Octopus Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Octopus Zen は、Octopus のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Octopus Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Octopus Zen を選んで API キーを貼り付けます。
...
Octopus 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
- MiniMax M2.5 Free は期間限定で Octopus で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ring 2.6 1T Free は期間限定で Octopus で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Super Free は期間限定で Octopus で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Octopus で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが Octopus Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Octopus では常にほかのプロバイダーも使えるようにすること。
