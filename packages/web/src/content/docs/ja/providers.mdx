...
`~/.local/share/leak-code/auth.json`で。
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`leak-code.json` を使用します。
...
`leak-code.json` ファイル。セッション共有を無効にすることもお勧めします。
...
4. プロジェクトディレクトリで `leak-code.json` ファイルを作成または更新します。
...
1. **認証設定を確認します**: `leak-code auth list` を実行して、資格情報が正しいかどうかを確認します。
