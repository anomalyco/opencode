...
macOS サンドボックスが有効になっている場合、ブロックされた bash コマンドは個別の `bash:unsandboxed` 権限リクエストをトリガーできます。
これは、OpenCode がサンドボックス拒否の可能性を検出した場合、またはコマンドが最初の空でない行で `# opencode:unsandboxed <理由>` を使用して明示的にサンドボックスのスキップを要求した場合に発生します。
サンドボックスの設定は [`experimental.sandbox`](/docs/config#sandbox) で行います。
