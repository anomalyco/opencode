---
title: 設定
description: Octopus JSON 設定を使用します。
---

JSON 設定ファイルを使用して Octopus を構成できます。

---

## 形式

Octopus は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
2. **グローバル設定** (`~/.config/octopus/octopus.json`) - ユーザー設定
3. **カスタム設定** (`OCTOPUS_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `octopus.json`) - プロジェクト固有の設定
5. **`.opencode` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`OCTOPUS_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド
...
グローバル Octopus 設定を `~/.config/octopus/octopus.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。

TUI 固有の設定には、`~/.config/octopus/tui.json` を使用します。
...
プロジェクトのルートに `octopus.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。
...
Octopus が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
`OCTOPUS_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`OCTOPUS_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
...
カスタム TUI 設定ファイルを指定するには、`OCTOPUS_TUI_CONFIG` を使用します。

`octopus.json` 内の従来の `theme`、`keybinds`、および `tui` キーは非推奨となり、可能であれば自動的に移行されます。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが Octopus サーバーを検出できるようになります。
...
`provider`、`model`、および `small_model` オプションを使用して、Octopus 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトルの生成などの軽量タスク用に個別のモデルを構成します。デフォルトでは、Octopus は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`~/.config/octopus/agents/` または `.octopus/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、Octopus は警告とともに `"build"` にフォールバックします。
...
`~/.config/octopus/commands/` または `.octopus/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
Octopus は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して Octopus を拡張します。

プラグインファイルを `.octopus/plugins/` または `~/.config/octopus/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
...
これは、Octopus を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
