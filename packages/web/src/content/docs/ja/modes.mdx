...
:::caution
モードは、CyberStrike 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、CyberStrike 設定を通じて独自の設定を行います。
...
CyberStrike には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.cyberstrike/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`cyberstrike.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/cyberstrike/modes/`
- プロジェクト: `.cyberstrike/modes/`
...
温度が指定されていない場合、CyberStrike はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.cyberstrike/modes/` に、グローバルモードの場合は `~/.config/cyberstrike/modes/` に作成します。
