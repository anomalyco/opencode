...
カスタム コマンドは、`/init`、`/undo`、`/redo`、`/share`、`/help` などの組み込みコマンドに追加されます。 [詳細はこちら](./tui#commands)。
...
OpenCode で `command` オプションを使用します [config](./config):
...
_!`command`_ を使用して、[bash command](./tui#bash-commands) の出力をプロンプトに挿入します。
...
オプションで、`agent` 設定を使用して、このコマンドを実行する [agent](./agents) を指定します。
これが [subagent](./agents/#subagents) の場合、コマンドはデフォルトでサブエージェントの呼び出しをトリガーします。
...
`subtask` ブール値を使用して、コマンドが [subagent](./agents/#subagents) 呼び出しを強制的にトリガーします。
これは、コマンドがプライマリ コンテキストを汚染せず、エージェントがサブエージェントとして動作するように**強制**する場合に便利です。
[agent](./agents) 設定で `mode` が `primary` に設定されている場合でも。
...
opencode には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](./tui#commands)。
