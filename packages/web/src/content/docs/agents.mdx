...
Control which subagents a primary agent can invoke via the Task tool with `permission.task`. Uses the same glob pattern syntax as bash permissions.
...
Hide a subagent from the `@` autocomplete with `visible: false`. Useful for internal subagents that should only be invoked by other agents via the Task tool.
