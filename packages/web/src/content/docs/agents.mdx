...
## Nested subagents and teams

Subagents can delegate to other subagents when their `task_budget` allows it. This is useful for nested specialist workflows where a parent subagent coordinates a small amount of deeper research or implementation.
...
Use teams when you want multiple teammate sessions to coordinate through `team_message`, `team_broadcast`, task claiming, and plan approval. A teammate can still call `task` to delegate to a nested subagent, but that child subagent is not added to the team graph and cannot use `team_*` tools. The teammate should relay any relevant child findings back to the team.

[Learn more about Agent Teams](/docs/agent-teams).
