...
Subagents can spawn subagents of their own. A session tree starts at your main session — that's **depth 1**. Every Task tool call, `@` mention, or workflow agent dispatch creates a child session one level deeper.

By default, nesting is allowed up to **depth 5**. Sessions at depth 1 through 4 can delegate; sessions at the maximum depth are pure workers — the `task` and `workflow` tools are removed from their tool list entirely, so the model plans around it instead of wasting turns on failed attempts.
...
| -------------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Maximum nesting depth      | `5` (root session = depth 1), configurable via `experimental.subagent_max_depth`, clamped to 1–10  | The `task` and `workflow` tools are removed at the last level; any remaining spawn attempt fails with a typed `SubagentDepthError` |
| Subagents per session tree | `200` started subagents per tree, counted **per OpenCode process** (the counter resets on restart) | Further spawns fail with `SubagentTreeLimitError`                                                                                  |
...
- `2` — restores the previous spawn behavior: your main session can spawn subagents, but subagents cannot spawn further subagents. Two things still differ from the old release: the `workflow` tool is additionally removed at the limit (a security fix), and custom agents with an explicit `task` allow — which previously nested without limit — are now bounded too.
- `1` — kill switch: the `task` and `workflow` tools are removed everywhere, including from the main session.
