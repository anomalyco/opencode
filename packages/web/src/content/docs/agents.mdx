...
OpenCode comes with three built-in primary agents, **Build**, **Plan**, and **Orchestrator**. We'll look at these below.
...
Orchestrator is the built-in primary agent for parallel execution. It plans a task, launches workers in isolated git worktrees, and integrates their results.

Git behavior is:

- Workers branch from the current `HEAD` when the plan starts.
- Each worker runs on its own `opencode/...` branch in its own worktree.
- Completed worker branches merge into an integration branch such as `parallel/<plan-id>`.
- By default, the result stays on that integration branch. Your current branch is unchanged unless you publish with a mode such as `direct` or `unstaged`.

Use this when the task can be split across mostly independent files or subsystems.
