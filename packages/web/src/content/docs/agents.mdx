...
You can also load agents from external directories using the `paths` key inside the `agents` config object. Each `.md` file in these directories is loaded as a subagent — the filename (without `.md`) becomes the agent name.

```json
{
  "agents": {
    "paths": [
      "~/.config/shared-agents",
      "/absolute/path/to/more-agents",
      "./relative/to/config"
    ]
  }
}
...
Paths support `~/` for home directory expansion, absolute paths, and paths relative to the config file declaring them. Paths can be combined with inline agent definitions in the same `agents` block.
