...
There are two types of agents in Leak Code; primary agents and subagents.
...
Leak Code comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Leak Code comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Leak Code comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Leak Code's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `leak-code.json` config file:
...
- Global: `~/.config/leak-code/agents/`
- Per-project: `.leak-code/agents/`

```markdown title="~/.config/leak-code/agents/review.md"
---
...
If no temperature is specified, Leak Code uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Leak Code config and the project specific config.
...
The model ID in your Leak Code config uses the format `provider/model-id`. For example, if you're using [OpenCode Zen](/docs/zen), you would use `leak-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="leak-code.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="leak-code.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `leak-code models` to see a list of the available models.
:::
...
```markdown title="~/.config/leak-code/agents/docs-writer.md"
---
...
```markdown title="~/.config/leak-code/agents/security-auditor.md"
---
