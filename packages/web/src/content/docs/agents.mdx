...
There are two types of agents in CrazyCode; primary agents and subagents.
...
CrazyCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
CrazyCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

CrazyCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `crazycode.json` config file:
...
- Global: `~/.config/crazycode/agent/`
- Per-project: `.crazycode/agent/`

```markdown title="~/.config/crazycode/agent/review.md"
---
...
If no temperature is specified, CrazyCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global CrazyCode config and the project specific config.
...
The model ID in your CrazyCode config uses the format `provider/model-id`. For example, if you're using [CrazyCode Zen](/docs/zen), you would use `crazycode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="crazycode.json" {3-6,9-12}
{
  "$schema": "https://crazycode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="crazycode.json" {3-5,8-10}
{
  "$schema": "https://crazycode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `crazycode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/crazycode).
:::
...
```markdown title="~/.config/crazycode/agent/docs-writer.md"
---
...
```markdown title="~/.config/crazycode/agent/security-auditor.md"
---
