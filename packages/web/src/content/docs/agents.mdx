...
There are two types of agents in xcsh; primary agents and subagents.
...
xcsh comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
xcsh comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

xcsh comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `xcsh.json` config file:
...
If no temperature is specified, xcsh uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global xcsh config and the project specific config.
...
The model ID in your xcsh config uses the format `provider/model-id`. For example, if you're using [xcsh Zen](/docs/zen), you would use `xcsh/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `xcsh models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/f5xc-salesdemos/xcsh).
:::
