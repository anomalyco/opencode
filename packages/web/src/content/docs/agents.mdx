...
There are two types of agents in ChaliceCode; primary agents and subagents.
...
ChaliceCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
ChaliceCode comes with one built-in subagent, **General**. We'll look at this below.

---

## Built-in

ChaliceCode comes with two built-in primary agents and one built-in subagent.
...
- Global: `~/.config/chalice-code/agent/`
- Per-project: `.chalicecode/agent/`

```markdown title="~/.config/chalice-code/agent/review.md"
---
...
If no temperature is specified, ChaliceCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global ChaliceCode config and the project specific config.
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/sst/chalice-code).
:::
...
```markdown title="~/.config/chalice-code/agent/docs-writer.md"
---
...
```markdown title="~/.config/chalice-code/agent/security-auditor.md"
---
