...
| Subagents per session tree | `200` started subagents over the lifetime of the tree                                             | Further spawns fail with `SubagentTreeLimitError`                                                                                  |
...
- `2` — restores the previous behavior: your main session can spawn subagents, but subagents cannot spawn further subagents.
...
- Answering a prompt with `always` sticks to the root session and applies **tree-wide**.
