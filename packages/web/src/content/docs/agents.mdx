...
The model ID in your OpenCode config uses the format `provider/model-id`. For example, if you're using [OpenCode Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex. For OpenRouter, the format is `openrouter/slug`, for example the OpenRouter slug for MiniMax M2.5 is `minimax/minimax-m2.5` so you need to use `openrouter/minimax/minimax-m2.5` as agent name.
