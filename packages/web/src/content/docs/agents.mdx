...
OpenCode comes with three built-in primary agents, **Build**, **Plan**, and **Auto**. We'll
look at these below.
...
Build with LLM-approved permissions. Auto has the same tools and permission rules as Build, but requests that would normally ask for your approval are first answered by a validator LLM — informed by a running summary of the session — instead of blocking on you.

The validator can allow the call, deny it (the agent receives the reason and can correct course), or escalate back to you when it is uncertain. If the validator is unreachable or its answer is invalid, the ask falls back to you, so a validator failure never approves or blocks anything on its own. Every decision is persisted for audit, with raw command inputs and validator prompts redacted from the audit trail. [Learn more](/docs/permissions#auto-agent-llm-validator) about how auto mode changes the permission flow.
...
### Use command-validator

_Mode_: `primary`

Hidden system agent that answers permission asks when the [auto agent](#use-auto) is active, returning `ALLOW`, `DENY`, or `UNCERTAIN` for each request. Its model and prompt are configurable via `agent.command-validator` — the default is the session's small model. It runs automatically and is not selectable in the UI.
...
Hidden system agent that maintains the incremental session summary (what was done, tested, and how) that the command-validator reads. It runs at the end of every completed turn in auto mode, off the critical path. Its model and prompt are configurable via `agent.session-summarizer`. It runs automatically and is not selectable in the UI.
