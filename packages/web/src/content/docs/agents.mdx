...
By default, new subagent sessions start isolated from the current conversation. When you launch a subagent through the `task` tool, set `fork_context: true` to fork the current session history into the new child session. You can also set `reasoning_effort` on a new `task` call to override the subagent's reasoning level for that launch when the selected model supports it.
