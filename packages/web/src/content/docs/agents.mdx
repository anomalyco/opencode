...
By default, file edits are **denied** for this agent. In this fork, approvals are disabled by default, so other tools (like `bash`) run without prompts unless you explicitly configure `ask` rules.
