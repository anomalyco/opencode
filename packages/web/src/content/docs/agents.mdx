...
You can also load agents from external directories using the `agent_paths` config option. Each `.md` file in these directories is loaded as a subagent — the filename (without `.md`) becomes the agent name.
...
Paths support `~/` for home directory expansion, absolute paths, and paths relative to the config file declaring them.
