...
`title` is the human-readable display name. Use it when the UI should show a localized or friendlier name while `name` stays a stable ID.

`aliases` is a list of alternate slash-command names. Aliases can be localized and resolve to the canonical `name`.
Each alias is trimmed and must be non-empty without whitespace.
...
If a skill defines `title` or `aliases`, those are included too.

```xml
<available_skills>
  <skill>
    <name>git-release</name>
    <title>Git release</title>
