...
:::caution
เมื่อใช้ `--hostname 0.0.0.0` ให้ตั้งค่า `LEAKCODE_SERVER_PASSWORD` เพื่อป้องกันเซิร์ฟเวอร์
...
1. **รัน `leak-code web` ใน terminal WSL** แทน PowerShell:
...
การรัน `leak-code web` จาก WSL ช่วยให้เข้าถึงระบบไฟล์และใช้งาน terminal ได้ถูกต้อง พร้อมเปิดใช้งานผ่านเบราว์เซอร์ Windows ได้เหมือนเดิม
...
- การตั้งค่าและเซสชันของ OpenCode จะถูกเก็บในสภาพแวดล้อม WSL ที่ `~/.local/share/leak-code/`
