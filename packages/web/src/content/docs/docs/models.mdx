...
You can globally configure a model's options (and pin important models) through the config.
...
Here we are pinning the `gpt-5` model and setting global options for it when used through the `openai` provider. Pinned models appear first in the model selector (before Recent and provider groups).
