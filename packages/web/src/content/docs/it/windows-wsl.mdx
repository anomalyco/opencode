...
:::caution
Quando usi `--hostname 0.0.0.0`, imposta `LEAKCODE_SERVER_PASSWORD` per proteggere il server.
...
1. **Esegui `leak-code web` nel terminale WSL** invece che in PowerShell:
...
Eseguire `leak-code web` da WSL garantisce accesso corretto al file system e integrazione del terminale, restando comunque accessibile dal browser Windows.
...
- Configurazione e sessioni di OpenCode sono salvate nell'ambiente WSL in `~/.local/share/leak-code/`
