...
Con CyberStrike puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, CyberStrike usa il tema `cyberstrike`.
...
CyberStrike include diversi temi integrati.
...
- Vuole che CyberStrike corrisponda all'aspetto del terminale
...
CyberStrike supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/cyberstrike/themes/*.json` o `$XDG_CONFIG_HOME/cyberstrike/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.cyberstrike/themes/*.json`
4. **Directory di lavoro corrente** - in `./.cyberstrike/themes/*.json`
