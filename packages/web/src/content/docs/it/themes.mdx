...
2. **Directory di configurazione utente** - in `~/.config/leak-code/themes/*.json` o `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.leak-code/themes/*.json`
4. **Directory di lavoro corrente** - in `./.leak-code/themes/*.json`
