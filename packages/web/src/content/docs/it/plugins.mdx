---
title: Plugin
description: Scrivi plugin per estendere CyberStrike.
---

I plugin ti permettono di estendere CyberStrike agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di CyberStrike.
...
- `.cyberstrike/plugins/` - plugin a livello progetto
- `~/.config/cyberstrike/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/cyberstrike/node_modules/`.
...
1. Config globale (`~/.config/cyberstrike/cyberstrike.json`)
2. Config di progetto (`cyberstrike.json`)
3. Directory plugin globale (`~/.config/cyberstrike/plugins/`)
4. Directory plugin di progetto (`.cyberstrike/plugins/`)
...
CyberStrike esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di cyberstrike per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere cyberstrike.
...
:::note
Se usi l'app desktop di CyberStrike, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a cyberstrike di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a cyberstrike:
...
L'helper `tool` crea uno strumento personalizzato che cyberstrike puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in cyberstrike insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://cyberstrike.io/docs/sdk) per i dettagli.
