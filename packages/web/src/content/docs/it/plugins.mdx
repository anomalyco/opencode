---
title: Plugin
description: Scrivi plugin per estendere xcsh.
---

I plugin ti permettono di estendere xcsh agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di xcsh.
...
- `.xcsh/plugins/` - plugin a livello progetto
- `~/.config/xcsh/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/xcsh/node_modules/`.
...
1. Config globale (`~/.config/xcsh/xcsh.json`)
2. Config di progetto (`xcsh.json`)
3. Directory plugin globale (`~/.config/xcsh/plugins/`)
4. Directory plugin di progetto (`.xcsh/plugins/`)
...
xcsh esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di xcsh per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere xcsh.
...
:::note
Se usi l'app desktop di xcsh, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a xcsh di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a xcsh:
...
L'helper `tool` crea uno strumento personalizzato che xcsh può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in xcsh insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://f5xc-salesdemos.github.io/xcsh/docs/sdk) per i dettagli.
