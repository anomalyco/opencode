...
Per esempi, dai un'occhiata ai [plugin](./ecosystem#plugins) creati dalla community.
...
Sfoglia i plugin disponibili nell'[ecosistema](./ecosystem#plugins).
...
I **plugin locali** vengono caricati direttamente dalla directory dei plugin. Per usare pacchetti esterni, devi creare un `package.json` nella directory di configurazione (vedi [Dipendenze](#dependencies)) oppure pubblicare il plugin su npm e [aggiungerlo alla configurazione](./config#plugins).
