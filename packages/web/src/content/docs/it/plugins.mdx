---
title: Plugin
description: Scrivi plugin per estendere Octopus.
---

I plugin ti permettono di estendere Octopus agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Octopus.
...
- `.octopus/plugins/` - plugin a livello progetto
- `~/.config/octopus/plugins/` - plugin globali
...
1. Config globale (`~/.config/octopus/octopus.json`)
2. Config di progetto (`octopus.json`)
3. Directory plugin globale (`~/.config/octopus/plugins/`)
4. Directory plugin di progetto (`.octopus/plugins/`)
...
Octopus esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di Octopus, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
