---
title: Plugin
description: Scrivi plugin per estendere Pencode.
---

I plugin ti permettono di estendere Pencode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Pencode.
...
- `.pencode/plugins/` - plugin a livello progetto
- `~/.config/pencode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/pencode/node_modules/`.
...
1. Config globale (`~/.config/pencode/pencode.json`)
2. Config di progetto (`pencode.json`)
3. Directory plugin globale (`~/.config/pencode/plugins/`)
4. Directory plugin di progetto (`.pencode/plugins/`)
...
Pencode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di pencode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere pencode.
...
:::note
Se usi l'app desktop di Pencode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a pencode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a pencode:
...
L'helper `tool` crea uno strumento personalizzato che pencode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in pencode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://pencode.ai/docs/sdk) per i dettagli.
