...
Pencode Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Pencode. Ti abboni a Pencode Go e
...
Pencode Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

Pencode Go funziona come qualsiasi altro provider in Pencode.

1. Accedi a **<a href={console}>Pencode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Pencode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Pencode Go.
:::
...
Pencode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Pencode config
utilizza il formato `pencode-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`pencode-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Pencode.
