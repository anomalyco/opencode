...
description: Usa la configurazione JSON di Octopus.
---

Puoi configurare Octopus usando un file di configurazione JSON.

---

## Formato

Octopus supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/octopus/octopus.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OCTOPUS_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`octopus.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.opencode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `OCTOPUS_CONFIG_CONTENT`) - sovrascritture a runtime
...
Metti la configurazione globale di Octopus in `~/.config/octopus/octopus.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/octopus/tui.json`.
...
Aggiungi `octopus.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Octopus si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `OCTOPUS_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OCTOPUS_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Usa `OCTOPUS_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `octopus.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Octopus.
...
Puoi configurare provider e modelli da usare in Octopus tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Octopus prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/octopus/agents/` o `.octopus/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Octopus fara fallback su `"build"` mostrando un avviso.
...
Puoi anche definire comandi usando file markdown in `~/.config/octopus/commands/` o `.octopus/commands/`. [Scopri di piu](/docs/commands).
...
Octopus scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono Octopus con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.octopus/plugins/` o `~/.config/octopus/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Octopus a usare solo alcuni provider invece di disabilitarli uno per uno.
