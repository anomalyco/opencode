...
[Scopri di piu sull'uso della TUI](./tui).
...
[Scopri di piu sul server](./server).
...
[Scopri di piu sugli strumenti](./tools).
...
Puoi anche configurare i [modelli locali](./models#local). [Scopri di piu](./models).
...
:::note
I bearer token (`AWS_BEARER_TOKEN_BEDROCK` o `/connect`) hanno precedenza sull'autenticazione basata su profilo. Vedi [ordine di precedenza dell'autenticazione](./providers#authentication-precedence) per i dettagli.
:::

[Scopri di piu sulla configurazione di Amazon Bedrock](./providers#amazon-bedrock).
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.opencode/agents/`. [Scopri di piu](./agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](./agents) che hai definito. Se l'agente specificato non esiste o e un subagent, OpenCode fara fallback su `"build"` mostrando un avviso.
...
Puoi configurare la funzione di [condivisione](./share) tramite l'opzione `share`.
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.opencode/commands/`. [Scopri di piu](./commands).
...
[Scopri di piu sui formatter](./formatters).
...
[Scopri di piu sui permessi](./permissions).
...
[Scopri di piu](./mcp-servers).

---

### Plugin

[I plugin](./plugins) estendono OpenCode con strumenti personalizzati, hook e integrazioni.
