...
2. **Config globale** (`~/.config/leak-code/leak-code.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `LEAKCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`leak-code.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.leak-code`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `LEAKCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.leak-code` e `~/.config/leak-code` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di OpenCode in `~/.config/leak-code/leak-code.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/leak-code/tui.json`.
...
Aggiungi `leak-code.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `LEAKCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `LEAKCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.leak-code` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.leak-code`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `LEAKCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `leak-code.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `leak-code serve` e `leak-code web` tramite l'opzione `server`.
...
Puoi anche definire agenti usando file markdown in `~/.config/leak-code/agents/` o `.leak-code/agents/`. [Scopri di piu](/docs/agents).
...
Questa impostazione si applica a tutte le interfacce: TUI, CLI (`leak-code run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/leak-code/commands/` o `.leak-code/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.leak-code/plugins/` o `~/.config/leak-code/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
