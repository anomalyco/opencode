---
title: Provider
description: Usa qualsiasi provider LLM in xcsh.
---

import config from "../../../../config.mjs"
export const console = config.console

xcsh usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di xcsh.
...
xcsh Zen e una lista di modelli forniti dal team xcsh, testati e
verificati per funzionare bene con xcsh. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da xcsh Zen.
:::

1. Esegui `/connect` nella TUI, seleziona xcsh e vai su [f5xc-salesdemos.github.io/xcsh/auth](https://f5xc-salesdemos.github.io/xcsh/auth).
...
Funziona come qualsiasi altro provider in xcsh ed e totalmente opzionale.
...
Per usare Amazon Bedrock con xcsh:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui xcsh:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `xcsh.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in xcsh non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché xcsh funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di xcsh.
...
salvare il token nello storage di auth di xcsh.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
xcsh usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare xcsh
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `xcsh.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [xcsh-gitlab-auth](https://www.npmjs.com/package/xcsh-gitlab-auth).

##### Configurazione

Personalizza tramite `xcsh.json`:
...
Per usare il tuo abbonamento GitHub Copilot con xcsh:
...
Per usare Google Vertex AI con xcsh:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da xcsh, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [xcsh-helicone-session](https://github.com/H2Shami/xcsh-helicone-session) per loggare automaticamente ogni conversazione di xcsh come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di xcsh elencata come sessione separata.
...
Puoi configurare xcsh per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare xcsh per usare modelli locali tramite LM Studio.
...
Puoi configurare xcsh per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per xcsh. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/xcsh) per dettagli.
:::
...
Per usare Ollama Cloud con xcsh:
...
3. Copia la chiave API da usare in xcsh.
...
6. **Importante**: Prima di usare modelli cloud in xcsh, devi scaricare le informazioni del modello localmente:
...
xcsh Zen è una lista di modelli testati e verificati forniti dal team xcsh. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>xcsh Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **xcsh Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di xcsh.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di xcsh.
...
5. Puoi anche personalizzarli tramite la configurazione di xcsh. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di xcsh. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con xcsh. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `xcsh.json` nella directory del progetto:
...
I campi `limit` permettono a xcsh di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `xcsh auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di xcsh e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config xcsh.
