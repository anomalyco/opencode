---
title: Provider
description: Usa qualsiasi provider LLM in Octopus.
---

import config from "../../../../config.mjs"
export const console = config.console

Octopus usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Octopus.
...
Octopus Zen e una lista di modelli forniti dal team Octopus, testati e
verificati per funzionare bene con Octopus. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Octopus Zen.
:::
...
Funziona come qualsiasi altro provider in Octopus ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Octopus:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `octopus.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Octopus non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Octopus usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Octopus
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `octopus.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `octopus.json`:
...
Per usare Google Vertex AI con Octopus:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Octopus come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Octopus elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per Octopus. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Octopus:
...
3. Copia la chiave API da usare in Octopus.
...
6. **Importante**: Prima di usare modelli cloud in Octopus, devi scaricare le informazioni del modello localmente:
...
Octopus Zen è una lista di modelli testati e verificati forniti dal team Octopus. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Octopus Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Octopus Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Octopus.
...
4. Crea o aggiorna il tuo file `octopus.json` nella directory del progetto:
...
I campi `limit` permettono a Octopus di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
