...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `leak-code.json`:
...
file `leak-code.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `leak-code.json`:
...
4. Crea o aggiorna il tuo file `leak-code.json` nella directory del progetto:
...
1. **Controlla il setup auth**: Esegui `leak-code auth list` per vedere se le credenziali
