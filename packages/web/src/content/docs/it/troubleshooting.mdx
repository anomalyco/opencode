...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
Se il tuo `leak-code.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `LEAKCODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `LEAKCODE_PORT` (o scegli una porta libera) e riavvia.
...
3. Assicurati di avere l'ultima versione con `leak-code upgrade`
...
Per capire a quali modelli hai accesso, esegui `leak-code models`
