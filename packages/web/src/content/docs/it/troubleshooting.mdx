...
Per diagnosticare problemi con Octopus, inizia controllando i log e i dati locali che salva su disco.
...
Octopus Desktop esegue in background un server locale di Octopus (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Octopus` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/octopus/octopus.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\octopus.jsonc`
...
Octopus puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Octopus possa ricostruirla.

1. Chiudi completamente Octopus Desktop.
...
3. Riavvia Octopus Desktop.

---

### Risolvi problemi di connessione al server

Octopus Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `octopus.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OCTOPUS_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OCTOPUS_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Octopus Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Octopus.

---

### Notifiche non visualizzate

Octopus Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Octopus nelle impostazioni del sistema operativo, e
...
1. Chiudi Octopus Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Octopus Desktop):
...
Se riscontri problemi con Octopus:
