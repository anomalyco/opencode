---
title: Supporto ACP
description: Usa Octopus in qualunque editor compatibile con ACP.
---

Octopus supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Octopus via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia Octopus come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Octopus' nel selettore agenti di AI Chat.
...
Per usare Octopus come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Octopus come agente ACP per la chat.

Se devi passare variabili d'ambiente (come `OCTOPUS_API_KEY`), vedi [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) nella documentazione di CodeCompanion.nvim.

## Supporto

Octopus funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Octopus
