---
title: Supporto ACP
description: Usa xcsh in qualunque editor compatibile con ACP.
---

xcsh supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare xcsh via ACP, configura l'editor in modo che esegua il comando `xcsh acp`.

Il comando avvia xcsh come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'xcsh' nel selettore agenti di AI Chat.
...
Per usare xcsh come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare xcsh come agente ACP per la chat.
...
xcsh funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione xcsh
