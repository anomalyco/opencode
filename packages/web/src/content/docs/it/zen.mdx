...
Jev è un modello System One di TypeSafe AI per decisioni rapide e strutturate. Anziché generare testo, valuta uno `state` rispetto a domande tipizzate e restituisce valori e probabilità che il tuo codice può usare direttamente. Supporta domande sì/no (`noul`), a scelta multipla (`choice`) e basate su rubriche di valutazione (`score`).

Usa la tua chiave API di OpenCode Zen con l’endpoint `https://opencode.ai/zen/v1/systemone`. Questo esempio verifica se una richiesta di assistenza è urgente:
...
Puoi porre più domande in un’unica richiesta. Jev le valuta in parallelo e restituisce ogni risposta sotto l’ID della domanda corrispondente:
...
Usa `jev-1.13-free` invece di `jev-1.13` per usare il modello gratuito disponibile per un periodo limitato. Consulta la [documentazione di TypeSafe AI](https://docs.typesafe.ai/) per maggiori dettagli sui tipi di domande e sui campi di risposta.
...
| Jev 1.13 Free                     | Gratis | Gratis  | -           | -            |
| Jev 1.13                          | $0.042 | Gratis  | -           | -            |
