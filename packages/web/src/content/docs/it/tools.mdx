...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. OpenCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](./custom-tools) o con [server MCP](./mcp-servers).

Di default, tutti gli strumenti sono **abilitati** e non richiedono permessi per essere eseguiti. Puoi controllare il comportamento degli strumenti tramite i [permessi](./permissions).
...
[Scopri di piu'](./permissions) su come configurare i permessi.
...
Per configurare quali server LSP sono disponibili per il tuo progetto, vedi [LSP Servers](./lsp).
...
Carica una [skill](./skills) (un file `SKILL.md`) e ne restituisce il contenuto nella conversazione.
...
:::note
Questo strumento e' disabilitato per i subagenti di default, ma puoi abilitarlo manualmente. [Scopri di piu'](./agents/#permissions)
:::
...
[Scopri di piu'](./custom-tools) su come creare strumenti personalizzati.
...
[Scopri di piu'](./mcp-servers) su come configurare i server MCP.
