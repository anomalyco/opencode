...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Octopus include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Octopus.
...
:::note
Questo strumento e' disponibile solo quando `OCTOPUS_EXPERIMENTAL_LSP_TOOL=true` (oppure `OCTOPUS_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Octopus o quando la variabile d'ambiente `OCTOPUS_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Octopus:

```bash
OCTOPUS_ENABLE_EXA=1 opencode
```
