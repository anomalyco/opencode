...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. CyberStrike include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in CyberStrike.
...
:::note
Questo strumento e' disponibile solo quando `CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true` (oppure `CYBERSTRIKE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider CyberStrike o quando la variabile d'ambiente `CYBERSTRIKE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di CyberStrike:
