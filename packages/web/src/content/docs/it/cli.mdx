---
title: CLI
description: Opzioni e comandi della CLI di xcsh.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di xcsh, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
xcsh
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con xcsh in modo programmatico.
...
Avvia l'interfaccia testuale di xcsh (interfaccia utente da terminale).
...
La CLI di xcsh include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per xcsh.
...
Collega un terminale a un backend server di xcsh già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
xcsh attach [url]
```

Questo consente di usare la TUI con un backend xcsh remoto. Per esempio:
...
xcsh si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `xcsh auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/xcsh/auth.json`.

```bash
xcsh auth login
```

Quando xcsh si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in xcsh.
...
Esegui xcsh in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `xcsh serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| `--attach`   |       | Attach a un server xcsh in esecuzione (es. http://localhost:4096)    |
...
Avvia un server xcsh headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
xcsh serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di xcsh senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `xcsh`).
...
Mostra statistiche di utilizzo token e costo per le sessioni xcsh.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione xcsh.

```bash
xcsh import <file>
```

Puoi importare da un file locale o da un URL di condivisione xcsh.
...
Avvia un server xcsh headless con interfaccia web.

```bash
xcsh web
```

Avvia un server HTTP e apre un browser per accedere a xcsh tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `xcsh`).
...
Disinstalla xcsh e rimuove tutti i file correlati.
...
Aggiorna xcsh all'ultima versione o a una versione specifica.
...
La CLI di xcsh accetta i seguenti flag globali.
...
xcsh può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `xcsh`)     |
