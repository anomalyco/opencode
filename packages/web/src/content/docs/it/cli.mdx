---
title: CLI
description: Opzioni e comandi della CLI di Octopus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Octopus, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Octopus in modo programmatico.
...
Avvia l'interfaccia testuale di Octopus (interfaccia utente da terminale).
...
La CLI di Octopus include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Octopus.
...
Collega un terminale a un backend server di Octopus già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend Octopus remoto. Per esempio:
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OCTOPUS_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OCTOPUS_SERVER_USERNAME` o `opencode`) |
...
Octopus si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/octopus/auth.json`.

```bash
opencode auth login
```

Quando Octopus si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Octopus.
...
Avvia un server Octopus headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
opencode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `OCTOPUS_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Elenca tutte le sessioni Octopus.
...
Mostra statistiche di utilizzo token e costo per le sessioni Octopus.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Octopus.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione Octopus.
...
Avvia un server Octopus headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a Octopus tramite interfaccia web. Imposta `OCTOPUS_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla Octopus e rimuove tutti i file correlati.
...
Octopus può essere configurato tramite variabili d'ambiente.

| Variabile                            | Tipo    | Descrizione                                                 |
| ------------------------------------ | ------- | ----------------------------------------------------------- |
| `OCTOPUS_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `OCTOPUS_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `OCTOPUS_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `OCTOPUS_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `OCTOPUS_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `OCTOPUS_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `OCTOPUS_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `OCTOPUS_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `OCTOPUS_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `OCTOPUS_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `OCTOPUS_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `OCTOPUS_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `OCTOPUS_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `OCTOPUS_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `OCTOPUS_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `OCTOPUS_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `OCTOPUS_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `OCTOPUS_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `OCTOPUS_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `OCTOPUS_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `OCTOPUS_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `OCTOPUS_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `OCTOPUS_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `OCTOPUS_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                      | Tipo    | Descrizione                                |
| ---------------------------------------------- | ------- | ------------------------------------------ |
| `OCTOPUS_EXPERIMENTAL`                         | boolean | Abilita tutte le funzionalità sperimentali |
| `OCTOPUS_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                     |
| `OCTOPUS_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI        |
| `OCTOPUS_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms  |
| `OCTOPUS_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM   |
| `OCTOPUS_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir      |
| `OCTOPUS_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                    |
| `OCTOPUS_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale         |
| `OCTOPUS_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                    |
| `OCTOPUS_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali      |
| `OCTOPUS_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python           |
| `OCTOPUS_EXPERIMENTAL_MARKDOWN`                | boolean | Abilita markdown sperimentale              |
| `OCTOPUS_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                          |
