...
La CLI di OpenCode, per impostazione predefinita, avvia la [TUI](./tui) quando viene eseguita senza argomenti.
...
Questo configura il workflow GitHub Actions necessario e ti guida nel processo di configurazione. [Scopri di piu'](./github).
...
E' utile per capire l'esatto nome del modello da usare nella [config](./config/).
...
Avvia un server OpenCode headless per accesso via API. Vedi le [server docs](./server) per l'interfaccia HTTP completa.
