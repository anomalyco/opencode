...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `LEAKCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `LEAKCODE_SERVER_USERNAME` o `opencode`) |
...
OpenCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `leak-code auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/leak-code/auth.json`.
...
Puoi anche collegarti a una istanza `leak-code serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `LEAKCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `LEAKCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `LEAKCODE_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `LEAKCODE_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `LEAKCODE_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `LEAKCODE_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `LEAKCODE_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `LEAKCODE_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `LEAKCODE_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `LEAKCODE_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `LEAKCODE_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `LEAKCODE_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `LEAKCODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `LEAKCODE_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `LEAKCODE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `LEAKCODE_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `LEAKCODE_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `LEAKCODE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `LEAKCODE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `LEAKCODE_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `LEAKCODE_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `LEAKCODE_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `LEAKCODE_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `LEAKCODE_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `LEAKCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `LEAKCODE_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `LEAKCODE_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `LEAKCODE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `LEAKCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `LEAKCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `LEAKCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `LEAKCODE_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `LEAKCODE_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `LEAKCODE_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `LEAKCODE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `LEAKCODE_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `LEAKCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `LEAKCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `LEAKCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `LEAKCODE_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `LEAKCODE_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `LEAKCODE_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `LEAKCODE_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `LEAKCODE_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
