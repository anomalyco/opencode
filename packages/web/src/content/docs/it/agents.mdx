...
In CyberStrike esistono due tipi di agenti: agenti primari e subagenti.
...
CyberStrike include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
CyberStrike include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

CyberStrike include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `cyberstrike.json`:
...
- Globale: `~/.config/cyberstrike/agents/`
- Per progetto: `.cyberstrike/agents/`
...
Se non specifichi una temperature, CyberStrike usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di CyberStrike sia per la config specifica del progetto.
...
L'ID modello nella configurazione CyberStrike usa il formato `provider/model-id`. Per esempio, se stai usando [CyberStrike Zen](/docs/zen), useresti `cyberstrike/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `cyberstrike models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/CyberStrikeus/cyberstrike).
:::
