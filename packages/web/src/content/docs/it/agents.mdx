...
:::tip
Se non specifichi un modello, gli agenti primari usano il [modello configurato globalmente](./config#models), mentre i subagenti useranno il modello dell'agente primario che li ha invocati.
:::
...
L'ID modello nella configurazione OpenCode usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](./zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
