...
In xcsh esistono due tipi di agenti: agenti primari e subagenti.
...
xcsh include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
xcsh include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

xcsh include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `xcsh.json`:
...
- Globale: `~/.config/xcsh/agents/`
- Per progetto: `.xcsh/agents/`
...
Se non specifichi una temperature, xcsh usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di xcsh sia per la config specifica del progetto.
...
L'ID modello nella configurazione xcsh usa il formato `provider/model-id`. Per esempio, se stai usando [xcsh Zen](/docs/zen), useresti `xcsh/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `xcsh models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/f5xc-salesdemos/xcsh).
:::
