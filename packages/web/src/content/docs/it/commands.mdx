...
I comandi personalizzati si aggiungono a quelli integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`. [Scopri di piu](./tui#commands).
...
Usa l'opzione `command` nella [config](./config) di OpenCode:
...
Usa _!`command`_ per inserire nel prompt l'output di un [comando bash](./tui#bash-commands).
...
Usa la config `agent` per specificare opzionalmente quale [agente](./agents) deve eseguire il comando.
Se e un [subagent](./agents/#subagents), il comando attiva per default l'invocazione di un subagent.
...
Usa il booleano `subtask` per forzare il comando ad attivare l'invocazione di un [subagent](./agents/#subagents).
E utile se vuoi evitare che il comando inquini il contesto principale e **forza** l'agente ad agire come subagent,
anche se `mode` e impostato su `primary` nella configurazione dell'[agente](./agents).
...
opencode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](./tui#commands).
