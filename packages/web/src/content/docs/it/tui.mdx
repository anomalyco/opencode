---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CyberStrike.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CyberStrike fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CyberStrike avvia la TUI per la directory corrente.
...
Quando usi la TUI di CyberStrike, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CyberStrike. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CyberStrike. _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di CyberStrike.
