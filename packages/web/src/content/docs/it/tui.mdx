---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Octopus.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Octopus fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Octopus avvia la TUI per la directory corrente.
...
Quando usi la TUI di Octopus, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Octopus. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Octopus. _Alias_: `/quit`, `/q`
...
Questo è separato da `octopus.json`, che configura il comportamento di server/runtime.
...
Usa `OCTOPUS_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
