---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di xcsh.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire xcsh avvia la TUI per la directory corrente.
...
Quando usi la TUI di xcsh, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a xcsh. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `xcsh.json`, che configura il comportamento di server/runtime.
