---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Pencode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Pencode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Pencode avvia la TUI per la directory corrente.
...
Quando usi la TUI di Pencode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Pencode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Pencode. _Alias_: `/quit`, `/q`
...
Questo è separato da `pencode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Pencode attende dopo il leader key. Default: `2000`.
...
Usa `PENCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `pencode.default`.
