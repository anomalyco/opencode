...
Le skill degli agenti permettono a Octopus di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Octopus cerca in queste posizioni:

- Config di progetto: `.octopus/skills/<name>/SKILL.md`
- Config globale: `~/.config/octopus/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Octopus risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.octopus/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.octopus/skills/git-release/SKILL.md` cosi':
...
Octopus elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `octopus.json`:
...
**Per agenti integrati** (in `octopus.json`):
