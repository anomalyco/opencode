...
Le skill degli agenti permettono a CyberStrike di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CyberStrike cerca in queste posizioni:

- Config di progetto: `.cyberstrike/skills/<name>/SKILL.md`
- Config globale: `~/.config/cyberstrike/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CyberStrike risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.cyberstrike/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/cyberstrike/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.cyberstrike/skills/git-release/SKILL.md` cosi':
...
CyberStrike elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `cyberstrike.json`:
...
**Per agenti integrati** (in `cyberstrike.json`):
