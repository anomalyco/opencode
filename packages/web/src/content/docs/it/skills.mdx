...
Le skill degli agenti permettono a Pencode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Pencode cerca in queste posizioni:

- Config di progetto: `.pencode/skills/<name>/SKILL.md`
- Config globale: `~/.config/pencode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Pencode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.pencode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/pencode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.pencode/skills/git-release/SKILL.md` cosi':
...
Pencode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `pencode.json`:
...
**Per agenti integrati** (in `pencode.json`):
