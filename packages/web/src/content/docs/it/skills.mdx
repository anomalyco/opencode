...
Le skill degli agenti permettono a xcsh di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
xcsh cerca in queste posizioni:

- Config di progetto: `.xcsh/skills/<name>/SKILL.md`
- Config globale: `~/.config/xcsh/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, xcsh risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.xcsh/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/xcsh/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.xcsh/skills/git-release/SKILL.md` cosi':
...
xcsh elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `xcsh.json`:
...
**Per agenti integrati** (in `xcsh.json`):
