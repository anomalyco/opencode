---
title: Server
description: Interagisci con il server pencode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `pencode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client pencode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `pencode.local` |
...
Imposta `PENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `pencode`, oppure imposta `PENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `pencode serve` sia per `pencode web`.
...
Quando esegui `pencode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server pencode per interagire con pencode in modo programmatico.
:::

Questa architettura permette a pencode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `pencode serve` per avviare un server standalone. Se la TUI di pencode e gia in esecuzione, `pencode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Pencode.
...
Il server pencode espone le seguenti API.
