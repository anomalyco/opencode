---
title: Server
description: Interagisci con il server xcsh via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `xcsh serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client xcsh.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `xcsh.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `xcsh`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `xcsh serve` sia per `xcsh web`.
...
Quando esegui `xcsh` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server xcsh per interagire con xcsh in modo programmatico.
:::

Questa architettura permette a xcsh di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `xcsh serve` per avviare un server standalone. Se la TUI di xcsh e gia in esecuzione, `xcsh serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di xcsh.
...
Il server xcsh espone le seguenti API.
