...
Il comando `leak-code serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client opencode.
...
Imposta `LEAKCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `opencode`, oppure imposta `LEAKCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `leak-code serve` sia per `leak-code web`.
...
Puoi eseguire `leak-code serve` per avviare un server standalone. Se la TUI di opencode e gia in esecuzione, `leak-code serve` avviera un nuovo server.
