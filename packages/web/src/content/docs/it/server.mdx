---
title: Server
description: Interagisci con il server cyberstrike via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `cyberstrike serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client cyberstrike.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `cyberstrike.local` |
...
Imposta `CYBERSTRIKE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `cyberstrike`, oppure imposta `CYBERSTRIKE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `cyberstrike serve` sia per `cyberstrike web`.
...
Quando esegui `cyberstrike` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server cyberstrike per interagire con cyberstrike in modo programmatico.
:::

Questa architettura permette a cyberstrike di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `cyberstrike serve` per avviare un server standalone. Se la TUI di cyberstrike e gia in esecuzione, `cyberstrike serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di CyberStrike.
...
Il server cyberstrike espone le seguenti API.
