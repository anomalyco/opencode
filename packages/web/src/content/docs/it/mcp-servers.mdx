...
Puoi aggiungere strumenti esterni a Octopus usando il _Model Context Protocol_, o MCP. Octopus supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Octopus Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Octopus gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Octopus:
...
Se il server richiede autenticazione, Octopus ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Octopus memorizzera' i token in modo sicuro in `~/.local/share/octopus/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Octopus insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Octopus come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Octopus al tuo account Sentry.
