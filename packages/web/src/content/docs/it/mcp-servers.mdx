...
Puoi aggiungere strumenti esterni a Pencode usando il _Model Context Protocol_, o MCP. Pencode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Pencode Config](https://pencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/pencode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Pencode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Pencode:
...
Se il server richiede autenticazione, Pencode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `pencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Pencode memorizzera' i token in modo sicuro in `~/.local/share/pencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Pencode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Pencode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Pencode al tuo account Sentry.
