...
Puoi aggiungere strumenti esterni a CyberStrike usando il _Model Context Protocol_, o MCP. CyberStrike supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CyberStrike Config](https://cyberstrike.io/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/cyberstrike`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
CyberStrike gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CyberStrike:
...
Se il server richiede autenticazione, CyberStrike ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `cyberstrike mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CyberStrike memorizzera' i token in modo sicuro in `~/.local/share/cyberstrike/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CyberStrike insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CyberStrike come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CyberStrike al tuo account Sentry.
