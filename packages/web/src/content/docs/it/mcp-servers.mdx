...
Puoi aggiungere strumenti esterni a xcsh usando il _Model Context Protocol_, o MCP. xcsh supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [xcsh Config](https://f5xc-salesdemos.github.io/xcsh/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/xcsh`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
xcsh gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, xcsh:
...
Se il server richiede autenticazione, xcsh ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `xcsh mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, xcsh memorizzera' i token in modo sicuro in `~/.local/share/xcsh/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in xcsh insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione xcsh come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare xcsh al tuo account Sentry.
