...
[**OpenCode**](/) è un agente di programmazione AI open source. E' disponibile come interfaccia per terminale, app desktop o estensione per IDE.
...
Il modo più semplice per installare OpenCode è usare lo script di installazione.
...
- **Con Homebrew su macOS e Linux**
  ...
  > Ti consigliamo di usare il tap di OpenCode per avere le release più aggiornate. La formula ufficiale `brew install opencode` è mantenuta dal team Homebrew e viene aggiornata meno spesso.
...
Il supporto per installare OpenCode su Windows usando Bun è attualmente in lavorazione.
...
Se è la prima volta che usi provider LLM, ti consigliamo [OpenCode Zen](/docs/zen).
...
In alternativa, puoi selezionare uno degli altri provider. [Scopri di più](/docs/providers#directory).
...
Se è la prima volta che usi un agente di programmazione AI, ecco alcuni esempi che possono aiutare.
...
Questo è utile se c'e una parte della codebase su cui non hai lavorato.

---

### Aggiungi funzionalità

Puoi chiedere a OpenCode di aggiungere nuove funzionalita al progetto. Però ti consigliamo prima di chiedergli di creare un piano.
...
2. **Itera sul piano**
   ...
   Una volta che ti da un piano, puoi dargli feedback o aggiungere più dettagli.
...
Per modifiche più semplici, puoi chiedere a OpenCode di implementarle direttamente senza dover prima rivedere un piano.
...
Ma ti accorgi che non è quello che volevi. Puoi **annullare** le modifiche usando il comando `/undo`.
...
:::tip
Puoi eseguire `/undo` più volte per annullare più modifiche.
:::
...
Questo creera un link alla conversazione corrente e lo copierà negli appunti.
