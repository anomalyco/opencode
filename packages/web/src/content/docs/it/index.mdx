...
Per la migliore esperienza su Windows, ti consigliamo di usare [Windows Subsystem for Linux (WSL)](./windows-wsl). Offre prestazioni migliori e piena compatibilita con le funzionalita di OpenCode.
:::
...
Se e la prima volta che usi provider LLM, ti consigliamo [OpenCode Zen](./zen).
...
In alternativa, puoi selezionare uno degli altri provider. [Scopri di piu](./providers#directory).
...
Le conversazioni che fai con OpenCode possono essere [condivise con il tuo team](./share).
...
Per renderlo davvero tuo, ti consigliamo di [scegliere un tema](./themes), [personalizzare i tasti rapidi](./keybinds), [configurare i formatter](./formatters), [creare comandi personalizzati](./commands) o sperimentare con la [configurazione di OpenCode](./config).
