...
Per impostare esplicitamente la modalita manuale nel tuo [file di config](./config):
...
Puoi abilitare la condivisione automatica per tutte le nuove conversazioni impostando l'opzione `share` su `"auto"` nel tuo [file di config](./config):
...
Puoi disabilitare completamente la condivisione impostando l'opzione `share` su `"disabled"` nel tuo [file di config](./config):
...
[Scopri di piu](./enterprise) su come usare opencode nella tua organizzazione.
