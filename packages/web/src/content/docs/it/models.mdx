...
[Scopri di piu](./providers) sui provider.
...
Qui l'ID completo e `provider_id/model_id`. Per esempio, se usi [OpenCode Zen](./zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.

Se hai configurato un [provider personalizzato](./providers#custom), `provider_id` e la chiave nella sezione `provider` della config e `model_id` e la chiave in `provider.models`.
...
Puoi anche configurare queste opzioni per gli agenti che usi. La config dell'agente sovrascrive le opzioni globali definite qui. [Scopri di piu](./agents/#additional).
...
Usa il keybind `variant_cycle` per passare rapidamente tra le varianti. [Scopri di piu](./keybinds).
