...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di cyberstrike. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in cyberstrike ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di cyberstrike.
...
cyberstrike include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.cyberstrike/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `cyberstrike.json`:
...
- Globale: `~/.config/cyberstrike/modes/`
- Progetto: `.cyberstrike/modes/`
...
Se non viene specificata alcuna temperatura, cyberstrike usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di cyberstrike sia per quella specifica del progetto.
...
Crea file di modalita in `.cyberstrike/modes/` per modalita specifiche del progetto oppure in `~/.config/cyberstrike/modes/` per modalita globali:
