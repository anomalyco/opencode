...
Puoi anche avere regole globali in un file `~/.config/octopus/AGENTS.md`. Queste si applicano a tutte le sessioni di opencode.
...
Per gli utenti che migrano da Claude Code, Octopus supporta come fallback le convenzioni di file di Claude Code:

- **Regole di progetto**: `CLAUDE.md` nella directory del progetto (usato se non esiste `AGENTS.md`)
- **Regole globali**: `~/.claude/CLAUDE.md` (usato se non esiste `~/.config/octopus/AGENTS.md`)
...
2. **File globale** in `~/.config/octopus/AGENTS.md`
3. **File Claude Code** in `~/.claude/CLAUDE.md` (se non disabilitato)

In ogni categoria vince il primo file corrispondente. Per esempio, se hai sia `AGENTS.md` sia `CLAUDE.md`, viene usato solo `AGENTS.md`. Allo stesso modo, `~/.config/octopus/AGENTS.md` ha priorita' su `~/.claude/CLAUDE.md`.

---

## Istruzioni personalizzate

Puoi specificare file di istruzioni personalizzati nel tuo `octopus.json` o nel globale `~/.config/octopus/octopus.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
L'approccio consigliato e' usare il campo `instructions` in `octopus.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `octopus.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
