...
:::tip[Senza apertura automatica]
Per avviare il server web senza aprire automaticamente il browser, usa `opencode serve` al suo posto:

```bash
opencode serve --hostname 0.0.0.0 --port 4096
```

Poi naviga manualmente su `http://localhost:4096` (o il tuo indirizzo di rete). Quando usi `0.0.0.0`, imposta `OPENCODE_SERVER_PASSWORD` per proteggere l'accesso.
:::
