...
:::caution
Se `LEAKCODE_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::

:::tip[Utenti Windows]
Per la migliore esperienza, esegui `leak-code web` da [WSL](/docs/windows-wsl) invece che da PowerShell. Questo garantisce accesso corretto al file system e integrazione col terminale.
:::
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `LEAKCODE_SERVER_PASSWORD`:

```bash
LEAKCODE_SERVER_PASSWORD=secret leak-code web
```

Lo username predefinito e `opencode`, ma puo essere cambiato con `LEAKCODE_SERVER_USERNAME`.
...
Puoi anche configurare le impostazioni del server nel file di config `leak-code.json`:
