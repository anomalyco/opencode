...
:::caution
Se `IMECODE_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `IMECODE_SERVER_PASSWORD`:

```bash
IMECODE_SERVER_PASSWORD=secret opencode web
```

Lo username predefinito e `opencode`, ma puo essere cambiato con `IMECODE_SERVER_USERNAME`.
