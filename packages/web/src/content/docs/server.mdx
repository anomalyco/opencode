---
title: Server
description: Interact with xcsh server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `xcsh serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an xcsh client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `xcsh`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `xcsh serve` and `xcsh web`.
...
When you run `xcsh` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the xcsh server to interact with xcsh programmatically.
:::

This architecture lets xcsh support multiple clients and allows you to interact with xcsh programmatically.

You can run `xcsh serve` to start a standalone server. If you have the
xcsh TUI running, `xcsh serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the xcsh [IDE](/docs/ide) plugins.
...
The xcsh server exposes the following APIs.
