---
title: Server
description: Interact with leak-code server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `leak-code serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that a leak-code client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `leak-code.local` |
...
Set `LEAKCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `leak-code`, or set `LEAKCODE_SERVER_USERNAME` to override it. This applies to both `leak-code serve` and `leak-code web`.
...
When you run `leak-code` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the leak-code server to interact with leak-code programmatically.
:::

This architecture lets leak-code support multiple clients and allows you to interact with leak-code programmatically.

You can run `leak-code serve` to start a standalone server. If you have the
leak-code TUI running, `leak-code serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Leak Code [IDE](/docs/ide) plugins.
...
The leak-code server exposes the following APIs.
