...
Set `OPENCODE_WEB_UI_TITLE` to change the title of the served web UI. This is useful when running multiple instances, since every browser tab would otherwise be titled `OpenCode`.

```bash
OPENCODE_WEB_UI_TITLE="work: frontend" opencode serve
```
