---
title: Server
description: Interact with pencode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `pencode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an pencode client can use.
...
Set `PENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `pencode`, or set `PENCODE_SERVER_USERNAME` to override it. This applies to both `pencode serve` and `pencode web`.
...
When you run `pencode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the pencode server to interact with pencode programmatically.
:::

This architecture lets pencode support multiple clients and allows you to interact with pencode programmatically.

You can run `pencode serve` to start a standalone server. If you have the
pencode TUI running, `pencode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Pencode [IDE](/docs/ide) plugins.
...
The pencode server exposes the following APIs.
