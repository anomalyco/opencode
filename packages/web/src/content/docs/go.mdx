...
2. You run the `/connect` command in the TUI, select `OpenCode Go`, and paste
   your API key.
3. **Enable the models in your workspace.** Go to Zen and ensure the Go models (GLM-5, Kimi K2.5, MiniMax M2.5) are enabled.
4. Run `/models` in the TUI to see the list of models available through Go.
