...
The following models are deprecated but still functional. They are hidden by default but can be shown by setting the `OPENCODE_SHOW_DEPRECATED_MODELS` environment variable to `true`:

- **Qwen3.5 Plus** (deprecated)
- **Qwen3.6 Plus** (deprecated)

To use deprecated models, run OpenCode with the flag:

```bash
OPENCODE_SHOW_DEPRECATED_MODELS=true opencode
```

When enabled, deprecated models will appear in the model list with a "(deprecated)" label.
...
¹ Deprecated - requires `OPENCODE_SHOW_DEPRECATED_MODELS=true`
