...
xcsh Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
xcsh Go is currently in beta.
:::

Go works like any other provider in xcsh. You subscribe to xcsh Go and
...
:::tip
We tested a select group of models and providers that work well with xcsh.
:::
...
xcsh Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

xcsh Go works like any other provider in xcsh.

1. You sign in to **<a href={console}>xcsh Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `xcsh Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to xcsh Go.
:::
...
xcsh Go includes the following limits:
...
The [model id](/docs/config/#models) in your xcsh config
uses the format `xcsh-go/<model-id>`. For example, for Kimi K2.5, you would
use `xcsh-go/kimi-k2.5` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with xcsh as well.
