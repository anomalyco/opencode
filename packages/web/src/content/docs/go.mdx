...
- **GLM models†:** The `Not used` / `0 days` values above describe the OpenCode Go route, not a universal provider-wide default. OpenCode does not publish the underlying provider arrangement in this table. Check the provider's current [privacy policy](https://docs.bigmodel.cn/cn/terms/privacy-policy) and [user agreement](https://docs.bigmodel.cn/cn/terms/user-agreement), and verify any current Go-specific terms before sending sensitive data.
