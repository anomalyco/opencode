...
Octopus Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
Octopus Go is currently in beta.
:::

Go works like any other provider in Octopus. You subscribe to Octopus Go and
...
:::tip
We tested a select group of models and providers that work well with Octopus.
:::
...
Octopus Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Octopus Go works like any other provider in Octopus.

1. You sign in to **<a href={console}>Octopus Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Octopus Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Octopus Go.
:::
...
Octopus Go includes the following limits:
...
The [model id](/docs/config/#models) in your Octopus config
...
4. Have **no lock-in** by allowing you to use any other provider with Octopus as well.
