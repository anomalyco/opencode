...
Go works like any other provider in Leak Code. You subscribe to OpenCode Go and
...
:::tip
We tested a select group of models and providers that work well with Leak Code.
:::
...
OpenCode Go works like any other provider in Leak Code.
...
The [model id](/docs/config/#models) in your Leak Code config
...
4. Have **no lock-in** by allowing you to use any other provider with Leak Code as well.
