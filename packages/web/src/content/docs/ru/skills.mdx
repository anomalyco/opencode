...
- Конфигурация проекта: `.leak-code/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/leak-code/skills/<name>/SKILL.md`.
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.leak-code/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/leak-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.leak-code/skills/git-release/SKILL.md` следующим образом:
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `leak-code.json`:
...
**Для встроенных агентов** (в формате `leak-code.json`):
