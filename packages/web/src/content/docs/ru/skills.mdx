...
| ------------- | -------- | -------- | ------------------------------------------------------------------------------------------------- |
| `name`        | `string` | yes      | Должен содержать 1–64 символа.                                                                   |
| `description` | `string` | yes      | Должен содержать 1–1024 символа. Отображается агенту, чтобы он мог решить, когда загрузить навык. |

Все остальные поля заголовка безопасно игнорируются.

:::tip
Делайте описание достаточно конкретным, чтобы агент мог сделать правильный выбор.
:::
