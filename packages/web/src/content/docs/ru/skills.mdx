...
- Конфигурация проекта: `.octopus/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/octopus/skills/<name>/SKILL.md`.
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.octopus/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/octopus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.octopus/skills/git-release/SKILL.md` следующим образом:
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `octopus.json`:
...
**Для встроенных агентов** (в формате `octopus.json`):
