...
в `~/.local/share/leak-code/auth.json`.
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `leak-code.json`:
...
`leak-code.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Настройте через `leak-code.json`:
...
4. Создайте или обновите файл `leak-code.json` в каталоге вашего проекта:
...
1. **Проверьте настройку аутентификации**: запустите `leak-code auth list`, чтобы проверить, верны ли учетные данные.
