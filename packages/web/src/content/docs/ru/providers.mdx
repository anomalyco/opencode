...
в `~/.local/share/octopus/auth.json`.
...
Octopus Zen — это список моделей, предоставленный командой opencode, которые были
протестировано и проверено на хорошую работу с opencode. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с Octopus Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `Octopus Zen` и перейдите по адресу [opencode.ai/auth](https://opencode.ai/zen).
...
Octopus Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой opencode, которые были
протестированы и проверены на хорошую работу с opencode.

1. Запустите команду `/connect` в TUI, выберите `Octopus Go` и перейдите по адресу [opencode.ai/auth](https://opencode.ai/zen).
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `octopus.json`:
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Octopus`, Области применения: `api`
...
`octopus.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Octopus Zen — это список протестированных и проверенных моделей, предоставленный командой opencode. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>Octopus Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **Octopus Zen**.
...
4. Создайте или обновите файл `octopus.json` в каталоге вашего проекта:
