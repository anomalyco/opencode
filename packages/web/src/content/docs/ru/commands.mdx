...
Пользовательские команды дополняют встроенные команды, такие как `/init`, `/undo`, `/redo`, `/share`, `/help`. [Подробнее](./tui#commands).
...
Используйте опцию `command` в вашем OpenCode [config](./config):
...
Используйте _!`command`_, чтобы ввести вывод команды bash](./tui#bash-commands) в приглашение.
...
Используйте конфигурацию `agent`, чтобы дополнительно указать, какой [агент](./agents) должен выполнить эту команду.
Если это [subagent](./agents/#subagents), команда по умолчанию инициирует вызов субагента.
...
Используйте логическое значение `subtask`, чтобы заставить команду инициировать вызов [subagent](./agents/#subagents).
...
даже если для `mode` установлено значение `primary` в конфигурации [agent](./agents).
...
открытый код включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](./tui#commands).
