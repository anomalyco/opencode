...
Создайте `.xcsh/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию xcsh или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем xcsh [config](/docs/config):
...
- Глобальный: `~/.config/xcsh/commands/`
- Для каждого проекта: `.xcsh/commands/`
...
xcsh включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
