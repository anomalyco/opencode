...
Создайте `.cyberstrike/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию cyberstrike или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем cyberstrike [config](/docs/config):
...
- Глобальный: `~/.config/cyberstrike/commands/`
- Для каждого проекта: `.cyberstrike/commands/`
...
cyberstrike включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
