...
`OPENCODE_CONFIG_DIRS` может задавать несколько пользовательских каталогов конфигурации, используя тот же разделитель, что и системный `PATH` (`:` в Linux/macOS, `;` в Windows). Каталоги загружаются слева направо, поэтому более поздние переопределяют более ранние. Если также задан `OPENCODE_CONFIG_DIR`, он добавляется последним и становится пользовательским каталогом с наивысшим приоритетом и глобальной целью для записи.
