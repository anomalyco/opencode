...
5. **Конфигурация `.opencode`** (`.opencode/opencode.json` в проекте) — то же, что конфигурация проекта, но отдельно
6. **Каталоги `.opencode`** — агенты, команды, плагины
7. **Встроенная конфигурация** (`OPENCODE_CONFIG_CONTENT` env var) – переопределяет время выполнения
