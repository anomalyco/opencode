...
Jev — это модель System One от TypeSafe AI для быстрых структурированных решений. Вместо генерации текста она оценивает `state` по типизированным вопросам и возвращает значения и вероятности, которые ваш код может использовать напрямую. Она поддерживает вопросы «да/нет» (`noul`), вопросы с несколькими вариантами ответа (`choice`) и вопросы с оценкой по критериям (`score`).

Используйте ключ API OpenCode Zen с конечной точкой `https://opencode.ai/zen/v1/systemone`. В этом примере проверяется, является ли запрос в службу поддержки срочным:
...
В одном запросе можно задать несколько вопросов. Jev оценивает их параллельно и возвращает каждый ответ под соответствующим идентификатором вопроса:
...
Чтобы использовать бесплатную модель, доступную ограниченное время, укажите `jev-1.13-free` вместо `jev-1.13`. Подробную информацию о типах вопросов и полях ответа см. в [документации TypeSafe AI](https://docs.typesafe.ai/).
...
| Jev 1.13 Free                     | Бесплатно | Бесплатно | -           | -            |
| Jev 1.13                          | $0.042    | Бесплатно | -           | -            |
...
- MiMo-V2.6-Flash Free доступна в OpenCode ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
...
- MiMo-V2.6-Flash Free: во время бесплатного периода собранные данные могут использоваться для улучшения модели.
