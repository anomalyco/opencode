...
:::caution
При использовании `--hostname 0.0.0.0` задайте `LEAKCODE_SERVER_PASSWORD`, чтобы защитить сервер.
...
1. **Запускайте `leak-code web` в терминале WSL**, а не в PowerShell:
...
Запуск `leak-code web` из WSL обеспечивает корректный доступ к файловой системе и интеграцию с терминалом, при этом интерфейс остается доступным из браузера Windows.
...
- Конфигурация и сессии opencode хранятся в среде WSL по пути `~/.local/share/leak-code/`
