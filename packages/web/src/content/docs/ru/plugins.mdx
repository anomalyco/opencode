...
Для примера ознакомьтесь с [plugins](./ecosystem#plugins), созданными сообществом.
...
Просмотрите доступные плагины в папке [ecosystem](./ecosystem#plugins).
...
**Локальные плагины** загружаются непосредственно из каталога плагинов. Чтобы использовать внешние пакеты, вы должны создать `package.json` в своем каталоге конфигурации (см. [Зависимости](#dependencies)) или опубликовать плагин в npm и [добавить его в свой config](./config#plugins).
