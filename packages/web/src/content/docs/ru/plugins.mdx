...
- `.leak-code/plugins/` – плагины уровня проекта.
- `~/.config/leak-code/plugins/` — глобальные плагины
...
1. Глобальная конфигурация (`~/.config/leak-code/leak-code.json`)
2. Конфигурация проекта (`leak-code.json`)
3. Глобальный каталог плагинов (`~/.config/leak-code/plugins/`)
4. Каталог плагинов проекта (`.leak-code/plugins/`)
