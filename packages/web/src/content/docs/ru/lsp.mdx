---
title: LSP-серверы
description: cyberstrike интегрируется с вашими серверами LSP.
---

cyberstrike интегрируется с вашим протоколом языкового сервера (LSP), чтобы помочь LLM взаимодействовать с вашей кодовой базой. Он использует диагностику для предоставления обратной связи LLM.

---

## Встроенные

cyberstrike поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `CYBERSTRIKE_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда cyberstrike открывает файл, он:
...
Вы можете настроить серверы LSP через раздел `lsp` в конфигурации cyberstrike.
...
```json title="cyberstrike.json" {5-7}
{
  "$schema": "https://cyberstrike.io/config.json",
  "lsp": {
    "rust": {
...
```json title="cyberstrike.json" {5-9}
{
  "$schema": "https://cyberstrike.io/config.json",
  "lsp": {
    "typescript": {
