...
- **macOS/Linux**: `~/.config/leak-code/leak-code.jsonc`（或 `~/.config/leak-code/leak-code.json`）
- **macOS/Linux**（旧版安装）: `~/.local/share/leak-code/leak-code.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\leak-code.jsonc`
...
如果你的 `leak-code.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。

#### 检查环境变量

如果你在环境中设置了 `LEAKCODE_PORT`，桌面应用将尝试使用该端口作为本地服务器端口。

- 取消设置 `LEAKCODE_PORT`（或选择一个空闲端口）并重新启动。
...
3. 使用 `leak-code upgrade` 确保你使用的是最新版本
...
要查看你有权访问哪些模型，请运行 `leak-code models`
