...
If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](./windows-wsl). WSL provides a Linux environment that works more seamlessly with OpenCode's features.
