...
为了在 Windows 上获得最佳体验，我们建议使用[适用于 Linux 的 Windows 子系统 (WSL)](./windows-wsl)。它提供了更好的性能并与 OpenCode 的功能完全兼容。
:::
...
如果您不熟悉使用 LLM 成功，我们建议使用[开放代码禅](./zen)。
...
或者，您可以选择其他巨头之一。 [了解更多](./providers#directory)。
...
要让您成为自己的，我们建议[选择一个主题](./themes)、[自定义交互绑定](./keybinds)、[配置代码整理程序](./formatters)、[创建自定义命令](./commands)或使用[开放代码配置](./config)。
