...
:::note
只有当 CYBERSTRIKE_EXPERIMENTAL_LSP_TOOL=true（或 CYBERSTRIKE_EXPERIMENTAL=true）时，此工具才可用。
:::
...
:::note
只有在使用 CyberStrike 提供程序时，或者当 CYBERSTRIKE_ENABLE_EXA 环境变量被设置为任何真值（例如 true 或 1）时，此工具才可用。
