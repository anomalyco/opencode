...
在您的程式码库中快速进行内容搜索。支持完整的正规表示式语法和文件模式过滤。
...
使用 `**/*.js` 或 `src/**/*.ts` 等全域性模式搜索文件。返回按时间排序的匹配文件路径修改。
...
该工具将补丁文件应用到您的程式码库。对于应用来自各种来源的差异和补丁很有帮助。
...
每个问题都包含标题、问题文字和选项列表。用户可以从提供的选项中进行选择或输入自定义答案。当存在多个问题时，用户可以在提交所有答案之前在这些问题之间导航。
...
要包含通常会被忽略的文件，请在项目根目录中建立 `.ignore` 文件。该文件可以明确允许某些路径。
