...
了解更多关于[提供商](/docs/zh-cn/providers)的信息。
...
这里完整的 ID 格式为 `provider_id/model_id`。例如，如果你使用 [OpenCode Zen](/docs/zh-cn/zen)，则 GPT 5.1 Codex 对应的值为 `opencode/gpt-5.1-codex`。

如果你配置了[自定义提供商](/docs/zh-cn/providers#custom)，`provider_id` 是配置中 `provider` 部分的键名，`model_id` 是 `provider.models` 中的键名。
...
你还可以为使用中的任何代理配置这些选项。代理配置会覆盖此处的全局选项。[了解更多](/docs/zh-cn/agents/#additional)。
...
使用快捷键 `variant_cycle` 可以快速在变体之间切换。[了解更多](/docs/zh-cn/keybinds)。
