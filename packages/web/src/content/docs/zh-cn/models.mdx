...
这里完整的ID是`provider_id/model_id`。例如，如果您使用[开放代码禅](./zen)，则您将使用`opencode/gpt-5.1-codex`来表示GPT 5.1 Codex。

如果您配置了[定制生产](./providers#custom)，则`provider_id` 是配​​置中`provider` 部分的按键，`model_id` 是`provider.models` 中的按键。
...
您还可以为您正在使用的任何代理配置这些选项。代理配置会覆盖此处的所有全局选项。 [了解更多](./agents/#additional)。
...
使用按键绑定`variant_cycle`在变体之间快速切换。 [了解更多](./keybinds)。
