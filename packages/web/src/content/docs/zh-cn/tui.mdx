...
大多数命令还支持以 `ctrl+x` 作为前导键的快捷键，其中 `ctrl+x` 是默认前导键。[了解更多](/docs/zh-cn/keybinds)。
...
创建或更新 `AGENTS.md` 文件。[了解更多](/docs/zh-cn/rules)。
...
取消分享当前会话。[了解更多](/docs/zh-cn/share#un-sharing)。
...
- `theme` - 设置 UI 主题。[了解更多](/docs/zh-cn/themes)。
- `keybinds` - 自定义键盘快捷键。[了解更多](/docs/zh-cn/keybinds)。
