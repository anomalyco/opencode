...
本地配置值会覆盖远程默认值。详情请参阅[配置优先级](/docs/zh-cn/config#precedence-order)。
...
1. 全局禁用该工具。
2. 在[代理配置](/docs/zh-cn/agents#tools)中，将 MCP 服务器作为工具启用。
...
你也可以在 [AGENTS.md](/docs/zh-cn/rules/) 中添加类似的规则。
