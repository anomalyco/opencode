...
您可以通过将 [config file](./config) 中的 `share` 选项设置为 `"auto"` 来为所有新对话启用自动共享：
...
您可以通过将 [config file](./config) 中的 `share` 选项设置为 `"disabled"` 来完全禁用共享：
...
[了解更多关于在您的组织中使用开放代码的](./enterprise)。
