...
:::提示
如果您不指定模型，主代理将使用[全局配置的模型](./config#models)，而子代理将使用调用子代理的主代理的模型。
:::
...
OpenCode配置中的模型ID使用格式`provider/model-id`。例如，如果您使用[开放代码禅](./zen)，则您将使用`opencode/gpt-5.1-codex`来表示GPT 5.1 Codex。
