...
除了`/init`、`/undo`、`/redo`、`/share`、`/help`等内置命令之外，还有自定义命令。 [了解更多](./tui#commands)。
...
在 OpenCode [配置](./config) 中使用 `command` 选项：
...
使用 _!`command`_ 将 [bash命令](./tui#bash-commands) 输出注入到提示符中。
...
使用`agent`配置选择指定哪个[代理人](./agents)应执行此命令。
如果是 [子代理](./agents/#subagents) 该命令将默认触发子代理调用。
...
使用`subtask`布尔值强制命令触发[子代理](./agents/#subagents)调用。
如果您希望命令不污染您的主要上下文並且將**強制**代理充當子代理，那麼這非常有用，
即使`mode`在[代理人](./agents)配置上设置为`primary`。
...
opencode包含`/init`、`/undo`、`/redo`、`/share`、`/help`等内置命令；【了解更多](./tui#commands)。
