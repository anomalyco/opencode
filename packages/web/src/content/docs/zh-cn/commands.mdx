...
使用 `agent` 配置可选地指定由哪个[智能体](/docs/agents)执行此命令。
如果这是一个[子智能体](/docs/agents/#subagents)，该命令默认会触发子智能体调用。
...
这是一个**可选的**配置选项。如果未指定，默认使用你当前的智能体。
...
如果你希望命令不污染主要上下文，这会很有用，它会**强制**智能体作为子智能体运行，
即使[智能体](/docs/agents)配置中的 `mode` 设置为 `primary`。
