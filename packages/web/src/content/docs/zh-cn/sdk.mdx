...
[了解更多关于服务器如何工作的](./server)。例如，检视社区构建的[projects](./ecosystem#projects)。
...
您可以向自定义行为传递配置对象。该实例项仍然会选择您的 `___W0___.json`，但您可以覆盖或添加内联配置：
...
| `session.init({ path, body })` | 分析应用方案并建立 `___W0___` | 返回 `boolean` |
