...
OpenCode Go 包含以下基础限制：

- **5 小时限制** — 20% 的使用配额
- **每周限制** — 50% 的使用配额
- **每月限制** — 100% 的使用配额

所有模型共享使用配额，以上限制以该配额的百分比表示。

### 各模型的使用额度

各模型包含的每月使用额度不同，按官方 API 价格折算。Token 价格按每 1M tokens 列示。

| 模型                                    | 输入   | 输出   | 缓存读取  | 缓存写入 | 每月使用额度 |
| --------------------------------------- | ------ | ------ | --------- | -------- | -------- |
...
可用请求数取决于模型的每月使用额度和 API 价格。以下预估基于典型的 Go 使用模式。
...
- Omen Alpha — 每次请求 300 个输入 token，40,000 个缓存 token，100 个输出 token
...
- **DeepSeek V4 Flash:** ZDR 协议每月续签。当前协议有效期至 2026 年 9 月 30 日。
