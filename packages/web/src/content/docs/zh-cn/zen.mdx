...
在你的 OpenCode 配置中，[模型 ID](/docs/zh-cn/config/#models) 使用 `opencode/<model-id>` 格式。例如，对于 GPT 5.5，你需要在配置中使用 `opencode/gpt-5.5`。
...
你可能会在使用记录中看到 _Claude Haiku 3.5_。这是一个[低成本模型](/docs/zh-cn/config/#models)，用于生成会话标题。
