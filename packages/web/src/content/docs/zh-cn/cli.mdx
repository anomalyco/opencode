...
默认情况下，OpenCode CLI 在不带任何参数运行时启动[途径易](./tui)。
...
这将设置必要的 GitHub Actions 工作流程并指导您完成配置过程。 [了解更多](./github)。
...
这对于确定[你的配置](./config/)中使用的意图模型名称很有用。
...
启动无头OpenCode服务器以进行API访问。查看[服务器文档](./server)以获取完整的HTTP 接口。
