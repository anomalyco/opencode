...
`___W0___ serve` 命令执行无头 HTTP 服务器，该服务器公开 opencode 客户端可以使用的 OpenAPI 端点。
...
| `--mdns-domain` | mDNS 服务的自定义域名 | `___W0___.local` |
| `--cors`        | 允许的其他浏览器来源  | `[]`             |
...
设置 `OPENCODE_SERVER_PASSWORD` 以使用 HTTP 基本身份验证保护服务器。用户默认名称为 `___W0___`，或设置 `OPENCODE_SERVER_USERNAME` 覆盖它。这适用于 `___W0___ serve` 和 `___W0___ web`。
...
当您执行 `___W0___` 时，它会启动 TUI 和服务器。其中 TUI 是
...
您执行可以 `___W0___ serve` 来启动独立服务器。如果您有
opencode TUI 执行， `___W0___ serve` 将启动一个新服务器。

---

#### 連線到現有伺服器

当您启动 TUI 时，它会随机分配端口和主机名。您可以重新设置 `--hostname` 和 `--port` [flags](./cli)。使用它连线到其服务器然后器。

[**_T2_**](#tui) 端点可用于跨境服务器驱动 TUI。例如，您可以预填充或执行提示。此设置由 OpenCode [IDE](./ide) 外挂使用。
...
| `POST`   | `/session/:id/init`                      | 分析应用程序并建立 `___W0___` | 正文：`{ messageID, providerID, modelID }`，返回 `boolean`                       |
