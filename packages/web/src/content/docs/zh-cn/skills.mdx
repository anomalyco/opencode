...
智能体技能让 OpenCode 能够从你的仓库或主目录中发现可复用的指令。
技能通过原生的 `skill` 工具按需加载——智能体可以查看可用技能，并在需要时加载完整内容。
...
在 `opencode.json` 中使用基于模式的权限来控制智能体可以访问哪些技能：
