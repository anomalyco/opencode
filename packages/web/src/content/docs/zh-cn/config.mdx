...
使用 `OPENCODE_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.opencode` 目录一样被搜索智能体、命令、模式和插件，并且应遵循相同的结构。
...
您还可以使用 `~/.config/opencode/agents/` 或 `.opencode/agents/` 中的 Markdown 文件定义智能体。[在此了解更多](/docs/agents)。

---

### 默认智能体

您可以使用 `default_agent` 选项设置默认智能体。当未明确指定智能体时，将使用该默认智能体。
...
默认智能体必须是主智能体（不能是子智能体）。可以是内置智能体（如 `"build"` 或 `"plan"`），也可以是您定义的[自定义智能体](/docs/agents)。如果指定的智能体不存在或是子智能体，OpenCode 将回退到 `"build"` 并发出警告。
