...
您还可以配置[本地模型](./models#local)。[了解更多](./models)。
...
：：：笔记
承载令牌（`AWS_BEARER_TOKEN_BEDROCK` 或`/connect`）优先于基于配置文件的身份验证。详情请参见【认证优先级](./providers#authentication-precedence)。
:::

[了解有关 Amazon Bedrock 配置的更多信息](./providers#amazon-bedrock)。
...
您还可以使用 `~/.config/opencode/agents/` 或 `.opencode/agents/` 中的 markdown 文件定义代理。 [在这里了解更多](./agents)。
...
您还可以使用 `~/.config/opencode/commands/` 或 `.opencode/commands/` 中的 Markdown 文件定义命令。 [在这里了解更多](./commands)。
...
[插件](./plugins) 使用自定义工具、挂钩和集成扩展 OpenCode。
