...
3. **自定义配置**（`IMECODE_CONFIG` 环境变量）- 自定义覆盖
4. **项目配置**（项目中的 `opencode.json`）- 项目特定设置
5. **`.opencode` 目录** - 代理、命令、插件
6. **内联配置**（`IMECODE_CONFIG_CONTENT` 环境变量）- 运行时覆盖
...
使用 `IMECODE_CONFIG` 环境变量指定自定义配置文件路径。
...
使用 `IMECODE_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.opencode` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
