---
title: 插件
description: 编写自己的插件来扩展 Octopus。
---

插件允许你通过挂钩各种事件和自定义行为来扩展 Octopus。你可以创建插件来添加新功能、集成外部服务，或修改 Octopus 的默认行为。
...
- `.octopus/plugins/` - 项目级插件
- `~/.config/octopus/plugins/` - 全局插件
...
1. 全局配置 (`~/.config/octopus/octopus.json`)
2. 项目配置 (`octopus.json`)
3. 全局插件目录 (`~/.config/octopus/plugins/`)
4. 项目插件目录 (`.octopus/plugins/`)
...
Octopus 会在启动时运行 `bun install` 来安装这些依赖项。之后你的插件和工具就可以导入它们了。
...
- `directory`：当前工作目录。
- `worktree`：git 工作树路径。
- `client`：用于与 AI 交互的 Octopus SDK 客户端。
...
:::note
如果你使用 Octopus 桌面应用，它可以在响应就绪或会话出错时自动发送系统通知。
:::
...
`tool` 辅助函数用于创建 Octopus 可调用的自定义工具。它接受一个 Zod schema 函数，并返回一个工具定义，包含：

- `description`：工具的功能描述
- `args`：工具参数的 Zod schema
- `execute`：工具被调用时执行的函数

你的自定义工具将与内置工具一起在 Octopus 中可用。
