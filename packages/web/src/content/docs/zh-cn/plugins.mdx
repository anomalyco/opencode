...
如需了解示例，请查看社区创建的[插件](/docs/zh-cn/ecosystem#plugins)。
...
浏览[生态系统](/docs/zh-cn/ecosystem#plugins)中的可用插件。
...
**本地插件**直接从插件目录加载。如果需要使用外部包，你必须在配置目录中创建 `package.json`（参见[依赖项](#dependencies)），或者将插件发布到 npm 并[将其添加到配置中](/docs/zh-cn/config#plugins)。
