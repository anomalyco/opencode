...
If you use Claude Code's `CLAUDE_CONFIG_DIR` environment variable to relocate its config directory, OpenCode respects it and reads `CLAUDE.md` and `skills/` from there instead of `~/.claude`.
