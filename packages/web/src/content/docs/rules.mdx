...
Hierarchical files and custom instructions support YAML frontmatter to further restrict their application:
...
Rules without `paths` frontmatter are considered global and are included in the initial system prompt. Rules with `paths` are dynamically injected when a matching file is read into the session.

---

## Command Injection

You can inject the output of shell commands directly into your instructions using the `!` followed by backticks syntax. This is useful for including dynamic information like project structure, environment variables, or current branch information.
...
When opencode encounters a command injection:

1. It parses the command within the backticks.
2. If a session is active, it requests `bash` permission from the user.
3. It executes the command and replaces the `!` followed by backticks syntax with the command's output.

:::warning
Command injections are executed whenever the instruction file is loaded. Ensure you only use safe, non-destructive commands.
:::
