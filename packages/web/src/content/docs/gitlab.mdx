---
title: GitLab
description: Use chalice in GitLab issues and merge requests.
---

opencode integrates with your GitLab workflow.
Mention `@opencode` in a comment, and chalice will execute tasks within your GitLab CI pipeline.

---

## Features

- **Triage issues**: Ask chalice to look into an issue and explain it to you.
- **Fix and implement**: Ask chalice to fix an issue or implement a feature.
  It will work create a new branch and raised a merge request with the changes.
- **Secure**: chalice runs on your GitLab runners.
...
6.  Create a flow config file, here's an example:
    ...
    commands:
      ...
      - npm install --global chalicecode-ai
      ...
      - echo $GITLAB_HOST
      - echo "Creating chalice auth configuration"
      - mkdir --parents ~/.local/share/chalice-code
      - |
        cat > ~/.local/share/chalice-code/auth.json << EOF
        {
          "anthropic": {
            "type": "api",
...
Here are some examples of how you can use chalice in GitLab.
...
- **Explain an issue**
  ...
  chalice will read the issue and reply with a clear explanation.
...
- **Fix an issue**
  ...
  chalice will create a new branch, implement the changes, and open a merge request with the changes.
...
- **Review merge requests**
  ...
  chalice will review the merge request and provide feedback.
