---
title: TUI
description: Using the xcsh terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

xcsh provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running xcsh starts the TUI for the current directory.
...
When using the xcsh TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to xcsh. Allows you to select from available providers and add their API keys.
...
This is separate from `xcsh.json`, which configures server/runtime behavior.
