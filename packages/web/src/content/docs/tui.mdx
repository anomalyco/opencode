...
The TUI includes an Automations dialog for creating, running, and managing scheduled prompts.

- Open the command palette and select **Automations**.
- Use the built-in keybinds to create, run, edit, or export automations.

Automations are stored on the server and synced to the TUI via the global event stream.
