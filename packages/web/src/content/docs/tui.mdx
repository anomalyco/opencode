---
title: TUI
description: Using the Octopus terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Octopus provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Octopus starts the TUI for the current directory.
...
When using the Octopus TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Octopus. Allows you to select from available providers and add their API keys.
...
This is separate from `octopus.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Octopus waits after the leader key. Defaults to `2000`.
...
Use `OCTOPUS_TUI_CONFIG` to load a custom TUI config path.
