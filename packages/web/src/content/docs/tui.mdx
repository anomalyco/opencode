...
Show usage limits for authenticated providers that support usage tracking.

```bash frame="none"
/usage
```

The usage dialog fetches all authenticated usage providers and shows provider-specific rate-limit windows, reset times, and credits or quota when available.

Use display flags to choose how percentages are shown for that dialog invocation.
...
Refresh usage in the background without opening the dialog.
...
:::note
The `/usage` dialog always shows all authenticated usage providers. The `show_usage_provider_scope` setting only controls the sidebar Usage section.
:::

The dialog can also be opened from the command palette or bound to a key through the `usage_show` keybind, which is unbound by default.
...
```json title="tui.json"
{
  ...
  "show_usage_provider_scope": "current",
  "show_usage_value_mode": "used"
}
```
...
- `show_usage_provider_scope` - Controls the sidebar Usage section. `"current"` shows usage for the currently selected provider; `"all"` shows all authenticated usage providers. The `/usage` dialog always shows all providers.
- `show_usage_value_mode` - Controls whether usage percentages are shown as `"used"` or `"remaining"` by default. The `/usage --used` and `/usage --remaining` flags override this for a single dialog invocation.
