...
Open a temporary, tool-free side conversation that shares the current session context without adding messages to the main conversation or interrupting its work.

```bash frame="none"
/btw why did the last test fail?
```

Press `esc` to return to the main conversation. You can also run `/btw` without a question and ask several follow-ups in the side conversation.
