---
title: TUI
description: Using the Leak Code terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Leak Code provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Leak Code starts the TUI for the current directory.
...
When using the Leak Code TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Leak Code. Allows you to select from available providers and add their API keys.
...
Exit Leak Code. _Aliases_: `/quit`, `/q`
...
This is separate from `leak-code.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Leak Code waits after the leader key. Defaults to `2000`.
...
Use `LEAKCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `leak-code.default`.
