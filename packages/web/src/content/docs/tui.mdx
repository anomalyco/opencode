...
- `attention` - Configures TUI desktop notifications and sounds.

Use `OPENCODE_TUI_CONFIG` to load a custom TUI config path.

### Attention

The TUI can request attention for questions, permissions, session errors, and completed sessions. Built-in attention events request desktop notifications when the terminal is blurred; sounds are enabled by default.

- `enabled` - Disable all attention notifications and sounds when `false`. Defaults to `true`.
- `notifications` - Enable terminal-mediated desktop notifications. Defaults to `true`.
- `sound` - Enable attention sounds. Defaults to `true`.
- `volume` - Default sound volume from `0` to `1`. Defaults to `0.4`.
- `sound_pack` - Initial semantic sound pack. Defaults to `opencode.default`.
- `sounds` - Override sound files for `default`, `question`, `permission`, `error`, or `done`. Paths can be absolute, `file://` URLs, or relative to `tui.json`.
