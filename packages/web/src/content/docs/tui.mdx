...
You can customize various aspects of the TUI view using the command palette (`ctrl+p`). Press `ctrl+p` and type to filter for any toggle option. These settings persist across restarts.

Available toggle options include:

| Command | Description |
|---|---|
| `Toggle session scrollbar` | Show or hide the session scrollbar. Useful on Linux/Ubuntu where the scrollbar may not be visible by default. |
| `Toggle username` | Show or hide your username in chat messages. |
| `Toggle message timestamps` | Show or hide timestamps on messages. |
| `Toggle thinking blocks` | Show or hide the model's reasoning/thinking blocks. |
| `Toggle tool details` | Show or hide tool execution details. |
| `Toggle file context` | Show or hide file context in the session. |
| `Toggle sidebar` | Show or hide the session sidebar. |
| `Toggle diff wrapping` | Toggle word wrap in diff views. |
| `Toggle generic tool output` | Show or hide generic tool output in the conversation. |
| `Toggle console` | Toggle the debug console panel. |
| `Toggle debug panel` | Toggle the debug panel overlay. |
