...
- `sidebar_sections` - Controls session sidebar sections. `order` lists section names to display first; unlisted sections retain their default order. `hidden` lists sections to omit. Built-in names are `context`, `mcp`, `lsp`, `todo`, and `files`; plugin sections use their plugin ID. Unknown names are ignored. Collapsed state is remembered across sessions and restarts.
