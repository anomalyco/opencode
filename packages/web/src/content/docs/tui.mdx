...
- `logo.animate` - Enable the idle shimmer and interactive burst animation on the home screen logo and Go upsell logo (default: `false`).
- `logo.sound` - Enable sound effects for the logo charge hum and burst. Only active when `logo.animate` is `true` (default: `false`).
