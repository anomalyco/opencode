...
This is separate from `opencode.json`, which configures server/runtime behavior, though TUI settings can also be specified in `opencode.json` under the `tui` key.
