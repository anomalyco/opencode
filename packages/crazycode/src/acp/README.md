...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for crazycode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal crazycode sessions
...
crazycode acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via crazycode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | crazycode acp
```
...
### Mapping to CrazyCode

ACP sessions map cleanly to crazycode's internal session model:
