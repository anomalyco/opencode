...
import { createOpencodeClient, type OpencodeClient } from "@crazycode-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run crazycode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running crazycode server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.CRAZYCODE_AUTO_SHARE || args.share)) {
