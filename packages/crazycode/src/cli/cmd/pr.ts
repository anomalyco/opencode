...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run crazycode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for crazycode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found crazycode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`crazycode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting crazycode...")
        UI.println()

        // Launch crazycode TUI with session ID if available
        const { spawn } = await import("child_process")
        const crazycodeArgs = sessionId ? ["-s", sessionId] : []
        const crazycodeProcess = spawn("crazycode", crazycodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          crazycodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`crazycode exited with code ${code}`))
          })
          crazycodeProcess.on("error", reject)
        })
      },
    })
  },
