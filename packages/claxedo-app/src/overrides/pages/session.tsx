...
export default function Page() {
  ...
  const groupNavigate = (path: string) => {
    if (sessionParams && claxedoLayout) {
      const groupId = sessionParams.groupId()
      const tabs = claxedoLayout.groupTabs(groupId)
      const dir = sessionParams.directory()

      const match = path.match(/\/session\/(.+)$/)
      const newSessionId = match?.[1]

      if (newSessionId) {
        const tabId = tabs.addSession(dir, newSessionId, "Session")
  ...
  const reviewContent = (input: {
    ...
    loadingClass: string
    emptyClass: string
    actions?: SessionReviewTabProps["actions"]
  }) => (
    <Switch>
  ...
  return (
    <div class="relative bg-background-base size-full overflow-hidden flex flex-col">
      ...
      <div class="flex-1 min-h-0 flex flex-col md:flex-row">
        ...
        {/* Session panel - hidden via CSS to avoid expensive remount */}
        <div
          classList={{
              ...
              <Match when={params.id}>
                <Show when={activeMessage()}>
                  <MessageTimeline
                    ...
                    onNavigateParent={() => {
                      groupNavigate(`/${params.dir}/session/${info()?.parentID}`)
                    }}
                    sessionID={params.id!}
