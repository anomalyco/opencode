...
function createPromptSession(serverUrl: string, dir: string, id: string | undefined) {
  ...
  const [store, setStore, _, ready] = persisted(
    SERVER_SCOPED_PERSIST ? Persist.serverScoped(serverUrl, dir, id, "prompt", [legacy]) : Persist.scoped(dir, id, "prompt", [legacy]),
    createStore<{
      prompt: Prompt
      cursor?: number
      context: {
...
export const { use: usePrompt, provider: PromptProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const load = (dir: string, id: string | undefined) => {
      const key = SERVER_SCOPED_PERSIST ? `${server.url}:${dir}:${id ?? WORKSPACE_KEY}` : `${dir}:${id ?? WORKSPACE_KEY}`
