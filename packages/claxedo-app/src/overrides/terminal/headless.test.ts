...
describe("TUI re-render duplication after resize", () => {
  ...
  //
  // These tests stay RED — they prove the bug exists at the xterm level.
  // The fix lives in the resize coordinator (clear-before-flush), tested
  // separately in the GREEN companion tests below.
  // ---------------------------------------------------------------------------

  test("resize shrink causes stale cursor-up — TUI header duplicated", async () => {
  ...
  test("buffer restore into narrower terminal + SIGWINCH causes duplication", async () => {
    const marker = "=== CODEX TUI ==="
  ...
  test("multiple long lines amplify the cursor-up deficit", async () => {
