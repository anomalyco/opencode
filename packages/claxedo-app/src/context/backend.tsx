...
import { getDefaultBaseUrl } from "@claxedo/utils/api"

export type AgentKind = "opencode" | "claude" | "codex" | "amp"

function endpoint(path: string): string {
  return `${getDefaultBaseUrl()}${path}`
}
...
async function requestSpawn(agent: string): Promise<boolean> {
  try {
    const res = await fetch(endpoint("/api/sandbox-agent/spawn"), {
      method: "POST",
...
function requestStop(): void {
  fetch(endpoint("/api/sandbox-agent/stop"), { method: "POST" }).catch(() => {})
}
...
async function checkSandboxStatus(): Promise<{ running: boolean; agent: string | null }> {
  try {
    const res = await fetch(endpoint("/api/sandbox-agent/status"))
