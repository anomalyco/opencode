...
import { useLayout, type LocalProject, useGlobalSync, useGlobalSDK, usePlatform, Titlebar, DialogSettings, useLanguage } from "@opencode-ai/claxedo-app"
    ...
    on(
      ...
      ([wsId, id, title, badge]) => {
        ...
        if (!id) {
          ...
          const hasNewTab = claxedo.topTabs.orderedItems()
...
function ClaxedoLayoutContent(props: ParentProps) {
  ...
  createEffect(() => {
    const dir = activeWorkspaceId()
    if (!dir) return
    // Check if this directory is already in the project list
  ...
  const findProjectForWorkspace = (workspaceDir: string): ProjectItem | undefined => {
    return projects().find(
      (p) => p.worktree === workspaceDir || p.sandboxes?.includes(workspaceDir)
    )
  }
  ...
  const handleNewWorkspace = async (project: ProjectItem): Promise<import("./layouts/top-tab-bar").WorkspaceBarItem | undefined> => {
    ...
    const handleError = (err: unknown) => {
      if (err && typeof err === "object" && "name" in err && (err as { name?: unknown }).name === "WorktreeNotGitError") {
        showToast({
    ...
    // Auto-generate worktree name based on existing sandboxes
    const existingCount = project.sandboxes?.length ?? 0
    const name = `sandbox-${existingCount + 1}`

    try {
      const result = await globalSDK.client.worktree.create({ directory: worktree, worktreeCreateInput: { name } })
      const created = result.data?.directory
      if (created) return onWorktreeCreated(created, name)
    } catch (err) {
      handleError(err)
    }
    return
        ...
        onDelete={async (dir) => {
          ...
          if (projectIndex !== -1) {
            ...
            if (project.sandboxes?.includes(dir)) {
              globalSync.set("project", projectIndex, "sandboxes", project.sandboxes.filter((s) => s !== dir))
            }
          }
  ...
  const renderEmpty = () => (
    <div class="flex flex-col items-center justify-center gap-4 h-full">
      <span class="text-16-medium text-text-strong">Welcome to OpenCode</span>
