 ...
 *
 * Usage:
 *   bun run desktop:dev:local              # Normal mode
 *   bun run desktop:dev:local --debug      # Debug mode (verbose logging)
 *   CLAXEDO_DEBUG=1 bun run desktop:dev:local  # Alternative debug mode
 */
...
async function main() {
  ...
  try {
    ...
    const opencodeProc = Bun.spawn({
      cmd: ["bun", "run", "src/index.ts", "serve", "--port", String(opencodePort)],
      cwd: tempDir,
      env: {
        ...process.env,
    ...
    const viteProc = Bun.spawn({
      cmd: ["bun", "run", "vite", "--config", "vite.cloud.config.ts", "--port", String(vitePort)],
      cwd: CLAXEDO_APP_DIR,
      env: {
        ...process.env,
        CLAXEDO_OVERRIDES: "1",
    ...
    const tauriProc = Bun.spawn({
      cmd: ["bun", "run", "--cwd", "../desktop", "tauri", "--", "dev", "--config", tauriConfig],
      cwd: CLAXEDO_APP_DIR,
      env: {
        ...process.env,
