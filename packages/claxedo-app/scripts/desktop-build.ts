 ...
 *
 * Usage:
 *   bun ./scripts/desktop-build.ts [options]
 *
 * Options:
 ...
 *
 * Examples:
 *   bun ./scripts/desktop-build.ts                        # Dev build for current platform
 *   bun ./scripts/desktop-build.ts --prod                 # Production build
 *   bun ./scripts/desktop-build.ts --platform=macos --target=universal
...
const envLocalPath = `${claxedoAppDir}/.env.local`
try {
...
const desktopDir = `${claxedoAppDir}/../desktop`
...
if (!skipSidecar) {
  ...
  const buildOpencode = await $`bun run ./scripts/build-opencode.ts --single`
    .cwd(claxedoAppDir)
    .nothrow()
  ...
  const distOpencode = `${claxedoAppDir}/dist-opencode`
  const sourceBinary = `${distOpencode}/${sidecarConfig.ocBinary}/bin/opencode${process.platform === "win32" ? ".exe" : ""}`
  const sidecarDir = `${desktopDir}/src-tauri/sidecars`
  const destBinary = `${sidecarDir}/opencode-cli-${rustTarget}${process.platform === "win32" ? ".exe" : ""}`
...
const tauriTargetDir = `${desktopDir}/src-tauri/target/${buildProfile}`
const bundleMacosDir = `${tauriTargetDir}/bundle/macos`
...
// Override Tauri config to use our built frontend
// Path is relative to src-tauri directory
const tauriConfigOverride: Record<string, unknown> = {
  build: {
    beforeBuildCommand: "", // Skip Tauri's default build command (we already built)
    frontendDist: "../../claxedo-app/dist-desktop",
  },
}

// Add Claxedo-specific branding for production
if (isProd) {
  tauriConfigOverride.productName = "Claxedo"
  tauriConfigOverride.identifier = "ai.claxedo.desktop"
}
...
const tauriBuild = await $`bun run ${tauriArgs}`.cwd(desktopDir).env(buildEnv).nothrow()
...
// eslint-disable-next-line no-console
console.log(`[claxedo] Output: ${desktopDir}/src-tauri/target/release/bundle/`)
...
const bundlePath = `${desktopDir}/src-tauri/target/release/bundle`
