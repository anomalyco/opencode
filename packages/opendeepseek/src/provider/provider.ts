...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "opendeepseek",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.OPENDEEPSEEK_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
        if (
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("opendeepseek")) {
        priority = ["gpt-5-nano"]
      }
    ...
    const opencodeProvider = await state().then((state) => state.providers["opendeepseek"])
    if (opencodeProvider && opencodeProvider.models["gpt-5-nano"]) {
      return getModel("opendeepseek", "gpt-5-nano")
    }
