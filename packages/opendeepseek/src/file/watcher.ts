...
export namespace FileWatcher {
  ...
  const watcher = lazy(() => {
    const binding = require(
      `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${OPENDEEPSEEK_LIBC || "glibc"}` : ""}`,
    )
  ...
  const state = Instance.state(
    async () => {
      ...
      if (Flag.OPENDEEPSEEK_EXPERIMENTAL_FILEWATCHER) {
        subs.push(
  ...
  export function init() {
    if (Flag.OPENDEEPSEEK_EXPERIMENTAL_DISABLE_FILEWATCHER) {
      return
    }
    state()
  }
