...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opendeepseek.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "opendeepseek.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .opendeepseek directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".opendeepseek")
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "opendeepseek.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .opendeepseek config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".opendeepseek")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .opendeepseek config with different plugins
      await Bun.write(
        path.join(opencodeDir, "opendeepseek.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opendeepseek config with some overlapping plugins
      await Bun.write(
        path.join(opencodeDir, "opendeepseek.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
