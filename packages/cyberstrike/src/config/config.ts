...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/cyberstrike"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "cyberstrike")
      default:
        return "/etc/cyberstrike"
    }
  }

  const managedConfigDir = process.env.CYBERSTRIKE_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  export const state = Instance.state(async () => {
    ...
    // 4) Project config (cyberstrike.json{,c})
    // 5) .cyberstrike directories (.cyberstrike/agents/, .cyberstrike/commands/, .cyberstrike/plugins/, .cyberstrike/cyberstrike.json{,c})
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: cyberstrike.local)"),
