...
const modelsData = await (async () => {
  ...
  if (file) return (await Bun.file(file).text()).trimEnd()

  const fixture = path.join(dir, "test", "tool", "fixtures", "models-api.json")
  if (await Bun.file(fixture).exists()) return (await Bun.file(fixture).text()).trimEnd()

  return (await fetch(`${modelsUrl}/api.json`).then((x) => x.text())).trimEnd()
})()
await Bun.write(
