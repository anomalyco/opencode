  ...
  for (const [pkg, pkgbuild] of [
    ...
    for (let i = 0; i < 30; i++) {
      try {
        ...
        const aurUrl = dryRun ? `https://aur.archlinux.org/${pkg}.git` : `ssh://aur@aur.archlinux.org/${pkg}.git`
        await $`git clone ${aurUrl} ./dist/aur-${pkg}`
        ...
        if (dryRun) {
          console.log(`DRY_RUN: skipping git commit/push for ${pkg}`)
        }
        break
      } catch (e) {
        continue
