...
export function withNetworkOptions<T>(yargs: Argv<T>): Argv<T & NetworkOptions> {
  return yargs.options({
    ...
    cors: { type: "string" as const, array: true, describe: "additional domains to allow for CORS", default: [] as string[] },
  }) as unknown as Argv<T & NetworkOptions>
}
...
export function resolveNetworkOptionsNoConfig(args: NetworkOptions, config?: ConfigV1.Info) {
  ...
  const mdnsDomain = mdnsDomainExplicitlySet ? args.mdnsDomain : (config?.server?.mdnsDomain ?? args.mdnsDomain)
