...
export const McpDebugCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      // Exercise the same SDK v2 negotiation and OAuth path as the runtime.
      // A raw `initialize` probe would misdiagnose modern-only 2026 servers,
      // because SDK v2 protocol negotiation starts with `server/discover`.
