...
import { ConfigMCPV1 } from "@leak-code/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in leak-code.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .leak-code/ subdirectory too)
  const candidates = [path.join(baseDir, "leak-code.json"), path.join(baseDir, "leak-code.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".leak-code", "leak-code.json"),
      path.join(baseDir, ".leak-code", "leak-code.jsonc"),
    )
  }
  ...
  // Default to leak-code.json if none exist
  return candidates[0]
}
