...
import { InstallationLocal, InstallationVersion } from "@yunpat/core/installation/version"
import { makeRuntime } from "@yunpat/core/effect/runtime"
...
export class Service extends Context.Service<Service, Interface>()("@yunpat/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // 4. `.yunpat-agent` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(AgentBrand.projectDir) || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(AgentBrand.projectDir) && dir !== Flag.OPENCODE_CONFIG_DIR) continue
