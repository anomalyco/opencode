...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run ohmycode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running ohmycode server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://ohmycode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
