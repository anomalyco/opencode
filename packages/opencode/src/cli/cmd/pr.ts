  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["mammouth", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        const code = await mammouthProcess.exited
        if (code !== 0) throw new Error(`mammouth exited with code ${code}`)
      },
    })
  },
})
