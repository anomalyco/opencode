...
export namespace LSPServer {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(js).exists())) {
          ...
          await Bun.spawn(
            [BunProc.which(), "install", "@vue/language-server"],
            {
              cwd: Global.Path.bin,
              env: {
                ...process.env,
  ...
  export const Oxlint: Info = {
    ...
    async spawn(root) {
      ...
      const serverTarget = path.join(
        "node_modules",
        ".bin",
        "oxc_language_server" + ext,
      );
  ...
  export const Biome: Info = {
    ...
    async spawn(root) {
      ...
      if (!bin) {
        const resolved = await Bun.resolve("biome", root).catch(
          () => undefined,
        );
        if (!resolved) return;
        bin = BunProc.which();
  ...
  export const Clangd: Info = {
    ...
    async spawn(root) {
      ...
      if (!token) {
        log.error(
          `Platform ${platform} is not supported by clangd auto-download`,
        );
        return;
      }
      ...
      await fs
        .symlink(bin, path.join(Global.Path.bin, "clangd"))
        .catch(() => {});
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(js).exists())) {
          if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return;
          await Bun.spawn(
            [BunProc.which(), "install", "@astrojs/language-server"],
            {
              cwd: Global.Path.bin,
              env: {
                ...process.env,
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      if (!java) {
        log.error(
          "Java 21 or newer is required to run the JDTLS. Please install it first.",
        );
        return;
      }
      ...
      const dataDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "opencode-jdtls-data"),
      );
      return {
        process: spawn(
          java,
    ...
    async spawn(root) {
      ...
      if (!installed) {
        ...
        if (!supportedCombos.includes(combo)) {
          log.error(
            `Platform ${platform}/${arch} is not supported by Kotlin LSP`,
          );
          return;
        }
      ...
      if (!(await Bun.file(launcherScript).exists())) {
        log.error(
          `Failed to locate the Kotlin LS launcher script in the installed directory: ${distPath}.`,
        );
        return;
      }
      return {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!exists) {
          if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return;
          await Bun.spawn(
            [BunProc.which(), "install", "yaml-language-server"],
            {
              cwd: Global.Path.bin,
              env: {
                ...process.env,
  ...
  export const TailwindCSS: Info = {
    id: "tailwindcss",
    extensions: [".css", ".html", ".htm", ".jsx", ".tsx", ".vue", ".svelte"],
    root: NearestRoot([
      "tailwind.config.js",
      "tailwind.config.ts",
      "postcss.config.js",
    ...
    async spawn(root) {
      ...
      if (!binary) {
        const js = path.join(
          ...
          "language-server",
          "bin",
          "tailwindcss-language-server.js",
        );
        if (!(await Bun.file(js).exists())) {
          if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return;
          await Bun.spawn(
            [BunProc.which(), "install", "@tailwindcss/language-server"],
            {
              cwd: Global.Path.bin,
              env: {
                ...process.env,
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(js).exists())) {
          if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return;
          await Bun.spawn(
            [BunProc.which(), "install", "bash-language-server"],
            {
              cwd: Global.Path.bin,
              env: {
                ...process.env,
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        if (!asset?.browser_download_url) {
          log.error(
            `Could not find asset ${assetName} in terraform-ls release`,
          );
          return;
        }
