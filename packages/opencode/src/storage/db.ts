...
declare const IMECODE_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.IMECODE_DB) {
    if (Flag.IMECODE_DB === ":memory:" || path.isAbsolute(Flag.IMECODE_DB)) return Flag.IMECODE_DB
    return path.join(Global.Path.data, Flag.IMECODE_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof IMECODE_MIGRATIONS !== "undefined"
        ? IMECODE_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof IMECODE_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
