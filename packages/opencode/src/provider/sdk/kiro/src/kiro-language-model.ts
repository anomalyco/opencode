...
export class KiroLanguageModel implements LanguageModelV2 {
  ...
  async doStream(
    ...
    // Debug log - also log original prompt structure
    const promptSummary = options.prompt.map((m, i) => `[${i}] role=${m.role}`).join(", ")
    const debugLog = `[${new Date().toISOString()}] URL: ${url}\nPrompt structure: ${promptSummary}\nPayload: ${JSON.stringify(payload, null, 2)}\n\n`
    const fs = await import("fs/promises")
    await fs.appendFile("/tmp/kiro-debug.log", debugLog)
