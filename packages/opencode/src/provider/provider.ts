...
export namespace Provider {
  ...
  export async function getModel(providerID: ProviderID, modelID: ModelID) {
    ...
    // Try extracting variant suffix joined by hyphen (e.g., "claude-opus-4-6-high" -> "claude-opus-4-6")
    const lastHyphen = modelID.lastIndexOf("-")
    if (lastHyphen > 0) {
      const baseModelId = modelID.slice(0, lastHyphen)
      const candidateVariant = modelID.slice(lastHyphen + 1)
      const baseInfo = provider.models[baseModelId]
      if (baseInfo?.variants && candidateVariant in baseInfo.variants) return baseInfo
    }
