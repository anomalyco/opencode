...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    "mammouth-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      const key = iife(() => {
        const envKey = input.env.find((item) => env[item])
        if (envKey) return env[envKey]
        if (auth?.type === "api") return auth.key
        return undefined
      })

      // Without a key, only the free models are usable — drop the paid ones so
      // the picker doesn't surface models the user can't reach.
      if (!key) {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from mammouth.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://mammouth.ai/",
            "X-Title": "mammouth",
            "X-Source": "opencode",
          },
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `mammouth/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `mammouth/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `mammouth auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `mammouth/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "mammouth",
          },
        },
      }),
    kilo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, mammouth auth, or provider options.`,
            )
          },
        }
      }
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      // Mammouth Code defaults to the mammouth-ai provider when available.
      const mammouth = s.providers[ProviderV2.ID.make("mammouth-ai")]
      if (mammouth && (!cfg.provider || Object.keys(cfg.provider).includes("mammouth-ai"))) {
