...
> = {
  ...
  "@ai-sdk/google-vertex": () =>
    import("@ai-sdk/google-vertex").then((m) => m.createVertex),
  "@ai-sdk/google-vertex/anthropic": () =>
    import("@ai-sdk/google-vertex/anthropic").then(
      (m) => m.createVertexAnthropic,
    ),
  ...
  "@requesty/ai-sdk": () =>
    import("@requesty/ai-sdk").then((m) => m.createRequesty),
  "@ai-sdk/xai": () => import("@ai-sdk/xai").then((m) => m.createXai),
  "@ai-sdk/mistral": () =>
    import("@ai-sdk/mistral").then((m) => m.createMistral),
  "@ai-sdk/groq": () => import("@ai-sdk/groq").then((m) => m.createGroq),
  "@ai-sdk/deepinfra": () =>
    import("@ai-sdk/deepinfra").then((m) => m.createDeepInfra),
  "@ai-sdk/cerebras": () =>
    import("@ai-sdk/cerebras").then((m) => m.createCerebras),
  "@ai-sdk/cohere": () => import("@ai-sdk/cohere").then((m) => m.createCohere),
  "@ai-sdk/gateway": () =>
    import("@ai-sdk/gateway").then((m) => m.createGateway),
  "@ai-sdk/togetherai": () =>
    import("@ai-sdk/togetherai").then((m) => m.createTogetherAI),
  "@ai-sdk/perplexity": () =>
    import("@ai-sdk/perplexity").then((m) => m.createPerplexity),
  "@ai-sdk/vercel": () => import("@ai-sdk/vercel").then((m) => m.createVercel),
  "@ai-sdk/alibaba": () =>
    import("@ai-sdk/alibaba").then((m) => m.createAlibaba),
  "gitlab-ai-provider": () =>
    import("gitlab-ai-provider").then((m) => m.createGitLab),
  "@ai-sdk/github-copilot": () =>
    import("@opencode-ai/core/github-copilot/copilot-provider").then(
      (m) => m.createOpenaiCompatible,
    ),
  "venice-ai-sdk-provider": () =>
    import("venice-ai-sdk-provider").then((m) => m.createVenice),
};
  ...
  return {
      ...
      Effect.succeed({
        ...
        async getModel(
          ...
          if (model && "endpoint" in model.api) {
            if (model.api.endpoint === "responses" && sdk.responses)
              return sdk.responses(modelID);
            if (model.api.endpoint === "chat" && sdk.chat)
    ...
    requesty: Effect.fnUntraced(function* (input: Info) {
      ...
      const apiKey = iife(() => {
        ...
        return env["REQUESTY_API_KEY"];
      });
      const baseURL =
        (input.options?.baseURL as string) || "https://router.requesty.ai/v1";
      ...
      return {
        ...
        async discoverModels(): Promise<Record<string, Model>> {
          // Sending the key scopes the listing to the account's routing policies
          // and BYOK entries; the endpoint also answers unauthenticated with the
          // full public catalog.
          ...
          return Object.fromEntries(
            json.data.flatMap((item) =>
              item.id
                ? [[item.id, requestyModel(item.id, item, baseURL)] as const]
                : [],
            ),
          );
        },
        ...
        options.fetch = async (url: RequestInfo | URL, init?: RequestInit) => {
          ...
          if (!response.ok && response.status === 400) {
            try {
              ...
              if (
                errorMessage.toLowerCase().includes("conversation complete")
              ) {
                return new Response(
                  JSON.stringify({
                    choices: [
...
// Requesty reports prices per token; opencode stores them per million.
const REQUESTY_PRICE_SCALE = 1_000_000;
...
function requestyModel(
  ...
  const vision = item.supports_vision ?? false;
  return {
    id: ModelV2.ID.make(id),
    providerID: ProviderV2.ID.make("requesty"),
    name: id,
    family: "",
    api: { id, url: baseURL, npm: "@requesty/ai-sdk" },
    status: "active",
    headers: {},
    options: {},
    cost: {
      input: (item.input_price ?? 0) * REQUESTY_PRICE_SCALE,
      output: (item.output_price ?? 0) * REQUESTY_PRICE_SCALE,
      cache: {
        read: (item.cached_price ?? 0) * REQUESTY_PRICE_SCALE,
        write: 0,
      },
    },
    limit: {
      context: item.context_window ?? 128_000,
      output: item.max_output_tokens ?? 4_096,
    },
    capabilities: {
      temperature: true,
...
function fromModelsDevModel(
  ...
  const variants =
    ...
    ProviderTransform.variants(base);
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const requesty = ProviderV2.ID.make("requesty");
        if (
          discoveryLoaders[requesty] &&
          providers[requesty] &&
          isProviderAllowed(requesty)
        ) {
          yield* Effect.promise(async () => {
            try {
              const discovered = await discoveryLoaders[requesty]();
              // The router listing is authoritative once we have it: a models.dev
              // entry it no longer serves would fail at request time. An empty
              // result means discovery failed, so the seed is left in place.
      ...
      try {
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          const headerTimeoutMs =
            headerTimeout === false ? undefined : headerTimeout;
        ...
        const fn =
          mod[Object.keys(mod).find((key) => key.startsWith("create"))!];
        const loaded = fn({
          name: model.providerID,
          ...options,
        });
