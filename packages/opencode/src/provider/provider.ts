...
export namespace Provider {
          ...
          typeof nested?.code === "string"
            ...
            : typeof nested?.type === "string"
              ...
              : typeof chunk.code === "string"
                ...
                : "",
        message:
          typeof nested?.message === "string" ? nested.message : typeof chunk.message === "string" ? chunk.message : "",
        param: typeof nested?.param === "string" ? nested.param : typeof chunk.param === "string" ? chunk.param : null,
      },
    }
  }
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    gitlab: async (input) => {
      ...
      // GitLab provider version is not exported in newer versions
      const GITLAB_PROVIDER_VERSION = "unknown"
    ...
    try {
      ...
      options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
        ...
        if (model.api.npm === "@ai-sdk/openai") {
          const url = requestURL(input) ?? res.url
          if (shouldNormalizeOpenAIResponsesStream(url, res)) {
            return normalizeOpenAIResponsesStream(!chunkAbortCtl ? res : wrapSSE(res, chunkTimeout, chunkAbortCtl))
          }
        }
