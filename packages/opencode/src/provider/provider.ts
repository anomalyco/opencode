...
export namespace Provider {
  ...
  export async function getLanguage(model: Model): Promise<any> {
    const s = await state()
    ...
    try {
      ...
      // Get apiType from the raw ModelsDev data since Info type doesn't include it
      const modelsDev = await ModelsDev.get()
      const providerInfo = modelsDev[model.providerID]
