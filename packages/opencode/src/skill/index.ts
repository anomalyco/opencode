...
function addLegacy(state: State, dir: string, cfg: Extract<Config.Info["skills"], readonly unknown[]>) {
  for (const item of cfg) {
    if (state.skills[item.name]) {
      log.warn("duplicate skill name", {
        name: item.name,
        existing: state.skills[item.name].location,
        duplicate: path.join(dir, "opencode.json"),
      })
    }
    ...
    state.skills[item.name] = {
      ...
      location: path.join(dir, "opencode.json"),
      content: [
        "# Legacy Inline Skill",
        "",
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (Array.isArray(cfg.skills)) {
    addLegacy(state, directory, cfg.skills)
    return {
