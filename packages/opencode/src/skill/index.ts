...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadState = Effect.fn("Skill.state.load")(function* (
      ...
      const discovered = yield* discoverSkills(config, discovery, fsys, global, ctx.directory, ctx.worktree, opts)
      const s: State = { skills: {}, dirs: new Set(discovered.dirs) }
      // Register the built-in skill before disk discovery so a user-disk skill
