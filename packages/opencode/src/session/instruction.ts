...
const FILES = [
  "AGENTS.md",
  ...(Flag.MAMMOUTH_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
  "CONTEXT.md", // deprecated
]

function globalFiles() {
  const files = []
  if (Flag.MAMMOUTH_CONFIG_DIR) {
    files.push(path.join(Flag.MAMMOUTH_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.MAMMOUTH_DISABLE_CLAUDE_CODE_PROMPT) {
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          if (!Flag.MAMMOUTH_DISABLE_PROJECT_CONFIG) {
            return yield* fs
          ...
          return yield* fs
            .globUp(instruction, Flag.MAMMOUTH_CONFIG_DIR, Flag.MAMMOUTH_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
