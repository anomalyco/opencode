      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "tool-call": {
            ...
            const toolCall = yield* ensureToolCall(value)
            // Apply Qwen-style input normalization after upstream's toolInput()
            // so malformed inputs (e.g. `questions` as a truncated JSON string)
            // don't leak into UI / persisted state. See issue #67.
            const input = normalizeToolInput(value.name, toolInput(value.input))
