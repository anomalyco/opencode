...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      let reloadedProviderAuth = false

      const isUnauthorized = (error: unknown) => {
        const parsed = parse(error)
        return MessageV2.APIError.isInstance(parsed) && parsed.data.statusCode === 401
      }
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        return yield* Effect.gen(function* () {
          const runStream = Effect.fn("SessionProcessor.runStream")(function* () {
            ctx.currentText = undefined
            ctx.reasoningMap = {}
          ...
          yield* runStream().pipe(
            ...
            Effect.catchIf(
              (error) => !reloadedProviderAuth && isUnauthorized(error),
              (error) =>
                Effect.gen(function* () {
                  reloadedProviderAuth = true
                  slog.info("reloading provider state after 401", { providerID: ctx.model.providerID })
