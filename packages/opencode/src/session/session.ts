...
export function* listGlobal(input?: {
  ...
  const rows = Database.use((db) => {
    ...
    const result = query.orderBy(desc(SessionTable.time_updated), desc(SessionTable.id)).limit(limit).all()
    return result
  })
...
export function* listOrphans() {
  const rows = Database.use((db) =>
    db.select().from(SessionTable).where(isNull(SessionTable.parent_id)).orderBy(desc(SessionTable.time_updated)).all(),
  )
...
export async function migrate(input: { sessionID: SessionID; projectID: ProjectID; directory: string }) {
  Database.use((db) => {
  ...
  if (!row) throw new Error(`Session not found after migration: ${input.sessionID}`)
  return fromRow(row)
}
