    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
          ...
          const pending = yield* Effect.promise(() => SessionStart.pending(SessionID.make(input.sessionID)))
          system.push(
            [
          ...
          // Consume the pending session-start context now that the provider
          // stream was successfully created. Failures before this point will
          // leave the context intact for retry.
          yield* Effect.promise(() => SessionStart.consume(SessionID.make(input.sessionID), pending))
        })
