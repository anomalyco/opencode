...
import { ConfigJevV1 } from "@opencode-ai/core/v1/config/jev"
...
export namespace Jev {
  ...
  export async function routeWith(
    ...
    if (excluded.length > 0)
      Effect.runSync(Effect.logInfo("jev layer0", { excluded: Object.fromEntries(excluded.map((e) => [e.id, e.reason])) }))

    // 0 available -> never guess, keep the default model.
    if (available.length === 0) {
      Effect.runSync(Effect.logWarning("jev no tiers available", { defaultModel: input.defaultModel }))
      return undefined
    }
  ...
  export function requestText(parts: readonly { type: string; text?: string; synthetic?: boolean; ignored?: boolean }[]): string {
    return parts
