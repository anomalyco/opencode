...
import { type Tool as AITool, tool, jsonSchema, type ToolCallOptions as ToolExecutionOptions, asSchema } from "ai"
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
          ...
          while (true) {
            ...
            const outcome: "break" | "continue" = yield* Effect.onExit(
              Effect.gen(function* () {
                ...
                const toonConfig = (yield* Effect.promise(() => Config.get())).experimental?.toon_format
                let finalMsgs = modelMsgs
                if (toonConfig?.enabled) {
                  const toonResult = yield* Effect.promise(() => TOONTransform.transform(modelMsgs, sessionID))
                  finalMsgs = toonResult.messages
                ...
                const result = yield* handle.process({
                  ...
                  sessionID,
                  system,
                  messages: [...finalMsgs, ...(isLastStep ? [{ role: "assistant" as const, content: MAX_STEPS }] : [])],
                  tools,
                  model,
  ...
  const { runPromise } = makeRuntime(Service, defaultLayer as unknown as Layer.Layer<Service>)
