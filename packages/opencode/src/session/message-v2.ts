  ...
  export function toModelMessage(
    ...
    for (const msg of input) {
      ...
      if (msg.info.role === "assistant") {
        const normal: UIMessage["parts"] = []
        const tools: UIMessage["parts"] = []

        for (const part of msg.parts) {
          if (part.type === "text") {
            normal.push({ type: "text", text: part.text, providerMetadata: part.metadata })
            continue
          }
          if (part.type === "reasoning") {
            normal.push({ type: "reasoning", text: part.text, providerMetadata: part.metadata })
            continue
          }
          if (part.type === "step-start") {
            normal.push({ type: "step-start" })
            continue
          }
          if (part.type === "tool") {
        ...
        result.push({ id: msg.info.id, role: "assistant", parts: [...normal, ...tools] })
      }
    }
