...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for imecode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal imecode sessions
...
imecode acp --cwd /path/to/project
```
...
```bash
IMECODE_ENABLE_QUESTION_TOOL=1 imecode acp
```
...
- **Terminal Integration**: Full terminal support via imecode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | imecode acp
```
...
ACP sessions map cleanly to imecode's internal session model:
