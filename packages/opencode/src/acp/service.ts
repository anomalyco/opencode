...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `mammouth auth login` in the terminal",
      name: "Login with Mammouth",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, mammouth-ai provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const mammouthProvider = providers[ProviderV2.ID.make("mammouth-ai")]
  const mammouthModel = mammouthProvider ? Provider.sort(Object.values(mammouthProvider.models))[0] : undefined
  if (mammouthProvider && mammouthModel) return { providerID: mammouthProvider.id, modelID: mammouthModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Mammouth Code service failure", service })
}
