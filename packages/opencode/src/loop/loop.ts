...
export const layer = Layer.effect(
	...
	Effect.gen(function* () {
			...
			Effect.gen(function* () {
				while (true) {
					...
					const nearIdentical =
						...
						similarity(result.output, updated.lastOutput) >=
							NoProgressSimilarityThreshold;
					const streak =
						noToolCalls && (updated.noProgressStreak === 0 || nearIdentical)
							? updated.noProgressStreak + 1
							: 0;
					...
					yield* waitBetween(
						id,
						(updated.info.interval ?? DefaultIntervalSeconds) * 1000,
					);
				}
			});
		...
		const create = (input: CreateInput) =>
			Effect.gen(function* () {
				...
				yield* Ref.update(state, (map) =>
					new Map(map).set(id, { info, noProgressStreak: 0 }),
				);
				yield* emit(id);
		...
		const list = (input?: { directory?: string }) =>
			Effect.gen(function* () {
				const all = Array.from((yield* Ref.get(state)).values()).map(
					(record) => record.info,
				);
		...
		const get = (id: LoopID) =>
			Ref.get(state).pipe(Effect.map((map) => map.get(id)?.info));
		...
		const cancel = (id: LoopID) =>
			Effect.gen(function* () {
				...
				if (record.info.status !== "running" && record.info.status !== "paused")
					return false;
				yield* patch(id, (current) => ({
					...current,
					info: {
