...
export namespace ToolRegistry {
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const desktop = yield* build(DesktopTool)
      const browser = yield* build(BrowserTool)
      const swarm = yield* build(SwarmTool)
      const brief = yield* build(BriefTool)
      const snip = yield* build(SnipTool)
      const worktreeEnter = yield* build(EnterWorktreeTool)
      const worktreeExit = yield* build(ExitWorktreeTool)
