...
export const TaskTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("TaskTool.execute")(function* (
      ...
      if (limit <= 0) {
        return yield* Effect.fail(new Error(`Invalid subagent timeout value: ${limit}. Timeout must be greater than 0.`))
      }
          ...
          Effect.gen(function* () {
            const text = yield* runTask().pipe(
              Effect.timeoutOrElse({
                duration: `${limit} millis`,
                orElse: () =>
                  cancel.pipe(Effect.andThen(Effect.fail(new Error(`Subagent timed out after ${limit}ms`)))),
              }),
            )
            return {
              title: params.description,
