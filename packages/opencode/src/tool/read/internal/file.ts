...
export const FileTool = Tool.defineEffect(
  "inspect_file",
  Effect.gen(function* () {
    ...
    const list = Effect.fn("FileTool.list")(function* (filepath: string) {
    ...
    return {
      description: "Internal file and directory reader backing inspect.",
      parameters,
...
async function lines(filepath: string, opts: { limit: number; offset: number }) {
  ...
  return { raw, count, offset: opts.offset, cut, more }
}

async function isBinaryFile(filepath: string, size?: number) {
  const fd = await open(filepath, "r")
  try {
    const bytesToRead = Math.min(size || 512, 512)
