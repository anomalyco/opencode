...
export namespace Tool {
  ...
  function flattenIssues(issues: z.ZodIssue[]): z.ZodIssue[] {
    return issues.flatMap((issue) => {
      ...
      const ranked = issue.errors
        .map((branch) => flattenIssues(branch))
        .map((branch) => {
          const filtered = branch.filter(
            (item) => !(item.code === "invalid_type" && item.message.includes("expected never")),
          )
          return filtered.length > 0 ? filtered : branch
        })
  ...
  function formatValidationError(id: string, error: z.ZodError) {
    ...
    return `The ${id} tool was called with invalid arguments: ${detail || String(error)}.\nPlease rewrite the input so it satisfies the expected schema.`
  }
      ...
      toolInfo.execute = async (args, ctx) => {
        ...
        try {
          ...
          if (error instanceof z.ZodError) {
            throw new Error(formatValidationError(id, error), { cause: error })
          }
          throw new Error(
