...
`~/.config/securecode/sandbox.json` を作成（または編集）。テンプレ:
...
セキュアコード起動中に `~/.config/securecode/sandbox.json`（global）または `./.securecode/sandbox.json`（project）を編集すると、TUI 内にトースト通知が出ます。**`/reload_sandbox` を叩くと、現在のセッション (会話履歴・todo) を保ったまま新しい設定を反映**できます。

```
/reload_sandbox
```

- 監視対象は **global と project の両方**。どちらを編集してもトーストが出て、`/reload_sandbox` で global+project を union し直して再 init します
- 設定に変更が **無い** 状態で `/reload_sandbox` を叩いても、**reload は一切実行されません**。代わりにコマンドの意図を説明するダイアログだけが出ます (TUI のちらつきもセッションの中断も起きないので、気軽に試し打ちして OK)
- 設定に変更が **ある** 状態で叩くと、TUI が一瞬チラついて再起動しますが、`--continue` で前のセッションが復元されます
- **AI からは global / project どちらの `sandbox.json` も、hash ファイルも読み書きできない**ため、AI が自分の檻を勝手に広げる経路は構造上存在しません。reload は常にユーザーの手動 `/reload_sandbox` 起動でしか発生しません
