...
export const frontend = {
  name: "frontend",
  description:
    "Specialist frontend implementation subagent for other agents. Use it when work is primarily UI components, interaction flows, responsive behavior, styling, accessibility, visual consistency, desktop-shell behavior, or other user-visible frontend behavior. It performs its own focused discovery, implementation, targeted documentation lookup, and verification without delegating to helper agents, does not own final review, can load dedicated \`frontend-impl-*\` workflow skills for component surfaces, design-system alignment, data-boundary honesty, desktop-shell work, and finish checks, and it stays inside a stricter capability envelope than the main coding lane. If backend or data work is still missing, it should report that boundary instead of faking completion. Critical concurrency guidance: run at most 2 \`frontend\` tasks in parallel, and only when the UI slices are clearly independent. Provide the target surface, expected behavior, visual constraints, acceptance criteria, and any known frontend risks; it returns a concise \`Status\`, \`Summary\`, \`Evidence\`, \`Frontend Acceptance\`, \`Other-Area Impacts\`, \`Questions For Caller\`, \`Changed Files\`, and \`Verification\` result.",
  color: "accent",
