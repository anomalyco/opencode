...
import { errorData, errorMessage } from "@leak-code/tui/util/error"
...
import { hasTheme, upsertTheme } from "@leak-code/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@leak-code/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@leak-code/core/v1/config/plugin"
import { createCommandShim } from "@leak-code/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@leak-code/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".leak-code"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".leak-code", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".leak-code", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.LEAKCODE_PURE ? [] : pluginOrigins
    if (Flag.LEAKCODE_PURE && pluginOrigins.length) {
    }
