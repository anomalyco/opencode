...
export namespace TOON {
  ...
  /**
   * Reverse TOON abbreviations back to full words.
   * Preserves code blocks (does not expand inside them).
   */
  export function deserialize(text: string): string {
    ...
    // Reverse symbols (compact mode)
    result = result.replace(/→/g, "returns")
    result = result.replace(/\bgreater than or equal\b/g, ">=") // already natural, skip
    result = result.replace(/\bless than or equal\b/g, "<=") // already natural, skip

    // Reverse conjunctions
    result = result.replace(/\b&\b/g, "and")
    result = result.replace(/\s\|\s/g, " or ")

    // Reverse abbreviations (all categories, full-word boundary match)
    const allAbbr: Record<string, string> = {}
    for (const dict of Object.values(abbreviations)) {
      for (const [word, abbr] of Object.entries(dict)) {
        allAbbr[abbr] = word
      }
    }
    for (const [abbr, word] of Object.entries(allAbbr)) {
      result = result.replace(new RegExp(`\\b${abbr}\\b`, "g"), word)
    }
  ...
  /**
   * Generate system prompt instructions telling the model to respond in TOON format.
   */
  export function outputSystemPrompt(mode: Mode): string {
    const lines = [
      "Respond using TOON (Token-Optimized Notation) abbreviations to reduce token usage.",
      "Use these abbreviations in your text responses (outside code blocks):",
      "",
    ]

    for (const [category, dict] of Object.entries(abbreviations)) {
      const pairs = Object.entries(dict)
        .map(([word, abbr]) => `${word}→${abbr}`)
        .join(", ")
      lines.push(`${category}: ${pairs}`)
    }

    if (mode === "compact" || mode === "balanced") {
      lines.push("")
      lines.push('Use & instead of "and", | instead of "or".')
    }

    if (mode === "compact") {
      lines.push("Use → instead of \"returns\". Omit articles (a, an, the) where unambiguous.")
    }

    lines.push("")
    lines.push("Do NOT abbreviate inside code blocks (``` ... ```).")
