...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "mammouth"),
    ...ConfigPaths.fileInDirectory(Global.Path.config, "opencode"),
    ...(await Filesystem.findUp(
      ["mammouth.json", "mammouth.jsonc", "opencode.json", "opencode.jsonc"],
      input.cwd,
      undefined,
      { rootFirst: true },
    )),
  ...
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "mammouth"))
