...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from leak-code.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "leak-code"),
    ...(await Filesystem.findUp(["leak-code.json", "leak-code.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "leak-code"))
  }
  if (Flag.LEAKCODE_CONFIG) files.push(Flag.LEAKCODE_CONFIG)
