...
export namespace Config {
	...
	function mergeConfigConcatArrays(target: Info, source: Info): Info {
		...
		if (target.skill?.dirs && source.skill?.dirs) {
			merged.skill = merged.skill || {};
			merged.skill.dirs = Array.from(
				new Set([...target.skill.dirs, ...source.skill.dirs]),
			);
		}
		return merged;
	}
	...
	export const state = Instance.state(async () => {
		...
		for (const [key, value] of Object.entries(auth)) {
			if (value.type === "wellknown") {
				...
				log.debug("fetching remote config", {
					url: `${key}/.well-known/opencode`,
				});
				const response = await fetch(`${key}/.well-known/opencode`);
				if (!response.ok) {
					throw new Error(
						`failed to fetch remote config from ${key}: ${response.status}`,
					);
				}
		...
		if (Flag.OPENCODE_CONFIG) {
			result = mergeConfigConcatArrays(
				result,
				await loadFile(Flag.OPENCODE_CONFIG),
			);
		...
		if (Flag.OPENCODE_CONFIG_CONTENT) {
			result = mergeConfigConcatArrays(
				result,
				JSON.parse(Flag.OPENCODE_CONFIG_CONTENT),
			);
		...
		for (const dir of unique(directories)) {
			if (dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
				for (const file of ["opencode.jsonc", "opencode.json"]) {
					...
					result = mergeConfigConcatArrays(
						result,
						await loadFile(path.join(dir, file)),
					);
			...
			skill: z
				.object({
					dirs: z
						...
						.array()
						.optional()
						.describe("Additional directories to search for SKILL.md files"),
				})
				.optional()
				.describe("Skills configuration, see https://opencode.ai/docs/skills"),
