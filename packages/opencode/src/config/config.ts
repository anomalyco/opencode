...
export namespace Config {
	...
	export const state = App.state("config", async (app) => {
		...
		// Load MCP configuration from standard format files
		const mcpConfig = await McpConfig.loadLocal(app.path.root);
		const convertedMcp = McpConfig.convertToOpencodeFormat(mcpConfig);
		if (Object.keys(convertedMcp).length > 0) {
			result.mcp = mergeDeep(result.mcp || {}, convertedMcp);
		}
	...
	export const Keybinds = z
		.object({
			...
			switch_mode: z
				...
				.optional()
				.default("none")
				.describe("@deprecated use switch_agent. Next mode"),
