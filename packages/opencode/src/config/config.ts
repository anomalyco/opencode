...
import type { ConsoleState } from "@leak-code/core/v1/config/console-state"
...
import { RemoteAuthError } from "@leak-code/core/v1/config/error"
import { ConfigPermissionV1 } from "@leak-code/core/v1/config/permission"
...
function globalConfigFile() {
  const candidates = ["leak-code.jsonc", "leak-code.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.LEAKCODE_CONFIG && !Flag.LEAKCODE_CONFIG_DIR && !Flag.LEAKCODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "leak-code.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "leak-code.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "LEAKCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.LEAKCODE_CONFIG) {
          yield* merge(Flag.LEAKCODE_CONFIG, yield* loadFile(Flag.LEAKCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.LEAKCODE_CONFIG })
        }

        if (!Flag.LEAKCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("leak-code", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.LEAKCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from LEAKCODE_CONFIG_DIR", { path: Flag.LEAKCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".leak-code") || dir === Flag.LEAKCODE_CONFIG_DIR) {
            for (const file of ["leak-code.json", "leak-code.jsonc"]) {
          ...
          // Auto-discovered plugins under `.leak-code/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.LEAKCODE_CONFIG_CONTENT) {
          const source = "LEAKCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.LEAKCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from LEAKCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["LEAKCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("LEAKCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.LEAKCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LEAKCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("LEAKCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.LEAKCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.LEAKCODE_DISABLE_PRUNE) {
