...
export class Service extends Context.Service<Service, Interface>()("@apex/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["apex.jsonc", "apex.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://apex.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.APEX_CONFIG && !Flag.APEX_CONFIG_DIR && !Flag.APEX_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://apex.ai/config.json" }, null, 2))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "APEX_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/apex`
        ...
        if (Flag.APEX_CONFIG) {
          yield* merge(Flag.APEX_CONFIG, yield* loadFile(Flag.APEX_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.APEX_CONFIG })
        }

        if (!Flag.APEX_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("apex", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.APEX_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from APEX_CONFIG_DIR", { path: Flag.APEX_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".apex") || dir === Flag.APEX_CONFIG_DIR) {
            for (const file of ["apex.json", "apex.jsonc"]) {
        ...
        const builtinAgentsDir = isBun
          ? path.join(path.dirname(fileURLToPath(import.meta.url)), "../../assets/agents")
          : path.join(path.dirname(process.execPath), "../assets/agents")
        result.agent = mergeDeep(result.agent ?? {}, yield* Effect.promise(() => ConfigAgent.load(builtinAgentsDir)))

        if (process.env.APEX_CONFIG_CONTENT) {
          const source = "APEX_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.APEX_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from APEX_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["APEX_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("APEX_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.APEX_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("APEX_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.APEX_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.APEX_DISABLE_PRUNE) {
