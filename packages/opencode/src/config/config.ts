...
function mergePatch(target: Info, source: Info): Info {
  const merged = mergeDeep(writable(target), writable(source))
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const update = Effect.fn("Config.update")(function* (config: Info, options?: { dispose?: boolean }) {
      ...
      yield* fs.writeFileString(file, JSON.stringify(mergePatch(existing, config), null, 2)).pipe(Effect.orDie)
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info) {
      ...
      if (!file.endsWith(".jsonc")) {
        ...
        const replaced = replaceProviderPatchJsonc(updated, input)
        next = ConfigParse.effectSchema(Info, ConfigParse.jsonc(replaced, file), file)
        yield* fs.writeFileString(file, replaced).pipe(Effect.orDie)
      }
...
export async function updateGlobal(config: Info) {
  return Effect.runPromise(
    Service.use((svc) => svc.updateGlobal(config)).pipe(Effect.scoped, Effect.provide(defaultLayer)),
  )
}
