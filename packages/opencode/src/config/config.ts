...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in imecode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for imecode serve and web commands",
  }),
...
function globalConfigFile() {
  const candidates = ["imecode.jsonc", "imecode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "imecode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "imecode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "IMECODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.IMECODE_CONFIG) {
          yield* merge(Flag.IMECODE_CONFIG, yield* loadFile(Flag.IMECODE_CONFIG, authEnv))
          log.debug("loaded custom config", { path: Flag.IMECODE_CONFIG })
        }
        ...
        if (Flag.IMECODE_CONFIG_DIR) {
          log.debug("loading config from IMECODE_CONFIG_DIR", { path: Flag.IMECODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".imecode") || dir === Flag.IMECODE_CONFIG_DIR) {
            for (const file of ["imecode.json", "imecode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.imecode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.IMECODE_CONFIG_CONTENT) {
          const source = "IMECODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.IMECODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from IMECODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["IMECODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("IMECODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.IMECODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.IMECODE_PERMISSION))
          } catch (err) {
            log.warn("IMECODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.IMECODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.IMECODE_DISABLE_PRUNE) {
