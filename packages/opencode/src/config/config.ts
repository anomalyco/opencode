...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "securecode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "securecode.jsonc"), env))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["securecode.json", "securecode.jsonc", "opencode.json", "opencode.jsonc"]) {
