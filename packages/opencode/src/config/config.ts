    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          ...
          // Upstream installed `@opencode-ai/plugin` here on every launch, pinned to the
          // running version. VeniceCode publishes no such package, so the request hit
          // npm and failed every time. Plugin authors can install the package they
