...
import { isRecord } from "@leak-code/tui/util/record"
...
import { TuiKeybind } from "@leak-code/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@leak-code/core/installation/version"
import { makeRuntime } from "@leak-code/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@leak-code/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@leak-code/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old leak-code.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.leak-code`
  // folders between cwd and home, and LEAKCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.LEAKCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit LEAKCODE_TUI_CONFIG override, if set.
  if (Flag.LEAKCODE_TUI_CONFIG) {
    const configFile = Flag.LEAKCODE_TUI_CONFIG
  ...
  // 4. `.leak-code` directories (and LEAKCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".leak-code") || dir === Flag.LEAKCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".leak-code") && dir !== Flag.LEAKCODE_CONFIG_DIR) continue
