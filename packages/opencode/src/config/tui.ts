...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, project config
  ...
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".opencode") || dir.endsWith(".mammouth") || dir === Flag.OPENCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && !dir.endsWith(".mammouth") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
