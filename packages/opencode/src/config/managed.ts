...
const MANAGED_PLIST_DOMAIN = "ai.yunpat.managed"

// Keys injected by macOS/MDM into the managed plist that are not YunPat config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/yunpat"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "yunpat")
    default:
      return "/etc/yunpat"
  }
}
...
export function managedConfigDir() {
  return process.env.YUNPAT_TEST_MANAGED_CONFIG_DIR || process.env.OPENCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
