      ...
      .add(
        ...
        HttpApiEndpoint.post("discover", `${root}/discover`, {
          ...
          OpenApi.annotations({
            ...
            description:
              "Probe an OpenAI-compatible endpoint at `{baseURL}/v1/models` and return the list of model IDs. Resolves env-var API keys on the server. Bypasses browser CORS. The server makes the request, so loopback and LAN hosts are reachable; it requires the same authorization as config writes.",
          }),
        ),
      )
      .annotateMerge(
