  ...
  .add(
    HttpApiGroup.make("instance")
      .add(
        ...
        HttpApiEndpoint.post("refreshSkills", InstancePaths.skillRefresh, {
          query: WorkspaceRoutingQuery,
          success: described(Schema.Array(Skill.Info), "List of refreshed skills"),
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "app.refreshSkills",
            summary: "Refresh skills",
            description: "Refresh remote and local skills for the current OpenCode instance.",
          }),
        ),
