...
type ServerOptions = CorsOptions & { auth?: ConfigServer.Server["auth"] }

export function createRoutes(corsOptions?: ServerOptions) {
  const authLayer = corsOptions?.auth
    ? ServerAuth.Config.layer(ServerAuth.fromConfig(corsOptions.auth))
    : ServerAuth.Config.defaultLayer
  const rootApiRoutes = HttpApiBuilder.layer(RootHttpApi).pipe(
    Layer.provide([controlHandlers, globalHandlers]),
    Layer.provide(schemaErrorLayer),
    Layer.provide(authorizationLayer.pipe(Layer.provide(authLayer))),
  )
  const instanceRouterLayer = authorizationRouterMiddleware
    .combine(instanceRouterMiddleware)
    .combine(workspaceRouterMiddleware)
    .layer.pipe(Layer.provide(Socket.layerWebSocketConstructorGlobal), Layer.provide(authLayer))
  ...
  const instanceRoutes = Layer.mergeAll(rawInstanceRoutes, instanceApiRoutes).pipe(
    Layer.provide([
      authorizationLayer.pipe(Layer.provide(authLayer)),
      workspaceRoutingLayer.pipe(Layer.provide(Socket.layerWebSocketConstructorGlobal)),
      instanceContextLayer,
      schemaErrorLayer,
    ]),
  )
  const authOnlyRouterLayer = authorizationRouterMiddleware.layer.pipe(Layer.provide(authLayer))
  return Layer.mergeAll(
    rootApiRoutes,
    eventApiRoutes,
    instanceRoutes,
    docRoute.pipe(Layer.provide(authOnlyRouterLayer)),
    uiRoute.pipe(Layer.provide(authOnlyRouterLayer)),
    ServerAuth.routes,
  ).pipe(
    Layer.provide([
      errorLayer,
...
export function webHandler(corsOptions?: ServerOptions) {
