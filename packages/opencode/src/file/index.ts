...
export type SearchResult = { files: string[]; dirs: string[] }
...
export namespace File {
  ...
  export async function search(input: {
    ...
    const root = await runPromiseInstance(FileService.use((s) => s.files()))
...
export namespace FileService {
  export interface Service {
    ...
    readonly files: () => Effect.Effect<SearchResult>
  }
}
...
export class FileService extends ServiceMap.Service<FileService, FileService.Service>()("@opencode/File") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const files = Effect.fn("FileService.files")(function* () {
        return yield* Effect.promise(async () => {
          void kick()
          return cache
        })
      ...
      return FileService.of({ init, status, read, list, search, files })
    }),
  )
}
