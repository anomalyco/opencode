...
export const advance = Effect.fn("Issue.AutoProgress.advance")(function* (
  ...
  if (head) {
    ...
    if (kids.length > 0 && kids.every((k) => DONE_OR_TERMINATED_STATUSES.has(k.status ?? ""))) {
      ...
      yield* events.publish(Issue.Event.Progressed, {
        directory,
        id: head.id,
        from: head.status,
        to: COMPLETED_STATUS,
  ...
  if (kids.length > 0) {
    yield* Effect.all(
      kids.map((k) =>
        Effect.gen(function* () {
          yield* issue.patchStatus({ directory, id: k.id, status: STARTED_STATUS })
          yield* events.publish(Issue.Event.Progressed, {
            directory,
            id: k.id,
            from: k.status,
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const tick = Effect.fn("Issue.AutoProgress.tick")(function* (directory: string) {
        ...
        yield* advance(issue, events, directory)
      })
      ...
      const start = Effect.fn("Issue.AutoProgress.start")(function* (directory: string) {
        ...
        if (!existed) yield* advance(issue, events, directory)
      })
