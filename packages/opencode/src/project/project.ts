  ...
  Effect.gen(function* () {
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const canonicalWorktreeChanged =
        previousWorktree !== result.worktree && FSUtil.resolve(previousWorktree) === FSUtil.resolve(result.worktree)
      if (
      ...
      if (projectID !== ProjectV2.ID.global) {
        const roots = canonicalWorktreeChanged ? [data.directory, previousWorktree] : [data.directory]
        yield* Effect.forEach(
          ...
          (directory) =>
            db
              ...
              .where(and(eq(SessionTable.project_id, ProjectV2.ID.global), eq(SessionTable.directory, directory)))
              .run()
              .pipe(Effect.orDie),
          { discard: true },
        )
      ...
      if (canonicalWorktreeChanged) {
        ...
        yield* db
          ...
          .where(and(eq(SessionTable.project_id, projectID), eq(SessionTable.directory, previousWorktree)))
          .run()
          .pipe(Effect.orDie)
      }
