...
export namespace PrComments {
  ...
  export async function fetch(): Promise<CommentsResponse> {
    ...
    if (!info.pr) {
      throw new PR.PrError({ code: "NO_PR", message: "No pull request found for current branch" })
    }
    if (!info.github?.authenticated || !info.github.repo) {
      throw new PR.PrError({ code: "GH_NOT_AUTHENTICATED", message: "GitHub CLI is not authenticated" })
    }
    ...
    do {
      ...
      const cmd = await withTimeout($`${args}`.quiet().nothrow().cwd(cwd), 30_000)
      ...
      if (cmd.exitCode !== 0) {
        ...
        throw new PR.PrError({
          ...
          message: stderr.trim() || `gh api graphql exited with code ${cmd.exitCode}`,
        })
      }

      if (!stdout.trim()) {
        log.error("gh api graphql returned empty response")
        throw new PR.PrError({ code: "COMMENTS_FETCH_FAILED", message: "GitHub API returned empty response" })
      }
      ...
      try {
        ...
        throw new PR.PrError({ code: "COMMENTS_FETCH_FAILED", message: "GitHub API returned invalid response" })
      }
      ...
      if (response.errors) {
        ...
        throw new PR.PrError({ code: "COMMENTS_FETCH_FAILED", message: msg || "GraphQL query failed" })
      }
