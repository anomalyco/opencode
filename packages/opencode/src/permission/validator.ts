...
import { Cause, Context, Deferred, Effect, Exit, Layer, Option, Ref } from "effect"
...
const make = (options?: Options) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // at a time and audit rows land in arrival order. A timed-out caller
      // cancels a queued job, but a job that reached the model owns the slot
      // until its real completion.
      const tails = new Map<string, Effect.Effect<void>>()
      const serial = <A, E>(
        sessionID: string,
        work: (cancelled: Ref.Ref<boolean>) => Effect.Effect<A, E>,
        timeout: number,
      ) =>
        Effect.gen(function* () {
          const prepared = yield* Effect.uninterruptible(
            Effect.gen(function* () {
              ...
              const completed = yield* Deferred.make<Exit.Exit<A, E>>()
              const job = Effect.gen(function* () {
                yield* previous
                if (yield* Ref.get(cancelled)) return
                const result = yield* Effect.exit(work(cancelled))
              ...
              yield* job.pipe(Effect.forkDetach)
              return { cancelled, completed }
            }),
          )
          const result = yield* Deferred.await(prepared.completed).pipe(
            Effect.timeoutOption(timeout),
            Effect.onInterrupt(() => Ref.set(prepared.cancelled, true)),
          )
          if (Option.isNone(result)) {
            yield* Ref.set(prepared.cancelled, true)
            return Option.none<A>()
          }
          const value = yield* Exit.match(result.value, {
      ...
      const audit = Effect.fn("PermissionValidator.audit")(function* (
        ...
        if (cancelled && (yield* Ref.get(cancelled))) return true
        ...
        const outcome = yield* decisions
          .insert({
            ...
            // Commands, paths, diffs, and tool inputs can contain secrets.
            // Keep only their count in the audit trail; the model still sees
            // the original values in buildPrompt above.
            patterns: input.patterns.map((_, index) => `<redacted:${index + 1}>`),
            // callID is an opaque transcript identifier and is safe to retain
            // for correlating the decision with the tool call.
            metadata: input.tool ? { callID: input.tool.callID } : undefined,
            verdict,
            reason,
            model,
            latencyMs,
      ...
      const run = Effect.fn("PermissionValidator.run")(function* (
        input: Permission.AutoInput,
        cancelled?: Ref.Ref<boolean>,
      ) {
        ...
        const fallback = Effect.fn("PermissionValidator.fallback")(function* (
          ...
          yield* audit(input, "fallback", model, Date.now() - started, reason, cancelled)
        ...
        if (prompt.truncated) {
          ...
          yield* audit(input, "uncertain", model, Date.now() - started, "payload truncated", cancelled)
        ...
        if (parsed.verdict === "allow") {
          const recorded = yield* audit(input, "allow", model, Date.now() - started, undefined, cancelled)
        ...
        if (parsed.verdict === "deny") {
          yield* audit(input, "deny", model, Date.now() - started, parsed.reason, cancelled)
          return yield* new PermissionV1.CorrectedError({ feedback: parsed.reason })
        }
        yield* audit(input, "uncertain", model, Date.now() - started, parsed.reason, cancelled)
      ...
      const validate: Interface["validate"] = (input) =>
        Effect.gen(function* () {
          const started = Date.now()
          const outcome = yield* serial(input.sessionID, (cancelled) => run(input, cancelled), askTimeout)
