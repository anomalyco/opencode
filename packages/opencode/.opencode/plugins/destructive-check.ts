 ...
 * 3. Log warnings and optionally block dangerous operations
 *
 * Installation:
  ...
  for (const [category, patterns] of Object.entries(DESTRUCTIVE_PATTERNS)) {
    for (const pattern of patterns) {
      if (pattern.test(command)) {
        const severity = getSeverity(category, pattern)
        return {
          category,
...
function getSeverity(category: string, _pattern: RegExp): "critical" | "high" | "medium" {
