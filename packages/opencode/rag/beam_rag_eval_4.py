#!/usr/bin/env python3
"""
BEAM Memory Evaluation — Solution 4: Chronological Hybrid RAG
架构：零成本全局背景 + BM25/Dense 双路大召回 + 强制时序重排 (解决时间线与逻辑矛盾)
"""
...
# ─── 杀手锏 1：带时序标签的上下文切分 ───
def build_solution4_db(case_id, chat_sessions, max_tokens_per_chunk=400):
    import chromadb
    print(f"\n⚙️ 正在为 Case {case_id} 构建【方案4：时序感知混合向量库】...")
    
    db_client = chromadb.PersistentClient(path="./rag_db")
    collection_name = f"beam_case_sol4_{case_id}"
    ...
    for msg in all_messages:
        if msg["role"] == "user":
            global_context = msg["content"][:300]
            break
    ...
    for i, msg in enumerate(all_messages):
        ...
        if current_tokens + tokens > max_tokens_per_chunk and current_chunk:
            ...
            # 零成本注入全局背景 + 【绝对时间序列表签】
            enriched_chunk = f"[Project Context]: {global_context}...\n[Conversation Timeline: Segment {chunk_index}]:\n{raw_chunk_text}"
    ...
    print(f"   [+] 生成 {len(documents)} 个时序 Chunk，正在写入 ChromaDB...")
    if documents:
    ...
    return collection, bm25_index, documents, metadatas, ids

# ─── 杀手锏 2：RRF混合大召回 + 强制时序重排 ───
def chronological_hybrid_search(question, collection, bm25_index, documents, metadatas, doc_ids, top_k=10):
    # 1. 扩大召回池：分别取 Top-20
    recall_size = min(20, len(documents))
    dense_results = collection.query(query_texts=[question], n_results=recall_size)
    ...
    sparse_top_ids = [x[0] for x in bm25_ranked[:recall_size]]
    ...
    # 3. 获取得分最高的 Top-K（这里我们扩大到 10，让大模型看更多的连贯上下文）
    fused_sorted = sorted(rrf_scores.items(), key=lambda x: x[1], reverse=True)
    top_k_ids = [x[0] for x in fused_sorted[:top_k]]
    
    # 4. 【核心灵魂】：不按分数排序，按时间线 (chunk_index) 强制重排！
    id_to_meta = dict(zip(doc_ids, metadatas))
    id_to_doc = dict(zip(doc_ids, documents))
    
    chronological_ids = sorted(top_k_ids, key=lambda doc_id: id_to_meta[doc_id]["chunk_index"])
    
    return [id_to_doc[doc_id] for doc_id in chronological_ids]


def rag_answer_sol4(question, collection, bm25_index, documents, metadatas, doc_ids, model_id, top_k=10):
    best_chunks = chronological_hybrid_search(question, collection, bm25_index, documents, metadatas, doc_ids, top_k=top_k)
    context_str = "\n\n========================\n\n".join(best_chunks)
    
    unique_id = str(uuid.uuid4())[:8]
    # 提示词增加时序引导
    prompt = f"""[req-{unique_id}] You are a highly accurate QA assistant. 
Please answer the question STRICTLY based on the provided [Chat History Context].
CRITICAL RULES:
1. The retrieved conversation segments are presented in **CHRONOLOGICAL ORDER**. 
2. Pay close attention to how plans, dates, or code change over time. The later segments represent the most up-to-date reality.
3. If the context does not contain the information to answer the question, reply exactly with "Insufficient information".
...
STOPWORDS = {'the', 'a', 'an', 'is', 'was', 'are', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does'}
        ...
        for q in qs:
            ...
            for field in ANSWER_FIELDS:
                if field in q and q[field]:
                    answer = str(q[field])
                    break
            if not answer and "rubric" in q and q["rubric"]: answer = "; ".join(q["rubric"])
            if answer: questions.append({"type": qtype, "question": q["question"], "expected_answer": answer})
    return questions
...
def run_single_rag_case(case_id, chat_sessions, questions, model_id):
    print(f"\n{'='*60}")
    print(f"🔬 Sol 4 RAG Case {case_id}: {len(chat_sessions)} sessions, {len(questions)} questions")
    print(f"{'='*60}")
    
    collection, bm25_index, documents, metadatas, doc_ids = build_solution4_db(case_id, chat_sessions)
    
    print(f"\n{'='*60}")
    print(f"📋 正在使用【时序感知混合大召回】(Top-10) 回答问题...")
    print(f"{'='*60}")
    ...
    for i, q in enumerate(questions):
        ...
        answer = rag_answer_sol4(q["question"], collection, bm25_index, documents, metadatas, doc_ids, model_id, top_k=10)
    ...
    print(f"\n  📊 Case {case_id} Solution 4 Summary: Avg Score = {a:.1%}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BEAM Memory Benchmark with Sol 4: Chronological Hybrid RAG")
    ...
    print(f"🚀 Sol 4 RAG Benchmark: {args.split} split, {len(case_ids)} cases")
    print(f"   Model: {PROVIDER_ID}/{MODEL_ID}")
    
    cases = load_beam(case_ids, split_name=args.split)
    os.makedirs(args.output, exist_ok=True)
    checkpoint_path = os.path.join(args.output, f"checkpoint_Sol4RAG_{PROVIDER_ID}_{MODEL_ID}_{args.split}.json")
    ...
    final_json_path = os.path.join(args.output, f"results_Scheme4_{args.split}_case{safe_cases}.json")
    table_txt_path = os.path.join(args.output, f"table_Scheme4_{args.split}_case{safe_cases}.txt")
