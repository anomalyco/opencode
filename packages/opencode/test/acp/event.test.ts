...
describe("acp event routing", () => {
  ...
  it("replays apply_patch diffs on completed tool updates", async () => {
    const harness = createHarness()
    await Effect.runPromise(harness.session.create({ id: "ses_patch_replay", cwd: "/workspace" }))
