...
describe("session.llm.stream", () => {
  ...
  test("keeps pending session start context when stream setup fails", async () => {
    ...
    // Use a port that refuses connections so stream setup fails before consuming context
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const resolved = await getModel(ProviderID.make(providerID), ModelID.make(model.id))
        ...
        // Stop the mock server so the stream setup fails with a connection error
        server.stop()
        await expect(
          drain({
            user,
