...
it.instance(
  "dispatches multiple subtasks in parallel",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Parallel" })

      // Queue responses for both subtasks (each will call the LLM once)
      yield* llm.text("result-a")
      yield* llm.text("result-b")

      const msg = yield* user(chat.id, "run two tasks")
      // Add two subtasks to the same parent message
      yield* addSubtask(chat.id, msg.id)
      yield* addSubtask(chat.id, msg.id)
      ...
      // Both subtasks should have run -- expect 2 LLM calls (one per subtask)
      expect(yield* llm.calls).toBe(2)

      // Both child assistant messages should exist
      const msgs = yield* MessageV2.filterCompactedEffect(chat.id)
      const subtaskMsgs = msgs.filter(
        (m) => m.info.role === "assistant" && m.info.agent === "general",
      )
      expect(subtaskMsgs.length).toBe(2)
    }),
  15_000,
)

it.instance(
  "all subtasks complete even if one fails",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig((url) => ({
        ...providerCfg(url),
      ...
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Partial failure" })

      yield* llm.text("ok")

      const msg = yield* user(chat.id, "two tasks one broken")
      // Two subtasks -- the agent config points to a missing model so both will
      // fail, but the parent loop should still complete (not hang).
      yield* addSubtask(chat.id, msg.id)
      yield* addSubtask(chat.id, msg.id)
      ...
      // Both subtasks should have been attempted (created as assistant messages)
      expect(subtaskMsgs.length).toBe(2)
      // Each should have an error tool part (model not found)
      for (const m of subtaskMsgs) {
        const tool = errorTool(m.parts)
