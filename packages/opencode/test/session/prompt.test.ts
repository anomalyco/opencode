  ...
  () =>
    Effect.gen(function* () {
      ...
      const tool = yield* pollWithTimeout(
        Effect.gen(function* () {
          const msgs = yield* MessageV2.filterCompactedEffect(chat.id)
          const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "recruit")
    ...
    withSh(() =>
      Effect.gen(function* () {
        ...
        const fiber = yield* prompt
          .shell({ sessionID: chat.id, agent: "recruit", command: "printf first && sleep 0.2 && printf second" })
          .pipe(Effect.forkChild)
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sh = yield* prompt
        .shell({ sessionID: chat.id, agent: "recruit", command: "sleep 0.2" })
        .pipe(Effect.forkChild)
      yield* waitForBusy(chat.id)
  ...
  () =>
    withSh(() =>
      Effect.gen(function* () {
        ...
        const sh = yield* prompt
          .shell({ sessionID: chat.id, agent: "recruit", command: ": > '.shell-ready'; sleep 30" })
          .pipe(Effect.forkChild)
        yield* pollWithTimeout(
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sh = yield* prompt.shell({ sessionID: chat.id, agent: "recruit", command: "sleep 30" }).pipe(Effect.forkChild)
      yield* waitForBusy(chat.id)
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) {
          expect(err.data.message).toContain("recruit")
        }
      }
    }),
  30_000,
