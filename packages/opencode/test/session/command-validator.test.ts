...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(rows.map((row) => row.patterns[0])).toEqual(["<redacted:1>", "<redacted:1>", "<redacted:1>"])
    }),
  { git: true },
)
  ...
  () =>
    Effect.gen(function* () {
      ...
      const validate = (pattern: string) =>
        validator.validate({ sessionID: chat.id, permission: "bash", patterns: [pattern], metadata: {} })
      const a = yield* validate("cmd-one").pipe(Effect.forkScoped)
      yield* poll(() => started.length === 1, "first validation never started")
      const b = yield* validate("cmd-two").pipe(Effect.forkScoped)
      ...
      // The second caller times out while its model call is still blocked.
      // Its queued successor must not start until the real model operation
      // releases the session slot.
      yield* Effect.sleep("1.1 seconds")
      expect(started).toEqual(["one", "two"])
      expect(yield* Fiber.join(b)).toEqual({ verdict: "fallback", reason: "timeout", model: "unknown" })
      const c = yield* validate("cmd-three").pipe(Effect.forkScoped)
      yield* Deferred.succeed(two, undefined)
