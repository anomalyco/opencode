...
export const REGISTRY: readonly Entry[] = [
  ...
  {
    file: "session/prompt.ts",
    symbol: "handleSubtask",
    claims: [
      {
        authority: "pre_fence_leased_execution",
        lines: [347, 362, 476, 479, 494, 519, 520],
        evidence:
          "Creates the subtask Assistant (`prompt.ts:347`) and running Task ToolPart (`:362`), then writes the normal terminal Assistant/ToolPart rows (`:476,479,494`) and optional synthetic continuation Message/Part (`:519,520`). `handleSubtask` takes a FRESH internal `SessionAdmission.admitted(..., { reuseAmbient: false })` before these writes, so it cannot inherit an ambient lease that predates a newly raised fence. Excluded Snapshot coverage helpers are absent from the target and contribute no writes here.",
      },
      {
        authority: "cancellation_owned_terminalization",
        lines: [444],
        evidence:
          "The `Effect.onInterrupt` callback calls `taskAbort.abort()` and finalizes the Assistant row at `prompt.ts:444`. Its ToolPart half routes through `SessionToolPart.terminalizeExact` in the same callback (`prompt.ts:439-458`), which carries `cancellation_owned_terminalization` under its own entry and performs the race-preserving check against a FRESH authoritative read rather than a caller-held Part. What remains directly attributed here is cancellation-owned because it runs only on the interrupt path and writes no generic ToolPart terminal state. Recorded as a distinct claim because collapsing it into the surrounding leased writes would erase the one authority story K107 needs from this file.",
      },
    ],
  },
  {
    ...
    uncertain:
      "Mixed authority in one binding: the immediate call from `handleSubtask` IS leased, only the stored promotion callback is not. K108 DELIBERATELY DID NOT ADD THE GUARD HERE, and the reason is structural rather than budgetary. (1) `background-job.ts:924` runs the callback as `result.onPromote.pipe(Effect.ignore)`, so a fenced rejection would be swallowed and could not satisfy §7.6's requirement that a post-fence mutation be \"rejected immediately through its existing domain/HTTP/SDK error path\" — the HTTP promote at `handlers/experimental.ts:158-170` would still answer success. (2) Decisively, `task.ts:1048-1054` composes `onPromote: Effect.all([ctx.metadata({...}), attach()])`. `Effect.all` is sequential and short-circuits, so a metadata guard that failed under a fence would SKIP `attach()` — a behaviour change to CP-021's attachment path, which Gate 3 is barred from touching and Gate 8 owns. A correct fix must therefore reorder or isolate the two effects, which is a CP-021 integration decision, not a mutation-lease decision. Promotion-time authority is already the Gate-4-keyed one-shot-transfer item (§7.3 row 15), so the guard belongs there and must be co-designed with Gate 8.",
