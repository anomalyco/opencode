...
describe("server /event", () => {
  ...
  test("emits stream.lagged on /event overflow", async () => {
    await using tmp = await tmpdir({ git: true })
    const prior = Bus.subscribeAll
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        // @ts-expect-error test override
    ...
    Bus.subscribeAll = prior
  })

  test("emits stream.lagged on /global/sync-event overflow", async () => {
    await using tmp = await tmpdir({ git: true })
    const prior = SyncEvent.subscribeAll
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        // @ts-expect-error test override
        ...
        ;(SyncEvent as unknown as { subscribeAll: typeof SyncEvent.subscribeAll }).subscribeAll = ((
          callback: (event: any) => void,
        ) => {
          for (let i = 0; i < 2000; i++) {
            callback({
              def: { type: "test.sync", version: 1 },
              event: {
                id: `event_${i}`,
                seq: i,
                aggregateID: "agg",
        ...
        }) as typeof SyncEvent.subscribeAll

        const stream = await app.request("/global/sync-event")
        expect(stream.status).toBe(200)
        const body = await stream.text()
        expect(body).toContain("server.stream.lagged")
      },
    })
    ;(SyncEvent as unknown as { subscribeAll: typeof SyncEvent.subscribeAll }).subscribeAll = prior
  })
})
