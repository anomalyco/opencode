...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.APEX_SERVER_PASSWORD
  else process.env.APEX_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.APEX_SERVER_USERNAME
  else process.env.APEX_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.APEX_SERVER_PASSWORD = auth.password
  Flag.APEX_SERVER_USERNAME = auth.username
  process.env.APEX_SERVER_PASSWORD = auth.password
  process.env.APEX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.APEX_SERVER_PASSWORD = undefined
  Flag.APEX_SERVER_USERNAME = auth.username
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.APEX_DISABLE_DEFAULT_PLUGINS
    process.env.APEX_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.APEX_DISABLE_DEFAULT_PLUGINS
      else process.env.APEX_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
