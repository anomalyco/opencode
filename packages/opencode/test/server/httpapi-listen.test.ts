...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.LEAKCODE_SERVER_PASSWORD
  else process.env.LEAKCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.LEAKCODE_SERVER_USERNAME
  else process.env.LEAKCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.LEAKCODE_SERVER_PASSWORD = auth.password
  Flag.LEAKCODE_SERVER_USERNAME = auth.username
  process.env.LEAKCODE_SERVER_PASSWORD = auth.password
  process.env.LEAKCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.LEAKCODE_SERVER_PASSWORD = undefined
  Flag.LEAKCODE_SERVER_USERNAME = auth.username
  delete process.env.LEAKCODE_SERVER_PASSWORD
  process.env.LEAKCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.LEAKCODE_DISABLE_DEFAULT_PLUGINS
    process.env.LEAKCODE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.LEAKCODE_DISABLE_DEFAULT_PLUGINS
      else process.env.LEAKCODE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
