...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.MOKS_SERVER_PASSWORD
  else process.env.MOKS_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.MOKS_SERVER_USERNAME
  else process.env.MOKS_SERVER_USERNAME = original.envUsername
...
async function startListener() {
  ...
  process.env.MOKS_SERVER_PASSWORD = auth.password
  process.env.MOKS_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.OPENCODE_SERVER_USERNAME = auth.username
  delete process.env.MOKS_SERVER_PASSWORD
  process.env.MOKS_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.MOKS_DISABLE_DEFAULT_PLUGINS
    process.env.MOKS_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.MOKS_DISABLE_DEFAULT_PLUGINS
      else process.env.MOKS_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
