...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      ...
      envPassword: process.env.LEAKCODE_SERVER_PASSWORD,
      envUsername: process.env.LEAKCODE_SERVER_USERNAME,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("LEAKCODE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("LEAKCODE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            LEAKCODE_SERVER_PASSWORD: input?.password,
            LEAKCODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
