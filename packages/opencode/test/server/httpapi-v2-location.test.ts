...
describe("v2 location HttpApi", () => {
  ...
  test("rejects foreign location directory query hints before v2 location lookup", async () => {
    ...
    const response = await request(
      `/api/fs/find?location[directory]=${encodeURIComponent("C:\\Work\\Repo")}&query=package`,
      process.cwd(),
    )
  ...
  test("rejects encoded foreign directory headers before v2 location lookup", async () => {
    ...
    const response = await request("/api/fs/find?query=package", encodeURIComponent("C:\\Work\\Repo"))
