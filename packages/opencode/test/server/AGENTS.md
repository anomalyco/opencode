...
- Use tiny `HttpApiBuilder` probe groups that declare the typed middleware under test and expose context such as `RouteContext` or `InstanceRef`.
- Declare middleware in the same order as production when testing interactions, for example `RouteContextMiddleware` followed by `InstanceContextMiddleware`.
