...
describeWatcher("FileWatcher", () => {
  ...
  it.instance(
    "publishes .git/logs/HEAD events for same-branch head movement",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const fs = yield* AppFileSystem.Service
        const git = yield* Git.Service
        const headLog = path.join(test.directory, ".git", "logs", "HEAD")
        const realHeadLog = yield* Effect.promise(() => realpath(headLog).catch(() => headLog))

        yield* withWatcher(
          test.directory,
          nextUpdate(
            test.directory,
            (evt) => (evt.file === headLog || evt.file === realHeadLog) && evt.event !== "unlink",
            fs
              .writeFileString(path.join(test.directory, "tracked.txt"), "same branch update")
              .pipe(
                Effect.andThen(git.run(["add", "tracked.txt"], { cwd: test.directory })),
                Effect.andThen(git.run(["commit", "-m", "same branch update"], { cwd: test.directory })),
              ),
          ).pipe(
            Effect.tap((evt) =>
              Effect.sync(() => {
                expect([headLog, realHeadLog]).toContain(evt.file)
                expect(["add", "change"]).toContain(evt.event)
              }),
            ),
          ),
