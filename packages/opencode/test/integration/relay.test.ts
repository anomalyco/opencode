...
describe("Relay SDK + Testcontainer", () => {
  ...
  test("health check shows relay is ready", { timeout: TEST_TIMEOUT }, async () => {
  ...
  test("agent connects to relay", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx) => {
      expect(ctx.agent.isConnected()).toBe(true)
    })
  })

  test("browser connects to relay", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (_, browser) => {
      expect(browser.isConnected()).toBe(true)
    })
  })

  test("agent sends request and receives response from browser", { timeout: TEST_TIMEOUT }, async () => {
  ...
  test("multiple requests get correct responses", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx, browser) => {
  ...
  test("unknown operation returns error", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx) => {
      try {
  ...
  test("Univer SDK command reaches browser", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx, browser) => {
      ...
      expect(receivedCommand).toBe("RangeRect.create")
  ...
  test("complex Univer workflow: create range → modify → get value", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx, browser) => {
      ...
      browser.on("univer.execute", async (params: any) => {
        switch (params.command) {
          ...
          case "RangeRect.getValue":
            ...
            return { value: `data_for_${params.params.id}`, range }
  ...
  test("backend code reaches frontend and we see it from SDK", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx, browser) => {
  ...
  test("relay forwards commands and deploys SDK commands to browser", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx, browser) => {
  ...
  test("browser disconnection notifies agent", { timeout: TEST_TIMEOUT }, async () => {
    let ctx: any
  ...
  test("health shows browser connected", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx) => {
      const health = await ctx.agent.health()
  ...
  test("big pickle message flows through relay", { timeout: TEST_TIMEOUT }, async () => {
    await withRelay(async (ctx, browser) => {
      ...
      expect(receivedMessage).toBe("Hi Big Pickle! This is an integration test!")
