...
void mock.module("@modelcontextprotocol/sdk/client/index.js", () => ({
  Client: class MockClient {
    ...
    async listTools(params?: { cursor?: string }) {
      ...
      const page = this._state?.toolPages[params?.cursor ?? ""]
      if (page) return page
    ...
    async request(
      ...
      if (request.method === "tools/list") {
        return schema.parse(this._state?.toolPages[request.params?.cursor ?? ""] ?? { tools: this._state?.tools ?? [] })
      }
    ...
    async listPrompts(params?: { cursor?: string }) {
      ...
      const page = this._state?.promptPages[params?.cursor ?? ""]
      if (page) return page
    ...
    async listResources(params?: { cursor?: string }) {
      ...
      const page = this._state?.resourcePages[params?.cursor ?? ""]
      if (page) return page
    ...
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        ...
        serverState.resourcePages = {
          "": { resources: [{ name: "resource-one", uri: "test://one" }], nextCursor: "resources-2" },
