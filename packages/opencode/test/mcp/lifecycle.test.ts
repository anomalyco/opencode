...
it.instance(
  "uses the experimental MCP timeout as the request fallback",
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        lastCreatedClientName = "fallback-server"
        const serverState = getOrCreateClientState("fallback-server")

        yield* mcp.add("fallback-server", {
          type: "local",
          command: ["echo", "test"],
        })
        yield* mcp.getPrompt("fallback-server", "test")
        yield* mcp.readResource("fallback-server", "test://resource")

        expect(serverState.getPromptTimeout).toBe(5000)
        expect(serverState.readResourceTimeout).toBe(5000)
      }),
    ),
  { config: { mcp: {}, experimental: { mcp_timeout: 5000 } } },
)
