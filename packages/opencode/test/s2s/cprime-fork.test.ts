...
function makeRunLoopLayer() {
  ...
  // ONE S2SStore instance over the SAME Database as everything else (deps
  // carries Database.defaultLayer). Used by the registry/prompt provides AND
  // exposed in the output so the test body can assert against the same DB the
  // forked C′ poller claims in.
  const s2sStore = S2SStore.layer.pipe(Layer.provideMerge(deps))
  ...
  const registry = ToolRegistry.layer.pipe(
    ...
    Layer.provideMerge(s2sStore),
    Layer.provide(s2sFlags),
  ...
  // S2SStore must be provided to the prompt layer too — the C′ fork resolves
  // Database via serviceOption, but the forked poller inherits S2SStore +
  // Messaging from this context.
...
const it = testEffect(reproLayer)
