...
describe("tool.question", () => {
  ...
  // Note: Qwen-style malformed input handling (stringified JSON / options:[] /
  // single object / bare string) was previously tested here via
  // `Schema.decodeTo` preprocess. The normalize logic now lives in the
  // `qwen-question-normalize` plugin (issue #119), which runs in the
  // `tool.execute.before` hook before the strict schema decode. End-to-end
  // tests for the normalize path live in
  // `test/securecode/plugins/qwen-question-normalize.test.ts`.
