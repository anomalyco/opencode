...
const toolDefinitionInputs: ToolDefinitionInput[] = []

const capturePluginLayer = Layer.mock(Plugin.Service)({
  trigger: <Name extends string, Input, Output>(name: Name, input: Input, output: Output) =>
    Effect.sync(() => {
      if (name === "tool.definition") toolDefinitionInputs.push(input as ToolDefinitionInput)
      return output
    }),
  list: () => Effect.succeed([]),
  init: () => Effect.void,
...
function makeRegistryLayer(pluginLayer = Plugin.defaultLayer) {
...
const it = testEffect(Layer.mergeAll(makeRegistryLayer(), node, Agent.defaultLayer))
const withCapturePlugin = testEffect(Layer.mergeAll(makeRegistryLayer(capturePluginLayer), node, Agent.defaultLayer))
afterEach(async () => {
...
describe("tool.registry", () => {
  ...
  withCapturePlugin.instance("passes session, model, and agent to tool.definition hooks", () =>
    Effect.gen(function* () {
      toolDefinitionInputs.length = 0
      ...
      const input = toolDefinitionInputs.find((item) => item.toolID === "read")
      expect(input).toMatchObject({
        toolID: "read",
        sessionID,
        model: { providerID: ProviderID.anthropic, modelID: "claude-sonnet-4-6" },
