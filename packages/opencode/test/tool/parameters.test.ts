...
describe("tool parameters", () => {
  ...
  describe("question", () => {
    ...
    // Note: Qwen-style malformed input normalization (stringified JSON / single
    // object / bare string) is handled by the `qwen-question-normalize` plugin
    // via the `tool.execute.before` hook, not at the schema level. The schema
    // here stays strict (`Array(Question.Prompt)`) — see
    // `test/securecode/plugins/qwen-question-normalize.test.ts` for the
    // normalize coverage.
  })
