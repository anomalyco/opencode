...
const remappedStorage = (root: string) =>
  Layer.fresh(Storage.layer.pipe(Layer.provide(remapped(root)), Layer.provide(Git.defaultLayer)))
...
const writeText = Effect.fnUntraced(function* (file: string, value: string) {
  const fs = yield* AppFileSystem.Service
  yield* fs.makeDirectory(path.dirname(file), { recursive: true })
...
describe("Storage", () => {
  ...
  it.live("migration 2 runs when marker contents are invalid", () =>
    Effect.gen(function* () {
      ...
      yield* writeText(path.join(storage, "migration"), "wat")
      yield* writeJson(path.join(storage, "session", "proj_test", "ses_test.json"), {
        id: "ses_test",
        projectID: "proj_test",
        title: "legacy",
        summary: { diffs },
  ...
  it.live("migration 1 tolerates malformed legacy records", () =>
    Effect.gen(function* () {
      ...
      yield* writeJson(path.join(legacy, "storage", "session", "message", "probe", "0.json"), [])
      yield* writeJson(path.join(legacy, "storage", "session", "message", "probe", "1.json"), {
        path: { root: tmp },
      })
      yield* writeJson(path.join(legacy, "storage", "session", "info", "ses_legacy.json"), {
        id: "ses_legacy",
        title: "legacy",
      })
      yield* writeJson(path.join(legacy, "storage", "session", "message", "ses_legacy", "msg_legacy.json"), {
        role: "user",
        text: "hello",
      })
  ...
  it.live("failed migrations do not advance the marker", () =>
    Effect.gen(function* () {
      ...
      yield* writeText(path.join(legacy, "storage", "session", "message", "probe", "0.json"), "{")
