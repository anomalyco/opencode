...
describe("selfRespawnArgv", () => {
  ...
  test("compiled binary mode works for downstream-forked names", () => {
    expect(selfRespawnArgv(["/usr/local/bin/securecode", "pr", "123"])).toEqual(["/usr/local/bin/securecode"])
...
describe("selfDisplayName", () => {
  test("returns binary basename for compiled binaries", () => {
    ...
    expect(selfDisplayName(["/usr/local/bin/securecode"])).toBe("securecode")
  })
