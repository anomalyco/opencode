...
describe("migrateFromGlobal", () => {
  ...
  it.live("repairs session directories when the project worktree path is canonicalized", () =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped({ git: true })
      const projects = yield* Project.Service
      const { project } = yield* projects.fromDirectory(tmp)
      const id = legacySessionID()
      const alias = `${tmp}${path.sep}.`

      yield* Database.Service.use(({ db }) =>
        db
          .update(ProjectTable)
          .set({ worktree: AbsolutePath.make(alias) })
          .where(eq(ProjectTable.id, project.id))
          .run()
          .pipe(Effect.orDie),
      )
      yield* seed({ id, dir: alias, project: project.id })
