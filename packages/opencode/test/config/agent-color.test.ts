...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(cfg.agent?.["recruit"]?.color).toBe("#FFA500")
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const recruit = yield* AgentSvc.use.get("recruit")
      expect(recruit?.color).toBe("accent")
    }),
  {
    git: true,
    config: {
