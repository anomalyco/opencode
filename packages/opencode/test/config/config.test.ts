...
const managedConfigDir = process.env.IMECODE_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.IMECODE_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.IMECODE_CONSOLE_TOKEN
  else process.env.IMECODE_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".imecode"), schemaConfig(input.local)) : undefined,
      ].filter(
...
it.effect("does not create global config when IMECODE_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:IMECODE_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.instance("loads config from .opencode directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .opencode/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .opencode/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .opencode/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only IMECODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("IMECODE_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable IMECODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "IMECODE_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".imecode", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
// Note: preload.ts sets IMECODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("local .opencode config can override MCP from project config", () =>
  Effect.gen(function* () {
    ...
    yield* AppFileSystem.use.ensureDir(path.join(test.directory, ".imecode"))
    yield* writeConfigEffect(
      path.join(test.directory, ".imecode"),
      {
        $schema: "https://opencode.ai/config.json",
        mcp: {
          docs: {
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".imecode", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("IMECODE_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .opencode/ directories when flag is set", () =>
    withProcessEnv(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".imecode", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { IMECODE_CONFIG_DIR: undefined, IMECODE_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "IMECODE_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { IMECODE_DISABLE_PROJECT_CONFIG: "true", IMECODE_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed IMECODE_PERMISSION JSON used to crash
...
describe("IMECODE_PERMISSION env var", () => {
  it.instance("does not crash when IMECODE_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "IMECODE_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("IMECODE_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in IMECODE_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "IMECODE_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in IMECODE_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
