...
test("handles experimental.skip_models_refresh configuration", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await Config.get()
      expect(config.experimental?.skip_models_refresh).toBe(true)
    },
  })
})

test("handles experimental.skip_models_refresh set to false", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await Config.get()
      expect(config.experimental?.skip_models_refresh).toBe(false)
    },
  })
})
