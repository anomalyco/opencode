...
test("rejects built-in LSP server with env override", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
...
test("rejects built-in LSP server with initialization override", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
...
test("rejects built-in LSP server with extensions override", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
...
test("accepts custom LSP server with command, env, and initialization", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(config.lsp).not.toBe(false)
      if (config.lsp && typeof config.lsp === "object") {
        expect(config.lsp["my-custom-lsp"]).toMatchObject({
          command: ["my-lsp", "--stdio"],
          extensions: [".custom"],
          env: { CUSTOM_VAR: "value" },
          initialization: { some: "option" },
...
test("accepts custom LSP server with command overriding built-in server", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({
          ...
          lsp: {
            typescript: {
              ...
              extensions: [".ts", ".tsx"],
              env: { CUSTOM_VAR: "value" },
              initialization: { preferences: { importModuleSpecifier: "relative" } },
              timeout: 120000,
            },
          },
        }),
