...
it.instance("agent .md with frontmatter prompt and empty body uses frontmatter prompt", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".opencode", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".opencode", "agent", "frontmatter-only.md"),
      `---
model: test/model
...
it.instance("agent .md with both body and frontmatter prompt uses body", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".opencode", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".opencode", "agent", "both.md"),
      `---
model: test/model
...
it.instance("agent .md prompt resolves {env:VAR} substitution", () =>
  withProcessEnv(
    "__TEST_AGENT_VAR",
    "resolved-value",
    Effect.gen(function* () {
      ...
      yield* mkdirEffect(path.join(test.directory, ".opencode", "agent"))
      yield* writeTextEffect(
        path.join(test.directory, ".opencode", "agent", "env-sub.md"),
        `---
model: test/model
---
...
it.instance("agent .md frontmatter prompt resolves {file:path} substitution", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".opencode", "agent"))
    yield* writeTextEffect(path.join(test.directory, "system-prompt.txt"), "File content here")
    yield* writeTextEffect(
      path.join(test.directory, ".opencode", "agent", "file-sub.md"),
      `---
model: test/model
