...
import { EventSequenceTable } from "@leak-code/core/event/sql"
...
import { WorkspaceV2 } from "@leak-code/core/workspace"
import { WorkspaceTable } from "@leak-code/core/control-plane/workspace.sql"
...
const originalEnv = {
  LEAKCODE_AUTH_CONTENT: process.env.LEAKCODE_AUTH_CONTENT,
  LEAKCODE_EXPERIMENTAL_WORKSPACES: process.env.LEAKCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.LEAKCODE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.LEAKCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.LEAKCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.LEAKCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.LEAKCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
