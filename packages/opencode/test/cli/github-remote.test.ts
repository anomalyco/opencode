...
test("parses git@ URL with .git suffix", () => {
  expect(parseGitHubRemote("git@github.com:anomalyco/opencode.git")).toEqual({ owner: "anomalyco", repo: "opencode" })
})

test("parses git@ URL without .git suffix", () => {
  expect(parseGitHubRemote("git@github.com:anomalyco/opencode")).toEqual({ owner: "anomalyco", repo: "opencode" })
})

test("parses ssh:// URL with .git suffix", () => {
  expect(parseGitHubRemote("ssh://git@github.com/anomalyco/opencode.git")).toEqual({
    owner: "anomalyco",
    repo: "opencode",
  })
})
...
test("parses ssh:// URL without .git suffix", () => {
  expect(parseGitHubRemote("ssh://git@github.com/anomalyco/opencode")).toEqual({ owner: "anomalyco", repo: "opencode" })
})
