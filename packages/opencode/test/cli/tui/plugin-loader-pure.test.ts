...
`,
  ...
  const pure = process.env.LEAKCODE_PURE
  const meta = process.env.LEAKCODE_PLUGIN_META_FILE
  process.env.LEAKCODE_PURE = "1"
  process.env.LEAKCODE_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.LEAKCODE_PURE
    } else {
      process.env.LEAKCODE_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.LEAKCODE_PLUGIN_META_FILE
    } else {
      process.env.LEAKCODE_PLUGIN_META_FILE = meta
    }
  }
})
