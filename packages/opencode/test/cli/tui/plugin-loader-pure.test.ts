...
`,
  ...
  const pure = process.env.APEX_PURE
  const meta = process.env.APEX_PLUGIN_META_FILE
  process.env.APEX_PURE = "1"
  process.env.APEX_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.APEX_PURE
    } else {
      process.env.APEX_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.APEX_PLUGIN_META_FILE
    } else {
      process.env.APEX_PLUGIN_META_FILE = meta
    }
  }
})
