...
test("build agent has correct default properties", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(evalPerm(build, "pyodide")).toBe("allow")
    },
  })
})
...
test("explore agent denies edit and write", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(evalPerm(explore, "pyodide")).toBe("allow")
    },
  })
})
...
test("compaction agent denies all permissions", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(evalPerm(compaction, "pyodide")).toBe("deny")
...
test("agent permission config merges with defaults", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("pyodide", "raise SystemExit(1)", build!.permission).action).toBe("deny")
      // Edit still allowed
      expect(evalPerm(build, "edit")).toBe("allow")
    },
  })
...
test("global permission config applies to all agents", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const build = await Agent.get("build")
      expect(build).toBeDefined()
      expect(evalPerm(build, "pyodide")).toBe("deny")
    },
  })
})
