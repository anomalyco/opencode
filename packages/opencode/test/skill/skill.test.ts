...
async function createGlobalSkill(home: string) {
  const dir = path.join(home, ".claude", "skills", "global-test-skill")
...
`,
  ...
  it.live("loads legacy inline skills from opencode.json", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
          ...
          expect(list[0].description).toBe("Test skill")
          expect(list[0].location).toBe(path.join(dir, "opencode.json"))
          expect(list[0].content).toContain("echo test")
        }),
      { git: true },
    ),
  )
  ...
  it.live("discovers global skills from ~/.agents/skills/ directory", () =>
    Effect.gen(function* () {
      ...
      yield* withHome(
        tmp.path,
        Effect.gen(function* () {
          const dir = path.join(tmp.path, ".agents", "skills", "global-agent-skill")
