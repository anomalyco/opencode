...
it.live("skills start enabled and can be disabled and re-enabled", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
...
`,
        ...
        expect(yield* skill.isEnabled("toggle-skill")).toBe(true)
        expect(yield* skill.status()).toEqual({ "toggle-skill": { status: "enabled" } })

        yield* skill.disable("toggle-skill")
        expect(yield* skill.isEnabled("toggle-skill")).toBe(false)
        expect(yield* skill.status()).toEqual({ "toggle-skill": { status: "disabled" } })
...
it.live("skills can start disabled from config", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          Bun.write(
            path.join(dir, ".opencode", "skill", "startup-skill", "SKILL.md"),
            `---
name: startup-skill
...
`,
        ...
        expect(yield* skill.isEnabled("startup-skill")).toBe(false)
        expect(yield* skill.status()).toEqual({ "startup-skill": { status: "disabled" } })

        yield* skill.enable("startup-skill")
        expect(yield* skill.isEnabled("startup-skill")).toBe(true)
      }),
    {
      git: true,
      config: {
