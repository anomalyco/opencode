...
describe("PermissionNext.evaluateWithSession for session-scoped patterns", () => {
  const createRuleset = (rules: Record<string, "allow" | "deny" | "ask">): PermissionNext.Ruleset =>
    Object.entries(rules).map(([pattern, action]) => ({
      permission: "edit",
      pattern,
  ...
  test("works with wildcard before sessionID placeholder", () => {
    ...
    expect(
      PermissionNext.evaluateWithSession("edit", "**/plan/session_xyz/step2.md", "session_xyz", ruleset).action,
    ).toBe("allow")
  })

  test("session-scoped allow followed by global deny", () => {
  ...
  test("returns ask when no match", () => {
    expect(PermissionNext.evaluateWithSession("edit", "/some/path.ts", "session_123", []).action).toBe("ask")
  })
...
// Integration tests for session-scoped permissions with config files
describe("session-scoped permissions with config files", () => {
  test("loads session-scoped edit permissions from config", async () => {
    await using tmp = await tmpdir({
      git: true,
      config: {
        permission: {
  ...
  test("session-scoped permissions work with evaluateWithSession but not evaluate", async () => {
    await using tmp = await tmpdir({
      git: true,
      config: {
        permission: {
          edit: {
            "*/plan/{sessionID}.md": "allow",
          },
        },
      },
    })
