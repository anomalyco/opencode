...
//   - MOKS_CONFIG_CONTENT      : provider config inline, no files to find
//   - MOKS_TEST_HOME           : pins os.homedir() → tmpdir
//   - MOKS_DISABLE_PROJECT_CONFIG : skip walking up for moks.json
//   - MOKS_PURE                : skip external plugin discovery + install
//   - MOKS_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    MOKS_CONFIG_CONTENT: configJson,
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          MOKS_CONFIG_CONTENT: JSON.stringify({
