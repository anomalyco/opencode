...
//   - IMECODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - IMECODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - IMECODE_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - IMECODE_PURE                : skip external plugin discovery + install
//   - IMECODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    IMECODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    IMECODE_CONFIG_CONTENT: configJson,
    IMECODE_DISABLE_PROJECT_CONFIG: "1",
    IMECODE_PURE: "1",
    IMECODE_DISABLE_AUTOUPDATE: "1",
    IMECODE_DISABLE_AUTOCOMPACT: "1",
    IMECODE_DISABLE_MODELS_FETCH: "1",
    IMECODE_AUTH_CONTENT: "{}",
  }
}
