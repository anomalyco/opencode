...
describe("GitLab Duo: toolExecutor wiring", () => {
  test("toolExecutor returns error for unknown tool", async () => {
    const executor = async (toolName: string, argsJson: string, requestID: string) => {
    ...
    const res = await executor("nonexistent_tool", "{}", "req-1")
    expect(res.error).toBe("Unknown tool: nonexistent_tool")
    expect(res.result).toBe("")
  })

  test("toolExecutor executes tool and returns string result", async () => {
    const executor = async (toolName: string, argsJson: string, requestID: string) => {
      const tools: Record<string, any> = {
        test_tool: {
          execute: async (args: any) => "tool output: " + args.input,
        },
      }
      const t = tools[toolName]
      if (!t || !t.execute) {
    ...
    const res = await executor("test_tool", '{"input":"hello"}', "req-2")
    expect(res.result).toBe("tool output: hello")
    expect(res.error).toBeUndefined()
  })

  test("toolExecutor returns JSON-stringified result for object returns", async () => {
    const executor = async (toolName: string, argsJson: string, requestID: string) => {
      const tools: Record<string, any> = {
        obj_tool: {
          execute: async () => ({ output: "file content", title: "Read file", metadata: { path: "/tmp/test" } }),
        },
      }
      const t = tools[toolName]
      if (!t || !t.execute) {
    ...
    const res = await executor("obj_tool", "{}", "req-3")
    expect(res.result).toBe("file content")
    expect(res.title).toBe("Read file")
    expect(res.metadata).toEqual({ path: "/tmp/test" })
  })

  test("toolExecutor returns error message on tool execution failure", async () => {
    const executor = async (toolName: string, argsJson: string, requestID: string) => {
      const tools: Record<string, any> = {
        fail_tool: {
          execute: async () => {
    ...
    const res = await executor("fail_tool", "{}", "req-4")
