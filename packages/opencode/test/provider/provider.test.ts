...
const set = async (ctx: InstanceContext, k: string, v: string) => {
  rememberEnv(k)
  process.env[k] = v
  return env.runPromise((svc) => svc.set(k, v).pipe(Effect.provideService(InstanceRef, ctx)))
}
const remove = async (ctx: InstanceContext, k: string) => {
  rememberEnv(k)
...
test("provider.reload picks up api auth changes", async () => {
  ...
  await withTestInstance({
    ...
    fn: async (ctx) => {
      ...
      expect(first[ProviderID.anthropic]).toBeUndefined()

      await AppRuntime.runPromise(
        Effect.gen(function* () {
          const auth = yield* Auth.Service
          yield* auth.set("anthropic", { type: "api", key: "test-api-key" }).pipe(Effect.orDie)
        }).pipe(Effect.provideService(InstanceRef, ctx)),
      )

      const stale = await list(ctx)
      expect(stale[ProviderID.anthropic]).toBeUndefined()

      await run(ctx, (provider) => provider.reload())

      const updated = await list(ctx)
      expect(updated[ProviderID.anthropic]).toBeDefined()
      expect(updated[ProviderID.anthropic].source).toBe("api")
      expect(updated[ProviderID.anthropic].key).toBe("test-api-key")
    },
  })
})
