...
describe("normalizeModelName", () => {
  ...
  test("converts claude-opus-4-6 to claude-opus-4.6", () => {
    expect(normalizeModelName("claude-opus-4-6")).toBe("claude-opus-4.6")
  })

  test("converts claude-sonnet-4-6 to claude-sonnet-4.6", () => {
...
describe("variant extraction (mirrors parseModelSelection from acp/agent.ts)", () => {
  const kiroModels: Record<string, { variants?: Record<string, any> }> = {
    "claude-sonnet-4-5": { variants: { high: {}, max: {} } },
    "claude-opus-4-5": { variants: { high: {}, max: {} } },
    "claude-opus-4-6": { variants: { high: {}, max: {} } },
  ...
  function extractVariant(modelID: string) {
    if (kiroModels[modelID]) return { baseModel: modelID, variant: undefined }
    const segments = modelID.split("/")
    if (segments.length > 1) {
      const candidate = segments[segments.length - 1]
      const base = segments.slice(0, -1).join("/")
      const info = kiroModels[base]
      if (info?.variants && candidate in info.variants) {
        return { baseModel: base, variant: candidate }
      }
    }
    return { baseModel: modelID, variant: undefined }
  }

  test("extracts high/max variants from reasoning models", () => {
    expect(extractVariant("claude-opus-4-6/high")).toEqual({ baseModel: "claude-opus-4-6", variant: "high" })
    expect(extractVariant("claude-opus-4-6/max")).toEqual({ baseModel: "claude-opus-4-6", variant: "max" })
    expect(extractVariant("claude-sonnet-4-5/high")).toEqual({ baseModel: "claude-sonnet-4-5", variant: "high" })
    expect(extractVariant("claude-sonnet-4-6/max")).toEqual({ baseModel: "claude-sonnet-4-6", variant: "max" })
  })

  test("passes through base models without variant", () => {
    expect(extractVariant("claude-opus-4-6")).toEqual({ baseModel: "claude-opus-4-6", variant: undefined })
    expect(extractVariant("claude-haiku-4-5")).toEqual({ baseModel: "claude-haiku-4-5", variant: undefined })
  })

  test("does not extract variant from non-reasoning models", () => {
    expect(extractVariant("claude-haiku-4-5/high")).toEqual({ baseModel: "claude-haiku-4-5/high", variant: undefined })
  })
