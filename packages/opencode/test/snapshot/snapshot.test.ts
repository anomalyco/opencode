...
it.instance(
  "restore with invalid hash fails",
  Effect.gen(function* () {
    yield* bootstrap()
    const snapshot = yield* Snapshot.Service
    expect(yield* failureMessage(snapshot.restore("invalid-hash-12345"))).toContain("exited with code")
  }),
  { git: true },
)
...
it.instance(
  "track fails closed when nested git repo has no checkout",
  Effect.gen(function* () {
    const tmp = yield* bootstrap()
    const snapshot = yield* Snapshot.Service
    yield* write(`${tmp.path}/file.txt`, "v1\n")
    expect(yield* snapshot.track()).toBeTruthy()
    yield* write(`${tmp.path}/file.txt`, "v2\n")
    yield* mkdirp(`${tmp.path}/nested-unborn`)
    yield* exec(`${tmp.path}/nested-unborn`, ["git", "init"])
    yield* write(`${tmp.path}/nested-unborn/file.txt`, "nested\n")
    expect(yield* failureMessage(snapshot.track())).toContain("does not have a commit checked out")
  }),
  { git: true },
)
