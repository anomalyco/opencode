...
// Snapshot.patch returns forward-slash paths; normalize tmp.path to match on Windows
...
test("tracks deleted files correctly", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect((await Snapshot.patch(before!)).files).toContain(`${posix(tmp.path)}/a.txt`)
    },
  })
})
...
test("binary file handling", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(patch.files).toContain(`${posix(tmp.path)}/image.png`)
...
test("symlink handling", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect((await Snapshot.patch(before!)).files).toContain(`${posix(tmp.path)}/link.txt`)
    },
  })
})
...
test("large file handling", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect((await Snapshot.patch(before!)).files).toContain(`${posix(tmp.path)}/large.txt`)
    },
  })
})
...
test("special characters in filenames", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(files).toContain(`${posix(tmp.path)}/file with spaces.txt`)
      expect(files).toContain(`${posix(tmp.path)}/file-with-dashes.txt`)
      expect(files).toContain(`${posix(tmp.path)}/file_with_underscores.txt`)
    },
  })
})
...
test("unicode filenames", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      for (const file of unicodeFiles) {
        expect(patch.files).toContain(posix(file.path))
      }
...
test.skip("unicode filenames modification and restore", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const patch = await Snapshot.patch(before!)
      expect(patch.files).toContain(posix(chineseFile))
      expect(patch.files).toContain(posix(cyrillicFile))
...
test("unicode filenames in subdirectories", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const patch = await Snapshot.patch(before!)
      expect(patch.files).toContain(posix(deepFile))
...
test("very long filenames", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const patch = await Snapshot.patch(before!)
      expect(patch.files).toContain(posix(longFile))
...
test("hidden files", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const patch = await Snapshot.patch(before!)
      expect(patch.files).toContain(`${posix(tmp.path)}/.hidden`)
      expect(patch.files).toContain(`${posix(tmp.path)}/.gitignore`)
      expect(patch.files).toContain(`${posix(tmp.path)}/.config`)
    },
  })
})
...
test("gitignore changes", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(patch.files).toContain(`${posix(tmp.path)}/normal.txt`)
      // Should not track ignored files (git won't see them)
      expect(patch.files).not.toContain(`${posix(tmp.path)}/test.ignored`)
    },
  })
})
...
test("git info exclude changes", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const patch = await Snapshot.patch(before!)
      expect(patch.files).toContain(`${posix(tmp.path)}/normal.txt`)
      expect(patch.files).not.toContain(`${posix(tmp.path)}/ignored.txt`)
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      try {
        ...
        const patch = await Snapshot.patch(before!)
        expect(patch.files).toContain(`${posix(tmp.path)}/normal.txt`)
        expect(patch.files).not.toContain(`${posix(tmp.path)}/global.tmp`)
        expect(patch.files).not.toContain(`${posix(tmp.path)}/info.tmp`)
      } finally {
...
test("snapshot state isolation between projects", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(patch1.files).toContain(`${posix(tmp1.path)}/project1.txt`)
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(patch2.files).toContain(`${posix(tmp2.path)}/project2.txt`)

      // Ensure project1 files don't appear in project2
      expect(patch2.files).not.toContain(`${posix(tmp1?.path ?? "")}/project1.txt`)
    },
  })
})
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const patch = await Snapshot.patch(before!)
        expect(patch.files).toContain(posix(worktreeFile))
      },
    })
  } finally {
...
test("revert should not delete files that existed but were deleted in snapshot", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(patch.files).toContain(`${posix(tmp.path)}/a.txt`)
...
test("revert preserves file that existed in snapshot when deleted then recreated", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(patch.files).toContain(`${posix(tmp.path)}/existing.txt`)
      expect(patch.files).toContain(`${posix(tmp.path)}/newfile.txt`)
