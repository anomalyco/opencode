...
- Start it in `tmux` instead: `tmux new-session -d -s fengru-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt fengru-dev`.
- Stop the session explicitly when done: `tmux kill-session -t fengru-dev`.
