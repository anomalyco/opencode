...
When `plugin` entries exist in a writable `.opencode` dir or `IMECODE_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- `--pure` / `IMECODE_PURE` skips external TUI plugins only.
