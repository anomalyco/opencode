...
- For package/directory plugins, TUI does not use `package.json` `main` as a fallback entry.
...
- If a path spec points at a directory, server loading requires `package.json` `main`; TUI loading requires `exports["./tui"]`.
