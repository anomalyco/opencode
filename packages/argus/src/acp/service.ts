...
import type { AssistantMessage, Message, ArgusClient, SessionMessageResponse } from "@argus-ai/sdk/v2"
...
export const AuthMethodID = "argus-login"
...
export class Service extends Context.Service<Service, Interface>()("@argus/ACP/Service") {}
...
function makeDirectoryService(sdk: ArgusClient) {
...
function makeUsageService(sdk: ArgusClient) {
...
async function loadDirectorySnapshot(sdk: ArgusClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, argus provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const argusProvider = providers[ProviderV2.ID.make("argus")]
  const argusModel = argusProvider ? Provider.sort(Object.values(argusProvider.models))[0] : undefined
  if (argusProvider && argusModel) return { providerID: argusProvider.id, modelID: argusModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Argus service failure", service })
}
