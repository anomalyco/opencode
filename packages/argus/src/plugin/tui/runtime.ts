...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@argus-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@argus-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@argus-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@argus-ai/tui/context/theme"
import { Global } from "@argus-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@argus-ai/core/util/flock"
import { Flag } from "@argus-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@argus-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@argus-ai/core/v1/config/plugin"
import { createCommandShim } from "@argus-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@argus-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".argus"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".argus", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".argus", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ARGUS_PURE ? [] : pluginOrigins
    if (Flag.ARGUS_PURE && pluginOrigins.length) {
    }
