...
export const DEPRECATED_PLUGIN_PACKAGES = ["argus-openai-codex-auth", "argus-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, argusVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(argusVersion) || semver.major(argusVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.argus
  if (typeof range !== "string") return
  if (!semver.satisfies(argusVersion, range)) {
    throw new Error(`Plugin requires argus ${range} but running ${argusVersion}`)
  }
}
