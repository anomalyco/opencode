...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for argus serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ArgusHttpApi)
...
export const PublicApi = ArgusHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "argus",
    version: "1.0.0",
