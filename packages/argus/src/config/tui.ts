...
import { Flag } from "@argus-ai/core/flag/flag"
import { isRecord } from "@argus-ai/tui/util/record"
...
import { TuiKeybind } from "@argus-ai/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@argus-ai/core/installation/version"
import { makeRuntime } from "@argus-ai/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@argus-ai/core/npm"
import { TuiConfig } from "@argus-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@argus/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old argus.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.argus`
  // folders between cwd and home, and ARGUS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ARGUS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ARGUS_TUI_CONFIG override, if set.
  if (Flag.ARGUS_TUI_CONFIG) {
    const configFile = Flag.ARGUS_TUI_CONFIG
  ...
  // 4. `.argus` directories (and ARGUS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".argus") || dir === Flag.ARGUS_CONFIG_DIR)
