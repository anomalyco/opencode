...
import type { ConsoleState } from "@argus-ai/core/v1/config/console-state"
import { FSUtil } from "@argus-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@argus-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@argus-ai/core/v1/config/config"
import { RemoteAuthError } from "@argus-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@argus-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@argus/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://argus.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.ARGUS_CONFIG && !Flag.ARGUS_CONFIG_DIR && !Flag.ARGUS_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://argus.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "argus.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "argus.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://argus.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ARGUS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/argus`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://argus.ai/config.json"
        ...
        if (Flag.ARGUS_CONFIG) {
          yield* merge(Flag.ARGUS_CONFIG, yield* loadFile(Flag.ARGUS_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.ARGUS_CONFIG })
        }

        if (!Flag.ARGUS_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("argus", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ARGUS_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ARGUS_CONFIG_DIR", { path: Flag.ARGUS_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          if (dir.endsWith(".argus") || dir === Flag.ARGUS_CONFIG_DIR) {
            for (const file of ["argus.json", "argus.jsonc"]) {
          ...
          // Auto-discovered plugins under `.argus/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (process.env.ARGUS_CONFIG_CONTENT) {
          const source = "ARGUS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ARGUS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ARGUS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ARGUS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ARGUS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ARGUS_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("ARGUS_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
