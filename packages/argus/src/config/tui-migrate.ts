...
const TUI_SCHEMA_URL = "https://argus.ai/tui.json"
...
async function argusFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "argus"),
    ...(await Filesystem.findUp(["argus.json", "argus.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
