...
import { NonNegativeInt, PositiveInt } from "@argus-ai/core/schema"
import { ConfigAttachmentV1 } from "@argus-ai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@argus-ai/core/v1/config/lsp"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by Argus V1. Use V1 "permission" rules or run argus2.',
      })),
    })
