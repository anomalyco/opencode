...
import type { LLMClientShape } from "@argus-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("argus"))
    return { type: "unsupported", reason: "provider is not openai, argus, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @argus-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains argus-owned. The native runtime only adapts
      // the @argus-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
