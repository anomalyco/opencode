 ...
 *
 * 依赖关系：
 * - client.ts：OpenCode V2 客户端实现
 * - server.ts：OpenCode V2 服务器实现
 *
 * 导出内容：
 * - createOpencodeClient：创建 OpenCode V2 客户端
 * - createOpencodeServer：创建 OpenCode V2 服务器
 * - createOpencode：一键创建客户端和服务器对
 *
 * 使用场景：
 * - 快速搭建 OpenCode V2 开发环境
 * - 测试和演示
 * - 本地开发
 *
 * @package sdk/js
...
/**
 * 创建 OpenCode V2 客户端和服务器对
 *
 * 这是一个便捷函数，用于同时创建 OpenCode V2 服务器和连接到它的客户端。
 * 适合用于本地开发、测试等场景。
 *
 * @param options - 服务器配置选项
 * @returns 包含客户端和服务器对象的对象
 *          - client: OpenCode V2 客户端实例
 *          - server: OpenCode V2 服务器实例（包含 url 和 close 方法）
 *
 * 执行流程：
 * 1. 使用提供的选项创建 OpenCode V2 服务器
 * 2. 等待服务器启动并获取其 URL
 * 3. 使用服务器 URL 创建客户端
 * 4. 返回客户端和服务器的组合对象
 *
