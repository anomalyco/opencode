...
const PersistedSkill = Schema.Struct({
  ...Base,
  type: Schema.tag("skill"),
  name: Schema.String,
  text: Schema.String,
...
const PersistedShell = Schema.Struct({
  ...Base,
  type: Schema.tag("shell"),
  shell: ShellSchema.Info,
  output: ShellSchema.Output.pipe(optional),
  time: Shell.fields.time,
})

const PersistedToolStatePending = Schema.Struct({ status: Schema.tag("pending"), input: Schema.String })
const PersistedToolState = Schema.Union([ToolState, PersistedToolStatePending]).pipe(
  Schema.decodeTo(
    ToolState,
    SchemaTransformation.transform({
      decode: (state) =>
        state.status === "pending" ? ToolStateStreaming.make({ status: "streaming", input: state.input }) : state,
      encode: (state) => state,
    }),
  ),
)
const PersistedAssistantTool = Schema.Struct({ ...AssistantTool.fields, state: PersistedToolState })
const PersistedAssistantContent = Schema.Union([AssistantText, AssistantReasoning, PersistedAssistantTool]).pipe(
  Schema.toTaggedUnion("type"),
)
const PersistedAssistant = Schema.Struct({ ...Assistant.fields, content: Schema.Array(PersistedAssistantContent) })

const PersistedCompactionFailed = Schema.Struct({
  ...CompactionBase,
  status: Schema.tag("failed"),
  reason: Schema.Literals(["auto", "manual"]),
  summary: Schema.String,
...
const PersistedCompactionQueued = Schema.Struct({
  ...CompactionBase,
...
const PersistedLegacyInfo = Schema.Union([
  PersistedSkill,
  PersistedShell,
  PersistedAssistant,
  PersistedCompactionFailed,
  PersistedCompactionQueued,
])
const PersistedCurrent = Info.pipe(
  Schema.decodeTo(
    Schema.Struct({ source: Schema.tag("current"), message: Schema.toType(Info) }),
    SchemaTransformation.transform({
      decode: (message): { readonly source: "current"; readonly message: Info } => ({ source: "current", message }),
      encode: (value) => value.message,
    }),
  ),
)
const PersistedLegacy = PersistedLegacyInfo.pipe(
  Schema.decodeTo(
    Schema.Struct({ source: Schema.tag("legacy"), message: Schema.toType(PersistedLegacyInfo) }),
    SchemaTransformation.transform({
      decode: (message): { readonly source: "legacy"; readonly message: typeof PersistedLegacyInfo.Type } => ({
        source: "legacy",
        message,
      }),
      encode: (value) => value.message,
...
const PersistedInfo = Schema.Union([PersistedCurrent, PersistedLegacy]).pipe(Schema.toTaggedUnion("source"))

/** Storage decoder for projected rows written before the current Session message contract. */
export const Persisted = PersistedInfo.pipe(
  Schema.decodeTo(
    Schema.toType(Info),
    SchemaTransformation.transform({
      decode: (persisted): Info => {
        ...
        if (message.type === "shell")
          return Shell.make({
            ...
            shellID: message.shell.id,
            command: message.shell.command,
            status: message.shell.status,
            exit: message.shell.exit,
            output: message.output,
            time: message.time,
          })
        if (message.type === "compaction" && message.status === "queued")
          return CompactionFailed.make({
            id: message.id,
            type: "compaction",
            metadata: message.metadata,
            status: "failed",
            reason: message.reason,
            error: { type: "compaction.queued", message: "Legacy queued compaction was not started" },
            time: message.time,
          })
        if (message.type === "compaction" && message.status === "failed")
          return CompactionFailed.make({
            ...
            status: "failed",
            reason: message.reason,
            error: { type: "compaction.failed", message: "Compaction failed before recording an error" },
            time: message.time,
          })
        return message
      },
