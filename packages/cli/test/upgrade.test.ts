...
describe("upgrade command", () => {
  ...
  test("reports version lookup failures without installing", async () => {
    ...
    expect(result.stdout).toContain("Could not check for OpenCode updates")
    expect(result.stdout).toContain("The update service returned HTTP 503.")
    expect(result.stdout).toContain("Retry    Try again in a few minutes.")
  })
  ...
  test("reports installation failures with a nonzero exit code", async () => {
    ...
    expect(result.stdout).toContain("npm could not install OpenCode")
    expect(result.stdout).toContain("Permission denied")
    expect(result.stdout).toContain("Command  npm install @opencode/cli@0.0.0-beta-new")
    expect(result.stdout).toContain("Retry    Fix the issue above, then run opencode upgrade again.")
