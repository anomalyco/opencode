...
describe("acp event behavior", () => {
  ...
  test("forwards scheduled retries and clears them when the next attempt starts", async () => {
    const updates: SessionUpdateParams[] = []
    const error = { type: "provider.rate-limit", message: "rate-limited upstream" }
    const fixture = createSseFixture({
      onPrompt({ id, send }) {
        send(durableEvent("session.inbox.delivered", { sessionID: "ses_retry", inboxID: id }))
        send(
        ...
        send(durableEvent("session.execution.succeeded", { sessionID: "ses_retry" }))
      },
    })
    ...
    try {
      ...
      expect(updates.map((item) => item.update)).toEqual([
        ...
        { sessionUpdate: "agent_message_chunk", messageId: "msg_retry", content: { type: "text", text: "recovered" } },
      ])
      expect(response).toMatchObject({ stopReason: "end_turn", _meta: {} })
    } finally {
      await fixture.stop()
    }
  })
