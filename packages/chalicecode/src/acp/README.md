...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for chalice.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal chalice sessions
...
- **Terminal Integration**: Full terminal support via chalice's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | chalice acp
```
...
### Mapping to ChaliceCode

ACP sessions map cleanly to chalice's internal session model:
