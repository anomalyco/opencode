...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.CHALICECODE_CONFIG) {
      result = mergeDeep(result, await loadFile(Flag.CHALICECODE_CONFIG))
      log.debug("loaded custom config", { path: Flag.CHALICECODE_CONFIG })
    }
    ...
    if (Flag.CHALICECODE_CONFIG_CONTENT) {
      result = mergeDeep(result, JSON.parse(Flag.CHALICECODE_CONFIG_CONTENT))
      log.debug("loaded custom config from CHALICECODE_CONFIG_CONTENT")
    }
    ...
    if (Flag.CHALICECODE_CONFIG_DIR) {
      directories.push(Flag.CHALICECODE_CONFIG_DIR)
      log.debug("loading config from CHALICECODE_CONFIG_DIR", { path: Flag.CHALICECODE_CONFIG_DIR })
    }
    ...
    if (Flag.CHALICECODE_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CHALICECODE_PERMISSION))
    }
  ...
  async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@chalicecode-ai/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
