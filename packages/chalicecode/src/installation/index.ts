...
declare global {
  const CHALICECODE_VERSION: string
  const CHALICECODE_CHANNEL: string
}
...
export namespace Installation {
  ...
  export async function method() {
    ...
    for (const check of checks) {
      ...
      if (output.includes(check.name === "brew" ? "chalice" : "opencode-ai")) {
        return check.name
      }
    }
  ...
  async function getBrewFormula() {
    ...
    if (tapFormula.includes("chalice")) return "sst/tap/opencode"
    const coreFormula = await $`brew list --formula opencode`.throws(false).text()
    if (coreFormula.includes("chalice")) return "chalice"
    return "chalice"
  }
  ...
  export const VERSION = typeof CHALICECODE_VERSION === "string" ? CHALICECODE_VERSION : "local"
  export const CHANNEL = typeof CHALICECODE_CHANNEL === "string" ? CHALICECODE_CHANNEL : "local"
