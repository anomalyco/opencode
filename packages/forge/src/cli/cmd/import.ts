...
export const ImportCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs.positional("file", {
      describe: "path to JSON file or forge.dev share URL",
      type: "string",
      demandOption: true,
    })
  },
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      if (isUrl) {
        ...
        if (!urlMatch) {
          process.stdout.write(`Invalid URL format. Expected: https://forge.dev/s/<slug>`)
          process.stdout.write(EOL)
          return
        }

        const slug = urlMatch[1]
        const response = await fetch(`https://api.forge.dev/share_data?id=${slug}`)
