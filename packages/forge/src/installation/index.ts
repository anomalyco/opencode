...
export namespace Installation {
  ...
  export async function method() {
    ...
    for (const check of checks) {
      ...
      if (output.includes(check.name === "brew" ? "opencode" : "forge")) {
        return check.name
      }
    }
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        cmd = $`curl -fsSL https://forge.dev/install | bash`.env({
          ...process.env,
          VERSION: target,
        })
        break
      case "npm":
        cmd = $`npm install -g forge@${target}`
        break
      case "pnpm":
        cmd = $`pnpm install -g forge@${target}`
        break
      case "bun":
        cmd = $`bun install -g forge@${target}`
        break
      case "brew": {
  ...
  export async function latest() {
    ...
    return fetch(`https://registry.npmjs.org/forge/${channel}`)
      .then((res) => {
