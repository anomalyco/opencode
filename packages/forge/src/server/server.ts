...
export namespace Server {
  ...
  export const App = lazy(() =>
    app
      ...
      .use(async (c, next) => {
        const directory = c.req.query("directory") ?? c.req.header("x-forge-directory") ?? process.cwd()
      ...
      .all("/*", async (c) => {
        return proxy(`https://desktop.dev.forge.dev${c.req.path}`, {
          ...c.req,
          headers: {
            host: "desktop.dev.forge.dev",
          },
        })
      }),
  )
