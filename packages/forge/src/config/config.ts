...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.FORGE_CONFIG) {
      result = mergeDeep(result, await loadFile(Flag.FORGE_CONFIG))
    ...
    if (Flag.FORGE_CONFIG_CONTENT) {
      result = mergeDeep(result, JSON.parse(Flag.FORGE_CONFIG_CONTENT))
      log.debug("loaded custom config from FORGE_CONFIG_CONTENT")
    }
    ...
    if (Flag.FORGE_CONFIG_DIR) {
      directories.push(Flag.FORGE_CONFIG_DIR)
      log.debug("loading config from FORGE_CONFIG_DIR", { path: Flag.FORGE_CONFIG_DIR })
    }
  ...
  async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@forge/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const name = (() => {
        const patterns = ["/.forge/command/", "/command/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const agentFolderPath = item.includes("/.forge/agent/")
        ? item.split("/.forge/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://forge.dev/docs/commands"),
      watcher: z
        .object({
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://forge.dev/docs/agent"),
      provider: z
        .record(
          z.string(),
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "forge.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "forge.jsonc"))),
    )
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://forge.dev/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://forge.dev/config.json"
