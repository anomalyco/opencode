...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("athena-find")
  api.delete("athena-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("athena-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("athena-find", new Highlight(...rest))
    return true
  }
