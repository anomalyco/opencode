...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("xcsh-find")
  api.delete("xcsh-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("xcsh-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("xcsh-find", new Highlight(...rest))
    return true
  }
