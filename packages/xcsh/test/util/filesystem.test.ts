...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "xcsh.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "xcsh.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "xcsh.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "xcsh.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["xcsh.json", "xcsh.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "xcsh.json"),
        path.join(tmp.path, "xcsh.jsonc"),
        path.join(project, "xcsh.json"),
        path.join(project, "xcsh.jsonc"),
      ])
    })
  })
