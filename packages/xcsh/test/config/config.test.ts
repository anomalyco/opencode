...
const managedConfigDir = process.env.XCSH_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "xcsh.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "xcsh.json") {
...
test("ignores legacy tui keys in xcsh config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "xcsh.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["XCSH_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:XCSH_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["XCSH_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["XCSH_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "xcsh.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .xcsh directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".xcsh")
...
test("loads agents from .xcsh/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".xcsh")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .xcsh/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".xcsh")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .xcsh/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".xcsh")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only XCSH_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.XCSH_CONFIG_DIR
  process.env.XCSH_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.XCSH_CONFIG_DIR
    else process.env.XCSH_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable XCSH_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    const mod = path.join(dir, "node_modules", "@f5xc-salesdemos", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@f5xc-salesdemos/plugin", version: "1.0.0" }),
    )
  })
...
test("dedupes concurrent config dependency installs for the same dir", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (d: string) => {
    ...
    const mod = path.join(d, "node_modules", "@f5xc-salesdemos", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@f5xc-salesdemos/plugin", version: "1.0.0" }),
...
test("serializes config dependency installs across dirs", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    ...
    const mod = path.join(cwd, "node_modules", "@f5xc-salesdemos", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@f5xc-salesdemos/plugin", version: "1.0.0" }),
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "xcsh.json"),
        JSON.stringify({ $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .xcsh config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".xcsh")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .xcsh config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "xcsh.json"),
        JSON.stringify({
          $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .xcsh config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "xcsh.json"),
        JSON.stringify({
          $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".xcsh")
      const global_ = path.join(dir, ".xcsh")
      await fs.mkdir(local, { recursive: true })
      await fs.mkdir(global_, { recursive: true })

      await Filesystem.write(
        path.join(global_, "xcsh.json"),
        JSON.stringify({
          $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json",
...
// Note: preload.ts sets XCSH_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .xcsh config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .xcsh directory config enables it
      const opencodeDir = path.join(dir, ".xcsh")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "xcsh.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "xcsh.json")
    expect(await Config.resolvePluginSpec("oh-my-xcsh@2.4.3", file)).toBe("oh-my-xcsh@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-xcsh@2.4.3", "file:///project/.xcsh/plugin/oh-my-xcsh.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.xcsh/plugin/demo.ts", "file:///project/.xcsh/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.xcsh/plugin/demo.ts"])
  })
...
describe("XCSH_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["XCSH_DISABLE_PROJECT_CONFIG"]
    process.env["XCSH_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["XCSH_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["XCSH_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .xcsh/ directories when flag is set", async () => {
    const originalEnv = process.env["XCSH_DISABLE_PROJECT_CONFIG"]
    process.env["XCSH_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .xcsh directory with a command
          const xcshDir = path.join(dir, ".xcsh", "command")
          await fs.mkdir(xcshDir, { recursive: true })
          await Filesystem.write(path.join(xcshDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .xcsh should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["XCSH_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["XCSH_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["XCSH_CONFIG_DIR"]
      process.env["XCSH_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["XCSH_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["XCSH_CONFIG_DIR"]
      } else {
        process.env["XCSH_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("XCSH_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["XCSH_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["XCSH_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["XCSH_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["XCSH_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from XCSH_CONFIG_DIR, not project
...
describe("XCSH_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in XCSH_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["XCSH_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["XCSH_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["XCSH_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["XCSH_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in XCSH_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["XCSH_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      ...
      PayloadIdentifier: "ai.xcsh.managed.test",
      PayloadType: "ai.xcsh.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://f5xc-salesdemos.github.io/xcsh/config.json" }),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://f5xc-salesdemos.github.io/xcsh/config.json")
})
