...
Replace runtime Bun shell template-tag usage in `packages/xcsh/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/xcsh/src` is removed except:
