...
- Author package entrypoint is `@f5xc-salesdemos/plugin/tui`.
...
```json
{
  "$schema": "https://f5xc-salesdemos.github.io/xcsh/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/xcsh-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@f5xc-salesdemos/plugin/tui`.
- `@f5xc-salesdemos/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@f5xc-salesdemos/plugin/tui"
...
- If `engines.xcsh` is absent, no check is performed (backward compatible).
...
- `xcsh plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `xcsh plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.xcsh` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.xcsh`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.xcsh`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.xcsh` dir or `XCSH_CONFIG_DIR`, OpenCode installs `@f5xc-salesdemos/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@f5xc-salesdemos/plugin/tui`.
...
- Local plugins persist installed themes under the local `.xcsh/themes` area near the plugin config source.
...
- Local smoke plugin: `.xcsh/plugins/tui-smoke.tsx`
- Local smoke config: `.xcsh/tui.json`
- Local smoke theme: `.xcsh/plugins/smoke-theme.json`
