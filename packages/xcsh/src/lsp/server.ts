...
import { Module } from "@f5xc-salesdemos/util/module"
...
export namespace LSPServer {
  ...
  export const Vue: Info = {
    ...
    async spawn(root) {
      ...
      const args: string[] = []
      if (!binary) {
        if (Flag.XCSH_DISABLE_LSP_DOWNLOAD) return
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "xcsh-jdtls-data"))
      return {
        process: spawn(
          java,
          [
