...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for xcsh.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal xcsh sessions
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 xcsh acp
```
...
- **Terminal Integration**: Full terminal support via xcsh's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | xcsh acp
```
...
ACP sessions map cleanly to xcsh's internal session model:
