...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/xcsh"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "xcsh")
      default:
        return "/etc/xcsh"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.XCSH_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }

  const managedDir = managedConfigDir()

  const MANAGED_PLIST_DOMAIN = "ai.xcsh.managed"
  ...
  export async function installDependencies(dir: string, input?: InstallInput) {
    ...
    }>(pkg).catch(() => ({ dependencies: {}, optionalDependencies: {} }))
    json.optionalDependencies = {
      ...json.optionalDependencies,
      "@f5xc-salesdemos/plugin": target,
    }
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.xcsh/command/", "/.xcsh/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.xcsh/agent/", "/.xcsh/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Keybinds = z
    .object({
      ...
      display_assistant_metadata: z
        .string()
        .optional()
        .default("none")
        .describe("Toggle assistant metadata visibility"),
    })
    .strict()
    .meta({
      ref: "KeybindsConfig",
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: xcsh.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for xcsh serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://f5xc-salesdemos.github.io/xcsh/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://f5xc-salesdemos.github.io/xcsh/docs/agents"),
      personality: z
        .object({
        ...
        .optional()
        .describe("Personality configuration, see https://f5xc-salesdemos.github.io/xcsh/docs/personality"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@xcsh/Config") {}

  function globalConfigFile() {
    const candidates = ["xcsh.jsonc", "xcsh.json", "config.json"].map((file) => path.join(Global.Path.config, file))
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in xcsh config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://f5xc-salesdemos.github.io/xcsh/config.json"
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "xcsh.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "xcsh.jsonc"))),
          )
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://f5xc-salesdemos.github.io/xcsh/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const scope = (source: string): PluginScope => {
            ...
            if (source === "XCSH_CONFIG_CONTENT") return "local"
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://f5xc-salesdemos.github.io/xcsh/config.json"
          ...
          if (Flag.XCSH_CONFIG) {
            merge(Flag.XCSH_CONFIG, yield* loadFile(Flag.XCSH_CONFIG))
            log.debug("loaded custom config", { path: Flag.XCSH_CONFIG })
          }

          if (!Flag.XCSH_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("xcsh", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.XCSH_CONFIG_DIR) {
            log.debug("loading config from XCSH_CONFIG_DIR", { path: Flag.XCSH_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".xcsh") || dir === Flag.XCSH_CONFIG_DIR) {
              for (const file of ["xcsh.json", "xcsh.jsonc"]) {
          ...
          if (process.env.XCSH_CONFIG_CONTENT) {
            const source = "XCSH_CONFIG_CONTENT"
            const next = yield* loadConfig(process.env.XCSH_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            })
            merge(source, next, "local")
            log.debug("loaded custom config from XCSH_CONFIG_CONTENT")
          }
          ...
          if (active?.active_org_id) {
            yield* Effect.gen(function* () {
              ...
              if (token) {
                process.env["XCSH_CONSOLE_TOKEN"] = token
                Env.set("XCSH_CONSOLE_TOKEN", token)
              }
          ...
          if (Flag.XCSH_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.XCSH_PERMISSION))
          }
          ...
          if (Flag.XCSH_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.XCSH_DISABLE_PRUNE) {
