...
const TUI_SCHEMA_URL = "https://f5xc-salesdemos.github.io/xcsh/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from xcsh.json files
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.XCSH_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("xcsh", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "xcsh")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "xcsh"))
  }
  if (Flag.XCSH_CONFIG) files.push(Flag.XCSH_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "xcsh"))
