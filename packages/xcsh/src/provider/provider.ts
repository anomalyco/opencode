...
export namespace Provider {
  ...
  function e2eURL() {
    const url = Env.get("XCSH_E2E_LLM_URL")
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from xcsh.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://f5xc-salesdemos.github.io/xcsh/",
            "X-Title": "xcsh",
          },
        },
      }
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `xcsh/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": async (input) => {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `xcsh/${Installation.VERSION} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": async (input) => {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `xcsh auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `xcsh/${Installation.VERSION} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "xcsh",
          },
        },
      }
    },
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@xcsh/Provider") {}
        ...
        Effect.gen(function* () {
          ...
          for (const [id, provider] of Object.entries(providers)) {
            ...
            for (const [modelID, model] of Object.entries(provider.models)) {
              ...
              if (model.status === "alpha" && !Flag.XCSH_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
