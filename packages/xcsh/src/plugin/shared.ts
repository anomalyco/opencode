...
export const DEPRECATED_PLUGIN_PACKAGES = ["xcsh-openai-codex-auth", "xcsh-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, opencodeVersion: string, pkg?: PluginPackage) {
  ...
  const range = engines.xcsh
  if (typeof range !== "string") return
  if (!semver.satisfies(opencodeVersion, range)) {
    throw new Error(`Plugin requires xcsh ${range} but running ${opencodeVersion}`)
  }
}
