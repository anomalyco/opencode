...
import {
  ...
  type TuiTheme,
} from "@f5xc-salesdemos/plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".xcsh"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".xcsh", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".xcsh", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.XCSH_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.XCSH_PURE && config.plugin_origins?.length) {
