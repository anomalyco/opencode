...
import xcsh from "./theme/xcsh.json" with { type: "json" }
...
import f5xc from "./theme/f5xc.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@f5xc-salesdemos/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "f5xc")
        draft.active = typeof active === "string" ? active : "f5xc"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.xcsh, store.mode)
    })
