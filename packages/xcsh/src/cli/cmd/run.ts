...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@f5xc-salesdemos/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run xcsh with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running xcsh server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.XCSH_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.XCSH_SERVER_USERNAME ?? "xcsh"
