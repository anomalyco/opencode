...
const WORKFLOW_FILE = ".github/workflows/xcsh.yml"
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://f5xc-salesdemos.github.io/xcsh/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
...
jobs:
  xcsh:
    if: |
      ...
      contains(github.event.comment.body, ' /xcsh') ||
      startsWith(github.event.comment.body, '/xcsh')
    ...
    steps:
      ...
      - name: Run xcsh
        uses: f5xc-salesdemos/xcsh/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.opencode.ai" : "https://f5xc-salesdemos.github.io/xcsh"
      ...
      try {
        ...
        console.log("xcsh session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/xcsh,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to xcsh...")
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `xcsh/${type}-${hex}-${timestamp}`
        }
        return `xcsh/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/xcsh-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[xcsh session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the xcsh infrastructure after your response",
