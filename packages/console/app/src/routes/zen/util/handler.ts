...
import { and, Database, eq, isNull, lt, or, sql } from "@pencode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@pencode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@pencode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@pencode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@pencode-ai/console-core/util/date.js"
...
import { ProviderTable } from "@pencode-ai/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-pencode-session") ?? ""
    const requestId = input.request.headers.get("x-pencode-request") ?? ""
    const ocClient = input.request.headers.get("x-pencode-client") ?? ""
    const projectId = input.request.headers.get("x-pencode-project") ?? ""
    ...
    if (
      ...
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://pencode.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const providerRequest = async () => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-pencode-request")
          if (!isNewInference) headers.delete("x-pencode-session")
          headers.delete("x-pencode-project")
          headers.delete("x-pencode-client")
          return headers
        })(),
        body: reqBody,
        duplex: "half",
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-pencode-endpoint-id")
        const resEndpointModelId = res.headers.get("x-pencode-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      ...
      try {
        const consoleGoUrl = `https://pencode.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://pencode.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://pencode.ai/workspace/${authInfo.workspaceID}/members`
