 ...
 * ============================================================================
 * 文件名：index.tsx
 * 所属包：packages/console/app/src/routes/zen
 * ============================================================================
 *
 * 文件作用：
 * OpenCode Zen 产品落地页。展示 Zen 服务的功能、定价和使用说明。
 *
 * 主要功能：
 * - 展示 Zen 服务的核心价值和优势
 * - 显示支持的 AI 模型提供商
 ...
 *
 * 依赖关系：
 * - @solidjs/router：路由查询和重定向
 * - @solidjs/meta：页面 Meta 标签
 * - ~/component/*：公共组件
 * - ~/config：应用配置
 * - ../workspace/common：工作区通用工具
 *
 * 导出内容：
 * - default：Zen 落地页组件
 * - checkLoggedIn：服务器查询函数，检查登录状态
 *
 * 路由：
 * - GET /zen → Zen 产品落地页
 *
 ...
 * 检查用户登录状态的服务器查询函数
 *
 * 如果用户已登录且有最近访问的工作区，则重定向到该工作区。
 * 这确保已登录用户不会看到 Zen 落地页，而是直接进入应用。
 *
 * 服务端执行标记：
 * - "use server"：此函数在服务器端执行
 *
 * @returns Promise<void> - 重定向或 undefined
 * @throws {redirect} - 如果已登录，重定向到工作区
 *
 * 执行流程：
 * 1. 尝试获取最近访问的工作区 ID
 * 2. 如果获取成功，重定向到该工作区
 * 3. 如果失败（未登录），返回 undefined
 */
