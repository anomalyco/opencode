...
- [x] Each call to an extension global is observed by the host's `onCall` as `{ type: "extension", extension, name, args }` with
      the host's own error on failure; calls to functions inside results are not.
