...
- Keep network reads bounded and preserve encoding, transport, and decoding failure details in `ToolError` values.
...
- Preserve tool failure messages and their underlying causes in model-visible diagnostics. Do not replace unknown host failures with generic messages. Interruption must remain interruption.
