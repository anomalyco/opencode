...
This is the living design and status document for `@pencode-ai/codemode` and its existing V2 Pencode adapter.
...
`packages/pencode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
