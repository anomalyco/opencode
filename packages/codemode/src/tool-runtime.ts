...
export const prepare = <R>(tools: Tools<R>, catalogBudget = defaultCatalogBudget): DiscoveryPlan => {
  ...
  const language = [
    ...
    "Modules/imports, classes, timers, fetch, eval, prototype access, and unlisted methods are unavailable. Sync and async generators support yield/yield* and iteration with for...of or for await...of; generator values are confined references, not returnable data. Use tools for external operations. Use await with try/catch.",
