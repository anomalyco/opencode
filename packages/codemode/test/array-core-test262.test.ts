...
describe("Array property keys", () => {
  test("canonical indexes and ordinary properties remain distinct", async () => {
    expect(
      await value(`
      ...
      maximum: "maximum",
      length: 2,
      keys: ["0", "1", "01", "1.0", "-1", "1.5", "4294967295"],
    })
  })

  test("assignments do not observe inherited array properties", async () => {
    expect(
      await value(`
        const values = []
        const logical = (values.toString ||= "own")
        const compound = (values.valueOf += "value")
        const updated = values.hasOwnProperty++
        return {
          logical,
          compound,
          updated: Number.isNaN(updated),
          next: Number.isNaN(values.hasOwnProperty),
        }
      `),
    ).toEqual({ logical: "own", compound: "undefinedvalue", updated: true, next: true })
  })

  test("assignments do not invoke inherited array accessors", async () => {
    const key = "__codemode_array_property__"
      ...
      expect(
        await value(`
          ...
          return [
            ...
            compound.${key},
            Number.isNaN(before),
            Number.isNaN(updated.${key}),
          ]
        `),
      ).toEqual([1, 2, "undefinedvalue", true, true])
      expect([gets, sets]).toEqual([0, 0])
    } finally {
