...
const reg = process.env.REGISTRY ?? "ghcr.io/CyberStrikeus"
...
const setup = async () => {
  ...
  if (list.includes("cyberstrike")) {
    await $`docker buildx use cyberstrike`
    return
  }
  await $`docker buildx create --name cyberstrike --use`
}
