...
Replace runtime Bun shell template-tag usage in `packages/octopus/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/octopus/src` is removed except:
