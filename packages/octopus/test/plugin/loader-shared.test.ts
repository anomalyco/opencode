...
const disableDefault = process.env.OCTOPUS_DISABLE_DEFAULT_PLUGINS
process.env.OCTOPUS_DISABLE_DEFAULT_PLUGINS = "1"

const { Plugin } = await import("../../src/plugin/index")
const { PluginLoader } = await import("../../src/plugin/loader")
const { readPackageThemes } = await import("../../src/plugin/shared")
const { Bus } = await import("../../src/bus")
const { Npm } = await import("@octopus-ai/core/npm")
const { TestConfig } = await import("../fixture/config")

afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.OCTOPUS_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.OCTOPUS_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
`,
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.OCTOPUS_PURE
    process.env.OCTOPUS_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.OCTOPUS_PURE
      } else {
        process.env.OCTOPUS_PURE = pure
      }
    }
  })
