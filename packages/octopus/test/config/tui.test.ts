...
afterEach(async () => {
  ...
  delete process.env.OCTOPUS_TUI_CONFIG
...
test("project config takes precedence over OCTOPUS_TUI_CONFIG (matches OCTOPUS_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.OCTOPUS_TUI_CONFIG = custom
    },
  })
...
test("OCTOPUS_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from OCTOPUS_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.OCTOPUS_CONFIG = path.join(customDir, "opencode.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
