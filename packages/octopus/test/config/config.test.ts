...
const managedConfigDir = process.env.OCTOPUS_TEST_MANAGED_CONFIG_DIR!
...
test("ignores legacy tui keys in octopus config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["OCTOPUS_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:OCTOPUS_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["OCTOPUS_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OCTOPUS_CONSOLE_TOKEN"]
    }
  }
})
...
test("loads config from .opencode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const octopusDir = path.join(dir, ".opencode")
      await fs.mkdir(octopusDir, { recursive: true })
      const agentDir = path.join(octopusDir, "agent")
...
test("loads agents from .opencode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const agentsDir = path.join(octopusDir, "agents")
...
test("loads commands from .opencode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const commandDir = path.join(octopusDir, "command")
...
test("loads commands from .opencode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const commandsDir = path.join(octopusDir, "commands")
...
test("does not try to install dependencies in read-only OCTOPUS_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OCTOPUS_CONFIG_DIR
  process.env.OCTOPUS_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OCTOPUS_CONFIG_DIR
    else process.env.OCTOPUS_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OCTOPUS_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const octopusDir = path.join(projectDir, ".opencode")
      await fs.mkdir(octopusDir, { recursive: true })
      ...
      await Filesystem.write(
        path.join(octopusDir, "opencode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets OCTOPUS_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-octopus@2.4.3", file)).toBe("oh-my-octopus@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-octopus@2.4.3", "file:///project/.opencode/plugin/oh-my-octopus.js"]
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const octopusDir = path.join(projectDir, ".opencode")
        const pluginDir = path.join(octopusDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OCTOPUS_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"]
    process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const octopusDir = path.join(dir, ".opencode", "command")
          await fs.mkdir(octopusDir, { recursive: true })
          await Filesystem.write(path.join(octopusDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OCTOPUS_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OCTOPUS_CONFIG_DIR"]
      process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OCTOPUS_CONFIG_DIR"]
      } else {
        process.env["OCTOPUS_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OCTOPUS_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OCTOPUS_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OCTOPUS_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OCTOPUS_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from OCTOPUS_CONFIG_DIR, not project
...
describe("OCTOPUS_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OCTOPUS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OCTOPUS_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OCTOPUS_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OCTOPUS_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OCTOPUS_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OCTOPUS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OCTOPUS_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.octopus.managed.test",
          PayloadType: "ai.octopus.managed",
