...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OCTOPUS_TEST ?? "")'), {
      env: {
        OCTOPUS_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OCTOPUS_TEST_SHELL"], {
      shell: true,
      env: {
        OCTOPUS_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OCTOPUS_TEST_SHELL=ok")
  })
