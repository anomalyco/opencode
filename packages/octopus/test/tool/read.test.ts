...
const experimentalScout = <A, E, R>(self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = Flag.OCTOPUS_EXPERIMENTAL_SCOUT
    ...
    (previous) =>
      Effect.sync(() => {
        Flag.OCTOPUS_EXPERIMENTAL_SCOUT = previous
      }),
  )
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.OCTOPUS_REPO_CLONE_GITHUB_BASE_URL
      process.env.OCTOPUS_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.OCTOPUS_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.OCTOPUS_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
