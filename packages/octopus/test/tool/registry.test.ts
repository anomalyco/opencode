...
const originalExperimentalScout = Flag.OCTOPUS_EXPERIMENTAL_SCOUT
...
afterEach(async () => {
  Flag.OCTOPUS_EXPERIMENTAL_SCOUT = originalExperimentalScout
...
describe("tool.registry", () => {
  it.instance("hides repo research tools unless experimental", () =>
    Effect.gen(function* () {
      Flag.OCTOPUS_EXPERIMENTAL_SCOUT = false
  ...
  it.instance("shows repo research tools when experimental scout is enabled", () =>
    Effect.gen(function* () {
      Flag.OCTOPUS_EXPERIMENTAL_SCOUT = true
      const registry = yield* ToolRegistry.Service
      const ids = yield* registry.ids()
  ...
  it.instance("loads tools from .opencode/tool (singular)", () =>
    Effect.gen(function* () {
      ...
      const octopus = path.join(test.directory, ".opencode")
      const tool = path.join(octopus, "tool")
  ...
  it.instance("loads tools from .opencode/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const octopus = path.join(test.directory, ".opencode")
      const tools = path.join(octopus, "tools")
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(octopus, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@octopus-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(octopus, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(octopus, "node_modules", "cowsay")
