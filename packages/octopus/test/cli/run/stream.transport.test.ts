...
import { OctopusClient, type GlobalEvent } from "@octopus-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OctopusClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OctopusClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OctopusClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OctopusClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OctopusClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: OctopusClient["event"]["subscribe"]
    globalEvent?: OctopusClient["global"]["event"]
    promptAsync?: OctopusClient["session"]["promptAsync"]
    status?: OctopusClient["session"]["status"]
    messages?: OctopusClient["session"]["messages"]
    children?: OctopusClient["session"]["children"]
    permissions?: OctopusClient["permission"]["list"]
    questions?: OctopusClient["question"]["list"]
  } = {},
) {
  const client = new OctopusClient()

  const subscribe: OctopusClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OctopusClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OctopusClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OctopusClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OctopusClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OctopusClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OctopusClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OctopusClient["question"]["list"] = input.questions ?? (() => ok([]))
