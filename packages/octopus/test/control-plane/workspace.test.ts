...
const originalWorkspacesFlag = Flag.OCTOPUS_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  OCTOPUS_AUTH_CONTENT: process.env.OCTOPUS_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.OCTOPUS_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      ...
      expect(JSON.parse(recorded.calls.create[0].env.OCTOPUS_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.OCTOPUS_WORKSPACE_ID).toBe(workspaceID)
...
describe("workspace sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.OCTOPUS_EXPERIMENTAL_WORKSPACES = false
