...
async function withExperimentalScout(enabled: boolean, fn: () => Promise<void>) {
  const original = Flag.OCTOPUS_EXPERIMENTAL_SCOUT
  Flag.OCTOPUS_EXPERIMENTAL_SCOUT = enabled
  try {
    await fn()
  } finally {
    Flag.OCTOPUS_EXPERIMENTAL_SCOUT = original
  }
}
...
`,
  ...
  process.env.OCTOPUS_TEST_HOME = tmp.path
