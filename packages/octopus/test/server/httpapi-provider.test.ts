...
describe("provider HttpApi", () => {
  ...
  it.live("serves provider lists when auth loaders add runtime fetch options", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "opencode-test-" })
      const previous = process.env.OCTOPUS_AUTH_CONTENT
      ...
      yield* Effect.sync(() => {
        process.env.OCTOPUS_AUTH_CONTENT = JSON.stringify({
          google: { type: "oauth", refresh: "dummy", access: "dummy", expires: 9999999999999 },
        })
      })
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.OCTOPUS_AUTH_CONTENT
