...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const originalWorkspaces = Flag.OCTOPUS_EXPERIMENTAL_WORKSPACES
...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      // This starts a second HTTP server that stands in for the octopus server
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-octopus-directory": "/secret/path",
          "x-octopus-workspace": "internal",
        }),
        HttpClient.execute,
      )
      ...
      expect(forwarded?.headers["x-octopus-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-octopus-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-octopus-directory", headerDir),
        HttpClient.execute,
      )
