...
const auth = { username: "octopus", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.OCTOPUS_SERVER_PASSWORD
  else process.env.OCTOPUS_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.OCTOPUS_SERVER_USERNAME
  else process.env.OCTOPUS_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.OCTOPUS_SERVER_PASSWORD = auth.password
  Flag.OCTOPUS_SERVER_USERNAME = auth.username
  process.env.OCTOPUS_SERVER_PASSWORD = auth.password
  process.env.OCTOPUS_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.OCTOPUS_SERVER_PASSWORD = undefined
  Flag.OCTOPUS_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-octopus-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-octopus-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-octopus-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-octopus-ticket": "1" },
      })
