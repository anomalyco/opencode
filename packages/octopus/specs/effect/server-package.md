...
Practical reference for extracting a future `packages/server` from the current `packages/octopus` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all octopus services, Effect-first source of truth
- `packages/server` - octopus server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full octopus server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/octopus`.
...
- those slices are mounted into the Hono server behind `OCTOPUS_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `octopus` package split is earlier in the migration.
...
- the current server host still lives in `packages/octopus`
...
- if `packages/server` imports services from `packages/octopus` while `packages/octopus` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/octopus` as the temporary Hono host
...
- `packages/server` must not import from `packages/octopus`

Allowed in phase 1:

- `packages/octopus` imports `packages/server`
...
- `packages/octopus` shrinks or disappears as package responsibilities are fully split
...
- `packages/octopus` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/octopus`
...
Keep in `packages/octopus` for now:
...
- host mounting from `packages/octopus`
...
- no imports from `packages/octopus` inside `packages/server`
...
- `packages/server` must still not import from `packages/octopus`
...
### PR 4. Mount `packages/server` question from `packages/octopus`

Scope:

- replace local experimental question route wiring in `packages/octopus`
...
- mount it in parallel from `packages/octopus`
...
- move adapter selection and server startup out of `packages/octopus`

Rules:

- do not start this while `packages/server` still depends on `packages/octopus`
...
- host logic no longer lives in `packages/octopus`
...
3. the route is mounted from the current host in `packages/octopus`
...
- `packages/server` imports services or runtime from `packages/octopus`
- `packages/octopus` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/octopus` imports
...
- [ ] mount `question` from `packages/octopus`
...
2. keep `packages/octopus` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/octopus`, stop and restructure the boundary first.
