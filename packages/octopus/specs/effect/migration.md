...
Practical reference for new and migrated Effect code in `packages/octopus`.
...
When a service uses Effect Schema internally but needs Zod schemas for the HTTP layer, derive Zod from Schema using the `zod()` helper from `@octopus-ai/core/effect-zod`:
