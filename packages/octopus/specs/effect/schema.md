...
Practical reference for migrating data types in `packages/octopus` from
...
schema via `@octopus-ai/core/effect-zod`.
...
The walker in `@octopus-ai/core/effect-zod` exposes two explicit escape hatches for
...
All of `packages/octopus/src/config/` has been migrated. Files that still
...
- Use `@octopus-ai/core/effect-zod` for all Schema → Zod conversion.
