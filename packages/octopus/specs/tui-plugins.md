...
- Author package entrypoint is `@octopus-ai/plugin/tui`.
...
```json
{
  "$schema": "https://octopus.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/octopus-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@octopus-ai/plugin/tui`.
- `@octopus-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@octopus-ai/plugin/tui"
...
- The value is a semver range checked against the running Octopus version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.octopus` is absent, no check is performed (backward compatible).
...
- `octopus plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `octopus plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.octopus` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.octopus`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.octopus`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.octopus` dir or `OCTOPUS_CONFIG_DIR`, Octopus installs `@octopus-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@octopus-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Octopus's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@octopus-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.octopus/themes` area near the plugin config source.
...
- `--pure` / `OCTOPUS_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.octopus/plugins/tui-smoke.tsx`
- Local vim plugin: `.octopus/plugins/tui-vim.tsx`
- Local smoke config: `.octopus/tui.json`
- Local smoke theme: `.octopus/plugins/smoke-theme.json`
