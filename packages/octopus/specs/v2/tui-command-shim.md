...
Delete `packages/octopus/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/octopus/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/octopus/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/octopus`
- TUI plugin loader tests in `packages/octopus` if runtime plugin API wiring changed
