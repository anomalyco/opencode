...
export class Service extends Context.Service<Service, Interface>()("@octopus/Reference") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Reference.state")(function* (ctx) {
        ...
        const materializeAll = yield* Effect.cached(
          Flag.OCTOPUS_EXPERIMENTAL_SCOUT
            ? Effect.gen(function* () {
    ...
    return Service.of({
      init: Effect.fn("Reference.init")(function* () {
        if (!Flag.OCTOPUS_EXPERIMENTAL_SCOUT) return
      ...
      contains: Effect.fn("Reference.contains")(function* (target?: string) {
        if (!Flag.OCTOPUS_EXPERIMENTAL_SCOUT) return false
        const full = normalizedTarget(target)
