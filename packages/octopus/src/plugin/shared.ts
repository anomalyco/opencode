...
export async function checkPluginCompatibility(target: string, octopusVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(octopusVersion) || semver.major(octopusVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.octopus
  if (typeof range !== "string") return
  if (!semver.satisfies(octopusVersion, range)) {
    throw new Error(`Plugin requires octopus ${range} but running ${octopusVersion}`)
  }
}
