...
import { Identifier } from "@octopus-ai/core/util/identifier"
...
export class Service extends Context.Service<Service, Interface>()("@octopus/v2/Auth") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Writable, AuthError> = Effect.fnUntraced(function* () {
      if (process.env.OCTOPUS_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.OCTOPUS_AUTH_CONTENT)
        } catch {}
      }
