...
import { Flock } from "@octopus-ai/core/util/flock"
...
export class Service extends Context.Service<Service, Interface>()("@octopus/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.OCTOPUS_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.OCTOPUS_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.OCTOPUS_DISABLE_MODELS_FETCH) return {}
    ...
    if (!Flag.OCTOPUS_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
