...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in octopus.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check subdirectories too)
  const candidates = [
    path.join(baseDir, "octopus.json"),
    path.join(baseDir, "octopus.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".octopus", "octopus.json"),
      path.join(baseDir, ".octopus", "octopus.jsonc"),
  ...
  // Default to octopus.json if none exist
  return candidates[0]
}
