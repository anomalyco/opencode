...
import { createOctopusClient, type OctopusClient, type ToolPart } from "@octopus-ai/sdk/v2"
import { Agent } from "@/agent/agent"
import { Permission } from "@/permission"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

const runtimeTask = import("./run/runtime")
type ModelInput = Parameters<OctopusClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OCTOPUS_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OCTOPUS_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: OctopusClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OctopusClient, sessionID: string) {
        const cfg = await sdk.config.get()
        if (!cfg.data) return
        if (cfg.data.share !== "auto" && !Flag.OCTOPUS_AUTO_SHARE && !args.share) return
      ...
      async function current(sdk: OctopusClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OctopusClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OctopusClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OctopusClient) {
        ...
        async function loop(client: OctopusClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
