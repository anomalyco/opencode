...
import type { OctopusClient, PermissionRequest, QuestionRequest, ToolPart } from "@octopus-ai/sdk/v2"
...
type PromptModel = Parameters<OctopusClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<OctopusClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<OctopusClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<OctopusClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<OctopusClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<OctopusClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<OctopusClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<OctopusClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<OctopusClient["question"]["reject"]>[0]
