...
export class Service extends Context.Service<Service, Interface>()("@octopus/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old octopus.json/opencode.json shape) still get their settings applied.
  ...
  // folders between cwd and home, and OCTOPUS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OCTOPUS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OCTOPUS_TUI_CONFIG override, if set.
  if (Flag.OCTOPUS_TUI_CONFIG) {
    const configFile = Flag.OCTOPUS_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and OCTOPUS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.OCTOPUS_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.OCTOPUS_CONFIG_DIR) continue
