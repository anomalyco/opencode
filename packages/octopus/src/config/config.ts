...
import { NamedError } from "@octopus-ai/core/util/error"
import { Flag } from "@octopus-ai/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { type InstanceContext } from "../project/instance"
import { InstallationLocal, InstallationVersion } from "@octopus-ai/core/installation/version"
...
import { EffectFlock } from "@octopus-ai/core/util/effect-flock"
import { containsPath } from "../project/instance-context"
import { zod } from "@octopus-ai/core/effect-zod"
import { NonNegativeInt, PositiveInt, withStatics, type DeepMutable } from "@octopus-ai/core/schema"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in octopus config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for octopus serve and web commands",
  }),
...
// Uses the shared `DeepMutable` from `@octopus-ai/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@octopus/Config") {}

function globalConfigFile() {
  const candidates = ["octopus.jsonc", "octopus.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "octopus.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "opencode.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "octopus.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OCTOPUS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            try {
              ...
              log.warn("skipping wellknown auth entry with non-URL key", { key, url })
              continue
            }
            process.env[value.key] = value.token
            log.debug("fetching remote config", { url: `${url}/.well-known/opencode` })
            const response = yield* Effect.tryPromise(() => fetch(`${url}/.well-known/opencode`))
            if (!response.ok) {
        ...
        if (Flag.OCTOPUS_CONFIG) {
          yield* merge(Flag.OCTOPUS_CONFIG, yield* loadFile(Flag.OCTOPUS_CONFIG))
          log.debug("loaded custom config", { path: Flag.OCTOPUS_CONFIG })
        }

        if (!Flag.OCTOPUS_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.OCTOPUS_CONFIG_DIR) {
          log.debug("loading config from OCTOPUS_CONFIG_DIR", { path: Flag.OCTOPUS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.OCTOPUS_CONFIG_DIR) {
            for (const file of ["octopus.json", "octopus.jsonc", "opencode.json", "opencode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.octopus/plugin(s)` or `.opencode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OCTOPUS_CONFIG_CONTENT) {
          const source = "OCTOPUS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OCTOPUS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from OCTOPUS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OCTOPUS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OCTOPUS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OCTOPUS_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OCTOPUS_PERMISSION))
        }
        ...
        if (Flag.OCTOPUS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OCTOPUS_DISABLE_PRUNE) {
