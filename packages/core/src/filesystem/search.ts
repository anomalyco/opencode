...
// Imported as a value from the schema package rather than through `../filesystem`:
// `filesystem.ts` reads `FileSystemSearch.node` while it initializes, so a runtime
// import back into it would make the two modules circular and let a bundler emit
// them in an order where `FileSystemSearch.node` is still undefined there.
