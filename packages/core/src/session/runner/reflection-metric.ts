...
export interface ModelReflectionStats {
  readonly agent: string
  readonly model: string
  triggers: number
  steers: number
  convergedNoop: number
  threshold: number
}

const modelStatsStore = new Map<string, ModelReflectionStats>()

const statsKey = (agent: string, model: string): string => `${agent}::${model}`

export const getLearnedHedgeThreshold = (agent: string, model: string): number => {
  const stats = modelStatsStore.get(statsKey(agent, model))
  return stats?.threshold ?? 0.25
}

export const getModelReflectionStats = (agent: string, model: string): ModelReflectionStats => {
  const key = statsKey(agent, model)
  const existing = modelStatsStore.get(key)
  if (existing) return existing
  const initial: ModelReflectionStats = {
    agent,
    model,
    triggers: 0,
    steers: 0,
  ...
  modelStatsStore.set(key, initial)
  return initial
}

export const updateLearnedHedgeThreshold = (
  agent: string,
  model: string,
  outcome: { readonly steered: boolean },
): number => {
  const stats = getModelReflectionStats(agent, model)
  stats.triggers++
  if (outcome.steered) {
    stats.steers++
    stats.threshold = Math.max(0.05, Math.round((stats.threshold - 0.03) * 100) / 100)
  } else {
    stats.convergedNoop++
    stats.threshold = Math.min(0.75, Math.round((stats.threshold + 0.02) * 100) / 100)
  }
  return stats.threshold
}

export const shouldTriggerLearnedHedge = (
  agent: string,
  model: string,
  hedgeScoreValue: number,
): boolean => {
  const threshold = getLearnedHedgeThreshold(agent, model)
  return hedgeScoreValue >= threshold
}
