...
// Pure draft selectors and the owned-assistant recipe runner are module-scoped so a
// projected event allocates no helper closures. `update()` dispatches through the
// static handler table below, threading the per-event `adapter` as an argument.
...
type Cases = typeof SessionEvent.All.cases
type Handlers = {
  [K in keyof Cases]: (adapter: Adapter, event: Cases[K]["Type"]) => Effect.Effect<void>
}

const handlers: Handlers = {
  "session.next.agent.switched": (adapter, event) => {
  ...
  "session.next.model.switched": (adapter, event) => {
    return adapter.appendMessage(
  ...
  "session.next.prompted": (adapter, event) => {
    return adapter.appendMessage(
  ...
  "session.next.context.updated": (adapter, event) =>
  ...
  "session.next.synthetic": (adapter, event) => {
    return adapter.appendMessage(
  ...
  "session.next.shell.started": (adapter, event) => {
    return adapter.appendMessage(
  ...
  "session.next.shell.ended": (adapter, event) => {
  ...
  "session.next.step.started": (adapter, event) => {
    return Effect.gen(function* () {
  ...
  "session.next.step.ended": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      draft.time.completed = event.data.timestamp
  ...
  "session.next.step.failed": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      draft.time.completed = event.data.timestamp
      draft.finish = "error"
  ...
  "session.next.text.started": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      draft.content.push(
  ...
  "session.next.text.delta": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestText(draft, event.data.textID)
      if (match) match.text += event.data.delta
    })
  },
  "session.next.text.ended": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestText(draft, event.data.textID)
      if (match) match.text = event.data.text
    })
  },
  "session.next.tool.input.started": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      draft.content.push(
        castDraft(
  ...
  "session.next.tool.input.ended": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestTool(draft, event.data.callID)
      if (match && match.state.status === "pending") match.state.input = event.data.text
    })
  },
  "session.next.tool.called": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestTool(draft, event.data.callID)
      if (match) {
  ...
  "session.next.tool.progress": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestTool(draft, event.data.callID)
  ...
  "session.next.tool.success": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestTool(draft, event.data.callID)
      if (match && match.state.status === "running") {
        match.provider = {
  ...
  "session.next.tool.failed": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestTool(draft, event.data.callID)
  ...
  "session.next.reasoning.started": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      draft.content.push(
        castDraft(
  ...
  "session.next.reasoning.delta": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestReasoning(draft, event.data.reasoningID)
      if (match) match.text += event.data.delta
    })
  },
  "session.next.reasoning.ended": (adapter, event) => {
    return updateOwnedAssistant(adapter, event.data.assistantMessageID, (draft) => {
      const match = latestReasoning(draft, event.data.reasoningID)
      if (match) {
  ...
  "session.next.compaction.ended": (adapter, event) => {
    return adapter.appendMessage(
...
export function update(adapter: Adapter, event: SessionEvent.Event): Effect.Effect<void>
