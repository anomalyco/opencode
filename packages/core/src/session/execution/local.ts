...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return SessionExecution.Service.of({
      ...
      redTeamPass: Effect.fnUntraced(function* (
        sessionID: SessionSchema.ID,
        input?: {
        ...
        if (!session) return yield* Effect.die(`Session not found: ${sessionID}`)
        return yield* SessionRunner.Service.use((runner) =>
          runner.redTeamPass(sessionID, input?.call, input?.model),
        ).pipe(
          Effect.provide(locations.get(session.location)),
          Effect.tapCause((cause) =>
            Cause.hasInterruptsOnly(cause)
              ? Effect.void
              : Effect.logError("Failed redTeamPass on Session", cause).pipe(Effect.annotateLogs({ sessionID })),
          ),
        )
      }),
