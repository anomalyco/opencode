...
const PROMPT_WORK = `You are OpenWork, an agentic AI for knowledge work — documents, files, research, and deliverables. You are NOT a coding assistant. You work like a competent human colleague who hands back finished work, not step-by-step explanations.
...
export const Plugin = define({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.agent.transform((draft) => {
      ...
      draft.update(AgentV2.ID.make("work"), (item) => {
        item.description =
          "OpenWork — knowledge work agent for documents, files, research, and deliverables. Hands back finished work, not narration."
        item.system = PROMPT_WORK
        item.mode = "primary"
        item.color = "accent"
        item.steps = 50
        // Folder protocol: OUTPUTS/ is writable, context folders are read-mostly, destructive ops always ask.
        // Resources are glob patterns relative to the work tree, matching PermissionV2 conventions used by `plan`.
        item.permissions.push(
          ...PermissionV2.merge(defaults, [
            { action: "question", resource: "*", effect: "allow" },
            { action: "plan_enter", resource: "*", effect: "deny" },
            ...
            { action: "edit", resource: path.join("TEMPLATES", "**", "*"), effect: "ask" },
            { action: "edit", resource: "FOLDER.md", effect: "ask" },
            // Destructive shell ops always ask, regardless of target.
            { action: "bash", resource: "rm *", effect: "ask" },
            { action: "bash", resource: "unlink *", effect: "ask" },
            { action: "bash", resource: "trash *", effect: "ask" },
