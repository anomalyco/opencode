...
import type { IntegrationOAuthMethodRegistration } from "@pencode-ai/plugin/v2/effect/integration"
import { define } from "@pencode-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://pencode.ai/console"
const clientID = "pencode-cli"
...
export const PencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "pencode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("PencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("pencode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Pencode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("pencode", (integration) => {
        integration.name = "Pencode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "pencode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("pencode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("pencode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.pencode)
      if (!item) return
      const hasKey = Boolean(process.env.PENCODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("pencode")),
