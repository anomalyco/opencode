...
import type { IntegrationOAuthMethodRegistration } from "@argus-ai/plugin/v2/effect/integration"
import { define } from "@argus-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://argus.ai/console"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    integrationID: Integration.ID.make("argus"),
    method: {
      id: methodID,
      type: "oauth",
      label: "Argus Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const ArgusPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "argus",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ArgusPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("argus")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Argus provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("argus", (integration) => {
        integration.name = "Argus"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "argus", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("argus")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("argus")
      ...
      const item = catalog.provider.get(ProviderV2.ID.argus)
      if (!item) return
      const hasKey = Boolean(process.env.ARGUS_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("argus")),
