...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const removeSession = Effect.fnUntraced(function* (id: PtyID) {
      ...
      sessions.delete(id)
      yield* Effect.logInfo("removing session", { service: "pty", id })
      teardown(session)
    ...
    const create = Effect.fn("Pty.create")(function* (input: PreparedCreate) {
      ...
      yield* Effect.logInfo("creating session", { service: "pty", id, cmd: input.command, args: input.args, cwd: input.cwd })
      ...
      session.listeners.push(
        ...
        proc.onExit(({ exitCode }) => {
          ...
          runFork(
            Effect.gen(function* () {
              yield* Effect.logInfo("session exited", { service: "pty", id, exitCode })
    ...
    const connect = Effect.fn("Pty.connect")(function* (id: PtyID, ws: Socket, cursor?: number) {
      ...
      yield* Effect.logInfo("client connected to session", { service: "pty", id, directory: location.directory })
      const sub = sock(ws)
