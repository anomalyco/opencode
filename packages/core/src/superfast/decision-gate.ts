export * as DecisionGate from "./decision-gate"

/**
 * Superfast Decision Gate — a "System One" front-door classifier.
 *
 * A small, non-autoregressive decision server evaluates the pending user turn in a
 * single forward pass and returns typed, calibrated answers (choice / noul / score)
 * without generating text. The gate turns those answers into a routing
 * recommendation so the harness can skip expensive work when the decision is
 * obvious.
 *
 * Design contract:
 *   - Fail-open. Any error, timeout, non-2xx, or schema miss returns `null`.
 *     Callers MUST treat `null` as "no opinion" and continue exactly as they would
 *     with the gate disabled. The gate can only ever make the harness faster, never
 *     change its behaviour when it is unsure.
 *   - No new runtime dependencies. The gate talks to a local HTTP endpoint with
 *     plain `fetch`, so OpenCode never bundles the decision model.
 *   - Off by default. Nothing here runs unless `superfast.enabled` is true.
 */

/** Wire-level question kinds supported by the decision protocol. */
...
/** Partial settings as read from config (all fields optional). */
...
export function resolveGateSettings(input?: SuperfastSettingsInput): DecisionGateSettings {
  return {
    ...
    timeoutMs: typeof input?.timeoutMs === "number" && input.timeoutMs > 0 ? input.timeoutMs : DEFAULT_GATE_SETTINGS.timeoutMs,
  }
}

/**
 * Issue one System One request. Returns the parsed answers on success, or `null` on
 * any failure (fail-open). Never throws except when the caller's own `signal` is
 * aborted, which propagates as an AbortError.
 */
export async function querySystemOne(
  ...
  try {
    ...
    if (!json || typeof json.answers !== "object" || json.answers === null) return null
    return json.answers
  } catch (err) {
...
export async function classifyTurn(
  ...
  return { route: deriveRoute(answers), answers, latencyMs: Date.now() - startedAt }
}
...
function deriveRoute(answers: Record<string, DecisionAnswer>): TurnRoute {
  ...
  if (typeof fromContext === "number" && fromContext >= 0.85 && (needsTool === undefined || needsTool <= 0.3)) {
...
/**
 * Lightweight health probe. Returns true only if the endpoint answers a trivial
 * noul question within the timeout.
 */
export async function probeBackend(settings: DecisionGateSettings): Promise<boolean> {
  const answers = await querySystemOne("health check", { ok: { type: "noul", instructions: "Is the service healthy?" } }, settings)
