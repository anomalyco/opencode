...
import { ToolFailure } from "@pencode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@pencode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.PENCODE_WEBSEARCH_PROVIDER === "exa" || process.env.PENCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.PENCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("PENCODE_EXPERIMENTAL") || truthy("PENCODE_ENABLE_EXA") || truthy("PENCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("PENCODE_ENABLE_PARALLEL") || truthy("PENCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `pencode/${InstallationVersion}`,
