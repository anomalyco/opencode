  ...
  Effect.gen(function* () {
    ...
    yield* tools
      .register({
        [name]: Tool.make({
          description:
            "Manage the active per-session goal. Re-read the current goal from the <session-goal> block in the system prompt first. Use sparingly. Allowed actions: update (change text), pause, resume, complete (only after verification). Provide verification evidence when completing.",
          input: Input,
          output: Output,
          ...
          execute: (input, context) =>
            Effect.gen(function* () {
              ...
              if (input.action === "update") {
                if (!input.text) {
                  const g = yield* goals.get(context.sessionID)
                  return { goal: g ?? null }
                }
