...
const Loop = Schema.Literals(["why", "then", "redteam"])

export const Input = Schema.Struct({
  loop: Loop.annotate({
    description:
      "Why iterates reflection from the current state backward to ground the goal; Then projects the current state forward to surface consequences, contradictions, or risks; Redteam simulates an adversarial attack against candidate actions/plans.",
  }),
...
export const Output = Schema.Struct({
  ...
  steered: Schema.Boolean.annotate({
    description:
      "Whether the loop admitted a steered session input. False means the Meta-Mirror / Then Loop / Red-Team pass converged without needing intervention.",
  }),
  ...
  text: Schema.String.annotate({
    description:
      "The reflection, projection, or attack critique text. For Why, the new goal if changed; for Then, the forward projection if risks were found; for Redteam, the attack outcome.",
  }),
})
...
const redteamConverged = "Plan survived red-team attack without fatal flaws."
...
const redteamSteered =
  "Red-team attack found fatal flaws; plan rejected. Steered input admitted with counter-model."

const statusText = (loop: "why" | "then" | "redteam", steered: boolean) => {
  if (loop === "why") return steered ? whySteered : whyConverged
  if (loop === "then") return steered ? thenSteered : thenConverged
  return steered ? redteamSteered : redteamConverged
}
              ...
              const result =
                input.loop === "why"
                  ...
                  : input.loop === "then"
                    ...
                    : yield* runner.redTeamPass(context.sessionID)
