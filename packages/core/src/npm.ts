...
async function fetchRegistryVersion(name: string): Promise<string | undefined> {
  ...
  try {
    ...
    return data?.["dist-tags"]?.latest as string | undefined
  } catch {
    return undefined
  }
}
  ...
  Effect.gen(function* () {
    ...
    const add = Effect.fn("Npm.add")(function* (pkg: string) {
      ...
      if (yield* afs.existsSafe(path.join(dir, "node_modules", name))) {
        const installed = yield* afs.readJson(path.join(dir, "node_modules", name, "package.json")).pipe(Effect.option)
        if (isLatest && Option.isSome(installed)) {
          const current = (installed.value as any)?.version as string | undefined
          if (current) {
