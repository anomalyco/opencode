      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const first = yield* credentials.create({
            integrationID: Integration.ID.make("lifecycle"),
          ...
          expect(switched.map((event) => event.data)).toEqual([
            { integrationID: Integration.ID.make("lifecycle"), from: undefined, to: first.id },
            { integrationID: Integration.ID.make("lifecycle"), from: first.id, to: second.id },
            { integrationID: Integration.ID.make("lifecycle"), from: second.id, to: first.id },
          ])
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const first = yield* credentials.create({
            integrationID: Integration.ID.make("switch"),
            methodID: Integration.MethodID.make("key"),
          ...
          expect((yield* credentials.active(Integration.ID.make("switch")))?.id).toBe(third.id)
          expect(Array.from(yield* Fiber.join(switchedFiber)).map((event) => event.data)).toEqual([
            { integrationID: Integration.ID.make("switch"), from: undefined, to: first.id },
            { integrationID: Integration.ID.make("switch"), from: first.id, to: second.id },
            { integrationID: Integration.ID.make("switch"), from: second.id, to: third.id },
          ])
        }).pipe(Effect.provide(testLayer(tmp.path))),
      ),
    ),
