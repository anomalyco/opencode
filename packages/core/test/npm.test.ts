...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@pencode/acme")).toBe("@pencode/acme")
    expect(Npm.sanitize("@pencode/acme@1.0.0")).toBe("@pencode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/pencode/acme.git"
    const expected = win ? "acme@git+https_//github.com/pencode/acme.git" : spec
