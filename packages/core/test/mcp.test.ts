...
import { codeModeTools, executeTool, toolDefinitions, toolIdentity, waitForTool } from "./lib/tool"
...
it.effect("advertises MCP output schemas to Code Mode", () =>
  Effect.gen(function* () {
    ...
    expect(codeModeTools(toolSet.codeModeCatalog!).find((tool) => tool.path === "demo.search")?.signature).toContain(
      "ok: boolean",
    )
...
it.effect("forwards the invoking session through direct and Code Mode MCP tools", () =>
  Effect.gen(function* () {
    ...
    expect(
      codeModeTools(toolSet.codeModeCatalog!).find((tool) => tool.path === "demo.search")?.signature,
...
it.effect("returns content-only MCP results through Code Mode", () =>
  Effect.gen(function* () {
    ...
    expect(codeModeTools(toolSet.codeModeCatalog!).some((tool) => tool.path === "demo.status")).toBe(true)
...
it.effect("waits for permission before calling an MCP tool", () =>
  Effect.gen(function* () {
    ...
    yield* registration.flush
    const toolSet = yield* registry.snapshot()
    expect(codeModeTools(toolSet.codeModeCatalog!).some((tool) => tool.path === "demo.search")).toBe(true)
