...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("excludes configuration-only environment variables from credentials", () =>
    Effect.gen(function* () {
      const integrations = yield* Integration.Service
      const catalog = yield* Catalog.Service
      const snapshots = [
        {
          ...
          expected: ["CLOUDFLARE_API_KEY"],
        },
        {
          id: "cloudflare-ai-gateway",
          environment: ["CLOUDFLARE_ACCOUNT_ID", "CLOUDFLARE_GATEWAY_ID", "CLOUDFLARE_API_TOKEN", "CF_AIG_TOKEN"],
          expected: ["CLOUDFLARE_API_TOKEN", "CF_AIG_TOKEN"],
        },
        {
          id: "google-vertex",
          environment: ["GOOGLE_VERTEX_PROJECT", "GOOGLE_VERTEX_LOCATION", "GOOGLE_APPLICATION_CREDENTIALS"],
          expected: ["GOOGLE_VERTEX_PROJECT", "GOOGLE_VERTEX_LOCATION", "GOOGLE_APPLICATION_CREDENTIALS"],
        },
        {
          id: "databricks",
          environment: ["DATABRICKS_HOST", "DATABRICKS_TOKEN"],
          expected: ["DATABRICKS_TOKEN"],
        },
        {
          id: "snowflake-cortex",
          environment: ["SNOWFLAKE_ACCOUNT", "SNOWFLAKE_CORTEX_PAT"],
          expected: ["SNOWFLAKE_CORTEX_PAT"],
        },
        {
          id: "neon",
          environment: ["NEON_AI_GATEWAY_BASE_URL", "NEON_AI_GATEWAY_TOKEN"],
          expected: ["NEON_AI_GATEWAY_TOKEN"],
        },
        {
          id: "infomaniak",
          environment: ["INFOMANIAK_API_KEY", "INFOMANIAK_PRODUCT_ID"],
          expected: ["INFOMANIAK_API_KEY"],
        },
        {
          id: "watsonx",
          environment: ["WATSONX_AI_APIKEY", "WATSONX_AI_PROJECT_ID"],
          expected: ["WATSONX_AI_APIKEY"],
        },
        {
          id: "amazon-bedrock",
          environment: ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_REGION", "AWS_BEARER_TOKEN_BEDROCK"],
          expected: ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_BEARER_TOKEN_BEDROCK"],
        },
      ]
      yield* ModelsDevPlugin.effect(
        host({
            ...
            get: () =>
              Effect.succeed(
                snapshots.map((snapshot) => ({
                  info: {
                    id: Provider.ID.make(snapshot.id),
                    name: snapshot.id,
                    package: Provider.aisdk("@ai-sdk/openai-compatible"),
                  },
      ...
      yield* Effect.forEach(snapshots, (snapshot) =>
        integrations.get(Integration.ID.make(snapshot.id)).pipe(
          Effect.tap((integration) =>
            Effect.sync(() =>
              expect(integration).toMatchObject({
                methods: [{ type: "key" }, { type: "env", names: snapshot.expected }],
              }),
            ),
          ),
        ),
