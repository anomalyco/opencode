...
describe("fromPromise", () => {
      ...
      yield* PluginPromise.fromPromise(
        define({
          ...
          setup: async (ctx) => {
            ...
            // Exercise the runtime boundary with inputs a typed caller cannot send.
            const read = ctx.experimental.terminal.read as (input: unknown) => Promise<unknown>
            for (const lines of [0, -1, 1.5, 65536, NaN, Infinity, "3"]) {
              await expect(read({ sessionID: "ses_terminal", lines })).rejects.toBeDefined()
            }
            await expect(read({ lines: 3 })).rejects.toBeDefined()
            expect(seen).toEqual([])
    ...
    Effect.gen(function* () {
      ...
      yield* PluginPromise.fromPromise(
        define({
          ...
          setup: async (ctx) => {
            // Exercise runtime validation without reflection erasing the call signature.
            const create = ctx.session.create as (input: unknown) => Promise<unknown>
            await expect(create({ title: 42 })).rejects.toBeDefined()
    ...
    Effect.gen(function* () {
      ...
      const context = host({
        session: {
          ...
          update: (input) => Effect.sync(() => seen.push(input)),
      ...
      yield* PluginPromise.fromPromise(
        define({
          ...
          setup: async (ctx) => {
            expect(await ctx.session.interrupt({ sessionID: "ses_success", resume: true })).toEqual({
              interrupted: false,
            })
            ...
            expect(await ctx.session.update({ sessionID: "ses_success", title: "Renamed" })).toBeUndefined()
            expect(
  ...
  it.effect("does not bridge Promise host calls started after setup", () =>
    Effect.gen(function* () {
      const input = { sessionID: "ses_after_setup", text: "setup work", resume: false }
      const token = {}
      const called = yield* Deferred.make<boolean>()
      const activation: PluginActivation.State = {
        active: true,
      ...
      const context = host({
        session: {
          synthetic: () =>
            Effect.gen(function* () {
              yield* Deferred.succeed(called, (yield* PluginActivation.Bridged) === token)
              return SessionInbox.Synthetic.make({
                id: SessionMessage.ID.make("msg_after_setup"),
                sessionID: Session.ID.make(input.sessionID),
                time: { created: DateTime.makeUnsafe(0) },
                type: "synthetic",
      ...
      yield* PluginPromise.fromPromise(
        define({
          id: "promise-after-setup",
          setup: (ctx) => {
            queueMicrotask(() =>
              queueMicrotask(() => queueMicrotask(() => void ctx.session.synthetic({ ...input, text: "late work" }))),
            )
          },
        }),
      )
      ...
      expect(yield* Deferred.await(called).pipe(Effect.timeout("1 second"))).toBe(false)
    }),
  )

  it.effect("drains bridged host calls before propagating setup failure", () =>
    Effect.gen(function* () {
      const input = { sessionID: "ses_failed_setup", text: "pending work", resume: false }
        ...
        session: {
          synthetic: () =>
            Deferred.succeed(entered, undefined).pipe(
              ...
              Effect.as(
                SessionInbox.Synthetic.make({
                  id: SessionMessage.ID.make("msg_failed_setup"),
                  sessionID: Session.ID.make(input.sessionID),
                  time: { created: DateTime.makeUnsafe(0) },
                  type: "synthetic",
                  payload: { text: input.text },
      ...
      yield* Effect.gen(function* () {
        const setup = yield* PluginPromise.fromPromise(
          define({
            id: "promise-failed-setup",
            setup: (ctx) => {
              void ctx.session.synthetic(input)
              throw new Error("setup failed")
            },
          }),
        )
          .effect(context)
          .pipe(Effect.provideService(PluginActivation.Current, activation), Effect.exit, Effect.forkChild)
        yield* Deferred.await(entered)
        yield* Effect.yieldNow
        const completedBeforeRelease = setup.pollUnsafe()
        yield* Deferred.succeed(release, undefined)
        const result = yield* Fiber.join(setup)

        expect(completedBeforeRelease).toBeUndefined()
        expect(result._tag).toBe("Failure")
      }).pipe(
        Effect.ensuring(
          Deferred.succeed(release, undefined).pipe(Effect.andThen(Effect.sync(() => (activation.active = false)))),
        ),
      )
    }),
  )

  it.effect("does not drain fire-and-forget prompts started during setup", () =>
    Effect.gen(function* () {
      const entered = yield* Deferred.make<void>()
      const release = yield* Deferred.make<void>()
      const activation: PluginActivation.State = {
            ...
            Deferred.succeed(entered, undefined).pipe(
              ...
              Effect.as(
                SessionInbox.User.make({
                  id: SessionMessage.ID.make("msg_setup_prompt"),
      ...
      const setup = yield* PluginPromise.fromPromise(
        define({
          id: "promise-setup-prompt",
          setup: (ctx) => {
            void ctx.session.prompt({ sessionID: "ses_setup_prompt", text: "nested prompt", resume: false })
          },
        }),
      )
        .effect(context)
      ...
      expect(completedBeforeRelease).toBeDefined()
    }),
  )
    ...
    Effect.gen(function* () {
      ...
      const promisePlugin = define({
        ...
        setup: async (ctx) => {
          ...
          await expect(ctx.integration.get({ integrationID: "missing" })).rejects.toThrow(
  ...
  it.effect("forwards direct agent and model list reads", () =>
    Effect.gen(function* () {
      ...
      yield* providers.transform((draft) =>
        draft.update(Provider.ID.make("test"), (provider) => {
          provider.activation = "enabled"
        }),
      )
      yield* models.transform((draft) =>
        draft.update(Provider.ID.make("test"), Model.ID.make("alias"), (model) => {
  ...
  it.effect("forwards session context hooks", () =>
    Effect.gen(function* () {
      ...
      expect(event.options).toEqual({ temperature: 0.4, reasoningEffort: "medium" })
    }),
  )
    ...
    Effect.gen(function* () {
      ...
      const promisePlugin = define({
        ...
        setup: async (ctx) => {
          ...
          expect((await ctx.vcs.branch.list({ search: "feat" })).data).toEqual(["feature"])
    ...
    Effect.gen(function* () {
      ...
      yield* PluginPromise.fromPromise(
        define({
          ...
          setup: async (ctx) => {
            expect(Object.keys(ctx.tool).sort()).toEqual(["hook", "list", "reload", "transform"])
  ...
  it.live("clears deleted tool options while retaining the namespace", () =>
    Effect.gen(function* () {
      ...
      expect(snapshot.codeModeCatalog?.tools).toMatchObject([
        { type: "namespace", name: "acme", tools: [{ type: "tool", name: "hello" }] },
      ])
      expect(original.definitions.map((tool) => tool.name)).toEqual(["acme_hello", "execute"])
      expect(
        yield* snapshot.execute({
  ...
  it.effect("returns content-only plugin results and rejected Promises through Code Mode", () =>
    Effect.gen(function* () {
        ...
        setup: async (ctx) => {
          await ctx.tool.transform((tools) => {
            tools.add({
              ...
              execute: async ({ fail }) => {
                if (fail) throw new Error("plugin request failed")
      ...
      expect(
        ...
        content: [{ type: "text", text: "plugin request failed" }],
        metadata: { error: true },
      })
    }),
  )
