...
import { ProjectV2 } from "@kancode/core/project"
...
import { WorkspaceV2 } from "@kancode/core/workspace"
...
describe("SessionV2.create", () => {
  ...
  it.effect("stores supplied immutable create attributes", () =>
    Effect.gen(function* () {
      ...
      ).toMatchObject({ location: { directory: location.directory, workspaceID }, agent: "default", model })
    }),
  )
  ...
  it.effect("returns the existing Session when one ID is reused with different create arguments", () =>
    Effect.gen(function* () {
      ...
      const changed = [
        ...
        { id, location, agent: AgentV2.ID.make("default") },
        {
          id,
          location,
  ...
  it.effect("returns the current Session projection after updates", () =>
    Effect.gen(function* () {
      ...
      yield* db.update(SessionTable).set({ agent: "default" }).where(eq(SessionTable.id, id)).run().pipe(Effect.orDie)

      expect(yield* session.create(input)).toMatchObject({ id: created.id, agent: "default" })
    }),
  )
  ...
  it.effect("returns the current Session projection after projected updates", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.create(input)).toMatchObject({ id, agent: "default" })
    }),
  )
