...
const itFile = testEffect(Layer.empty)
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const { db } = yield* Database.Service
          const events = yield* EventV2.Service
          const sessionID = SessionID.descending("ses_event_log_compaction_busy")
          const messageID = SessionV1.MessageID.ascending("msg_event_log_compaction_busy")
          yield* db
            .insert(ProjectTable)
            .values({ id: Project.ID.global, worktree: AbsolutePath.make("/project"), sandboxes: [] })
            .run()
          yield* db
            ...
            .values({
              id: sessionID,
              project_id: Project.ID.global,
              slug: "compaction-busy",
              directory: "/project",
              title: "compaction busy",
              version: "test",
            })
            .run()
