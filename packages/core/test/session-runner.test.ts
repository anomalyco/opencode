...
const execution = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const coordinator = yield* SessionRunCoordinator.make<SessionV2.ID, void, SessionRunner.RunError>({
      drain: (sessionID, options) => sessionRunner.run({ sessionID, force: options.force }),
    })
