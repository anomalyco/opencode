...
// test/preload.ts pins LEAKCODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.LEAKCODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.LEAKCODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.LEAKCODE_MODELS_PATH = undefined
  Flag.LEAKCODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.LEAKCODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.LEAKCODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.LEAKCODE_DISABLE_MODELS_FETCH = false
        }),
