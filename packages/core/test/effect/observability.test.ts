...
test("file logger keeps an active oversized log", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opencode-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
  ...
  await Effect.logInfo("concurrent run").pipe(
    Effect.provide(Logger.layer([fileLogger(file, "run-a")]).pipe(Layer.provide(NodeFileSystem.layer), Layer.orDie)),
    Effect.scoped,
    Effect.runPromise,
  )

  expect((await fs.stat(file)).size).toBeGreaterThan(20 * 1024 * 1024)
})
