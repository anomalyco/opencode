...
import { define } from "@leak-code/plugin/v2/effect"
...
import { LocationServiceMap } from "@leak-code/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "leak-code.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "leak-code.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
