...
import { define } from "@kancode/plugin/v2/effect"
...
import { LocationServiceMap } from "@kancode/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "kancode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "kancode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
