...
export const DEFAULT_USER_PROFILE: UserProfileData = {
  ...
  frameworks: ["react", "vite", "effect", "fastapi"],
  style: {
    explicitness: 0.9,
    abstraction_tolerance: 0.25,
    verbosity: 0.2,
    typing_rigor: 0.95,
  ...
  testing: {
    ...
    mock_preference: "avoid_mocks_test_actual_impl",
    benchmark_testing: true,
  },
  error_handling: {
    concurrency_pattern: "structured_concurrency",
    error_handling_pattern: "result_option_monads",
  ...
  workspace_ui: {
    ...
    output_presentation: "diffs_and_tables_first",
    conversational_filler_tolerance: 0.1,
    prefer_dense_tables: true,
  },
  git_vcs: {
    branch_naming_convention: "max_3_words_kebab_case_no_slashes",
    commit_message_format: "conventional_commits_type_scope_summary",
    default_branch: "dev",
    pr_workflow: "merge_to_dev_with_audit_summary",
  },
  documentation: {
    docstring_standard: "google_docstrings",
    comment_density: "non_obvious_constraints_only",
    in_chat_full_delivery: true,
    clickable_links: true,
  },
  playbooks: [
    {
      routine_name: "add_database_table",
      ...
      action_sequence: [
        ...
        "Generate or update migration script",
        "Add table to Database service schema export",
        "Run bun test to verify persistence",
      ],
      preferred_commands: ["bun run migration", "bun typecheck"],
      frequency: 5,
    },
    {
      routine_name: "build_go_worker_pool",
      trigger_pattern: "When creating concurrent worker pool in Go",
      action_sequence: [
        "Use golang.org/x/sync/errgroup for structured concurrency",
        "Handle context.Context cancellation with graceful drain",
        "Implement metrics with sync/atomic and Prometheus",
        "Include table-driven unit tests and BenchmarkWorkerPool with b.ResetTimer()",
      ],
      preferred_commands: ["go test -v ./...", "go test -bench=."],
      frequency: 4,
    },
  ],
  database_style: "drizzle_sqlite_snake_case",
}
