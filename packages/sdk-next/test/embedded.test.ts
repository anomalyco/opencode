...
const testSuiteDir = await mkdtemp(join(tmpdir(), "opencode-embedded-suite-"))
const originalDatabase = Flag.OPENCODE_DB
Flag.OPENCODE_DB = join(testSuiteDir, "opencode.sqlite")

afterAll(async () => {
  Flag.OPENCODE_DB = originalDatabase
  await rm(testSuiteDir, { recursive: true, force: true })
})

test("embedded client uses the real router and handlers", async () => {
  const directory = join(testSuiteDir, "handlers")
...
test("Location-owned runner events reach the ready global client", async () => {
  const directory = join(testSuiteDir, "events")
  await mkdir(directory, { recursive: true })
...
test("independent embedded hosts do not share live notifications", async () => {
  const directory = join(testSuiteDir, "hosts")
  await mkdir(directory, { recursive: true })
...
test("embedded client is available as a Layer service", async () => {
  const directory = join(testSuiteDir, "layer")
  await mkdir(directory, { recursive: true })
