...
import type { LLMClientShape } from "@pencode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("pencode"))
    return { type: "unsupported", reason: "provider is not openai, pencode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @pencode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains pencode-owned. The native runtime only adapts
      // the @pencode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
