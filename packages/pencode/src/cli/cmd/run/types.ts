// Shared type vocabulary for the direct interactive mode (`pencode --mini`).
//
...
import type { PencodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@pencode-ai/sdk/v2"
import type { TuiConfig } from "@pencode-ai/tui/config"
...
type PromptModel = Parameters<PencodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<PencodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<PencodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<PencodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<PencodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<PencodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<PencodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<PencodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<PencodeClient["question"]["reject"]>[0]
