import type { PermissionV1 } from "@pencode-ai/core/v1/permission"
import { FSUtil } from "@pencode-ai/core/fs-util"
// CLI entry point for `pencode run` and `pencode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`pencode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`pencode --mini --attach`): connects to a running
//      pencode server and runs interactive mode against it.
//
...
import { createPencodeClient, type PencodeClient, type ToolPart } from "@pencode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<PencodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run pencode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running pencode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to PENCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to PENCODE_SERVER_USERNAME or 'pencode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: PencodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: PencodeClient, sessionID: string) {
      ...
      async function current(sdk: PencodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: PencodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: PencodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: PencodeClient) {
        ...
        async function loop(client: PencodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createPencodeClient({
        baseUrl: "http://pencode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
