...
import { InstallationVersion } from "@pencode-ai/core/installation/version"
import { AppNodeBuilder } from "@pencode-ai/core/effect/app-node-builder"
import type { AssistantMessage, Message, PencodeClient, SessionMessageResponse } from "@pencode-ai/sdk/v2"
...
export const AuthMethodID = "pencode-login"
...
export class Service extends Context.Service<Service, Interface>()("@pencode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `pencode auth login` in the terminal",
      name: "Login with pencode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: PencodeClient) {
...
function makeUsageService(sdk: PencodeClient) {
...
async function loadDirectorySnapshot(sdk: PencodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, pencode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const pencodeProvider = providers[ProviderV2.ID.make("pencode")]
  const pencodeModel = pencodeProvider ? Provider.sort(Object.values(pencodeProvider.models))[0] : undefined
  if (pencodeProvider && pencodeModel) return { providerID: pencodeProvider.id, modelID: pencodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Pencode service failure", service })
}
