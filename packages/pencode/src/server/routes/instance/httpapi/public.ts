...
import { PencodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for pencode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(PencodeHttpApi)
...
export const PublicApi = PencodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "pencode",
    version: "1.0.0",
    description: "pencode api",
