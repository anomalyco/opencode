...
const TUI_SCHEMA_URL = "https://pencode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from pencode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const pencode = await pencodeFiles(input)
  for (const file of pencode) {
...
async function pencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "pencode"),
    ...(await Filesystem.findUp(["pencode.json", "pencode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "pencode"))
  }
  if (Flag.PENCODE_CONFIG) files.push(Flag.PENCODE_CONFIG)
