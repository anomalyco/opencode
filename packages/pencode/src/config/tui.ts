...
import { isRecord } from "@pencode-ai/tui/util/record"
...
import { TuiKeybind } from "@pencode-ai/tui/config/keybind"
...
import { TuiConfig } from "@pencode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@pencode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old pencode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.pencode`
  // folders between cwd and home, and PENCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.PENCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit PENCODE_TUI_CONFIG override, if set.
  if (Flag.PENCODE_TUI_CONFIG) {
    const configFile = Flag.PENCODE_TUI_CONFIG
  ...
  // 4. `.pencode` directories (and PENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".pencode") || dir === Flag.PENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".pencode") && dir !== Flag.PENCODE_CONFIG_DIR) continue
