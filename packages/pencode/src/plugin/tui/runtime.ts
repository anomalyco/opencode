...
import { errorData, errorMessage } from "@pencode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@pencode-ai/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@pencode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@pencode-ai/core/v1/config/plugin"
import { createCommandShim } from "@pencode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@pencode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".pencode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".pencode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".pencode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.PENCODE_PURE ? [] : pluginOrigins
    if (Flag.PENCODE_PURE && pluginOrigins.length) {
    }
