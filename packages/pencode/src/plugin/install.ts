...
import { Flock } from "@pencode-ai/core/util/flock"
...
export type PatchDeps = {
  ...
  files: (dir: string, name: "pencode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".pencode")
}

function patchName(kind: Kind): "pencode" | "tui" {
  if (kind === "server") return "pencode"
  return "tui"
}
