...
export const DEPRECATED_PLUGIN_PACKAGES = ["pencode-openai-codex-auth", "pencode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, pencodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(pencodeVersion) || semver.major(pencodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.pencode
  if (typeof range !== "string") return
  if (!semver.satisfies(pencodeVersion, range)) {
    throw new Error(`Plugin requires pencode ${range} but running ${pencodeVersion}`)
  }
}
