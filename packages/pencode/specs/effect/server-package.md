...
Practical reference for a future `packages/server` split after the pencode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/pencode`.
...
Until enough shared service code lives outside `packages/pencode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/pencode`

It should not import `packages/pencode` services while `packages/pencode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on pencode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/pencode` implementation details.
