...
Trim `packages/pencode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/pencode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/pencode`.
- `[x]` `bun typecheck` from `packages/pencode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/pencode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/pencode`.
- `[x]` `bun dev generate > /tmp/pencode-openapi.json` from `packages/pencode`.
...
- Edit `packages/pencode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/pencode-openapi.json` from `packages/pencode`.
...
- Check source schemas in files like `packages/pencode/src/session/schema.ts`, `packages/pencode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/pencode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/pencode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
