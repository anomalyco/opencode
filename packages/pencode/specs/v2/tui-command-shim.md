...
Delete `packages/pencode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/pencode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/pencode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/pencode`
- TUI plugin loader tests in `packages/pencode` if runtime plugin API wiring changed
