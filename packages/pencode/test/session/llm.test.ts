...
import { LLMClient, RequestExecutor } from "@pencode-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const pencodeFixture = { providerID: "pencode-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for pencode providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(pencodeFixture.providerID),
          ModelV2.ID.make(pencodeFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(pencodeFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [pencodeFixture.providerID],
          provider: {
            [pencodeFixture.providerID]: {
              name: "Pencode Test",
