...
import { SessionV1 } from "@pencode-ai/core/v1/session"
import { ModelsDev } from "@pencode-ai/core/models-dev"
import { HttpRecorder } from "@pencode-ai/http-recorder"
import { HttpRecorderInternal } from "@pencode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@pencode-ai/llm"
import { RequestExecutor } from "@pencode-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.pencode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.PENCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["pencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("PENCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("PENCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["pencode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "pencode-proxy",
    name: "Pencode proxy",
    providerID: ProviderV2.ID.pencode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["pencode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.PENCODE_RECORD_CONSOLE_TOKEN && process.env.PENCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.pencode,
        name: "Pencode Zen",
        env: ["PENCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.PENCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.PENCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.PENCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["pencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("PENCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("PENCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("PENCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set PENCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the pencode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "pencode.json"),
      JSON.stringify({ $schema: "https://pencode.ai/config.json", ...scenario.config(model) }),
    ),
  )
