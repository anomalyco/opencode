...
import type { Event, PencodeClient } from "@pencode-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<PencodeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "pencode-acp-permission-"))
