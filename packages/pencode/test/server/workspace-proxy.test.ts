...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips pencode-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("http://localhost/test", {
          headers: {
            "x-pencode-directory": "/secret/path",
            "x-pencode-workspace": "ws_123",
            "x-custom": "preserved",
          },
        }),
      )
      ...
      expect(forwarded["x-pencode-directory"]).toBeUndefined()
      expect(forwarded["x-pencode-workspace"]).toBeUndefined()
