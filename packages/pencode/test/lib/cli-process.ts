// Subprocess test harness for the pencode CLI. Spawns the real binary against
...
//
// Configuration flows through pencode's built-in test affordances:
//   - PENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - PENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - PENCODE_DISABLE_PROJECT_CONFIG : skip walking up for pencode.json
//   - PENCODE_PURE                : skip external plugin discovery + install
//   - PENCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `pencode.run` is fully wired. The shape supports adding more
// builders (`pencode.serve(opts)`, `pencode.acp(opts)`, `pencode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of PencodeCli.
...
import { AppProcess } from "@pencode-ai/core/process"
...
const pencodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(pencodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    PENCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    PENCODE_CONFIG_CONTENT: configJson,
    PENCODE_DISABLE_PROJECT_CONFIG: "1",
    PENCODE_PURE: "1",
    PENCODE_DISABLE_AUTOUPDATE: "1",
    PENCODE_DISABLE_AUTOCOMPACT: "1",
    PENCODE_DISABLE_MODELS_FETCH: "1",
    PENCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `pencode run`. New flags should
...
// `pencode serve` is a long-lived process — it never exits on its own.
...
// `pencode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type PencodeCli = {
  ...
  // Spawn `pencode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `pencode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("pencode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          PENCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("pencode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("pencode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "pencode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `pencode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("pencode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const pencode: PencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, pencode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `pencode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "pencode") {
...
// every call site. Use this for any test that needs the pencode CLI fixture.
//
...
// resources (e.g. `pencode.serve`) without an extra `Effect.scoped` wrapper —
