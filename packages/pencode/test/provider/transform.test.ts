    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".pencode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, pencodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.pencode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.pencode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              pencode: { itemId: "msg_pencode_part" },
    ...
    expect(result[0].providerOptions?.pencode?.itemId).toBe("msg_pencode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.pencode?.itemId).toBe("msg_pencode_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["pencode", "deepseek-v4-flash"],
    ["pencode-go", "deepseek-v4-flash"],
