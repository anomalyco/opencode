...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "pencode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "pencode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "pencode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "pencode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["pencode.json", "pencode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "pencode.json"),
        path.join(project, "pencode.jsonc"),
      ])
    })
  })
