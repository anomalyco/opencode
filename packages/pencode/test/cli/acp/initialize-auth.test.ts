...
describe("pencode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ pencode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ pencode }))
        ...
        expect(initialized.agentInfo?.name).toBe("Pencode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ pencode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("pencode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "pencode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.PENCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("pencode-login")
