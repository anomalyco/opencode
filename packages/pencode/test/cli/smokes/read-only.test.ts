...
describe("pencode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // PENCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["mcp", "list"])
        pencode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["providers", "list"])
        pencode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["models"])
        pencode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["agent", "list"])
        pencode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh PENCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["session", "list"])
        pencode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["stats"])
        pencode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under PENCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ pencode }) =>
      Effect.gen(function* () {
        const r = yield* pencode.spawn(["db", "path"])
        pencode.expectExit(r, 0, "db path")
