...
import { PencodeClient, type GlobalEvent } from "@pencode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<PencodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<PencodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<PencodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<PencodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<PencodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://pencode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: PencodeClient["event"]["subscribe"]
    globalEvent?: PencodeClient["global"]["event"]
    promptAsync?: PencodeClient["session"]["promptAsync"]
    status?: PencodeClient["session"]["status"]
    messages?: PencodeClient["session"]["messages"]
    children?: PencodeClient["session"]["children"]
    permissions?: PencodeClient["permission"]["list"]
    questions?: PencodeClient["question"]["list"]
  } = {},
) {
  const client = new PencodeClient()

  const subscribe: PencodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: PencodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: PencodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: PencodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: PencodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: PencodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: PencodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: PencodeClient["question"]["list"] = input.questions ?? (() => ok([]))
