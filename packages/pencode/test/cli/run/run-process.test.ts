// Subprocess integration tests for `pencode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `pencode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `PENCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("pencode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pencode.run("say hi")
        pencode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pencode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* pencode.run("think", { extraArgs: ["--thinking"] })
        pencode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* pencode.run("think again")
        pencode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pencode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pencode.run("say hi", { format: "json" })
        pencode.expectExit(result, 0)

        const events = pencode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ pencode }) =>
      Effect.gen(function* () {
        const result = yield* pencode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pencode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* pencode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* pencode.run("request permission", { permission: { bash: "ask" } })
        pencode.expectExit(denied, 0)
        ...
        const allowed = yield* pencode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        pencode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* pencode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        pencode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, pencode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* pencode.serve()

        const result = yield* pencode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, pencode }) =>
      Effect.gen(function* () {
        const result = yield* pencode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, pencode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* pencode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
