...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ pencode }) =>
    Effect.gen(function* () {
      const result = yield* pencode.spawn(["--replay-limit", "10"])

      pencode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ pencode }) =>
    Effect.gen(function* () {
      const result = yield* pencode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ pencode }) =>
    Effect.gen(function* () {
      const result = yield* pencode.spawn(["--mini", "--port", "4096"])
