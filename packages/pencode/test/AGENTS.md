...
- `config?: Partial<Config.Info>` - Write an `pencode.json` config file
...
- Directories are created in the system temp folder with prefix `pencode-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("pencode-test-")
  }),
)
```
