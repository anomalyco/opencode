...
it.instance("plan agent denies edits except .pencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".pencode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".pencode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.PENCODE_TEST_HOME
      process.env.PENCODE_TEST_HOME = test.directory
