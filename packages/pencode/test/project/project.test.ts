...
import { Hash } from "@pencode-ai/core/util/hash"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const pencodeFile = path.join(tmp, ".git", "pencode")
      expect(yield* Effect.promise(() => Bun.file(pencodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "pencode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "pencode")
      const wrongCache = path.join(parentDir, ".git", "pencode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "pencode")
      const cacheB = path.join(bareB, "pencode")
      const wrongCache = path.join(parentDir, ".git", "pencode")
