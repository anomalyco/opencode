...
describe("OpenAI Responses route", () => {
  ...
  it.effect("lowers chronological system updates to escaped user wrappers in order", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.body.input).toEqual([
        ...
        { role: "assistant", status: "completed", content: [{ type: "output_text", text: "After." }] },
      ])
    }),
  )
    ...
    Effect.gen(function* () {
      ...
      expect(prepared.body).toMatchObject({
        input: [
          ...
          { role: "assistant", status: "completed", content: [{ type: "output_text", text: "It shows a small test image." }] },
          ...
          { role: "assistant", status: "completed", content: [{ type: "output_text", text: "Paris is 22 degrees." }] },
          {
            role: "user",
            ...
            Effect.gen(function* () {
              ...
              expect(body).toMatchObject({
                input: [
                  ...
                  { role: "assistant", status: "completed", content: [{ type: "output_text", text: "The parser changed." }] },
  ...
  it.effect("preserves assistant content order around reasoning items", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.body.input).toEqual([
        { role: "assistant", status: "completed", content: [{ type: "output_text", text: "Before." }] },
        {
          type: "reasoning",
          encrypted_content: "encrypted-state",
