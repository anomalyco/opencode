...
describe("OpenAI-compatible Chat route", () => {
  it.effect("prepares generic Chat target", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.model.route.transport.endpoint).toMatchObject({
        baseURL: "https://api.deepseek.test/v1/",
  ...
  it.effect("provides model helpers for compatible provider families", () =>
    Effect.gen(function* () {
      expect(
        providerFamilies.map(([provider, family]) => {
          ...
          return {
            ...
            baseURL: model.route.transport.endpoint?.baseURL,
            apiKey: model.apiKey,
          }
        }),
      ).toEqual(
      ...
      expect(custom.route.transport.endpoint?.baseURL).toBe("https://custom.deepseek.test/v1")
    }),
  )
