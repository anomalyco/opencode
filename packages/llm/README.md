...
Schema-first LLM core for pencode. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@pencode-ai/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@pencode-ai/llm/providers"
