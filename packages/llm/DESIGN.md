...
> current private `@pencode-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@pencode-ai/ai`
...
- Secondary audience: Pencode and other durable agent runtimes
...
import { LLM } from "@pencode-ai/ai"
import { OpenAI } from "@pencode-ai/ai/providers/openai"
...
```ts
import { Message } from "@pencode-ai/ai"
...
import { LLM, Tool } from "@pencode-ai/ai"
...
Pencode and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@pencode-ai/ai/promise"
import { OpenAI } from "@pencode-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@pencode-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@pencode-ai/ai/provider"
...
@pencode-ai/ai/promise
  Promise/AsyncIterable LLM facade

@pencode-ai/ai/schema
  serializable domain schemas

@pencode-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@pencode-ai/ai/providers/openai
@pencode-ai/ai/providers/anthropic
@pencode-ai/ai/providers/google
...
```
...
Pencode should migrate to `generateTurn` / `streamTurn`, preserving its durable
