...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Pencode launch", () => {
    expect(hasExistingAppState([file("pencode.settings")])).toBe(true)
    expect(hasExistingAppState([file("pencode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("pencode")])).toBe(true)
  })
})
