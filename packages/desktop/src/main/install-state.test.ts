...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Leak Code launch", () => {
    expect(hasExistingAppState([file("leak-code.settings")])).toBe(true)
    expect(hasExistingAppState([file("leak-code.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("leak-code")])).toBe(true)
  })
})
