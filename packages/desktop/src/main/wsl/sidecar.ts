...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export LEAKCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export LEAKCODE_CLIENT=desktop",
    `export LEAKCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export LEAKCODE_SERVER_PASSWORD=${shellEscape(password)}`,
