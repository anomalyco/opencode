...
import { type WslCommandLine, resolveWslPencode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const pencode = await resolveWslPencode(distro)
  if (!pencode) throw new Error(nativeT("desktop.wsl.error.pencodeNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "pencode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export PENCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export PENCODE_CLIENT=desktop",
    `export PENCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export PENCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(pencode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
