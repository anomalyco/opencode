...
let releasePencodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectPencodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectPencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Pencode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Pencode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.pencode/bin/pencode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, pencodeChecks: {} })
})
...
test("ignores stale background Pencode checks after removing a WSL server", async () => {
  persistedServers = []
  releasePencodeResolve = undefined
  ...
  await waitFor(() => !!releasePencodeResolve)
  await controller.removeServer("wsl:Debian")
  releasePencodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().pencodeChecks).toEqual({})
})

test("ignores stale startup Pencode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Pencode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const pencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolvePencode: async (distro) => {
      pencode.push(distro)
      return "/home/me/.pencode/bin/pencode"
    },
  })
  ...
  expect(pencode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(pencode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().pencodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Pencode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const pencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(pencode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().pencodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolvePencode: async () => {
      await new Promise<void>((resolve) => {
        releasePencodeResolve = resolve
      })
      return "/home/me/.pencode/bin/pencode"
    },
  }
