  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-opencode", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Cedric CLI installation failed")
        }
        await refreshOpencodeCheck(name, { signal: abort.signal })
...
function opencodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "Cedric CLI (opencode) is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "Cedric CLI (opencode) is installed but could not run",
    }
  }
  return {
    distro,
