...
const TEST_ONBOARDING = process.env.OCTOPUS_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.OCTOPUS_DISABLE_EMBEDDED_WEB_UI = "true"
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("octopus://"))
    if (urls.length) {
  ...
  const needsMigration = ((): boolean => {
    if (process.env.OCTOPUS_DB === ":memory:") return false
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.OCTOPUS_PORT
    if (fromEnv) {
