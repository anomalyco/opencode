...
const TEST_ONBOARDING = process.env.IMECODE_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.IMECODE_DISABLE_EMBEDDED_WEB_UI = "true"
  ...
  const needsMigration = ((): boolean => {
    if (process.env.IMECODE_DB === ":memory:") return false
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.IMECODE_PORT
    if (fromEnv) {
