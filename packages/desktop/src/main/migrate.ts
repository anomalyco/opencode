...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.pencode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `pencode.settings.dat` is special: it maps to the `pencode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"pencode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // pencode.settings.dat → the electron settings store ("pencode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "pencode.global.dat").
  const storeName = filename === "pencode.settings.dat" ? "pencode.settings" : filename
