...
import "./styles.css"
import { Splash } from "@pencode-ai/ui/logo"
import { useTheme } from "@pencode-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.PENCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "pencode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__PENCODE__ ??= {}
  const pending = window.__PENCODE__.deepLinks ?? []
  window.__PENCODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `pencode.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://pencode.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("pencode.global.dat").getItem("language")
