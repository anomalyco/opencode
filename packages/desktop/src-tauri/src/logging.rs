...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("athena-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("athena_lib=debug,athena_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("athena_lib=info,athena_desktop=info,sidecar=info")
        }
    });
