...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("xcsh-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("xcsh_lib=debug,xcsh_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("xcsh_lib=info,xcsh_desktop=info,sidecar=info")
        }
    });
