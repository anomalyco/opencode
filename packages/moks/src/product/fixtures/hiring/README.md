...
```bash
cp packages/moks/src/product/fixtures/hiring/HIRING.md .
mkdir -p candidates
cp packages/moks/src/product/fixtures/hiring/candidates/jordan-lee.md candidates/
...
Path constant for tests/tools: `HiringFixtures` in `packages/moks/src/product/fixtures.ts`.

Mock-LLM E2E (no paid API): from `packages/moks`,  
