...
From `packages/moks` (package root):
...
```json
{
  "mcp": {
    "ashby": {
      "type": "local",
      "command": ["bun", "run", "/abs/path/to/packages/moks/src/product/fixtures/mcp/ashby-mock.ts"],
      "enabled": true
    }
  },
  "permission": {
...
Programmatic defaults: `ashbyPermissionDefaults()` / `ashbyMockMcpConfig(command)` in `packages/moks/src/product/ashby-edge.ts`. The native `recruit` agent already denies Ashby write tool names and allows the four reads.
