...
import { PromptInput } from "@pencode-ai/schema/prompt-input"
import { Session } from "@pencode-ai/schema/session"
import { Project } from "@pencode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@pencode-ai/schema/schema"
...
import { SessionEvent } from "@pencode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Pencode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Pencode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
