import { SessionMessage } from "@argus-ai/schema/session-message"
import { SessionInput } from "@argus-ai/schema/session-input"
import { PromptInput } from "@argus-ai/schema/prompt-input"
import { Session } from "@argus-ai/schema/session"
import { Project } from "@argus-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@argus-ai/schema/schema"
import { Workspace } from "@argus-ai/schema/workspace"
...
import { Agent } from "@argus-ai/schema/agent"
import { Model } from "@argus-ai/schema/model"
import { Location } from "@argus-ai/schema/location"
import { Revert } from "@argus-ai/schema/revert"
import { SessionEvent } from "@argus-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Argus process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Argus process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
