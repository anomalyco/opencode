    ...
    Effect.acquireUseRelease(
      ...
      (fixture) =>
        Effect.gen(function* () {
          ...
          expect((yield* request(base, "GET", `/api/session/${sessionID}/terminal`)).data).toEqual([])
          expect(existsSync(path.join(fixture.directory, "service.json"))).toBeFalse()
          const first = Schema.decodeUnknownSync(PersistentPty.Info)(
            (yield* request(base, "POST", `/api/session/${sessionID}/terminal`, {
              command: "/bin/sh",
          ...
          const terminals = Schema.decodeUnknownSync(Schema.Array(PersistentPty.Info))(
            (yield* request(base, "GET", `/api/session/${sessionID}/terminal`)).data,
          )
          ...
          expect((yield* request(base, "GET", `/api/session/${sessionID}/terminal`)).data).toMatchObject([
...
function waitForTerminals(base: string, sessionID: string, expected: unknown[]) {
  return Effect.tryPromise({
    try: async () => {
      for (let attempt = 0; attempt < 40; attempt++) {
        const response = await Effect.runPromise(request(base, "GET", `/api/session/${sessionID}/terminal`))
