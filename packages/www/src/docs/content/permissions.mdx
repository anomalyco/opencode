...
consulted. It analyzes Bash-compatible shells (`bash`, `dash`, `ksh`, `sh`, and
`zsh`) and PowerShell. Other shells and unsupported syntax, including heredocs,
are treated as opaque. Shell startup-hook environments such as `BASH_ENV`,
`ZDOTDIR`, and `ENV` are also opaque because they can execute code outside the
submitted command.
