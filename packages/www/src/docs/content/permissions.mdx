...
When enabled, supported commands use the portable scanner. Cases whose syntax
or output compatibility is not yet covered fall back to the existing tree-sitter
analysis. Tree-sitter remains available during this transition toward a
...
Arithmetic expressions, home-directory paths, sequential directory changes,
and commands under Zsh do not require stricter approval just because the
portable scanner cannot yet handle a case independently.
