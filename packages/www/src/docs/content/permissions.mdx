...
consulted. It analyzes Bash-compatible shells (`bash`, `dash`, `ksh`, and `sh`)
and PowerShell. Other shells and unsupported syntax, including heredocs, are
treated as opaque. `zsh` is opaque because it always loads `.zshenv`; shell
startup-hook environments such as `BASH_ENV` and `ENV` are also opaque because
they can execute code outside the submitted command.
