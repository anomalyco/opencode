...
The plugin context exposes the same worktree operations as the client. Create and refresh use the plugin's current
location unless overridden; `list` reads project-wide inventory without discovery.
...
The source defaults to the location's checkout. A supplied starting ref must be supported by the selected strategy;
...
New inventory entries record `configurationDirectory`, so removal without a location can reload the plugin from the
creating location even when the worktree lives elsewhere. If that directory is gone, supply another location that
registers the recorded strategy. Older entries without provenance resolve removal from their own directory.
