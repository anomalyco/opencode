...
Modify the title request before dispatch. Title generation is not an agent conversation: the event carries the system
prompt, the source message, and request options, but no agent or tools. A configured `title` agent still supplies the
