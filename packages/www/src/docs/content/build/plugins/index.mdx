...
Inspect or replace `tool` and `input` before top-level tool lookup, including for unknown names. `inputSchema` is
no longer provided. Corrected names still use the request's existing aliases and availability checks. Code Mode's
internal name resolution is unchanged.
