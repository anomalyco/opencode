...
Modify assembled system instructions, messages, tools, or request options immediately before model dispatch. Typed
`options` keys are the protocol-neutral generation settings; any other key is passed to the protocol as a provider
option.
...
Title generation has its own hook with the same shape minus the agent and tools.
