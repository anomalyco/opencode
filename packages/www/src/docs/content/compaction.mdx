...
compact short histories that would not trigger automatic compaction.

- By default, compaction takes priority over pending steering messages at the
  next safe step boundary, even if those messages were submitted first. It never
  interrupts an active model response or tool operation.
- Pending steers remain outside the checkpoint and are delivered afterward in
  their original order. Compaction does not cross an earlier session move in
  the same delivery lane.
- Explicit `delivery: "queue"` retains idle-boundary scheduling, after steers.
- Repeated requests while one is pending coalesce into that request. Completed
  or failed compaction settles the barrier so pending prompts can proceed.
