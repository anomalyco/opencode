...
The V2 terminal client is published on the `beta` distribution tag. Homebrew users can install the separate
`anomalyco/tap/opencode@2` formula. Both methods install `opencode2` without replacing V1's `opencode` executable. See the
[terminal startup guide](/cli).
