...
`/diff` defaults to **All**, combining branch and local changes. Press `d` (`diff.switch_source`) to open **Diff source**, then select **All** (branch + local changes), **Committed** (branch commits only), or **Uncommitted** (local changes only). The current scope is marked in the chooser. This selection applies only to the open viewer; set [`diffs.source` in `cli.json`](/cli/config#diffs) to change the default.

The one-row header shows the scope, base, and reviewed/total count. The source menu footer shows the branch base and PR number when known; opening it from **Uncommitted** does not resolve a base just for display.

For Git, branch scopes compare from the merge base of the open PR's base, the configured `branch.<current-branch>.gh-merge-base`, or the repository default, in that order. Candidates must resolve to locally available commit refs; a missing common ancestor is a comparison error, not an automatic fallback. Providers without base metadata can still supply combined branch comparisons, labeled **Base not reported**. A clean working tree or missing metadata does not switch sources automatically.
