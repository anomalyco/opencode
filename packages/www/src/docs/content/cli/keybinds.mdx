...
`/diff` defaults to **Branch + uncommitted**. Press `d` (`diff.switch_source`) to open **Diff source**, then select **Branch + uncommitted**, **Branch only**, or **Uncommitted only**. The current scope is marked in the chooser. This selection applies only to the open viewer; set [`diffs.source` in `cli.json`](/cli/config#diffs) to change the default.

For Git, branch scopes compare from the merge base of the open PR's base, the configured `branch.<current-branch>.gh-merge-base`, or the repository default, in that order. Candidates must resolve to locally available commit refs; a missing common ancestor is a comparison error, not an automatic fallback. The viewer shows the actual base and PR number when known. Providers without base metadata can still supply combined branch comparisons, labeled **Base not reported**. A clean working tree or missing metadata does not switch sources automatically.
