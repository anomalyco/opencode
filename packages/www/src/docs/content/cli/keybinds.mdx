...
`/diff` initially defaults to **All**, combining branch and local changes. Press `d` (`diff.switch_source`) to open **Diff source**, then select **All** (branch + local changes), **Committed** (branch commits only), or **Uncommitted** (local changes only). The current scope is marked in the chooser. Selecting a scope updates [`diffs.source` in `cli.json`](/cli/config#diffs), so reopening remembers your last choice.
...
Git uses a saved choice, recorded worktree-creation base, or validated named branch-creation reflog entry. When local inference is unclear, choose a base once; there is no network lookup or automatic fallback to the repository default from another branch. Saved bases belong to the Git branch or detached worktree. Reopening reloads fresh diff content without retaining review marks or scroll position. Providers without base metadata can still supply combined branch comparisons, labeled **Base not reported**. A clean working tree or missing metadata does not switch sources automatically.
