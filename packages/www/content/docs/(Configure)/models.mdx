...
takes precedence over the default; switching models changes that session and does not rewrite your config. When the
configured reference includes a variant, execution retains and applies it. Model-default APIs and plugin surfaces continue
to return the canonical base model because their `Model.Info` contract does not represent a selected variant.
...
- The root `model` retains an optional variant for execution. Model-default surfaces still report only the canonical base
  provider and model.
