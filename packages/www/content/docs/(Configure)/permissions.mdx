...
Experimental scanning is compatibility-gated: OpenCode uses portable output
only when its command and directory permission resources are a subset of the
default scanner's output. Unsupported or incomparable syntax, including
heredocs, falls back to the default scanner and cannot add permission prompts
or denials.
