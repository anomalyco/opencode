...
OpenCode normalizes model references using these legacy IDs to the canonical ID in memory. If the same config document
declares an exact native `providers` entry under the old-looking ID, that native declaration remains authoritative and the
model reference is left unchanged.
