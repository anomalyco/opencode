...
A model reference using one of the legacy IDs falls back to the canonical provider only when the legacy namespace is
absent. If native V2 configuration or a plugin explicitly defines the old-looking provider ID, that exact namespace is
authoritative and OpenCode does not redirect it. A successful fallback uses the canonical provider's package, settings,
credentials, and model identity; it does not borrow configuration from the legacy namespace.
