...
Set the default model in `provider/model` format, optionally followed by a
`#variant`. Execution retains the configured variant. Surfaces that report the
default model return the canonical base provider and model because their
`Model.Info` response does not include variant selection.
