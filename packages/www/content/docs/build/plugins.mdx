...
| `ctx.provider.hook("available", callback)`     | `available` for providers using automatic activation                           |
| `ctx.provider.hook("model.prepare", callback)` | `modelID` and `settings` immediately before provider model construction        |
...
Provider availability hooks run only for providers whose activation is `auto`;
explicitly enabled or disabled providers keep their configured state. Provider
preparation hooks run after catalog, variant, and credential settings
are merged and after native package mapping. They run for both native and AI SDK
models before URL variables are validated or the provider model is constructed.
The `package` and catalog `model` fields are readonly; mutate only the runtime
`modelID` and `settings`. Resolved secrets should remain in these transient
settings rather than being written through `catalog.transform`.
