...
test("a buffered diff does not overwrite a newer full message replacement", async () => {
  ...
  const newer = { ...raceUser, summary: { diffs: raceDiff("PATCH-B") } }
  ...
  try {
    ...
    emit(global({ id: "evt_message_b", type: "message.updated", properties: { sessionID, info: newer } }))
    await wait(() => {
      const message = sync.data.message[sessionID]?.[0]
      return message?.role === "user" && message.summary?.diffs[0]?.patch === "PATCH-B"
    })
