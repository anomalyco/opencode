...
test("clears a rehydrated pending compaction when it ends", async () => {
  const events = createEventStream()
  const sessionID = "session-compaction-rehydrated"
  const inputID = "message-compaction-rehydrated"
  const pending = [
    {
      admittedSeq: 1,
      id: inputID,
  ...
  try {
    await wait(() => client.connection.status() === "connected")
    await wait(() => rows.some((row) => row.type === "compaction-queued"))
    ...
    emitEvent(events, {
      ...
      type: "session.compaction.started",
      durable: durable(sessionID, 2),
      data: { sessionID, reason: "manual", recent: "", inputID },
    })
    await wait(() => data.session.pending.list(sessionID).length === 0)
