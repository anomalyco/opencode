...
import { EmbeddedTerminalRenderable, type OptimizedBuffer, type Renderable, ScrollBoxRenderable } from "@opentui/core"
...
test.each([0, 2, 50])(
  "keeps sidebar headings stable with %s details across tab switches, scrolling, and resizing",
  async (count) => {
    await using state = await tmpdir()
    const sessions = [
      "Investigate sidebar heading wrapping while switching between cached sessions",
      "Review terminal layout measurements and preserve stable title geometry",
    ].map((title, index) => ({
      id: `ses_sidebar_${index}`,
      title,
      projectID: "project",
      location: { directory },
      agent: "build",
    ...
    await using setup = await createAppFixture({
      ...
      args: { sessionID: sessions[0].id },
      fetch: (url) => {
        if (url.pathname === "/api/session")
          return json({ data: url.searchParams.get("parentID")?.startsWith("ses_") ? [] : sessions, cursor: {} })
        ...
        const session = sessions.find((session) => url.pathname.startsWith(`/api/session/${session.id}`))
        if (!session) return undefined
        if (url.pathname === `/api/session/${session.id}` || url.pathname.endsWith("/view"))
          return json({ data: session })
            ...
            data: [
              {
                ...
                finish: "stop",
                content: [{ type: "text", text: `Answer for ${session.id}` }],
              },
            ],
            cursor: {},
          })
    ...
    const sidebar = () =>
      setup
        .captureCharFrame()
        .split("\n")
        .map((line) => line.slice(-42))
    const findScroll = (node: Renderable): ScrollBoxRenderable | undefined =>
      node instanceof ScrollBoxRenderable && node.screenX >= setup.renderer.width - 42
        ? node
        : node.getChildren().map(findScroll).find(Boolean)
    await setup.waitForFrame(
      (frame) => frame.includes(`Answer for ${sessions[0].id}`) && (count === 0 || frame.includes("server-00")),
    )
    setup.mockInput.pressKey("o", { ctrl: true })
    await setup.waitForFrame((frame) => frame.includes("Search sessions"))
    await setup.waitForVisualIdle()
    await setup.mockInput.typeText("Review terminal")
    await setup.renderOnce()
    setup.mockInput.pressEnter()
    await setup.waitForFrame((frame) => frame.includes(`Answer for ${sessions[1].id}`))
    await setup.waitForVisualIdle()

    for (const index of [0, 1]) {
      const frames: string[][] = []
      const capture = (buffer: OptimizedBuffer) => {
        const pane = new TextDecoder()
          .decode(buffer.getRealCharBytes(true))
          .split("\n")
          .map((line) => line.slice(-42))
        // The sidebar can paint before the transcript; include the frame with its first title cell corrupted.
        if (pane.some((line) => line.includes(sessions[index].title.slice(1, 20)))) frames.push(pane)
      }
      setup.renderer.addPostProcessFn(capture)
      try {
        setup.mockInput.pressKey(String(index + 1), { ctrl: true })
        await setup.waitForFrame((frame) => frame.includes(`Answer for ${sessions[index].id}`))
        await setup.waitForVisualIdle()
        expect(frames.length).toBeGreaterThan(0)
        for (const frame of frames) expect(frame).toEqual(sidebar())
      } finally {
        setup.renderer.removePostProcessFn(capture)
      }
    }

    const heading = sidebar().slice(1, 3)
    const scroll = findScroll(setup.renderer.root)
    if (!scroll) throw new Error("Sidebar scrollbox not found")
    expect(scroll.horizontalScrollBar.visible).toBe(false)
    expect(scroll.verticalScrollBar.visible).toBe(count === 50)
    if (count === 50) {
      expect(scroll.verticalScrollBar.screenX).toBe(scroll.screenX + scroll.width - 1)
      await setup.mockMouse.scroll(scroll.screenX + 3, scroll.screenY + 2, "down")
      await setup.waitFor(() => scroll.scrollTop > 0)
      expect(sidebar().slice(1, 3)).toEqual(heading)
      await setup.mockMouse.drag(
        scroll.verticalScrollBar.screenX,
        scroll.verticalScrollBar.screenY + 1,
        scroll.verticalScrollBar.screenX,
        scroll.verticalScrollBar.screenY + scroll.verticalScrollBar.height - 1,
      )
      await setup.waitForFrame((frame) => frame.includes("server-49"))
      expect(sidebar().slice(1, 3)).toEqual(heading)
    }
    setup.resize(180, 80)
    await setup.waitForVisualIdle()
    ...
    setup.resize(180, 40)
    await setup.waitForVisualIdle()
    expect(findScroll(setup.renderer.root)).toBe(scroll)
    expect(scroll.verticalScrollBar.visible).toBe(count === 50)
    expect(sidebar().slice(1, 3)).toEqual(heading)
    setup.resize(120, 24)
    setup.mockInput.pressKey("F6")
    await setup.waitForFrame(() => sidebar().join("\n").includes(heading.join("\n")))
    await setup.waitForVisualIdle()
    const overlay = findScroll(setup.renderer.root)
    if (!overlay) throw new Error("Sidebar overlay not found")
    expect(overlay.verticalScrollBar.visible).toBe(count === 50)
    if (count === 50) {
      await setup.mockMouse.scroll(overlay.screenX + 3, overlay.screenY + 2, "down")
      await setup.waitFor(() => overlay.scrollTop > 0)
      expect(sidebar().join("\n")).toContain(heading.join("\n"))
    }
    setup.resize(180, 40)
    await setup.waitForVisualIdle()
    expect(sidebar().join("\n")).toContain(heading.join("\n"))
    expect(findScroll(setup.renderer.root)?.verticalScrollBar.visible).toBe(count === 50)
  },
)
