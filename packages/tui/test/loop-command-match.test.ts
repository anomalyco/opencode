...
describe("loop/backlog/queue command matching", () => {
  ...
  test("verbs with arguments are commands", () => {
    ...
    expect(isLoopCommand("/backlog my-change other --sync")).toBe(true)
  ...
  test("a real message that merely starts with the letters is NOT a command", () => {
    ...
    expect(isLoopCommand("/backlogged all week")).toBe(false)
