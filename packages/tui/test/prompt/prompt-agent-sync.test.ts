...
import { syncPromptMetadataFromUserMessage } from "../../src/component/prompt"
...
test("prompt metadata sync is a no-op without a session or message", () => {
  ...
  expect(
    recorder.sync({ sessionID: undefined, message: userMessage({ id: "msg_plan", agent: "plan" }), syncedKey: "previous" }),
  ).toBe("previous")
