      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>moks {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  const url = new URL("https://github.com/artemysone/moks/issues/new")
  ...
  const marker = "\n... (truncated)"
  const head = `The moks TUI crashed with an unexpected error.\n\n**Version:** ${InstallationVersion}\n**OS:** ${describeOS()}\n**Terminal:** ${describeTerminal()}\n\nReported automatically from the moks crash screen. If you can, describe what you were doing when it crashed.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
