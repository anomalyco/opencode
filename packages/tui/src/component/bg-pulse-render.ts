...
export class GoUpsellArtPainter {
  ...
  private paintedPhase: number | undefined
  ...
  private drawCached(frameBuffer: OptimizedBuffer, rgb: boolean) {
    // Returning to an exact phase can land just below its boundary after floating-point addition.
    const phase = Math.floor((this.elapsed / PERIOD) * CACHE_FRAME_COUNT + 1e-9) % CACHE_FRAME_COUNT
    if (phase === this.paintedPhase) return
    const frame = this.frameCache[phase]
    if (frame) {
