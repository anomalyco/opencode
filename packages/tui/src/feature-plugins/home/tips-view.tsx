...
const TIPS: Tip[] = [
  ...
  (shortcuts) => press(shortcuts.agentCycle(), "to cycle between Default, Plan, and CruiseControl agents"),
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public share link (when enabled)",
  ...
  "Switch to {highlight}CruiseControl{/highlight} for autonomous careful execution (set its model first)",
  ...
  "Create {highlight}kancode.json{/highlight} or {highlight}opencode.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/kancode/tui.json{/highlight} (falls back to opencode config dir)",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.kancode/commands/{/highlight} (or {highlight}.opencode/commands/{/highlight}) for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.kancode/agents/{/highlight} (or {highlight}.opencode/agents/{/highlight}) for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.kancode/tools/{/highlight} (or {highlight}.opencode/tools/{/highlight}) to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.kancode/plugins/{/highlight} (or {highlight}.opencode/plugins/{/highlight}) for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent KanCode from reading sensitive files",
  "Use {highlight}kancode run{/highlight} for non-interactive scripting",
  "Use {highlight}kancode --continue{/highlight} to resume the last session",
  "Use {highlight}kancode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}kancode serve{/highlight} for headless API access to KanCode",
  "Use {highlight}kancode run --attach{/highlight} to connect to a running server",
  "Run {highlight}kancode upgrade{/highlight} to update to the latest version",
  "Run {highlight}kancode auth list{/highlight} to see all configured providers",
  "Run {highlight}kancode agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencode{/highlight} in GitHub issues/PRs to trigger AI actions (upstream Action mention)",
  "Run {highlight}kancode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opencode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.kancode/themes/{/highlight} (or {highlight}.opencode/themes/{/highlight}) directory",
  ...
  "Run {highlight}kancode debug config{/highlight} to troubleshoot configuration",
