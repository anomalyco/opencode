...
function Notifications(context: Plugin.Context) {
  ...
  const ended = (sessionID: string) => {
    ...
    notify(context, sessionID, language.t("tui.plugins.sessionDone"), session?.parentID ? "subagent_done" : "done")
  }
  ...
  const dispose = [
    context.data.on("form.created", (event) => {
      ...
      notify(
        ...
        language.t("tui.plugins.inputNeedsResponse"),
        "question",
    ...
    context.data.on("permission.asked", (event) => {
      ...
      notify(context, event.data.sessionID, language.t("tui.plugins.permissionNeedsInput"), "permission")
    }),
    ...
    context.data.on("session.execution.failed", (event) => {
      ...
      if (route.type === "session" && route.sessionID === sessionID)
        context.ui.toast.show({
          title: language.t("tui.plugins.sessionFailed"),
  ...
  onCleanup(() => dispose.reverse().forEach((cleanup) => cleanup()))
  return null
}
...
export default Plugin.define({
  ...
  setup(context) {
    context.ui.slot({ append: "app", render: () => Notifications(context) })
  },
})
