import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@argus-ai/plugin/tui"
...
import type { useArgusKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useArgusKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getArgusModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getArgusModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
