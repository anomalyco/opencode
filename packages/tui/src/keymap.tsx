...
export const PENCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const PENCODE_MODE_KEY = "pencode.mode"

export const PencodeKeymapProvider = KeymapProvider
export const usePencodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type PencodeModeStack = ReturnType<typeof createPencodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, PencodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createPencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(PENCODE_MODE_KEY, PENCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(PENCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(PENCODE_MODE_KEY, stack.at(-1)?.mode ?? PENCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? PENCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(PENCODE_MODE_KEY, undefined)
...
export function usePencodeModeStack() {
  return getPencodeModeStack(usePencodeKeymap())
}

export function getPencodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Pencode mode stack is not registered for this keymap")
  return value
}
...
export function registerPencodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createPencodeModeStack(keymap)
