...
export const ARGUS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const ARGUS_MODE_KEY = "argus.mode"

export const ArgusKeymapProvider = KeymapProvider
export const useArgusKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ArgusModeStack = ReturnType<typeof createArgusModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ArgusModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createArgusModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ARGUS_MODE_KEY, ARGUS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ARGUS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ARGUS_MODE_KEY, stack.at(-1)?.mode ?? ARGUS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ARGUS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ARGUS_MODE_KEY, undefined)
...
export function useArgusModeStack() {
  return getArgusModeStack(useArgusKeymap())
}

export function getArgusModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Argus mode stack is not registered for this keymap")
  return value
}
...
export function registerArgusKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createArgusModeStack(keymap)
