...
// TestExactBugReproduction tests the exact scenario described in the spec:
// "read this:\n@path/to/my/file.txt" should restore without duplication
func TestExactBugReproduction(t *testing.T) {
	appInstance := &app.App{
		State: &app.State{
			MessageHistory: []app.Prompt{},
		},
	...
	// This is the exact message pattern that causes the bug
	exactBugText := "read this:\n@path/to/my/file.txt"

	// Create a prompt that simulates how GetAttachments() calculates indices
	// In multi-line text, GetAttachments() includes newlines in position tracking
	prompt := app.Prompt{
		Text: exactBugText,
		Attachments: []*attachment.Attachment{
			{
				...
				Type:    "file",
				Display: "@path/to/my/file.txt",
				// Position after "read this:\n" (11 characters including newline)
				StartIndex: 11,
				// End position after the full attachment path
				EndIndex: 31, // 11 + len("@path/to/my/file.txt") = 11 + 20 = 31
				Source: &attachment.FileSource{
					Path: "/test/path/to/my/file.txt",
					Mime: "text/plain",
				},
	...
	// Add to message history and restore (simulate up arrow key navigation)
	appInstance.State.MessageHistory = append(appInstance.State.MessageHistory, prompt)
	editor.RestoreFromHistory(0)

	// The restored text should exactly match the original
	result := editor.Value()
	if result != exactBugText {
		t.Errorf("EXACT BUG REPRODUCTION FAILED!")
		t.Errorf("Original: %q", exactBugText)
		t.Errorf("Restored: %q", result)
		t.Errorf("Original length: %d, Restored length: %d", len(exactBugText), len(result))

		// Check for the specific corruption pattern: duplicated attachment paths
		if len(result) > len(exactBugText) {
			t.Errorf("Text corruption detected: attachment path appears to be duplicated")
		}

		// Show character-by-character comparison for debugging
		minLen := len(exactBugText)
		if len(result) < minLen {
			minLen = len(result)
		}

		for i := 0; i < minLen; i++ {
			if exactBugText[i] != result[i] {
				t.Errorf("First difference at position %d: expected %q, got %q", i, exactBugText[i], result[i])
				break
			}
		}
	}

	// Verify that we have exactly one attachment in the correct position
	attachments := editor.textarea.GetAttachments()
	if len(attachments) != 1 {
		t.Errorf("Expected exactly 1 attachment after restoration, got %d", len(attachments))
	}

	if len(attachments) > 0 {
		att := attachments[0]
		if att.Display != "@path/to/my/file.txt" {
			t.Errorf("Attachment display text incorrect: expected %q, got %q", "@path/to/my/file.txt", att.Display)
		}
	}
}

func TestEdgeCaseLongAttachmentPaths(t *testing.T) {
	appInstance := &app.App{
		State: &app.State{
			MessageHistory: []app.Prompt{},
		},
	...
	// Test with very long attachment paths spanning multiple potential line wraps
	longPath := "@/very/long/path/to/some/deeply/nested/directory/structure/with/many/subdirectories/file.txt"
		...
		Attachments: []*attachment.Attachment{
			{
				...
				Type:       "file",
				Display:    longPath,
				StartIndex: 15, // After "Please review:\n"
				EndIndex:   15 + len(longPath),
				Source: &attachment.FileSource{
					Path: "/test" + longPath[1:], // Remove @ prefix
					Mime: "text/plain",
				},
			},
		},
	...
	result := editor.Value()
	if result != originalText {
		t.Errorf("Long attachment path test failed. Expected %q, got %q", originalText, result)
	}
}

func TestEdgeCaseAttachmentAtStartAndEnd(t *testing.T) {
	appInstance := &app.App{
		State: &app.State{
			MessageHistory: []app.Prompt{},
		},
	...
	// Test attachment at very start and very end of text
	originalText := "@start.txt middle content @end.txt"
	...
	prompt := app.Prompt{
		...
		Attachments: []*attachment.Attachment{
			...
			},
			{
				...
				Type:       "file",
				Display:    "@end.txt",
				StartIndex: 26, // Position of @end.txt
				EndIndex:   34,
				Source: &attachment.FileSource{
					Path: "/test/end.txt",
					Mime: "text/plain",
