...
The poster opens with all-time tokens and the full two-year totals. Its header shows the date range. The heatmap currently caps its display at the latest 53 weeks (fewer on narrow terminals).
...
Try a wide terminal (112 x 38) and a narrow one (48 x 30). Narrow views show the latest weeks and label the cropped activity range. The heatmap measures daily steps, as in the CLI; totals include all history. Reopening `/stats` loads a new snapshot.
