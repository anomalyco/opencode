...
import { InstallationLocal, InstallationVersion } from "@kancode/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@kancode/core/v1/config/console-state"
import { FSUtil } from "@kancode/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@kancode/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@kancode/core/v1/config/config"
import { RemoteAuthError } from "@kancode/core/v1/config/error"
import { ConfigPermissionV1 } from "@kancode/core/v1/config/permission"
import { ConfigPluginV1 } from "@kancode/core/v1/config/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@kancode/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://raw.githubusercontent.com/puetsua/kancode/main/schemas/kancode.schema.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://raw.githubusercontent.com/puetsua/kancode/main/schemas/kancode.schema.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://raw.githubusercontent.com/puetsua/kancode/main/schemas/kancode.schema.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://raw.githubusercontent.com/puetsua/kancode/main/schemas/kancode.schema.json"
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.kancode/plugin(s)` are already local files, so ConfigPlugin.load
