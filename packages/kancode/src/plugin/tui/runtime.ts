...
import { errorData, errorMessage } from "@kancode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@kancode/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@kancode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@kancode/core/v1/config/plugin"
import { createCommandShim } from "@kancode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@kancode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".kancode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".kancode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".kancode", "tui.json"),
  }
}
