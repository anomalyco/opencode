...
- The server still lives in `packages/kancode`.
...
Until enough shared service code lives outside `packages/kancode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/kancode`

It should not import `packages/kancode` services while `packages/kancode`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/kancode` implementation details.
