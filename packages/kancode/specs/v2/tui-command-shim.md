...
Delete `packages/kancode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/kancode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/kancode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/kancode`
- TUI plugin loader tests in `packages/kancode` if runtime plugin API wiring changed
