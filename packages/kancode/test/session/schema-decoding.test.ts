  ...
  test("retry carries attempt/message/action/next", () => {
    const input = {
      ...
      action: {
        ...
        message: "Provider free usage limit reached. Switch models or providers.",
        label: "docs",
        link: "https://github.com/puetsua/kancode",
      },
      next: 500,
    }
    expect(decode(input)).toEqual(input)
...
describe("SessionPrompt input schemas", () => {
  ...
  test("ShellInput requires agent + command", () => {
    ...
    const expected = { sessionID, agent: "default", command: "echo hi" }
