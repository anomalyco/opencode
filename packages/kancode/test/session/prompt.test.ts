...
import { NamedError } from "@kancode/core/util/error"
...
import { SessionMessageTable } from "@kancode/core/session/sql"
...
import { Ripgrep } from "@kancode/core/ripgrep"
...
import { LocationServiceMap, locationServiceMapLayer } from "@kancode/core/location-services"
...
it.instance("shell rejects with BusyError when loop running", () =>
  Effect.gen(function* () {
    ...
    const exit = yield* prompt.shell({ sessionID: chat.id, agent: "default", command: "echo hi" }).pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
    if (Exit.isFailure(exit)) {
      expect(Cause.squash(exit.cause)).toBeInstanceOf(Session.BusyError)
      expect(Cause.squash(exit.cause)).toMatchObject({ _tag: "SessionBusyError", sessionID: chat.id })
    ...
    withSh(() =>
      Effect.gen(function* () {
        ...
        const fiber = yield* prompt
          .shell({ sessionID: chat.id, agent: "default", command: "printf first && sleep 0.2 && printf second" })
          .pipe(Effect.forkChild)
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sh = yield* prompt
        .shell({ sessionID: chat.id, agent: "default", command: "sleep 0.2" })
        .pipe(Effect.forkChild)
      yield* waitForBusy(chat.id)
  ...
  () =>
    withSh(() =>
      Effect.gen(function* () {
        ...
        const sh = yield* prompt
          .shell({ sessionID: chat.id, agent: "default", command: ": > '.shell-ready'; sleep 30" })
          .pipe(Effect.forkChild)
        yield* pollWithTimeout(
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sh = yield* prompt.shell({ sessionID: chat.id, agent: "default", command: "sleep 30" }).pipe(Effect.forkChild)
      yield* waitForBusy(chat.id)
  ...
  () =>
    withSh(() =>
      Effect.gen(function* () {
        ...
        const a = yield* prompt
          .shell({ sessionID: chat.id, agent: "default", command: "sleep 30" })
          .pipe(Effect.forkChild)
        yield* waitForBusy(chat.id)
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/kancode/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) {
          expect(err.data.message).toContain("default")
        }
      }
    }),
  30_000,
