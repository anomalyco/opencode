...
for (const reasoningDefaultOpen of [false, true]) {
  story(
    ...
    async ({ mount }) => {
      ...
      await expect(group.locator('[data-slot="context-tool-group-title"]')).toHaveText("Used 4 Read, Skill")
      ...
      await expect(group.locator('[data-slot="context-tool-group-title"]')).toHaveText("Used 5 Read, Skill")
      await expect(group.locator('[data-slot="context-tool-group-item"]')).toHaveText([
        /Read.*group\.ts/,
        /Thought/,
        /Loaded.*opencode.*frontend-design.*skills/,
...
story("summarizes subagents as Agent while retaining their card titles", async ({ mount }) => {
  ...
  const title = header.locator('[data-slot="context-tool-group-title"]')
  await expect(title).toHaveText("Used 4 Shell, Read, Agent")
...
for (const width of [840, 390]) {
  story(`keeps grouped cards inside their trigger bounds at ${width}px`, async ({ mount, page }) => {
    ...
    const header = group.locator('[data-component="context-tool-group-trigger"]')
    await expect(header.locator('[data-slot="context-tool-group-title"]')).toHaveText("Used 4 Shell, Read, Agent")
