...
story("renders every admitted tool family and hides timeline-only exclusions", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--complete-agent-workflow")
  await expect(
...
story("renders every tool error outcome without leaking hidden tools", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--recover-from-tool-failures")
...
story("transitions shell and question through running error outcomes", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--failed-command-and-question")
...
story("labels all web search provider variants", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--compare-search-providers")
...
story("labels completed searches with result counts", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--search-results-and-files")
...
story("labels skill tools from IDs and result metadata", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--loading-specialized-skills")
...
story("groups singleton and separated context operations at correct boundaries", async ({ mount }) => {
  const timeline = await mount("current-session-research-agents--research-across-steps")
...
story("combines adjacent edit calls and repeated files into one group", async ({ mount }) => {
  const timeline = await mount("current-session-file-changes--repeated-edits")
