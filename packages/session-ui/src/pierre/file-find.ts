...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("argus-find")
  api.delete("argus-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("argus-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("argus-find", new Highlight(...rest))
    return true
  }
