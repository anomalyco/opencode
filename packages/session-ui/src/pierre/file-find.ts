...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("pencode-find")
  api.delete("pencode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("pencode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("pencode-find", new Highlight(...rest))
    return true
  }
