...
import { createPencode, type ToolPart } from "@pencode-ai/sdk"
...
console.log("🚀 Starting pencode server...")
const pencode = await createPencode({
  port: 0,
})
console.log("✅ Pencode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await pencode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new pencode session...")
    const { client, server } = pencode
    ...
    console.log("✅ Created pencode session:", createResult.data.id)
  ...
  console.log("📝 Sending to pencode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Pencode response:", JSON.stringify(result, null, 2))
