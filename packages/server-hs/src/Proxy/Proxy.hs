...
handleHttp state manager logFile req respond = do
  ...
  let bodyText = if LBS.length body > 0 
                 then Just $ decodeUtf8 $ LBS.toStrict $ 
  ...
  case result of
    ...
    Right resp -> do
      let respBody = HC.responseBody resp
          ...
          isStream = maybe False ("text/event-stream" `BS.isInfixOf`) $
      ...
      forM_ tokens $ \t -> atomically $ modifyTVar' (psTokenTotals state) $
      ...
      -- Forward response
      respond $ responseLBS respStatus 
...
getSessionLogs ProxyServer{..} sessionId = do
  ...
  let lines = C8.lines content
      entries = catMaybes [decode (LBS.fromStrict l) | l <- lines]
