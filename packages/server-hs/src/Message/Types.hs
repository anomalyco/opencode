...
instance FromJSON MessageTime where
  parseJSON = withObject "MessageTime" $ \v -> MessageTime
    <$> v .: "created"
...
instance ToJSON ToolState where
  toJSON (ToolPending inp raw) = object
    [ "status" .= String "pending"
    , "input" .= inp
    , "raw" .= raw
    ]
  toJSON (ToolRunning inp title) = object
    [ "status" .= String "running"
    , "input" .= inp
    , "title" .= title
    ]
  toJSON (ToolCompleted inp out title) = object
    [ "status" .= String "completed"
    , "input" .= inp
    , "output" .= out
    , "title" .= title
    ]
  toJSON (ToolError inp err) = object
    [ "status" .= String "error"
    , "input" .= inp
    , "error" .= err
    ]
...
instance ToJSON Part where
  ...
  toJSON (PartReasoning rp) = object
  ...
  toJSON (PartStepStart ssp) = object
  ...
  toJSON (PartStepFinish sfp) = object
  ...
  toJSON (PartSnapshot snp) = object
...
instance FromJSON TextPartInput where
  parseJSON = withObject "TextPartInput" $ \v -> TextPartInput
    <$> v .: "type"
    <*> v .: "text"
...
instance FromJSON FilePartInput where
  parseJSON = withObject "FilePartInput" $ \v -> FilePartInput
    <$> v .: "type"
    <*> v .: "mime"
    <*> v .: "url"
    <*> v .:? "filename"
