...
data NetworkMode
  = NetworkNone      -- ^ No network access (loopback only)
  | NetworkHost      -- ^ Share host network namespace
  | NetworkSlirp     -- ^ User-mode networking via slirp4netns
...
data MountSpec = MountSpec
  { msSource   :: FilePath   -- ^ Host path
  , msDest     :: FilePath   -- ^ Container path
  , msReadOnly :: Bool       -- ^ Read-only bind
  } deriving (Eq, Show, Generic)
...
data ResourceLimits = ResourceLimits
  { rlMemoryMax   :: Maybe Word64  -- ^ Memory limit in bytes
  , rlCpuMax      :: Maybe Word64  -- ^ CPU quota (microseconds per period)
  , rlCpuPeriod   :: Word64        -- ^ CPU period (default 100000us = 100ms)
  , rlPidsMax     :: Maybe Word64  -- ^ Max number of processes
  , rlNoNewPrivs  :: Bool          -- ^ PR_SET_NO_NEW_PRIVS
  } deriving (Eq, Show, Generic)
...
data Coeffects = Coeffects
  { cfNetwork    :: Bool         -- ^ Needs network access
  , cfAuth       :: [Text]       -- ^ Auth providers needed
  , cfFilesystem :: [FilePath]   -- ^ Filesystem paths needed (beyond sandbox)
  } deriving (Eq, Show, Generic)
...
data SandboxConfig = SandboxConfig
  { scRootfs      :: Maybe FilePath   -- ^ Custom rootfs (default: overlay on /)
  , scWorkdir     :: FilePath         -- ^ Working directory inside sandbox
  , scNetwork     :: NetworkMode      -- ^ Network isolation mode
  , scMounts      :: [MountSpec]      -- ^ Additional bind mounts
  , scEnv         :: [(Text, Text)]   -- ^ Environment variables
  , scLimits      :: ResourceLimits   -- ^ Resource limits
  , scCoeffects   :: Coeffects        -- ^ Declared coeffects
  , scSeccomp     :: Bool             -- ^ Enable seccomp-bpf filtering
  , scTmpfsSize   :: Word64           -- ^ Size of tmpfs overlay (bytes)
  } deriving (Eq, Show, Generic)
...
defaultConfig workdir = SandboxConfig
  { scRootfs    = Nothing
  , scWorkdir   = workdir
...
data SandboxStatus
  = SandboxRunning
  | SandboxExited Int   -- ^ Exit code
  | SandboxKilled Int   -- ^ Signal number
  deriving (Eq, Show, Generic)
...
data SandboxState = SandboxState
  ...
  , ssPid       :: ProcessID       -- ^ PID of the sandbox init process
  , ssOverlayDir :: FilePath       -- ^ Path to overlay upperdir (for cleanup)
