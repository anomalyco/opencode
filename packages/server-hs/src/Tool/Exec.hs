...
import Tool.Types
import LLM.Types (ToolUse(..), ToolResult(..))
...
parseAndRun :: ToolContext -> Value -> (ToolContext -> a -> IO ToolOutput) -> IO ToolOutput
...
execEdit ctx EditInput{..} = do
  ...
  case result of
    ...
    Right content -> do
      ...
      else if count > 1 && not replaceAll then
        pure $ ToolOutput "Edit Error" 
...
execBash ctx BashInput{..} = do
  ...
  let cmd = "timeout " <> show timeoutS <> " bash -c " <> show (T.unpack biCommand)
