  ...
  return {
    ...
    patent_draft: tool({
      ...
      async execute(args, ctx) {
        ...
        switch (action) {
          case "understand":
            return await draftUnderstand(disclosure, patent_type, invention_type, pluginContext)
          case "search":
            return await draftSearch(disclosure, pluginContext)
          case "specification":
            return await draftSpecification(disclosure, patent_type, invention_type, pluginContext)
          case "claims":
            return await draftClaims(disclosure, patent_type, invention_type, pluginContext)
          case "abstract":
            return await draftAbstract(disclosure, pluginContext)
          case "integrate":
            return await draftIntegrate(disclosure, pluginContext)
          default:
 ...
 */
async function loadYunPatDraftAgent(action: string, context: PatentPluginContext): Promise<any> {
  const agentMap: Record<string, string> = {
    understand: "agents/invention",
    specification: "agents/specification-drafter",
    claims: "agents/claim-generator",
    abstract: "agents/abstract-drafter",
  }

  const moduleName = agentMap[action]
