 ...
 * 注册质量检查工具集
 */
export async function registerCheckTools(context: PatentPluginContext) {
  return {
    /**
     * 专利质量检查（7 维度）
     */
    patent_check: tool({
      ...
      async execute(args, ctx) {
        ...
        if (action === "quality") {
          return await checkQuality(document, document_type, context)
        }

        if (action === "subject_matter") {
          return await checkSubjectMatter(document, context)
        }

        if (action === "unity") {
          return await checkUnity(document, context)
        }

        if (action === "formality") {
          return await checkFormality(document, document_type, context)
        }

        if (action === "consistency") {
          return await checkConsistency(document, context)
        }
...
async function checkQuality(document: string, docType: string, context: PatentPluginContext) {
...
async function checkSubjectMatter(document: string, context: PatentPluginContext) {
  const response = await context.llm.chat({
    messages: [
...
async function checkUnity(document: string, context: PatentPluginContext) {
  const response = await context.llm.chat({
    messages: [
...
async function checkFormality(document: string, docType: string, context: PatentPluginContext) {
  return `## 形式检查\n\n文件类型：${docType}\n\n检查项：\n- [ ] 格式符合国知局要求\n- [ ] 页码/段落编号正确\n- [ ] 附图标记一致\n- [ ] 引用文件格式正确\n\n> 注：完整形式检查需要接入 YunPat SpecFormalityChecker。`
}

async function checkConsistency(document: string, context: PatentPluginContext) {
  const response = await context.llm.chat({
    messages: [
