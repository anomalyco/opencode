...
async function startPostgresOnNetwork(
  ...
  console.log("[E2E] → Postgres: pulling / starting postgres:16-alpine (usually quick if cached)…")
  ...
  console.log("[E2E] ✓ Postgres accepting connections.")
...
async function startOllamaOnNetwork(
  ...
  console.log(
    "[E2E] → Ollama: pulling / starting ollama/ollama:latest — first-time image pull is often multi-GB and silent here; use Activity Monitor or `docker stats`.",
  )
  ...
  console.log("[E2E] → Ollama: container up; waiting for HTTP API…")
  await waitHttp(`${baseUrl}/api/tags`, 120_000)
  console.log("[E2E] → Ollama: pulling model llama3.2:1b inside container (minutes on first run; no progress bar here)…")
  ...
  if (pull.output.trim()) console.log("[E2E] ollama pull output:\n", pull.output.slice(-2000))
  ...
  console.log(`[E2E] Ollama models: ${names.join(", ")}`)
...
export async function startE2eTestcontainers(infra: ReadonlySet<E2eInfraLayer>): Promise<E2eTcLayer> {
  ...
  console.log(
    `[E2E] Testcontainers: Postgres${needOllama ? " + Ollama" : ""} on shared Docker network. Next lines explain each blocking step.`,
  )
  ...
  try {
    if (needOllama) {
      ...
      console.log(`[E2E] Postgres (Testcontainers): ${pg.databaseUrl}`)
      console.log(`[E2E] Ollama (Testcontainers): ${om.baseUrl}`)
      return {
...
Server.listen({ port: ${opencodeInternalApiPort}, hostname: "0.0.0.0" })
`.trim()
  ...
  const c = await new GenericContainer("oven/bun:1.1")
  ...
  console.log(`[E2E] OpenCode (Testcontainer) → http://127.0.0.1:${opts.hostApiPort}`)
  return {
    c,
    stop: async () => {
      await c.stop()
...
async function minioOnNetwork(appsNetwork: StartedNetwork) {
  ...
  try {
    child = await new GenericContainer("minio/minio:latest")
...
export async function startUniverE2e(
  ...
  try {
    console.log("[E2E] → Univer: MinIO on Docker network + bucket init…")
    minio = await minioOnNetwork(net)
    await minioReady(minio.endpoint)

    console.log("[E2E] → Univer: starting univer-compat (Testcontainer, alias veritly-univer)…")
    compat = await new GenericContainer("oven/bun:1.1")
      .withNetwork(net)
    ...
    console.log("[E2E] ✓ univer-compat ready (host:", origin, "cluster:", clusterUniverHttpOrigin, ")")
