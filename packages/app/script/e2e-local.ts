...
const athenaDir = path.join(repoDir, "packages", "athena")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "athena-e2e-"))
...
const serverEnv = {
  ...
  OPENCODE_E2E_MODEL: "athena/gpt-5-nano",
  OPENCODE_CLIENT: "app",
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../athena/src/util/log")
    const install = await import("../../athena/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../athena/src/server/server")
    inst = await import("../../athena/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`athena server listening on http://127.0.0.1:${serverPort}`)
