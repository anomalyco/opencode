...
import { parseExecutionTarget } from "./e2e/superpowers/execution-target"
...
const executionTarget = parseExecutionTarget(process.env.EXECUTION_E2E_TARGET)
const serverHost = process.env.PLAYWRIGHT_SERVER_HOST ?? executionTarget?.host ?? "127.0.0.1"
const serverPort = process.env.PLAYWRIGHT_SERVER_PORT ?? (executionTarget ? String(executionTarget.port) : "4096")
const command = built
