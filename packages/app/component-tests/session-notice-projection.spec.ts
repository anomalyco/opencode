...
story("renders current protocol notices in CLI order", async ({ mount, page }) => {
  ...
  const timeline = await mount("current-session-notice-projection--protocol-notice-order")
...
story("renders a compaction summary while it streams and after completion", async ({ mount }) => {
  const timeline = await mount("current-session-notice-projection--compaction-lifecycle")
...
story("shows a delegating row while subagent input streams", async ({ mount }) => {
  const timeline = await mount("current-session-notice-projection--streaming-delegation")
...
story("waits for completion before labeling requested background work", async ({ mount }) => {
  const timeline = await mount("current-session-notice-projection--requested-background-work")
