// 设 --total-scale-factor=1.5 后给 textLayer span 加红框,看是否对齐 canvas
...
await ev(`(() => {
  document.querySelectorAll(".pdf-page-wrapper").forEach(w => w.style.setProperty("--total-scale-factor", "1.5"))
  // 清掉 outline 再重设
  const tl = document.querySelector(".textLayer")
  tl.querySelectorAll("span").forEach(s => {
    s.style.outline = ""; s.style.outlineOffset = ""
...
writeFileSync("D:/tmp/deskfox-scale-fix-bounds.png", Buffer.from(r.data, "base64"))
console.log("Saved: D:/tmp/deskfox-scale-fix-bounds.png")
