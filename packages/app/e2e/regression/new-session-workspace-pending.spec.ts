...
for (const viewport of [
  ...
  for (const colorScheme of ["light", "dark"] as const) {
    test(`keeps the normal composer while creating a worktree on ${viewport.name} in ${colorScheme}`, async ({
      ...
      const placeholder = composer.getByText("Ask anything, / for commands, @ for context…", { exact: true })
      ...
      await editor.fill("")
      await expect(composer.getByText("Add follow-up, / for commands, @ for context…", { exact: true })).toBeVisible()
