...
const sessionA = session("ses_toggle_a", "Toggle A session")
const sessionB = session("ses_toggle_b", "Toggle B session")

test("toggling the tab strip hides it while keeping tab keybinds", async ({ page }) => {
  ...
  await expect(page.locator(`a[data-titlebar-tab-link][href="${hrefB}"]`)).toBeVisible()

  // The test browser emulates Win32, so "mod" resolves to Control.
  await page.keyboard.press("Control+Shift+B")
  await expect(strip).toBeHidden()

  // Per-tab shortcuts stay registered while the strip is only hidden with CSS.
...
function session(id: string, title: string) {
  return {
    id,
    slug: id,
    projectID: "project-tabs-toggle",
    directory: "C:/tab-toggle-project",
    title,
    version: "dev",
