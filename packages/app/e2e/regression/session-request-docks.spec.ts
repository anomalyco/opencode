...
test("shows a pending question dock", async ({ page }) => {
  ...
  const option = question.getByRole("radio", { name: /Minimal/ })
  await expect(option).toBeVisible()
  await expect(question.getByRole("radio", { name: /Extended/ })).toBeVisible()
  const optionLabel = option.locator('[data-slot="option-label"]')
  await expect(optionLabel).toBeVisible()
  const box = await optionLabel.boundingBox()
  if (!box) throw new Error("Missing option label bounds")
  await page.mouse.move(box.x + 2, box.y + box.height / 2)
  await page.mouse.down()
  await page.mouse.move(box.x + box.width - 2, box.y + box.height / 2)
  await page.mouse.up()
  expect(await page.evaluate(() => window.getSelection()?.toString())).toContain("Minimal")
