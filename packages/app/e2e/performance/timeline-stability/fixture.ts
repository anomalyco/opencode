...
import { Event } from "@pencode-ai/schema/event"
import { SessionStatusEvent } from "@pencode-ai/schema/session-status-event"
import { SessionV1 } from "@pencode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockPencodeServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/Pencode/TimelineStability"
...
export const model = { providerID: "pencode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("pencode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["pencode"],
    default: { providerID: "pencode", modelID: "claude-opus-4-6" },
  }
}
