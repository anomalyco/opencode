...
`session-close-release-benchmark.spec.ts` covers that eviction workload. It restores one kept tab plus six closable tabs with the same 20-message pages, visits every closable tab until its answer Markdown is ready, returns to the kept tab, closes the six visited tabs, and then reopens the first closed session from its Home row. Every message ID, tool ID, and answer text carries its own session ID. The fixture reports the message and inbox reads during reopen (the on-demand refetch cost of releasing) and the reopen latency. With `OPENCODE_PERFORMANCE_MEMORY=1` it also forces GC and takes a heap snapshot before and after closing, counting live string objects and bytes that mention each session ID; keep those runs separate from clean timing. Live events for closed sessions are not part of this workload.
