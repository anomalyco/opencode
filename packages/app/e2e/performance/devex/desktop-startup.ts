...
async function readService(profile: Awaited<ReturnType<typeof createColdProfile>>) {
  ...
  if (url.hostname !== "127.0.0.1" || Number(url.port) !== profile.serverPort)
...
async function availablePort() {
  ...
  if (!address || typeof address === "string") throw new Error("Failed to allocate a service port")
