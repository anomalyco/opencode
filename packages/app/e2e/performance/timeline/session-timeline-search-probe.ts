...
export async function installTimelineSearchProbe(
  page: Page,
  input: { targetPartID: string },
) {
      ...
      frame = requestAnimationFrame(() => {
        ...
        setTimeout(() => {
          ...
          samples.push({
            ...
            counter: search.querySelector<HTMLElement>('[data-slot="timeline-search-count"]')?.textContent?.trim() ?? "",
...
export async function waitForStableTimelineSearch(
  ...
  await page.waitForFunction(
    ({ counter, targetPartID }) => {
      const samples = (window as Window & { __timelineSearchBenchmark?: TimelineSearchProbe })
        .__timelineSearchBenchmark?.samples
...
export async function collectTimelineSearchMetrics(
  page: Page,
  input: { counter: string; targetPartID: string },
) {
  ...
  const first = (predicate: (sample: TimelineSearchSample) => boolean) =>
    result.samples.find(predicate)?.observedAtMs
  ...
  return {
    summary: {
      ...
      scrollDistancePx:
        final === undefined ? undefined : Math.abs(result.initialScrollTopPx - final.scrollTopPx),
