...
export async function mockOpenCodeServer(page: Page, config: MockServerConfig) {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (path === "/file" && config.fileList) return json(route, await config.fileList(url.searchParams.get("path") ?? ""))
    if (path === "/file/content" && config.fileContent) return json(route, await config.fileContent(url.searchParams.get("path") ?? ""))
