export const dict = {
  ...
  "desktop.ssh.error.invalidHost": "無効なSSHホスト: {{host}}",
  "desktop.ssh.error.alreadyAdded": "{{host}}はすでに追加されています",
  "desktop.ssh.error.opencodeNotInstalled": "{{host}}にはopencodeがインストールされていません",
  "desktop.ssh.error.installOpencode": "opencodeのインストールに失敗しました",
  "desktop.ssh.error.connectFailed": "{{host}}に接続できませんでした",
  "desktop.ssh.error.serverExited": "SSHサーバーが起動後に終了しました (code={{code}} signal={{signal}})",
  "desktop.ssh.error.serverExitedBeforeHealthy":
    "{{host}}のSSHサーバーが正常に稼働する前に終了しました (code={{code}} signal={{signal}}){{output}}",
  "desktop.ssh.error.healthTimeout": "{{host}}のサーバーのヘルスチェックが{{timeout}}ms後にタイムアウトしました",
  "desktop.ssh.error.commandTimeout": "{{host}}へのsshコマンドが{{timeout}}ms後にタイムアウトしました",
  "desktop.ssh.error.failedPort": "ポートを取得できませんでした",
  "desktop.ssh.error.terminalFailed": "ターミナルを開けませんでした",
  ...
  "ssh.server.add": "SSHサーバーを追加",
  "ssh.server.label": "SSH",
  "ssh.server.menu.label": "SSHサーバー",
  "ssh.server.retryStart": "再接続",
  "ssh.server.disconnect": "切断",
  "ssh.server.openTerminal": "ターミナルを開く",
  "ssh.server.updating": "更新中...",
  "ssh.server.status.connecting": "接続中...",
  "ssh.server.status.disconnected": "切断済み",
  "ssh.add.hostLabel": "SSHホスト",
  "ssh.add.hostPlaceholder": "user@example.com",
  "ssh.add.hint":
    "システムのsshクライアント経由で接続するため、SSH設定のホスト、キー、エージェントをそのまま利用できます。",
  "ssh.add.configHosts": "SSH設定から",
  "ssh.add.connect": "接続",
  "ssh.add.checking": "接続中...",
  "ssh.add.addServer": "サーバーを追加",
  "ssh.add.adding": "追加中...",
  "ssh.add.installOpencode": "opencodeをインストール",
  "ssh.add.updateOpencode": "opencodeを更新",
  "ssh.add.installing": "インストール中...",
  "ssh.add.status.unreachable": "接続できませんでした",
  "ssh.add.status.opencodeMissing": "このホストにはopencodeがインストールされていません",
  "ssh.add.status.opencodeMismatch":
    "opencode v{{version}}がインストールされていますが、デスクトップはv{{expected}}を想定しています",
  "ssh.add.status.opencodeFound": "opencode v{{version}}が見つかりました",
  "ssh.add.authHint":
    "このホストでキーベースのSSH認証が使えるか、ターミナルで`ssh {{host}}`を実行して確認してください。",
