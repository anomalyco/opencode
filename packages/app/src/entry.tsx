...
const getCurrentUrl = () => {
  if (location.hostname.includes("argus.ai")) return "http://localhost:4096"
  if (import.meta.env.VITE_ARGUS_SERVER_HOST || import.meta.env.VITE_ARGUS_SERVER_PORT)
    return `http://${import.meta.env.VITE_ARGUS_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_ARGUS_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" && !(import.meta.env.ARGUS_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
