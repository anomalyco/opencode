import type { Todo } from "@f5xc-salesdemos/sdk/v2"

export const composerEvent = "xcsh:e2e:composer"
...
export const composerEnabled = () => {
  ...
  return (window as ComposerWindow).__xcsh_e2e?.composer?.enabled === true
}

const root = () => {
  if (!composerEnabled()) return
  const state = (window as ComposerWindow).__xcsh_e2e?.composer
  if (!state) return
  state.sessions ??= {}
