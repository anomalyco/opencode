import type { Todo } from "@athena/sdk/v2"

export const composerEvent = "athena:e2e:composer"
...
export const composerEnabled = () => {
  ...
  return (window as ComposerWindow).__athena_e2e?.composer?.enabled === true
}

const root = () => {
  if (!composerEnabled()) return
  const state = (window as ComposerWindow).__athena_e2e?.composer
  if (!state) return
  state.sessions ??= {}
