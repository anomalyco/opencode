...
describe("resolveServerList", () => {
  test("lets startup auth_token credentials override a persisted same-url server", () => {
    const list = resolveServerList({
      stored: Schema.decodeUnknownSync(serverState())({ list: [{ url: "https://server.example.test" }] }).list,
      props: [
        {
          type: "http",
          authToken: true,
  ...
  test("keeps persisted credentials when startup has no auth_token", () => {
    const list = resolveServerList({
      stored: Schema.decodeUnknownSync(serverState())({
...
test("keeps exact persisted server identities and prevents removing provided servers", () => {
  const stored = Schema.decodeUnknownSync(serverState())({
...
test("project actions update schema-derived state and follow dynamic server scopes", () => {
  const [store, setStore] = createStore(Schema.decodeUnknownSync(serverState())({}))
