...
import { type Session } from "@octopus-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("octopus://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("octopus://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("octopus://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("octopus://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("octopus://open-project")).toBeUndefined()
    expect(parseDeepLink("octopus://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "octopus://open-project?directory=/a",
      "octopus://other?directory=/b",
      "octopus://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("octopus://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("octopus://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("octopus://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("octopus://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "octopus://new-session?directory=/a",
      "octopus://open-project?directory=/b",
      "octopus://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OCTOPUS__: {
        deepLinks: ["octopus://open-project?directory=/a"],
      },
    } as unknown as Window & { __OCTOPUS__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["octopus://open-project?directory=/a"])
