...
import { type Session } from "@f5xc-salesdemos/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("xcsh://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("xcsh://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("xcsh://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("xcsh://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("xcsh://open-project")).toBeUndefined()
    expect(parseDeepLink("xcsh://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "xcsh://open-project?directory=/a",
      "xcsh://other?directory=/b",
      "xcsh://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("xcsh://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("xcsh://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("xcsh://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("xcsh://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "xcsh://new-session?directory=/a",
      "xcsh://open-project?directory=/b",
      "xcsh://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __XCSH__: {
        deepLinks: ["xcsh://open-project?directory=/a"],
      },
    } as unknown as Window & { __XCSH__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["xcsh://open-project?directory=/a"])
