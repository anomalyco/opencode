...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("leak-code://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("leak-code://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("leak-code://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("leak-code://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("leak-code://open-project")).toBeUndefined()
    expect(parseDeepLink("leak-code://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "leak-code://open-project?directory=/a",
      "leak-code://other?directory=/b",
      "leak-code://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("leak-code://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("leak-code://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("leak-code://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("leak-code://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "leak-code://new-session?directory=/a",
      "leak-code://open-project?directory=/b",
      "leak-code://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __LEAKCODE__: {
        deepLinks: ["leak-code://open-project?directory=/a"],
      },
    } as unknown as Window & { __LEAKCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["leak-code://open-project?directory=/a"])
