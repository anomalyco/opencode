...
import { type Session } from "@athena/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("athena://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("athena://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("athena://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("athena://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("athena://open-project")).toBeUndefined()
    expect(parseDeepLink("athena://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "athena://open-project?directory=/a",
      "athena://other?directory=/b",
      "athena://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("athena://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("athena://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("athena://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("athena://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "athena://new-session?directory=/a",
      "athena://open-project?directory=/b",
      "athena://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ATHENA__: {
        deepLinks: ["athena://open-project?directory=/a"],
      },
    } as unknown as Window & { __ATHENA__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["athena://open-project?directory=/a"])
