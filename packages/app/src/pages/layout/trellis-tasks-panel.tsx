...
const meta = (task: TrellisTask) => [task.priority, task.assignee, task.package].filter((item): item is string => !!item)
...
function TaskCard(props: { task: TrellisTask; onOpen: (path: string) => void }): JSX.Element {
  ...
  const done = createMemo(() => props.task.completedAt || props.task.status === "done" || props.task.status === "completed")
  ...
  return (
    ...
    >
      <div class="flex items-start gap-3">
        <div
          class="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg border border-border-weak-base bg-background-base text-icon-base"
          classList={{ "text-icon-success-base": !!done(), "text-icon-brand-base": props.task.current }}
        >
          <Icon name={done() ? "circle-check" : "task"} size="small" />
        </div>
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            <div class="min-w-0 truncate text-14-medium text-text-strong">{props.task.title}</div>
          ...
          <div class="mt-1 flex flex-wrap items-center gap-1.5 text-12-regular text-text-base">
            <span class="rounded-md bg-surface-panel px-1.5 py-0.5">{labelStatus(props.task.status)}</span>
            <For each={items()}>{(item) => <span class="rounded-md bg-surface-panel px-1.5 py-0.5">{item}</span>}</For>
          </div>
          <div class="mt-2 truncate text-12-regular text-text-weaker">{getFilename(props.task.path)}</div>
        </div>
      </div>
    </button>
  )
      ...
      <div class="shrink-0 px-1 py-3">
        <div class="flex items-start justify-between gap-2 py-1 pl-2">
          <div class="min-w-0">
            ...
            <div class="mt-1 truncate text-12-regular text-text-base">{dir() || language.t("trellis.tasks.noProject")}</div>
          </div>
        ...
        <Show when={platform.listTrellisTasks} fallback={<Empty text={language.t("trellis.tasks.desktopOnly")} />}>
          <Show when={dir()} fallback={<Empty text={language.t("trellis.tasks.noProject")} />}>
            <Show when={!data.loading} fallback={<Empty text={language.t("trellis.tasks.loading")} />}>
              <Show when={!data.error} fallback={<ErrorCard err={errorMessage(data.error, language.t("common.requestFailed"))} />}>
