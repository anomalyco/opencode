...
export default function Page() {
  ...
  const waiting = createMemo(() => {
    const id = params.id
    if (!id) return false
    if (!messagesReady()) return true
    return !lastUserMessage() && busy(id)
  })
          ...
          <div class="flex-1 min-h-0 overflow-hidden">
            <Switch>
              <Match when={params.id}>
                <Show
                  when={!waiting()}
                  fallback={<div class="px-4 py-4 text-14-regular text-text-weak">{language.t("session.messages.loading")}</div>}
                >
