      ...
      <Show
        ...
        fallback={
          <div class="flex flex-col h-full pointer-events-none">
            ...
            <div class="flex-1 flex items-center justify-center text-text-weak">{props.language.t("terminal.loading")}</div>
          </div>
        }
      >
        <DragDropProvider
                ...
                {(pty) => (
                  ...
                  >
                    <Show when={pty.id} keyed>
                      <Terminal pty={pty} onCleanup={props.terminal.update} onConnectError={() => props.terminal.clone(pty.id)} />
                    </Show>
                  </div>
                )}
              </For>
                    ...
                    {(t) => (
                      <div class="relative p-1 h-10 flex items-center bg-background-stronger text-14-regular">
                        {terminalTabLabel({
                          ...
                          t: props.language.t as (key: string, vars?: Record<string, string | number | boolean>) => string,
                        })}
                      </div>
                    )}
                  </Show>
