...
type ExploreForkState =
  | {
      step: "point"
    }
  | {
      step: "target"
      point: DialogForkPoint
    }
...
export const DialogFork: Component<{ mode?: "message" | "worktree" }> = (props) => {
  ...
  const [state, setState] = createSignal<ExploreForkState>({ step: "point" })
  ...
  const restoredPrompt = (messageID: string): Prompt => {
    return extractPromptFromParts(sync.data.part[messageID] ?? [], {
  ...
  const exploreItems = createMemo(() => [
    { id: "full" as const, text: language.t("dialog.fork.fullSession"), time: "" },
    ...messages(),
  ])
  ...
  const handleExplorePoint = (item: ForkableMessage | { id: "full"; text: string; time: string } | undefined) => {
    if (!item) return
    if (item.id === "full") {
      setState({ step: "target", point: { type: "full" } })
      return
    }
    setState({
      step: "target",
      point: {
        type: "message",
        messageID: item.id,
        restored: restoredPrompt(item.id),
      },
    })
  }
  ...
  const handleExploreTarget = (target: ExploreForkTarget | undefined) => {
    const sessionID = params.id
    const current = state()
    if (!sessionID || current.step !== "target" || !target) return
    ...
    runExploreFork({
      ...
      markWorktreePending: WorktreeState.pending,
      waitForWorktree: async (directory) => {
        ...
        const waited = await Promise.race([WorktreeState.wait(directory), timeout])
      ...
      .then((forked) => {
        ...
        if (current.point.restored) prompt.set(current.point.restored, undefined, { dir, id: forked.sessionID })
  ...
  if (props.mode === "worktree") {
    return (
      <Dialog title={language.t("command.session.fork.worktree")}>
        <Show
          when={state().step === "target"}
          fallback={
            <>
              <div class="px-3 pb-2 text-12-regular text-text-weak leading-5">
                {language.t("dialog.fork.worktree.point.description")}
              </div>
              <List
