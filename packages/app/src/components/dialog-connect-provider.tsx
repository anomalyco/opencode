import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@pencode-ai/client/promise"
import { Button } from "@pencode-ai/ui/button"
import { useDialog } from "@pencode-ai/ui/context/dialog"
import { Dialog } from "@pencode-ai/ui/dialog"
import { Icon } from "@pencode-ai/ui/icon"
import { IconButton } from "@pencode-ai/ui/icon-button"
import { List, type ListRef } from "@pencode-ai/ui/list"
import { ProviderIcon } from "@pencode-ai/ui/provider-icon"
import { Spinner } from "@pencode-ai/ui/spinner"
import { Tag } from "@pencode-ai/ui/tag"
import { TextField } from "@pencode-ai/ui/text-field"
import { ButtonV2 } from "@pencode-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@pencode-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@pencode-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "pencode-go") return language.t("dialog.provider.pencodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "pencode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.pencode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["pencode", "pencode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "pencode" || provider.id === "pencode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "pencode"
                                ? "dialog.provider.pencode.tagline"
                                : "dialog.provider.pencodeGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "pencode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.pencodeZen.line1")}</div>
              <div>{language.t("provider.connect.pencodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.pencodeZen.visit.prefix")}
                <ExternalLink
                  href="https://pencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.pencodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.pencodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "pencode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.pencodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.pencodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.pencodeZen.visit.prefix")}
                <ExternalLink href="https://pencode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.pencodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.pencodeZen.visit.suffix")}
              </div>
