...
export function DialogSelectDirectory(props: DialogSelectDirectoryProps) {
  ...
  const missingBase = createMemo(
    () => !(sync.data.path.home || sync.data.path.directory),
  );
  ...
  const home = createMemo(
    () => sync.data.path.home || fallbackPath()?.home || "",
  );
  ...
  const cache = new Map<
    string,
    Promise<Array<{ name: string; absolute: string }>>
  >();
  ...
  function scoped(value: string) {
    ...
    if (raw.startsWith("~/"))
      return { directory: trimTrailing(h ?? base), path: raw.slice(2) };

    const root = rootOf(raw);
    if (root)
      return { directory: trimTrailing(root), path: raw.slice(root.length) };
  ...
  const directories = async (filter: string) => {
    ...
    if (!isPath) {
      ...
      return results
        .map((rel) => join(scopedInput.directory, rel))
        .slice(0, 50);
    }
    ...
    for (const part of head) {
      ...
      const next = (
        await Promise.all(paths.map((p) => match(p, part, branch)))
      ).flat();
    ...
    const out = (
      await Promise.all(paths.map((p) => match(p, tail, 50)))
    ).flat();
    ...
    const exact = deduped.filter(
      (p) => getFilename(p).toLowerCase() === needle,
    );
      ...
      <div class="flex flex-col">
        <Show when={recentProjects().length > 0}>
          <div class="px-3 pb-2 mb-2">
            <div class="text-12-regular text-text-weak mb-2 ml-2">
              Recent Projects
            </div>
            <div class="space-y-1">
              {recentProjects().map((project) => {
                const path = sync.data.path.home
                  ? project.worktree.replace(sync.data.path.home, "~")
                  : project.worktree;
