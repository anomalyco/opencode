...
function createHarness(input?: { patch?: string; createdDirectory?: string; applyFailsIn?: string }) {
  ...
  const run = (target: ExploreForkTarget, point: ExploreForkPoint = { type: "full" }) =>
    runExploreFork({
...
describe("runExploreFork", () => {
  test("creates a worktree, copies the diff, forks with the target client", async () => {
    ...
    const result = await harness.run({ type: "create" }, { type: "message", messageID: "msg_1" })
    ...
    expect(harness.calls.forks).toEqual([
      { directory: harness.createdDirectory, input: { sessionID: "ses_1", messageID: "msg_1" } },
    ])
  })
  ...
  test("forks in the current worktree without copying changes", async () => {
    ...
    const result = await harness.run(
      { type: "current", directory: harness.sourceDirectory },
      { type: "message", messageID: "msg_2" },
    )
    ...
    expect(harness.calls.applies).toEqual([])
    expect(harness.calls.forks).toEqual([
      { directory: harness.sourceDirectory, input: { sessionID: "ses_1", messageID: "msg_2" } },
    ])
  })
