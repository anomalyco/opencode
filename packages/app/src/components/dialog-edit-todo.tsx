 ...
 * fetched. "Backlog" is Linear's default unstarted state — it is an Active
 * status but will not be auto-promoted until moved to "Todo", so the todo
 * stays idle while the user is editing (per PRD requirement: the default
 * status must not trigger the agent auto-workflow).
 */
...
// `SlashCommand` is imported from `@/components/prompt-input/slash-popover` so
// the dialog shares the same type as the chat composer. The `type: "custom"`
// discriminator is required by that contract; we don't expose builtin
...
export const DialogEditTodo: Component<DialogEditTodoProps> = (props) => {
  ...
  // field is needed because Active/Archived classification matches the 7
  // Linear default status names directly. Falls back to ["Backlog"] when
  ...
  const [ui, setUi] = createStore({
    ...
    // `atOptions` uses the shared `AtOption` type so we can hand the list
    // directly to `PromptPopover` (the same dropdown the chat composer uses).
    atOptions: [] as AtOption[],
    // `atActive` / `slashActive` are key/id strings (not indices) — that's
    // what `PromptPopover` expects. Index-based navigation is derived from
    // the flat list inside the keyboard handler.
    atActive: "" as string,
    slashCommands: [] as SlashCommand[],
    slashQuery: "",
    slashActive: "" as string,
    textareaEl: null as HTMLTextAreaElement | null,
    triggerRange: null as { start: number; end: number } | null,
  })

  // `atKey` mirrors the chat composer's key function (see
  // `packages/app/src/components/prompt-input.tsx`). The dialog only uses the
  // `file` variant, but the full discriminated union is kept for
  // future-proofing and to keep the contract identical to the composer.
  const atKey = (x: AtOption | undefined): string => {
    if (!x) return ""
  ...
  const loadLinearStatuses = async () => {
    await sdk()
      ...
      .then((res) => {
        ...
        const statuses = (res.data as LinearStatus[]).filter((s) => !!s.name)
  ...
  // Active/Archived classification matches against the 7 Linear default
  ...
  const loadLinearUsers = async () => {
    ...
    await sdk()
      ...
      .then((res) => {
        ...
        const users = (res.data as LinearUser[]).filter((u) => !!u.id && !!u.name)
  ...
  const loadSlashCommands = async () => {
    ...
    const cmds: SlashCommand[] = (res.data as Array<{ name: string; description?: string; source?: string }>)
      ...
      .map((c) => ({
        ...
        // `type: "custom"` is required by the shared `SlashCommand` contract
        // (builtin commands like /clear are session-scoped and excluded).
  ...
  const detectTrigger = (text: string, cursor: number) => {
    ...
    if (atMatch) {
      ...
      setUi("popover", "at")
      // Set active to first option's key once results load (handled in
      // runAtQuery). Reset to empty here so a stale key from a previous
      // session doesn't highlight a non-existent item.
      setUi("atActive", "")
      return
    }
    ...
    if (slashMatch) {
      ...
      setUi("popover", "slash")
      // Default to the first filtered command's id (see filteredSlash below).
      setUi("slashActive", "")
      return
    }
    setUi("popover", null)
  ...
  const runAtQuery = async (query: string) => {
    ...
    // composer's behavior where the first item is active by default).
    setUi("atActive", options.length > 0 ? atKey(options[0]) : "")
  }
  ...
  // `insertAtSelection` accepts the shared `AtOption` type so it can be wired
  // directly to `PromptPopover`'s `onAtSelect`. The dialog only inserts the
  // `file` variant; other variants (agent/resource/reference) are ignored
  // because the Todo description is a plain markdown string, not a structured
  // prompt-parts list. If the user picks a non-file option (e.g., from a
  // future expansion), we fall back to `display` so the insertion is still
  // observable.
  const insertAtSelection = (option: AtOption | undefined) => {
    if (!option) return
    const el = ui.textareaEl
    if (!el) return
    const range = ui.triggerRange
    if (!range) return
    const ref = option.type === "file" ? option.path : option.display
  ...
  const insertSlashSelection = (cmd: SlashCommand | undefined) => {
    if (!cmd) return
    const el = ui.textareaEl
    if (!el) return
    const range = ui.triggerRange
  ...
  const handleDescriptionKeyDown = (e: KeyboardEvent) => {
    if (ui.popover === "at") {
      const items = ui.atOptions
      const currentIdx = items.findIndex((it) => atKey(it) === ui.atActive)
      if (e.key === "ArrowDown") {
        e.preventDefault()
        const nextIdx = items.length === 0 ? -1 : (currentIdx + 1) % items.length
        setUi("atActive", nextIdx >= 0 ? atKey(items[nextIdx]) : "")
        return
      }
      if (e.key === "ArrowUp") {
        e.preventDefault()
        const nextIdx = items.length === 0 ? -1 : (currentIdx - 1 + items.length) % items.length
        setUi("atActive", nextIdx >= 0 ? atKey(items[nextIdx]) : "")
        return
      }
      if (e.key === "Enter" || e.key === "Tab") {
        e.preventDefault()
        const item = currentIdx >= 0 ? items[currentIdx] : items[0]
    ...
    if (ui.popover === "slash") {
      ...
      const currentIdx = items.findIndex((it) => it.id === ui.slashActive)
      if (e.key === "ArrowDown") {
        e.preventDefault()
        const nextIdx = items.length === 0 ? -1 : (currentIdx + 1) % items.length
        setUi("slashActive", nextIdx >= 0 ? items[nextIdx].id : "")
        return
      }
      if (e.key === "ArrowUp") {
        e.preventDefault()
          ...
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <div class="relative">
                ...
                />
                {/* Reused chat-composer dropdown (PromptPopover).
                    Per workspace rule: "The New Todo dialog must reuse the
                    existing composer instead of reimplementing it." We hand
                    the same `AtOption` / `SlashCommand` shapes the chat
                    composer uses to the shared `PromptPopover` component so
                    visual styling, keyboard nav, and i18n keys stay in sync.
                    The `slashMenu` search input is disabled (we trigger via
                    `/` in the textarea); `commandKeybind(s)` are wired to
                    `useCommand()` so any registered keybinds render. */}
                <PromptPopover
                  popover={ui.popover}
                  setSlashPopoverRef={() => {}}
                  atFlat={ui.atOptions}
                  atActive={ui.atActive || undefined}
                  atKey={atKey}
                  setAtActive={(key) => setUi("atActive", key)}
                  onAtSelect={insertAtSelection}
                  slashFlat={filteredSlash()}
                  slashActive={ui.slashActive || undefined}
                  setSlashActive={(id) => setUi("slashActive", id)}
                  onSlashSelect={insertSlashSelection}
                  slashMenu={false}
                  slashMenuQuery=""
                  onSlashMenuInput={() => {}}
                  onSlashMenuKeyDown={() => {}}
