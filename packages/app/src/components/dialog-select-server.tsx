...
export function DialogSelectServer() {
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    if (!result.healthy) {
      setStore("error", language.t("server.connectFailed"))
      return
    }
  ...
  return (
    <Dialog title={language.t("server.title")} description={language.t("server.description")}>
      <div class="flex flex-col gap-4 pb-4">
        <List
          search={{ placeholder: language.t("server.search"), autofocus: true }}
          emptyMessage={language.t("server.empty")}
          items={sortedItems}
          key={(x) => x}
          current={current()}
          onSelect={(x) => {
        ...
        <div class="mt-6 px-3 flex flex-col gap-1.5">
          <div class="px-3">
            <h3 class="text-14-regular text-text-weak">{language.t("server.add")}</h3>
          </div>
          <form onSubmit={handleSubmit}>
            <div class="flex items-start gap-2">
              <div class="flex-1 min-w-0 h-auto">
                <TextField
                  type="text"
                  label={language.t("server.urlLabel")}
                  hideLabel
              ...
              <Button type="submit" variant="secondary" icon="plus-small" size="large" disabled={store.adding}>
                {store.adding ? language.t("common.checking") : language.t("common.add")}
              </Button>
            </div>
          </form>
        </div>
        ...
        <Show when={isDesktop}>
          <div class="mt-6 px-3 flex flex-col gap-1.5">
            <div class="px-3">
              <h3 class="text-14-regular text-text-weak">{language.t("server.default")}</h3>
              <p class="text-12-regular text-text-weak mt-1">
                {language.t("server.defaultDescription")}
              </p>
            </div>
            <div class="flex items-center gap-2 px-3 py-2">
              <Show
                ...
                fallback={
                  <Show
                    when={server.url}
                    fallback={<span class="text-14-regular text-text-weak">{language.t("server.noneSelected")}</span>}
                  >
                    <Button
                      variant="secondary"
                      size="small"
