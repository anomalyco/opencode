...
export function NewSessionView(props: NewSessionViewProps) {
  ...
  const label = (value: string) => {
    if (value === MAIN_WORKTREE) {
      if (isWorktree()) return language.t("session.mainBranch")
      const branch = sync.data.vcs?.branch
      if (branch) return language.t("session.mainBranchWithName", { branch })
      return language.t("session.mainBranch")
    }

    if (value === CREATE_WORKTREE) return language.t("session.createWorktree")
  ...
  return (
    ...
    >
      <div class="text-20-medium text-text-weaker">{language.t("session.new")}</div>
