...
export function DialogSelectFile() {
  ...
  const commandItem = (option: CommandOption): Entry => ({
    ...
    category: language.t("commandPalette.commands"),
    option,
  })
  ...
  const fileItem = (path: string): Entry => ({
    ...
    type: "file",
    title: path,
    category: language.t("commandPalette.files"),
    path,
  })
  ...
  return (
    <Dialog class="pt-3 pb-0 !max-h-[480px]">
      <List
        search={{
          placeholder: language.t("commandPalette.search"),
          autofocus: true,
          hideIcon: true,
          class: "pl-3 pr-2 !mb-0",
        }}
        emptyMessage={language.t("commandPalette.empty")}
        items={items}
        key={(item) => item.id}
