...
function ProviderDetailView(props: { providerID: string; providerName: string; onBack: () => void }) {
  ...
  const loadBrowserSessions = async () => {
    try {
      const result = await globalSDK.client.provider.browser.sessions({})
      const map: Record<string, BrowserSessionStatus> = {}
      for (const session of (result.data ?? []) as BrowserSessionStatus[]) {
  ...
  const setupBrowserSession = async (recordId: string) => {
    ...
    try {
      await globalSDK.client.provider.browser.session.setup({ recordId })
  ...
  const refreshBrowserSession = async (recordId: string) => {
    ...
    try {
      const result = await globalSDK.client.provider.browser.session.refresh({ recordId })
      if (result.data?.success === false) alert(`Refresh failed: ${result.data.message}`)
      await refetch()
      await loadBrowserSessions()
    } catch (e) {
  ...
  const rebindBrowserSession = async (recordId: string) => {
    ...
    try {
      const result = await globalSDK.client.provider.browser.session.setup({ recordId })
      if (result.data?.success === false) alert(`Rebind failed: ${result.data.message}`)
      await refetch()
      await loadBrowserSessions()
    } catch (e) {
  ...
  const submitRename = async (recordId: string) => {
    ...
    try {
      await globalSDK.client.auth.updateAccount({ providerID: props.providerID, recordID: recordId, label })
      await refetch()
    } finally {
      cancelRename()
    }
  ...
  const removeBrowserSession = async (recordId: string) => {
    ...
    try {
      await globalSDK.client.provider.browser.session.remove({ recordId })
      await loadBrowserSessions()
    } finally {
  ...
  const formatTimeAgo = (dateMs: number): string => {
  ...
  const switchAccount = async (recordID: string) => {
    ...
    try {
      const result = await globalSDK.client.auth.setActive({ providerID: props.providerID, recordID })
      const current = usage()
      if (current && result.data?.success) {
        mutate({
          ...current,
          ...
          anthropicUsage: (result.data.anthropicUsage as AnthropicUsage | undefined) ?? current.anthropicUsage,
        })
      }
    } catch (e) {
  ...
  const deleteAccount = async (recordID: string) => {
    ...
    try {
      const result = await globalSDK.client.auth.removeAccount({ providerID: props.providerID, recordID })
