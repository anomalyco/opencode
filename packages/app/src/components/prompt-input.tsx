...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    if (!currentModel || !currentAgent) {
      showToast({
        title: language.t("prompt.selectAgentModelTitle"),
        description: language.t("prompt.selectAgentModelDescription"),
      })
      return
    }
    ...
    if (isNewSession) {
      if (worktreeSelection === "create") {
        const createdWorktree = await client.worktree
          ...
          .catch((err) => {
            showToast({
              title: language.t("prompt.failedCreateWorktree"),
    ...
    if (mode === "shell") {
      ...
      client.session
        ...
        .catch((err) => {
          showToast({
            title: language.t("prompt.failedSendShell"),
            description: errorMessage(err),
          })
          restoreInput()
        })
    ...
    if (text.startsWith("/")) {
      ...
      if (customCommand) {
        ...
        client.session
          ...
          .catch((err) => {
            showToast({
              title: language.t("prompt.failedSendCommand"),
              description: errorMessage(err),
            })
            restoreInput()
          })
    ...
    client.session
      ...
      .catch((err) => {
        showToast({
          title: language.t("prompt.failedSendPrompt"),
          description: errorMessage(err),
        })
          ...
          <Switch>
            <Match when={store.popover === "at"}>
              <Show
                ...
                fallback={<div class="text-text-weak px-2 py-1">{language.t("prompt.noMatches")}</div>}
              >
            ...
            <Match when={store.popover === "slash"}>
              <Show
                ...
                fallback={<div class="text-text-weak px-2 py-1">{language.t("prompt.noCommands")}</div>}
              >
                <For each={slashFlat()}>
                  {(cmd) => (
                    <button
                      ...
                      <div class="flex items-center gap-2 shrink-0">
                        <Show when={cmd.type === "custom"}>
                          <span class="text-11-regular text-text-subtle px-1.5 py-0.5 bg-surface-base rounded">
                            {language.t("prompt.customCommand")}
                          </span>
                        </Show>
        ...
        <Show when={store.dragging}>
          <div class="absolute inset-0 z-10 flex items-center justify-center bg-surface-raised-stronger-non-alpha/90 pointer-events-none">
            <div class="flex flex-col items-center gap-2 text-text-weak">
              ...
              <span class="text-14-regular">{language.t("prompt.dropFiles")}</span>
            </div>
          </div>
        </Show>
          ...
          <div class="flex flex-wrap items-center gap-2 px-3 pt-3">
              ...
              {(path) => (
                <div class="flex items-center gap-2 px-2 py-1 rounded-md bg-surface-base border border-border-base max-w-full">
                  ...
                  <div class="flex items-center text-12-regular min-w-0">
                    ...
                    <span class="text-text-weak whitespace-nowrap ml-1">{language.t("prompt.active")}</span>
                  </div>
                  <IconButton
                    type="button"
                    icon="close"
            ...
            <Show when={!prompt.context.activeTab() && !!activeFile()}>
              ...
              >
                ...
                <span>{language.t("prompt.includeActiveFile")}</span>
              </button>
            </Show>
        ...
        <div class="relative max-h-[240px] overflow-y-auto" ref={(el) => (scrollRef = el)}>
          ...
          <Show when={!prompt.dirty()}>
            <div class="absolute top-0 inset-x-0 px-5 py-3 pr-12 text-14-regular text-text-weak pointer-events-none whitespace-nowrap truncate">
              {store.mode === "shell"
                ? language.t("prompt.shellPlaceholder")
                : language.t("prompt.askAnything", { example: PLACEHOLDERS[store.placeholder] })}
            </div>
          </Show>
        </div>
            ...
            <Switch>
              <Match when={store.mode === "shell"}>
                <div class="flex items-center gap-2 px-2 h-6">
                  ...
                  <span class="text-12-regular text-text-primary">{language.t("prompt.shellMode")}</span>
                  <span class="text-12-regular text-text-weak">{language.t("prompt.shellExitHint")}</span>
                </div>
              </Match>
              <Match when={store.mode === "normal"}>
                  ...
                  fallback={
                    ...
                    >
                      <Button as="div" variant="ghost" onClick={() => dialog.show(() => <DialogSelectModelUnpaid />)}>
                        ...
                        {local.model.current()?.name ?? language.t("dialog.selectModel")}
                ...
                <Show when={local.model.variant.list().length > 0}>
                  ...
                  >
                    ...
                    >
                      {local.model.variant.current() ?? language.t("model.defaultVariant")}
                    </Button>
                  </TooltipKeybind>
                </Show>
                <Show when={permission.permissionsEnabled() && params.id}>
                  <TooltipKeybind
                    placement="top"
                    title={language.t("command.title.autoAcceptEdits")}
          ...
          <div class="flex items-center gap-3 absolute right-2 bottom-2">
            ...
            <div class="flex items-center gap-2">
              ...
              <Show when={store.mode === "normal"}>
                <Tooltip placement="top" value={language.t("prompt.attachFile")}>
            ...
            <Tooltip
              ...
              value={
                <Switch>
                  <Match when={working()}>
                    <div class="flex items-center gap-2">
                      <span>{language.t("common.stop")}</span>
                      <span class="text-icon-base text-12-medium text-[10px]!">ESC</span>
                    </div>
                  </Match>
                  <Match when={true}>
                    <div class="flex items-center gap-2">
                      <span>{language.t("common.send")}</span>
