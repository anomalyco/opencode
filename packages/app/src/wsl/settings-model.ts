...
export function wslArgusAction(check?: WslArgusCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installArgus"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateArgus"
}
...
export function addServerViewModel(input: {
  ...
  const argusCheck = selectedDistro ? (state?.argusChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.argusChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateArgus" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.argusMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installArgus" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingArgus =
    input.state?.job?.kind === "install-argus" && input.state.job.distro === input.selectedDistro
  if (!ready || probingArgus) {
    return {
      variant: "contrast",
  ...
  if (!addServerArgusReady(input.argusCheck)) {
    const update = !!input.argusCheck?.resolvedPath && input.argusCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingArgus
        ? { key: "wsl.onboarding.updatingArgus" }
        : update
          ? { key: "wsl.onboarding.updateArgus" }
          : { key: "wsl.onboarding.installArgus" },
      disabled: !!input.state?.job || input.adding,
      action: "install-argus",
      loading: installingArgus,
...
function addServerArgusReady(check: WslArgusCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.argusChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.argusChecks[item.name]) return [`argus:${item.name}`]
    return []
  })
