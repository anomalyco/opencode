...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Pencode needs attention", () => {
    expect(wslPencodeAction(undefined)).toBeUndefined()
    expect(
      wslPencodeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installPencode")
    expect(
      wslPencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/pencode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updatePencode")
    expect(
      wslPencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/pencode",
  ...
  test("does not report ready when Pencode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        pencodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.pencode/bin/pencode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "pencode is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installPencode" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-pencode")
  })
