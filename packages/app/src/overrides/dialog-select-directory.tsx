...
// CUSTOM: Restrict project selector to this subdirectory
const RESTRICTED_BASE_PATH = "/home/ubuntu/projects"
...
export function DialogSelectDirectory(props: DialogSelectDirectoryProps) {
  ...
  const home = createMemo(() => RESTRICTED_BASE_PATH || sync.data.path.home || fallbackPath()?.home || "")

  // CUSTOM: Start from restricted base path instead of user's home
  const start = createMemo(() => RESTRICTED_BASE_PATH || sync.data.path.home || sync.data.path.directory || fallbackPath()?.home || fallbackPath()?.directory)
  ...
  function parentOf(input: string) {
    ...
    // CUSTOM: Don't allow navigating above restricted base path
    const parent = v.slice(0, i)
    const restrictedBase = trimTrailing(RESTRICTED_BASE_PATH)
    if (restrictedBase && !parent.startsWith(restrictedBase)) {
    ...
    const request = sdk.client.file
      ...
      .then((nodes) =>
        nodes
          ...
          // CUSTOM: Filter out directories outside the restricted base path
          .filter((n) => {
            if (!RESTRICTED_BASE_PATH) return true
            const restrictedBase = trimTrailing(RESTRICTED_BASE_PATH)
            return n.absolute.startsWith(restrictedBase)
          }),
      )
