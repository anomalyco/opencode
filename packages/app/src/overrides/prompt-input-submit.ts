...
type PromptSubmitInput = {
  ...
  newSessionWorktree?: (() => string | undefined)
...
export function createPromptSubmit(input: PromptSubmitInput) {
  ...
  const errorMessage = (err: unknown) => extractErrorMessage(err, language.t("common.requestFailed"))
  ...
  const handleSubmit = async (event: Event) => {
    ...
    let context: ReturnType<typeof prompt.context.items>
    let commentItems: typeof context
    let messageID: string
    let requestParts: ReturnType<typeof buildRequestParts>["requestParts"]
    let optimisticParts: ReturnType<typeof buildRequestParts>["optimisticParts"]
    let optimisticMessage: Message

    try {
      context = prompt.context.items().slice()
      commentItems = context.filter((item) => item.type === "file" && !!item.comment?.trim())
      ...
      optimisticParts = built.optimisticParts
      ...
      console.error("[submit] Failed to build request:", err)
      showToast({
    ...
    try {
      ...
      console.error("[submit] Failed to prepare send:", err)
      showToast({
        title: language.t("prompt.toast.promptSendFailed.title"),
        description: errorMessage(err),
      })
    ...
    void send().catch((err) => {
      ...
      console.error("[submit] Prompt send failed:", err)
      showToast({
        title: language.t("prompt.toast.promptSendFailed.title"),
        description: errorMessage(err),
      })
