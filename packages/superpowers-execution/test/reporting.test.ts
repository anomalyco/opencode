...
test("the companion registers one uniquely named skill whose content is the packaged skill file", async () => {
  ...
  expect(skill.content).toContain("Read the approved plan and `execution_read`")
  expect(skill.content).toContain("Register stable tasks, dependencies, required gates, and a final review")
  expect(skill.content).toContain("sha256sum")
  expect(skill.content).toContain("run.start")
  expect(skill.content).toContain("Link the real returned child ID")
  expect(skill.content).toContain("evidence.add")
  expect(skill.content).toContain("task.verify")
  expect(skill.content).toContain("plan.revise")
