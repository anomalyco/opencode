...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.xcsh.desktop.dev",
  beta: "ai.xcsh.desktop.beta",
  prod: "ai.xcsh.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.xcsh.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `xcsh.settings.dat` is special: it maps to the settings store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"xcsh.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // xcsh.settings.dat → the electron settings store ("xcsh.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "xcsh.global.dat").
  const storeName = filename === "xcsh.settings.dat" ? "xcsh.settings" : filename
