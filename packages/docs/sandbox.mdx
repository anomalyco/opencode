...
You can prevent the agent from writing to specific files inside your working directory using the `deny_workspace_patterns` array. For example, to prevent the agent from modifying environment and secrets files:

```bash
> echo "MODIFIED" > .env
/usr/bin/bash: line 1: .env: Read-only file system
```

<Note>
**Read Access Limitation:** Because the workspace requires explicit `allowRead` permissions to function, and `srt` processes `allowRead` configurations with a higher precedence than `denyRead` rules, `deny_workspace_patterns` only blocks write operations. The agent can still read the contents of files matched by this pattern.
</Note>
