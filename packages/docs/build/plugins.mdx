...
Create an executable definition with `Tool.make`, then register it with a name
...
`agent`, `messageID`, `callID`, and `progress`. A definition with `output`
must return `output`; Effect and Standard Schema codecs validate it, while raw
JSON Schema definitions enforce JSON compatibility only. A definition
...
`tools.add(name, definition, options?)` registration shape. Effect executors
