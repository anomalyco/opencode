...
Create an executable declaration with `Tool.make`, then register it with a name
...
`agent`, `messageID`, `callID`, and `progress`. A declaration with `output`
must return `output`; Effect and Standard Schema codecs validate it, while raw
JSON Schema declarations enforce JSON compatibility only. A declaration
...
`tools.add(name, declaration, options?)` registration shape. Effect executors
