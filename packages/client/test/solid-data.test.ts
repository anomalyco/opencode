...
test("serializes revert mutations before prompt admission", async () => {
  ...
  const api = OpenCode.make({
    ...
    fetch: async (input, init) => {
      ...
      if (request.method === "DELETE" && request.url.endsWith("/revert")) {
...
test("keeps an unacknowledged replacement when a committed revert rewinds later history", async () => {
  const listeners = new Set<Parameters<CreateDataInput["event"]["listen"]>[0]>()
  const api = OpenCode.make({
    baseUrl: "http://opencode.local",
    fetch: async (input, init) => {
      ...
      if (url.pathname === "/api/session/ses_refresh/message") {
        return Response.json({
          data: [
            { id: "msg_mmm", type: "user", text: "Later", time: { created: 3 } },
            { id: "msg_boundary", type: "user", text: "Boundary", time: { created: 2 } },
            { id: "msg_aaa", type: "user", text: "Earlier", time: { created: 1 } },
          ],
          cursor: {},
        })
      }
      if (url.pathname === "/api/session/ses_refresh/inbox") return Response.json({ data: [] })
      if (url.pathname === "/api/session/ses_refresh/prompt") {
        return Response.json({
          data: {
            id: "msg_replacement",
            sessionID: "ses_refresh",
  ...
  try {
    setup.data.session.remember(session(0))
    await setup.data.session.message.sync("ses_refresh")
    const admitted = setup.data.session.prompt({
      id: "msg_replacement",
      sessionID: "ses_refresh",
      text: "Replacement",
    })
    expect(setup.data.session.message.list("ses_refresh").map((item) => item.id)).toContain("msg_replacement")

    listeners.forEach((listener) =>
      listener({
        name: "session.revert.committed",
        details: {
          id: "evt_revert_committed",
          created: 5,
          type: "session.revert.committed",
          durable: { aggregateID: "ses_refresh", seq: 5, version: 1 },
          data: { sessionID: "ses_refresh", to: "msg_boundary" },
        },
      }),
    )

    expect(setup.data.session.message.list("ses_refresh").map((item) => item.id)).toEqual([
      "msg_aaa",
      "msg_replacement",
    ])
    await admitted
