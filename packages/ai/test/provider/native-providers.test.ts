  ...
  it.effect("assigns provider-owned metadata namespaces across native routes", () =>
    Effect.gen(function* () {
      ...
      const providers = [
        ...
        [AmazonBedrock.configure({ apiKey: "test", region: "us-east-1" }).model("model"), "bedrock"],
        [AmazonBedrockMantle.configure({ apiKey: "test", region: "us-east-1" }).chat("model"), "mantle"],
        [AmazonBedrockMantle.configure({ apiKey: "test", region: "us-east-1" }).responses("model"), "mantle"],
