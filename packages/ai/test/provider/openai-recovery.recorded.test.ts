...
describe("OpenAI Responses interrupted recovery recorded", () => {
    ...
    () =>
      Effect.gen(function* () {
        const response = yield* LLMClient.generate(
          LLM.request({
            id: "recorded_openai_interrupted_parallel_recovery",
            model,
  ...
  recorded.effect.with(
    ...
    { tags: ["tool", "parallel", "recovery", "sad-path"] },
    () =>
      Effect.gen(function* () {
        const error = yield* LLMClient.generate(
          LLM.request({
            id: "recorded_openai_incomplete_parallel_recovery",
            model,
