...
describe("OpenAI Responses route", () => {
  ...
  it.effect("decodes and replays local_shell_call with its output", () =>
    Effect.gen(function* () {
      const action = {
        type: "exec",
        command: ["ls", "-la"],
      ...
      const localShellRequest = LLMRequest.update(request, { tools: [OpenAI.localShell()] })
      const preparedRequest = yield* compileRequest(localShellRequest)
      expect(preparedRequest.body.tools).toEqual([{ type: "local_shell" }])

      const response = yield* LLMClient.generate(localShellRequest).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents(
              {
      ...
      expect(response.events.filter((event) => event.type === "tool-call" || event.type === "tool-result")).toEqual([
        {
          ...
          input: { action },
          providerMetadata: {
            openai: { itemId: "local_shell_1", itemType: "local_shell_call", status: "completed" },
          },
        },
      ])
      ...
      const prepared = yield* compileRequest(
        LLM.request({
          ...
          messages: [
            response.message,
            Message.tool({ id: "call_1", name: "local_shell", result: "files", resultType: "text" }),
          ],
        }),
      )
      expect(prepared.body.input).toEqual([
        ...
        { type: "local_shell_call_output", id: "call_1", output: "files" },
      ])
    }),
  )

  it.effect("rejects malformed local_shell_call output", () =>
    Effect.gen(function* () {
      const error = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents(
              {
                type: "response.output_item.done",
                item: { type: "local_shell_call", id: "local_shell_1", call_id: "call_1" },
              },
              { type: "response.completed", response: {} },
            ),
          ),
      ...
      expect(error.reason._tag).toBe("InvalidProviderOutput")
      expect(error.message).toContain("local_shell_call is malformed")
    }),
  )

  it.effect("rejects unsupported local shell tool constraints", () =>
    Effect.gen(function* () {
      const tool = OpenAI.localShell()
      const selected = yield* compileRequest(
        LLM.request({ model, prompt: "List files.", tools: [tool], toolChoice: "local_shell" }),
      ).pipe(Effect.flip)
      expect(selected.reason._tag).toBe("InvalidRequest")
      expect(selected.message).toContain("cannot select the local shell tool by name")
      ...
      const allowed = yield* compileRequest(
        LLM.request({
          ...
          prompt: "List files.",
          tools: [tool],
          providerOptions: { allowedTools: { toolNames: ["local_shell"] } },
        }),
      ).pipe(Effect.flip)
      expect(allowed.reason._tag).toBe("InvalidRequest")
      expect(allowed.message).toContain("allowed tools cannot include the local shell tool")
    }),
  )
