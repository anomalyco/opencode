...
// Notion バックエンド（Cloud Run の waitlist-api）への送信先。ビルド時に env から注入する。
const ENDPOINT = process.env.NEXT_PUBLIC_WAITLIST_ENDPOINT ?? ""
const PRIVACY_POLICY_URL = "https://www.acompany.tech/privacy-policy"

// 流入元の簡易計測（utm_source があれば付与）。
function detectSource(): string {
  if (typeof window === "undefined") return "lp-waitlist"
  const utm = new URLSearchParams(window.location.search).get("utm_source")
  return utm ? `lp-waitlist (${utm})` : "lp-waitlist"
}
...
function useWaitlistForm() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!ENDPOINT) {
      // NEXT_PUBLIC_WAITLIST_ENDPOINT がビルド時に未設定
      setStatus("error")
      return
    }
      ...
      <div className="mx-auto flex max-w-4xl flex-col items-center px-6 py-20 text-center md:py-28">
        ...
        {status === "success" ? (
          ...
          >
            ...
            <p className="mt-3 text-[10px] leading-relaxed text-sc-text-dim">
              登録をもって
              <a
                href={PRIVACY_POLICY_URL}
                target="_blank"
