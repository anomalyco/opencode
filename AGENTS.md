...
* **OpenCode web UI served via CDN proxy, no local static serving**: The opencode binary is built via Bun.build() with compile option, cross-compiling to 11 targets. Web UI assets from packages/app/dist are embedded via generated src/server/app-manifest.ts that imports each file with \`{ type: 'file' }\` — Bun's file loader embeds them in $bunfs with content-hashed names. A \`static-asset-loader\` Bun plugin intercepts non-JS files. The manifest maps URL paths → $bunfs paths at runtime (OPENCODE\_APP\_EMBEDDED define gates this). Static serving runs BEFORE Instance.provide() to avoid DB migration checks on assets. Resolution order: embedded $bunfs → OPENCODE\_APP\_DIR env → auto-detect packages/app/dist → CDN proxy to app.opencode.ai. serveLocal() handles MIME types, SPA fallback, CSP header. resolveAppDir() caches. For UI dev: separate Vite dev server + opencode serve --port 4096.
...
* **OpenCode server: static middleware must precede Instance.provide()**: In server.ts, the Instance.provide() middleware (line ~248) runs DB migrations via InstanceBootstrap for every request. If the catch-all static file handler is placed after it, asset requests trigger migration checks and fail with ENOENT in compiled binaries. Fix: add static serving as a middleware before Instance.provide() that returns early for non-API paths, or move the catch-all route before the Instance.provide() \`.use()\` call. The current pattern checks resolveAppDir() + serveLocal() and returns if matched, otherwise calls next().
