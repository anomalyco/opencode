...
- Tests cannot run from repo root (guard: `do-not-run-tests-from-root`); run from package dirs like `packages/xcsh`.

## Type Checking

- Always run `bun typecheck` from package directories (e.g., `packages/xcsh`), never `tsc` directly.
