...
- Keep delivery vocabulary explicit. Prompts steer by default. At safe step boundaries, steered compaction takes priority over pending steers without crossing a preceding steered move control; remaining steers retain enqueue order. Never interrupt active model/tool work to prioritize compaction. Explicit queue delivery still waits for an idle boundary, where steers take priority; otherwise exactly one queued item delivers before the runner reevaluates continuation. Inbox items may be cancelled or changed between queue and steer before delivery. Promoting new user input resets the selected agent's step allowance; a batch of steers resets it once.
