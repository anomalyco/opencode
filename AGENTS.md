...
- `/opsx:*` is only the upstream generated templates' logical notation, not literal OpenCode syntax. In OpenCode, invoke the matching `/opsx-<name>` slash command; in Codex, invoke the corresponding `openspec-*` skill using the current skill mechanism. Do not execute `/opsx:*` as a shell command.
