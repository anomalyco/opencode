...
import type { OpenDeepSeekClient, SessionMessageResponse } from "@opendeepseek/sdk/v2"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk }: { sdk: OpenDeepSeekClient }) {
  ...
  export class Agent implements ACPAgent {
    ...
    private sdk: OpenDeepSeekClient
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    return model ?? { providerID: "opendeepseek", modelID: "big-pickle" }
  }
