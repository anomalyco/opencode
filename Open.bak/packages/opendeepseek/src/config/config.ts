...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.opendeepseek_CONFIG) {
      result = mergeConfigWithPlugins(result, await loadFile(Flag.opendeepseek_CONFIG))
      log.debug("loaded custom config", { path: Flag.opendeepseek_CONFIG })
    }

    for (const file of ["opendeepseek.jsonc", "opendeepseek.json"]) {
    ...
    if (Flag.opendeepseek_CONFIG_CONTENT) {
      result = mergeConfigWithPlugins(result, JSON.parse(Flag.opendeepseek_CONFIG_CONTENT))
      log.debug("loaded custom config from OPENDEEPSEEK_CONFIG_CONTENT")
    }
    ...
    if (Flag.opendeepseek_CONFIG_DIR) {
      directories.push(Flag.opendeepseek_CONFIG_DIR)
      log.debug("loading config from OPENDEEPSEEK_CONFIG_DIR", { path: Flag.opendeepseek_CONFIG_DIR })
    }
    ...
    for (const dir of unique(directories)) {
      ...
      if (dir.endsWith(".opendeepseek") || dir === Flag.opendeepseek_CONFIG_DIR) {
        for (const file of ["opendeepseek.jsonc", "opendeepseek.json"]) {
    ...
    if (Flag.opendeepseek_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.opendeepseek_PERMISSION))
    }
  ...
  async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@opendeepseek/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const name = (() => {
        const patterns = ["/.opendeepseek/command/", "/command/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const agentFolderPath = item.includes("/.opendeepseek/agent/")
        ? item.split("/.opendeepseek/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "opendeepseek.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "opendeepseek.jsonc"))),
    )
