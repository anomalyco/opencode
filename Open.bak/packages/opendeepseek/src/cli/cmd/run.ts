...
import { createOpenDeepSeekClient, type OpenDeepSeekClient } from "@opendeepseek/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: OpenDeepSeekClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createOpenDeepSeekClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.OPENDEEPSEEK_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpenDeepSeekClient({ baseUrl: `http://${server.hostname}:${server.port}` })
