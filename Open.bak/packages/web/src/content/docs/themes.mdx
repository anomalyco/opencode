...
By default, OpenCode uses our own `opendeepseek` theme.
...
```json title="opendeepseek.json" {3}
{
...
3. **Project root directory** - Defined in the `<project-root>/.opendeepseek/themes/*.json`
4. **Current working directory** - Defined in `./.opendeepseek/themes/*.json`
...
```bash no-frame
mkdir -p .opendeepseek/themes
vim .opendeepseek/themes/my-theme.json
```
