...
Configure modes in your `opendeepseek.json` config file:
...
Create mode files in `.opendeepseek/mode/` for project-specific modes or `~/.config/opencode/mode/` for global modes:

```markdown title=".opendeepseek/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
