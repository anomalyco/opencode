...
- Locally by placing them in the `.opendeepseek/tool/` directory of your project.
...
```ts title=".opendeepseek/tool/database.ts" {1}
...
```ts title=".opendeepseek/tool/math.ts"
import { tool } from "@opendeepseek/plugin"
...
```ts title=".opendeepseek/tool/project.ts" {8}
import { tool } from "@opendeepseek/plugin"
...
```python title=".opendeepseek/tool/add.py"
import sys
...
```ts title=".opendeepseek/tool/python-add.ts" {10}
import { tool } from "@opendeepseek/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .opendeepseek/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
