...
OpenCode integrates with VS Code, Cursor, or any IDE that supports a terminal. Just run `opendeepseek` in the terminal to get started.
...
3. Run `opendeepseek` - the extension installs automatically
...
- Ensure you’re running `opendeepseek` in the integrated terminal.
