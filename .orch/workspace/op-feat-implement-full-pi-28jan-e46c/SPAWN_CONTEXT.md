...
To:
```typescript
export const USER_AGENT = `claude-cli/2.1.15 (external, cli)`
```

### 2. Add System Prompt Identity (packages/opencode/src/session/llm.ts)
This is the CRITICAL missing piece. When OAuth is detected, prepend to system prompt:
```typescript
{
  type: 'text',
...
Look at how system prompts are built and inject this for OAuth tokens.

### 3. Apply Full Stealth Headers (packages/opencode/src/session/llm.ts)
When isOAuth is true, ensure these headers are set:
- `x-app: cli`
- `anthropic-dangerous-direct-browser-access: true`
- `anthropic-beta: claude-code-20250219,oauth-2025-04-20,{existing-beta-features}`
- `user-agent` from Installation.USER_AGENT

The hook system exists (Hook.getChatHeaders) but may need direct injection instead.

### 4. Verify SDK Auth Parameter
Check if the Anthropic SDK is using `authToken` vs `apiKey`. For OAuth tokens, pi-ai uses:
```typescript
new Anthropic({
  apiKey: null,
...
Study: `~/Documents/personal/pi-mono/packages/ai/src/providers/anthropic.ts`
- Lines 370-429: createClient() - header setup
- Lines 431-460: buildParams() - system prompt injection

## Verification Steps
1. Build: `cd packages/opencode && bun run build`
2. Unset API key: `unset ANTHROPIC_API_KEY`
3. Verify OAuth token exists: `cat ~/.local/share/opencode/auth.json`
4. Test: `./dist/opencode-darwin-arm64/bin/opencode run --model anthropic/claude-opus-4-5-20251101 'say hello'`
5. Should succeed using Max subscription OAuth

## Constraints
- Don't break non-OAuth API key path
- Don't break other providers (OpenAI, Gemini, DeepSeek)
- Keep config-driven hook system as fallback option
...
- Claude Max OAuth Is Viable via Stealth Mode
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/decisions/2026-01-26-claude-max-oauth-stealth-mode-viable.md
...
- Verify Stealth Mode Confirm You
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2026-01-26-inv-verify-stealth-mode-confirm-you.md
...
- Recovery Mode - Spawn System Failure
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2026-01-09-inv-recovery-mode-spawn-system-failure.md
- Set Up Claude Sneakpeek for Native Swarm Mode
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2026-01-24-inv-set-up-claude-sneakpeek-documents.md
- Report Model You Confirm Stealth
  - See: /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2026-01-26-inv-report-model-you-confirm-stealth.md
