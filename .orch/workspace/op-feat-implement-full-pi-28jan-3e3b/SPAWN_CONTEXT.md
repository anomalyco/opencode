...
- User-Agent: claude-code/2.0.32 in installation/index.ts:185
- No system prompt injection for OAuth
- Headers via config hook but not configured
- OAuth tokens detected via sk-ant-oat prefix in session/llm.ts:156

## Required Changes

### 1. Fix User-Agent Format (packages/opencode/src/installation/index.ts)
Change line 185 to: export const USER_AGENT = \`claude-cli/2.1.15 (external, cli)\`

### 2. Add System Prompt Identity (packages/opencode/src/session/llm.ts)
When OAuth is detected, prepend to system: { type: 'text', text: 'You are Claude Code...', cache_control: { type: 'ephemeral' } }

### 3. Apply Full Stealth Headers (packages/opencode/src/session/llm.ts)
When isOAuth is true: x-app: cli, anthropic-dangerous-direct-browser-access: true, anthropic-beta: claude-code-20250219,oauth-2025-04-20

### 4. Verify SDK Auth Parameter
Check authToken vs apiKey usage for OAuth tokens

## Reference Implementation
Study: ~/Documents/personal/pi-mono/packages/ai/src/providers/anthropic.ts lines 370-460

## Verification
Build and test with unset ANTHROPIC_API_KEY
