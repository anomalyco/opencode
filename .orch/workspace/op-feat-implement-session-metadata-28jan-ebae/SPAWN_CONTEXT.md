TASK: Implement session.metadata.role in OpenCode fork to expose worker identity to plugins. 

Context: orch-go sends 'x-opencode-env-ORCH_WORKER' header when spawning workers. OpenCode should read this header and populate session.metadata.role so plugins can reliably detect workers.

Steps:
1. Find where session.created events are emitted in OpenCode
2. Find where custom headers (x-opencode-env-*) are processed
3. Add logic to set session.metadata.role = 'worker' when x-opencode-env-ORCH_WORKER header is present
4. Ensure this is available in session.created event for plugins

Reference: ~/Documents/personal/orch-go/.kb/investigations/2026-01-28-inv-orchestrator-coaching-plugin-cannot-reliably.md
