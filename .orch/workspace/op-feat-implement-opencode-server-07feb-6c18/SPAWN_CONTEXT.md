TASK: Implement OpenCode server instance eviction and SSE cleanup. CRITICAL reliability fix. Investigation complete at /Users/dylanconlin/Documents/personal/orch-go/.kb/investigations/2026-02-07-inv-opencode-server-memory-leak-4gb.md — read it first. Root cause proven: Instance cache in packages/opencode/src/project/instance.ts grows unboundedly by directory key. Each unique x-opencode-directory header creates permanent Instance with bootstrapped services/subscriptions. After 120 directories, RSS grew 50MB with no model prompts. In production, multi-agent orch-go usage causes 8.4GB RSS then macOS jetsam kill. Fix needed: (1) Add LRU/TTL eviction to Instance cache (max 20 live instances, 30min TTL for idle). Instance.provide() should check and evict before creating new. (2) Refactor SSE /event route cleanup in packages/opencode/src/server/server.ts — stream.close() path does NOT fire onAbort cleanup (verified: connected=40/disconnected=0 in stress test). Create shared cleanup() function called by both abort and close. (3) Add counters: log instance create/evict/dispose counts. Key files: packages/opencode/src/project/instance.ts (cache Map), packages/opencode/src/server/server.ts (SSE route lines ~503-527), packages/opencode/src/project/bootstrap.ts (per-instance service init). Watch out: evicting active instance could interrupt in-flight ops — must be activity-aware. SESSION SCOPE: Medium (2-4h). IN SCOPE: instance eviction + SSE cleanup only. OUT OF SCOPE: session storage, ACP session manager.
...
**Query:** "opencode server instance"
