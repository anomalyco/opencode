...
| [updater-disable-adapter](docs/features/updater-disable-adapter/3-changelog.md) | (本笔 commit) | 上游 sync prep — 禁自动升级 `desktop/src/index.tsx` createPlatform 从"条件 spread 藏 UI"改成"上游 sentinel pattern(每方法首行 `if (!UPDATER_ENABLED) return ...`)";`menu.ts` "Check for Updates..." 从"条件 spread 不渲染"改成 `enabled: UPDATER_ENABLED` 灰显(放弃原 spec "连 disabled 灰也不要",换永久 structural 对齐 + future 自家 updater 上线 0 回滚);`constants.rs` 注释更新强调 backend 总开关语义;**保留 `update` 旧名 + 旧行为**,rename + relaunch 由 upstream merge 自然 take(one-time 事件);typecheck 15/15 + DeskFox.exe build 通过;**0 R4**(三档原则下 Method 1 vs Method 2 推演选 1) | Tiny,3 文件 ~34/-35 行,changelog 含未来 fork updater 路径铺垫说明 |
