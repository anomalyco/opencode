...
| [office-installer-macos](docs/features/office-installer-macos/3-changelog.md) | 待回填 | LibreOffice 自动安装 macOS 适配 — DMG 下载 + hdiutil 挂载 + cp -R 到 ~/Applications + soffice 检测路径(Win 行为不变) | Tiny,只 changelog,2 文件 净 +119 行;**R4 override 第 4 笔本季**(超 ≤2/季,下季补回);初版 `66c8fa523` 已 override 入,wrapper 不可行性论证见 changelog |
