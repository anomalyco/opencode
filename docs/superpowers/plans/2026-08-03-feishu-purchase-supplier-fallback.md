...
- Purchase fallback requires `BillState=3`, empty `Redflg`, positive `ListBuy.Prod_Number`, and non-empty `b_vw_master.UnitName`.
...
- [ ] **Step 3: Implement the minimal mapper behavior**
...
- [ ] **Step 5: Mark OpenSpec tasks 8.1 and 8.2 complete**
...
JOIN b_vw_master AS bill ON bill.AutoID = purchase.Bill_ID
bill.BillState = 3
COALESCE(bill.Redflg, '') = ''
...
- [ ] **Step 3: Implement the fixed query and strict row mapping**

Add a `purchaseSupplierSQL` CTE that joins selected product IDs through `ListBuy` and `b_vw_master`, filters the four reliability conditions, ranks each product with `ROW_NUMBER()`, and returns only `rank_no = 1`. Query shelves, structured sources, and purchase fallbacks together with `Promise.all`, parse `product_id` and `supplier_name` with `requiredString`, and call:
...
- [ ] **Step 4: Run adapter and mapper tests and verify GREEN**
...
- [ ] **Step 5: Mark OpenSpec tasks 8.3 and 8.4 complete**
...
Extend the compatible column fixture with the exact `ListBuy` and `b_vw_master` columns from the approved spec, expect `contractVersion: "mysql-inventory-v2"`, and add a missing-`b_vw_master.UnitName` failure case.
...
- [ ] **Step 3: Implement the v2 preflight contract**

Add `ListBuy` and `b_vw_master` to `columnsSQL`, add exact type/nullability/table-kind expectations to `requiredColumns`, and update the returned contract version to `mysql-inventory-v2`.

- [ ] **Step 4: Update trace and gold fixtures**
...
- [ ] **Step 6: Mark OpenSpec tasks 8.5 and 8.6 complete**
...
- [ ] **Step 1: Run the explicit live read-only contract check**
...
- [ ] **Step 2: Mark OpenSpec task 8.7 complete**
...
- [ ] **Step 4: Run repository and OpenSpec verification**
...
- [ ] **Step 5: Restart and verify the gateway safely**
...
- [ ] **Step 6: Mark OpenSpec task 8.8 complete**
