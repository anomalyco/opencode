...
The maintained package documentation describes installation-wide policy and manual update/deployment without restarting the currently running app or server during debugging. Edits to cached external Superpowers skills are not a durable integration point. Since the global policy is outside this repository, document its exact required wording in the package and install it on this machine; repository tests validate the packaged reporting instructions and hook behavior.
