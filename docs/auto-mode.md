...
the session, with every decision persisted to SQLite for audit. Implementation
lives in:
...
turn. Every decision lands in `permission_decisions`. Retention is total — no
TTL.
...
runs on behalf of the effective `auto` turn (`tools.ts`/`prompt.ts` propagate
the top-level agent through subtasks). The validator registers its handler at
layer build (`permission.registerValidator`); if no handler is registered,
`auto` asks degrade to the normal human flow.

Validations run through a strict FIFO queue per session. Parallel tool calls
validate one at a time and audit rows land in arrival order. Each ask has a
total budget of 45s covering the queue wait plus the validation itself; on
expiry the ask degrades to the human flow (`verdict=fallback`,
`reason=timeout`). A queued validation that expires is cancelled before it
starts; a validation already talking to the model keeps the queue slot until
the real call completes, so a timeout cannot create concurrent validator calls.
Each validation:
...
5. Writes exactly one `permission_decisions` row with the verdict, reason,
   model, latency, redacted pattern placeholders, and (when available) the
   opaque tool `callID`. Raw commands, tool metadata, and the validator prompt
   are never persisted or returned by the audit API. The audit write never breaks or blocks the ask itself
...
Both hidden agents follow the existing hidden-agent pattern:
...
Two tables, a compatibility migration pair (`20260817132554_magical_galactus`
and `20260817140444_open_jackpot`), all typed,
...
| `patterns`   | text (JSON) | redacted placeholders preserving only the number of evaluated patterns                    |
| `metadata`   | text (JSON) | only the opaque tool `callID`, when available; command/path/diff values are not persisted |
...
- **Sidebar section**: an `Auto` section (feature plugin
  ...
  verdict icon, the permission and redacted pattern count, and per-verdict
  counts in the header. Clicking a row expands the verdict, full model, latency, reason,
...
- **Decisions dialog**: `session.decisions` (slash `/decisions`, keybind
  ...
  as description, reason and extra redacted pattern placeholders as expandable
  details, timestamp in the footer. The detail view deliberately excludes the
  raw command, metadata, and validator prompt.
