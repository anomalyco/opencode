...
| [gitee-release-mirror](./gitee-release-mirror/) | done | GitHub Release publish 后自动同步到 Gitee Release(标题 / body / 附件)— 新 fork-only workflow `release-mirror-gitee-deskfox.yml`,curl + Gitee REST v5(无第三方 action 依赖),幂等 + 部分失败容忍;依赖 `GITEE_TOKEN` secret(user 自己配 Gitee PAT scope `projects`),首次实测靠 user 手动 publish 历史 draft `ship-prod-2026.5.3.1` 触发 |
