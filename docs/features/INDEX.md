...
| [updater-disable-adapter](./updater-disable-adapter/) | done | 上游 sync prep — 禁自动升级走法从"条件 spread 藏 UI"改成"上游 sentinel + menu 灰显"风格,structural surface 永久 0 冲突;同时为未来 fork 自家 updater 上线铺好载体(UI 留着,翻 `UPDATER_ENABLED=true` + 在 createPlatform 内换后端即可);0 R4 |
