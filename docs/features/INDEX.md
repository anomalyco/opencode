...
| [release-mac-ci](./release-mac-ci/) | in-progress | GitHub Actions 自动打 macOS .dmg(arm64,不签名)— 新增 `release-mac-deskfox.yml`,push `ship-mac-(prod\|beta)-*` tag 触发,与 Win workflow 独立;复用 mac 端现有 build-deskfox.sh + workflow 内自带 .dmg 重命名 |
