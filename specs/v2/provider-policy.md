...
Both `action` and `resource` use pencode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.pencode` files is intentionally deferred until `.pencode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether pencode permits an operation through its managed execution paths.
