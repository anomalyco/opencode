...
The pencode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@pencode-ai/llm` tool loop after replacing its
...
- plugins get global "pencode" instance like in that post i showed
- pencode instance has stuff like `pencode.session.prompt()` or
  `pencode.tool.register({...})`
...
- decide whether to preserve deprecated `@pencode-ai/llm` orchestration exports
