...
Background tool work (shell commands and subagents) is tracked by `Job`, which transports the producer's typed outcome and never classifies the work itself: a job is `completed` when its run reported how the work ended, `error` when the run died, and `cancelled` when it was abandoned before reporting. The Shell service owns a command's terminal state, including `killed` for an explicit stop that keeps the command and its captured output readable; `SessionExecution.resume` reports a user interruption of a joined execution as a value, while shutdown interruption still interrupts process-local joiners so the durable claim resumes the Session after restart. One `BackgroundNotice` path renders live and recovered terminals into synthetic notices. A user stop is a quiet notice: admitted with `resume: false` so an idle owner records it for its next step without waking.
