...
A process-global `SessionRunCoordinator` serializes execution for each local Session while allowing different Sessions to run concurrently. A resume joins any active execution instead of scheduling another forced drain. Wakes received during execution collapse into one advisory follow-up, closing the race between the runner's final inbox check and becoming idle. Prompt admission registers its wake inline after the durable commit, so interruption is process-local cancellation rather than a durable ordering protocol: it stops current execution, clears an already-registered follow-up, waits for cleanup, and leaves durable inbox rows available for later execution. A wake registered during cleanup is fresh work and runs afterward. The coordinator enters the Session's current Location only when a drain starts. A Location runner also fences every new provider turn against its captured Location so a moved Session cannot begin another turn through source-Location tools or context. An already-dispatched provider turn may still settle source-Location calls until a future move-control slice interrupts active ownership. Automatic startup discovery, durable multi-node ownership, stale-owner fencing, durable interruption, and retry policy remain future work.
