# Remove `packages/pencode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/pencode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the pencode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/pencode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/pencode/src/account/repo.ts`
- `packages/pencode/src/cli/cmd/db.ts`
- `packages/pencode/src/cli/cmd/import.ts`
- `packages/pencode/src/cli/cmd/stats.ts`
- `packages/pencode/src/control-plane/workspace.ts`
- `packages/pencode/src/index.ts`
- `packages/pencode/src/node.ts`
- `packages/pencode/src/permission/index.ts`
- `packages/pencode/src/project/project.ts`
- `packages/pencode/src/server/projectors.ts`
- `packages/pencode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/pencode/src/server/shared/fence.ts`
- `packages/pencode/src/session/message-v2.ts`
- `packages/pencode/src/session/projectors.ts`
- `packages/pencode/src/session/prompt.ts`
- `packages/pencode/src/session/session.ts`
- `packages/pencode/src/session/todo.ts`
- `packages/pencode/src/share/share-next.ts`
- `packages/pencode/src/storage/db.ts`
- `packages/pencode/src/sync/index.ts`
- `packages/pencode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy pencode database wrapper; `packages/pencode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the pencode wrapper.
...
Status: Completed. `SyncEvent` and the pencode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy pencode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy pencode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy pencode database wrapper.
...
All migration groups are complete or superseded. `packages/pencode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No pencode data-migration group remains.

The previous pencode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate pencode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/pencode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/pencode/src`
- `bun typecheck` from `packages/pencode`
- Relevant package tests from `packages/pencode`, not the repo root
