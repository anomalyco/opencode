...
Move the canonical Pencode terminal application from
`packages/pencode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/pencode`,
`packages/cli`, or `@pencode-ai/core`.

The SDK is the TUI's Pencode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/pencode`.
- Do not replace private `packages/pencode` imports with `@pencode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@pencode-ai/tui`.
...
- Pencode domain data displayed by the TUI
...
```text
packages/pencode/src/cli/cmd/tui
```

Its private dependency on `packages/pencode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/pencode/src/*`.
...
Status: Completed. The private `@pencode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@pencode-ai/tui`.
...
- The package has no dependency on `pencode`, `@pencode-ai/cli`, or
  `@pencode-ai/core`.
...
primitives now live in `@pencode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@pencode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Pencode globals or process state;
...
Replace process-global Pencode state with resolved TUI inputs.
...
`@pencode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/pencode` initially.
...
canonical in `@pencode-ai/tui`. Configured references resolve through the new
...
adapters rather than Pencode domain implementations.

Make the SDK the only Pencode domain boundary used by the TUI.
...
- Domain-facing TUI code imports Pencode data and operations only from
  `@pencode-ai/sdk`.
...
`@pencode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/pencode`.
...
- The package has no imports from `packages/pencode`, `packages/cli`, or
  `@pencode-ai/core`.
...
public `@pencode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/pencode`.
- Keep the legacy embedded worker and server startup in `packages/pencode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@pencode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/pencode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@pencode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@pencode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Pencode domain state.
...
cd packages/pencode && bun typecheck
...
rg '@pencode-ai/core|packages/pencode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/pencode/src packages/pencode/test
```
...
- Build the current-platform `packages/pencode` binary.
