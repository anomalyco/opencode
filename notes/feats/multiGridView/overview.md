...
- **Hover-to-Focus for New Chats**: Fixed a bug where hovering over a brand-new, unsaved chat slot (which has no `id`) failed to trigger hover focus. Updated the `SessionGrid` to strictly handle `id === ""` comparisons when determining if a slot is currently focused.
