...
const { client, server } = createOpenDeepSeek()
...
try {
  ...
  await assertOpenDeepSeekConnected()
...
function createOpenDeepSeek() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`opencode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createOpenDeepSeekClient({ baseUrl: url })
...
async function assertOpenDeepSeekConnected() {
  let retry = 0
  let connected = false
  do {
    try {
...
function useShareUrl() {
  return isMock() ? "https://dev.opendeepseek.ai" : "https://opencode.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.opendeepseek.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    response = await fetch("https://api.opendeepseek.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
