...
# パッケージマネージャー
npm i -g pencode-ai@latest        # bun/pnpm/yarn でもOK
scoop install pencode             # Windows
choco install pencode             # Windows
brew install anomalyco/tap/pencode # macOS と Linux（推奨。常に最新）
brew install pencode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S pencode            # Arch Linux (Stable)
paru -S pencode-bin               # Arch Linux (Latest from AUR)
mise use -g pencode               # どのOSでも
nix run nixpkgs#pencode           # または github:kiyosh11/pencode で最新 dev ブランチ
```
...
Pencode はデスクトップアプリとしても利用できます。[releases page](https://github.com/kiyosh11/pencode/releases) から直接ダウンロードするか、[pencode.ai/download](https://pencode.ai/download) を利用してください。
...
1. `$PENCODE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.pencode/bin` - デフォルトのフォールバック
...
Pencode には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://pencode.ai/docs/agents) の詳細はこちら。

### ドキュメント

Pencode の設定については [**ドキュメント**](https://pencode.ai/docs) を参照してください。

### コントリビュート

Pencode に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Pencode の上に構築する

Pencode に関連するプロジェクトで、名前に "pencode"（例: "pencode-dashboard" や "pencode-mobile"）を含める場合は、そのプロジェクトが Pencode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/pencode) | [X.com](https://x.com/pencode)
