...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@/sdk/v2"
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}athena -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.ATHENA_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "browser_patchright"}>
          <BrowserInline icon="🛡" pending="Patchright stealth..." label={(toolprops.input as any).action} {...toolprops} />
        </Match>
        <Match when={true}>
