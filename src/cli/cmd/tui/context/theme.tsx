...
import athenaTheme from "./theme/athena.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@/plugin/types/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "athena")
        draft.active = typeof active === "string" ? active : "athena"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.athena, store.mode)
    })
