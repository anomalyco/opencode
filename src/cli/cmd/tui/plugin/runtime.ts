...
function createThemeInstaller(meta: TuiConfig.PluginMeta, root: string, spec: string): TuiTheme["install"] {
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".athena"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".athena", "themes")
...
function defaultPluginMeta(state: RuntimeState): TuiConfig.PluginMeta {
  return {
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".athena", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const plugins = Flag.ATHENA_PURE ? [] : (config.plugin ?? [])
        if (Flag.ATHENA_PURE && config.plugin?.length) {
