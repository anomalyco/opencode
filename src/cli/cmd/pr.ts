...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run athena",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for athena session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found athena session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["athena", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting athena...")
        UI.println()

        const athenaArgs = sessionId ? ["-s", sessionId] : []
        const athenaProcess = Process.spawn(["athena", ...athenaArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await athenaProcess.exited
        if (code !== 0) throw new Error(`athena exited with code ${code}`)
      },
    })
  },
})
