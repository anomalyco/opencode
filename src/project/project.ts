...
export namespace Project {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@athena/Project") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const fakeVcs = Info.shape.vcs.parse(Flag.ATHENA_FAKE_VCS)
      ...
      const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
        return yield* fs.readFileString(pathSvc.join(dir, "athena")).pipe(
          Effect.map((x) => x.trim()),
      ...
      const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
        ...
        const data: DiscoveryResult = yield* Effect.gen(function* () {
          ...
          if (!id) {
            ...
            if (id) {
              yield* fs.writeFileString(pathSvc.join(worktree, ".git", "athena"), id).pipe(Effect.ignore)
            }
          }
        ...
        if (Flag.ATHENA_EXPERIMENTAL_ICON_DISCOVERY)
