...
const TUI_SCHEMA_URL = "https://athena.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from athena.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const athenaFiles = await getAthenaFiles(input)
...
async function getAthenaFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.ATHENA_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("athena", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "athena")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "athena"))
  }
  if (Flag.ATHENA_CONFIG) files.push(Flag.ATHENA_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "athena"))
