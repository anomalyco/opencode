...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/athena"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "athena")
      default:
        return "/etc/athena"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.ATHENA_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export async function installDependencies(dir: string, input?: InstallInput) {
    ...
    await BunProc.run(
      ...
      if (err instanceof Process.RunFailedError) {
        ...
        if (Flag.ATHENA_STRICT_CONFIG_DEPS) {
  ...
  export async function needsInstall(dir: string) {
    ...
    const mod = path.join(dir, "node_modules", "@athena-agent", "plugin")
    ...
    const depVersion = dependencies["@athena-agent/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      if (!online()) return false
      const stale = await PackageRegistry.isOutdated("@athena-agent/plugin", depVersion, dir)
      if (!stale) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@athena-agent/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.athena/command/", "/.athena/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.athena/agent/", "/.athena/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: athena.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for athena serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://athena.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://athena.ai/docs/agents"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@athena/Config") {}

  function globalConfigFile() {
    const candidates = ["athena.jsonc", "athena.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in athena config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://athena.ai/config.json"
              const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://athena.ai/config.json",')
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "athena.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "athena.jsonc"))),
          )
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://athena.ai/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              log.debug("fetching remote config", { url: `${url}/.well-known/athena` })
              const response = yield* Effect.promise(() => fetch(`${url}/.well-known/athena`))
              if (!response.ok) {
                throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
              }
              const wellknown = (yield* Effect.promise(() => response.json())) as any
              const remoteConfig = wellknown.config ?? {}
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://athena.ai/config.json"
              result = mergeConfigConcatArrays(
                result,
                yield* loadConfig(JSON.stringify(remoteConfig), {
                  dir: path.dirname(`${url}/.well-known/athena`),
                  source: `${url}/.well-known/athena`,
                }),
              )
          ...
          if (Flag.ATHENA_CONFIG) {
            result = mergeConfigConcatArrays(result, yield* loadFile(Flag.ATHENA_CONFIG))
            log.debug("loaded custom config", { path: Flag.ATHENA_CONFIG })
          }

          if (!Flag.ATHENA_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("athena", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.ATHENA_CONFIG_DIR) {
            log.debug("loading config from ATHENA_CONFIG_DIR", { path: Flag.ATHENA_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".athena") || dir === Flag.ATHENA_CONFIG_DIR) {
              for (const file of ["athena.jsonc", "athena.json"]) {
          ...
          if (process.env.ATHENA_CONFIG_CONTENT) {
            result = mergeConfigConcatArrays(
              result,
              yield* loadConfig(process.env.ATHENA_CONFIG_CONTENT, {
                dir: ctx.directory,
                source: "ATHENA_CONFIG_CONTENT",
              }),
            )
            log.debug("loaded custom config from ATHENA_CONFIG_CONTENT")
          }
          ...
          if (active?.active_org_id) {
            yield* Effect.gen(function* () {
              ...
              if (token) {
                process.env["ATHENA_CONSOLE_TOKEN"] = token
                Env.set("ATHENA_CONSOLE_TOKEN", token)
              }
          ...
          if (Flag.ATHENA_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ATHENA_PERMISSION))
          }
          ...
          if (Flag.ATHENA_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.ATHENA_DISABLE_PRUNE) {
