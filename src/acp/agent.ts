...
import type { AssistantMessage, Event, AthenaClient, SessionMessageResponse, ToolPart } from "@/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: AthenaClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `athena auth login` in the terminal",
        name: "Login with Athena Agent",
        id: "athena-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Athena Agent stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const athenaProvider = providers.find((p) => p.id === "athena")
    if (athenaProvider) {
      if (athenaProvider.models["big-pickle"]) {
        return { providerID: ProviderID.athena, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(athenaProvider.models))
      if (best) {
        return {
