...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for athena.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal athena sessions
...
athena acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 athena acp
```
...
- **Terminal Integration**: Full terminal support via athena's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | athena acp
```
...
ACP sessions map cleanly to athena's internal session model:
