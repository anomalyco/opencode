...
const cli = yargs(hideBin(process.argv))
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.ATHENA = "1"
    process.env.ATHENA_PID = String(process.pid)

    Log.Default.info("athena", {
      version: Installation.VERSION,
      args: process.argv.slice(2),
    })

    const marker = path.join(Global.Path.data, "athena.db")
