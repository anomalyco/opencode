...
export const DEPRECATED_PLUGIN_PACKAGES = ["athena-openai-codex-auth", "athena-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, athenaVersion: string) {
  if (!semver.valid(athenaVersion) || semver.major(athenaVersion) === 0) return
  const pkg = await readPluginPackage(target).catch(() => undefined)
  if (!pkg) return
  const engines = pkg.json.engines
  if (!isRecord(engines)) return
  const range = engines.athena
  if (typeof range !== "string") return
  if (!semver.satisfies(athenaVersion, range)) {
    throw new Error(`Plugin requires athena ${range} but running ${athenaVersion}`)
  }
}
