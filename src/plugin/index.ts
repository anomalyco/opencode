import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@/plugin/types"
import { Config } from "../config/config"
import { Bus } from "../bus"
import { Log } from "../util/log"
import { createAthenaClient } from "@/sdk"
...
export namespace Plugin {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@athena/Plugin") {}
      ...
      const cache = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createAthenaClient({
            ...
            headers: Flag.ATHENA_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.ATHENA_SERVER_USERNAME ?? "athena"}:${Flag.ATHENA_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.ATHENA_PURE ? [] : (cfg.plugin ?? [])
          if (Flag.ATHENA_PURE && cfg.plugin?.length) {
