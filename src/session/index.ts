import { Slug } from "@/util/lib/slug"
...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    if (!result.parentID && (Flag.ATHENA_AUTO_SHARE || cfg.share === "auto")) {
      share(result.id).catch(() => {
        // Silently ignore sharing errors during session creation
      })
    }

    if (!Flag.ATHENA_EXPERIMENTAL_WORKSPACES) {
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // It looks like Athena Agent's cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
