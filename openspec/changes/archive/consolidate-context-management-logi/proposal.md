# Proposal: Consolidate context management logic in `packages/tui/src/components/ContextSidebar.tsx` to enable cross-provider memory state sharing.
...
## What
Consolidate context management logic in `packages/tui/src/components/ContextSidebar.tsx` to enable cross-provider memory state sharing.
