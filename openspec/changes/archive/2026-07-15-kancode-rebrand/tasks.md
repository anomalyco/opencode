...
- [x] 1.3 Dual-read config filenames and `.kancode`/`.opencode` dirs in `packages/kancode/src/config/{paths,config,tui,tui-migrate,managed}.ts` with documented precedence
...
- [x] 3.2 Run `bun typecheck` in `packages/kancode`, `packages/tui`, `packages/core`
