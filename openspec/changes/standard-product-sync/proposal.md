...
The live MySQL product data no longer matches the business-approved workbook `商品信息(1)_结构化清洗.xlsx`: authoritative products are missing or duplicated, shelf relations and supplier/origin values drift, and the Feishu inventory reader currently falls back to a purchase supplier that is not the approved product supplier. The workbook is now the authoritative master-data answer, so the database must be reconciled before the robot can return reliable results.
...
- Records source workbook identity, run identity, row counts, field-level before/after evidence, transaction outcome, validation, and rollback outcome without logging credentials or connection strings.
